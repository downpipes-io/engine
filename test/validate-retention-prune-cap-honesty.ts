...
//
//  1. THERE IS NO SECOND CALL, and this file MEASURES that rather than arguing it. POST
...
//     On the apply side the batch cap and precheckCandidateCoverage (R-62 fix 2, deliberate) are MUTUALLY
...
//   - REPRODUCTION CONTROL: the SAML_CERTS_MAX edge measured below, re-measured on THIS rig in THIS session, so a
//     reader can tell this result from this instrument.
...
// An earlier pass measured the SAML signing-certificate cap at exactly 8/9 through validateSamlCerts. Re-measure
// it here, on this rig, in this session, so a reader can separate this pass's result from this pass's
// instrument. If this disagrees with the banked figure, nothing below is trustworthy.
...
// Quoted verbatim from engine ba57dab4 (`git show ba57dab4:src/admin/router-retention-prune.ts`, lines
// 284 and 362). They are carried here rather than measured in a throwaway worktree because the point is
// not that they existed, it is that the SAME honesty questions the new sentence passes, these fail. Four
// of the six checks above fail against them, and the two that pass are the two that were never wrong:
// the count and the bound were always named correctly. The product always refused at the right place and
// only lied about what to do next -- the same shape as all three defects before this one.
console.log("\nTHE PRE-FIX SENTENCES, PUT THROUGH THE SAME QUESTIONS (engine ba57dab4):");
