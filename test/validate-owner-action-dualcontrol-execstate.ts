// Two execute-time single-use / live-authority guards on the OWNER-ACTION APPROVE path itself, complementing
// the coverage the existing groups give the consume path and the config-change path:
//
//   A SECOND approve on an already-executed owner action is REFUSED (single use). The owner-action
//            approve path runs canApproveOwnerAction at the top of approveOwnerAction; once the record's
//            effective status is "executed" the verdict is "the action was already carried out" (400), so the
//            op cannot double-execute. (The asymmetric blockConcurrencyWhile RE-ASSERT inside the critical
//            section is the CONCURRENT-double-approval guard; under the offline validator the DO's
//            blockConcurrencyWhile falls back to running fn() directly, so the closest faithful offline proof
//            is the SERIAL second-approve-on-executed refusal driven here.)
//
//   A proposer DEMOTED from owner between propose and approve is caught at EXECUTE. approveOwnerAction
//            runs the DO-executed method AS THE PROPOSER (proposerReplayCaller, re-resolved live by subject);
//            putDest re-checks roleForCaller(...).role === "owner", so a proposer who is no longer an owner
//            throws AuthError -> 403 and the op does NOT execute, even though the (distinct, still-owner)
//            approver's own approve verdict is valid. This is the apply-time authority ceiling on the
//            owner-action path (the sibling config-change path proves the same principle).
//
...
//            gone, wrong flow, hash mismatch, not-yet-armed/already-used/rejected) is refused cleanly (the
//            correct 400) WITHOUT resetting the DO (guardThrows()===0): a bare throw inside
//            blockConcurrencyWhile would reset the DO for an ordinary, benign refusal, so each of these
//            paths carries the same sentinel treatment as the demoted-proposer guard above.
//
...
export async function run(ctx: Ctx): Promise<void> {
  const { OWNER, OWNER2, call, setGate, doFetch, ownerCaller, destConfig, listDestinations, inbox, ageOwnerAction } = ctx;
  ...
  // controlled (queued) action. The demoted-proposer case below needs the proposer demotion to land
  // INLINE, so turn the toggle OFF here. The high-blast dest-put auto-gate is toggle-INDEPENDENT once a
  // second owner exists, so the dest-put proposals below are still auto-gated regardless of this toggle.
  await setGate(OWNER, false);

  // ===========================================================================================
  // A SECOND approve on an already-executed owner action is refused (single use).
  // ===========================================================================================
  {
    ...
    ok("a high-blast dest-put is auto-gated (202, pending)", propose.status === 202);
    const pid = ((await propose.json()) as { id?: string }).id ?? "";
    ok("the 202 carries an owner-action id", pid.length > 0);
    ...
    ok("the first distinct-owner approve executes the action (200, executed)", firstApprove.status === 200 && firstRec.status === "executed");
    ok("the destination was added exactly once by the first approve", (await labelCount(LABEL)) === 1);
    ...
    ok("a SECOND approve on the already-executed action is refused (400)", secondApprove.status === 400);
    ok("the refusal reason is 'the action was already carried out' (single use)", /already carried out/.test(secondBody.error ?? ""));
    ok("the second approve did NOT double-execute (still exactly one destination)", (await labelCount(LABEL)) === 1);
  }

  // ===========================================================================================
  // A proposer demoted from owner between propose and approve -> execute re-resolves live and refuses.
  // ===========================================================================================
  {
    ...
    ok("precondition - OWNER2 is an owner before proposing", (await roleOf(OWNER2)) === "owner");
    ...
    ok("the owner proposer's high-blast dest-put is auto-gated (202, pending)", propose.status === 202);
    const pid = ((await propose.json()) as { id?: string }).id ?? "";
    ok("the 202 carries an owner-action id", pid.length > 0);
    ...
    ok("the proposer is demoted from owner to viewer between propose and approve (200)", demote.status === 200);
    ok("the demotion took effect (proposer is now a viewer)", (await roleOf(OWNER2)) === "viewer");

    // Capture the pending state BEFORE the approve so we can prove it SURVIVES the refused approve.
    ok("the proposed action is pending before the approve", (await inbox(OWNER)).some((r) => r.id === pid && r.status === "pending"));
    ...
    ok("approving a demoted proposer's action is refused at execute (403)", approve.status === 403);
    ok("the refusal is the generic forbidden body (the proposer's owner ceiling re-checked live)", approveBody.error === "forbidden");
    ok("the action did NOT execute (the destination was never added)", (await labelCount(LABEL)) === 0);
    // The FORESEEABLE proposer AuthError is caught INSIDE blockConcurrencyWhile and surfaced OUTSIDE it, so
    // nothing throws OUT of the guarded fn. In the REAL workerd runtime a throw there RESETS the Durable
    // Object (wiping its in-memory scheduler state) and returns 500 instead of the clean 403 an AuthError
    // maps to; guardThrows()===0 proves the guard does NOT throw, i.e. a clean 403 with the DO intact.
    ok("the proposer AuthError did NOT throw out of blockConcurrencyWhile (no DO reset -> 403, not 500)", ctx.sched.guardThrows() === 0);
    // The DO's in-memory state SURVIVES: the pending owner action is still pending (never wiped, never flipped).
    ok("the pending owner action SURVIVES the refused approve (still pending)", (await inbox(OWNER)).some((r) => r.id === pid && r.status === "pending"));
    ...
    ok("cleanup - OWNER2 restored to owner", (await roleOf(OWNER2)) === "owner");
    ...
    ok("re-approving the survived action after restore executes it (200, executed)", reApprove.status === 200 && reRec.status === "executed");
    ok("the destination is added exactly once by the re-approve", (await labelCount(LABEL)) === 1);
  }

  // ===========================================================================================
  // The proposer is OFFBOARDED and their ADDRESS is then RE-ISSUED. The demoted-proposer case above proves
  // the live re-resolution catches a DEMOTED proposer, which is the axis the replay was built for. This is
  // the axis it was blind to.
  //
  ...
  // re-used address hands the DEPARTED subject a live owner grant at execute time, and the queued action runs.
  // The comment on proposerReplayCaller cites the ASVS V10.3.3 / V10.5.2 closure for this, and it is true of
  ...
  //
  // The pair below distinguishes a safe build from a vulnerable one by more than a status code: a vulnerable
  // build EXECUTES the re-issue, and it also silently consumes the invitation the account issued to somebody
  // else, which no route anywhere reports.
  {
    const LABEL = "auth46b-reissued-address";
    const pendingKey = `role:pending:${OWNER2}`;
    ok("precondition - OWNER2 is an owner and proposes", (await roleOf(OWNER2)) === "owner");
    const propose = await doFetch("/destinations", ownerCaller(OWNER2), { label: LABEL, config: destConfig("auth46b-bucket") });
    ok("the proposal is auto-gated (202, pending)", propose.status === 202);
    const pid = ((await propose.json()) as { id?: string }).id ?? "";
    ...
    ok("the proposer is OFFBOARDED between propose and approve (200)", off.status === 200);
    ok("the offboard took effect (no role entry at all)", (await roleOf(OWNER2)) === "none");
    ageOwnerAction(pid);
    const offApprove = await doFetch("/owner-actions/approve", ownerCaller(OWNER), { id: pid });
    ok("approving an OFFBOARDED proposer's action is refused at execute (403)", offApprove.status === 403);
    ok("the action did NOT execute", (await labelCount(LABEL)) === 0);
    ...
    ok("the address is re-issued to the next person (200)", reissue.status === 200);
    ok("the re-issued grant is PENDING, claimed by no subject yet", ctx.sched.storage.rawGet(pendingKey) !== undefined);

    ctx.sched.resetGuardThrows();
    const reissuedApprove = await doFetch("/owner-actions/approve", ownerCaller(OWNER), { id: pid });
    ok("THE CLAIM UNDER TEST - re-issuing the departed proposer's ADDRESS does not re-arm their action (403)", reissuedApprove.status === 403);
    ok("THE ASSERTION THAT MATTERS - the action still did NOT execute", (await labelCount(LABEL)) === 0);
    ok("the NEW person's grant is untouched - the replay did not consume an invitation that is not its own", ctx.sched.storage.rawGet(pendingKey) !== undefined);
    ok("and the refusal is still the clean one, with the DO intact", ctx.sched.guardThrows() === 0);

    // NO STRANDING, and the arm that stops the guard becoming "refuse whenever the proposer has no bound row".
    // The address's real holder signs in, their OWN subject claims the grant through the same bind-on-first-
    // auth every live request performs, and the queued action becomes approvable again. In production that
    // holder is the new person and the departed proposer stays refused for ever; the harness's one identity
    // per address exercises the recovery half.
    await call(OWNER2, "GET", "/admin/whoami");
    ok("a real sign-in by the address's holder claims the pending grant, exactly as it always did", ctx.sched.storage.rawGet(pendingKey) === undefined);
    ...
    ok("NO STRANDING - with the recorded SUBJECT authorised again the same action executes (200)", reclaimApprove.status === 200 && reclaimRec.status === "executed");
    ok("and it executed exactly once", (await labelCount(LABEL)) === 1);
    ok("cleanup - OWNER2 is an owner again for the proofs below", (await roleOf(OWNER2)) === "owner");
  }
  ...
  // single-use case proves at the OUTER check, serially). The offline validator's blockConcurrencyWhile has no
  // real concurrency to race (see the single-use comment above), so a single controlled stale read on the SAME storage
  ...
  // ===========================================================================================
  {
    ...
    ok("[race] the refusal is the same single-use verdict proved at the outer check above (already carried out)", /already carried out/.test(approveBody.error ?? ""));
    // Without this guard, the inner canApproveOwnerAction recheck's failure would throw a bare Error INSIDE
    // blockConcurrencyWhile, resetting the DO in real workerd; guardThrows()===0 proves it is sentinelled out
    // cleanly instead, mirroring the demoted-proposer guardThrows()===0 proof above.
  ...
  // ===========================================================================================
  // CONSUME RE-SUBMIT REFUSALS: consumeOwnerAction's guarded body catches every refusal (record gone, wrong
  // flow, hash mismatch, not-yet-armed/already-used/rejected) so none of them is a bare throw INSIDE
  // blockConcurrencyWhile, which would otherwise reset the DO exactly like the CONCURRENT-APPROVAL RACE above.
  ...
  // ===========================================================================================
  {
    ...
    // above) -- they gate only when the opt-in toggle is ON, which the setup above turned OFF.
    ...
    // (e.g. a page-refresh double-fire).
    ...
    // Without a try/catch around this branch it would throw a bare Error INSIDE blockConcurrencyWhile;
    // guardThrows()===0 proves it is sentinelled out cleanly instead.
