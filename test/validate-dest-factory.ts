...
async function testHttpsEnforcement(): Promise<void> {
  ...
  // onto a cleartext request. requireHttpsEndpoint's allowance is now a seam this builder does not pass, so
  // the loopback hosts are refused here exactly like any other http endpoint.
  for (const endpoint of ["http://localhost:9000", "http://127.0.0.1:9000", "http://[::1]:9000"]) {
    const msg = await rejects(() => buildDestination(env({ DEST_KIND: "s3", ...S3_OK, DEST_ENDPOINT: endpoint })));
    ok(`S3 path refuses ${endpoint} in a deployed configuration (no test seam is reachable from here)`, msg !== null);
  }
  // The refusal discriminates: an https endpoint still builds, so the loop above is not passing because every
  // buildDestination call now throws.
  ok("CONTROL: an https endpoint still builds an S3Destination",
    (await buildDestination(env({ DEST_KIND: "s3", ...S3_OK, DEST_ENDPOINT: "https://s3.example.com" }))) instanceof S3Destination);
}
