...
//
// The fault classes below share one property that makes them the nastiest kind of support ticket: the run
...
//
//   - a D1 crawl that silently loses its point-in-time guarantee (no session, or a resume that re-anchors
//     to a DIFFERENT snapshot) seals a MIXED-SNAPSHOT archive: the restore shows rows referencing rows that
//     do not exist, or duplicated rows, behind a clean run.
//   - a corrupt resume token wedges a downpipe (every slice fails) but matches no coarse error class, so it
//     reads as a generic "run failed" with no route to its known remedy (clear the resume token).
//   - retry pressure is invisible unless recorded: withRetry only surfaces the TERMINAL outcome, so a
//     chronically throttled account looks healthy right up to the day the retries exhaust.
//   - a security refusal (a byte-fetch redirect, an off-Cloudflare download host) is otherwise an anonymous
//     "_refused" marker count sealed inside the archive.
//
...
// silent mixture of two". All three ride on runs that report ok, which is the entire bug.
async function g096SnapshotConsistency(): Promise<void> {
  ...
  const pinned = drain("g096-pinned");
  ...
  const unpinned = drain("g096-unpinned");
  ...
  const reanchored = drain("g096-reanchored");
  ...
  const fallback = drain("g096-rowid-fallback");
...
async function g213ResumeTokenDefects(): Promise<void> {
  console.log("resume-token corruption carries a closed defect class");

  // The D1 parser REJECTS rather than coerces these two fields to null: a coercion would be a
  // data-integrity bug (bad-bookmark -> mixed snapshot; bad-cursor -> duplicated rows).
  ...
  for (const c of d1Cases) {
    ...
    const l = drain(`g213-d1-${c.defect}`);
  ...
  // Every OTHER resumable adapter's token parser must classify too: an unclassified bare JSON.parse
  // SyntaxError would otherwise land in the generic "run failed" bucket.
  ...
  for (const a of adapters) {
    ...
    const l = drain(`g213-${a.type}-unparseable`);
...
// The 429s a retry SWALLOWS are the whole point: they are the early warning that is otherwise invisible.
async function g170ThrottlePressure(): Promise<void> {
  ...
  const coping = drain("g170-coping");
...
async function g327SecurityRefusals(): Promise<void> {
  ...
  const redirect = drain("g327-redirect");
...
async function main(): Promise<void> {
  await g096SnapshotConsistency();
  await g213ResumeTokenDefects();
  await g170ThrottlePressure();
  await g327SecurityRefusals();
  redactionSweep();
