...
function testTheCallSiteChecks(): void {
  ...
  // THE EXACT INVARIANT, and it replaced two weaker attempts worth recording because both are the failure
  // this file is about. The first was `x === false || true`, a tautology asserting nothing. The second was a
  // window-based sweep for a compensating catch near each call, which PASSED when a site was reverted by
  // hand, because the window did not reach that site: a check that cannot see the thing it checks.
  //
  // What is checkable without heuristics: NO recordAudit call outside the helper may carry a key-ceremony
  // target. The helper is then the single place that decides whether the append landed.
