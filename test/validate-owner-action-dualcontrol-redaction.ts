...
export async function run(ctx: Ctx): Promise<void> {
  const { sched, OWNER, OWNER2, OPERATOR, subjectOf, call, readLog, setGate, doFetch, ownerCaller, ownerActionKeyCount, destConfig, oidcProposal, listDestinations, inbox, applyOwnerOp, ageOwnerAction } = ctx;
  ...
  // ===========================================================================================
  {
    ...
    // FOLD 2: the inbox LISTING must also omit the secret from params (the redaction docstrings + the IdP
    ...
    // secretAccessKey — ML-04, the sibling field the original redaction missed.
    {
      ...
      // ML-04-r2 sibling: reject needs no maker != checker check (any Owner may veto a pending proposal), so
      // its RESPONSE BODY is the easiest second read path into the same live externalId the listing redacts.
      // Before the fix this echoed the raw at-rest record straight through this.json() -- assert it does not.
    ...
    // ML-04-r2 sibling: the APPROVE response is the SAME kind of echo as reject, but via the OTHER dual-control
