...
// The reported ladder: a config edit reset lastRunId, the next scheduled test
// deferred "no completed run yet" on a downpipe with a FULL archive, the deferral was recorded
// ok:false, the console read it as "Last test failed", and the verdict then sat for a whole cadence
// because nothing retested after the next successful backup. These vectors pin each rung.
