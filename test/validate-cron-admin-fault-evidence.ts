// Support-pack diagnostic coverage for the CRON and ADMIN subsystems. These fault sites sit outside the
// main evidence-recording surface, so each one still swallows its evidence.
//
...
//   A downpipe that silently stops producing runs -> the pre-run dispatch throw is ATTRIBUTED to
//        the downpipe with a closed class (its pinned destination record was deleted / the DO would not
//        allocate a run / the lock-clear failed and wedged the lease), instead of one anonymous sealErrors int.
//   A SIEM S3-drop that retries forever with a fixed s3-put-failed string -> the string is SPLIT into the
//        same closed class set the destination probes use, and the OTLP truncation is SIZED (droppedCount).
//   Prometheus 401ing with an unexpired credential -> the /metrics scrape surface records its own
//        health: the closed outcome (including auth-check-unavailable, the DO hiccup misreported as a 401), the
//        silent shape fallback that empties every series while up == 1, and the failing pull trail.
//   Ed25519-only keys that can never enrol, and a passkey rejected on every login -> the 14 structural
//        CBOR/COSE/DER/authData defects are split by class AND ceremony phase.
//   Every recovery code failing -> a corrupt STORED record and an unusable
//        signing key are recorded, instead of being indistinguishable from the operator mistyping.
//   The pack under-reporting failures during an outage -> the recorders' OWN dropped writes are
//        counted, so the pack stops under-counting during the exact outage it is meant to explain.
//
...
async function testWebauthnFaults(): Promise<void> {
  ...
  // a text string (major 3, len 2) carrying invalid UTF-8: without this classification the decoder throws a
  // raw TypeError that escapes the PasskeyError contract entirely.
  ok("Invalid UTF-8 in a CBOR text string is cbor-utf8 (never a raw TypeError escaping the decoder)", structuralClassOf(() => new CborReader(new Uint8Array([0x62, 0xff, 0xfe])).decodeTop()) === "cbor-utf8");
