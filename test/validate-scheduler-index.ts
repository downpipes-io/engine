// Due-time index vectors: due() in O(due) not O(N), the persisted index, and the reconciliation
// backstop. Covers index create/advance/delete on add/complete/remove/disable, brute-force parity,
// drift reconciliation, build-from-empty, the small-env transparency case and the O(due) cost at
// scale. Behaviour-preserving: same assertions, same order.
import {
  ok,
  MockStorage,
  makeScheduler,
...
export async function run(): Promise<void> {
  ...
  // ---- the index is CREATED on add, ADVANCED on completeRun, deleted on remove/disable ----------
  {
  ...
  // ---- PARITY: due() (index path) === brute-force listDownpipes().filter(...) ---------------------
  ...
  // ---- RECONCILIATION: inject drift, due() misses it, rebuildDueIndex() heals it -------------------
  {
    const { stub, storage } = makeScheduler();
  ...
  // ---- BUILD-FROM-EMPTY: several dp: states, no due: keys -> reconcile builds correctly -------------
  ...
  // ---- SMALL ENV: a 3-downpipe account schedules EXACTLY as before ----------------------------------
  ...
  // ---- O(due): at large N, due() reads only the due CANDIDATES, not the whole fleet -----------------
  ...
  // { start, end } (so the range scan is real) and records per-call key counts (listKeysScanned).
  {
    ...
    for (let i = 0; i < N; i++) {
      ...
      // O(N) not O(N^2); the fleet is fresh so there is no stale entry to clear.
