// Prove the engine governance backbone, parts D1 (whoami) and D3 (server-side RBAC + the
// last-Owner guard), end to end and SERVER-SIDE, with in-memory doubles only. No network, no
// deploy, no cost. Run:
...
//    bypassing the SPA does not bypass the control (the F1 hard rule);
//  - the last-Owner guard refuses demotion or removal of the only Owner (the F3 rule), in the
...
//
// This file is a THIN ORCHESTRATOR (finding engine-test-012-M1): the proofs now live in sibling
...
async function main(): Promise<void> {
  ...
  // Restore the stubbed global fetch in a finally (finding engine-test-012-11): if any group throws
  ...
  verdictReached(counter.failures);
