...
import { putIdpSecret, resolveIdpSecret } from "../src/admin/oidc-store.ts";
import type { KvStorage } from "../src/admin/oidc-store-kv.ts";
...
// The discovery token sits behind CONFIG_WRAP_KEY like the other secrets, so a Durable Object storage
// read never yields a live estate-wide credential. These cases pin its own AAD domain and the read path
// that opens it.
...
console.log("config-secret: the IdP client secret at rest");
// The IdP client secret sits under CONFIG_WRAP_KEY like the other secret classes, sealed with its own AAD
// domain so it cannot be swapped for a destination credential or a discovery token. These cases pin that
// AAD and the read path the token exchange uses.
//
// The storage double is the same in-memory KvStorage shape oidc-store.ts is written over, so the path under
// test is the production path rather than a re-implementation of it.
const idpRows = new Map<string, unknown>();
const idpKv: KvStorage = {
  get: async <T>(k: string): Promise<T | undefined> => idpRows.get(k) as T | undefined,
  put: async (k: string, v: unknown): Promise<void> => {
    idpRows.set(k, v);
  },
  delete: async (k: string): Promise<boolean> => idpRows.delete(k),
  list: async <T>(opts?: { prefix?: string }): Promise<Map<string, T>> => {
    const out = new Map<string, T>();
    for (const [k, v] of idpRows) if (opts?.prefix === undefined || k.startsWith(opts.prefix)) out.set(k, v as T);
    return out;
  },
};
const IDP_SECRET = "idp-client-secret-value-9876543210";
const idpEnv = await wrapConfigSecret(KEY, IDP_SECRET, IDP_SECRET_AAD);
ok("an IdP client secret round-trips under its own AAD", (await unwrapConfigSecret(KEY, idpEnv, IDP_SECRET_AAD)) === IDP_SECRET);
await throwsAsync("an IdP envelope does NOT open as a destination credential", async () => unwrapConfigSecret(KEY, idpEnv));
await throwsAsync("an IdP envelope does NOT open as a discovery token", async () => unwrapConfigSecret(KEY, idpEnv, DISCOVERY_SECRET_AAD));
await throwsAsync("a destination envelope does NOT open as an IdP client secret", async () => unwrapConfigSecret(KEY, destEnv, IDP_SECRET_AAD));

// The exchange's actual read, through the store.
await putIdpSecret(idpKv, "entra", idpEnv);
ok("the stored row is the envelope, not the value (a DO storage read discloses nothing)", idpRows.get("idpsecret:entra") !== IDP_SECRET && isWrappedSecret(idpRows.get("idpsecret:entra")));
ok("resolveIdpSecret opens the sealed secret with the key", (await resolveIdpSecret(idpKv, "entra", KEY)) === IDP_SECRET);
await throwsAsync("a sealed IdP secret with NO wrap key fails loud, never exchanged as a credential", async () => resolveIdpSecret(idpKv, "entra", undefined));
const OTHER_KEY = b64urlDecode(b64urlEncode(new Uint8Array(32).fill(9)));
await throwsAsync("a sealed IdP secret does not open under the WRONG wrap key", async () => resolveIdpSecret(idpKv, "entra", OTHER_KEY));

// The back-compat floor: a record written before the wrap shipped (or on an engine with no wrap key) is a
// bare string and must keep working, because a lazy migration that stranded every existing connection would
// lock an estate out of its own single sign-on.
await putIdpSecret(idpKv, "legacy", "bare-legacy-secret");
ok("a legacy plaintext IdP secret resolves unchanged, with or without a key", (await resolveIdpSecret(idpKv, "legacy", undefined)) === "bare-legacy-secret" && (await resolveIdpSecret(idpKv, "legacy", KEY)) === "bare-legacy-secret");
ok("an unknown connection resolves to undefined (the exchange then fails closed)", (await resolveIdpSecret(idpKv, "nope", KEY)) === undefined);
