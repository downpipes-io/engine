// H2: the integrity-vs-availability classifier on the restore / 3-2-1 fallback path must be ROBUST, not a
...
//
// The bug this validator guards (red-before-green): several genuine integrity/completeness failures the
...
async function main(): Promise<void> {
  ...
  console.log("-- 5. end to end through runRestore: truncated -> non-fallback integrity; missing -> fallback --");
  {
    ...
    // INT-3 end to end: delete a `seg/` data object from an otherwise-intact, signed run. restoreRecord
  ...
  console.log("-- 6. INT-3: a deleted SEGMENT is a completeness shortfall (integrity, non-fallback) --");
  {
    ...
    // the FIRST failing record and so cannot compute the reader's dangling-only predicate.
    //
  ...
  console.log("-- 7. a PRESENT-but-corrupt SEGMENT is an INTEGRITY fault, distinct from a genuine fetch fault --");
  {
    ...
    // Because the bytes WERE fetched, the reader now re-labels that decrypt/AEAD failure as a STRUCTURED
    ...
    // (INT-3). RED before the fix: the raw AES-GCM OperationError was a plain Error matching no integrity
    // keyword, so classifyRestoreFailure defaulted it to the availability catch-all "recovery check failed" --
