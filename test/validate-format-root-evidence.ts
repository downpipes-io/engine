// VALIDATOR: the FORMAT + ROOT fault evidence the support pack needs to diagnose four ticket classes.
//
// The four cases, and why each is a real ticket:
//
//   DATA LOSS / SECURITY. The engine detects a possible ROLLBACK ATTACK -- a rewritten, forked or
//        duplicated RUNLOG chain, a replay of an older validly-signed document, a corrupt line -- and records
//        NOTHING about it. The only thing it produces is a reason string that interpolates the customer's run
//        and downpipe ids, so it can never be carried. Support therefore cannot separate a TAMPER from a
//        destination-side bit-flip, which are opposite tickets. The forensic row (closed kind, the two
//        DISAGREEING indices, the unparseable line ordinal, the chain length, a one-way digest of the RUNLOG
//        bytes the customer can reproduce from their own object) is recorded at the detection site.
//
//   WRITER REFUSALS. The writer's three loud refusals collapse into a generic "run failed". source-enumerated-zero is a
//        CUSTOMER fault the pack could not name (an emptied KV namespace, a de-scoped token); the two
//        stream-wiring invariants are ENGINE faults. Telling them apart IS the diagnosis.
//
//   DEGRADED COVERAGE. A very large run attested against a store with no list() silently SKIPS the per-shard presence pass,
//        so complete:true is an OVERCLAIM (a durably missing shard outside the strided sample reads as
//        complete) and nothing said so. The coverage mode is a closed enum on the attestation AND a row in
//        the run's integrity evidence -- the one piece of evidence that rides an otherwise CLEAN verdict.
//
//   MALFORMED runId. "Our integration script keeps getting invalid runId 400s": nothing is persisted server-side, so
//        support could not say whether the ids arrive truncated, case-mangled or out of range (three different
//        customer-side fixes). The closed KIND is counted at the four rejecting routes.
//
// It proves the three things this validator requires:
...
const SENTINEL_SECRET = "example-secret-51H8xQzAcmeSuperSecretTokenValue";
...
// ---- the RUNLOG fixtures (the anti-rollback forensics) ------------------------------------------------------
...
// ---- the archive fixture (writer refusals / degraded attestation coverage) ------------------------------------
...
async function main(): Promise<void> {
  ...
  // -------------------------------------------------------------------------------------------------------
  await check("vocabulary: RUNLOG_ANOMALY_KINDS is pinned to the DO's re-declared set", () => {
    assert.deepEqual([...RUNLOG_ANOMALY_KINDS], [...RUNLOG_ANOMALY_KIND_NAMES]);
  });
  await check("vocabulary: WRITER_REFUSAL_KINDS is pinned to the DO's re-declared set", () => {
    assert.deepEqual([...WRITER_REFUSAL_KINDS], [...WRITER_REFUSAL_KIND_NAMES]);
  });
  await check("vocabulary: KEYLESS_COVERAGE_MODES is pinned to the DO's re-declared set", () => {
    assert.deepEqual([...KEYLESS_COVERAGE_MODES], [...KEYLESS_COVERAGE_NAMES]);
  });
  await check("vocabulary: every runId malformation kind has an admin counter name", () => {
  ...
  await check("a DUPLICATE index records the kind, the disagreeing index pair, the chain length and the RUNLOG digest", async () => {
    ...
    assertRedacted("runlogAnomalies (duplicate-index)", snap);
  });
  ...
  await check("a FORKED prevRunId records forked-prev with both entries' indices", async () => {
    ...
    assertRedacted("runlogAnomalies (forked-prev)", snap);
  });
  ...
  await check("a DANGLING prevRunId records dangling-prev with the offending entry's index", async () => {
    ...
    assertRedacted("runlogAnomalies (dangling-prev)", snap);
  });
  ...
  await check("a whole-document REPLAY under the min-index pin records index-regression with observed-vs-pinned", async () => {
    ...
    assertRedacted("runlogAnomalies (index-regression)", snap);
  });
  ...
  await check("a RUNLOG signature that will not verify records sig-invalid with the document digest", async () => {
    ...
    assertRedacted("runlogAnomalies (sig-invalid)", snap);
  });
  ...
  await check("a CORRUPT RUNLOG line records parse-field with the line ordinal, the line count and the digest", async () => {
    ...
    assertRedacted("runlogAnomalies (parse-field)", snap);
  });
  ...
  await check("parseRunlog notes the ordinal at the SHAPE gate itself (a non-canonical index)", async () => {
    ...
    assertRedacted("runlogAnomalies (canonical-number gate)", snap);
  });
  ...
  await check("a source that enumerated ZERO records is recorded as source-enumerated-zero, not a generic run failure", async () => {
    ...
    assertRedacted("writerRefusals (zero records)", snap);
  });
  ...
  await check("a streamed record with NO destination is recorded as stream-no-destination (the record name never rides)", async () => {
    ...
    assertRedacted("writerRefusals (stream-no-destination)", snap);
  });
  ...
  await check("a SECRET handed in as a stream is recorded as secrets-streamed", async () => {
    ...
    assertRedacted("writerRefusals (secrets-streamed)", snap);
  });
  ...
  await check("a sampled attestation against a store with NO list() records sampled-no-presence behind complete:true", async () => {
    ...
    assertRedacted("attestCoverage (no-presence)", snap);
  });
  ...
  await check("POST /restore with a malformed runId 400s AND counts the closed kind, carrying no byte of the candidate", async () => {
    ...
    assertRedacted("the /diag/admin-counters body", body);
    ...
    assertRedacted("adminCounters record", rec);
