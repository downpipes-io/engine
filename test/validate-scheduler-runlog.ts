// SchedulerDO runlog / alarm / removal vectors (engine-test-018-01 split, second half of the
// lifecycle suite). Covers the no-op completion, throughput round-trip, disabled-excluded due(),
// unknown-id 400, idempotent/stale/superseded completions, the rearmAlarm clamp + soonest tracking,
// removeDownpipe, nextWithJitter bounds and the in-flight exclusion reason. Behaviour-preserving:
// every assertion below ran in the original main() in this exact order, after TC-09.
...
export async function run(): Promise<void> {
  ...
  // ---- TC-14a: a DUPLICATE success completion is a no-op (idempotency, ENG-M13) -------
  ...
  // ---- TC-14c: a stale completion does not clear a NEWER in-flight run (ENG-M13) -------
  ...
  // ---- TC-15: rearmAlarm never schedules an alarm in the past (XC-M8) -----------------
  ...
  // ---- TC-15c: rearmAlarm tracks the SOONEST enabled pipe (ENG-M11) ---------------------
