// Proves the contended single-RUNLOG path no longer SHEDS runs under a concurrent trigger storm.
// In-memory doubles only, no network/deploy/cost. Run:
//   node test/validate-runlog-contention.ts
//
// A ~30-way concurrent finaliser storm can exhaust the account-global RUNLOG's 6-attempt CAS and strike
// ~1/3 of the runs to terminal failure ("runlog write contended"), orphaning committed run-trees. Bounded
// cross-invocation QUEUEING turns that loud shed into a wait instead:
...
// ---- Per-destination independence + same-destination serialisation -----------------
...
// ---- The per-slot lease bounds a crashed holder, per destination -------------------
...
// ---- The patient acquire waits across a briefly-held lock and still commits --------
...
// ---- Deterministic CAS-fail raises the TYPED RunlogContendedError (the park signal) -
...
// ---- A LARGE pre-seeded RUNLOG appends cleanly with ONE acquire and ONE write ------
