...
// the tree builders, the faithful XML serialiser, the enveloped XML-DSig signer (with the InclusiveNamespaces
// PrefixList option), the self-signed cert builder with a controllable validity window, and the fixed
...
export interface AOpts {
  ...
  // declare xsi:/xs: on the ASSERTION root (an ancestor of the value), exercising the inherited-namespace +
  // PrefixList path.
...
// signResponse is signElement applied to a samlp:Response (the envelope signature). The Response's
...
// X.509 cert with a CONTROLLABLE validity window live in saml-der.ts (imported at the top), shared with
