...
console.log("-- SRC-4 pagination: a multi-page list surface accumulates EVERY page, not just the first --");
{
  // A zone with 3 pages of DNS records (page-based result_info). The crawl must back up ALL of them,
  // not silently truncate at page 1 (the SRC-4 bug). per_page here is whatever the surface requests;
...
console.log("-- SRC-4 cursor pagination: a cursor-style list follows result_info.cursor to exhaustion --");
{
...
console.log("-- SRC-4 max-pages guard: an endless list TRUNCATES with an honest marker, never loops or silently drops --");
{
...
console.log("-- RESUMABLE: a stale/unknown resume watermark FAILS LOUD, never an empty crawl (engine-src-048-03) --");
{
  const surf = (id: string): CfConfigSurface => ({ id, scope: "account", restoreTier: "idempotent", read: async () => ({ id }) });
  const surfaces = [surf("s1"), surf("s2"), surf("s3")];
  const src = new CloudflareConfigSource("tok", "acct", undefined, surfaces, stubFetch([]));
  // A resume token naming a surface id that is no longer in the registry (removed/renamed surface). Before
  // the fix the skip-through loop never cleared and the crawl returned ZERO records with no signal.
