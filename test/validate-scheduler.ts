...
//
// engine-test-018-01: this file was one ~1700-line module with a ~1294-line main(). It is now a thin
// ORCHESTRATOR: each cohesive group of vectors lives in a sibling validate-scheduler-<area>.ts module
// that exports a run(), and the shared in-memory doubles, the ok() assertion sink and the config
// builders live in validate-scheduler-shared.ts. main() imports and CALLS each group in the same order
...
//  - completeRun is idempotent (ENG-M13): a duplicate or stale completion is a no-op and never
//    clobbers a row another run already resolved, nor clears the in-flight flag of a NEWER run;
//  - rearmAlarm never schedules an alarm in the past (XC-M8): a wedged enabled downpipe with a
