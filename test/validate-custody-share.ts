...
//
//   1. Body-only. buildShareEmail returns EXACTLY { subject, text, html } -- no attachment surface.
//   2. Static subject. The subject is a fixed string that never carries the share, the label, or the counts,
//      so no operator-controlled or secret value can land in a header (the header-injection surface stays closed).
//   3. HTML escaping. The one operator-controlled string in the HTML body (the custodian label) is escaped,
//      so a crafted label cannot inject markup into the email.
//   4. The share is actually delivered in the body (text AND html), with the custodian save-then-delete
//      instructions and the "reveals nothing on its own" framing (the honest-copy requirement).
...
function main(): void {
  ...
  // --- 1. Body-only: exactly subject/text/html, no attachment or any extra field. ---
  ...
  // --- 2. Static subject: fixed, and free of the share, the label, and the counts. ---
  ...
  // --- 3. HTML escaping of the operator-controlled label. ---
  ...
  // --- 4. The share is delivered, with the custodian instructions and honest framing. ---
  ...
  // customer chose, and must never be signed with a vendor identity such as "Tim O'Connor / downpipes
  // (Maelstrom AI Pty Ltd)". email-theme.ts states the rule for this whole class of mail in its own
  // header, and it binds hardest
  ...
  ok(`no vendor sign-off in either part (engine mail carries no vendor identity)`, !/Maelstrom|O'Connor/.test(mail.text) && !/Maelstrom|O&#39;Connor|O'Connor/.test(mail.html));
  ...
  // The card itself, not a stand-in for it: each of these properties is what keeps it rendering
  // consistently with every other downpipes email.
  ...
  // The plain-text part is WRAPPED, so a client that does not soft-wrap never makes a custodian scroll
  // sideways on a long logical line. The share is the one line allowed to run long: it is a single token
  // and a break in it would look like part of the value.
  {
  ...
  // the custodian reads it. A 121-character name must be refused rather than silently cut to 120 and emailed,
  // because a route that answers identically whether or not the label was shortened leaves nothing anywhere
  // saying the name had been changed. A truncated name is not a rejected one: it looks valid, it sends
  // without complaint, and it fails the only job it has.
  //
  ...
  //
  // Slicing to 120 before stripping control characters would quietly email a legal 120-character name
  // carrying a control character as 119 or fewer characters. That is the same cap-before-clean shape
  // that also matters for the change-number normaliser, and it is caught here by the control that proves
  // a legal value at exactly the boundary is still accepted, rather than by looking for it directly.
...
// the half a unit test cannot see: the route must refuse an over-length name rather than shorten it and
// answer identically to a request that sent no label at all.
//
...
async function route(): Promise<void> {
  ...
  // PINNING CONTROL. The identical request with the label ABSENT must answer DIFFERENTLY: if these ever
  // came back as byte-identical 200s, that identity would prove the label was never judged. Requiring a
  // difference here proves the refusal belongs to the label rather than to the request.
