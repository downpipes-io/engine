...
async function main(): Promise<void> {
  ...
  // route drops but the validator accepts is a source nobody can reach. The Artifact Registry beta gate was
  // written out by hand in both places (and a third time, in the console), with no reference between them,
  // so they could part company on one edit. They now derive from one exported list.
  //
  // This asks each authority BEHAVIOURALLY rather than reading either file. That is deliberate: the line
  // that used to hold the allow-list carried a trailing comment naming the very value it excluded, so a
  // scanner matching source text would have read the comment as code and reported the gate open. Calling
  // the route and calling the validator cannot be fooled that way.
  //
  ...
  // as "the type was accepted" for every type alike. The first draft of this check did exactly that and
  // reported a disagreement on artifacts that was its own fault. So the config below is otherwise VALID
  // (the identity, cadence and enabled fields the earlier validators want) and differs only in source.type.
