...
//
// What this proves (the fleet-scale scheduling behaviour a fleet larger than one DO list page needs):
//
//  Paging past the DO list page limit: listDownpipes / the scheduling scan must PAGE past the DO list
//  page limit.
//    The Cloudflare Durable Object storage list() returns at most one page (the platform caps it
//    at ~1000 keys). A single storage.list({prefix:"dp:"}) would silently TRUNCATE an
//    account with MORE than one page of downpipes: every downpipe past the
//    first page would never be returned, hence never scheduled by due(). This suite seeds >1000
...
//    Every other record family in the DO has a cap (audit AUDIT_CAP, history RING_CAP); an
//    unbounded drill-evidence log would grow without limit. This suite records MORE than the cap's worth
...
// would pass even a scan that never pages past the first page.
...
async function main(): Promise<void> {
  // ---- >1000 downpipes are ALL enumerated and ALL scheduled --------------------
  {
    ...
    // meaningful (a scan that calls list() only once would see only this).
    const onePage = await storage.list<DownpipeState>({ prefix: "dp:" });
    ok("a single un-paged list() returns only ONE page (the boundary a paged scan must cross)", onePage.size === PLATFORM_LIST_PAGE);
  ...
  // ---- rebuildDueIndex FULLY clears a >1-page stale due: index (no leaked tail) -
  {
    ...
    // an entry per current enabled dp: state. A single un-paged
    // storage.list({ prefix: DUE_INDEX_PREFIX }) returns at most ONE page (~1000 keys), so a drifted
    // index holding MORE than a page of stale entries (downpipes since deleted/disabled) would have only
    // its first page deleted and the tail LEAKED past the rebuild, so the index would never
    // converge to the dp: truth at fleet scale. The cleanup pages the deletion via listAllByPrefix.
    const { storage, stub } = makeScheduler();
    ...
    ok("a single un-paged list() returns only ONE page of the stale due: keys (silent truncation without paging)", onePage.size === PLATFORM_LIST_PAGE && onePage.size < seededStale);
    ...
    // The decisive assertion: NOT ONE stale "gone-*" due: key survives. An un-paged cleanup would
    // delete only the first page and leave the >1000th..tail stale keys behind.
    ...
    ok(`every stale due: key was cleared across ALL pages (0 leaked; an un-paged cleanup would leak ${seededStale - PLATFORM_LIST_PAGE})`, survivingStale.length === 0);
