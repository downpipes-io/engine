...
async function main(): Promise<void> {
  ...
  console.log("shapers: envelope correctness (the documented wire shapes)");
  {
    ...
    // status maps the outcome so Datadog's severity facet is accurate (never defaulting every event to
    // "info" for lack of the field).
  ...
  console.log("\nDO: cursor advances on 2xx, holds on failure; bounded trail; failureCount derived from the trail (invariant 2's storage half)");
  {
    ...
    ok("MONOTONIC: a lower toSeq from an overlapping tick does NOT regress the cursor (max, never backward)", (await dobj.getSiemPushCursor()) === 90);
  ...
  console.log("\nrunSiemPushPass: full drain integration (opt-in gate; cursor advance/hold; false-green on a genuine fault)");
  {
    ...
    {
      // loadConfigWrapKey runs INSIDE the pass's own try, so a
      // malformed CONFIG_WRAP_KEY (present but not base64url-of-32-bytes) throws loudly -- by design, so a
      // misconfigured key is never mistaken for "unset" -- but that throw must land on THIS pass's
      // false-green path (return false), never escape runSiemPushPass uncaught. A malformed key throws
      // before any DO round-trip, so no route on the fake scheduler is ever hit (throwing on ANY fetch
      // proves that).
    ...
    }
    {
      ...
      // advances) and returns false (the false-green signal). This forces a genuine tail throw with a
      // stored format the exhaustive shaper does not handle (a real cross-version contract violation): the
      // switch returns undefined, so `shaped.body` throws INSIDE the tail (the audit export + everything
      // above it serialise cleanly, so this isolates the throw to the tail, not the DO round-trips).
      const state: FakeState = {
      ...
      ok("DRAIN TAIL: a shape/deliver tail throw returns false (never escapes the pass, never skips silently)", passResult === false);
      ok("DRAIN TAIL: the tail throw recorded a FAILURE outcome (ok:false), so the cursor never advances", state.recorded.length === 1 && state.recorded[0]?.ok === false && state.recorded[0]?.reason === "shape-or-deliver-fault");
    }
  }
  ...
  console.log("\nS3-drop sink: deliverResolvedPush PUTs one NDJSON object via the archive S3 client (no bespoke SigV4); ok on 2xx, hold on failure");
  {
    ...
    try {
      ...
      // The generic "s3-put-failed" string is SPLIT into the closed S3 class set, so a 403 denial names the
      // operator action (a bucket policy / broken credential) rather than the undiagnosable catch-all.
  ...
  console.log("\nGELF over http: N events -> N requests, never a newline-joined batch (Graylog drops a batched body by default)");
  {
