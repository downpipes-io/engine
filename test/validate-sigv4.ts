...
async function main(): Promise<void> {
  ...
  // correctly leaves '.' unescaped per the SigV4 spec) rather than the guarded encodePath below, so
  // this demonstrates exactly what a destination's `new URL(io.url(key))` step (dest/s3-read-ops.ts /
  ...
  // encodePath refuses to build a path for such a key at all -- it is the single function every
  // S3Destination operation (put/putConditional/get/headStatus/del) funnels a key through (dest/s3.ts's
  // private url()), so this closes the collapse for the whole sink, not just the restore call site.
  {
    let threw = false;
    try {
