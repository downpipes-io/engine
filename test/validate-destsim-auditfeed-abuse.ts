...
// silent gap a retention rollover opens in the collector's view. See test/destsim/PULL-ABUSE-BEHAVIOUR.md for the
// confirmed behaviours this pins, including a non-finite afterSeq pagination bypass, and the
// plain-garbage-cursor-no-400 diagnosability gap.
//
...
// captureConsole swaps console.log/console.error for collectors so the V16.3.1 authentication-line proofs
...
interface FeedEnvelope {
  ...
  earliestSeq?: number; // the oldest retained seq, so a lagging collector can detect a rollover gap
  gapBefore?: boolean; // true when afterSeq+1 < earliestSeq (the collector's checkpoint skipped pruned events)
}
...
async function testForwardCursorCorrectness(): Promise<void> {
  ...
  const N = 97; // an odd, non-round total so a fixed page size never divides it evenly. Kept comfortably
  // under the engine's new per-clientId pull budget (INGEST_PULL_RATE_LIMIT_MAX_PER_WINDOW, 120 successful
  // pulls per 60 seconds, support-ingest.ts): the limit=1 walk below issues N+1 real pulls on one
  // credential, so N must stay well under 119 for that walk alone to fit inside the budget.
  for (let i = 0; i < N; i++) await stub.appendAudit(DRAFT);

  for (const limit of [1, 3, 7, 50, 500, 1000]) {
    // A fresh credential per page size: this walk is proving PAGINATION correctness, not credential
    // reuse, and re-minting keeps each limit's own pull count against its OWN rate-limit bucket rather
    // than accumulating across all six walks in this one test run.
...
async function testCursorAbuse(): Promise<void> {
  ...
  // STRICT CURSOR: a present-but-malformed afterSeq (non-numeric / non-finite / negative) is a clear 400,
  // never silently coerced to 0 (which would replay the whole retained log and hide a client bug re-sending
  // a corrupt cursor). Absent/empty defaults to 0; a valid finite non-negative number is accepted and
  // floored (consistent with the DO's own parseAuditFilter).
  // negative -> 400 (never silently clamped to 0)
  {
    const resp = await pull(stub, bearer, "?afterSeq=-5&limit=10");
    ok("afterSeq=-5 (negative): 400 (strict cursor, not a silent clamp to 0)", resp.status === 400);
  }
  // huge but finite (> headSeq) -> accepted as-is, yields an honest empty page, never an error
  {
    ...
    let resp: Response;
    try {
      resp = await pull(stub, bearer, "?afterSeq=999999999&limit=10");
    } finally {
      cap.restore();
    }
    ok("afterSeq=999999999 (> headSeq): 200, never a throw/500", resp.status === 200);
    // V16.3.1: the accepted pull's authentication is a per-attempt log line, without the bearer value.
    ok("V16.3.1: an accepted pull logs authn success method=token surface=support-pull", cap.lines.some((l) => /authn success method=token surface=support-pull/.test(l)) && !cap.lines.some((l) => l.includes(bearer)));
    const capBad = captureConsole();
    let bad: Response;
    try {
      bad = await pull(stub, "dpc_nope.dps_wrong", "?afterSeq=0&limit=10");
    } finally {
      capBad.restore();
    }
    ok("V16.3.1: a refused pull is 401 and logs authn failure surface=support-pull with its closed reason, no success line", bad.status === 401 && capBad.lines.some((l) => /authn failure method=token surface=support-pull reason=[a-z-]+/.test(l)) && !capBad.lines.some((l) => /authn success/.test(l)));
    const body = (await resp.json()) as FeedEnvelope;
  ...
  // "1e400" overflows Number() to Infinity, which would otherwise bypass BOTH the cursor filter and the
  // page limit (the DO returning the whole chain unpaginated). The Worker's finite check rejects it with a
  // 400 before the DO is ever reached.
  {
    const resp = await pull(stub, bearer, "?afterSeq=1e400&limit=3");
    ok("afterSeq=1e400 (overflows to Infinity): 400 -- pagination bypass closed", resp.status === 400);
  }
...
async function testPagingCostIsONotOLimit(): Promise<void> {
  ...
  // exportAudit's pure forward-feed page (afterSeq + limit, no other filter) is a BOUNDED storage.list
  // from the cursor key -- the `audit:` keys are zero-padded seq -- plus one bounded read each for the
  // head and the earliest. None of them scans the whole retained prefix, so the DO's internal work is
  // O(limit), not O(retained).
