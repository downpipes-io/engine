...
//
// The scenarios below share the same shape: the scheduler DO ABSORBS a fault (a closed-set
...
// default), and the fact of it must be captured somewhere a support pack can carry it. Each block below proves the same
// two things every new recording must satisfy:
//
...
//
// The scenarios, and the ticket each one makes diagnosable:
...
//        as "tested and failed". Each of those is a closed-set gate dropping a token without counting, unless it is caught here.
//   "our SAML cert expired with no warning" / "the console stopped warning about our licence expiry":
//        the observation is rejected, throws, or carries no usable date, in silence, unless the ladder records it.
//   "prove nobody tried to self-approve a restore", "our veto kept failing and the apply went through",
//        and the accounting hole: a SUCCESSFUL apply whose consume finds its lease reclaimed, so one approval
//        may authorise two applies with nothing recording it, unless the ledger catches the reclaim.
...
// ageApprovalRecord back-dates a stored restore-approval's requestedAt through the ordinary async put (this
// harness's MockStorage carries no rawPut), so a same-turn approveRestore is not itself refused as too-soon.
async function ageApprovalRecord(storage: { get(key: string): Promise<unknown>; put(key: string, value: unknown): Promise<void> }, planHash: string): Promise<void> {
  const key = `approval:${planHash}`;
  const rec = (await storage.get(key)) as { requestedAt: string };
  const at = Date.parse(rec.requestedAt);
  await storage.put(key, { ...rec, requestedAt: new Date(at - AGE_PAST_DWELL).toISOString() });
}
...
// ---------------------------------------------------------------------------------------------------------
// the credential-expiry warning ladder.
// ---------------------------------------------------------------------------------------------------------
...
async function testApprovalFaults(): Promise<void> {
  ...
  await ageApprovalRecord(storage, planHash);
  ...
  await ageApprovalRecord(clean.storage, planHash);
...
// ---------------------------------------------------------------------------------------------------------
// what the DR auto-heal silently drops, re-points and strips.
// ---------------------------------------------------------------------------------------------------------
async function testRecoveryResume(): Promise<void> {
  ...
  // (1) A tolerant resume SKIPS a malformed downpipe. Without the evidence below, the customer's downpipe simply does not come
  // back and the pack holds only the integer resumeSkipped.
  const exp = {
    configVersion: 4,
  ...
  // (2) A staged apply refused for a version mismatch: the recovery stalls forever, and the class is the
