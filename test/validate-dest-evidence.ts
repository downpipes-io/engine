...
//
// Stored/env destination config that is silently DROPPED. Every destination-config read is fail-safe by
...
// back silently. The drop is correct; its invisibility would be the bug. This proves the DROP EVENT is named
// in a closed vocabulary, recorded (stored half) or derivable (env half), and carries no value.
//
// Destination DEGRADATION on runs that SUCCEED. Store backpressure driving the pacer down, multipart
...
// ===================================================================================================
// the STORED destination config -- the drop event, classified and counted
// ===================================================================================================
...
// ---------------------------------------------------------------------------------------------------
// end to end: fetchDestConfig must POST the drop to the DO, with the sentinels nowhere in the body
// ---------------------------------------------------------------------------------------------------
...
// buildDestination is the piggy-back point: it flushes whatever the previous instance left pending.
{
  ...
  // buildDestination makes TWO documented diagnostic writes: the degradation-tally flush (asserted here) and
  // the destination BUILD-health observation (a success posts once per isolate, which is what clears a
  // standing "unbuildable since" streak). Assert the flush is among them rather than pinning the call count,
  // and assert the build-health post carries only its closed outcome.
...
// ---------------------------------------------------------------------------------------------------
// endpoint-unparseable, vhost-bucket-unsafe and endpoint-not-https are DEST_BUILD_CAUSES members.
// destBuildFaultOf classifies on the ERROR TYPE and never on the message (deliberately: the message carries
// the endpoint and the bucket), so each of these three build faults needs its own typed error out of
// s3-addressing.ts rather than a plain Error -- a plain Error would land in the destination BUILD-health
// record as the residual cause "other", which says a backup is failing to even build its destination but not
// WHY, when each of the three has a different one-line operator fix.
//
...
// The cause is read back off the health record the DO is posted, not off the thrown error.
{
  ...
  // carries the defect into a running fleet: without the guard, the request would go to
  // "<bucket>.<bucket>.s3.ap-southeast-2.amazonaws.com" and fail as a DNS lookup, which the pack could only
  // ever report as a network fault against credentials that were never wrong.
  causes.length = 0;
  threw = false;
