...
//
// The never-reported-replication signal needs a discriminator or it cries wolf: a destination configured
// minutes ago has reported no heartbeat, and so has a destination that has silently held nothing since
// March. Firing on the first devalues every true finding the bot will ever report, at the expense of a
// customer who did nothing wrong. The engine projects the fan-out a downpipe actually uses (configuredIds
// / neverReportedIds) and anchors each destination at the count it was added, so the two cases read
// differently.
//
...
async function doAnchors(): Promise<void> {
  ...
  // An edit must not hide a fault: a rename must not restart the clock on the destination that is failing.
...
// since March. Without the anchor these would read as the same row.
const FLEET = [
  ...
  // DE-DUPLICATED replicas, which is what the run path writes to), so it holds ONE key for d2 -- and the
  // pack's configured list must agree, or the one silent destination would be named twice and
  // neverReportedCount would read 2. The count is what the bot thresholds on, so a duplicated id would
  // inflate the finding. One destination is silent here, and the count must say one.
...
async function pack(): Promise<void> {
  ...
  ok("THE REVERSAL CRITERION: the two rows are DISTINGUISHABLE", JSON.stringify(repl("new")) !== JSON.stringify(repl("dark")) && repl("new").neverReportedSealedRunsSince !== repl("dark").neverReportedSealedRunsSince);
