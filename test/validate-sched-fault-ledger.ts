...
//
// These fault sites share one shape: the scheduler DO ABSORBS a fault (a coarse ceremony reason, a fail-open
// limiter, a `?? []` coercion, a fixed "unreachable" string, an early return) and the WHY exists only in a
...
//
// What each case makes diagnosable:
//   "sign-in failed (err:ab12cd)" / "internal error, errorId 7f3a..." -> the OPAQUE id the customer quotes
//        can be DEREFERENCED from the pack (stage + coarse class + a recurrence count), separating a
//        stored-COSE-key corruption from an ordinary bad_request.
...
//        OWN dropped writes are counted, and the shared rate limiter's silent fail-open is visible.
...
async function testDestinationAttribution(): Promise<void> {
  ...
  // A FAILED run whose coarse class carries the store's sanitised rejection code (an EXPIRED CREDENTIAL).
  // Booking every down destination under a single literal "unreachable" would leave support unable to tell
  // this from a WORM refusal or a timeout - three faults with three different owners and three different fixes.
  ...
  // The ENV-DEFAULT destination (no console destination id): recordReplicationState requires an id, so without
  // this path such a tenant gets NO per-destination down indicator ANYWHERE and triage starts on the source side.
  ...
  scanClean("the defaultDestination record", d.defaultDestination);
...
async function testFreshnessUncomputable(): Promise<void> {
  ...
  scanClean("the freshnessFaults record", d.freshnessFaults);
...
// ---- the DO-side ceremony fault classes + the errorId ring -----------------------------------------
async function testCeremonyFaults(): Promise<void> {
  ...
  // every user as their own bad_request. The wire response is unchanged; the cause is counted.
  ...
  scanClean("ceremonyFaults (the email, the credential id and the invite token were all planted)", d.ceremonyFaults);
  ...
  scanClean("the recentErrors ring", d.recentErrors);
...
async function testRecoveryCauses(): Promise<void> {
  ...
  // failure to the caller (the no-oracle property, unchanged) - the pack says WHICH bucket denied.
  for (let i = 0; i < 6; i++) {
  ...
  scanClean("the auth signals", signals);
  void storage;
}

// ---- the Worker<->DO contract faults, coerced in silence without this recording --------------------
async function testContractFaults(): Promise<void> {
  ...
  scanClean("the contractFaults record", d.contractFaults);
...
async function testDroppedWritesAndLimiter(): Promise<void> {
  ...
  scanClean("the droppedWrites record (the raw storage error was planted in the throwing put)", dropped);
  ...
  // The shared RATE LIMITER's fail-open: "Cloudflare is 429-ing our crawls" while the limiter is degraded to
  // no-throttle for weeks. Without this record the pack carries a PRESENCE bit only.
  ...
  scanClean("the rate-limit DO health record (the raw storage error was planted)", health);
...
async function testDoRouteErrorId(): Promise<void> {
  ...
  // it to support, who (holding only the pack) would have nothing to dereference it against without this ring.
