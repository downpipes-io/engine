// Pins the /support/* DoS-ordering hardening: an ABSENT bearer is rejected 401 BEFORE any SchedulerDO
// round-trip, so an unauthenticated flood cannot make each rejected request cost one DO fetch (a softer DoS
// target than /metrics, which already short-circuits its empty bearer first). A present (even wrong) bearer
// still incurs exactly the one grant lookup a real credential check needs.
//
...
async function main(): Promise<void> {
  // 1. Empty bearer -> 401, and NOT a single DO fetch (the ordering fix).
  {
    ...
    ok("empty bearer: ZERO DO round-trips (rejected before any DO fetch is issued)", count() === 0);
  }
  ...
  //    therefore throttled to one write per window per isolate.
  {
    ...
    // (the rejected-pull counter, so "our SIEM stopped collecting" finally arrives with a cause). The count
    // is therefore 1 or 2, and the assertion below is the one that actually matters.
  ...
  //     the distinct windows in which rejections occurred, and its lastAt stays exact), which is the whole
  //     point of the counter -- support can say "you have had rejected pulls since Tuesday" -- but an
  //     attacker cannot turn it into a write multiplier.
  {
