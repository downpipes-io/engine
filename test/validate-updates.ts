...
// trusts the PINNED SIGNATURE, not the host (see wrangler.toml "singular"), and every engine config
// file (wrangler.toml, wrangler.demo.toml) and the deploy ceremony comments use the singular spelling.
// The plural "updates.downpipes.io" is a drift risk in the external publish tool and design doc; pin the
// singular here so any reintroduction of a divergent host into the engine's own config FAILS the gate
// before a deploy could point UPDATE_CHANNEL_URL at a host the channel is not served on.
...
async function main(): Promise<void> {
  ...
  // V13.2.5 -- the channel is a FIXED vendor host: a mirror on another host is a url-config fault and nothing
  // is fetched from it.
  let fetchCalledMirror = false;
  const mirrorResult = await checkUpdates(
    { UPDATE_CHANNEL_URL: "https://mirror.example/stable.json", UPDATE_SIGNER_PUBLIC: validSignerPublic } as never,
    async (_u: string): Promise<Uint8Array | null> => { fetchCalledMirror = true; return null; },
  );
  ok("mirror host rejected (configured:false, channelFault url-config)", mirrorResult.configured === false && (mirrorResult as { channelFault?: string }).channelFault === "url-config");
  ok("mirror host rejected (reason names the pinned host)", typeof mirrorResult.reason === "string" && mirrorResult.reason.includes("update.downpipes.io"));
  ok("mirror host rejected (fetch not called)", !fetchCalledMirror);
  ...
  const redirectResult = await checkUpdates(
    { UPDATE_CHANNEL_URL: "https://update.downpipes.io/channel.json", UPDATE_SIGNER_PUBLIC: validSignerPublic } as never,
    redirectFetch,
  );
  ...
  // The LEGACY artefacts[] path hands richMetadataView the raw parse (only the v2
  ...
  const garbledResult = await checkUpdates({ UPDATE_CHANNEL_URL: "https://update.downpipes.io/channel.json", UPDATE_SIGNER_PUBLIC: validSignerPublic } as never, garbledFetch);
  ...
  const legacyProvResult = await checkUpdates({ UPDATE_CHANNEL_URL: "https://update.downpipes.io/dir/channel.json", UPDATE_SIGNER_PUBLIC: validSignerPublic } as never, legacyProvFetch);
  ...
  ok("channelBase is the channel URL's directory", legacyProvResult.channelBase === "https://update.downpipes.io/dir/");

  // Rich SIGNED release metadata. The whole channel is signed (detached hybrid sig), so every new
  ...
  // ENGINE_VERSION rather than a typed-in number, so the assertion cannot go stale as the engine version
  // advances: the same discipline test/validate-version-constants-parity.ts gates in src/format/version.ts.
  ...
  // The engine fetches a static signed channel from the host in UPDATE_CHANNEL_URL. wrangler.toml and
  ...
  // The plural check covers the PUBLISH TOOL as well as the engine's own config: the external publish tool
  // and design doc must align to this host, not the other way round, so the loop below reads
  // tools/publish-channel.mjs alongside the deploy configs. updates.downpipes.io does not resolve at all,
  // while update.downpipes.io/stable.json answers 200: an operator following a documented example that
  // names the plural host would publish a channel whose artefact url points at a host that does not exist,
  // and every engine applying that update would fail to download the bundle. A gate whose scope excludes
  // the file its own comment blames is not a gate.
  ...
  // ===================================================================================================
  // Channel freshness / replay protection (checkChannelFreshness is pure + total). `now` is fixed.
  // ===================================================================================================
  ...
  {
    const freshBody = utf8(JSON.stringify({ channel: "stable", recommendedVersion: "0.2.0", sequence: 9, issuedAt: "2026-06-20T00:00:00Z", artefacts: [{ version: "0.2.0", url: "https://update.downpipes.io/a.mjs", sha384: "abc123" }] }));
    ...
    const resolved = await loadVerifiedChannel({ UPDATE_CHANNEL_URL: "https://update.downpipes.io/channel.json", UPDATE_SIGNER_PUBLIC: validSignerPublic } as never, freshFetch);
  ...
  {
    const plainBody = utf8(JSON.stringify({ channel: "stable", recommendedVersion: "0.2.0", artefacts: [{ version: "0.2.0", url: "https://update.downpipes.io/a.mjs", sha384: "abc123" }] }));
    ...
    const resolved = await loadVerifiedChannel({ UPDATE_CHANNEL_URL: "https://update.downpipes.io/channel.json", UPDATE_SIGNER_PUBLIC: validSignerPublic } as never, plainFetch);
  ...
  // ===================================================================================================
  {
    ...
    const v2Env = { UPDATE_CHANNEL_URL: "https://update.downpipes.io/channel.json", UPDATE_SIGNER_PUBLIC: validSignerPublic } as never;
