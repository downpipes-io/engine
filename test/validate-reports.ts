...
// The in-memory DO storage double is shared across the DO validators (finding engine-test-013-14).
...
function testSlaComplianceReport(): void {
  {
    ...
    // RTO companion (E4/C1): the SLA row surfaces the recovery-time estimate NEXT TO the freshness/RPO
    // signal, but ONLY when an estimate was derivable; honestly absent (no fabricated number) otherwise.
...
function testImmutabilityReport(): void {
  {
    ...
    // The absent branch used to state only what is missing, and an auditor reading that alone could
    // reasonably conclude nothing is verified in this posture. It must state the assurance that remains.
    ...
    // The retired framing: the key was said to enable "self-test and self-restore", which stopped being
    // true when verification at seal and the canary moved onto the run's own per-run key.
...
// NotImplemented and AWS S3 with ObjectLockConfigurationNotFoundError, so the write is refused, not accepted
// and stripped. These cases pin each reading against the other two so the shared sentence cannot come back.
function testImmutabilityWormReadings(): void {
  ...
  //
  // THE RULE THESE ASSERTIONS EXPRESS IS NOT LOCAL TO THIS SUITE, and treating it as local is how it
  // failed. This block forbade /ignored|discarded/ on the not-enforced reading while
  // test/validate-posture-compute.ts pinned "the retention headers are ignored by the store" on THE SAME
  // INPUT, byte for byte, as a control. One suite required the words the other forbade, about the same
  // fact, in the same landing, and neither could see the other because each graded only its own builder.
  // The rule now lives in ONE gate that grades every surface at once,
  // test/validate-worm-refusal-parity.ts, which also carries the live evidence for it. These assertions
  // stay, because they pin THIS builder's exact phrasing and a rule cannot do that. Read the parity gate
  // before you change any wording here.
...
function testImmutabilityRetentionAxis(): void {
  ...
  // nothing is protected. This is the reading the report used to answer with the strong claim.
  ...
  // are controls: they pass on both sides, and they are here so the fourth axis cannot be used to soften
  // the refusal or the cannot-confirm reading that landed earlier today.
...
// Reports screen to any Owner or auditor on day one of every install. This drives the exact sequence the
// rehearsal drove: create a downpipe, record three successful runs over three hours at hourly cadence,
// then ask for the SAME 90-day report GET /admin/reports/sla-compliance and the console serve. Fails
// without the fix: expectedRuns reads 2160 (90 days / 1 hour), not 3.
...
async function testDORoutes(): Promise<void> {
  {
    ...
    // the fixed test NOW, for the just-recorded entry to fall in window. That much was already understood.
    //
    ...
    //
    // The floor is now taken off the same clock as the runs it is meant to contain, which is the property
    // that was actually wanted, and the ceiling stays on the real clock for the reason above.
    //
    // The ceiling carries an HOUR OF HEADROOM, and that is a live intermittent failure rather than a
    // tidiness. A bare `Math.floor(Date.now() / 1000)` is not ABOVE the real clock, it is level with it and
    // rounds DOWN, and the entry it has to admit is stamped by the DO some milliseconds LATER. Cross a
    // second boundary in that gap and recordedAt is realNowSec + 1, withinPeriod (reports.ts) compares
    // `sec <= toSeconds` inclusive, and the evidence assertion fails. Demonstrated rather than argued: a
    // 1500 ms sleep inserted between this line and the drill-evidence POST reddens this file with exactly
    // one FAIL, DO restore-tests evidence includes the recorded entry. Untouched, it is a roughly
    // one-in-a-hundred flake that would have been re-run and forgotten. The runs above are all months in
...
// A table header must never be the last thing on a page. This defect was found by RASTERISING the
// evidence pack and looking at it, not by measuring: the header sat alone above dead space at the foot
// of page 2 and was repeated at the top of page 3, so the reader met it twice and the first copy held
// no data. The website's pdf-appearance-gate cannot see it (its stranded-heading check reads text
// headings, this is a table header BAND, and every margin and size on that page is correct), so the
// guard belongs here, at the paginator, which is where the invariant actually lives.
//
