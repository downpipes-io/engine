// Proofs 2..9c: role grant/revoke, the per-route role matrix at the router, the restore-apply rule,
// Owner-only administration, the last-Owner guard, the DO's independent Owner re-check, the auth
// boundary, the token break-glass, ADMIN_TOKEN_DISABLED and the require-access posture surface. These
// proofs are STATEFUL over the shared scheduler in `ctx` and run in a fixed order; 9b/9c stand up their
// own fresh schedulers.
...
export async function runMatrix(ctx: Ctx): Promise<void> {
  ...
  // Reads are open to any authenticated role.
  {
    ...
    // These nine reads gate on: posture.read for the six the posture.read-gated support pack already
    // discloses (notify rules/history, expiry, destinations, push, otlp-push), downpipe.read for the
    // roster-hygiene report whose own comment names GET /downpipes as its read class, and roles.read for
    // the two role-table reads beside GET /roles.
    //
  ...
  // ---- PROOF 4: the restore-apply rule, enforced SERVER-SIDE (direct API) -----------------
  ...
  // does not live in the SPA. This is the launch-gate proof.
  {
    ...
    const b = (await r.json()) as { error: string; required: string; have: string };
    ok("operator restore APPLY refused 403 (server-side)", r.status === 403);
  ...
  // An Approver passes the ROLE gate, which is the D3 rule this validator proves. Restore dual control is
  // OWNER-OPT-IN, so a solo operator can recover, and with no policy set there is no approval to be
  // missing: the approver gets 200 with ok:false and "break-glass-only posture: no in-account read-back
  // key", having reached the apply path. Proving the role gate this way, rather than through the
  // dual-control refusal behind it, does not depend on that gate's shape.
  //
  // This grades the ROLE GATE ITSELF rather than the gate behind it, and it does not do that by weakening
  // to "not 403", which would pass for any refusal at all. It names the operator's refusal EXACTLY (the
  // shape asserted twelve lines above: 403, error "forbidden", required "restore.apply") and requires the
  // approver's answer to differ in that specific way, PLUS positive evidence that the apply path was
  // actually reached. Both directions of the opt-in policy are driven end to end through the real DO by
  // validate-restore-approval-optional.ts, so nothing is lost by not re-proving the dual-control refusal
  // here.
  {
  ...
  // ---- PROOF 9b: ADMIN_TOKEN_DISABLED hardens away the token fallback (P9) ----------------
  {
  ...
  // ---- PROOF 9c: POST /policy/require-access surfaces the Access-only posture (P9/console-access-1) -----
