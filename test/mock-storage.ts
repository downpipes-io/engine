...
// prefix list) lands in one place rather than diverging across separate copies.
...
export class MockStorage {
  ...
  // form the DO's findRunRing read path uses.
  ...
  // a paged feed issues a BOUNDED list -- start + limit -- rather than an unbounded full-prefix scan).
  // Additive and inert for callers that never inspect it.
  ...
  // object after seeding does not corrupt what is on-store.
  ...
  // rawKeys/keysWithPrefix/keys/rawListKeys are the several names validators use for the same
  // underlying operation -- read the stored keys, optionally filtered by prefix. rawKeys returns them
  // lexicographically sorted (the shape audit/history validators that assert ring order rely on);
  // keysWithPrefix and keys return them in map (insertion) order; rawListKeys returns every key with
  // no filter. All four are kept under their names, so a validator using any of them needs no
  // call-site rewrite.
