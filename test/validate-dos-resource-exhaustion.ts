// The workspace's chaos test suite, axis DoS / resource-exhaustion, Tier 1 (deploy-free, net-zero) -- the ENGINE half of the cluster.
// It proves the engine degrades SAFELY under resource pressure across THREE distinct planes, getting the split
...
// them.
//
// NET-ZERO: in-memory DO-storage / KV / destination doubles, an injected clock, harness-minted signer +
// recipients, fake records. No estate, bucket, network, seed, deploy or spend, no real flood. Teardown is
// process exit. ~/Desktop/downpipes-live-keys is never read.
...
async function planeOne(): Promise<void> {
  ...
  // bucket-maths pacing plus honest fail-open HEALTH counters (noteFailOpen), never a hard stop. Driving it
  // expecting a 429 would be mis-specified against a correct engine.
...
async function planeThree(): Promise<void> {
  ...
  // PARKS on (handleContention) instead of striking the run dead (the shed-one-third fix).
  // ---------------------------------------------------------------------------------------------------
  ...
  console.log("\n=== CELL (e): no false green under DoS (no plane presents a DoS condition as a clean success) ===");
  {
    ...
    // noteFailOpen, limiter-verdict-malformed).
