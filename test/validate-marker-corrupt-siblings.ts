// validate-marker-corrupt-siblings: three markers coerce a corrupt record to zero, and only one says so.
//
// THE INVARIANT THIS PINS. `readEmergencyChangeMarker` (scheduler-do-change-management.ts) reads a stored
// `{count,lastAt}` marker, coerces a PRESENT-but-malformed record to `count: 0`, and books
// `marker-corrupt-defaulted`, with its own note naming the harm: "the compliance posture saying no emergency
// changes were raised about an account that raised them. The safe default is right; the silence is not."
//
// `getChangeControlRefusals` and `getConfigSnapshotHealth` perform the IDENTICAL coercion but book nothing.
// Both are served to the support pack, over GET /change-control/refusals and GET /config-snapshot-health, and
// read by `fetchConfigIntegrity`.
//
// ZERO IS NOT ABSENCE, AND HERE IT IS WORSE THAN A LOW NUMBER: the pack surfaces both blocks ONLY when
// `count > 0`, so a coerced zero does not read as "few refusals", it makes the block VANISH. Left
// unaddressed, `fetchConfigIntegrity` returns `{}` for a clean account and `{}` for a corrupt marker at
// either site, with `unavailableProbes` EMPTY -- and `unavailableProbes` is the mechanism that function
// already carries to tell a faulted sub-read from a clean one. A throw is the only failure a bare coercion
// can see; answering 200 with a silent default is the quiet way to fail.
//
// THE CONFIG-SNAPSHOT SITE CARRIES A SEPARATE FAULT BENEATH THE SHARED SHAPE, which is why this measures the
// premise rather than inheriting it. `autoSnapshotConfig` reads the prior RAW -- trusting
// `storage.get<ConfigSnapshotFailureState>`, which is a type ASSERTION and not a check -- and then applies
// JavaScript's `+`: a stored count of `"7"` becomes `"71"`, then `"711"`. The record moves FURTHER from a
// healthy value with every failure and the health read stays at zero PERMANENTLY. `NaN` and `Infinity` latch
// the same way. Its two siblings (`bumpEmergencyMarker`, `bumpChangeControlRefusal`) bump through their
// validating reader and heal in one write. So "coerces exactly as readEmergencyChangeMarker does" is true of
// the READER and false of the SITE, and section E is the difference.
//
// THE FIX ALREADY EXISTS IN THE PRODUCT AND IS ALREADY USED AT THE SIBLING SITE, twice over, so nothing new
// is invented: `recordStorageAnomaly(storage, "marker-corrupt-defaulted")` in the DO, and `unavailableProbes`
// in the pack. Closing the config-snapshot gap is a DELETION of a raw read, not a new mechanism.
//
...
// `JSON.parse(JSON.stringify(v))`, which turns `NaN` and `Infinity` into `null` -- a JSON mock would report
// the NaN and Infinity doses as HEALING, because the instrument heals them, not the code under test. Section
// B asserts the difference directly, so a future move back to a JSON mock fails loudly here rather than
// quietly reporting a repair that belongs to the mock.
//
...
async function sectionDoseResponse(): Promise<void> {
  ...
  for (const d of MALFORMED) {
    ...
    ok(`C-cc ${d.label}: coerced to 0, coerced:true, and one marker-corrupt-defaulted anomaly booked`, ccR.count === 0 && ccR.coerced === true && (await anomalyCount(cc.storage)) === 1);

    const cs = subject();
    await cs.storage.put(CONFIG_SNAPSHOT_FAILURE_KEY, d.value);
    const csR = await cs.stub.getConfigSnapshotHealth();
    ok(`C-cs ${d.label}: coerced to 0, coerced:true, and one marker-corrupt-defaulted anomaly booked`, csR.count === 0 && csR.coerced === true && (await anomalyCount(cs.storage)) === 1);
  }
...
// ---------------------------------------------------------------------------------------------------------
// E. THE LATCH. The config-snapshot WRITER does not go through its reader, so a malformed count is carried
// into JavaScript's `+`, and the record gets further from a healthy value with every real failure. Ten REAL
// failures against each malformed seed must arrive at exactly ten, and the stored count must be a finite
// number.
// ---------------------------------------------------------------------------------------------------------
async function sectionLatch(): Promise<void> {
  ...
  // And the sibling writer, which always went through its reader, is unchanged by this pass.
  const cc = subject();
...
async function sectionOverFix(): Promise<void> {
  ...
  // And the healed record stops booking, so the counter does not stick on once the record heals.
...
async function sectionRedaction(): Promise<void> {
  ...
  ok("I2 the storage-anomaly ledger projection carries no planted value", !JSON.stringify(diag.storageAnomalies ?? {}).includes(POISON));
  ...
  // The redaction check catches a gap the count assertions alone cannot see: gating `lastAt` on being a
  // NON-EMPTY STRING and `lastActionKind` on LENGTH ALONE would let a corrupt record's arbitrary bytes return
  // verbatim from a method whose own contract says it "never returns an operator value". A coerced record's
  // qualifiers are dropped instead, and `lastAt` is gated on PARSING at both sites.
