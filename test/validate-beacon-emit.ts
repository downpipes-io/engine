...
async function main(): Promise<void> {
  ...
  console.log("\negress screen on BEACON_URL (ASVS V1.3.6): the bearer never leaves for a target the screen refuses");
  // Each case is paired with the ON case above, which proved this same stub DOES capture a POST to a public
  // https base, so a null here is a refusal and not a probe that never saw the send.
  for (const [base, why] of [
    ["http://cp.example", "cleartext: the bearer would travel unencrypted"],
    ["https://169.254.169.254", "cloud-metadata address"],
    ["https://10.0.0.5", "RFC 1918 private address"],
    ["https://localhost", "loopback name"],
  ] as [string, string][]) {
    resetPosted(); captureFetch(); beaconStatePosts = [];
    await runBeaconEmitPass({ BEACON_URL: base, BEACON_INGEST_KEY: "ik", CF_ACCOUNT_ID: "acct-1" } as unknown as Env, fakeScheduler());
    ok(`${base} (${why}): NO POST is issued`, posted === null);
    ok(`${base}: the attempt is recorded as failed with the url-invalid class, so the pack says "fix BEACON_URL"`, beaconStatePosts.length === 1 && beaconStatePosts[0]?.ok === false && (beaconStatePosts[0] as { errorClass?: string }).errorClass === "url-invalid");
  }
  // A redirect is a failure, never a place to follow the bearer to: with redirect:"manual" a 302 comes back as
  // a non-ok response and is recorded as such. Contrast the 200 case above, which recorded ok:true.
  resetPosted(); captureFetch(302); beaconStatePosts = [];
  await runBeaconEmitPass({ BEACON_URL: "https://cp.example", BEACON_INGEST_KEY: "ik", CF_ACCOUNT_ID: "acct-1" } as unknown as Env, fakeScheduler());
  ok("a 302 from the receiver is recorded as a failed attempt (non-2xx), not followed", beaconStatePosts.length === 1 && beaconStatePosts[0]?.ok === false && beaconStatePosts[0]?.status === 302);
  // Re-establish the ON case's state, which the attempt-outcome assertions below read.
  resetPosted(); captureFetch(); beaconStatePosts = [];
  await runBeaconEmitPass(
    { BEACON_URL: "https://cp.example", BEACON_INGEST_KEY: "ik", CF_ACCOUNT_ID: "acct-1", CF_VERSION_METADATA: { id: "cf-v9", tag: "" } } as unknown as Env,
    fakeScheduler(),
