...
async function notifyDeliveryCode(): Promise<void> {
  ...
  // Granular failure codes: 401/403/429/400 do not collapse into the blanket http-4xx bucket, so an
  // operator can tell "fix your credential" from "you're throttled" from "you sent a malformed request"
  // from any other 4xx.
  ...
  // email-platform-rejected + the shape-gated platform code (this separates the engine-side formatting
  // faults from the platform's own refusal, which have opposite owners; see
