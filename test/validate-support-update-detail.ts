...
async function main(): Promise<void> {
  ...
  //
  // The free-text `reason` is classified out because it carries the raw Cloudflare deploy error. Two more
  // fields carry the same risk, in the SAME object literal, behind a 160-char clamp: settleTrace[].detail
  ...
  // leak, not its content. Both are classified into closed members instead.
  //
  ...
  const leaky = {
    last: {
      ...
      settleTrace: [
        ...
        // ENGINE-AUTHORED by probeSettleVerdict against the real gate, and it is distinct from a legitimately
        // slow "pending" flight.
        ...
        // A HOSTILE / DRIFTED detail carrying a live platform URL. No production path can author this (the
        // "flight-errored" class it would otherwise select is unproducible), so what matters is that it is
        // CLASSIFIED to the residual rather than clamped through. A clamp would bound the length of the leak.
