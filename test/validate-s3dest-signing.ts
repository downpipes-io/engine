// Signing-and-addressing vectors for the S3 destination: the STS session token (x-amz-security-token)
// signed and sent on every request, path-style vs virtual-hosted addressing and the "auto" default, and
// the creation-only signed storage-class header.
...
async function addressingStyles(): Promise<void> {
  ...
  {
    // ML-19 (V12.3.1): explicit vhost + a DOTTED bucket must stay honoured (a legitimate, if
  ...
  {
    // ML-19: an explicit vhost bucket is concatenated RAW into the request host (s3.ts's
