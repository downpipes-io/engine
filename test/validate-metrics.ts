...
async function testHandleMetricsRouteAuthGating(): Promise<void> {
  ...
  }
  {
    ...
    let resp: Response;
    try {
      resp = await handleMetricsRoute(new Request(metricsUrl, { headers: { authorization: "Bearer dpc_nope.dps_wrong" } }), asStub(stub));
    } finally {
      cap.restore();
    }
    ok("auth: an unrecognised bearer -> 401 (no grant exists yet)", resp.status === 401);
    ok("V16.3.1: the refused scrape logs authn failure surface=metrics with its closed reason", cap.lines.some((l) => /authn failure method=token surface=metrics reason=[a-z-]+/.test(l)) && !cap.lines.some((l) => /authn success/.test(l)));
  }
  {
  ...
  const bearer = `${minted.clientId}.${minted.secret}`;
  {
    ...
    let resp: Response;
    try {
      resp = await handleMetricsRoute(new Request(metricsUrl, { headers: { authorization: `Bearer ${bearer}` } }), asStub(stub));
    } finally {
      cap.restore();
    }
    ok("auth: the minted metrics bearer -> 200", resp.status === 200);
    ok("V16.3.1: the accepted scrape logs authn success method=token surface=metrics, without the bearer", cap.lines.some((l) => /authn success method=token surface=metrics/.test(l)) && !cap.lines.some((l) => l.includes(minted.secret)));
