...
//
// WHY THIS TEST EXISTS. Both fault ledgers are recorded at the exact fault site, but recording alone is not
// enough: without a caller draining it and a reader on the run path, the evidence sits in an isolate-local
// accumulator and never reaches anywhere else. Evidence is not useful until it is in the bundle a remote
// diagnosis can read, so this validator walks the WHOLE chain and refuses to accept any single hop as proof:
//
...
async function main(): Promise<void> {
  ...
  ok("the seal path DRAINS the ledger and POSTS it (drainSourceFaultLedger's only caller)", posted.length === 1);
  ...
  // ---- 8. the storageClass SHAPE GATE --------------------------------------------------------------------
  // destResolution.destinations[].storageClass is an OPERATOR-CONTROLLED field, so it is gated to the
  // engine's own S3 storage-class allow-list rather than accepted on a bare `typeof === "string"` check: a
  // value a drifted writer stored outside that allow-list cannot ride verbatim into the confidential pack.
  const destStub = {
        ...
        return new Response(
          JSON.stringify({
            destinations: [
              ...
              // An operator string outside the allow-list, which the gate must not carry verbatim.
