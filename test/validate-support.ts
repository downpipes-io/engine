...
// CPR fixtures: a real CONFIG_WRAP_KEY + a destination secret wrapped under it, so the pack's
...
function schedulerDouble(): { stub: DurableObjectStub; grants: Record<string, IngestGrant | null>; pulls: { scope: string }[]; calls: string[]; auditStatusPosts: Array<Record<string, unknown>> } {
  ...
  // that have nothing to do with the pack. Ninety days ahead is unambiguously unexpired and cannot age. The
  // lapsed twin at the expired-grant check stays a written-down 2020 instant on purpose: an instant in the
  // PAST only gets further past, so its assertion cannot flip and the fresh-against-lapsed pair keeps meaning
  // what it says.
  ...
  // A restore-verified anchor carries the redaction-safe apply SUMMARY. This apply was WINDOWED and
  ...
  // Two dest-config events carrying the structured dest-change target (config-change modes): seq 20 = a
  ...
  // seq 18-19: dual-control config-approval lifecycle events, placed at role-change churn slots 17-18 (indices
  // 19-21 / seq 20-22 already carry the dest-change + restore-verified anchors).
  ...
  // by the server-side action filter. This is exactly the busy-account case the keystone retention targets: the
    ...
    async fetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
      ...
      switch (url.pathname) {
        ...
        case "/history": {
          ...
          // incompleteIds carries WHICH surface each kind hit (cf-config surface ids -- the closed/product
        ...
        case "/notify/history":
          ...
          // The page never existed: a REAL non-delivery hiding inside a delivered:true row, so the pack must
          // carry both `unconfirmed` and `ackOutcome`. seq 5 carries a HOSTILE ackOutcome the closed gate must
          // drop. seq 2 carries the rotated-wrap-key code (credential-undecryptable), distinct from the
          // misleading `no-transport`.
        ...
        case "/control-plane/recovery-status":
          ...
          // path; the pack MUST project it to a boolean and never forward the reason, but it DOES forward
        ...
        case "/control-plane/export-state":
          // export-lag: the last-export pointer carries the config VERSION the signed control-plane export
        ...
        case "/replication":
          // dp1 has a healthy origin AND a replica proven DOWN this run (lastOk:false). The down
        ...
        case "/reconcile-inventory":
          ...
          // origin-r2 carries the SPLIT counts the DO inventory always made but the three-way summary folds
          // away -- `undetermined` splits into a benign too-young run, a HARD read fault, and a read-BUDGET
        ...
        case "/seal-faults":
          ...
          // The three RETENTION kinds: a prune that ABSTAINED (deferClass + the opaque id of the
        ...
        case "/scheduler-signals":
          ...
          return new Response(JSON.stringify({
            ...
            // schedHealth: the housekeeping faults that are otherwise fully silent. listTruncations > 0 is the
        ...
        case "/retention-state":
          ...
          // replicationUnreadable: the coverage gate ran on NO proof, so every replicated run was HELD.
        ...
        case "/destinations":
          ...
          // Each destination's IMMUTABILITY posture rides here. d-r2 is the dangerous immutability SHADOW: a
          // WORM policy is CONFIGURED but the bucket does NOT actually enforce Object-Lock, so the customer
          // believes their archive is immutable and it is not. d-s3 has a HOSTILE objectLock value the pack
          // must drop. The posture is visible for every destination, not only the default one.
          return new Response(JSON.stringify({ destinations: [
            ...
            // storageClass is SHAPE-GATED to the engine's own S3 storage-class allow-list: a bare
            // `typeof === "string"` check would let an OPERATOR-CONTROLLED value ride into the confidential
            // pack ungated, so anything outside the allow-list collapses to unknown-code -- never carrying the
            // value verbatim.
        ...
        // DO-owned status opts: the pack round-trips these so buildStatus lights up the same
        ...
        case "/sso-failures":
          // The bounded per-code SSO sign-in failure aggregate. Includes a code OUTSIDE the closed vocabulary
        ...
        case "/sso-failures-by-kind":
          // The per-connection-KIND breakdown. "bogus-kind" (outer) and "bogus-code" (inner) are OUTSIDE their
        ...
        case "/auth-signals":
          // The bounded auth-signal aggregate. "not-a-real-signal" is OUTSIDE the closed vocabulary and must be
          ...
          return new Response(JSON.stringify({
            ...
            // A sample of the newly-wired closed-vocabulary signals must project through unchanged.
        ...
        case "/auth-posture":
          // session signing-key presence + age + adequate-length (never the key), the do-plaintext missing
        ...
        case "/audit/export": {
          // The server-side action filter (the keystone fetch): return the single newest matching event,
          ...
          // Mirror the real DO's afterSeq+limit pushdown: when the feed pull supplies
          // both, return ONLY the matching window (events strictly after the cursor), the OLDEST limit
          // ascending; otherwise the whole ascending set. This is the behaviour the Worker trusts the
        ...
        case "/expiry":
          ...
          //
          // THIS DOUBLE COMPUTES ITS `state`, IT DOES NOT WRITE IT DOWN. A literal state ("ok" beside fixed
          // instants) cannot drift, so an assertion pinned against it stays green even after the route it
          // emulates starts returning something different: scheduler-do-expiry.ts returns
          // expiryStatuses(items, Date.now()), and stateFor maps daysRemaining <= 0 to "expired" and
          // <= APPROACHING_DAYS to "approaching" -- a fixed literal stops reflecting that as the clock moves.
          //
          // A CLOCK SHIFT ALONE CANNOT CATCH THIS: shifting the clock moves the producer, and a literal
          // fixture stays constant, so nothing flips and a sweep still sees a stable green. Moving the
          // literals forward is not the fix either: that buys time and rebuilds the same defect with a later
          // fuse.
          //
          // Both halves matter. Calling the real expiryStatuses makes the double structurally incapable of
          // returning a row the route could not produce, and deriving the instants from this run's own clock
          // keeps the intended states fixed for ever. The offsets sit clear of the APPROACHING_DAYS boundary
          // so a slow run cannot tip a row into "approaching".
        ...
        // The source and destination fault ledgers (drainSourceFaultLedger needs a caller for either to reach
        // the bundle), plus the restore fault ring and the admin silent-fallback counters. Each carries a
        // HOSTILE member the closed gate must drop, and a customer value the redaction sweep must never find.
        case "/source-faults":
          return new Response(JSON.stringify({
            dp1: {
...
async function main(): Promise<void> {
  ...
  console.log("\nthe persist-state storage-fault counter (pure classify/apply):");
  {
    ...
    // lastDownpipeId: null is the "no fault attributed yet" prior on StorageFaultCounter. Left null
    // deliberately: this case is about CLAMPING the numeric fields, and an unattributed prior is simply a
    // counter with no fault attributed yet.
  ...
  console.log("\nthe support bundle:");
  {
    ...
    // Capture-timing race: the bundle build SELF-OBSERVES the deploy identity -- the same
    ...
    // The evidence recorded at the fault site is not diagnosable until it reaches the BUNDLE. The source and
    // destination fault ledgers are written at the fault site, and drainSourceFaultLedger() is their only path
    // to a remote diagnosis. validate-run-fault-evidence.ts proves the whole chain from the adapter; here the
    // BUNDLE contract is pinned.
    {
      ...
      // The REDACTION HOLE: destResolution.destinations[].storageClass must never ride ungated.
    ...
    // buildStatus WITH DO opts, and statusSource / destResolved: buildStatus is called with
    // the DO round-trip opts, so the pack's status carries the break-glass-disposal latches, the
    // expiry/cleanup counts, the restorability count, and the LIVE console-set destination. statusSource
    // "live" proves the DO /dest-status read resolved; a DO blip reads "env-fallback" (the
    // status-preflight-conflict tell). destResolved names the resolved archive kind (here the console-set R2
    // reached via its S3-compatible endpoint).
    ...
    ok("status reports statusSource 'live' when the DO dest read resolved", st["statusSource"] === "live");
    ...
    // WORM / Object-Lock posture, FAIL-SOFT side: the pack carries the default destination's live
    ...
    // the run row surfaces recordsVanished (in-scope objects deleted mid-crawl between list and read) as
    ...
    // per-kind ATTRIBUTION: the pack run row names WHICH surface each incompleteness kind hit (cf-config
    ...
    // v:2 configEvents KEYSTONE smart-retain: the recent window here is role-change churn (+ the seq 23-25
    ...
    // restore-APPLY summary projection: a restore-verified event carries HOW the apply landed. The excerpt
    ...
    // v:2 dest-change STRUCTURED projection (config-change): the excerpt carries the CLOSED signals so the
    ...
    // notify-config change legibility: a config-change event's CLOSED-ENUM changeKind is projected
    // so a NOTIFY-CONFIG mutation is legible in the excerpt ("someone changed a rule and broke alerting"). The
    // config-change-* actions are allowlisted, and changeKind is what makes one legible.
    ...
    // v:2 ssoFailures: the bounded per-code SSO sign-in FAILURE aggregate. Projects ONLY closed-vocabulary
    ...
    // ssoFailuresByKind: the per-connection-KIND split. Projects a member connKind -> member SSO code -> {count},
    ...
    // authSignals: the bounded auth/RBAC defensive-branch counters. Closed vocabulary only; DROPS an out-of-vocab
    ...
    // the newly-wired closed-vocabulary signals project through unchanged (cf-access denial, SAML SP-initiated,
    // session revocation), which is what makes the idp-auth failure clusters diagnosable from the pack.
    ok("authSignals projects the idp-auth signals (cf-access-aud-mismatch, saml-relaystate-missing, session-revoked-idp-epoch)", asig?.["cf-access-aud-mismatch"]?.count === 4 && asig?.["saml-relaystate-missing"]?.count === 2 && asig?.["session-revoked-idp-epoch"]?.count === 6);
    ...
    // authPosture: the session signing-key presence/age/adequate-length (never the key), the do-plaintext-
    ...
    // keys pre-ceremony: build a bundle with a NO-KEY env (SIGNER_PRIVATE only) so all three sub-blocks report
    ...
    // v:2 recovery latch: recoveryRequired surfaces; `refused` is a BOOLEAN, never the DO's free-text
    ...
    ok("recovery.refused is projected to a boolean, not the DO's free-text reason object", rec.refused === true);
    ...
    // NO-CUSTODY: the refusal reason embedded a secret field name + a storage JSON path; it must NOT
    // appear anywhere in the signed bundle. (The static top-level latch reason is safe and may appear.)
    ok("the refusal reason free-text (secret field name + path) never reaches the bundle (no-custody)", !text.includes("secretAccessKey") && !text.includes("no-custody violation") && !text.includes("destinations[0]"));
    // The recovery projection carries the CLOSED refusal code, the recovery latch's own amnesia-probe class,
    // the deterministic deploy-identity marker, the export-pass health, and the staged resume diagnostics —
    // all redaction-safe (closed enums / ints / flags).
    ...
    // control-plane export-lag: the export-state pointer rides as controlPlaneExport -- the config VERSION
    ...
    // v:2 source selector: include/exclude globs + the CF accountId the downpipe ran under.
    ...
    // class OUTSIDE the closed vocabulary never reaches the pack (redaction defence), but it does not take the
    ...
    // replication attribution: WHICH destination is down. The down replica is recorded only in the
    ...
    // The engine self-diagnostic sections: each new recorder needs a gatherer, a roster entry
    // and a section() wrapper, so the health vector stays COMPLETE (a gatherer added without a wrapper would
    // already break the exact-equality check above). Their PROJECTIONS and their redaction posture are
    ...
    // last outcome + REASON + canary verdict, the rollback latch,
    ...
    // The free-text `reason` is never projected: the update paths interpolate the raw Cloudflare deploy-driver
    // error into that sentence (`...: ${msg(e)}`), so a platform message carrying a URL, an account id or a
    // script name would ride into the sealed bundle behind a 200-char clamp, and a clamp is not a redaction.
    // The pack carries the closed reasonClass instead, and the operator still sees the full sentence in their
    // own console. See test/validate-update-reason-redaction.ts.
    ...
    ok("updates does NOT carry the free-text reason (it embeds the raw platform error)", upd.last?.["reason"] === undefined);
    // The DECISION TRACE: the pack carries selfCheckOk + the per-attempt settleTrace, so a self-apply rollback
    // is diagnosable FROM THE PACK -- was it a dead canary (data) or a swap-window flake -- without a live
    // tail. Each step carries its closed detailClass, NOT the engine's sentence. update-gate.ts's catch arm
    // interpolates the raw canary-flight exception (which embeds the canary URL and hostname) into that
    // sentence, which would ride into the sealed bundle behind a 160-char clamp. A clamp bounds the LENGTH of
    ...
    // orphan inventory + RUNLOG health the report-only reconcile pass persists.
    ...
    // sealFaults (seal-integrity modes shard-list-truncated / orphan-root-worm-leak /
    ...
    // sealKnobs (seal-integrity: verify-at-seal-disabled / seal-verify-knobs-invisible /
  ...
  // ---------------------------------------------------------------------------------------------------
  // Every field the engine records must actually RIDE the bundle, be gated on its CLOSED vocabulary, and
  // carry nothing custody-unsafe: scheduler health + state refusals, blackout resolution, retention prune
  // evidence, reconcile splits, per-destination immutability posture, the async-create ack outcome, the
  // key-material classes, and the unknown-code placeholder (never a silent drop).
  console.log("\nthe support bundle carries the gap-audit evidence (closed-gated, redaction-safe):");
  {
    ...
    // --- the reconcile splits the three-way summary folds away -----------------------------------------
    ...
    ok("destResolution STILL never carries an endpoint host or bucket name (the contract is unchanged)", !bStr.includes("my.dotted.bucket") && !bStr.includes("r2.cloudflarestorage.com") && !bStr.includes("plainbucket"));
  ...
  // these pin the arms a single roster row cannot exercise at once). The degraded posture is the one that
  // matters most: a source that cannot re-identify itself CANNOT be reconstructed after a control-plane loss.
  ...
  console.log("\nthe restore-test failure-reason gate (defence-in-depth redaction):");
  {
    ...
    // The RAW code is never carried (the redaction guarantee is absolute), but the field does not VANISH
    // either -- a downpipe whose restore tests have failed for weeks still reports a cause, via the drift
    // placeholder, rather than reading as though it had never reported one at all.
    ok(
  ...
  {
    // A configured WORM policy (env DEST_WORM_*) + an in-account R2 destination. The pack carries the
  ...
  console.log("\nthe sealed bundle (vendor key):");
  {
    ...
    // DERIVATION-IDENTITY PROOF: the sealed envelope's capsule wrap
  ...
  console.log("\nconfigEvents keystone breadth under churn:");
  {
  ...
  console.log("\nthe credentialed pull routes:");
  {
    ...
    // The bare-text body is replaced with a structured
    // { error } field, matching the bad-cursor 400 this route already returned as JSON.
    ok("a pull without a bearer is a JSON 401 carrying error", noAuth.status === 401 && ((await noAuth.json()) as { error?: string }).error === "unauthorised");
  ...
  console.log("\nthe support bundle (sparse and edge shapes):");
  {
    ...
    // the env-fallback side: the sparse double answers {} for /dest-status, so consoleDestSet
    ...
    ok("a bundle whose DO dest read did not resolve reports statusSource 'env-fallback' + destResolved 'unknown'", sparseStatus["statusSource"] === "env-fallback" && sparseStatus["destResolved"] === "unknown");
      ...
      async fetch(input: RequestInfo | URL): Promise<Response> {
        ...
        switch (url.pathname) {
              ...
              JSON.stringify([
                {
                  ...
                  // Restorability assurance. restoreProven.by is a verified Access EMAIL -- it must be
                  ...
                  // cf-config surface discovery. present/empty carry whole (both under the 64 cap);
                  ...
                  dp1: [
                    {
                      ...
                      // The verdict-projection passthrough carries the WHOLE redaction-safe verdict, including
                      // tier0Cause (mode tier0-only-reason-unknown) + attempts/recovered (mode
                      // runlog-sig-stale-window, the self-healed read-after-write window).
          ...
          case "/tick-info":
            return new Response(JSON.stringify({ lastTickAt: Date.now() }));
          case "/licence-token":
            // A no-custody, console-activated record. The token itself need not verify (it has no
  ...
  console.log("\nthe support bundle (rich and present shapes):");
  {
    ...
    // Restorability assurance. restoreProven carries at/method/runId + a BOOLEAN `attributed` derived from
    ...
    // licence.setBy is the same no-custody class as restoreProven.by (a verified Access e-mail); it is
    ...
    // cf-config surface discovery: the present/empty/unavailable surface-id sets + the probe timestamp
    ...
    // surfaces" prose is accurate even though the array itself is capped at 64.
    ok(
    ...
    // The diagnostic verdict fields ride the same whole-object passthrough (modes
...

  {
    ...
    // and the bundle still BUILDS: a section fetch throwing never crashes the whole bundle build.
  ...
  // A try/catch that returned {} on ANY fault would let a DO-unreachable read reach section() as a value and
  // be recorded "empty" (honest absence) -- byte-identical to a genuinely quiet aggregate. On the two highest-
  // frequency ticket classes ("SSO is broken", "alerts stopped") that would mean a mid-outage snapshot reads
  // clean. The auth + notify posture gatherers instead let the fault PROPAGATE to section() (the same
  // bare-block pattern fetchSsoFailures and fetchNotifyHistory use), so the section reads "error". This drives
  // buildSupportBundle with a stub that throws on each endpoint and asserts the section vector reads "error",
  // never "empty".
  console.log("\nauth/notify gather faults read 'error', not a clean 'empty':");
  {
    ...
    const cases: Array<[string, string]> = [
      ...
      // audit / config / seal gatherers use a bare-block, not a try/catch that swallows the fault to {}.
      ["/audit/verify", "audit"],
    ...
    // The OFF-ROSTER blocks (called outside section(), so they cannot use the sections vector) each keep
    // their own catch, and flag a read fault instead of returning a silent {} that would be omitted from the
    // pack (byte-identical to honest absence). A fault makes the block PRESENT with an *Unavailable marker.
  ...
  console.log("\nthe pull routes (rejection and bound edges):");
  {
    ...
    // See the 401 case above for the same shape.
    ok("an unavailable grant store is a JSON 503 carrying error", unavailable.status === 503 && ((await unavailable.json()) as { error?: string }).error === "unavailable");

    // The audit feed CURSOR is strict: a present-but-non-numeric afterSeq is a 400, not a
    ...
    ok("a non-numeric afterSeq is a 400 (strict cursor)", badCursor.status === 400);
  ...
  console.log("\nthe keys health block (security-sensitive: public fingerprints + KCV only):");
  {
    const sched = schedulerDouble();
    ...
    // DERIVATION-IDENTITY PROOF: the pack's break-glass
  ...
  console.log("the fault ledgers are in the ROSTER, and a bundle built against a silent DO says so:");
  {
    // A gap is not closed until its evidence is IN THE BUNDLE: a recorder that writes a durable record on the
    // fault path is worthless if nothing reads it. The roster is the structural defence: buildSupportBundle
    // stamps any roster member MISSING from the produced map as "error", and this suite pins the produced keys
    // to the roster, so a gatherer that is added to the roster and never called (or called without a
    // section() wrapper) fails the gate rather than silently carrying nothing. The DEEP end-to-end +
    // redaction proof for these ten lives in validate-support-round4.ts.
    ...
    // A DO that answers every one of these routes with an empty record must produce an honestly EMPTY section and
    // OMIT the field entirely: the healthy fleet's steady state here is silence, not a wall of zeroes.
    ok("a clean fleet reports these sections \"empty\" and omits every field", round4.every((n) => sections[n] === "empty" && !(n in bundle)));
    // And the pack must still name the routes it read: a section that was never fetched is a silent hole.
    ok("every one of these DO routes was actually fetched during the build", ["/unwrap-faults", "/binding-alarms", "/update-faults", "/admin-refusals", "/integrity-faults", "/dispatch-faults", "/cron-health", "/dest-probe-faults", "/dest-build-health", "/cost-sizing"].every((r) => sched.calls.includes(`GET ${r}`)));
  }
