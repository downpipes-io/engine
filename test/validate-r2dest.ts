...
// A native R2Bucket binding fault carries no HTTP status and no S3 <Code> element (it is an
// in-process call, not a request over a wire), so coarseRunError -- which classifies a run failure by status/
// code/keyword found in the raw message -- must not lose the fact that a DESTINATION write failed at all. This
// drives the REAL code path: R2Destination.put against a binding double that throws the bare, unattributed
// error shape a live estate observed, then feeds the exact message that reaches the seal pipeline into
// coarseRunError, exactly as seal/slice.ts does on a failed run.
async function nativeFaultAttribution(): Promise<void> {
  ...
  // a native-binding fault writing the RUNLOG is a destination fault, not CAS contention, and an operator who
  // retries a contention message expecting it to clear will retry forever on a fault that never will.
