...
// captured + restored fully by the dry-run (engine-src-040-02) so a mere proposal leaves no side effect at
// tenant scale. Split out for size (engine-test-004-01); byte-identical, runs in order. The audit proof
...
export async function runAuditAndScale(ctx: Ctx): Promise<void> {
  ...
  // (engine-src-040-02). dryRunConfigMutation must checkpoint AND roll back over the WHOLE keyspace.
