...
function partA(): void {
  ...
  // WHAT IS ACCEPTED IS STORED WHOLE. normaliseChangeText used to cap BEFORE it trimmed, so leading
  // whitespace ate into the budget and a padded 64-character reference was accepted as legal and then stored
  // as 61 characters: the same silent-alteration defect, in a case the length rule alone does not catch.
...
async function partB(): Promise<void> {
  ...
  const { OWNER, OWNER2, call, doFetch, ownerCaller, readLog, setGate, listDestinations, destConfig, ownerActionKeyCount, ageOwnerAction } = ctx;
  ...
  try {
    ...
    // ITS OWN literal kind string rather than a sibling's -- a copy-paste/typo defect this campaign has hit
    // repeatedly (a fix landed at one call site, siblings missed). That is a WIRING question, not a
    ...
    //
    // WHAT THIS REPLACED. A 200-character change number returned 200 and put a 64-character TRUNCATION into
    // the immutable CR ledger as the operator's attested reference. It looked like a change number, it was
    // stored without complaint, and it matched nothing in the customer's change-management system, which is
