...
// and the clean approve that applies via the real path. Split out for size (engine-test-004-01); every
...
export async function runQueue(ctx: Ctx, shared: Shared): Promise<void> {
  const { ok, call, readLog, listDownpipes, pendingChanges, pendingKeyCount, dp, ageChange } = ctx;
  ...
  // ===========================================================================================
  {
    ...
    const renamed = canApproveChange(base, "new-name@acme.example", "https://team|sub-of-maker", true, now);
    ...
    const distinct = canApproveChange(base, "old-name@acme.example2", "https://team|sub-of-checker", true, now);
    ...
    const legacySelf = canApproveChange(legacy, "old-name@acme.example", "https://team|sub-of-anyone", true, now);
  ...
  // ===========================================================================================
  {
    ...
    ageChange(shared.queuedDownpipeId);
