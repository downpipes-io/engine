// DEAD-VOCABULARY PRODUCERS. The closed-vocabulary members that are DECLARED, documented, and
// have NO PRODUCER: nothing in the engine can ever put them in a pack. Each one is a promise the pack could
// not keep -- the vocabulary tells a support engineer "we will tell you when X happened" and an empty class
// reads as "X did not happen" rather than "nobody ever looked".
//
...
// asserts it comes back: that self-certifying shape would only prove the RING could carry the class, never
// that the PRODUCT could put it there, and a test that would still pass with the producer deleted proves
// nothing at all. So each case additionally asserts the
