...
// was taken. Cancelling the response body and returning ok:true for any 2xx would record a 200 carrying a
// NON-ZERO code as a clean delivery, advancing the cursor PAST audit events HEC had positively refused. Those
// events would never be re-sent: silent, permanent audit-log loss on the one feed a compliance record depends
// on. Discarding the body on a non-2xx would likewise lose the payload that says WHICH refusal it was, so
// deliverSiemPush reads the body in both cases rather than every HEC 400 collapsing to http-bad-request.
//
// This validator asserts four things:
//   1. a HEC error payload behind a 2xx is VISIBLE (a closed hec-declined-* class), not discarded;
...
//      failure -- it rides as an ok:true caveat, the same shape the OTLP push sender uses for its own
//      unreadable-answer case);
...
// channel plus an ack poll), which is costed and deliberately not built. This file pins the boundary of
// what the drain can honestly say.
//
// PRECEDENT: OTLP has the same shape (a 200 that reads as full success while partial_success says otherwise),
// closed the same way in notify/otlp-push-sender.ts and pinned by test/validate-destsim-otlp-partial.ts.
//
...
async function main(): Promise<void> {
  ...
  // ---- 1. A 2xx whose body declares a non-zero code must be visible AND must fail, so the cursor holds
  //         and the events are re-sent, rather than being recorded as a clean delivery.
  console.log("\na HEC error payload behind a 2xx is visible, and holds the cursor");
  {
    ...
    ok("200 + code 7: ok:false, so the cursor holds and those audit events are re-sent", r.ok === false);
  ...
  //         refusal: inventing a failure from silence would retry-storm a healthy sink. This is the same
