...
// (TC-N-15..TC-N-18b), split out of validate-notify.ts per finding engine-test-009-01. Covers the
...
import { verdictReached, isEntryPoint } from "./lib/verdict-guard.ts";

// ---- TC-N-15: notify channels/rules DO CRUD + notify.config re-check + default-on rule ----
// Split into focused sub-tests per finding engine-test-009-02: access control, channel CRUD (create,
...
async function testNotifyCrudAccessControl(): Promise<void> {
  ...
  // A viewer (no notify.config) is refused at the DO re-check (403: an authorisation refusal, 037-03).
...
// behave as documented. Split per finding engine-test-009-08.
async function testNotifyResolve(): Promise<void> {
  ...
  // R9 (parseEmission canonicalisation): an `at` posted in a NON-canonical precision/offset must be
  ...
  // admission is the guarantee under test here.)
  {
    ...
    ok("[FOLD1] resolve admits update-rollback-needed (not silently dropped)", body.emission !== null);
  }
}

// record + history: a delivery record appends one redaction-safe history entry newest-first; malformed
// records are skipped fail-open. Split per finding engine-test-009-08.
...
// ---- TC-N-18b: history ring cap holds under platform paging (engine-src-040-M1) -------------
...
export async function runCrud(): Promise<void> {
  ...
  console.log("history ring cap holds under platform paging (engine-src-040-M1)");
