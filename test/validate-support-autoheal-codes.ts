...
// in support-sections-recovery.ts), so the owner-#1 "did my control plane recover, and if not why" question
// is answerable from the sealed bundle. If that allowlist ever falls out of step with the canonical codes --
// missing "sealed-no-op-key" (the break-glass-only posture), say -- the exact recovery-blocked path a
// customer would raise a ticket about drops its reason silently, and the pack shows presence without a
// reason. The runtime tuple AUTO_HEAL_REFUSAL_CODES is the single source of truth (the type derives from it,
// the pack builds its Set from it). This test is the belt-and-braces runtime guard: it pins the pack's
// projected set to the canonical tuple, so any FUTURE refusal reason added in control-plane that a
// maintainer forgets to surface fails here rather than going invisible in production bundles.
//
// The projection code path (fetchRecoveryStatus in support-sections-recovery.ts) is also driven end to end
// against an in-memory DO stub to prove "sealed-no-op-key" reaches the bundle, and that an out-of-vocabulary
// code never rides as raw text (the redaction contract is preserved).
//
...
async function main(): Promise<void> {
  ...
  // 3. Redaction contract preserved: an out-of-vocabulary code is NEVER propagated as raw text. It collapses
  // to the fixed "unknown-code" placeholder rather than being dropped in silence -- a silent drop would leave
  // `refused:true` with no cause at all, indistinguishable from a refusal the pack simply had no code for.
  // The raw value never rides (that is the redaction guarantee), and the placeholder keeps the gap legible.
  ...
  // 4. The specific regression: sealed-no-op-key must be present in the bundle.
