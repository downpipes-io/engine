...
// /admin/push/test sends) through destsim's STRICT parsers -- the check that had never been run on it.
//
// WHY THIS EXISTS. buildSyntheticPushEvent's two hash placeholders were hand-typed runs of 94 zeros where
// the audit chain's prevHash/hash is 96 hex characters wide. Real Splunk HEC does not validate the digest,
// so it answered 200 and the wiring test read as clean. destsim's parser does validate it, so it answered
// 400. Two implementations disagreed about the same payload, which means every run of that path had one of
// them lying and nothing could say which. The width is now derived from ONE constant (SHA384_HEX_LEN in
// src/admin/audit-types.ts): the producer's placeholder is GENESIS_PREV_HASH, built from it, and destsim's
// accepting regex is built from it too.
//
// The prior coverage could not catch this. test/validate-siem-push.ts only asserted the synthetic event
// "shapes cleanly under every format (no throw)", and a shaper passes a hash through verbatim without
// looking at it, so a malformed hash cannot make a shaper throw. And no destsim test ever fed the synthetic
// event to a parser: validate-destsim-formats.ts round-trips its OWN matrix, whose helper is correctly 96
// wide, so the one payload the button actually emits was the one payload never round-tripped.
//
...
ok(`the synthetic event's prevHash is SHA384_HEX_LEN wide (was 94, needs ${SHA384_HEX_LEN})`, synthetic.prevHash.slice("sha384:".length).length === SHA384_HEX_LEN);
ok(`the synthetic event's hash is SHA384_HEX_LEN wide (was 94, needs ${SHA384_HEX_LEN})`, synthetic.hash.slice("sha384:".length).length === SHA384_HEX_LEN);
...
for (const wrongLen of [SHA384_HEX_LEN - 2, SHA384_HEX_LEN - 1, SHA384_HEX_LEN + 1]) {
  ...
  for (const rt of ROUND_TRIPS) {
    ...
    rejects(`${rt.format}: a ${wrongLen}-character hash is REJECTED (94 is the width that shipped)`, () => rt.parse(body));
  }
}
