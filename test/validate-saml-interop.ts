// Real-world-IdP interop + anti-regression corpus for the native SAML SP.
...
//
//   Response-level / envelope signature: Okta / Entra / OneLogin sign the samlp:Response ELEMENT, not
//     the inner Assertion. A Response-signed message (unsigned assertion) MUST verify - AND, the critical
//     anti-XSW binding, the consumed assertion MUST be the single direct child covered by the verified Response
//     signature, never an attacker-chosen one. The negative vectors prove the Response-level path opens NO
...
//
//   ec:InclusiveNamespaces PrefixList + inherited namespaces: an ADFS/Shibboleth-style assertion that
//     declares xsi:/xs: on the Assertion, uses xsi:type="xs:string" inside an AttributeValue, and signs with an
//     exc-c14n# Transform carrying <ec:InclusiveNamespaces PrefixList="xs"/> MUST verify. The negative twin:
...
//
//   Signing-cert validity window: a signature made under a cert whose validity window has ENDED (or has
...
// constants live in saml-interop-fixtures.ts; the suite is split into three group modules (response-level /
// inclusive-namespaces / cert-validity) that each export run(h, keys). This file is the THIN ORCHESTRATOR: it
// generates the key material once, builds the shared harness and calls each group in sequence, so running
// this one file still executes the full suite.
//
...
async function main(): Promise<void> {
  ...
  // ---- summary ----
  console.log("");
  verdictReached(state.fails.length);
