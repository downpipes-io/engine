// validate-dataloss-evidence: four DATA-LOSS support-pack gaps, each driven as a REAL fault,
...
//         and "the account is empty" would otherwise be the same observation everywhere; the recorder below
//         tells them apart.
//   THE FAILED AUDIT-LOG EXPORT. An export path that fails without recording anything leaves a customer who
//         cannot get their tamper-evident evidence OUT of the account with no trace: the pack would show an
//         intact chain, every event present, and nothing saying none of it could be produced.
...
//         product; retaining it by ACTION NAME alone would let a later benign dest clear evict it from the
//         excerpt, and 40 events of ordinary churn evict it from the recent window.
//   CONFIG-VERSION BODY RETENTION. The history lists headers; the snapshot you would roll back TO is a
//         separate body, and without a body-retention check, ten versions could list as restorable while you
//         find out which are real only in the middle of an incident.
//
// THE BAR: a gap is not closed until its evidence is in the BUNDLE. So no hop here is taken on trust. Every
// case drives the REAL fault site,
...
// The DO's own snapshot path, reached directly to seed config versions: it is the ONE production write path for a
...
async function g098(): Promise<void> {
  ...
  // 5. THE ROLLED-OVER ARM. The keys admitted to the closed set above are UNREACHABLE on the fixture that
  //    admits them, because that chain begins at version 1, so widening the allowlist alone would leave them
  //    untested by the very check that lets them in -- a gate whose allowlist grows without its coverage
  //    growing too. This drives the state they exist for.
  //
