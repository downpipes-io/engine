// Proves restorePlanHash (src/admin/approvals.ts) is REPRODUCIBLE for the same input and
...
async function main(): Promise<void> {
  ...
  // isUsableApproval can never find a usable approval for the mismatched apply (a destination-relative
  // isLatest cue reviewed against one copy must never authorise applying against another).
  ...
  // The surface allow-list is a decision field and re-arms like every other one. If the hash bound only
  // the account and zone, an approver who signed "re-apply Cloudflare config into this account" would
  // authorise whatever the in-band set happened to be, and widening it would silently widen an
