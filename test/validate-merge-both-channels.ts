// The merge proof: BOTH evidence channels ride on the one bundle.
//
// WHY THIS TEST EXISTS. Two branches independently changed the SAME third parameter of buildSupportBundle /
// signedSupportBundle / sealedSupportBundle:
//
//   support-pack-expansion   (env, scheduler, ctx: SupportBundleContext)                 // the D5 section context
//   support-clientdiag       (env, scheduler, clientDiagnostics?: ClientDiagnosticsSection) // the browser ring
//
// Git conflicts there, and EITHER careless resolution compiles cleanly while silently dropping a whole evidence
// channel: take one side and the browser evidence never reaches the bundle (every console gap quietly reopens),
// take the other and the Access-perimeter boolean goes. A type error would have been a kindness; instead the
// pack would simply have been missing a section that nobody was looking for.
//
// The resolution folds clientDiagnostics INTO SupportBundleContext, so there is one object and no positional
// collision to get wrong again. This test is what proves the resolution actually worked, and it is the check to
// re-run after any future merge that touches these signatures.
//
// It also pins the bug the merge itself surfaced: the clientdiag branch's POST /support/bundle passed ONLY the
// ring, so accessPerimeter was LOST on the one path a customer actually uses to generate a pack.
...
// is the whole point of this ring: a fixture covering only a subset of kinds exercises only that subset of
// the vocabulary and none of the fields the other kinds carry, which is exactly how fifteen kinds of browser
// evidence can silently drop and auto-post stay forbidden on every customer-generated pack, with a green suite
// on both sides. A pairing proof is only worth what its payload covers, so this payload covers the vocabulary.
//
...
const ring = {
  ...
  engineAttempts: 40,
  // consoleBuild: the browser's own build stamp. The console sets it on EVERY session, so a fixture without
  // it is not the bundle a customer sends -- and its absence here is exactly why the bot's pairing proof could stay
  // green while the bot rejected the key on every real pack and silently forbade auto-post.
  consoleBuild: "0.1.10",
  records: [
    ...
    // discriminator between a six-week silent signature failure and a channel host that is merely down.
    { kind: "update-channel-unverified", screen: "updates", channelReasonClass: "signature", count: 1, firstMs: 3800, lastMs: 3800 },
    // The POSTURE round: each of these is a live kind in the console's vocabulary, and a fixture without a
    // row for it would leave the bundle the bot's pairing proof reads never exercising it.
    ...
    // The POSTURE round, group 3. Every one of these is a browser-side fact this
...
async function main(): Promise<void> {
  ...
  // TypeScript checks nothing about it: a member that no console can send, or one deleted from the vocabulary, used
  // to ride into the dumped fixture unchallenged, and the bot then built its whole contract proof on a shape the
  // product cannot produce. (It happened: the fixture asserted handoffClass "wizard-account-lost" long after the
  // console stopped being able to send it, AND it carried no consoleBuild at all, which is why the bot's pairing
  // test stayed green while the bot rejected consoleBuild on every real customer pack.)
  //
  // projectClientDiagnostics IS the receiver the POST /support/bundle route runs. Putting the ring through it means
  // the fixture the bot pairs against is one THIS ENGINE actually validated and would actually sign, and a dead or
  // misspelled member now fails HERE, loudly, instead of becoming a green test over dead evidence.
  ...
  // And the D5 section context still composes: the sections roster must not have been lost in the resolution.
  // --dump <path> writes the BOTH-CHANNELS bundle, so the diagnostics bot can prove it ingests the one shape a
