...
async function main(): Promise<void> {
  ...
  console.log("\ndeliver: CREATE POSTs to the channel's OWN em_event url with HTTP Basic auth; body is the event shape; no-custody (password never in body/url)");
  {
    ...
    globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      if (String(input).startsWith("https://cloudflare-dns.com/dns-query?")) {
        // The send-time SSRF resolve screen. Answered with a PUBLIC address so this case still
        // exercises the delivery it is about rather than becoming an SSRF refusal, and NOT captured,
        // because these captures are the record of what was DELIVERED.
        resolveCalls++;
    ...
    // The sink name is resolve-screened before the POST (A and AAAA). Asserted rather than merely
    // tolerated, so this case also proves the SSRF screen runs on the ServiceNow path.
    ok("servicenow create: the sink was resolve-screened before the POST", resolveCalls === 2);
  ...
  console.log("\ndeliver: item 11 (endpoint tolerance) -- the Table API keeps the flat body; em/jsonv2 wraps it in {records:[...]}");
  {
    ...
    ok("item 11: /api/now/table/em_event keeps the FLAT body (no records wrapper)", tableBody === ev && !("records" in tableBody));
    ...
    ok("item 11: /api/global/em/jsonv2 wraps the event as {records:[event]}", Array.isArray(jsonv2Body.records) && jsonv2Body.records.length === 1 && jsonv2Body.records[0] === ev);
    ...
    ok("item 11: the em/jsonv2 match is case-insensitive", Array.isArray(jsonv2UpperBody.records) && jsonv2UpperBody.records[0] === ev);
    ...
    try {
      ...
      ok("item 11: deliver() to a jsonv2 endpoint reports ok:true", r.ok === true);
      const bodyDoc = JSON.parse(captures[0]?.body ?? "{}") as EmJsonV2Envelope;
      ok("item 11: deliver() actually POSTed the {records:[...]} envelope, not the flat body", Array.isArray(bodyDoc.records) && bodyDoc.records.length === 1 && bodyDoc.records[0]?.message_key === "downpipe:pipe-1:backup-failure");
    } finally {
      globalThis.fetch = realFetch;
    }
  }
  ...
  console.log("\ndeliver: fail-open on a network fault (non-2xx and unreachable); never throws; classified into the closed DeliveryFailCode vocabulary");
  {
    ...
    try {
      const r = await deliver(env, channel, EMISSION);
      // Item 9 (granular failure codes): 401/403 now classify as the distinct http-auth code, so an
