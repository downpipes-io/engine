...
// out of validate-notify.ts per finding engine-test-009-01. Covers the first-alert retry,
...
// This tests Fix 1: a NEWLY stale/failed downpipe whose first alert POST fails retries
...
async function testTwoPhaseRetry(): Promise<void> {
  ...
  // P3.2b: the reconcile answer carries NO delivery destination. The legacy single webhook used to ride
...
async function testRecoveryClears(): Promise<void> {
  ...
  // AUTO-RESOLVE (mon-autoresolve): the falling edge of a "stale" cooldown emits exactly one
