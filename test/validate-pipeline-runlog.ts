...
async function main(): Promise<void> {
  ...
  // ---- RUNLOG-1: a rapid re-trigger / crashed-run RECLAIM no longer signs a null-prev root ----
  ...
  // idx 2. Before the fix the signed root recorded prevRunId=null while the relinked RUNLOG entry
  // linked run A, so the strict reader rejected the intact run B as a rollback (rc=5). After the fix
  // the root is seeded from the destination-local RUNLOG (the same source the relink uses), so both
  // agree on run A and run B opens fresh WITHOUT allowStale.
  {
