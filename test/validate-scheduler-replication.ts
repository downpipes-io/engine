// reconcileReplicationAlerts vectors: replication-degraded transition + cooldown, run-at-risk-eviction at
// the ring cap (escalated to critical), no alert for a single-destination pipe, and the fully-replicated
// case where recovery clears the cooldown. Detail strings must stay secret-free.
import { ok, makeScheduler, stubFetch, makeConfig } from "./validate-scheduler-shared.ts";

export async function run(): Promise<void> {
  // ---- reconcileReplicationAlerts: replication-degraded transition + cooldown ----------
  ...
  // ---- reconcileReplicationAlerts: run-at-risk-eviction at the ring cap ---------------
  ...
  // ---- a single-destination downpipe never replication-alerts -------------------------
  ...
  // ---- a fully-replicated fan-out downpipe never alerts, and recovery clears the cooldown
  ...
  // recovering to fully-proven must CLEAR the cooldown so a future relapse re-alerts immediately.
  {
    ...
    // AUTO-RESOLVE (mon-autoresolve): the falling edge of the replication-degraded cooldown emits
  ...
  // ---- a run-at-risk-eviction recovery carries the ESCALATED (critical) severity through --
  ...
  // ---- the backup-success notification carries recordsIncomplete ----------
  ...
  // ---- a fully-captured run reports the plain success detail (0 markers) ----
  {
    const { stub, storage } = makeScheduler();
    await storage.put("notify-channel:ch-detect", { id: "ch-detect", kind: "webhook", name: "Sink", url: "https://sink.example.com/hook", enabled: true, createdAt: "2026-01-01T00:00:00.000Z" });
