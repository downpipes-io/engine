...
async function main(): Promise<void> {
  ...
  {
    // replicateBacklog catches each destination up on EVERY run it is missing, not just the
  ...
  console.log("-- 7. replication runs BEFORE retention so a trailing replica catches up before the primary prunes --");
  {
    ...
    // runRetentionPrunes in the tick. Reading the source order guards the real fix (red under the old order
    // where retention ran first), not just the in-memory model above.
    ...
    ok("drive() orders replication BEFORE retention (replicate-first)", replIdx >= 0 && retIdx >= 0 && replIdx < retIdx);
  }

  // ---- SM-FAILOVER (broad DO state-machine, failover half): fuzz a run->destination routing sequence with
  ...
  // (retention and prune are the other state-machine half; this is the failover routing half the tick asks for).
