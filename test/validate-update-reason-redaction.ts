// The update lifecycle's `reason` must NOT ride into the pack, because it is not engine-authored text.
//
// WHY. The update paths build the sentence as
//
...
//
// so the raw Cloudflare deploy-driver error, which can embed a URL, an account id or a script name, rides
...
async function main(): Promise<void> {
  ...
  //
  // A hand-built `throwingGate` whose flyNow() throws the platform error, asserting the resulting
  // "flight-errored" class, would prove nothing about the product: the real route (router-updates.ts) ALWAYS
  // passes makeHealthGate(...), and makeHealthGate's flyNow CANNOT THROW: it wraps its own scheduler fetch in
  // try/catch, and runCanaryCycle never throws by construction (every stage catches its own I/O and returns a
  // result). That class has ZERO production producers, so a green assertion built that way would prove only
  // that the PROJECTOR could carry it -- not that the PRODUCT could put it there. The class is gone, and this
  // drives the REAL gate instead.
  //
  // THE PLATFORM IS TOTALLY DEAD: every DO round trip throws the hostile Cloudflare error, driven honestly.
  ...
  ok("the REAL production gate does NOT throw on a totally dead platform (flight-errored has no producer)", !threw);
  ...
  // 4. THE DISCRIMINATION THAT MATTERS. An unreachable platform without this fix would project "pending", which
  // is exactly what a legitimately slow post-swap flight projects, so "nothing could be measured" and "wait and
  // settle again" would read as the same row with opposite remedies. They are two classes, and this row is
  // produced by the real gate on the real route.
  ...
  ok("...and it is NOT 'pending': the two no longer coalesce", trace.every((e) => e.step?.toString().startsWith("canary-flight") !== true || e.detailClass !== "pending"));
  ...
  // 5. THE NOISE HALF, DRIVEN BY THE REAL GATE. A hand-built `slowGate` whose flyNowMeasured returns
  // {status:"pending", measured:true}, asserting the projector emits "pending", would rest on a shape
  // makeHealthGate CANNOT return -- the same defect as the throwingGate above, on the claim's other leg: it
  // would prove the RING could carry "pending", not that the PRODUCT could put it there. Both
  // "pending" and "disabled" are now GONE from SETTLE_STEP_DETAIL_CLASSES because no production shape produces
  // them, and what follows is the assertion the real gate CAN prove in their place.
  //
  ...
  // which is the class reserved for the flight that resolved no destination and attempted nothing. This is the
  // discrimination the gap exists for, driven from the destination up.
  const brokenBucket = {
  ...
  ok("'pending' and 'disabled' are GONE from the closed vocabulary: no production shape produces either", !(SETTLE_STEP_DETAIL_CLASSES as readonly string[]).includes("pending") && !(SETTLE_STEP_DETAIL_CLASSES as readonly string[]).includes("disabled"));
  ...
  // The sibling of the settleTrace fix above: the OTHER free-text field on the SAME pending record, behind the
