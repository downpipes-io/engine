...
// runFailedEvents reads every "run-failed" audit event currently in the chain, for the
// transition-gate vectors below.
async function runFailedEvents(storage: Store): Promise<AuditEvent[]> {
  const all = [...(await storage.list<AuditEvent>({ prefix: AUDIT_PREFIX })).values()];
  return all.filter((e) => e.action === "run-failed");
}
// targetRunId reads the runId off a "run"-kind audit target, or undefined for any other target kind.
function targetRunId(e: AuditEvent): string | undefined {
  return e.target.kind === "run" ? e.target.runId : undefined;
}
...
async function main(): Promise<void> {
  ...
  // ============================================================================================
  // 8) run-failed audit action: the transition gate fires on isTransition &&
  //    toAlert === "failed", NOT a direct classify() read. A healthy->failed transition appends
  //    exactly one event; repeated failing ticks (same-state, within cooldown) append none; and
  //    a healthy->stale transition appends none at all.
  // ============================================================================================
  {
    const { storage, stub } = makeScheduler();
    await storage.put(`${NOTIFY_CHANNEL_PREFIX}ch-rf`, { id: "ch-rf", kind: "webhook", url: "https://hooks.example.com/rf" });

    // healthy -> failed: no prior cooldown, newest resolved run failed.
    await seedDp(storage, "dp-rf-fail");
    await storage.put("hist:dp-rf-fail", [{ runId: "rf-run-1", index: 1, startedAt: iso(NOW - 60_000), status: "failed" }] satisfies RunEntry[]);

    // healthy -> stale: no prior cooldown, last SUCCESSFUL run well past the cadence budget
    // (default cadenceSeconds 3600 * STALE_CADENCE_MULTIPLE), so classify -> "stale", never "failed".
    const staleBudgetMs = 3600 * 1000 * STALE_CADENCE_MULTIPLE;
    await seedDp(storage, "dp-rf-stale");
    await storage.put("hist:dp-rf-stale", [{ runId: "rf-run-2", index: 1, startedAt: iso(NOW - staleBudgetMs - 60_000), status: "ok" }] satisfies RunEntry[]);

    const r1 = await stubFetch(stub, "POST", "/reconcile-alerts");
    const b1 = (await r1.json()) as { alerts: Array<{ id: string; state: string }> };
    ok("run-failed: dp-rf-fail alerts failed on tick 1", find(b1.alerts, "dp-rf-fail")?.state === "failed");
    ok("run-failed: dp-rf-stale alerts stale (not failed) on tick 1", find(b1.alerts, "dp-rf-stale")?.state === "stale");

    const afterTick1 = await runFailedEvents(storage);
    ok("run-failed: healthy->failed appends exactly one run-failed event", afterTick1.filter((e) => targetRunId(e) === "rf-run-1").length === 1);
    ok("run-failed: healthy->stale appends NO run-failed event", afterTick1.some((e) => targetRunId(e) === "rf-run-2") === false);
    ok("run-failed: the appended event carries the failed run's id as target.runId", afterTick1.length === 1 && targetRunId(afterTick1[0]!) === "rf-run-1");
    ok("run-failed: the appended event is engine-observed, outcome failed", afterTick1.length === 1 && afterTick1[0]!.actorMethod === "engine" && afterTick1[0]!.actorEmail === null && afterTick1[0]!.outcome === "failed");

    // Tick 2, immediately after: dp-rf-fail is STILL failed, same state, well within ALERT_COOLDOWN_MS
    // -> shouldAlert re-fires (same-state cooldown not yet elapsed is suppressed entirely; still, even
    // if it re-fired, isTransition is false) so no NEW run-failed event is appended.
    const r2 = await stubFetch(stub, "POST", "/reconcile-alerts");
    void r2;
    const afterTick2 = await runFailedEvents(storage);
    ok("run-failed: a repeated failing tick appends no additional event", afterTick2.length === afterTick1.length);

    // Tick 3: dp-rf-fail relapses via a fresh failed run past the cooldown; simulate the cooldown having
    // elapsed AND a genuinely new failed run landing, which is a fresh transition worth a fresh event.
    await storage.put(`${ALERT_COOLDOWN_PREFIX}dp-rf-fail`, { state: "stale", at: NOW - 2 * ALERT_COOLDOWN_MS });
    await storage.put("hist:dp-rf-fail", [{ runId: "rf-run-3", index: 1, startedAt: iso(NOW - 5_000), status: "failed" }] satisfies RunEntry[]);
    await stubFetch(stub, "POST", "/reconcile-alerts");
    const afterTick3 = await runFailedEvents(storage);
    ok("run-failed: a fresh transition into failed (post stale-cooldown swap) appends a new event", afterTick3.some((e) => targetRunId(e) === "rf-run-3"));
    ok("run-failed: the tick-1 event is untouched (append-only)", afterTick3.some((e) => targetRunId(e) === "rf-run-1"));
  }
