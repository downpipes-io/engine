// Destination HTTP contract vectors for the S3 destination (engine-test-015-M1 split): the conditional
// 412 -> ok:false / get 404 -> null / exists surface, the https endpoint enforcement at construction
// (V12.3.1), the ENG-M14 absent-ETag-no-unconditional-overwrite fix, redirect:"manual" on every fetch
...
// so the default answer is a refusal and the seam is asserted separately, right below, so this is a
// discrimination rather than a blanket "http always throws".
function httpsEnforcement(): void {
  ...
  const makeSeamed = (endpoint: string) => () => new S3Destination(endpoint, BUCKET, CREDS.region, CREDS.accessKeyID, CREDS.secretKey, { now: FIXED_NOW, allowInternalEndpoint: true });
  ...
  ok("refuses http://localhost with NO test seam (it used to construct)", threw(make("http://localhost:9000")));
  ok("refuses http://127.0.0.1 with NO test seam (it used to construct)", threw(make("http://127.0.0.1:9000")));
  ok("refuses http://[::1] with NO test seam", threw(make("http://[::1]:9000")));
  // The seam still opens the loopback door for the emulator-driven validators, so the refusals above are the
  // absence of an opt-in and not a rule that http can never be spoken at all.
  ok("the allowInternalEndpoint seam still accepts http://localhost", !threw(makeSeamed("http://localhost:9000")));
  ok("the allowInternalEndpoint seam still accepts http://127.0.0.1", !threw(makeSeamed("http://127.0.0.1:9000")));
  // And the seam is a LOOPBACK seam, not an https bypass: a public http endpoint stays refused with it set,
  // so a validator cannot accidentally open cleartext to a real host.
  ok("the seam does NOT accept a public http:// endpoint", threw(makeSeamed("http://s3.example.com")));
...
// Part 5: ENG-M14 - empty or absent ETag from GET never degrades the RUNLOG conditional write
...
async function etagMissingNoUnconditionalOverwrite(): Promise<void> {
  console.log("ENG-M14 - absent ETag does not produce an unconditional overwrite:");
