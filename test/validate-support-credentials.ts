...
//
// Without these, the pack would carry only two expiry COUNTS (status.expiryWarnings / cleanupPending) and the
// audit-feed grant; the metrics-scope ingest credential and the per-row expiry registry would never ride, so
// "our metrics scraper stopped collecting" and "which credential is expiring / needs cleanup" would be
// undiagnosable. fetchMetricsFeed mirrors the audit-feed grant for the metrics scope; fetchExpiryRegistry
