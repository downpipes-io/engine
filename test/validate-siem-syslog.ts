// Validates the syslog-over-TLS delivery sink (notify/siem-syslog-sender.ts).
// cloudflare:sockets cannot run under Node, so the sender exposes
...
async function main(): Promise<void> {
  try {
    ...
    console.log("\nfailure classes (non-throwing; the cursor holds and the batch retries): connect, handshake, write");
    {
      ...
      ok("a synchronous connect throw is ok:false 'syslog-connect-refused' (split from a TLS-trust failure), never a throw", rConnect.ok === false && rConnect.code === "syslog-connect-refused");
    ...
    console.log("\nHARD TIMEOUT: a genuinely black-holed connection resolves ok:false 'timeout' WITHOUT depending on close() ever settling");
    {
      ...
      // if the bound depended on close() rejecting the in-flight await, this call would hang forever.
      // opts.timeoutMs is a tiny override so the test proves the race resolves promptly rather than
      // waiting out the real 5s WEBHOOK_TIMEOUT_MS production bound.
    ...
    // ---- THE FORMAT ON THE WIRE IS THE FORMAT THAT IS CONFIGURED -------------------------------------
    // buildSyslogRecord must not fall back to `format === "leef" ? shapeLeef(...) : shapeCef(...)` for every
    // other format: a destination configured splunk-hec over syslog-tls that fell into that CEF arm would pass
    // the router's validator, pass the DO's, open a socket and DELIVER ARCSIGHT CEF, reporting ok:true, with
    // no counter, no trail reason and no audit event recording that the operator's chosen format was
    // substituted. That is why these assertions are on the IDENTITY of the bytes and on the absence of any
    // write, not on whether a send happened: "it sent something" is not enough to catch CEF shipping under a
    // Splunk label.
    //
    ...
    console.log("\nformat identity: a format this sink cannot carry is REFUSED, never silently shipped as CEF");
    {
      ...
      for (const format of notSyslog) {
        ...
        // The identity assertion here catches exactly this: nothing resembling a CEF line reaches the
