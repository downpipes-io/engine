...
// only, no escalation, no Owner-touching) and the router-trusted `method` axis at the DO. These
// proofs are STATEFUL over the shared scheduler in `ctx` (they follow PROOF 9c and assume the
// OWNER/OWNER2 roster the matrix proofs left) and run in order.
...
export async function runAccessAdmin(ctx: Ctx): Promise<void> {
  ...
  // by PROOF 1 (unchanged).
  {
    ...
    ok("the refusal is the generic forbidden body, leaking no capability name (037-03)", goperBody.error === "forbidden" && !/downpipe\.write/.test(JSON.stringify(goperBody)) && !/do not hold/.test(JSON.stringify(goperBody)));
    ...
    ok("the refusal is the generic forbidden body, not naming the owner-escalation reason (037-03)", goBody.error === "forbidden" && !/only an Owner/i.test(JSON.stringify(goBody)));
    ...
    ok("the demote refusal is the generic forbidden body, not naming the owner-escalation reason (037-03)", doBody.error === "forbidden" && !/only an Owner/i.test(JSON.stringify(doBody)));
    ...
    ok("the delete-owner refusal is the generic forbidden body, not naming the owner-escalation reason (037-03)", delOwnerBody.error === "forbidden" && !/only an Owner/i.test(JSON.stringify(delOwnerBody)));
    ...
    // ASVS V2.4.2 (approval-dwell): an approve inside APPROVAL_MIN_DWELL_MS of the propose is refused
    // 429 too-soon, on the reasoning that no checker could genuinely have reviewed it that fast. This
    // proof's whole point is the DISTINCT-approver identity/authority check, not the dwell timer, so the
    // record is backdated past the floor directly in storage (the same technique
    // test/validate-approval-dwell.ts uses), rather than the proof either sleeping real time or the dwell
    // floor being weakened for this one route.
    const ownerMintRecord = await sched.storage.get<PendingConfigChange>(changeKey(ownerMintId));
    if (ownerMintRecord) {
      await sched.storage.put(changeKey(ownerMintId), { ...ownerMintRecord, proposedAt: new Date(Date.now() - APPROVAL_MIN_DWELL_MS - 1_000).toISOString() });
    }
  ...
  // ---- requireNotOwnerEscalation on the SESSION/CREDENTIAL kill paths ------------------
  // The role-TABLE escalation guard is proven above (PROOF 10c/10d). This proof closes the SIBLING surfaces a
  ...
  // DO's owner-escalation guard, not a router gate.
  {
    ...
    ok("precondition: OWNER is an Owner (the protected target)", ownerRole === "owner");
    ...
    ok("an access-admin CAN terminate a non-owner member's sessions (200, roles.write)", termNonOwner.status === 200);
    ...
    ok("an access-admin CANNOT terminate an Owner's sessions (refused 403)", termOwner.status === 403 && termOwnerBody.error === "forbidden");

    // ===== terminate-user ANSWERS ok:true FOR AN ADDRESS NO ROSTER ROW HOLDS. THAT IS DELIBERATE. =====
    //
    // This is pinned here so a later reader does not "fix" it into an outage.
    //
    ...
    ok("terminate-user answers 200 ok:true for an address no roster row holds (DELIBERATE, see the comment)", termNobody.status === 200 && ((await termNobody.json()) as { ok?: boolean }).ok === true);
    ...
    ok("an UNPARSEABLE address is still refused, so the ok:true above is about membership, not about validation", termJunk.status !== 200);
    ...
    ok("an access-admin CANNOT revoke an Owner's passkey credential (refused 403)", revOwner.status === 403 && revOwnerBody.error === "forbidden");
    const survived = await sched.storage.get(`${PASSKEY_CRED_PREFIX}${ownerCredId}`);
    ok("the refused revoke left the Owner's credential intact (Owner not locked out of the passkey path)", survived !== undefined);
