// Confirms the support pack's canary section carries the WHY of a death.
//
// fetchCanaryHealth carries the aggregate liveness, the transition ring (a bird flipped dead at T), the
// coarse deadReason, which flight aspect failed, the stray byte-delta, and whether the schedule is still
// armed, so the console never has to fabricate those fields for the customer. The DO's lightweight
// /canary/transitions view also returns nextRunAt + a per-dead-destination deadDetail derived from each
// destination's lastCheck, and the pack projects it redaction-safely. This test drives the real
// fetchCanaryHealth through a DO double and asserts the detail rides, the failed-aspect list is gated to
// the closed 8-member vocabulary, reasonAbsent flags a reason-less death, scheduleArmed is derived, and no
// coarse detail string leaks a value.
//
