...
//
// pathToFileURL changes the SCHEME, not the PATH. Node resolves a module to its realpath before it sets
// import.meta.url, while argv[1] stays exactly as the caller typed it, so reached through a symlinked
// path the two strings differ, the guard concludes it is not the entry point, and the script stamps
// nothing and exits 0.
//
// WHY THAT MATTERS MORE HERE THAN ELSEWHERE. scripts/deploy.sh line 41 runs it as:
//
...
// the exact message written to cover it: the deploy ships an unstamped engine and says nothing.
//
// SILENCE WAS THE DEFECT, so an exit-code-only assertion cannot see it. Both the broken and the fixed
// script exit 0 on the symlink path; only the OUTPUT tells them apart. Every case below asserts on
// captured stdout, and case 5 re-evaluates the retired expression against the same argv/meta pair to
// prove the fixed cases are not passing vacuously.
//
// MEASURED EXPOSURE, recorded here because the fix's value is easy to overstate. deploy.sh does
// `cd "$(dirname "$0")/.."` and then invokes a RELATIVE path, and process.cwd() is PHYSICAL, so the
// resolved argv[1] is already canonical and the retired guard fired correctly on that route even
// through a symlinked checkout. The live deploy path was NOT losing its stamp. What the retired guard
// could not survive is any absolute path through a symlink, or a relative path carrying a symlink
// component, either of which is one edit to how deploy.sh spells the invocation away.
//
...
try {
  ...
  // ---- 6. the source no longer carries the retired shape ------------------------------------------
  // Checked AT the comparison, not file-wide. A file-wide /realpathSync/ test passes the moment the
  // file imports realpathSync from node:fs, which this one does on line 53, so it would clear a
  // retired comparison sitting 150 lines below the import. scripts/verdict-guard-gate.mjs was briefly
  // written that exact way and reported a clean pass over the retired expression; this mirrors the
  // statement-window rule it now uses.
  {
