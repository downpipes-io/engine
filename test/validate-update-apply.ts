...
// Fixtures bundles the shared builders/values so each scenario group receives them as a single
// parameter: the same artefact, hash and input builders, run in the same order.
interface Fixtures {
  artefact: Uint8Array;
  goodHash: string;
...
async function testDecideKeepAndCoreApply(fx: Fixtures): Promise<void> {
  const { input } = fx;
  // ---- decideKeep: the OPTIMISTIC gate rule, every branch. A verified atomic promote is KEPT unless
  // there is POSITIVE evidence the build is bad -- and the ONLY such evidence is a DEAD canary (a strayed
  // byte on a COMPLETED flight, a data-integrity failure). A canary that cannot COMPLETE (pending/ailing)
  // is the post-swap DO-reset window, NOT a bad build, so it KEEPS (confirmation pending) regardless of
  // the self-check; the hourly canary confirms from clear of the window. The self-check does not gate --
  // it is recorded for diagnosis and shapes the reason only. ----
  ...
  // ---- engineRollbackSatisfiesConsoleFloor: the conditional-bundling rollback
  ...
  ok("no floor (undefined) -> satisfied regardless of the target's shape", engineRollbackSatisfiesConsoleFloor("0.5.0", undefined) === true);
  ok("no floor (blank string) -> satisfied", engineRollbackSatisfiesConsoleFloor("0.5.0", "") === true);
  ok("no floor -> satisfied even when the target itself is unparseable", engineRollbackSatisfiesConsoleFloor("not-a-version", undefined) === true);
  ok("target strictly above the floor -> satisfied", engineRollbackSatisfiesConsoleFloor("0.5.0", "0.3.0") === true);
  ok("target exactly equal to the floor -> satisfied", engineRollbackSatisfiesConsoleFloor("0.5.0", "0.5.0") === true);
  ok("target strictly below the floor -> VIOLATED (does not satisfy)", engineRollbackSatisfiesConsoleFloor("0.2.0", "0.5.0") === false);
  ok("an unparseable target vs a set floor fails CLOSED (does not satisfy)", engineRollbackSatisfiesConsoleFloor("not-a-version", "0.1.0") === false);
  ok("a genuine Cloudflare version id (the realistic rollback target) vs a set floor fails CLOSED", engineRollbackSatisfiesConsoleFloor("017e156b-2b34-4a3e-b3a4-9a5f5a5f5a5f", "0.1.0") === false);
  ok("an unparseable FLOOR vs a parseable target also fails CLOSED", engineRollbackSatisfiesConsoleFloor("0.5.0", "not-a-version") === false);
  ...
  // ---- 2c. FORWARD-ONLY GUARD: a correctly-signed OLDER channel must NOT silently downgrade ----
  {
  ...
  // ---- 5. HAPPY PATH: alive canary -> applied (promote only, no rollback) ----
  {
    ...
    // a keep the CANARY ITSELF confirmed (verdict alive) carries NO confirmationPending -- the
    // canary sang, there is nothing left to confirm in the background.
    ok("a canary-confirmed keep carries no confirmationPending", r.confirmationPending === undefined);
  }
  ...
  // ---- 7. ALIVE BASELINE + AILING VERDICT: OPTIMISTIC keep -- a flight that cannot complete in the
  // post-swap window is not evidence of a bad build; only a DEAD canary is, so a non-dead verdict KEEPS
  // whatever the self-check says, and the hourly canary confirms. ----
  {
    const d = new FakeDriver();
    ...
    ok("an optimistic keep is confirmation-pending", r.confirmationPending === true);
  }
  {
    const d = new FakeDriver();
    ...
    // a keep decided while the canary never sang is honest-but-provisional.
    ok("a self-check-decided keep sets confirmationPending:true", r.confirmationPending === true);
  }
  ...
  // ---- 8. NO-DESTINATION (pending baseline + pending verdict): OPTIMISTIC keep, either self-check ----
  {
    ...
    ok("a self-check-decided keep (no destination) also sets confirmationPending:true", r.confirmationPending === true);
  }
  {
    const d = new FakeDriver();
    ...
    ok("the optimistic no-destination keep is confirmation-pending", r.confirmationPending === true);
  }
  ...
  // rolls back. ----
  {
    ...
    ok("the flight-error keep is confirmation-pending", r.confirmationPending === true);
  }

  // ---- 12. ROLLBACK fails -> outcome 'rollback-failed', reason flags manual action + data safe ----
  //
  // The outcome enum must say rollback-failed, never rolled-back, when a rollback deploy fails and the
  // engine is still serving the dead build. Every consumer keys off this enum (the support pack's
  // updates.last.outcome, the console badge, the notify rules, the diagnostics bot's
  // update-failed-or-rollback-needed signal), and none of them read the prose `reason` field, so a wrong
  // enum would tell all of them the exact opposite of reality.
  {
  ...
  // the rollback target. currentLiveVersionId() only ever returns the DOMINANT slice: a second apply
  // reaching planAndPromote while the account is split would otherwise silently record the just-ramped,
  // not-yet-trusted release as its OWN rollback target, so a later unhealthy verdict's auto-rollback would
  // redeploy the SAME untrusted build while reporting "rolled back, nothing was lost". Modelled via
  // liveSlices, the same fake the standalone-rollback split tests use, so currentLiveVersions() reports
  // two slices while currentLiveVersionId() would (wrongly) report only one. ----
  {
  ...
  // ---- an UNCONDITIONAL split-refusal permanently disables the module's own documented completion step
  // ("Promote it to 100% with Update now", console/src/screens/licence/shared.ts) for every ramp that
  // ever settles "applied": settling one does NOT collapse the split (settleAfterRamp's applied branch
  // makes no driver call at all), so the SAME split is still live on the very next apply, and nothing else
  // in this codebase ever collapses it. The router resolves the persisted lifecycle's own recorded
  // fromVersion/toVersion (resolveTrustedRampTarget, router-updates.ts, the SAME fact resolveRollbackTarget
  // already trusts for the standalone rollback route) and threads it through as
  ...
  // EXACTLY that pair, so the documented flow can complete while an unrecognised split still refuses. ----
  {
    ...
    // but this apply carries the router's own resolved known-good (as if a prior ramp settled "applied"
    // with fromVersion=OLD_ID, toVersion=NEW_ID). It collapses the split by promoting NEW_ID to 100%,
    // exactly like the documented "Update now" completion step.
    const d = new FakeDriver({ liveSlices: [{ versionId: NEW_ID, percentage: 60 }, { versionId: OLD_ID, percentage: 40 }] });
    ...
    ok("a RECOGNISED split (the router's own trusted ramp record) -> the finish-the-ramp apply succeeds", r.outcome === "applied");
    ok("the TRUSTED fromVersion is recorded, never re-guessed from the dominant slice", r.fromVersion === OLD_ID && r.toVersion === NEW_ID);
    ok("it genuinely collapses the split (uploads + promotes NEW_ID to 100%)", d.uploadCalls === 1 && d.deployCalls.length === 1 && d.deployCalls[0] === NEW_ID);
  }
  {
    ...
    ok("an UNRECOGNISED split (a version neither known endpoint names) -> STILL refused", r.outcome === "refused" && d.uploadCalls === 0 && d.deployCalls.length === 0);
    ok("unrecognised-split refusal still names the split", /split/i.test(r.reason ?? ""));
  }
...
// real traffic.
//
// startGradualRamp is PHASE 1 ONLY -- upload + traffic-shift, nothing else. It does not take a
// HealthGate (a request-boundary-shaped proof, not just a runtime spy: there is no `gate` in scope
// inside this function or rampUploadAndGate for a canary flight to be flown against, so it is
// structurally impossible for this call to invoke flyNow() before rampVersion's promise resolves in the
// same call -- the compiler flags a missing parameter if that ever changes). The canary-gate +
// auto-rollback logic lives in settleAfterRamp (phase 2, tested in testRampSettle below), which a
// genuinely separate later call must invoke.
// =====================================================================================================
async function testGradualRamp(fx: Fixtures): Promise<void> {
  ...
  {
    // The forward-only guard applies to the ramp too; an older recommended is refused before any
  ...
  {
    // the ramp shares verifyAndGuard with the atomic apply, so the same split-check applies here
...
// =====================================================================================================
// PHASE 2: settleAfterRamp. This is what actually canary-gates a ramp, invoked by a genuinely separate
// later call (in production, a fresh HTTP request Cloudflare routes independently of the one that
// ramped, giving it a real `percentage`% chance of landing on the ramped slice). Proves: applied
// (genuine hit), inconclusive (a keep-looking verdict that did NOT land on the ramped code -- the exact
// false-assurance this guards against), and every rolled-back arm.
// =====================================================================================================
async function testRampSettle(): Promise<void> {
  ...
  {
    // ailing/pending + not a healthy baseline + self-check FAILS: decideKeep is OPTIMISTIC and keeps any
    // non-dead verdict regardless of the self-check, so this is not decideKeep itself saying rollback -- it
    // falls to settleAfterRamp's OWN self-identity gate instead (this settle's ONLY proof it actually ran
    // the ramped code), which a failed self-check fails just as it does on an alive-but-unidentified
    // verdict: inconclusive, never a rollback a failed self-check alone cannot justify. No deploy call at
    // all (the ramp is left exactly as-is).
    const d = new FakeDriver();
  ...
  {
    // A non-singing verdict AND the rollback-to-100% deploy itself FAILS -> "rollback-failed-still-split".
    // The outcome must say this, never "rolled-back", while a LIVE TRAFFIC SPLIT is still routing a share
    // of real customer requests to the suspect version. The ramp's entire purpose is to bound blast radius,
    // and this is the one outcome that says the blast radius is UNBOUNDED.
    const d = new FakeDriver({ rollbackFails: true });
    const g = new FakeGate({ verdict: "dead" });
    const r = await settleAfterRamp(d, g, settleArgs());
...
async function testErrorAndFallbackPaths(fx: Fixtures): Promise<void> {
  ...
  // Phase 1 does not take a gate at all (it cannot fly a canary mid-ramp, see the function header
  // above); the canary-flight-error-mid-ramp coverage lives in testRampSettle (settleAfterRamp's own
  // "canary flight error -> folded to ailing -> rolled back" and "dead + rollback deploy fails" cases).
}
...
async function testAntiRollbackFloor(fx: Fixtures): Promise<void> {
  ...
  // A target BELOW the settled high-water mark is REFUSED even with allowDowngrade, nothing uploaded.
  {
    const d = new FakeDriver();
  ...
  // A FORWARD-from-running target that is still below the HWM is refused INDEPENDENT of allowDowngrade
  ...
  // A target EQUAL to the HWM is allowed (a re-pin of the highest settled version still applies).
  {
    const d = new FakeDriver();
  ...
  // An UNCOMPARABLE target vs a set floor is refused (fail-closed: cannot prove it is at/above the floor).
  {
    const d = new FakeDriver();
  ...
  // Happy path: a forward update ABOVE the HWM is unaffected, it applies exactly as before.
  {
    const d = new FakeDriver();
  ...
  // No floor: an absent HWM leaves the apply unchanged (first-ever apply, nothing settled yet).
  {
    const d = new FakeDriver();
  ...
  // On the RAMP path: below-HWM and uncomparable-vs-HWM both refuse before any traffic shift.
  {
  ...
  // Invariant: a migration release is NEVER silently applied, allowDowngrade cannot bypass the migration
...
async function testReadbackGate(fx: Fixtures): Promise<void> {
  ...
  // warn + identical bytes -> verified, promoted, digest recorded.
  ...
  // warn + DIFFERENT bytes -> mismatch recorded, still promoted (warn never blocks).
  ...
  // enforce + identical bytes -> promoted (the gate passes and the verdict rides the result).
  ...
  // enforce + DIFFERENT bytes -> REFUSED before promotion; deployVersion never called.
  ...
  // enforce + driver without read-back support -> refused (fail-closed on unverifiable).
  ...
  // enforce + read-back endpoint error -> refused as unavailable (never a throw).
  ...
  // warn + endpoint error -> promoted, unavailable recorded (the honest non-answer).
  ...
  // off -> the driver is never asked and no verdict is recorded.
  ...
  // absent mode defaults to warn (the safe default records evidence on every apply).
  ...
  // a dry-run never reads back (nothing was uploaded to read).
