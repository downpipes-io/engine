...
//
// WHAT WAS WRONG. 180 files sat under test/vectors/*/archive/_RECOVERY/downpipe/0.1.0/ and no gate in this
// repository opened any of them. Appending one byte to a vector's FORMAT.md left validate-reader.ts,
// validate-bundle-verify-hostile.ts and validate-format-prims.ts all at exit 0. The bundle CODE PATH was
// well covered the whole time (validate-format-prims.ts writes a bundle, verifies it, and asserts a
// one-byte tamper of FORMAT.md fails; validate-bundle-verify-hostile.ts attacks the parser with synthesised
// bundles), so this is not a gap in verifyBundle. It is a gap in the CORPUS: shipped, signed,
// version-labelled artefacts that no assertion ever read. A file nothing reads produces the same green as a
// file that passes.
//
// NOT THE SAME FINDING AS THE CI ONE IT WAS FOUND BESIDE. engine 63379acb removed "**.md" and "docs/**" from
// ci.yml's paths-ignore, and test/validate-ci-path-shadow.ts holds that closed. That fix does not touch
// this. A gate CI skips can be restored by fixing CI; a gate that does not exist cannot.
//
// WHY THESE FILES SPECIFICALLY. FORMAT.md and RECOVER.md are the in-bucket recovery instructions, the only
// guidance a recoverer holding nothing but a bucket has, and the vectors are the reference corpus a
// clean-room re-implementer is pointed at.
//
...
//      still exist and still name it. run-gate-chain.mjs's own header records why that matters, from the
//      sibling repo where a declared entry read plausibly for a day after its stated cause stopped being
//      true, because nothing re-reads prose.
//
...
const NO_ARCHIVE: Array<{ dir: string; consumer: string; why: string }> = [
  ...
  { dir: "spec", consumer: "test/validate-format-constants.ts", why: "the Go reference spec package, vendored byte-for-byte and pinned by SHA-384, not an archive" },
];
...
// Control one: the tamper the finding was measured with, one byte appended to FORMAT.md. The signature over
