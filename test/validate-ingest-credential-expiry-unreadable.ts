// THE BEARER THAT COULD NEVER EXPIRE, AND THE FOUR READERS THAT AGREE WITH IT.
//
// WHAT THIS PINS. A stored ingest grant's expiry must never be read with the idiom
// `Date.now() > Date.parse(grant.expiresAt)`: Date.parse answers NaN for anything it cannot read, and every
// comparison with NaN is false, so an unparseable expiresAt would read as NOT EXPIRED at all four sites that
// consult it:
//   1. classifyIngestCredentialCheck (the GATE for /support/diagnostics, /support/audit-feed and /metrics --
//      the credential family presented by an UNAUTHENTICATED caller from the public internet) would authenticate it;
//   2. redactGrant (the console's supportability view) would report `expired: false`;
//   3. fetchAuditFeedState (the support pack's SIEM-feed block) would write `expired: false`;
//   4. fetchMetricsFeed (the pack's scrape block) would write `expired: false`.
// A credential that can never expire, with no surface anywhere to disagree, contradicts the mint comment's
// promise that the longest-lived scope is "still expiring and re-mintable, never permanent".
//
// QUESTION ONE IS ANSWERED HERE, NOT ASSERTED. Section W drives the ONLY two writers of the stored record --
...
// produce an unparseable value. The producer of a corrupt record is a corrupted write, a half-flushed page or
// a hand-edited record, the same premise the same expiry-reading idiom carries wherever it recurs
// (approvals.ts, prune-approvals.ts, owner-action.ts, expiry.ts).
//
...
// "unreadable" into either "expired" or "live" is how the idiom above goes wrong; the two suppressions later
// in section F that collapse the third state each way must fail this test.
//
...
// measuring nothing at all -- a failure mode that can look like a working rig. Section A proves the bridge
// before any zero below is trusted.
...
/** THE PRE TREE: the exact expression this file guards against, quoted once. */
...
async function main(): Promise<void> {
  ...
  console.log("A -- known positives (the rig reaches the code, both trees)");
  {
    ...
    // THE INSTRUMENT CONTROL for every section below: the ORDINARY expiry class must still book correctly on
    // the same rig in the same session. If this is missing, every absence below is a dead ledger rather than
    // a suppressed booking.
  ...
  const doses: Dose[] = [
    ...
    // stable instants: they are in the past and stay there.
  ...
  console.log("\nC -- the four readers share ONE expression");
  {
    ...
    // CODE ONLY. A naive census would also match this file's own comment above, which quotes the removed
    // idiom verbatim so the next reader knows what to avoid; a scanner that reads commentary as code
  ...
  //    would repeat a familiar defect shape: a class decided and discarded.
  // ------------------------------------------------------------------------------------------------------
  ...
  console.log("\nF -- two-sided: the suppressed directions, with the instrument control passing in each");
  {
    ...
    const sups: Sup[] = [
      // THE UNREPAIRED TREE: the NaN-blind comparison, applied at every site.
      { name: "unrepaired (the raw Date.parse comparison)", apply: (v) => ({ gate: preExpired(v) ? "credential-expired" : "ok", expired: preExpired(v), unreadable: false }) },
      // COLLAPSE INTO EXPIRED: NaN-safe, but the third state is thrown into the first. This is a common
      // failure shape, and it loses the "your stored record is corrupt" ticket.
      { name: "collapse unreadable INTO expired", apply: (v) => ({ gate: truth(v).expired ? "credential-expired" : "ok", expired: truth(v).expired, unreadable: false }) },
      // COLLAPSE INTO LIVE: the original defect, restated as a deliberate choice.
      { name: "collapse unreadable INTO live", apply: (v) => ({ gate: grantExpiryState(v).expired ? "credential-expired" : "ok", expired: grantExpiryState(v).expired, unreadable: false }) },
      // OVER-FIX: a HEALTHY grant refused as unreadable. This needs live subjects in the loop to be seen at
      // all -- without them an over-fix direction can fail 0 of 5 and read as green.
      { name: "over-fix (a live grant refused as unreadable)", apply: () => ({ gate: "credential-expiry-unreadable", expired: true, unreadable: true }) },
      // INVERTED SURFACE: the class is decided and the surface still says the credential is fine.
      { name: "inverted surface (unreadable, but expired:false)", apply: (v) => ({ ...truth(v), expired: truth(v).unreadable ? false : truth(v).expired }) },
    ];
    for (const s of sups) {
      let sFail = 0;
      let controlOk = 0;
  ...
  //    from the pack in silence. This section guards against exactly that silent-drop shape.
  // ------------------------------------------------------------------------------------------------------
  console.log("\nG -- the /metrics scrape half, through its own pack projection");
  {
    ...
    ok("G3 an UNREADABLE expiry refuses the scrape 401 (never renders the whole fleet as live forever)", corrupt.status === 401);
