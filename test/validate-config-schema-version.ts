...
// DownpipeState (the dp:<id> record in the SchedulerDO). Without a version field and a read-time
// validator on this surface, a future engine that changed a load-bearing config field could read an
// old/new-shape record back as undefined and seal the WRONG/EMPTY source set while still reporting ok
// (a silent partial). The guard mirrors the seal CHECKPOINT's v:1 fail-closed gate and the
// migrateCanaryState read-time-migration pattern. Proven SERVER-SIDE with the shared in-memory storage
// double only (no network, no deploy):
...
async function main(): Promise<void> {
  ...
  // A PRESENT-but-non-number version stamp is REFUSED, not accepted. A corrupt or hand-edited stamp (e.g.
  // the string "2") would otherwise DEFEAT the guard silently and let this older code read new-shape state.
