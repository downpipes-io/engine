...
// buildAdapter used to drop SecretBindingSpec.storeId when constructing the BoundSecret (it set only
// name/get/bindingVar), so the crawl's `if (s.store !== undefined)` never fired and every secret archived with
// no store id -- indistinguishable stores + an unreconstructable re-attach. This wires storeId through.
