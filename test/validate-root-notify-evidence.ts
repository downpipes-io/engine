// Validates the SUPPORT-PACK fault evidence the Worker ENTRY and the EMAIL channel record. A real fault at
// either site is otherwise either swallowed whole or coarsened into a code that names the WRONG owner:
//
//   The manual (run-now) dispatch resolves a pre-seal failure by POSTing a FAILED /complete, which frees
//   the in-flight lease and records the cause. If the scheduler DO is unavailable when that post fires
//   (usually the very reason the dispatch failed), the completion would vanish silently, the downpipe would
//   dangle in flight until the 30-minute lease expires, and the pack would show a STALL with no cause at
//   all -- byte-identical to a genuinely wedged seal. Instead the loss is CHECKED: it is noted in the
//   isolate-local pending tally and flushed to the DO's bounded droppedWrites aggregate the moment the DO
//   comes back, which is exactly when the pack is generated.
//
//   Every email non-delivery outside the three named arms would otherwise collapse to `email-rejected`, so
//   an ENGINE formatting bug (a subject/body that fails the send primitive's own bounds) would read
//   identically to the email PLATFORM refusing the send (an un-onboarded sending domain) -- opposite
//   owners, opposite fixes. A platform error code that does not match the documented E_UPPER_SNAKE shape is
//   never dropped either: it is recorded as the closed E_OTHER placeholder, so "the platform refused us and
//   told us why" stays distinguishable from "the platform refused us silently".
//
...
async function g224(): Promise<void> {
  ...
  // This is exactly that state: the dispatch fails because the DO is flapping, so the post that would
  ...
  ok("a THROWN completion post is counted (never swallowed by a .catch(() => {}))", pendingThrow["run-completion"] === 1);
...
// emailEnv builds an env whose send_email binding REJECTS the send with a platform error. `code` is what
// the platform puts on the exception (conforming or not).
...
async function g236(): Promise<void> {
  ...
  ok("a subject that failed the send primitive's bounds is named, distinct from the residual email-rejected", emailReasonToCode("email-subject-invalid") === "email-subject-invalid");
  ok("a body that failed the send primitive's bounds is named, distinct from the residual email-rejected", emailReasonToCode("email-body-invalid") === "email-body-invalid");
  ok("the PLATFORM refusing the send is named, distinct from the residual email-rejected", emailReasonToCode("email-send-failed") === "email-platform-rejected");
  ...
  // The send primitive genuinely produces the two engine-side reasons the split names, so these arms are
  ...
  // The customer's Workers Logs are durable too, so an unbounded platform string logged raw would carry a
  // whole sentence, including the recipient address or an internal host. The log carries the shape-gated
  // token (or the E_OTHER placeholder) only.
  const lines: string[] = [];
