...
//     because Cloudflare had not been called yet. Three wrong facts in one row, in the owner's #1 fear domain.
//
...
async function testChannelFaults(): Promise<void> {
  ...
  {
    const art = await loadVerifiedChannel({ UPDATE_CHANNEL_URL: "https://update.downpipes.io/channel.json", UPDATE_SIGNER_PUBLIC: "not-a-key" } as unknown as Env, never);
