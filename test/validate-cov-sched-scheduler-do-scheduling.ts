...
async function main(): Promise<void> {
  ...
  // ===== BLOCK 7: recordReplicationState - no-id no-op, first record, forward-only, failure ===
  {
    ...
    // holdsFrom (the proven FLOOR) advances forward-only UP alongside holdsIndex (the ring-floor over-claim
    // fix, BD-RETENTION-HOLDSINDEX-RINGFLOOR-OVERCLAIM): a record carrying a higher floor raises it; a lower
    // or absent floor never lowers it, so a stale/duplicate record can never widen the covered window.
  ...
  // ===== BLOCK 18: persist-state storage-fault counter (SCHED persist-state-storage-fault / INFRA do-value) ==
  {
