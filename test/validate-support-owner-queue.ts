...
// un-approved proposal can roll past the 40-event window with no live queue state to fall back on, leaving
// "a high-blast-radius change is stuck waiting for a second owner" undiagnosable. fetchOwnerActionQueue reads
// the caller-independent DO aggregate instead, and projects counts + oldest proposal + a closed-kind
// breakdown + the expired-undecided stall count. This drives the real gatherer through a DO double and
// asserts the counts ride, the kinds are gated to the closed vocabulary, and no id/summary/actor is echoed.
//
