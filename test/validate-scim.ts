...
//  unexpected non-200 status (here a 202, which the real DO should never send this caller, since SCIM
//  offboarding bypasses the dual-control queue entirely) still fails CLOSED to a 400, NEVER a false
...
//  re-implementation.
//
//  PART C (REAL SchedulerDO, dual control ON): arms "Require Config Approval" via the real
//  /admin/config/approval-policy route, then SCIM-offboards a non-owner member and proves the removal
//  APPLIES IMMEDIATELY (204; the member is gone from the roster; NO pending role-delete change is ever
//  created) - dual control has NO effect on SCIM offboarding at all. SCIM offboarding must never silently
//  fail closed, and it must never queue like a human proposer: no login flow can ever bind the queue's
//  synthetic proposer subject for the approve-time replay to re-resolve authority from (see
//  isScimOffboardCaller in admin/identity.ts). A companion case proves a TRUE last-Owner SCIM
...
interface RecordingStub {
  ...
  // EVERY DO path the facade touched, in order. This channel exists because the stub's catch-all was a
  // SILENT SINK: it answered a benign 200 to anything it did not recognise and recorded nothing, so
  // `rec.deletes.length === 1` meant "exactly one /roles/delete post", NOT the "exactly one delete" its label
  // claimed. The day the facade issues a second mutating call - the planned /signin-factors/revoke is the
  // obvious one - every such assertion would keep passing while saying nothing about it. Recording the paths
...
// captureConsole swaps console.log/console.error for collectors so the authentication-line proofs
// can read what the surface logged, and restores them after.
...
async function testUnsetToken(): Promise<void> {
  ...
  ok("503 recorded the scim-unconfigured auth-signal (no-deprovision-path)", rec.signals.includes("scim-unconfigured"));
}
...
async function testMissingBearer(): Promise<void> {
  ...
  ok("401 recorded the scim-unauthorised auth-signal (bearer rotated/wrong)", rec.signals.includes("scim-unauthorised"));
}
...
async function testServiceProviderConfig(): Promise<void> {
  ...
  let r: Response;
  try {
    r = await handleScim(scimReq("GET", "/scim/v2/ServiceProviderConfig"), env);
  } finally {
    cap.restore();
  }
  ok("GET ServiceProviderConfig -> 200", r.status === 200);
  // The accepted bearer is logged as an authentication success for the scim surface, and the token
  // value never appears in the line.
  ok("an accepted SCIM bearer logs authn success method=token surface=scim", cap.lines.some((l) => /authn success method=token surface=scim/.test(l)));
  ok("the success line carries no token value", !cap.lines.some((l) => l.includes(SCIM_TOKEN)));
  {
    const capBad = captureConsole();
    let bad: Response;
    try {
      bad = await handleScim(scimReq("GET", "/scim/v2/ServiceProviderConfig", { bearer: "not-the-token" }), env);
    } finally {
      capBad.restore();
    }
    ok("a rejected SCIM bearer is 401 and logs authn failure surface=scim reason=bearer-rejected", bad.status === 401 && capBad.lines.some((l) => /authn failure method=token surface=scim reason=bearer-rejected/.test(l)));
    ok("a rejected bearer logs no success line", !capBad.lines.some((l) => /authn success/.test(l)));
  }
...
//
// THE SECOND CASE HERE IS THE ONE THAT WAS MISSING, AND IT WAS A LIVE MISS. The classifier used to test the
// response text for "would remove the last Owner", which is one of the guard's TWO sentences. deleteRole
// refuses with the DUAL-CONTROL FLOOR sentence whenever the raised two-Owner floor fires, and that sentence
// does not contain the substring, so that refusal was filed in the generic bucket on shipped code. The third
// case is the negative control: a refusal that is genuinely NOT the Owner floor must still land in the
// residual bucket, so the new test is not simply passing everything.
async function testLastOwnerRefusal(): Promise<void> {
  ...
  // The DUAL-CONTROL FLOOR refusal: same guard, different sentence, and the old prose test could never see it.
  const rec2 = recordingScheduler();
  rec2.delResult = {
    status: 400,
    body: JSON.stringify({
  ...
  ok("its sentence does NOT contain the string the old classifier matched on", !rec2.delResult.body.includes("would remove the last Owner"));
...
// caller, since SCIM offboarding bypasses the dual-control queue entirely - see isScimOffboardCaller in
...
// PART C: dual control ("Require Config Approval") must have NO effect on automated SCIM deprovisioning at
// all - it must never silently fail closed, and it must never queue the removal like a human proposer
// (see the file header + isScimOffboardCaller in admin/identity.ts). It must also NOT be
...
async function partC(): Promise<void> {
  ...
  // SCIM-DELETE while the gate is ON must APPLY IMMEDIATELY (204) - never queue and never
  // silently or permanently fail.
