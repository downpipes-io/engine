// Validates the heal-dests resilience + auto-heal work (chaos findings DEST-1, DEST-2 and the replica
// catch-up surfacing) end to end against the REAL scheduler DO and the REAL restore-destination fallback:
//
//   DEST-1  A single/default-destination outage now lights the map's per-destination "down" indicator,
//           not merely a failed run. A FAILED completion carrying downDestinationIds records a lastOk:false
//           reachability heartbeat (forward-only on holdsIndex, so a down record never drops a proven copy),
//           and a later successful run refreshes the same destination to lastOk:true (self-heal).
//   DEST-2  Restoring a run whose recorded destination(s) were force-removed (the sole copy) no longer
//           returns a generic "object missing": withRunDestFallback re-labels it with the actionable
...
async function testDest1SingleDestDown(): Promise<void> {
  console.log("-- DEST-1: a single-destination outage records a per-destination down heartbeat --");
...
async function testDest1NonDestFailureNoHeartbeat(): Promise<void> {
  console.log("-- DEST-1: a non-destination failure records no false down heartbeat --");
  const { dobj } = makeDO({
    "dp:single": { config: { id: "single", name: "Solo", destinationIds: ["default"] } },
...
// ---- DEST-2: a removed-origin restore names the cause instead of a generic object-missing -------------
...
async function testDest2RemovedOrigin(): Promise<void> {
  console.log("-- DEST-2: a run whose only recorded destination was force-removed names the cause --");
