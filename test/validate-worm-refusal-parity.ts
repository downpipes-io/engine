...
//
// WHY THIS FILE EXISTS. The same input can produce two contradictory sentences from two
// builders in the same repository, each pinned by its own suite:
//
...
//
// One suite can require the words the other forbids, about the same fact. Neither sees the other, because
// each grades its own builder. This gate grades BOTH from one place, so a divergence is a red rather than
// a contradiction that survives two green suites.
//
...
// (src/dest/s3-worm.ts) and drives the worm-refused classification. R2's answer was measured live: PUT
// _RECOVERY/.write-probe returned status 501 (NotImplemented), with a no-WORM destination on the SAME
// bucket and the SAME credential answering ok, so the refusal is attributable to the policy rather than to
// the bucket or the credential. A separate live probe shows R2 cannot enable Object-Lock on any bucket at
// all.
//
// THE SECOND RULE is about the REMEDY rather than the reading. Four providers are
...
//
// Serving Amazon's sentence to all four, with r2.ts additionally claiming R2 DOES support Object-Lock
// through its S3 API, leads four surfaces to publish "reach the bucket by its S3 endpoint" as the fix. An
// operator who follows it gets a 501 on every write and an archive holding nothing. So the rule is: the
// remedy a surface offers must name THAT store's mechanism, and must never offer Amazon's to a store that
// does not have it.
//
...
function theNeighbouringReadingsAreControls(): void {
  ...
  for (const [label, worm] of [
    ...
    // "S3 Object-Lock", not the bare "Object-Lock". The posture check reads its noun and its mechanism
    // from dest/worm-remedy.ts like the report does, so the unknown-provider fallback spells the mechanism
    // the same way on both surfaces. The store noun is left open here because the per-provider block below
    // grades it properly; pinning "bucket" in this control would keep the check saying "bucket" to an
    // Azure customer.
...
function theTwoSurfacesDoNotContradictEachOther(): void {
  ...
  // ignored, that is the same defect back, whichever reading it lands on.
...
function theRemedyIsTheStoreSOwn(): void {
  ...
  for (const provider of Object.keys(expected) as DestProvider[]) {
    ...
    // their own console does not have, which is the same defect in its general form.
 ...
 *
 * This block exists because per-provider naming in the signed report and in the REMEDY sentence of this
 * check is not enough on its own: the check's DETAIL, the line the console renders to a customer under
 * "Observed", must also speak the store's own vocabulary, or it can still say "the destination bucket
 * enforces S3 Object-Lock" on an Azure container. A gate whose cannot-confirm control requires the word
 * "bucket" would pin exactly that half-fixed shape, which is worse than no gate, because it reads as
 * coverage.
 *
