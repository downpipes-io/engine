...
// register -> login round trip is exercised end to end elsewhere (validate-passkey-roundtrip.ts), but the
// finding (engine-src-014-11) called out that the error branches of sessionCookieForFinish were never
// isolated: a malformed body, a non-ok body, an empty email, and a DO mint failure each yield null and must
...
// guards, byte-identical to before the COV.18 B15 fault seam was added.
...
export async function run(): Promise<void> {
  ...
  // 8. COV.18 B15: a production env NEVER touches the fault store, even with a fault armed underneath it --
  //    the flag gate is checked first inside consumePasskeySessionMintFault, so the happy path is unaffected.
  {
    const cookie = await sessionCookieForFinish(PROD_ENV, fakeIssueScheduler({ ok: true, token: "minted-token-abc" }), JSON.stringify({ ok: true, email: "a@example.com" }));
