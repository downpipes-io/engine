...
async function multipartErrorPaths(): Promise<void> {
  ...
  //
  // TWO things must hold here, neither of which the generic 501 path gives for free: multipartStep's
  // transient regex matches "status 501", so a plain permanent refusal would otherwise be retried three
  // times per segment; and the exhaustion wrapper would otherwise build a NEW Error, dropping the refusal
  // stamp and taking the worm-refused down-reason with it.
  {
