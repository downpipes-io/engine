// stepUpBegin must never emit an allowCredentials list larger than the browser will accept, because past
// that ceiling the browser refuses the assertion and the account can never step up again. Run:
//   node test/validate-stepup-allowcred-cap.ts
//
// THE ASSERTION CEREMONY'S OWN CEILING. `excludeCredentials` on the REGISTRATION ceremony is bounded to
// Chromium's measured 64-entry ceiling. The assertion ceremony has a second descriptor list,
// `allowCredentials`, which must be bounded the same way rather than emitting an account's ENTIRE
// credential list into it.
//
// MEASURED on Chromium, CDP virtual authenticator on a local origin, dose-response over allowCredentials
// sizes 0, 1, 32, 63, 64, 65, 66, 100, 128, 256:
//
...
//
// The rig reproduces the excludeCredentials measurement (first throw at exactly 65) in the same session as
// its control, so its allowCredentials verdicts are not the instrument reporting on itself. The refusal
// does not move when the genuine credential is placed LAST rather than first: the browser rejects on list
