// Validates the guard families whose call sites a CALL COUNT cannot hold, and closes the multi-component
// engine promote leg that had the self-redeploy guard on neither of its two follow-up writes.
//
// WHY THIS EXISTS. A gate elsewhere checks, for every repair, whether the set of sites the FIX touches
// equals the set of sites the GATE drives. Over five guard families and thirty-five call sites, two families
// resist that sweep: `recordAuditAfterSelfDeploy` (7 sites) and `recordBookkeepingAfterSelfDeploy` (6),
// because THE GUARD THERE IS THE CHOICE OF FUNCTION RATHER THAN AN ARGUMENT, so deleting a call changes two
// things at once -- the guard AND the work it wraps -- and a red proves nothing about either.
//
// THE METHOD THAT UNBLOCKS IT. A choice-of-function guard IS mutable per site, by a SIGNATURE-IDENTICAL
// SHIM: a twin that takes the same arguments, does the same work, returns the same booleans on every path,
// and drops exactly ONE property. The call site then changes by one identifier and nothing else. All
// thirteen sites are swept that way.
//
// AND THE SHIM IS ONLY HALF THE ANSWER, WHICH IS THE PART WORTH CARRYING. A shim changes the identifier, so
// a call census SEES it. What a census cannot see is the narrowing one level down: shorten the shared ladder
// inside `retryAfterSelfDeploy` and every one of the thirteen sites still names the guarded function, the
// census stays green, and the property is gone at all thirteen at once. So a choice-of-function guard needs
// its gate in TWO independent halves, and neither half is optional:
...
//
// THE NEGATIVE CENSUS asks the census question in the OPPOSITE direction. Rather than count the sites that
// DO name the guard, this file asks which engine self-redeploys are followed by a write that does NOT.
// `handleComponentApply` -- the multi-component apply leg, the one the console drives whenever a release
// carries a console component -- calls the SAME `planAndPromote` against the SAME `makeCfDeployDriver`, so
// it rolls a new Worker version and resets this Durable Object exactly as the legacy leg does. Its own
// comment claims "The flow mirrors the legacy engine-only apply exactly". It does not: the legacy leg's two
// follow-up writes are `recordBookkeepingAfterSelfDeploy` and `recordAuditAfterSelfDeploy` next to a comment
// naming the reset, while the components leg's were `recordDiagWrite` and a BARE `recordAudit`.
// `recordDiagWrite` catches, so the pending record is simply LOST -- its own comment says a dropped pending
// record "makes an IN-FLIGHT update invisible to the canary gate and to the pack". `recordAudit` does NOT
// catch, so the reset THROWS out of the route and into the outer catch at router-updates.ts, which answers
// "could not start the update right now; nothing was changed" WHILE THE ENGINE IS LIVE ON THE NEW VERSION --
// exactly the state the legacy leg's comment engineers against, reachable on the sibling route nobody had
// counted against it.
//
// WHAT IS GRADED HERE.
...
//   C. CENSUS OF THE VERDICT, for the widened families: a call count is blind to a narrowing -- the
//      mutation arms below narrow twenty-five sites by making each call's verdict be DISCARDED through a
//      comma expression, and every call is still there to be counted. So these censuses assert the VERDICT
//      IS CONSUMED, not that the call exists.
//
...
async function main(): Promise<void> {
  ...
  console.log("\nA2. and it survives the OPAQUE form of the SAME rollout, which is the form that lost a row live");
  {
    ...
    // This dose is not a guess: the opaque form is logged on the audit stub and the named form on the
    // marker stub in the same request, and only the named one is matched.
  ...
  {
    // This is the mechanism of the multi-component leg's defect, driven rather than argued. recordAudit awaits the DO
  ...
  //    self-redeploy in the tree -- which is the direction that finds a missing follow-up write.
  // =================================================================================================
  console.log("\nB1. CENSUS: the fifteen self-redeploy-safe call sites are all present");
  {
    ...
    // stylistic preference. Asserting a total while summing a hand-kept list of files risks the total
    // reading as covering everything when it covers only a subset -- the same defect class this whole file
    // exists for, one level up -- so the sum walks src/ and the per-file table is graded as a DISTRIBUTION
    // against it.
