...
// first STREAM_LIST_LIMIT (1000) page. estimate is a floor, not a
// "cheap upper bound", for libraries over 1000 videos.
...
console.log("\n-- captions: a captions LIST that throws emits an _unavailable MARKER (not silent), run intact --");
...
// ---------------------------------------------------------------------------
// INTRA-VIDEO RESUMABILITY: a single very large video captured WITH content
// emits several records (metadata, the MP4 bytes streamed in BYTE_RANGE_WINDOW
// windows read twice by the seal, then each caption track). Marking only AFTER
// the whole video would let one huge video exceed a single slice's
// budget with no intra-video yield and wedge the run on that one item. So the code
// emits an intra-video {mark} between a video's records when the slice budget is
// low (shouldYield), so the capture of one very large video spans slices and
// resumes. A BudgetMeter exposes shouldYield() the way the sliced seal's
// SliceBudget does; a plain Meter (no shouldYield) runs the video whole.
// ---------------------------------------------------------------------------
console.log("\n-- a single large video yields mid-item (intra-video mark) and resumes --");
{
  ...
  // BASELINE: a plain Meter has no shouldYield(), so the video is captured whole with NO
  // intra-video mark (the only mark is the per-video boundary one).
  ...
  ok("a plain Meter emits ZERO intra-video marks (only the per-video boundary mark)", intraVideoMarks.length === 0 && wholeMarks.length === 1);
  ...
  // A malformed inVideo cursor throws rather than silently restarting the crawl (mirrors parseStreamToken's
  // afterUid assertion and the inRepo guard).
  let threw = false;
  try {
