...
// also[] entry. The labels and segCount fields are fully typed and asserted wherever
...
async function runKeylessAttestation(): Promise<void> {
  ...
  // and keylessErrId, plus the format-version major gate reached on signature-valid bytes.
  {
    ...
    // directly ("archive format version not supported") rather than falling into the generic structure
    // bucket, because a caller facing an unimplemented major needs the true statement, not an opaque
...
async function runKeylessStructuralAttestation(): Promise<void> {
  ...
  //
  // THIS REFUSAL MUST NOT READ AS "OBJECT MISSING". coarseKeylessReason's `is missing` net is a substring
  // match, so a refusal worded "...is missing the major.minor.patch components" would land in the ABSENT
  // OBJECT bucket. That bucket is replica-fallback-eligible, so the answer would send the caller to a 3-2-1
  // copy holding the same signed run and the same label, which can only refuse again.
  {
    ...
    // "the signature held and the body did not validate". What matters is that it is not "object missing",
    // a reason that is replica-fallback-eligible and would send the caller to a copy holding the identical
    // label.
  ...
  // for. It lands in the same structure bucket as the one-component case above. No build emits a bare
  // MAJOR.MINOR label, so the true reading of these bytes is an anomaly in our own signed manifest.
  {
...
// nothing), so the chain-anomaly signal alone cannot catch it -- only pinning minRunlogIndex to an
// out-of-band high-water mark (e.g. the scheduler DO's own runlogCounter) does.
async function runRollbackPinAttestation(): Promise<void> {
  ...
  // The exploit exactly as described, with NO minRunlogIndex pin: the replayed document carries no chain
  // anomaly, so both tiers still read clean -- the residual gap the chain-anomaly signal alone cannot close.
  ...
  // Pinning minRunlogIndex to 2 (simulating the scheduler DO's live runlogCounter, which has already
  // moved past the later run) closes it, exactly as router-restore.ts's runKeylessAttest does via GET
  // /scheduler-signals. The replayed document's own global max (1) falls below the pin, so both tiers
  // correctly reject it, even though the replay itself is clean.
...
//
// checkRunlogFreshness must distinguish five inputs that leave the check unable to run at all (the RUNLOG
// absent, its signature not verifying, the run not in it, and either disagreement with the signed root) from
// a benign chain that merely sits below a min-index pin, the least severe of the six. Every engine caller
// passes allowStale:true, so any of the five reporting a plain "no rollback" would open the run on an
// unchecked chain; only the below-pin case may legitimately refuse while still reporting a check that ran.
// This drives the whole table through the PRODUCTION openRun over a genuinely sealed archive with a REAL
// hybrid signature over the RUNLOG bytes, in both stale postures.
//
// THE TWO CONTROLS ARE LOAD-BEARING and are asserted alongside the refusals: a clean latest run still opens
// with stale tolerance on, and a tampered signature already refuses without it. A check that simply refused
...
async function runUncheckedFreshnessRefusal(): Promise<void> {
  ...
  for (const c of cases) {
    ...
    // that keeps attestKeyless independent of the checked/rollbackDetected fields above: it ANDs that field,
    // so its verdict never depends on them. This invariant is asserted directly, because a mutation making
    // this branch claim isLatest TRUE would otherwise pass unnoticed. maxIndexForDownpipe 0 says no maximum
    // was observed rather than inventing one, and runlogIndex carries the index the SIGNED ROOT claims for
    // this run, which is the one number a support row has when the RUNLOG itself cannot be read.
    ...
    // The forensic trail matters here: without a row for each of these states, the highest-stakes ticket the
    // product can raise (has our history been rewritten, or did the destination lose an object?) starts with
    // nothing to investigate from.
    ok(`${c.label} leaves a forensic row (${c.anomaly})`, rows.some((a) => a.kind === c.anomaly));
    // The keyless tier is correct here regardless (it ANDs isLatestForDownpipe, false in every one of these
    // states); asserted directly rather than assumed.
  ...
  // note, it leaves a forensic row, and it still opens under stale tolerance, because refusing it would
  // block restore from exactly the diverged replica a 3-2-1 restore reaches for when the primary is gone.
  {
    ...
    // a canary landing on a diverged replica must report it rather than pass. The negative half is asserted
    // directly, because a branch hardcoding ok:true here would otherwise make the canary silently green on
    // the one destination shape it exists to notice.
  ...
  // every anomaly shape against the production detector, but it asserts on ok and the reason only, not on
  // `rollbackDetected` or `checked`, so this asserts the checked field on the chain-anomaly branch directly:
  // an unconditional anti-rollback signal that a defended service must be able to rely on regardless of the
  // freshness posture it was opened with.
  {
  ...
  // to `<=` would refuse every archive at its own high-water mark, which is most of them, so the boundary is
  ...
  // downpipe while the document as a whole reaches 9, and a pin of 9 must clear. Every other fixture in this
  // file holds exactly one downpipe, so the two maxima would otherwise be indistinguishable; this case swaps
  // them apart deliberately, because a pin that quietly stops working is worse than no pin -- the operator
  // believes a replay is covered.
  {
  ...
  // (checked true) and NO rollback, and refuses only because staleness was not acknowledged. Both halves are
  // asserted directly: reporting a rollback for ordinary staleness would refuse every stale-tolerant restore
  // in the product, which is exactly the failure mode this guard exists to prevent.
  {
...
async function main(): Promise<void> {
  ...
  // The anti-rollback replay exploit (a clean whole-document RUNLOG replay) end to end on both the
  // keyless and keyed tiers, proving the minRunlogIndex pin -- not the chain-anomaly check alone --
  // is what closes it.
  ...
  // openRun/attestKeyless refuse a non-ULID runId before it ever reaches an object-key template,
  // proven with a store that fails the test if it is read at all.
