...
// 15. xsi:type AttributeValue + an ANCESTOR-DECLARED prefix used only inside the value. The real
...
//     Attribute sort: xmlns declarations precede attrs; xsi:type sorts under the xsi namespace URI.
{
  ...
  eq(
    ...
    "xsi:type renders xmlns:xsi (visibly used) but DROPS the value-only xs prefix WITHOUT a PrefixList",
  );
  ...
  eq(
    ...
    "InclusiveNamespaces PrefixList [xs] force-renders the ancestor-declared, value-only xs prefix",
  );
  ...
  eq(
    ...
    "ancestor-declared saml: (element-name prefix, visibly used) renders alongside the PrefixList xs",
  );
}
