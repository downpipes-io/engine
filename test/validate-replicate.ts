...
// run-tree keys and the seg keys separately so a test can assert the replica holds BOTH: DEST-2 is that a
// manifest-only copy (run-tree without seg/ data) is a silently non-restorable replica. The seg ids
...
async function testMirrorToEmptyReplica(signer: Signer): Promise<void> {
  console.log("-- mirror a finalised run to an empty replica: objects copied + RUNLOG appended (prevRunId null) --");
  {
    ...
    ok("the replica holds every DATA segment (DEST-2: a restorable copy, not manifests-only)", segKeys.every((k) => rep.has(k)));
...
function testBacklogContiguity(): void {
  console.log("-- backlog contiguity: a recorded copy-count never advances past a gap (the over-claim guard) --");
  {
    ...
    // The E1 repro: a LATER run is present but an EARLIER one is missing (its origin was unreadable this
...
async function testKeylessVerify(signer: Signer): Promise<void> {
  console.log("-- DEST-1: post-copy keyless integrity verify (verify:true) passes a faithful copy, rejects a corrupted shard --");
  {
    ...
    // A faithfully-copied REAL sealed run passes the keyless post-copy verify (signature + shard completeness).
    {
      ...
      // DEST-2 on a REAL sealed archive: the replica must hold the run's DATA segments (seg/), not just the
      // manifests. Before the seg-store sync this set was EMPTY on the replica while the verify still passed
      // (attestKeyless cannot see segment data), so the "copy" was silently non-restorable. This is the
      // regression that proves the C1 fix.
      ...
      ok("every primary data segment is present on the replica (DEST-2: independently restorable)", primSegs.every((k) => repKeys.has(k)));
    }

    // A shard the signed root pins is corrupted in transit: the verify must REJECT (throw) and leave NO
    // done-marker, so the replica is never recorded as holding a good copy (the DEST-1 fail-closed guarantee).
    {
      const primary = new MemoryDestination();
      const replica = new MemoryDestination();
...
async function testIncompleteDataSync(signer: Signer): Promise<void> {
  console.log("-- DEST-2: an incomplete data sync does NOT mark the run held (no done-marker; resumes next tick) --");
  {
    const primary = new MemoryDestination();
    const replica = new MemoryDestination();
...
async function testStructuralSegCheck(signer: Signer): Promise<void> {
  console.log("-- L2: opt-in keyless STRUCTURAL seg/ check (verifySegments) catches truncated/empty/wrong-object copies; default OFF unchanged --");
  {
...
function testSegVerifyDefaultOn(): void {
  console.log("-- M5(a): the cron replication pass VERIFIES replica segment bytes BY DEFAULT (not opt-in/blind) --");
  {
...
async function testVanishingSegment(signer: Signer): Promise<void> {
  console.log("-- L3: a segment that vanishes mid-sync is deferred when its run is still live, but skipped (no livelock) when the run is pruned --");
  {
    ...
    // or leaves it absent (run pruned), which is exactly the L3 distinguishing signal isRunLive reads.
    class VanishingOrigin extends MemoryDestination {
      ...
      override async listPage(prefix: string, cursor?: string): Promise<{ keys: string[]; cursor?: string }> {
        ...
        // vanish). The merge-walk then GETs it, sees null, and applies the L3 distinction by run liveness.
...
async function testBoundedMemorySegSync(signer: Signer): Promise<void> {
  console.log("-- M6: a seg/ store larger than one page syncs in BOUNDED memory (page-by-page, never materialising the whole keyspace) --");
  {
