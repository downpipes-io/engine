...
//
// WHAT THIS PINS: the operator-supplied auth header VALUE (a bearer token / API key) accepts any
// string with no length cap and no control-character screen if the value screen is missing, while the
// header NAME is strictly validated. A value carrying a CR/LF (header-injection shaped) or a
// pathological length would otherwise pass config-time and surface only later as an opaque send-time
// network error (undici rejects the header), stranding the destination with no clear cause. This
// validator asserts a control-char or over-long value is REJECTED at config time (a clear error), while
// legitimate token shapes -- including those with spaces like "Basic <b64>", "Splunk <token>",
// "GenieKey <token>" -- are accepted unchanged.
//
// WHY IT EXISTS: the auth header value needs a positive validation screen so malformed or oversized
// values are rejected at config time rather than stranding the destination with an opaque network error.
//
