...
// ============================ 1b. AUTHNSTATEMENT SESSION BOUND (ASVS V7.6.1) ============================
{
  ...
  //
  // claim-drop-saml-auth-time is documented as "an AuthnInstant was asserted and would not parse". Without a
  // presence test, parseSamlInstantMs would fold an unreadable instant to null, and null would reach
  // buildAuthContext as `undefined`, which is byte-identical to an IdP that asserted no AuthnInstant at all.
  // The presence test lives in assertion.ts, at the raw attribute. Driven through the REAL consumer, from
  ...
  ok("an AuthnInstant that WILL NOT PARSE is recorded (it used to be silent, and identical to an absence)", dropsOf({ authnInstant: "not-a-real-instant" }).includes("claim-drop-saml-auth-time"));
