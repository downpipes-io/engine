...
{
  // A real two-version split where the SUSPECT new version is versions[0] at the SMALLER share. The old code
  // returned versions[0] (the suspect); the fix returns the DOMINANT slice and exposes the full set.
...
console.log("-- W4 rampVersion: POSTs a TWO-version percentage split (new + prior summing to 100) --");
{
  let sentBody: DeploymentsBody | null = null;
  const stub = (async (input: RequestInfo | URL, init?: RequestInit) => {
    const url = String(input);
  ...
  const body = sentBody as DeploymentsBody | null;
  ok("[W4] ramp body is a percentage strategy", body?.strategy === "percentage");
  ok("[W4] ramp sends TWO versions (new + prior)", Array.isArray(body?.versions) && body!.versions.length === 2);
  ok("[W4] ramp serves 25% to the new version", body?.versions?.find((v) => v.version_id === "v-new")?.percentage === 25);
  ok("[W4] ramp serves the remaining 75% to the prior version", body?.versions?.find((v) => v.version_id === "v-old")?.percentage === 75);
}

console.log("-- W4 rampVersion: an out-of-range percentage is refused before any call (defence in depth) --");
{
  let called = false;
  ...
  ok("[W4] ramp at 100% is refused (that is the atomic promote) without a network call", /between 1 and 99/.test(threw) && called === false);
}

console.log("-- W4 rampVersion: a CF error becomes a cfErr message; the prior version stays at 100% --");
{
  const stub = (async (input: RequestInfo | URL, init?: RequestInit) => {
    const url = String(input);
    if ((init?.method ?? "GET") === "POST" && /\/deployments$/.test(url)) return new Response(JSON.stringify({ success: false, errors: [{ code: 10000, message: "Authentication error" }] }), { status: 403 });
  ...
  ok("[W4] a ramp failure surfaces the reason and says the prior version is still serving 100%", /Authentication error/.test(threw) && /still serving 100%/.test(threw));
}
