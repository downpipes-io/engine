...
// in the scheduler DO and the status builder is pure. Measured at engine dce9f842: NO call site anywhere
// supplied one. `git log -S auditCount -- src/admin/router-status.ts` is empty, so the field has never
// appeared in that file since the retention cap was introduced. GET /admin/status posts the status
// observation to the DO -- whose reply carries the count -- and DISCARDS the response.
//
