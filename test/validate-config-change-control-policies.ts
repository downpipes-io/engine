...
// apply, the dry-run leaves no side effect across the FULL keyspace + the alarm, and a custom-role
// proposer's queued notify change applying on approval without widening authority. Split out for size;
// byte-identical, runs in order against the shared live DO.
...
export async function runPolicies(ctx: Ctx): Promise<void> {
  const { ok, call, listDownpipes, pendingChanges, pendingKeyCount, setGate, sched, listChannels, listExpiry, configHistory, configDiff, dp, ageChange } = ctx;
  ...
  //
  // A notify channel's url is a bearer credential whose token usually rides in the PATH (a Slack/Teams/
  // generic-webhook ingest token), and config-snapshot.ts redacts it through the webhookHost() helper to
  // urlHost + urlConfigured. POST /admin/notify/channels is the route through which a url of that class
  // enters the version chain, which is why the proof targets it.
  // ===========================================================================================
  {
  ...
  // ===========================================================================================
  {
    ...
    // Aged so the dwell floor does not itself intercept before the integrity recompute the message
  ...
  // unchanged (assert exhaustively, not just the one downpipe; this includes the alarm rollback).
  // ===========================================================================================
  {
    ...
    // The audit append folds a DIAGNOSTIC counter when it records a human actor with no captured source IP
    // (diag:admincounters, the pack's bounded {closed name -> count, lastAt} aggregate). That is bookkeeping of
    // exactly the same class as the audit entry that caused it -- a count, never a config value, never a key an
    // operator set -- so it belongs in this allowlist and NOT in the config-mutation set this assertion protects.
    // The invariant the test exists for is unchanged and is asserted below: no config-ish key
    // (dp:/role:/notify-/expiry:/policy:/confighist:) may move on a dry run.
  ...
  // ===========================================================================================
  {
    ...
    // approve replays AS the custom-role proposer: without the resolved capability set forwarded, a
    // by-role notify.config re-check would refuse them on their viewer floor; with it forwarded, it applies.
    await setGate(OWNER, false);
    ...
    // the notify.config re-check consults their RESOLVED set, so it APPLIES.
