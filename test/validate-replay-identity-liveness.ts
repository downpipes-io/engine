...
// same identity would. d3099399 built the spend-time binding on that principle and 9622c01b closed the first
// way it was broken (a recorded EMAIL matching a pending grant and binding it onto the recorded subject,
// which is a WRITE). This is the second way, and it is a whole axis rather than a stale value.
//
...
//
// THIS SITE, AND ITS SIBLING. The owner-action replay caller already carried the `replay` flag before this
// change (9622c01b set it there), so this file measures the LIVENESS gate and nothing else. The CONFIG-CHANGE
// replay caller did not carry the flag at all -- a third replay site, built inline rather than through a
// named helper, which 9622c01b's "the only two" missed -- and it is proved separately in
...
async function sectionA(): Promise<void> {
  ...
  const { sched, OWNER2, call, doFetch, ownerCaller, destConfig, setGate, ageOwnerAction } = ctx;
