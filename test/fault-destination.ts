// FaultDestination is the fault-injecting in-memory Destination double for the workspace's chaos test suite's dest-fault /
// verify-at-seal axis (Tier 1, deploy-free). It is the ONE new double the axis needs; it is modelled on the
// three shipped in-memory Destinations (memdest.ts MemoryDestination, validate-slice.ts MemDest, and
// validate-verify-at-seal.ts SpyDestination) and adds a small fault schedule so the harness OWNS the ground
...
// still name it transient (slice.ts:715 vs classify.ts NETWORK_RE vs run-pressure.ts TRANSIENT_RE).
...
function buildFaultMessage(verb: string, key: string, spec: WriteFaultSpec): string {
  switch (spec.shape) {
    ...
    case "nativeInternal":
      ...
      // outage carries "internal error" but no "status NNN" (the shape slice.ts:715 omits).
