...
//
// THE MEASUREMENT THAT STARTED IT. Three armed-authority artefacts in this subsystem were reported as living
// for ever. Driven, two of the three do not: `approval:sha384:` / `prune-approval:` carry a 24h TTL applied
// lazily at read (APPROVAL_TTL_MS / PRUNE_APPROVAL_TTL_MS) and `owneraction:` carries the same
// (OWNER_ACTION_TTL_MS), so what each record can DO is bounded by a day. What none of them bounds is what a
// record HOLDS. An owner action stores its params VERBATIM because the approved execution must replay the
// identical action, and for the destination, IdP, push and discovery kinds that verbatim store includes the
// live credential the proposer typed. Nothing has ever deleted an owner-action record, so that credential
// outlived the action it belonged to without limit: the TTL expired the AUTHORITY and left the SECRET.
//
// WHY THIS ONE AND NOT THE OTHERS. It is the only residue here whose exposure GROWS with time rather than
// merely persisting. An expired approval is inert the moment it lapses; a stored credential is worth more to
// an attacker the longer it sits, because the customer has forgotten it is there and the key it names may
// well have outlived every other trace of the request. And the read-path control that already exists made it
// easy to miss: viewOwnerAction strips the secret from every caller-facing echo (the inbox, the approve
// response, the reject response), so nothing an operator can look at ever showed the credential, and the
// store kept it anyway.
//
// THE FIX IS A SCRUB AT THE POINT THE ACTION BECOMES UNSPENDABLE, in three deterministic places (the
// DO-executed approve, the router-executed consume, and any reject) plus a lazy sweep for the records nobody
// ever decided. It reuses the listing's own secret-field list, so there is one definition of what a secret is.
//
// HOW THIS FILE AVOIDS PROVING NOTHING. Two hazards, both handled explicitly.
...
//   2. THE HARNESS'S OWN BLIND SPOT. MockStorage.list returns the STORED object by reference, while the real
//      Durable Object returns a deserialised copy. A sweep that mutated the listed record and forgot to
//      persist it would therefore pass here and do nothing in production: the exact shape of a gate that
//      silently does not gate. So this file installs a cloning list() over the harness's storage for the
//      whole run, restoring production value semantics, and additionally counts the storage.put calls the
//      sweep issues. Without both, section F would be satisfied by a fix that never wrote anything.
//
...
async function main(): Promise<void> {
  ...
  const { sched, OWNER, OWNER2, call, doFetch, ownerCaller, destConfig, inbox, setGate, ageOwnerAction } = ctx;
  ...
  try {
    ...
    // ===========================================================================================
    {
      ...
      // E7/E8: THE REASON MUST NOT DEPEND ON WHETHER THE ACTION CARRIED A SECRET. The first version of this
      // fix threw its own refusal from the integrity guard, which DISPLACED canApproveOwnerAction's reason
      // and made a duplicate approve answer one way for a secret-bearing action and another way for a
      // secret-free one: the same situation described two ways, decided by the params. It is pinned here
      // because this file is where the scrub lives, and the control is a spent action carrying NO secret,
      // whose record is never scrubbed at all.
    ...
    // ===========================================================================================
    {
      ...
      // ONE inbox call, and the mark is taken before it. Asserting F2 against a separate inbox call from the
      // one whose writes F3 measures would let a second call find nothing left to do and issue no put: F3
      // would fail while F4 passed, on a tree where the fix works perfectly. The same mistake in the other
      // order (measure the first call, assert on the second) would produce a green run that proves nothing
