// The refusal class the /support pull surface decides and discards.
//
...
// with an unexpired credential' was undiagnosable from the pack". The /metrics scrape threads this outcome
// into METRICS_SCRAPE_OUTCOMES (admin/metrics.ts); the two /support pulls run the SAME classifier through
// checkIngestCredential's boolean wrapper and book one generic `ingest-pull-auth-failed` instead. The
// discriminating detail the classifier already computes goes unused on the surface whose entire purpose --
// the SIEM audit feed -- is to be answerable.
//
...
// the unrepaired path is driven beside the repaired one, on a distinct subject, over the same doses. The pre
// tree's discriminating key must be ABSENT -- not zero -- and the post tree's must be a real count. Absent,
// zero and inverted are asserted apart, because they are easy to conflate.
//
...
//
// THIS IS A KNOWN POSITIVE, NOT A TIDY-UP. Without it every DO fetch throws "Invalid URL" inside the route,
// the 500 body parses to an object with no `grant`, and handleSupportPull reads that as `no-grant` and answers
// 401 -- so the rig would produce a plausible refusal for every dose while measuring nothing at all. The
// instrument control below catches exactly this: the class assertions alone would look like a partial pass.
...
async function main(): Promise<void> {
  ...
  //    ABSENT (pre) vs a real COUNT (post) -- asserted apart from ZERO, because a lost value reported as a
  //    legitimate zero reads as "we looked and there were none".
  // ------------------------------------------------------------------------------------------------------
  ...
  //    Two canary sites can disagree at the boundary exactly. The same grant's expiry is read by
  //    classifyIngestCredentialCheck (the gate) and redactGrant (the customer-facing view), so a disagreement
  //    would mean the console says "live" about a credential the gate refuses, or the reverse.
  // ------------------------------------------------------------------------------------------------------
  ...
  console.log("\nF -- two-sided: the suppressed directions, with the instrument control passing in each");
  {
    ...
    const sups: Sup[] = [
      ...
      // S4 INVERTED: the two remedies swapped -- it answers confidently and points at the wrong repair
      //    (rotate the credential vs update the collector).
      {
    ...
    // THE OK SUBJECT IS PART OF THE SUPPRESSION SET: with only the five refusals in the loop, S3's over-fix
    // (a refusal booked on a SUCCESSFUL pull) fails 0 of 5, because no subject ever takes the ok path. A
    // two-sided rig that cannot see one of its own suppressions shows up as a green line, not a red one.
