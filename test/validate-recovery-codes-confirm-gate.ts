// Proves the recovery-codes staged-confirm sequence end to end over the REAL scheduler DO and the REAL
// router (src/admin/router.ts), in-memory storage only. Run:
//   node test/validate-recovery-codes-confirm-gate.ts
//
// THE RISK THIS GUARDS AGAINST. An Owner who lost their passkey signs in with a saved recovery code (the
// documented "lost your passkey" path), is forced to enrol a fresh passkey, and if completing that
// enrolment called generateRecoveryFor unconditionally -- the SAME immediate mint the first-ever
// (bootstrap) enrolment uses -- it would REPLACE the live recovery record on the spot. The old
// (still-unspent) codes would die the instant the DO stored the fresh set, before the console's
// save-confirm panel had even rendered, so a closed tab or a crashed browser between register/finish and
// the operator ticking "I have saved my recovery codes" would leave them with nothing but the ONE code
// they had just spent to get in.
//
// THE DESIGN proved here: when register/finish's self-add mint runs while a LIVE record already exists, it
...
// invalidation boundary -- which is what the console's save-confirm "Continue" calls before it proceeds.
// An abandoned staged offer (confirm never called) costs nothing: the old set is still the working one.
//
// What this proves, in sequence:
...
async function run(): Promise<void> {
  console.log("validate-recovery-codes-confirm-gate: the staged recovery-codes confirm sequence");
