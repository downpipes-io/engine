...
//
// This is deliberately the ENGINE's floor, not its ceiling. The digit-exact, column-aware, control-by-control
// reconcile lives in the harness (lib/oracles/pdf-evidence-pack-reconcile.ts, driven live by
// spec/journeys/reports-evidence-pack.spec.ts), because it needs a real estate's posture and a second,
// independent reader of the bytes. What belongs HERE is the part that must never depend on another repo
// being run: that the pack names its frameworks, names its controls, and states real evaluated statuses.
// ------------------------------------------------------------------------------------------------------
    ...
    for (; j < s.length && depth > 0; j++) {
      ...
      if (ch === "\\") {
        ...
        if (next >= "0" && next <= "7") {
          ...
          // rest of this decoder already does under noUncheckedIndexedAccess. Hoisting the read is what makes
          // the cast expressible; it is NOT a behaviour change. Past the end of the buffer the read is
          // undefined, and `undefined >= "0"` is false, so the loop ends exactly where it ended before.
          // Driven rather than reasoned: both loops were run over every string of length 0..4 across a
          // 12-character alphabet chosen to straddle the octal boundary, at every index from -1 to length,
          // 133,670 pairs, and they agree on both the digits collected and the final cursor, while a
          // deliberately-wrong 2-digit-cap variant disagrees on 675 of them, so the comparison discriminates.
          while (oct.length < 3) {
