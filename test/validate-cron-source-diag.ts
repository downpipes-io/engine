...
//
// Covers: SIEM shaping and cursor fallbacks (including the latent batch-tail data loss), per-pass
// attribution, destination-probe class and dangling fan-out refs, tick-ring holes, auto-heal stalls,
// discovery skip causes, beacon class and partial configuration, cf-config discovery classes (truncation,
// stale-fallback selector mode), D1 defect class and locus, and cf-config restore skip classes.
//
...
const SENTINELS = [
  ...
  "example-secret-51ABCDEFsupersecretvalue",
...
function noSentinel(recorded: unknown, what: string): void {
  ...
  // named `url` is fine: envPresent records PRESENCE, never the value.)
...
section("cron pass attribution (which of the ~20 passes failed, and why)");
{
  ...
  noSentinel(rec, "cron-health record");
  ...
  noSentinel(hostile, "hostile body");
...
section("cf-config discovery skip causes (the stale cache that makes every run crawl ~214 surfaces)");
{
  ...
  noSentinel(hostile, "discovery skips");
}
...
{
  resetCronFaultLedger();
  // A malformed BEACON_URL never self-heals; a network blip does. The bare ok:false record does not
  // distinguish them; the fault class does.
  ...
  ok(rec.beacon.stateWriteFailures === 1, "recordBeaconState's OWN swallowed failure is counted (a stale beacon field is attributable)");
  // THE BINDING ASSERTION: the URL and the ingest key were both in the fault text and in the env.
  noSentinel(rec, "beacon record");
  ...
  noSentinel(hostile, "hostile beacon body");
}
...
section("auto-heal stalls (latched, no progress, reason swallowed) with the DO's FREE-TEXT reason");
{
  ...
  noSentinel(rec, "auto-heal record");

  // The amnesia-probe POST failure (an empty catch) leaves the probe field silently stale.
  resetCronFaultLedger();
  ...
  noSentinel(hostile, "hostile auto-heal body");
}
...
section("SIEM shaping + cursor fallbacks (incl. the LATENT batch-tail data-loss path)");
{
  ...
  noSentinel(hostile, "hostile siem body");
}
...
{
  resetCronFaultLedger();
  // The probe's catch classifies the S3 error instead of dropping it. Each of these has a different remedy.
  ...
  noSentinel(map, "dest-probe record");
  ...
  noSentinel(hostile, "hostile probe row");
}
...
section("cf-config discovery: WHY a surface is unavailable, truncation, and the stale-fallback mode");
{
  ...
  // Both surface as `unavailable` alone; the class is what tells them apart.
  const surfaces = [
  ...
  noSentinel(disc, "discovery record");
  ...
  noSentinel(clean, "sanitised discovery");
...
section("cf-config restore skip classes (why did 20 of 200 records skip?)");
{
  ...
  // The apply's summary carries a {class: count} map, not a bare integer.
  const skipped = [
...
{
  // Evidence counts only once it reaches the record the pack projects. applySourceFaults re-gates EVERY
  // field of the posted ledger against the closed vocabularies and DROPS anything out of vocabulary -- so a
  // field added to the ledger and forgotten here would be recorded at the fault site and then silently
  // discarded at the DO, which is exactly the failure mode this chokepoint exists to prevent.
  resetSourceFaultLedger();
