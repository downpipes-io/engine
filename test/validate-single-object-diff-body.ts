...
// WHY THIS EXISTS
// ---------------
// writeSingleObject used to PATCH the entire snapshot. That was a contract breach and a live defect, and
// the second is what made it worth finding.
//
//   The contract. The published promise for an in-band restore is that it "applies only the fields that
//   differ". Every list writer does that item by item. This one did not, and the gap was invisible
//   because whole-object and diff-only produce the same result whenever every field is writable.
//
//   The defect. Cloudflare validates the whole body, so ONE field the account cannot set refuses the
//   ENTIRE write, and none of the fields that would have applied are applied. Measured on a Free zone:
//   PATCH /zones/{id}/dns_settings with the whole object returns HTTP 400 "Custom SOA records are not
//   available to this account or zone", while the same change sent as {multi_provider: true} returns 200
//   and takes. dns-settings read as unwritable for as long as its plan-gated neighbour was in the body,
//   and it passed the full round trip within minutes of the body being narrowed.
//
// WHAT IT CHECKS
// --------------
...
//
// The reason this option exists. account-dns-settings reads an `soa` block it never changes, and a Free
// account refuses the whole-object body with "Custom SOA records are not available to this account or
// zone", so the surface could not be restored at all over a field nobody touched. Cloudflare deep-merges
// the partial body, which was proven against the live API before this was switched on.
{
  ...
  ok("nestedDiff: the untouched soa block is NOT sent, which is the entitlement it used to trip", !("soa" in zd), JSON.stringify(zd));
}
