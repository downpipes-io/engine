...
//
// THE DEFECT THIS PINS, MEASURED RATHER THAN IMAGINED (following the published /deploy/ steps against
// a genuinely fresh routeless worker). `npx wrangler deploy` uploaded the
// worker, armed its cron and printed "Deployed ... triggers". Then wrangler 4.116.0 asked, on the way OUT
// of that successful deploy, whether to install Cloudflare skills for a detected AI coding agent, and the
// command ended non-zero. deploy.sh runs under `set -e`, so the script stopped there, BETWEEN a successful
// deploy and key generation. Verified state afterwards: worker live with its schedule armed, no
// recovery-kit/, `wrangler secret list` empty. No signer key, no break-glass key, no admin token, and no
// published page saying the deploy is resumable.
//
...
// WRANGLER'S REAL ANSWER, CAPTURED FROM A LIVE `wrangler deployments status` RUN (4.116.0) RATHER THAN
// WRITTEN FROM MEMORY. This matters more than it looks. The first draft of deploy.sh's version probe
// grepped for "version id", which is what `wrangler versions list` prints, and this test fed it a fixture
// reading "Version ID: 9f144b71" that agreed with the probe and with nothing else. Both were wrong
// together, the test passed, and a real deploy against a throwaway worker then reported a landed upload as
// one that never happened. A fixture invented to match the code under test grades nothing.
...
function main(): void {
  ...
  {
    // The first draft grepped for "version id" and read NOTHING out of the answer above, so before and
    // after compared equal and a landed deploy read as a failed one. This asserts against the captured
    // wording rather than against the probe's own idea of it.
    const r = runStage({ deployExit: 1, versionBefore: "", versionAfter: REAL_DEPLOYMENTS_STATUS });
