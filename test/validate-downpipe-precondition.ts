...
//
// WHAT IS BEING GRADED, and it is the sentence rather than the status code. The chaos test suite's interleave pass
// drove estate `crud` against the deployed engine and measured two operators saving one downpipe with
// 153.6 ms of wire overlap, both answered 200, one edit discarded, and 175 ms of overlap between a delete
// and a save where the delete answered {"deleted":true} and the downpipe survived enabled and on its
// cadence. This validator REPLAYS THOSE TWO INTERLEAVES DETERMINISTICALLY against the real router and a
// real SchedulerDO, by holding one operator's read while the other operator's write lands, which is
// exactly the state the measured overlap produced. It then reads what each operator is TOLD.
//
...
// The overlap is established on the wire against a live estate; what
// is established HERE is the product's answer once the interleave has happened.
//
...
async function main(): Promise<void> {
  ...
  // pre reads the structured refusal WITHOUT assuming there is one. Under a plant that restores the
  // shipped overwrite-anyway behaviour the answer is a 200 with no `precondition` object at all, and a
  // bare property read would throw there and abort the run before the later arms were graded. The verdict
  // guard caught exactly that on the first plant of this file: a validator that stops short under the
  // violation it exists to detect reports FEWER reds than the violation causes.
  ...
  try {
    ...
    // MEASURED ON THE LIVE ESTATE FIRST, and it is why the key is stripped rather than merely ignored: the
    // shipped upsert STORES an unrecognised body field verbatim into the customer's configuration. Driving
    // `ifMatchRev:1` at engine 0.2.0 on estate crud came back 200 with `ifMatchRev` sitting INSIDE
    // `body.config`, so a client that adopted the precondition against an engine that did not understand it
    // would have written a statement about a request into the record, and from there into every
    // config-history diff and support-pack projection of that downpipe.
    ...
    // across a hash, which is precisely the shape interleave diagnosed in the audit-chain append and could
    // not drive in-process. So the window would reopen and no test in this file would notice, because the
    // interleave it reopens cannot be manufactured here at all.
    //
    // THE ATOMICITY ARGUMENT RESTS ON THE PLATFORM GATE AND IS NOT DRIVEN HERE, which is said plainly rather
    // than implied: proving it needs a workerd harness driving the object concurrently, which is not built
    // here. What IS checkable, cheaply and exactly, is the premise the argument depends on, so that is
    // what is checked: the span contains storage awaits and synchronous JS and nothing else.
    {
      ...
      // ANCHORED ON THE METHOD SIGNATURE. Without the anchor this line scanned addDownpipe's span and said
      // it had scanned the delete's, because the two reads are byte-identical once their template literals
      // are stripped. See the comment on scanSpan; a plant is what exposed it.
      ...
      // and this workspace has recorded a scanner that read a comment as code and one that matched nothing at
      // all. Both directions are pinned here rather than trusted.
      ...
      // WebCrypto namespace and a digest call, and both patterns are meant to catch it. Asserting one was my
      // own over-tight expectation and the known positive is what surfaced it.
