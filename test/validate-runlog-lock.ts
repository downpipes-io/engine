...
//
// Why this exists (ENG-H5 / XC-H9): the account-wide RUNLOG read-modify-write is serialised
// through the scheduler DO so the RUNLOG and its detached .sig are written by ONE run at a
// time (the F10 single-writer refinement that closes the two-object window). That lock plane
