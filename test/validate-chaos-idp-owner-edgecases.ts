// validate-chaos-idp-owner-edgecases.ts -- proves the guard against puppet-owner laundering, driven
// through the REAL router + DO with forged callers (the same in-process chaos pattern as
// validate-chaos-auth-races.ts: real handlers, no cloud).
//
// With the dual-control toggle OFF and two or more owners, destination and IdP ops auto-gate
// (HIGH_BLAST_ALWAYS_GATED), so a lone compromised owner cannot repoint a destination without a second
// owner approving. Role grants must be gated the same way regardless of the toggle: without that, a
// compromised owner could inline-mint its own colluding OWNER and use it as the "distinct" checker to
// approve its own exfil. Minting only an access-admin does not suffice on its own (a dest owner-action
// needs an Owner approver), so the guard also has to close the access-admin hop: an access-admin used to
// approve an owner-mint.
//
// The guard has two parts, both in scheduler-do-change-control.ts:
//  1. autoGateOwnerMint auto-gates an OWNER-minting role-set once a second owner exists, toggle or not. It
//     is narrow -- access-admin and every other appointment stay inline; only minting a new Owner dual-gates.
//  2. approveChange requires an OWNER (not merely a roles.write holder) to approve an owner-conferring
//     change, so a minted access-admin cannot approve the owner-mint (the hop), closing off
//     owner-escalation-by-approval.
// Together a lone compromised owner cannot manufacture the Owner checker its own high-blast op needs.
//
// A FAILING "auto-gated (202)" or "hop closed" assertion means this guard has a gap.

import { buildContext, ok, failureCount } from "./validate-owner-action-dualcontrol-harness.ts";
import { changeKey } from "../src/admin/change-control.ts";
import { APPROVAL_MIN_DWELL_MS } from "../src/admin/approvals.ts";
import { ageRecord } from "./testutil.ts";
import { verdictReached } from "./lib/verdict-guard.ts";

// AGE_PAST_DWELL (ASVS V2.4.2, approval-dwell): back-dates a queued config change's proposedAt past
...
console.log("Scenario A: owner-mint auto-gate + owner-approver rule close the laundering (toggle OFF)");
{
  ...
  ageRecord(ctx.sched.storage, changeKey(mintId), "proposedAt", AGE_PAST_DWELL);
