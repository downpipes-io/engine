...
//
// It survived every earlier sweep because the proving account allows ZERO custom page assets: the create
// was refused, so the delete never ran and the 204 was never seen. It appeared within minutes of pointing
// the same harness at an account where the create succeeds, reported as
// "CREATE-REFUSED: DELETE ... HTTP 204", which is a success being read as a failure.
//
// Offline and hermetic: a stub fetch, no credentials, no network. Runs in `validate`, because the whole
// point is that finding this needed a particular account and re-finding it should not.
