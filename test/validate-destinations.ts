...
async function testRemovalGuard(): Promise<void> {
  console.log("-- removing a destination is GUARDED against orphaning runs it is the only proven copy of --");
  {
    ...
    // the current default is and records NO origin id (destinationId undefined). Matching only
    // destinationId === id would let a default-routed run count as ZERO coverage, so the last proven
    // copy's destination could be removed (200 instead of the 400 orphan refusal). An unpinned downpipe has no
    ...
    // (i) FORCE overrides the default-routed refusal (a deliberate drop). This force:true path is the one
    // the console's type-to-confirm gate guards, and it must be reachable for default-routed runs too.
    {
      const dobj = makeDO({ destinations: soleDefault, "dp:plain": dpUnpinned, "hist:plain": histDefaultRouted });
...
async function testPruneSidecarFold(): Promise<void> {
  console.log("-- retention-prune fold (pure): destination-grain aggregation, the two halves, closed vocabularies --");
  {
...
async function testPruneSidecarJoin(): Promise<void> {
  console.log("-- retention-prune join: POST /retention-record folds at the DO write boundary and DestStatusView carries lastPrune --");
  const dests = { list: [{ ...LEGACY, id: "default", label: "Default" }, { ...LEGACY, id: "dest-two", label: "Two", bucket: "two" }], defaultId: "default" };
  {
