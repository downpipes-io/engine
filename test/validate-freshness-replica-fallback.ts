...
// WHAT THIS GUARDS
// ----------------
// The freshness gate refuses an open for two different reasons that must not be reported as one:
//
...
// simply not true of the RUNLOG. openVerifiedRun opens with verifyFreshness:true, so without this split a
// customer whose primary has lost its RUNLOG (deleted, denied after a credential rotation, throttled, moved to
// a cold tier) is refused the whole restore, including the two copies that would verify end to end.
//
...
//
// A silent success from a replica would be a different defect of the same family: a check that established
// nothing reporting that nothing was wrong.
//
// WHAT SECTIONS 5 AND 6 PROVE
// ----------------------------
// Section 5 drives the production walk with the reason a reader that does not distinguish these two classes
// would produce for this exact state (freshnessError -> REASON_FRESHNESS), and proves it stops on the primary
// after ONE attempt with the healthy replica never read. Section 6 then drives the identical two-destination
// estate through the real restore and reaches the replica on attempt 2.
//
...
async function main(): Promise<void> {
  ...
  for (const [label, mutate] of [
    ...
    // This split does not weaken the refusal on the destination the fault was found on: it still REFUSES.
  ...
  console.log("-- 5. CONTROL: the reason an unsplit reader produces stops the walk on the primary --");
  {
    // If the !checked arm threw freshnessError undifferentiated, a lost RUNLOG would classify to
    // REASON_FRESHNESS. Driven through the production walk, that is one attempt and a healthy replica never
    // read: the exact cost of conflating the two failure classes.
    const preChange = classifyRestoreFailure(freshnessError("freshness: RUNLOG absent"));
    ok("classifying a lost RUNLOG without the split yields REASON_FRESHNESS", preChange === REASON_FRESHNESS);
    let tries = 0;
    ...
    ok("that reason stops the walk on the primary after ONE attempt", tries === 1 && stopped.ok === false);
  ...
  console.log("-- 6. a primary that lost its RUNLOG falls to a replica that VERIFIES, and says so --");
  ...
  console.log("-- 7. it does NOT fall back when it should not, and a first-choice restore is unchanged --");
  {
    ...
    ok("and neither does its receipt, so a receipt with no walk canonicalises without the field", clean.receipt !== undefined && clean.receipt.destFallback === undefined);
