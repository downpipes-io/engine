...
//
// THE BOUND, AND THE SECOND QUESTION ABOUT IT. FRESHNESS_FAULTS_CAP bounds the record of downpipes
// whose staleness rule can NEVER arm, which exists to answer one ticket in the product's own
// words -- "the map said Fresh for weeks while my backups had stopped". The bound itself is right and this
// file proves it is: the map stops growing at exactly 50 and nothing beyond it is corrupted. The question
// worth asking is what the record SAYS when it refuses.
//
// IT SAID NOTHING, and it is not a ring. `recordFreshnessFault` does not evict the oldest to admit the
// newest; once full it admits NOBODY, so on a fleet larger than the cap the downpipes past it are ABSENT
// rather than stale, and the only sentence anywhere was a trailing comment reading "capped: the fleet-wide
// fact is already made".
//
// THE FLEET-WIDE FACT IS NOT WHAT THE PACK READS. `freshnessUncomputableIndex` joins this map onto
// downpipes[], and a downpipe in it carries `freshnessComputable: false`. A downpipe past the cap carries
// NO field, and the projector's convention is that an absent field does not apply. So the pack does not
// merely fail to mention the 51st downpipe: it ASSERTS that its staleness rule is fine, which is exactly
// the silent failure this cap exists to prevent, and the evidence bundle states it explicitly rather than
// leaving it to the map alone.
//
// A COUNT ALONE DOES NOT CLOSE IT. Declaring how many rows were dropped does not say WHICH SUBJECT,
// so the clean assertion can stay on a downpipe's row while the caveat sits in a different section. The
// refusal site already holds the id, so it emits it, and the row carries a third field.
//
// AND THE PRODUCT ALREADY OWNS THE REMEDY. capTruncations exists precisely for "the capped record that
// never says it was capped", with its own note naming the harm as "a truncated-away FAILED destination
// read as fully covered -- a MISSING RECOVERY COPY, invisible". It is the pack's one place to read "a
// record you are looking at was clamped", and freshness-faults is one of its registered surfaces.
//
...
//     subject is this site's choice and not a house convention. Driven, not read.
//   - THE PACK JOIN, MEASURED: a downpipe past the cap is absent from the fault map, which is why a count
//     alone could never reach its row.
//   - THE SUBJECT AND THE ROW: the refusal NAMES the downpipe it
//     refused, and `projectDownpipeRow` turns that name into `freshnessComputableUnknown` on the row. Past
//     the id list's own cap the record declares itself INCOMPLETE and every unmarked row takes the caveat.
//   - POPULATION: a genuinely unaffected downpipe must still carry NEITHER field, because a marker on every
//     row would discriminate nothing and "no row asserts health" would be vacuously true.
//   - INSTRUMENT CONTROL: a DECLARED surface books a truncation on this same rig in this same session, so
//     a zero result elsewhere reflects a clean surface rather than a broken probe.
...
console.log("freshness-fault cap honesty (record and declaration)\n");
...
//
// THE COUNT ALONE IS NOT ENOUGH. The fault map is what `freshnessUncomputableIndex` joins onto
// downpipes[], a downpipe past the cap is ABSENT from it, so it carries NO field, and the projector's
// convention reads an absent field as "does not apply". A count of dropped rows cannot say WHICH row was
// dropped, so the clean assertion can stay on a row while the caveat sits in a different section. The
// checks below hold for the fault MAP on its own; the row also needs to be joined against the subject
// ledger, checked next.
...
// ---- REDACTION -----------------------------------------------------------------------------------
//
// THE ARGUMENT. Keeping the dropped row's id out of the ledger looks like the safer redaction posture, but
// it is not, because a count alone cannot reach the row it describes. The subject ledger carries the id,
// and doing so is redaction-NEUTRAL rather than a widening, because the id emitted is EXACTLY the id that
// would have been the KEY of this map had there been room -- the same key space `schedDiag.freshnessFaults`
// and `downpipes[].id` already carry verbatim. Declaring a REFUSAL of a key cannot disclose a class the
// ADMISSION of the same key does not. What must never ride is anything BESIDE the key, and that is what the
// checks below pin.
