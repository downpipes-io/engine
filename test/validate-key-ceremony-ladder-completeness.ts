...
//
// WHY THIS EXISTS, and it is a criticism of the gate that already passes. R-10 / G-P0-001 was the first key
// install that returned 200, wrote all four secrets and left no keys-installed row on a contiguous chain:
// each Cloudflare secret PUT rolls a NEW WORKER VERSION, a new version replaces the script, and replacing a
// Worker's script RESETS its Durable Objects, so the append that follows races the rollout. It was repaired
// twice (cc631198 shipped the ladder; 82656241 widened the predicate to the OPAQUE form the runtime actually
// raised, which is why the first repair kept losing rows), and both repairs applied the remedy to SIX call
// sites, because every one of them follows a secret PUT or DELETE and so sits in the same rollout window.
//
...
// `wrangler tail` carried on the live request that lost a row (82656241); the named one is what the runtime
// says when it names the reset.
