...
// authenticator holding the private half, so seven orphaned
// virtual-authenticator keys on `updfault` keep answering "a second factor is present". The witness is the
...
export async function run(): Promise<void> {
  ...
  // whose row has been deleted.
  {
    ...
    // Delete the role row the way deleteRole does, leaving the credential behind. This is the measured
    // residue: five such credentials were found on one live estate.
