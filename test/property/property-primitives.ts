// Property-based tests for the crypto PRIMITIVES (src/crypto/primitives.ts): the seal's core. Until now no
// test exercised hkdfSha384 / hmacSha384 / aesGcmSeal / aesGcmOpen by name, so the stryker mutation gate (which
// mutates primitives.ts) left survivors there. These properties pin the behaviour that matters for a backup
// tool: AES-256-GCM round-trips and REJECTS any tamper / wrong key / wrong AAD / wrong nonce length; the 16-byte
// tag is appended; AAD is bound and an empty AAD equals no AAD; HKDF-SHA-384 is deterministic, returns exactly n
// bytes and is salt/info sensitive; HMAC-SHA-384 is deterministic, 48 bytes and key sensitive; SHA-384 is 48
// bytes, deterministic and avalanches. In the stryker oracle. Run: node test/property/property-primitives.ts
