...
async function main(): Promise<void> {
  ...
  // excluded; a DO/email/service binding is skipped before any storage tier matches.
  {
    ...
    // NON-ENUMERABLE own property (the shape a live estate showed: the binding answers direct env[name]
    ...
    // A typeof-"function" value is excluded before the duck-type ladder runs: a live secrets_store_secret
    // binding measures as typeof "object", never "function", so accepting a typeof-"function" value would
    // misclassify a callable double without matching any real binding shape (see the function's own header
    ...
    ok("setup: the callable binding double is typeof \"function\" (not the live binding shape)", typeof callableSecret === "function");
    ...
    ok("enumerateBoundSources excludes a typeof-\"function\" value, not surfaced as a secret", !outCallable.secrets.includes("MY_CALLABLE_SECRET"));
    ok("enumerateBoundSources still enumerates the object-typed KV binding alongside it", outCallable.kv.includes("MY_KV"));

    // The measured live shape: a generic Secrets Store binding answers typeof "function" for ALL TWELVE
    // method names this ladder checks (a generic, property-name-agnostic capability stub -- its own
    // Object.getOwnPropertyNames is empty), while every other live binding on the same estate (KV, R2, D1,
    // three Durable Object bindings) matches exactly one coherent capability group and no other. This
    // fixture reproduces that measured shape directly: a plain object (typeof "object", so it reaches the
    // ladder without needing the excluded typeof-"function" widening) whose get/put/list/head/
    ...
    ok("setup: the generic-stub double is typeof \"object\" (the measured live shape, not a typeof \"function\" guess)", typeof genericStub === "object");
    ...
    // The multi-group threshold (>= 3) has real headroom, not a coincidence of the one shape measured
  ...
  // ---- 4. fetchArtefactBytes: https-only, bounded, fail-null -------------------------------------
  {
    ...
    const got = await withFetch(okFetch, () => fetchArtefactBytes("https://update.downpipes.io/bundle"));
    ...
    ok("fetchArtefactBytes returns null on a non-2xx response", (await withFetch(notOkFetch, () => fetchArtefactBytes("https://update.downpipes.io/x"))) === null);

    const emptyFetch: typeof fetch = async () => new Response(new Uint8Array(0), { status: 200 });
    ok("fetchArtefactBytes returns null on an empty body", (await withFetch(emptyFetch, () => fetchArtefactBytes("https://update.downpipes.io/x"))) === null);
    ...
    ok("fetchArtefactBytes returns null over the 30 MiB cap", (await withFetch(bigFetch, () => fetchArtefactBytes("https://update.downpipes.io/big"))) === null);

    const throwFetch: typeof fetch = async () => { throw new Error("network"); };
    ok("fetchArtefactBytes swallows a thrown fetch and returns null", (await withFetch(throwFetch, () => fetchArtefactBytes("https://update.downpipes.io/x"))) === null);
  }
  ...
  // COMPLETE stored destination so it returns a non-null config without throwing, then the op decides.
  {
    ...
    // AVAILABILITY failures beyond a bare 404 must ALSO fall over to a replica. The most common real
    ...
    // REASON_RECOVERY_CHECK. A wrapper that falls back ONLY on "object missing" would surface each of
    // these primary failures outright and never read the healthy replica.
    ...
    // INTEGRITY GUARD: a TAMPER / verification failure on the primary is a real corruption signal, NOT a
  ...
  {
    // CF_ACCOUNT_ID set does not short-circuit the API call -- resolveDiscoveryAccounts must
  ...
  // ---- 11. resolveEngineAccountId: marked | env | single-visible | ambiguous | none --------------
  {
    ...
    // resolveDiscoveryAccounts never reads env.CF_ACCOUNT_ID at all, so a bad env shape here cannot
    // make it throw -- the type guard alone routes around it, stubbed so it never touches a real
    // network.
