// Proves the audit READ paths -- readAudit (GET /admin/audit), verifyAudit (GET /admin/audit/verify),
// exportAudit's WITH-limit branch (the support pack's per-action keystone probe, action=X&limit=1), and
// loadAuditHead's migration-reconstruction fallback -- all funnel through listAuditEntries()/a direct
// list(), never a SINGLE UNBOUNDED storage.list({prefix: AUDIT_PREFIX}) with no limit and no cursor. If
// the platform caps a single list() at ~1000 keys (this codebase asserts it repeatedly, e.g.
// scheduler-do-limits.ts / validate-fleet-scale.ts, though current Cloudflare docs are ambiguous on it),
// a chain longer than a page would otherwise silently TRUNCATE: readAudit would show a stale head,
// verifyAudit would under-check the chain, and a keystone probe hunting for the latest occurrence of an
// action could settle for a stale match from the first page instead of the true latest, the same defect
// shape the whole-log (no-limit) export path is guarded against.
//
// listAuditEntries() (the ONE method all four sites read through) and loadAuditHead()'s
// reconstruction fallback read via listAllByPrefix, the same bounded startAfter-cursor loop the
// whole-log export path uses, so the read is complete regardless of whether the ~1000-key cap is real.
//
...
// 1000 events does not, by itself, reproduce a truncation against an unbounded read under this mock -- a
...
// `startAfter`), so this file combines two independent checks that together pin the behaviour:
//
...
//     and that the >1-page chain issues MULTIPLE such calls, cursoring on `startAfter`. This assertion
//     holds only when the read is genuinely bounded: an unbounded read issues exactly one list() call
//     with no `limit` at all, and reverting to that turns every assertion in this section red, which is
//     the practical negative control.
//
...
async function main(): Promise<void> {
  ...
  ok("readAudit's underlying read is NEVER an unbounded list() (every call carries limit: DO_LIST_PAGE)", readCalls.every((c) => c.limit === DO_LIST_PAGE));
  ...
  ok("verifyAudit's underlying read is NEVER an unbounded list() (every call carries limit: DO_LIST_PAGE)", verifyCalls.every((c) => c.limit === DO_LIST_PAGE));
  ...
  ok("keystone probe's underlying read is NEVER an unbounded list() (every call carries limit: DO_LIST_PAGE)", probeCalls.every((c) => c.limit === DO_LIST_PAGE));
  ...
  ok("loadAuditHead's reconstruction read is NEVER an unbounded list() (every call carries limit: DO_LIST_PAGE)", rebuildCalls.every((c) => c.limit === DO_LIST_PAGE));
