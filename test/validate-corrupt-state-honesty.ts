...
//
// WHAT THIS PINS, AND WHY IT IS ONE FILE RATHER THAN THREE. An earlier pass corrupted stored state
// systematically and graded the SENTENCE each surface answers with, and three defects came back with one
// shape: the damaged state was byte-identical to a healthy one, so the surface was read as an all-clear.
// They are pinned together because the property is one property, and a later reader who fixes one of these
// sites needs to see the other two beside it.
//
