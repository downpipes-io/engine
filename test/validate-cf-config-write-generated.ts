...
// WHY THIS EXISTS
// ---------------
// 30 of the 47 surfaces carrying a write() were generated from Cloudflare's published schema. Until now
// the only thing exercising them was test/live-cf-writer-sweep.ts, which needs a real Cloudflare account
// and therefore never runs in CI. So the contract that keeps them safe, that an unproven writer does not
// run unless an operator names it, was enforced by code with nothing asserting it.
//
// That contract is the whole reason the catalogue can be complete without the product over-claiming. If
// `resolveCfConfigSurfaces` ever defaults to every write() instead of the proven set, a restore that
// named no scope would reach 30 writers whose natural key has never been tested against real data, and
// the adversarial review demonstrated what that costs: a Zero Trust posture rule overwritten with
// another platform's criteria, reported as success.
//
...
// that file, once in the gate and once in the branch that explains why a surface was excluded, so a
// substring check passes even with the gate deleted. Verified by deleting it: the weak version stayed
// green. The condition must guard the configPlan.push itself.
