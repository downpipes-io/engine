...
async function main(): Promise<void> {
  ...
  console.log("-- destStatusOf: endpoint parse + catch, keys vs sts, default flag, every optional spread --");
  {
    ...
    // authMode NAMES THE MECHANISM THE WRITES WILL ACTUALLY USE. Reporting "keys" for an Entra destination
    // would not be merely imprecise: the console renders an Authentication row only for "sts", so an Entra
    // destination would show NO authentication row at all and a customer could not tell from the console
    // that it signs in as a service principal rather than with a storage key.
  ...
  console.log("-- buildDestRecord: secret kinds, https + completeness rejections, optionals, label/probe/verifiedAt --");
  {
    ...
    // The currency label. validateDestPricing promises a TRIMMED label: trimming and THEN slicing to 8
    // would break that promise on its own output, because a value whose only non-space tail sits past the
    // cut would come back carrying whitespace. The truncation stays (a label's tail costs detail rather
    // than identity, and it names nothing elsewhere); trimming again after the slice is what makes the
    // promise true. Both the clean baseline and the exact boundary must still pass, so the label is never
    // over-truncated.
    {
  ...
  console.log("-- auditDestChange: force-orphan removal, default promotion, rejected set, reject-class map --");
  {
    ...
    // default promotion (dest-removed-force-orphans-runs + dest-default-promotion-silent-redirect).
    {
      ...
      // The count answers "how bad" and the names answer "who": a whitelist that only forwards the count
      // while silently dropping the names understates the audit record. This reads the WRITTEN audit record
      // end to end, not a hand-built target, which is the difference between proving the projection and
      // proving the producer.
      ok("a forced removal also records WHICH downpipes lost their only proven copy (names, not just the count)", JSON.stringify(t.affectedDownpipeNames ?? []) === JSON.stringify(["Fan"]));
    }
