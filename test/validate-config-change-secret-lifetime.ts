...
async function main(): Promise<void> {
  ...
  const { call, sched, setGate, pendingChanges, ageChange } = ctx;
  ...
  try {
    ...
    // ===========================================================================================
    {
      ...
      // /admin/config/snapshot does NOT work here and the first draft of this section used it: the snapshot
      // route de-dupes an unchanged posture, so the head never moved and the stale approve simply applied.
      // That failure mode is worth leaving written down, because it presented as the fix not working.
      ...
      // Aged so the dwell floor does not itself intercept before the base-moved guard C20/C21 depend on.
      ageChange(supersedeTarget.id);
    ...
    // ===========================================================================================
    {
      ...
      // ways, decided by whether the params happened to hold a credential, which is the exact defect the
      // owner-action fix hit and had caught for it. The control is REJECTED, matching the scrubbed
      // record's terminal state exactly: comparing a rejected record's refusal with an applied one's would
      ...
      // Aged so the dwell floor does not itself intercept before the integrity recompute D9 depends on.
      ageChange(tamperTarget.id);
