// This module covers the RESPONSE-LEVEL (envelope) signature group of the real-world-IdP interop corpus
// (Okta / Entra / OneLogin default). The orchestrator in validate-saml-interop.ts imports run() and calls it
// with the shared harness and keys; every assertion here runs in the declared order.
...
export async function run(h: Harness, keys: Keys): Promise<void> {
  ...
  // ========================= RESPONSE-LEVEL (envelope) SIGNATURE =========================
  console.log("Response-level signature (Okta / Entra / OneLogin default):\n");
  ...
  // consumed principal is the legit one.
  {
    ...
    h.ok("CONTROL: a Response-signed message with an unsigned assertion is ACCEPTED", r.ok === true);
    if (r.ok) {
  ...
  // negative 1: a Response-signed message + a SECOND unsigned sibling assertion. The exactly-one-direct-
  ...
  // second assertion.
  {
    ...
    const signedResp = await signResponse(resp, rsa.privateKey);
    const r = await verifySamlResponse(encodeResponse(signedResp), baseConn([certPem]), baseCtx());
    h.ok("neg: Response-signed + a second sibling assertion is rejected (exactly-one-Assertion)", r.ok === false);
  }

  // negative 2: take a VALIDLY Response-signed message and MUTATE the assertion's NameID afterwards. The
  ...
  // signature genuinely binds the assertion bytes (no swap behind a valid envelope signature).
  {
    ...
    const r = await verifySamlResponse(encodeResponse(signedResp), baseConn([certPem]), baseCtx());
    h.ok("neg: mutating the assertion under a valid Response signature is rejected (Response digest mismatch)", r.ok === false);
  }

  // negative 3: the classic XSW against the envelope path - a forged unsigned assertion in the canonical
  ...
  // requireReferenceIsTarget + unique-ID walk reject (Reference resolves to no element / not the root).
  {
    ...
    const r = await verifySamlResponse(encodeResponse(signedResp), baseConn([certPem]), baseCtx());
    h.ok("neg: a Response signature whose Reference no longer resolves to the root is rejected", r.ok === false);
  }

  // negative 4: Response signed by a NON-pinned (attacker) key -> reject (the pinned cert governs; KeyInfo
  // is ignored just as for the assertion path).
  {
    ...
    const r = await verifySamlResponse(encodeResponse(signedResp), baseConn([certPem]), baseCtx());
    h.ok("neg: a Response signed by a non-pinned key is rejected", r.ok === false);
  }

  // negative 5: a Response-signed message whose Status is NOT Success still rejects (Status gate runs
  ...
  // way it must reject.
  {
    ...
    const r = await verifySamlResponse(encodeResponse(signedResp), baseConn([certPem]), baseCtx());
    h.ok("neg: a non-Success Status on a Response-signed message is rejected", r.ok === false);
  }

  // negative 6: a Response-signed message with the wrong @Destination -> reject (envelope wrong-SP gate).
  {
    const resp = buildResponse([buildAssertion()], { destination: "https://attacker.example/acs" });
    const signedResp = await signResponse(resp, rsa.privateKey);
    const r = await verifySamlResponse(encodeResponse(signedResp), baseConn([certPem]), baseCtx());
    h.ok("neg: a wrong @Destination on a Response-signed message is rejected", r.ok === false);
  }

  // mixed: an assertion-level signature ALSO works when present (the OR semantics) - this is the
  ...
  // pipeline (regression guard that adding the Response path did not break the assertion path).
  {
    ...
    h.ok("mixed: an assertion-level signature still verifies (OR semantics preserved)", r.ok === true);
  }

  // belt-and-braces: a doc with BOTH an assertion-level AND a Response-level signature is ACCEPTED via the
  ...
  // We build it by signing the assertion, putting it in a Response, then signing the Response too.
  {
    ...
    h.ok("both-signed: accepted via the assertion-level signature (its own signature genuinely covers it)", r.ok === true && r.principal.nameId === LEGIT_NAMEID);
  }
}
