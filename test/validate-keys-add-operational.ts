...
//      is audited.
//   7. B7: a successful add CLEARS the durable OPERATIONAL_RETIRED marker (best-effort: a failed clear
...
function makeSecretsStub(): { fetch: typeof fetch; puts: Put[]; dels: string[]; calls: string[] } {
  ...
  const fetchImpl = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    // The B7 marker-clear DELETE (best-effort): succeed it here so the happy-path tests below see the
...
async function main(): Promise<void> {
  ...
  console.log("\n-- addOperationalSecrets: GOOD KEYS -> exactly two PUTs, in order, correct values --");
  {
    ...
    // B7: after a successful add, the OPERATIONAL_RETIRED marker is cleared (best-effort), and only AFTER
