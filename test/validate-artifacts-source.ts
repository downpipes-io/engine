...
console.log("\n-- per-namespace fail-open: an UNAVAILABLE MARKER, never a silent drop --");
// A transient 5xx that survives retry (or a scope gap, or an inner CfPaginationTruncated) on ONE
// namespace's repo list must NOT silently drop every repo in that namespace with no marker and no
// recordsIncomplete signal (a bare `continue` would leave `fo` holding only "good/ok" and the run
// reporting a clean "ok"). Instead a "<namespace>/_unavailable" sentinel record names the namespace,
// the same shape stream/images emit and that seal/marker.ts isIncompleteMarkerValue + the
// recordsIncomplete counter detect, so the operator is told the namespace is short.
...
// ---------------------------------------------------------------------------
// RESUMABILITY: a registry with more repos than fit one slice resumes from the
...
// the same order.)
// ---------------------------------------------------------------------------
console.log("\n-- crawlFrom is RESUMABLE (marks after each repo, resume skips the yielded prefix) --");
{
...
// ---------------------------------------------------------------------------
// A repo whose CONTENT walk (includeContent) exceeds one invocation's subrequest budget must NOT run to
// the platform cap with no checkpoint and wedge/restart the run. Making the source-LIST resumable is not
// enough on its own: a per-repo content walk that only emits a {mark} at the COARSE per-repo boundary
// leaves a single huge repo's content walk with no resume mark, and the run wedges. Instead an INTRA-repo
// {mark} (carrying an `inRepo` cursor) is emitted after a content record whenever the slice budget is low,
// and a resume from that cursor replays the repo's deterministic walk, skipping the already-yielded content
// records, then continues to completion across slices.
//
...
console.log("\n-- the per-repo content walk CHECKPOINTS mid-repo and resumes across slices --");
...
// repo's walk never yielded and the run wedged.
{
  ...
  ok("an INTRA-repo mark (carrying an inRepo cursor) is emitted mid content walk", intraRepoMarks.length > 0);
