...
// have an equivalent for (ENG-M7, ENG-M4). The wired vector harness (validate-reader.ts)
...
// Invalid UTF-8 / lone surrogates (ENG-L4). A string carrying an unpaired surrogate has no
...
// (Go resolvePath), not by scanning the source text for the leaf name. ENG-M6 was a
// substring scan that resolved the wrong field on a leaf-name collision; these cases fail
// if that regression returns. Field range/canonicality is also asserted here.
// ---------------------------------------------------------------------------------------
...
// so both must throw (the exact ENG-M7 divergence the Go reader refuses).
...
// bundled file must fail (ENG-M4). Mirrors bundle_test.go TestBundleWriteVerify.
// ---------------------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------------------
// reader structural rejection (ENG-L2, SPEC 7.4): a record that carries a packed slice on a
...
// ---------------------------------------------------------------------------------------
// signer fingerprint (ENG-L3, SPEC 11.4): the root's signingKeyFingerprint must be the REAL
