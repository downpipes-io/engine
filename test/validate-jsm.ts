...
async function main(): Promise<void> {
  console.log("format()/formatClose(): pure shape correctness");
  {
    ...
    // The source field is capped at 100 chars on both format() and formatClose(), so an unusually long
    // downpipe name cannot land oversized on a short-label field.
    const longName = "N".repeat(150);
    const pLongSource = format({ ...EMISSION, downpipeName: longName });
    ok("format: source is capped at 100 chars", pLongSource.source.length === 100 && pLongSource.source === longName.slice(0, 100));
    const cLongSource = formatClose({ ...EMISSION, downpipeName: longName });
    ok("formatClose: source is ALSO capped at 100 chars", cLongSource.source.length === 100);
    const pShortSource = format({ ...EMISSION, downpipeName: "short" });
    ok("format: a short name is untouched (no padding, no needless truncation)", pShortSource.source === "short");
  }
  ...
  console.log("\ndeliver: CREATE POSTs to the channel's OWN url with GenieKey auth; body is the create shape; no-custody (token never in body/url)");
  {
    ...
    // This create answers 202 with a non-JSON body (no requestId to poll), so the bare 202 is honestly
    // reported as accepted-but-unconfirmed, never silently folded into a plain "delivered".
    ok("jsm create: a 202 with no parseable requestId is reported unconfirmed:true", result.unconfirmed === true);
  }
  ...
  console.log("\ndeliver: a RECOVERED emission CLOSEs the SAME alias at the documented close-by-alias path, same auth, no-custody");
  {
    ...
    // A plain 200 (not 202) is a SYNCHRONOUS confirmation, so no unconfirmed caveat applies.
    ok("jsm close: a plain 200 carries no unconfirmed flag", result.unconfirmed === undefined);
    const last = captures[captures.length - 1];
  ...
  console.log("\ndeliver: a 202 async-accepted response -- requestId capture + the single Get Request Status poll");
  {
    ...
    // A 202 whose body carries a requestId, and the poll POSITIVELY confirms success -- a fully
    // confirmed delivery (ok:true, no unconfirmed caveat). Exactly TWO requests: the create, then ONE poll.
    {
      ...
      try {
        const r = await deliver(env, channel, EMISSION);
        ok("a poll-confirmed 202 is ok:true with NO unconfirmed caveat", r.ok === true && r.unconfirmed === undefined);
        ok("exactly 2 requests (the create, then ONE poll)", calls.length === 2);
        ok("the poll GETs the documented Get Request Status endpoint (same base, /requests/{id})", calls[1]?.url === "https://api.opsgenie.com/v2/alerts/requests/req-abc-123" && calls[1]?.method === "GET");
        ok("the poll carries the SAME GenieKey auth header", calls[1]?.headers.Authorization === "GenieKey genie-secret-poll");
      } finally {
        globalThis.fetch = realFetch;
      }
    }

    // A 202 with a requestId, but the poll POSITIVELY confirms FAILURE (data.success:false) -- still
    ...
    // integration every tick), but honestly flagged unconfirmed:true, never a silent plain "delivered".
    {
      ...
      try {
        const r = await deliver(env, channel, EMISSION);
        ok("a poll that confirms FAILURE is still ok:true (idempotent retry-safe) but unconfirmed:true, and ackOutcome names it async-create-failed", r.ok === true && r.unconfirmed === true && r.ackOutcome === "async-create-failed");
      } finally {
        globalThis.fetch = realFetch;
      }
    }

    // A 202 with a requestId, but the poll itself fails (503) -- cannot confirm, so unconfirmed:true;
    // never a throw, never ok:false (the create itself DID succeed).
    {
      ...
      try {
        const r = await deliver(env, channel, EMISSION);
        ok("a failing poll degrades to unconfirmed:true (never ok:false, never a throw), ackOutcome confirmation-unavailable", r.ok === true && r.unconfirmed === true && r.ackOutcome === "confirmation-unavailable");
      } finally {
        globalThis.fetch = realFetch;
      }
    }

    // A 202 whose body carries NO requestId at all -- no poll is even attempted (nothing to poll),
    // so it stays unconfirmed:true by default. Exactly ONE request (the create only).
    {
      ...
      try {
        const r = await deliver(env, channel, EMISSION);
        ok("a 202 with no requestId is unconfirmed:true and polls nothing (1 request only)", r.ok === true && r.unconfirmed === true && calls.length === 1);
      } finally {
        globalThis.fetch = realFetch;
      }
    }
  ...
  {
    // The guards emit disjoint codes (no-url / no-credential) rather than one collapsed no-transport code, so
    // "my ServiceNow/JSM channel never delivers" can say whether the url, the username, the credential or the
    // recipient list is the missing field -- a missing field is the single commonest cause of a channel that
    // silently never fires. This is the same discipline the sibling credential-undecryptable assertion below
    // pins: the pack must point at the field to fix, never at "re-enter everything".
    const env = {} as unknown as Env;
    const noUrl: NotifyChannel = { id: "c1", kind: "jsm", name: "Ops", apiKey: "k", enabled: true, createdAt: "2026-01-01T00:00:00.000Z" };
    ok("jsm: channel with no url -> no-url", (await deliver(env, noUrl, EMISSION)).code === "no-url");
    ...
    // The invariant: a defensive guard never throws and never delivers -- it returns a closed, non-delivering
    // code.
  ...
  console.log("\nKEEP-SECRET (DO addNotifyChannel): a first jsm create needs an apiKey; an edit that omits it keeps the prior sealed value; a set WITH a new one rotates it");
  {
    ...
    // allowInternalSink persists. A url change requires re-supplying the apiKey (keep-secret is tied to the
    // endpoint identity), so this repoint carries a fresh one.
    ...
    ok("allowInternalSink actually PERSISTS on the stored channel", withOverride.allowInternalSink === true);
  ...
  console.log("\nthe router seam: a whitespace-only apiKey is STRIPPED at wrapNotifyChannelSecret, never forwarded as an unwrapped value");
  {
    const KEY = crypto.getRandomValues(new Uint8Array(32));
