...
//
// A hard-coded exemplar breaks whenever the named surface gains a writer and stops being an example of the
// case, and the assertion then goes red for a reason that has nothing to do with the rule it tests, which is
// a false failure and costs a diagnosis. Which surfaces are read-only moves continuously as the proving work
// lands, so the fixture asks the registry instead of remembering.
//
...
// ---- WINDOWED-RESTORE HONESTY: maxRecords < the matched set leaves records UNRESTORED, and the
...
// ---- GRANULAR single-record restore: recordName plans + applies EXACTLY one record ----
...
async function proofGranular(fx: Fixture): Promise<void> {
  const { baseEnv } = fx;
  {
    ...
    // THE OTHER HALF. The plan states its resolved surface set (asserted elsewhere) and the hash must
    ...
    // bound nowhere that a test could see.
    {
      ...
      ok("WIDENING the surface set changes the plan hash, so an old approval cannot cover it", narrow !== wider);
      ...
      ok("narrowing changes it too, so the two are not interchangeable", wider !== narrow);
...
//
// A plannedWrites count lower than the run's record count, with skipped empty, would point at one of two
// places: buildSourceBindingMap's dp.source/dp.config.source shape, or archive-listing pagination. Neither
...
//      (restore-sinks.ts:348) means an explicit target -- which every APPLYING harness journey must supply,
//      the redirect-into-a-scratch-binding rule -- makes sourceBindings irrelevant to the plan (part (c)
//      below). And on the original-bindings path it resolves, it is ALL-OR-NOTHING per shared binding (parts
//      (a)/(b)), so it cannot split 1-of-4 records that share one namespace.
//
...
// plannedWrites lower than the run's record count with skipped=none cannot come from buildRestorePlan/
// runDryRun on a complete run: the two never disagree by construction (asserted here as an accounting
// invariant), so any occurrence of skipped=none alongside a real shortfall points OUTSIDE this file -- at
// the request the caller actually sent, or at which of two responses a driver captured -- not at this
// classification loop.
async function proofPlanAccountingAndDeterminism(fx: Fixture): Promise<void> {
  ...
  // the 3 records must be accounted for: NOT a silent 0.
  {
    ...
    // The block's own heading says every record must be ACCOUNTED FOR rather than silently dropped, and the
    // next two lines prove exactly that: all three are named in skipped with the honest reason, and the
    // accounting invariant balances. A plan that can write NOTHING, over a run that holds three records, must
    // not report itself ok and complete: the engine's own kv-binding-fallback proof warns about this shape in
    // its header -- "a fixture invented from the same assumption as the code proves only that they share it"
    // -- and this is that, one file over. Reporting ok:true complete:true over records that are all
    // unwritable would put a false-clean verdict on the last screen an operator reads before authorising a
    // write-back.
  ...
  // (b) ORIGINAL bindings, sourceBindings CORRECTLY resolved (buildSourceBindingMap, reading config.source
  // and keying kv:<namespaceId>): the SAME 3 records, all planned, nothing skipped. Proves the function's
  // OUTPUT actually changes resolveSink's outcome end-to-end (the fallback validator proves the map's shape
  // in isolation; this connects it to a real plan).
  {
    const env = baseEnv();
    delete env[`KV_${NS}`];
...
// trip the platform subrequest cap mid-apply and half-complete with no clean resume. The restore counts
// media + cf-config toward MAX_IN_ACCOUNT_RESTORE_RECORDS, refuses an oversized combined restore before any
// write, and bounds a windowed restore so it proceeds in resumable batches (windowed/outOfWindow/complete:false),
// rather than counting only the data `plan` and leaving a media/cf-config-heavy restore UNBOUNDED. ----
async function proofInAccountBoundsMedia(fx: Fixture): Promise<void> {
...

  {
    ...
    // cf-config surfaces = 213 combined in-account writes, over the 200 ceiling. The data `plan` is EMPTY, so
    // a ceiling check scoped to data alone would pass and attempt all 213 writes; counting media + cf-config
    // toward the ceiling refuses BEFORE any upload/cf-config write, steering to the offline CLI.
    ...
    // stays true for the intentional partial. maxRecords bounds the combined set, not only the data records.
    const BATCH = 120;
  ...
  const { baseEnv, r2, signer, breakGlass, op } = fx;
  {
    ...
    const ovArchive = await buildArchive({
      ...
      records: [
        ...
        // An unknown sourceType is refused at READ by the KNOWN_SOURCE_TYPES gate before the planner ever
        // runs (proved separately below, which is strictly stronger), so the sinkless probe here is
        // "workers": it is IN KNOWN_SOURCE_TYPES, so the archive reads, and it has no write sink, so the
        // planner reports it out of band.
    ...
    // Bind the empty-name conventions so the no-name KV/R2 records resolve a present binding: both KV_ and
    // R2_ must be bound, or the "nor2" record resolves the absent R2_ convention and skips with "target
    // binding not present", riding unnoticed inside an ok===true meant to prove something else entirely --
    // that a SINKLESS record (wkr1, out of band) does not fail the run. Binding R2_ makes the assertion test
    // the thing it names, so its ok===true turns only on the out-of-band record it is actually about.
    ...
    // closed set is REFUSED at read, rather than being restored under a guessed behaviour. "vectorize" is a
    // reserved type deliberately absent from the set, so an archive carrying one must not open at all.
...
async function proofSummariseDiff(fx: Fixture): Promise<void> {
  const { baseEnv, r2, signer, breakGlass, op } = fx;
  {
    ...
    // The DNS write classifies a matched-but-edited record as a CHANGE. The extra live-only record stays LEFT
    // IN PLACE (additive restore never deletes) but is REPORTED, so the operator can see what the restore did
    // not touch: on an allow-list surface an unreported live extra is exactly what an attacker could have
    // added, and the restore must not say nothing about it.
...
// the apply both skip it (rec.incompleteMarker, the signed manifest field, adapter-asserted at seal time --
// never re-derived from the decrypted value), surface it out of band, do NOT count it restored, and stay
// ok:true. This proves the restore side of the _vanished marker and the over-ceiling R2 _skipped marker
// together, and that a real customer value which merely LOOKS marker-shaped -- with no adapter assertion
// behind it -- is never caught in the same net. ----
async function proofDataRecordMarkersSkipped(fx: Fixture): Promise<void> {
  ...
  // gone:mid-crawl and huge/object.bin carry an explicit incompleteMarker, exactly what pipeline.ts/slice.ts
  // stamp from a source adapter's OWN markerKind assertion at seal time (never derived by sniffing the
  // value's shape). real:looks-like-marker carries NO incompleteMarker: it is an innocuous real customer
  // object that happens to share a MARKER_KEYS name (_pending) with no adapter ever asserting it is
  // synthetic -- proving the restore-side collision: it must restore like any other real record, not be
  // silently dropped.
