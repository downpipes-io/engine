// VERIFY-ON-READBACK + SIGNED RECEIPT for the media re-upload path (images / stream), end to end through
// runRestore, in-memory only (no network). A media restore re-uploads the captured bytes and reads the
// uploaded object back to prove the write, and the media records are included in the signed restore
// receipt, so a media restore carries the same assurance as KV/R2/D1/secrets. This proves, within the
// platform's reality:
...
async function sectionHappy(): Promise<void> {
  ...
  ok("the IMAGE record is in the receipt (media entries are not excluded)", imgEntry !== undefined);
  ...
  ok("the STREAM record is in the receipt (media entries are not excluded)", vidEntry !== undefined);
