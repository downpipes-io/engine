// The self-update anti-rollback high-water FLOOR, proven net-zero and deploy-free. It drives the REAL engine
// floor-check and the REAL channel-signature verifier in process, the same way test/validate-residency-tier1.ts
// drives the real selection and fan-out functions.
//
// Three source facts this test turns on, and none of them need a deploy:
...
// refuses any recommended version strictly below this string, EVEN with allowDowngrade set (update-types.ts:184-
// 189). "0.1.10" is the harness self-update estate's high-water.
...
async function cellFloorRefusal(signer: TestSigner, verifier: HybridVerifier): Promise<void> {
  ...
  //    the anti-rollback floor (forward-only is bypassed), so a refusal at anti-rollback-guard proves the floor
  //    itself, not forward-only.
  ...
  ok("the refusing step is anti-rollback-guard, even with allowDowngrade set", antiStep !== undefined && antiStep.ok === false);
...
async function main(): Promise<void> {
  ...
  console.log("\nANTI-ROLLBACK FLOOR VECTORS PASS (the floor refuses a below-floor descriptor on the version string, before any upload; the wrong-key companion catches a masked verification bug; the above-floor refuter is cleared)");
}
