// Validates the CRON-PASS FAULT EVIDENCE the support pack reasons over, for the SIEM + OTLP push drains and
// the retention prune. Each of these passes takes an early `return` on a pre-delivery fault -- a malformed
// CONFIG_WRAP_KEY, an unreadable DO config/cursor/export/snapshot, a break-glass-only posture, an unloadable
// signer, an unreadable RUNLOG, an abstaining planner -- and without recording the fault, "our SIEM/metrics
// push silently stopped" and "storage keeps growing despite retention.enforce=true" would both reach support
// with an empty pack.
//
...
console.log("\nRetention prune: the pass record classes every skip/deferral/error");
{
  ...
  // BOTH DIRECTIONS of the `\bput\b` word-boundary match matter: a native binding throws "put:" and an S3 leg
  // writes "PUT seg/0001", and neither carries a trailing space, so a trailing-space pattern would miss both
  // and the pass would lose the label that says where to look. Meanwhile "input " does contain "put ", so a
  // trailing-space pattern would misclassify an unrelated validation message as a destination write failure
  // that never happened. Each case here exercises one of these directions.
...
console.log("\nRetention prune: the pass POSTS its record on the skip paths (the 'nothing ever gets deleted' ticket)");
{
  ...
  // all four assertions read a record that was never posted. Fifty sibling fixtures set the field; this one
  // did not, which is why it has never been green.
  const withRetention = [
  ...
  }
  {
    ...
    // recovers. A bare `catch {}` here would let the pack show the previous pass as the latest one with
    // nothing saying a record was lost. That pending count is the evidence; a zero here would mean the loss
    // went silent again.
