...
async function main(): Promise<void> {
  ...
  console.log("control-plane-seal: the CONFIG recipient opens the export, and opens nothing else");
  {
    ...
    // wipe without holding a key that opens customer archives. A break-glass-only engine, sealed only to
    // the archive recipient set, cannot auto-heal and requires an offline rebuild.
  ...
  // own destination bucket whenever sealing is off (the default state for a new customer). This branch
  // calls assertNoPlaintextSecretInExport itself, so a leaky export is refused here too, not only on the
  // SEALED branch that goes through sealControlPlaneExport.
  await throwsAsync(
  ...
  ok("bodyHash: absence does not fail the shape gate (back-compat with artefacts sealed before bodyHash existed)", isSealedControlPlaneExport(withoutBodyHash));

  console.log("control-plane-seal: verifySealedControlPlaneSignatureDetailed (the split verdict, for the sealed wrapper)");
