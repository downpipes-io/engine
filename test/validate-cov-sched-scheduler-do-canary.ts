...
// ---- the full multi-destination lifecycle (config clamps, due/lease, complete folding) -----------------
async function sectionLifecycle(): Promise<void> {
  ...
  ok("fresh canary defaults to enabled, all destinations, pending", v0.config.enabled === true && v0.flyingToAll === true && v0.status === "pending");
  ok("fresh view lists both destinations as pending with the default flagged", v0.dests.length === 2 && v0.dests.find((d) => d.destinationId === "r2")!.isDefault === true && v0.dests.find((d) => d.destinationId === "s3")!.isDefault === false && v0.dests.every((d) => d.status === "pending"));
  ...
  ok("a mid-range cadence is accepted verbatim", cMid.config.intervalSeconds === 7200 && cMid.nextRunAt === null);
  const cLow = (await (await doFetch(stub, "POST", "/canary/config", { intervalSeconds: 100 })).json()) as CanaryView;
  ok("a too-small cadence is clamped up to the 300s floor", cLow.config.intervalSeconds === 300);
  const cHigh = (await (await doFetch(stub, "POST", "/canary/config", { intervalSeconds: 999_999 })).json()) as CanaryView;
  ok("a too-large cadence is clamped down to the 86400s ceiling", cHigh.config.intervalSeconds === 86_400);
  ...
  ok("first flight is due with a per-destination plan and distinct runIds", due1.due === true && due1.run!.dests.length === 2 && due1.run!.dests[0]!.runId !== due1.run!.dests[1]!.runId);
  ok("the first flight has no prior cell to clean up", due1.run!.dests.every((d) => d.cleanupRunId === null));
  const due1b = (await (await doFetch(stub, "POST", "/canary/due", { nowMs: 1_000_100 })).json()) as { due: boolean };
  ok("a second due-check inside the lease is not re-flown", due1b.due === false);
  ...
  ok("exactly the dead destination transitions, labelled from the collection", t1.length === 1 && t1[0]!.destinationId === "s3" && t1[0]!.transitioned === "dead" && t1[0]!.label === "S3 Backup");
  const v1 = await getView(stub);
  ok("the aggregate is dead when any destination died, with a per-destination deadSince", v1.status === "dead" && v1.dests.find((d) => d.destinationId === "r2")!.status === "alive" && v1.dests.find((d) => d.destinationId === "s3")!.deadSince !== null && v1.dests.find((d) => d.destinationId === "s3")!.lastCheck !== null);
  ...
  ok("a due-check before nextRunAt is not due", dueEarly.due === false);
  ...
  ok("run-now makes the next flight due, carrying the prior cell to clean up", due2.due === true && due2.run!.dests.find((d) => d.destinationId === "s3")!.cleanupRunId !== null);
  ...
  ok("the recovered destination transitions back to alive", t2.length === 1 && t2[0]!.destinationId === "s3" && t2[0]!.transitioned === "recovered");
  ok("the aggregate is alive once every destination is alive again", (await getView(stub)).status === "alive");
  ...
  ok("a fresh death transitions to dead", t3.length === 1 && t3[0]!.transitioned === "dead");
  const t4 = await flyOnce(stub, [aliveOf("r2"), deadOf("s3")]);
  ok("an already-dead destination does not transition again", t4.length === 0);
  ...
  ok("an ailing flight sets the previously-alive destination to ailing", vAil.dests.find((d) => d.destinationId === "r2")!.status === "ailing");
  ok("an ailing flight cannot clear a sticky death", vAil.dests.find((d) => d.destinationId === "s3")!.status === "dead" && vAil.status === "dead");
  ...
  ok("pinning a subset stops flying to all and lists only the pinned destination", vPin.flyingToAll === false && vPin.dests.length === 1 && vPin.dests[0]!.destinationId === "r2");
  await flyOnce(stub, [aliveOf("r2")]);
  const vPruned = await getView(stub);
  ok("the de-selected destination is pruned from the per-destination state", vPruned.dests.length === 1 && vPruned.dests[0]!.destinationId === "r2" && vPruned.status === "alive");

  // Config rejections + no-op + the empty-selection-is-all-destinations rule + the null-is-all rule.
  ok("a pin to an unknown destination is refused 400", (await doFetch(stub, "POST", "/canary/config", { destinationIds: ["nope"] })).status === 400);
  const vEmpty = (await (await doFetch(stub, "POST", "/canary/config", { destinationIds: [] })).json()) as CanaryView;
  ok("an empty selection means all destinations", vEmpty.flyingToAll === true);
  ...
  ok("a null selection means all destinations", vNull.flyingToAll === true);
  const vNoop = (await (await doFetch(stub, "POST", "/canary/config", {})).json()) as CanaryView;
  ok("an empty config body changes nothing", vNoop.config.enabled === true && vNoop.flyingToAll === true);
  ...
  ok("disabling reads disabled and clears the schedule", vOff.status === "disabled" && vOff.nextRunAt === null && vOff.dests.every((d) => d.status === "disabled"));
  ok("a disabled canary is never due", ((await (await doFetch(stub, "POST", "/canary/due", { nowMs: 9_000_000 })).json()) as { due: boolean }).due === false);
  ok("run-now on a disabled canary is refused 400", (await doFetch(stub, "POST", "/canary/run-now")).status === 400);
  const vOn = (await (await doFetch(stub, "POST", "/canary/config", { enabled: true })).json()) as CanaryView;
  ok("re-enabling re-derives the aggregate liveness from the destinations", vOn.status === "alive" && vOn.nextRunAt !== null);
}

// ---- no console destinations -> the canary flies to the env default (the null id) ---------------------
async function sectionNoDestinations(): Promise<void> {
  const { stub } = makeScheduler();
  const v = await getView(stub);
  ok("with no destinations the canary flies to the single default (null) cell", v.dests.length === 1 && v.dests[0]!.destinationId === null && v.dests[0]!.isDefault === true && v.dests[0]!.label === "Default destination");
  ok("the destination collection is reported empty", v.destinationCount === 0 && v.allDestinations.length === 0);
  const due = (await (await doFetch(stub, "POST", "/canary/due", { nowMs: 1 })).json()) as { due: boolean; run?: CanaryFlightPlan };
  ok("the default-cell flight is due with one null-destination descriptor", due.due === true && due.run!.dests.length === 1 && due.run!.dests[0]!.destinationId === null);
  ...
  ok("a dead default cell transitions with the default-destination label", t.length === 1 && t[0]!.destinationId === null && t[0]!.label === "the default destination" && t[0]!.transitioned === "dead");
}

// ---- pinned ids that later dangle -> the default (or the null cell) is flown instead ------------------
async function sectionDanglingPins(): Promise<void> {
  ...
  ok("when every pin dangles the canary falls back to the default destination", vDefault.flyingToAll === false && vDefault.dests.length === 1 && vDefault.dests[0]!.destinationId === "c");

  // The pins that no longer resolve are NAMED in the view. Without this field, the drop would be silent:
  // the pin would vanish from `dests` and the aggregate would go on reading healthy for whatever remained,
  // so an operator who had deliberately pinned a bucket could lose that proof with nothing on screen.
  ok("the view names both dangling pins", JSON.stringify([...vDefault.danglingPins].sort()) === JSON.stringify(["a", "b"]));
  ...
  ok("with dangling pins and no destinations at all the canary flies the null cell", vNull.dests.length === 1 && vNull.dests[0]!.destinationId === null);

  // A PARTIAL dangle: one pin survives, one is gone. The all-dangling case records a fault-ledger entry;
  // this asserts the partial case is equally visible, so the canary does not quietly stop proving one
  // destination the operator chose.
  {
    ...
    ok("a partial dangle still flies the surviving pin", v.dests.length === 1 && v.dests[0]!.destinationId === "keep");
    ok("and names ONLY the pin that no longer resolves", JSON.stringify(v.danglingPins) === JSON.stringify(["gone"]));
  }
  ...
  // able to treat non-empty as "something is wrong" without also handling undefined.
  {
    ...
    ok("a healthy pinned canary reports no dangling pins", Array.isArray(v.danglingPins) && v.danglingPins.length === 0);
  }
  ...
  // Flying to all: there are no pins, so there is nothing that can dangle, even after the collection changes.
  {
    ...
    ok("flying to all reports no dangling pins even after the collection is replaced", v.flyingToAll === true && v.danglingPins.length === 0);
  }
}

// ---- migration of the pre-multi-destination single-record shapes ----------------------------------
async function sectionMigration(): Promise<void> {
  // A. an OLD record pinned to one destination, with a last run + history + a death stamp + a pass count.
  {
    ...
    const v = await getView(stub);
    ok("an old pinned record migrates to flying to exactly that destination, liveness + history preserved", v.flyingToAll === false && v.dests.length === 1 && v.dests[0]!.destinationId === "r2" && v.dests[0]!.status === "alive" && v.dests[0]!.lastCheck !== null && v.history.length === 1);
  }
  // B. an OLD DEFAULT record (no pin, no last run, no history) -> the all-destinations default, empty state.
  {
    ...
    const v = await getView(stub);
    ok("an old default record migrates to flying to all destinations with no per-destination state", v.flyingToAll === true && v.dests.length === 1 && v.dests[0]!.destinationId === "r2" && v.dests[0]!.status === "pending");
  }
  ...
  //    per-destination state seeds with a null latest-check and a null deadSince (the ?? fallbacks).
  {
    ...
    const v = await getView(stub);
    ok("an old record with a run but no history migrates with a null latest-check and null deadSince", v.dests.length === 1 && v.dests[0]!.destinationId === "r2" && v.dests[0]!.status === "alive" && v.dests[0]!.lastCheck === null && v.dests[0]!.deadSince === null);
  }
}

// ---- the in-flight lease + reclaim edges (seeded states the normal flow cannot reach mid-stream) -----
async function sectionLeaseEdges(): Promise<void> {
  ...
  // inFlight but NO inFlightSince (a record from before the lease stamp) -> the lease guard cannot hold
  //     it (the inFlightSince!==undefined arm is false), so it is due and re-flown.
  {
    ...
    ok("an in-flight record with no lease stamp is treated as reclaimable and re-flown", due.due === true);
  }
  // inFlight with an OLD inFlightSince beyond the lease -> the lease has expired, so it is due.
  {
    const { stub, storage } = makeScheduler();
    await seedDests(storage, [{ id: "r2", label: "R2 Primary" }], "r2");
    ...
    ok("an in-flight record past the lease is reclaimed and re-flown", due.due === true);
  }
  // run-now reclaims an in-flight record with no lease stamp.
  {
    const { stub, storage } = makeScheduler();
    await seedDests(storage, [{ id: "r2", label: "R2 Primary" }], "r2");
    await storage.put(CANARY_KEY, base({ inFlight: true }));
    ok("run-now succeeds on a stuck (lease-less) in-flight record", (await doFetch(stub, "POST", "/canary/run-now")).status === 200);
    ok("run-now reclaimed the lease-less in-flight flag", (await getView(stub)).inFlight === false);
  }
  // run-now reclaims an in-flight record whose lease has expired.
  {
    const { stub, storage } = makeScheduler();
    await seedDests(storage, [{ id: "r2", label: "R2 Primary" }], "r2");
    await storage.put(CANARY_KEY, base({ inFlight: true, inFlightSince: Date.now() - CANARY_LEASE_MS - 100_000 }));
    await doFetch(stub, "POST", "/canary/run-now");
    ok("run-now reclaims an expired-lease in-flight record", (await getView(stub)).inFlight === false);
  }
  // run-now does NOT reclaim a FRESH in-flight record (a flight genuinely in progress) -> the flag stays.
  {
    const { stub, storage } = makeScheduler();
    await seedDests(storage, [{ id: "r2", label: "R2 Primary" }], "r2");
    await storage.put(CANARY_KEY, base({ inFlight: true, inFlightSince: Date.now() }));
    await doFetch(stub, "POST", "/canary/run-now");
    ok("run-now leaves a fresh in-flight flight alone (no premature reclaim)", (await getView(stub)).inFlight === true);
  }
}

// ---- canaryComplete input validation, skip-and-default folding, and the unknown-id fallbacks ----------
async function sectionComplete(): Promise<void> {
  const { stub, storage } = makeScheduler();
  await seedDests(storage, [{ id: "r2", label: "R2 Primary" }], "r2");

  // Each of the three required-shape arms refused with a 400.
  ok("completion with no run is refused 400", (await doFetch(stub, "POST", "/canary/complete", { results: [] })).status === 400);
  ok("completion with a run lacking a dests array is refused 400", (await doFetch(stub, "POST", "/canary/complete", { run: { runSeq: 1 }, results: [] })).status === 400);
  ok("completion with no results array is refused 400", (await doFetch(stub, "POST", "/canary/complete", { run: { runSeq: 1, dests: [] } })).status === 400);
  ...
  ok("a sparse alive result folds to safe defaults (0 duration, no aspects, null byteDelta) and is recorded alive", lc.durationMs === 0 && lc.aspects.length === 0 && lc.byteDelta === null && lc.status === "alive");
  ...
  ok("a result for a destination off the plan transitions with its raw id as the label", t.length === 1 && t[0]!.destinationId === "ghost" && t[0]!.label === "ghost" && t[0]!.transitioned === "dead");
  ok("the off-plan destination is pruned from the persisted per-destination state", (await getView(stub)).dests.every((d) => d.destinationId !== "ghost"));
}

// ---- the owner gate, the corrupt-record fault, and the label fallback + non-finite cadence edges ------
async function sectionGuards(): Promise<void> {
  ...
  // router-bypassed (defence in depth).
  {
    const { stub, storage } = makeScheduler();
    await seedDests(storage, [{ id: "r2", label: "R2 Primary" }], "r2");
    ok("a non-owner canary config change is refused 403 at the DO", (await doFetch(stub, "POST", "/canary/config", { enabled: false }, VIEWER_CALLER)).status === 403);
    ok("the refused change did not disable the canary", (await getView(stub)).config.enabled === true);
  }
  // A corrupt stored record with no config object surfaces as a clean 500, not an unhandled fault.
  {
    ...
    ok("a stored canary record with no config yields a clean 500", (await doFetch(stub, "GET", "/canary")).status === 500);
  }
  // A label-less destination falls back to its id as the rendered label.
  {
    ...
    const v = await getView(stub);
    ok("a destination with no label renders its id as the label", v.dests.find((d) => d.destinationId === "x1")!.label === "X One" && v.dests.find((d) => d.destinationId === "x2")!.label === "x2");
  }
  ...
  // Number.isFinite guard leaves the existing cadence untouched.
  {
    ...
    ok("a non-finite cadence is ignored and the existing cadence is kept", after.config.intervalSeconds === 1800);
  }
}

// ---- ailingCause folds through canaryComplete into the persisted CanaryCheck --
async function sectionAilingCauseFold(): Promise<void> {
  ...
  const v1 = await getView(stub);
  ok(
    "an ailing result WITH a cause folds it into the persisted lastCheck verbatim",
  ...
  ok("an ailing result with NO cause folds with ailingCause absent (never fabricated)", v2.dests.find((d) => d.destinationId === "r2")!.lastCheck!.ailingCause === undefined);

  // A clean pass never carries a cause either.
  await flyOnce(stub, [aliveOf("r2")]);
  const v3 = await getView(stub);
  ok("an alive result folds with no ailingCause", v3.dests.find((d) => d.destinationId === "r2")!.lastCheck!.ailingCause === undefined);
}
