...
//
// WHY THIS EXISTS. The coverage campaign reports percentages over four denominators. The destination surface
...
//               of its objects back.
//   syslog-tls  test/destsim/server.ts startSyslogEmulator + makeRealSyslogConnect: genuine TLS bytes, and
//               the same seam, because deliverSiemSyslog now refuses an internal host before it opens a
//               socket. The emulator listens on 127.0.0.1, which is exactly what that screen catches.
//
...
async function driveS3(format: PushFormat, corrupt: boolean): Promise<Graded> {
  ...
  try {
    ...
    // allowInternalSink: the emulator is on 127.0.0.1 and both endpoint screens refuse that by default.
    // It is the ONE substitution on this path and it changes no shaping, signing or transport behaviour.
    const r = await deliverResolvedPush(cfg, EVENTS, META, { allowInternalSink: true });
...
async function driveSyslog(format: PushFormat, corrupt: boolean): Promise<Graded> {
  ...
  try {
    ...
    // allowInternalSink: the syslog emulator listens on 127.0.0.1, which deliverSiemSyslog's send-time
    // internal-host screen refuses by default. The mock connect is still the REAL node:tls client.
    const r = await deliverResolvedPush(cfg, EVENTS, META, { allowInternalSink: true });
