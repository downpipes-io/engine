...
export async function runLifecycle(ctx: Ctx): Promise<void> {
  const { ok, call, readLog, listDownpipes, listRoles, pendingChanges, pendingKeyCount, setGate, sched, dp, ageChange } = ctx;
  ...
  // ===========================================================================================
  {
    ...
    // Now approve A: its base (headBefore) no longer matches the live head -> superseded, no apply. Aged so
    // the dwell floor (which fires before the base-moved check) does not itself intercept and mask which
    // guard actually answered.
    ageChange(idA);
  ...
  // ===========================================================================================
  {
    ...
    // THE APPLY-TIME CEILING, IN TWO ARMS. The first arm drives the DEMOTION route -- queue a change,
    // demote the proposer, approve -- and asserts the approve returns 4xx and nothing applies. A demotion
    // is itself a config mutation, so it auto-snapshots, and approveChange's baseMoved check fires BEFORE
    // the authority re-resolution ever runs: this route is answered as SUPERSEDED rather than by the
    // ceiling. The route is still real and worth pinning, so the arm asserts the DISCRIMINATOR that says
    // which guard answered, rather than only the 4xx-and-no-apply outcome a superseded change would also
    // produce.
    ...
    // Aged so the dwell floor does not itself intercept before the base-moved guard the record.status
    // assertion below depends on (the demotion above already moved the head).
    ageChange(grpId);
    ...
    // Aged so the dwell floor does not itself intercept before the authority re-resolution the "still
    // PENDING, not superseded" assertion below depends on.
    ageChange(lapseId);
  ...
  // ===========================================================================================
  {
    ...
    // SINGLE USE (the same rule enforced for owner-action approvals): the change is now APPLIED. A SECOND approve of
