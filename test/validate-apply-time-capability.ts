...
// not eighteen. roleForCaller RETURNS viewer rather than throwing, so d3099399's binding, 9622c01b's
// bind-on-first-auth suppression and e86e1cd2's connection-liveness axis all compute the right answer for
// these three kinds and then nothing reads it.
//
...
//   CONNECTION DEATH  an IdP connection is not part of the config snapshot (e86e1cd2). Section E.
//
// So the two loose ends the previous pass named are one thing: the expiry question IS the reachability proof
// for the addDownpipe gap.
//
...
async function run(): Promise<void> {
  ...
  const { call, sched, setGate, dp, ageChange } = ctx;
  ...
  try {
    ...
    // =========================================================================================
    // SECTION E: THE SECOND REACHABLE ROUTE, and it is the one that shows e86e1cd2's connection-
    // liveness fix computing an answer nothing read. That commit proved its fix on
    // notify-channel-set precisely BECAUSE a downpipe-upsert would have proved nothing. This is
    // the same drive against the kind it had to avoid.
    // =========================================================================================
    {
    ...
    // =========================================================================================
    {
      ...
      // Aged so the dwell floor does not itself intercept before the base-moved guard F5 depends on.
      ageChange(q);
