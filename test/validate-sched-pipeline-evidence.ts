...
//
// Every case below is the same shape: the scheduler DO ABSORBS a fault -- a resolver that substitutes a
// destination the customer did not choose, a flight that is allocated and abandoned, a drill verdict discarded
// because the downpipe was renamed, an alert rejected at the input boundary, an apply that refuses forever, a
// role row that will not parse, a session signing key that quietly regenerates -- and the FACT of it must exist
// somewhere a support pack can carry. Each block proves the same two things:
//
...
//       through the auth-signal aggregate the pack already carries (section 4.18).
...
//
// The tickets these make diagnosable:
...
//        destination USED; nothing says the ENGINE substituted it.
...
//        having drilled, because the evidence write fails in silence.
...
//        failed attempts; and a detected record TAMPER or a REPLAYED one-shot approval is seen only by a browser.
//   a licence retried 5 times with 3 different causes reads as count=5 and ONE code (the least useful one).
//   the incident timeline must prove WHEN dual control was disarmed, that the break-glass token was
...
//        key silently regenerated, and the verify-failure spike that PAIRS with it must be counted too.
//   "I cannot get past Verify and save" / "I cannot remove this destination" -- the engine computes
//        a precise refusal that must reach the support pack, not only a toast that is now closed.
//   the console's own crash / bulk-casualty / update-settle / contract-skew evidence, which the
//        engine SIGNS INTO the pack (POST /client-diag) rather than beaconing anywhere.
//
...
// ---------------------------------------------------------------------------------------------------------
// THE RESOLVERS that SILENTLY write to a destination the customer did not choose. THE DATA-LOSS ONE.
// ---------------------------------------------------------------------------------------------------------
async function testDestResolveFallback(): Promise<void> {
  ...
  // only arise from a CORRUPTED or legacy stored row (nothing downstream re-validates, primaryDestinationId
  // just falls through). Plant it the way it really occurs: a valid downpipe whose stored row is then corrupted
...
// ---------------------------------------------------------------------------------------------------------
// THE CANARY'S own losses -- abandoned flights, skipped results, destinations never flown.
// ---------------------------------------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------------------------------------
// THE DRILL PIPELINE'S silent discards, and the evidence trail that quietly diverges.
// ---------------------------------------------------------------------------------------------------------
async function testDrillDrops(): Promise<void> {
  console.log("\nrestore-test + fleet-drill drops ('stuck at 3 remaining for two days')");
  const { storage, stub } = makeScheduler();
  ...
  // (5) A drill-evidence append REFUSED by validation. Without a recorded refusal, the dated trail the
  // customer's auditor reads silently has a hole. The note carries the poison.
  ...
  scanClean("drillDrops", d.drillDrops);
}

// ---------------------------------------------------------------------------------------------------------
// WHICH alert died, not just how many.
// ---------------------------------------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------------------------------------
// GOVERNANCE apply attempts, tamper/replay, and the last-only refusal counters.
// ---------------------------------------------------------------------------------------------------------
async function testGovernanceAndRefusals(): Promise<void> {
  console.log("\ngovernance faults + refusal rings ('our second owner approved and nothing happened')");
  ...
  // And the DO-side SHAPE reject must not bypass the refusal counter (the invisible branch: the ROUTER posts
  // its own refusals to the counter, so a token the router accepts and the DO then rejects on shape must also
  // reach it, not touch NOTHING at all). Seat an owner so the DO's owner re-check passes and the test reaches
  // the shape guard under test.
  ...
  check("the DO-side licence SHAPE reject reaches the counter rather than bypassing it entirely", ring.filter((r) => r.code === "malformed-token").length >= 2);
...
// ---------------------------------------------------------------------------------------------------------
// THE ENGINE-COMPUTED REFUSAL the browser shows once and forgets.
// ---------------------------------------------------------------------------------------------------------
async function testAdminRefusals(): Promise<void> {
  console.log("\nadmin refusals ('I cannot get past Verify and save' / 'I cannot remove this destination')");
  ...
  scanClean("adminRefusals", d.adminRefusals);
}

// ---------------------------------------------------------------------------------------------------------
// ENROLMENT refusals, RBAC degradation, session lifecycle. All ride the auth-signal
...
async function testAuthSignalGaps(): Promise<void> {
  console.log("\nenrolment, RBAC and session signals (all on the existing authSignals aggregate)");
  const { storage, stub } = makeScheduler();

  // --- THE INVITE LIFECYCLE. The EXPIRED case matters most: the record is deleted-then-refused, so afterwards
  // nothing would prove the invite ever existed unless it is recorded BEFORE the burn.
  ...
  // --- A role row that will not parse is filtered out of authority resolution -- the member VANISHES.
  ...
  // The owner TAMPER-CLAMP: a stored group mapping that CLAIMS owner is clamped, silently.
  const clamped = stub.capGroupRole("owner");
  check("the owner clamp still clamps (behaviour unchanged)", clamped !== "owner");

  // --- THE HEADLINE. The session signing key is absent/corrupt and is regenerated, so every live
  ...
  // The verify-failure SPIKE that PAIRS with it -- the pair a support engineer needs to assemble the diagnosis.
  ...
  vocabClean("authSignals", sig, AUTH_SIGNAL_SET);
  scanClean("authSignals", sig);
}

// ---------------------------------------------------------------------------------------------------------
// DEAD VOCABULARY: auth-signal members must have a real producer, or the pack promises a support engineer an
// answer nothing can ever put there. Each block below drives the REAL entry point (the DO route / the
// resolution path the product actually takes) and asserts the signal lands. A hand-written record posted
// directly into the recorder would prove only that the aggregate can carry the name, which is deliberately not
// what any of these do.
// ---------------------------------------------------------------------------------------------------------
async function testDeadAuthSignalProducers(): Promise<void> {
  ...
  // the account stops resolving at once -- "all our AD-group users dropped to viewer" -- silently, unless recorded.
  {
    const { storage, stub } = makeScheduler();
    await storage.put("role:sub:sub-owner", { subject: "sub-owner", email: POISON_EMAIL, role: "owner", grantedBy: "bootstrap", grantedAt: "2026-07-01T00:00:00.000Z" });
  ...
  // spent, so nobody holds authority and nobody can claim it. Without this row, a pack reads this state as
  // byte-identical to "an account whose members happen to all be viewers", though the two have completely
  // different remedies.
...
// ---------------------------------------------------------------------------------------------------------
// THE ENGINE SINK for the console's OWN diagnostic ring. The body is
...
async function testClientDiagnostics(): Promise<void> {
  console.log("\nclient diagnostics (the console's ring, SIGNED INTO the pack -- no beacon)");
  const { storage, stub } = makeScheduler();
  ...
  check("the bulk casualty summary is recorded ('created only 300 of 1000 then logged me out')", bulk !== undefined && bulk.attempted === 1000 && bulk.succeeded === 300 && bulk.halted === true);
  ...
  scanClean("clientDiagnostics (from a HOSTILE browser-authored body)", c);

  // And the applier is pure: the same hostile body against no prior record must be equally clean.
  scanClean("applyClientDiagnostics (pure)", applyClientDiagnostics(undefined, hostile, "2026-07-11T00:00:00.000Z"));
  check("a non-object body is a no-op, never a throw", JSON.stringify(applyClientDiagnostics(undefined, POISON_ERROR, "2026-07-11T00:00:00.000Z").crashes) === "{}");
}

// ---------------------------------------------------------------------------------------------------------
// The DROPPED-WRITE invariant: a recorder whose OWN write fails must COUNT that loss, because a pack
// that under-counts during the outage it exists to explain defeats the pack's purpose.
// ---------------------------------------------------------------------------------------------------------
