...
//
//   validateCanonicalCounts must resolve a dotted field by walking the
...
//
//   The freshness parseRunlog must apply the canonical-numeric gate to each RUNLOG
...
//
//   The shard read path must apply the gate to the preamble's recordCountInShard and
...
// ---------------------------------------------------------------------------------------
// Leaf-name collision resolves the correct field structurally, not by a substring scan that would match
// the FIRST textual occurrence of the leaf key anywhere in the document -- which would inspect a
// sibling/ancestor decoy with the same leaf name instead of the intended dotted path.
// ---------------------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------------------
// Shard preamble recordCountInShard and record plaintextSize field gate. These are
...
// ---------------------------------------------------------------------------------------
// parseRunlog applies the gate per line (index, recordCount).
// ---------------------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------------------
// End-to-end: a VALIDLY SIGNED RUNLOG whose sibling entry carries an out-of-range
...
// ---------------------------------------------------------------------------------------
// rollbackDetected must be an UNCONDITIONAL signal, independent of allowStale/ok -- a
...
async function freshnessRollbackDetectedIsUnconditional(): Promise<void> {
  ...
  ok("freshness: a chain anomaly sets rollbackDetected:true regardless of allowStale", anomalyRes.rollbackDetected === true);
  ...
  ok("freshness: below the min-runlog-index pin sets rollbackDetected:true regardless of allowStale", pinRes.rollbackDetected === true);
}
