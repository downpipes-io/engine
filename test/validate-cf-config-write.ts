...
// the live-only report is only useful if the operator-facing summary renders it, so the test
// asserts the whole path from buildListOps through to the string a human reads, not just the change list.
...
console.log("-- DNS write(): unchanged skipped, changed -> PUT/{id}, missing -> POST, live extra kept --");
{
  ...
  // reported as a "remove" (which summariseDiff renders "1 live-only (left in place)"). Counting only
  // the changed and missing records would hide a live extra that is neither applied nor reported, which
  // on an allow-list surface means a restore run meant to recover from an attack would silently preserve
  // the attacker's entry while reporting success. Reporting the extra is required; it is still never deleted.
  ...
  ok("the live-only report carries the live value in `from` and nothing in `to`", liveOnly[0]!.from.includes("9.9.9.9") && liveOnly[0]!.to === "");
...
console.log("-- SINGLE-SETTING writer: covers the nine surfaces that route through it --");
{
  // writeSingleSetting backs the nine zone settings Cloudflare omits from its aggregate /settings
  // response. The only thing that otherwise exercises it is the LIVE round trip, which needs a real
  // Cloudflare account and never runs in CI, so the plan-gate guard and the converge-and-do-nothing
  // behaviour need coverage offline too.
...
console.log("-- AMBIGUOUS natural key: a colliding key must REFUSE, never guess which twin was meant --");
{
  ...
  // natural() exists for. Without this guard the writer would bind the macOS snapshot to the WINDOWS live
  // rule and PUT macOS criteria over it, reporting success, with the outcome depending on Cloudflare's
  // list ordering. Both orderings are asserted because only one of them corrupts.
  const live = [
...
console.log("-- rulesets write(): a live rule the snapshot lacks is NAMED and REFUSED, never deleted --");
{
  ...
  // would have removed. Without this guard, the apply would go ahead and the preview would say "2 rules
  // became 1", which is a count, and a count cannot be reviewed.
  const liveIndex = [{ id: "rs-http", phase: "http_request_firewall_custom", name: "zone" }];
  const liveFull = {
    id: "rs-http",
...
{
  // writeListSpec asks the spec for the live item's server id to build the update path. The generated
  // writer answered `it.id`, and device-managed-networks keys on `network_id`, so the answer was always ""
  // and every changed item was skipped with "no id to update in place": the writer could CREATE and never
  // UPDATE. No round trip could see it, because a restore after a delete only ever creates, which is why
  // this is pinned offline rather than left to the live harness that eventually found it.
...
console.log("-- nested collections: an update must not silently replace one (gateway-lists) --");
{
  ...
  // an item is replaced wholesale and any entry the snapshot lacks goes with it: the same shape one
...
console.log("-- nested collections: the writer must read live the SAME way the surface does --");
{
  ...
  // plain list. gateway-lists sub-reads each list's items, so a writer that does not use the surface's
  // own reader sees no `items` key at all: the snapshot's extra key makes every list differ, and the
  // writer re-applies an identical list on every run while still restoring correctly each time.
  //
  // The stub deliberately serves the entries ONLY on the sub-path, so a writer that does not use the
  // surface's reader sees a list with no items and reports a change; removing the readLive wiring
  // makes this assertion fail, confirming it actually exercises that path.
...
// the diff preview and the write strip must agree. SERVER_STAMPED fields (id, version, zone_id, ...) are
// stripped before a write and so must NOT surface in the diff preview as spurious "change" rows. Without
// SKIP_IN_DIFF unifying the two sets, the diff walker would skip only the volatile keys, so a
// stamped-only field such as `version` would show as a change a restore would never apply.
...
{
  // The rest of the nested-collection guard is already covered above, through the real gateway-lists
  // surface. This one branch was not, and it is the branch that decides whether the guard is usable at all:
  // nestedLiveOnly skips a field unless BOTH sides are arrays, because a snapshot that never captured the
  // nested collection is not a snapshot that captured it as empty.
  //
  // Treat the absent case as empty and every live entry reads as a loss, so the guard refuses every restore
  // of that surface forever, on an archive that is perfectly good. Verified to be the only assertion that
  // notices: neutering just that check fails this and nothing else.
  const liveIndex = [{ id: "L1", name: "allow", description: "d", type: "SERIAL", count: 3 }];
...
console.log("\nCF-CONFIG WRITE-BACK (SRC-5) VECTORS PASS");
