// Pins the ADMIN-PLANE auth diagnostic evidence for the support pack:
//
//   Cloudflare Access JWKS fetch/import outages classify as ordinary {reason} refusals rather than throwing
//   out of verifyAccessJWT: a throw would unwind authorise() into a 500 and leave accessDenySignal with
//   nothing to classify, so a cloudflareaccess.com blip, a bad CF_ACCESS_TEAM_DOMAIN or a malformed JWKS
//   entry would be indistinguishable in the pack from "nobody tried to sign in". They classify instead to
//   the closed cf-access-jwks-* names.
//
//   The ~7 Access verify-failure causes (clock skew, an alg downgrade, a wrong typ, a bad signature, a
//   corrupt token) each classify to their own name rather than collapsing into one cf-access-verify-failed
//   counter, because each is a different ticket with a different fix.
//
//   The CSRF refusal sub-classes separate two opposite diagnoses that would otherwise share one counter:
//   CONSOLE_ORIGIN unset on the engine (a deploy dropped the var -> a configuration fix) vs a foreign
//   Origin genuinely presented (a security event). Same for the double-submit second factor.
//
//   The session-cookie fault classes sit behind verifySession's deliberately-bare null.
//
...
console.log("Access verify sub-causes (each of the ~7 causes gets its own name rather than sharing cf-access-verify-failed)");
{
...
console.log("an ATTACKER-CHOSEN alg/typ tail cannot hijack its own classification (the anchored rule holds)");
{
  // The reason `unexpected alg <header.alg>` interpolates a field the ATTACKER supplies in the presented JWT.
  // Without the anchored rule, the leading whole-string `includes("aud mismatch")` check would match first, so
  // a token whose header carried alg:"aud mismatch" would classify its own refusal as cf-access-aud-mismatch --
  // and would send support to re-check CF_ACCESS_AUD on a perfectly healthy Access application.
...
console.log("the JWKS throw classifier reads a message ONLY to select a fixed literal");
{
...
console.log("a JWKS outage is a RECORDABLE deny, not a 500 out of the auth path");
{
  ...
  ok("verifyAccessJWT does NOT throw on a JWKS outage (never 500s the request)", res.ok === false);
  ...
  // A JWKS whose selected entry is a malformed RSA key: importKey REJECTS, and that rejection must also fail closed rather than 500.
  const badKey = await verifyAccessJWT(token, {
    ...
    // which is a different ticket (and a different fix) from "the JWKS endpoint is unreachable".
...
console.log("the CSRF Origin refusal names WHICH branch fired (config fault vs attack)");
{
...
console.log("the double-submit second factor names its three branches");
{
...
console.log("verifySession's bare null keeps its anti-oracle contract, and the fault CLASS is nameable");
{
  ...
  // minted under a signing key this engine no longer holds -- which is a posture signal, not a config fault,
  // and which a bare null would otherwise conflate with the silence of an ordinary expiry.
...
console.log("bootstrap-link and role-invite send failures are recorded (the no-oracle 200 is unchanged)");
{
  // The bootstrap route answers a no-oracle 200 on EVERY skip reason by design, so the reason was the only
  // evidence there ever was -- and it went to Workers Logs, which remote support structurally cannot read.
  // "The first-Owner set-up email never arrives" is the most common bootstrap ticket, and these are its causes.
