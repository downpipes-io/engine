...
import { advanceChunk, sealStream, STREAM_MAX_CHUNK_INDEX } from "../src/crypto/stream.ts";
...
// PIN_FILE_KEY_B64 is a verbatim copy of the crypto-kat fileKey, and that vector's fileKey is derived
// through INFO_SEG_KEY / INFO_PAYLOAD, which mixes in the format's version label, so this copy must be
// re-taken from the current crypto-kat vector whenever that label changes, or the anchoring claim above
// stops holding (the pin would no longer ride the crypto-kat keystream at all). The four OUTPUT pins below
// are computed from the current vector. The INPUTS (nonce, tail length, aad, plaintext fill) stay the same
// frozen construction, anchored to the live vector.
...
async function main(): Promise<void> {
  ...
  // Frozen multi-chunk byte pin (ENG-L1): a fixed 3-chunk seal under the Go-pinned
  ...
  // never invoked (the guard throws first), so no real bytes are needed.
  {
    ...
    // ceiling with a documented out-of-band remedy, distinct from the generic "source read error" bucket a
    // 500 from a KV get would fall into, and the failed run row says so.
  ...
  // The no-wrap chokepoint (S5.2). A chunk counter stepped with a bare `counter += 1n` on both the
  // buffered and the streaming path would have u64be truncate modulo 2^64, so a counter that reached the
  // ceiling would re-emit a nonce this key had already used. The Go reference refuses at
  // internal/crypto/stream.go:310; advanceChunk is that refusal ported, message included. What is graded
  // here is the refusal itself, because the bound is unreachable through the public seal entry points (a
  // 2^64-chunk stream is 2^80 bytes, and the caller's windowing caps a segment at 16384 chunks), so the
  // chokepoint can only be exercised at the function.
  {
    console.log("\n-- the no-wrap chunk-counter chokepoint --");
    ok("advanceChunk steps a fresh counter", advanceChunk(0n) === 1n);
    ok("advanceChunk reaches the ceiling itself (the last usable index is not refused)", advanceChunk(STREAM_MAX_CHUNK_INDEX - 1n) === STREAM_MAX_CHUNK_INDEX);
    ok("the ceiling is the Go reference's streamMaxChunkIndex (2^64 - 1)", STREAM_MAX_CHUNK_INDEX === (1n << 64n) - 1n);
    let wrapped: Error | undefined;
    try {
      advanceChunk(STREAM_MAX_CHUNK_INDEX);
    } catch (e) {
      wrapped = e as Error;
    }
    ok("advanceChunk THROWS at the ceiling rather than returning a wrapped counter", wrapped !== undefined);
    ok("...and the message is the Go reference's, so both sides read alike", wrapped !== undefined && /stream would exceed the safe chunk count: chunk index \d+ wraps the per-stream nonce counter/.test(wrapped.message));

    // The positive control that keeps this from being a chokepoint nobody can pass: a multi-chunk seal
    // through the SAME stepped counter still produces the buffered bytes, so the refusal above is a bound
    // on the unreachable case and not a bound on ordinary sealing.
    const controlKey = new Uint8Array(32).fill(0x1d);
    const controlNonce = new Uint8Array(16).fill(0x2e);
    const controlValue = fill(3 * CHUNK_SIZE + 11);
    const controlBuffered = await sealStream(controlKey, controlValue, controlNonce);
    const controlStreamed = await streamToBytes(controlKey, chunked(controlValue, 4096), controlNonce);
    ok("a 4-chunk stream still seals, buffered and streamed alike (the chokepoint does not fire in normal operation)", b64urlEncode(controlStreamed) === b64urlEncode(controlBuffered) && controlBuffered.length > 3 * CHUNK_SIZE);
  }
