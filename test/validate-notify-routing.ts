// Severity/vocabulary, validation, routing and formatter vectors for the notify suite. Covers
...
// ---- Severity mapping + helpers (contract section 2.1) ---------------------------
...
// ---- Replication-degraded + run-at-risk-eviction vocabulary + pure detectors -----
// Two warning-class events (the data-loss redundancy signals) have their own pure detectors.
...
// ---- Structural wiring guard (every NotifyEvent is fully routed) -----------------
...
// ---- Every event must be PRODUCED, not merely routable --------------------------
...
// ---- What the restore-applied alert actually SAYS ------------------------------
...
// ---- CHANNEL-KIND wiring completeness (the sibling structural guard, for channel kinds) -----
...
// path), the exact drop class the structural wiring guard above guards for events. ALL_CHANNEL_KINDS is the runtime list the loop
...
// ---- validateChannel (per-kind transport validation) ----------------------------
...
// ---- deliverPayload SSRF re-check at SEND time (defence-in-depth) ----------
...
// caller passes allowInternal. This proves the second layer of SSRF defence without any network.
...
async function testDeliverPayloadSsrf(): Promise<void> {
  ...
  // host must. This is the property the delivery path (P3.2b) guarantees.
  {
...
// ---- ruleSelects + resolveDelivery (routing resolution, contract section 2.3) ----
...
// ---- Channel formatters carry only the redaction-safe surface --------------------
...
function testFormatters(): void {
  ...
  // email
  {
    ...
    ok("formatEmail: html carries no vendor sign-off", !/O'Connor|Maelstrom/.test(m.html ?? ""));
  }
}

// ---- redactChannelSecretForRead (the operator-facing list read redacts the sealed apiKey) ----
