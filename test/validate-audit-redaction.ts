// PROOF 10 / 11 / 12 for the D4 audit validator: redaction holds (no secret material anywhere in the log);
// the export carries the chain head hash in both JSON and CSV; a filtered read narrows correctly.
...
export async function runRedaction(ctx: Ctx): Promise<void> {
  ...
  // ---- PROOF 11: the export carries the chain HEAD HASH (JSON + CSV) -----------------------
  {
    ...
    // Column-position pin on the head row, guarding against a 10-vs-11-cell misalignment: the
    // head row must split into exactly 11 quoted fields and field index 10 (the hash column) must equal
    // the quoted head hash, not the prevHash column, so a misaligned column cannot pass unnoticed.
