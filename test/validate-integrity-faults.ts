// VALIDATOR: the FORMAT + CRYPTO integrity fault evidence, the last-resort dispatch / CORS / destination-probe
// evidence, the notify delivery-code splits, and the replication-detector self-disable flag.
//
...
async function main(): Promise<void> {
  ...
  // problems" failure this evidence exists to prevent. Pin them.
  // -------------------------------------------------------------------------------------------------------
  ...
  await check("a 403 on the RUNLOG read records access-denied, not 'absent', and leaks nothing", async () => {
    ...
    const snap = await drainAfter(async () => {
      ...
      // The result names the real cause; the reason string is unchanged for the operator.
  ...
  await check("hexDecode REJECTS non-hex instead of silently coercing it to zero bytes", () => {
    // Coercing non-hex to zero (parseInt("zz",16) is NaN, and `out[i] = NaN` writes 0) would let a corrupt
    // object key decode to a well-formed-looking all-zero segment id that matches nothing and reads as "object missing".
  ...
  await check("a signature that will not verify splits structural-fault from mismatch", async () => {
    ...
    // tamper or a signer rotation. A bare false return would merge both into one indistinguishable reason.
    ...
    assertRedacted("cryptoFaults (signature)", snapShort);
  });
  ...
  await check("a corrupt segment OBJECT KEY records malformed-object-key and carries no key", async () => {
    const snap = await drainAfter(() => {
      // The object key is customer-derived; hexDecode rejects non-hex here rather than silently coercing it
      // to a run of zero bytes.
  ...
  // -------------------------------------------------------------------------------------------------------
  // The SILENT ZERO-BYTE SEAL. The single most dangerous case in this set: a green run whose restore
