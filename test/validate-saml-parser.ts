...
console.log("\nred - illegal characters (WFC: Legal Character - NUL/control bytes, ML-17):\n");
...
// literal NUL (the ML-17 exploit: an embedded NUL surviving into an audit-log actorEmail / session cookie).
...
// ML-44: a bare '&' immediately before a CDATA section must be rejected as a dangling/incomplete Reference,
...
console.log("\nred/green - ML-44 bare '&' vs CDATA merge (per-span pre-CDATA entity validation):\n");
...
expectFail("<a>&<![CDATA[lt;]]></a>", "ML-44 PoC: a bare '&' immediately before CDATA is rejected (not completed by borrowed CDATA content)");
...
// ML-44-r2: many chained small text+CDATA pairs in one run must stay O(n), not reintroduce the O(n^2) cost an
...
console.log("\nred/green - ML-44-r2 many chained CDATA sections must parse in O(n), not O(n^2):\n");
{
  let body = "";
