...
function expectedTargetIdName(t: AuditTarget): { id?: string; name?: string } {
  switch (t.kind) {
    ...
    case "idpconnection":
      return { id: t.connId };
    // authn-attempt: the id is the connection ONLY when one RESOLVED, so a probe at an unknown connection
    // projects no id rather than the attacker-chosen path segment it submitted, and the ceremony is the name
    // so a SIEM rule can key on which factor refused without parsing the message. attempts is deliberately
    // NOT projected into this pair: it is a count that belongs on the raw event, exactly as posture-ack's
    // statement hash and test-fault's armedAt do.
    case "authn-attempt":
    ...
    //
    // ITS ABSENCE WAS NOT COSMETIC: this switch is exhaustive by design so that a new target kind fails the
    // type-check rather than silently dropping its id, and that is precisely what it did -- engine main has
    // been RED on typecheck:test since the test-fault target landed, with the identical error reproduced at
    // origin/main. The shaper this oracle checks (cron/siem-push-shape.ts) already had the case, so no SIEM
    // wire output was ever wrong; what was lost is the independent check that it stays right.
    case "test-fault":
