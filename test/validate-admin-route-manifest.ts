...
// see a member that never joined it, so a check built on one is blind to exactly the case it is for. This is
// the same approach used to derive the change-controlled kinds from enforceChangeControl's call sites,
// applied here to the admin router. Enumeration drift is easy to miss this way: GET /downpipes, GET /history
// and GET /replication can dispatch with no capability gate while the two reads beside them each name one;
// auditDestChange's hand-kept target-field list can drop affectedDownpipeNames on the floor; and router.ts's
// own endpoint catalogue can go missing an entry such as POST /admin/keys/add-operational. None of these is a
// wrong sentence. Each is an absence, which is the thing prose cannot show and a scan can.
//
...
// mechanism for that route. Saying "reviewed" over 35 routes I read once would be the rubber stamp this
...
// .ts's structural check for the approve legs already reads that source directly. (2) The scan asks
...
//   none-established      NO mechanism is established. Not a verdict, and it must never be read as one. It is
//                         EMPTY: every route that would otherwise carry it has a resolved mechanism, either a
//                         viewer-floor gate or one of the classes above. It stays in the union because a route
//                         whose mechanism is genuinely unknown must have somewhere honest to sit, and
//                         inventing a reason for one is the rubber stamp this whole file is against.
//
// WHY THE CLASSES ARE SHAPED THIS WAY. "The DO enforces it" looks like an obvious reason to give the forwarded
// reads, but it is FALSE whenever a route forwards callerHeaders(caller) exactly as its gated write sibling
// does while the arm on the other side takes no caller argument at all -- GET /notify/rules, GET
// /notify/history, GET /expiry, GET /group-roles and GET /custom-roles are examples, forwarding to
// listNotifyRules, listNotifyHistory, listExpiryStatuses, listGroupRoles and listCustomRoles respectively.
// THE CALLER IS CONVEYED AND DISCARDED, which is worse than sending none, because the forwarding line is the
// evidence a reader uses to conclude the route is authorised elsewhere. That is why do-recheck-capability is
// checked at the ARM and the HANDLER and not merely at the router's header call, and why authenticated-only
// is checked by proving no arm decodes a caller rather than by asserting it.
//
...
const UNGATED: ReadonlyArray<{ key: string; cls: UngatedClass; why?: string }> = [
  ...
  { key: "GET /test-fault/status", cls: "spoke-owner-only" },
  { key: "POST /test-fault/arm", cls: "spoke-owner-only" },
  { key: "POST /test-fault/disarm", cls: "spoke-owner-only" },
  { key: "POST /test-fault/canary-tick", cls: "spoke-owner-only" },
  { key: "POST /test-fault/retention-tick", cls: "spoke-owner-only" },
  { key: "POST /test-fault/control-plane-clear", cls: "spoke-owner-only" },
  ...
  { key: "GET /sessions", cls: "self-scoped-handler" },
  { key: "POST /sessions/terminate", cls: "self-scoped-handler" },
...
// The two numbers router.ts's catalogue header states about itself. They are asserted, not decorative: the
// header used to say "roughly a third", which is the one form of the claim that can never be checked.
const HEADER_CATALOGUED = 51;
const HEADER_DISPATCHED = 172;
...
for (const { key, cls, why } of UNGATED) {
  ...
  if (cls === "break-glass-token") {
    ...
    // Three parts, and the middle one cannot be assumed: the router sending a caller and the DO arm decoding
    // one are DIFFERENT facts, and a route can have the first without the second. The third resolves the
    // handler the arm names and reads ITS body, because an arm that
