...
console.log("\n-- auto mode ACCOUNTS for the unavailable partition, and ONLY for it --");
// Auto mode narrows on the discovery cache. If it dropped the UNAVAILABLE surfaces out of the include set,
// the crawl would skip them before its attempted counter -- no record, no marker, no run-level count.
// `notAttempted` is what carries them to the crawl instead. The negative direction matters just as much: an
// EMPTY surface (the account does not use the product) and a GATED one (its plan does not include the
// product) are evidence of ABSENCE and MUST stay silent, or "you do not use this" would read as "we could
// not read this" on every run.
...
console.log("\n-- effectiveCfConfigSelector: a newly-REGISTERED surface fails safe to captured --");
...
console.log("\n-- END TO END: probe -> auto selector -> the REAL crawl, both directions --");
// Drives the real path end to end, with no network: the discovery probe classifies three stub surfaces, the
// auto-mode selector narrows on that cache, and the REAL CloudflareConfigSource crawls with the selector auto
// mode actually computes. The two assertions below that name the refused surface guard against it producing
// no record, no marker and no run-level count. The empty surface's assertion is the other direction: it must
// STILL produce nothing, so "you do not use this product" never reads as "we could not read this".
...
// Archive half: a refused surface must not vanish from the archive.
...
// Run half: the seal folds markerKind into recordsIncomplete/incompleteByMarker/incompleteIds, and the
// adapter files the reason + the surface id in the fault ledger the support pack carries.
...
console.log("\n-- a plan-entitlement gate stamps its own marker; a token-scope 403 does not --");
