...
// confused-deputy guard (ASVS V4) being deleted without a single validator noticing.
//
...
//
// THE MEASUREMENT THAT PUT IT HERE. Before this file, NO test in the repo referenced runDiscoveryPass or
// discovery-pass: `command grep -rn 'runDiscoveryPass|discovery-pass' test` returned nothing, while the
// known-positive control `command grep -rln 'accountInDiscoveryScope' test` returned exactly one file,
// test/validate-source-account-scope.ts, which covers the OTHER three call sites (the write-time gate, the
// rediscover re-check and the run-path token chokepoint) and never reaches the cron pass. Deleting the cron
// guard was invisible to all 489 validators. It is not invisible now: neutering line 55 turns this file red.
//
...
// classifies AUTH, so withRetry fails it on attempt one. This was measured the hard way. A message carrying
// any of the network vocabulary instead takes the transient arm, and API_READ_RETRY's six attempts with
// backoff, on every account surface, hung the run past two minutes.
//
