...
function goodFor(slot: KeySlotId): string {
  // Derived from the module's own SLOT_BYTES, never restated here: a ternary over three slot names would
  // silently produce a WRONG-LENGTH value the moment a fourth slot appears, so the test would fail for its
