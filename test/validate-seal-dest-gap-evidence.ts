// Validates the SEAL and DESTINATION diagnostic evidence for the eng-seal-dest subsystem.
//
// Each case proves TWO things:
...
const SECRET_VALUE = "example-secret-51ACMEsecretVALUE";
...
// ---------------------------------------------------------------------------------------------------------
{
  ...
  // A KV namespace accidentally bound under a SECRET's name: it has no get(), and it is not a string. Coercing
  // it with String(store) would seal "[object Object]" as the secret, every run, silently.
  ...
  // or a value -- and this reuses an existing contract rather than adding a new seam.
...
// ---------------------------------------------------------------------------------------------------------
{
  ...
  // A conditional PUT that succeeds with no ETag is silently tolerated, so RUNLOG concurrency can degrade
  // with no evidence. The tolerance stays; the non-conformance is recorded.
...
// ---------------------------------------------------------------------------------------------------------
{
  ...
  // The owner's #1 fear: a redeploy wiped DEST_ENDPOINT. The var NAME is product vocabulary; its VALUE is not.
...
// ---------------------------------------------------------------------------------------------------------
{
  // The risk: WORM_REFUSAL_RE must not fold "access denied" and "status 403" into the WORM arm, or a ROTATED
  // credential reads as "locked by design, nothing to fix".
...
// ---------------------------------------------------------------------------------------------------------
{
  // The reason that matters most: the copy VERIFIED BAD on the replica.
