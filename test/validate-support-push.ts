...
//
// Both push drains record a bounded delivery trail on their scheduler-DO records; the siemPush/otlpPush
// sections project that EXISTING coarse trail so "our SIEM feed stopped" / "OTLP metrics stopped arriving"
// is diagnosable from the bundle. This test drives the real gatherers through a DO double and
...
async function main(): Promise<void> {
  ...
  // Every recording site coarsens the trail's `reason` to a closed enum member, so the pack gates on the
  // union of those vocabularies. Two properties are load-bearing:
  ...
  // drain cannot read would otherwise read as a fake clean delivery ("the collector silently stopped").
  ...
  ok("otlpPush carries the partial-success body class on an ok:true row (the fake clean delivery is named)", (otlpPartial.trail as Array<{ ok?: boolean; reason?: string }>)[0]?.reason === "otlp-partial-body-unparseable");

  // 8. THE PACK'S FORMAT AND SINK VOCABULARIES ARE THE ENGINE'S, NOT A COPY OF THEM.
  //
  // Gating the format and sink through a copied allowlist here would silently drop any engine format or sink
  // the copy has not listed. A dropped value is not a visible failure: the key is simply ABSENT, which reads
  // exactly like an engine that never reported it, so a Splunk push bundle could carry nothing anywhere in
  // it naming Splunk.
  //
  ...
  // Checked by name because a Splunk destination's pack section must contain the string a diagnoser would
  // search for.
