...
//
// What this covers:
//   refusal classification      loud, self-describing refusals get their own closed class instead of a
//                                generic "run failed" -- and instead of a MISATTRIBUTED "destination access
//                                error" that would blame the customer's bucket (and light the map's
//                                down-destination indicator) for a seal-DO handoff refusal, a fault entirely
//                                inside the engine's control plane. coarseRunError carries seven closed
//                                classes; the refusing DO's FIXED error body folds into a closed handoff class
//                                (classifyHandoffRefusal); the body text itself never rides.
//   checkpoint corruption        a corrupt checkpoint names itself: WHICH field failed, WHICH unwrap failed,
//                                and how many slices of durable progress it discards (the mechanism behind
//                                "the big backup restarts from scratch every night and never completes").
//                                CheckpointInvalidError / CheckpointUnwrapError type the throws (messages
//                                verbatim), and the seal DO records checkpoint-invalid /
//                                checkpoint-unwrap-failed / checkpoint-coerced / resume-abandoned.
//   refuse-to-sign completeness  the refuse-to-sign completeness guards' counts reach the ring: a fan-out run
//                                that produces NO ARCHIVE carries a sized shortfall rather than a raw message
//                                coarsened to a generic failed row, so support can size the shortfall and
//                                tell tamper (a hash mismatch) from lifecycle eviction (a missing object).
//                                The merge NOTES each guard into the run-observation ledger and the run-fault
//                                reporter drains it.
//   lost completion verdicts     the engine COMPUTES the run's terminal outcome and records it even when the
//                                completion POST fails or is refused, so a perfect archive does not read
//                                "abandoned" and a fully-sealed BUFFERED archive is not booked FAILED. Every
//                                lost/refused completion records completion-lost.
//   RUNLOG restart               a missing RUNLOG on a destination with prior runs is never silently
//                                normalised (the signed root would otherwise chain onto nothing and the
//                                history would restart). destinationLocalPrev notes runlog-absent.
//
// PART A  refusal classification: the seven classes, the handoff class, and the destination-misattribution fix.
// PART B  refusal classification: REDACTION -- customer sentinels planted in every raw message reach no returned class.
// PART C  checkpoint corruption: the typed checkpoint faults, the coercion inspector, and the DO's four observe records.
// PART D  refuse-to-sign completeness: the merge's refuse-to-sign guards -> ledger -> drain -> posted seal-fault records.
// PART E  refuse-to-sign completeness: the DO's open-shard guards (the /start handoff refusal and the missing wrapped batch).
// PART F  lost completion verdicts: a lost / refused completion records the computed verdict.
// PART G  RUNLOG restart: an absent RUNLOG with prior runs records the history-chain restart.
...
// makeScheduler records every DO call. `completeStatus` drives the completion-lost arms: 200 = the completion
// lands, a non-2xx = the scheduler REFUSES it (which never throws, so nothing would see it without this
// record), "throw" = a DO outage.
...
// ---- PART A: refusal classification -- the seven closed classes + the handoff misattribution fix -------------------
function partA(): void {
  console.log("\nPART A: self-describing refusals get their own CLOSED class instead of a generic 'run failed':");

  ok(
    "an over-ceiling record reads as 'record over the in-band size ceiling', not a generic 'source read error'",
  ...
  ok("a shard-id overflow is named -- the sharpest latent archive-corruption risk, not a generic 'run failed'", coarseRunError("shard id index 100000 is outside the 5-digit shard-id range [0, 100000); refusing to seal an archive whose shard ids would overflow and silently reorder the signed root") === "shard id overflow (refused to seal a misordered archive)");
  ...
  console.log("\n  the seal-DO handoff refusal (an engine control-plane fault, never the customer's bucket):");
  // THE MISATTRIBUTION RISK: the refusal's message ends in "status 500" and carries no other keyword, so
  // without a dedicated class it falls through to the generic status catch-all -- the DESTINATION class, which
  // DEST-1 turns into "this customer's bucket is down". Both halves are pinned here: the catch-all still
  // classifies a real destination fault that way, and the handoff refusal is classified separately, before it
  // ever reaches that catch-all.
  ...
  ok("a handoff refusal is not swept into that destination class (it is an engine control-plane fault)", coarseRunError(legacy) !== REASON_DESTINATION_ACCESS && coarseRunError(legacy) === "seal DO handoff refused");
  ok("...so DEST-1 does not name the customer's bucket down for an engine-internal refusal", JSON.stringify(downDestinationFlag(coarseRunError(legacy), "dest_r2_syd")) === "{}");
  for (const cls of HANDOFF_REFUSAL_CLASSES) {
    const m = `seal DO handoff refused: ${cls}`;
    ok(`this form classifies as 'seal DO handoff refused (${cls})'`, coarseRunError(m) === `seal DO handoff refused (${cls})`);
    ok(`...and it does not name the destination down (${cls})`, JSON.stringify(downDestinationFlag(coarseRunError(m), "dest_r2_syd")) === "{}");
  }
...
// ---- PART B: refusal classification REDACTION ---------------------------------------------------------------------
function partB(): void {
...
// ---- PART C: checkpoint corruption -- the typed checkpoint faults + the DO's observes -----------------------------
...
// ---- PART D: refuse-to-sign completeness -- the merge's refuse-to-sign guards reach the ring ----------------------------

// MemDest: the minimal Destination the merge reads. `drop` removes an object (the lifecycle-eviction case);
// `corrupt` rewrites its bytes (the tamper case). Both raise refusals whose counts the ring captures, so
// support can size the shortfall.
...
async function partD(): Promise<void> {
  console.log("\nPART D: the merge REFUSES TO SIGN (no archive at all) and the counts reach the ring:");
...
// ---- PART E: refuse-to-sign completeness -- the DO's own open-shard completeness guards ---------------------------------
async function partE(): Promise<void> {
  ...
  ok("...and records open-shard-count-mismatch found=0 / expected=4 (the run died before a single slice)", mismatch !== undefined && mismatch.found === 0 && mismatch.expected === 4 && mismatch.downpipeId === DP);
...
// ---- PART F: lost completion verdicts -- a lost or refused verdict is recorded ---------------------------------------
async function partF(): Promise<void> {
  console.log("\nPART F: the engine computes the terminal outcome and records it even when the completion POST does not land:");

  // F1: the scheduler REFUSES the ok completion (a non-2xx). It never throws, so nothing else observes the
  // refusal: the archive is complete, the run row later resolves as a generic "abandoned", and the pack
  // contradicts the bucket. This is the "run shows abandoned but the archive restores fine" ticket.
  ...
  ok("a REFUSED ok completion records completion-lost{outcome:ok} instead of vanishing in silence", lostOk !== undefined && lostOk.outcome === "ok" && lostOk.runId === RUN && lostOk.downpipeId === DP);
...
// ---- PART G: RUNLOG restart -- an absent RUNLOG with prior runs --------------------------------------------
