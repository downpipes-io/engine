...
//
// runRoutes() calls named per-section helpers, each exercising one banner-delimited group, in a fixed order
// through the shared ok() counter, so the assertion sequence and the suite total stay consistent.
...
async function routesComputeAndAccept(): Promise<void> {
  ...
  ok("DO: an owner can accept recipient-set-expected (200)", rseAccepted.status === 200);
...
// ---- restore-test-recency GRADES through the real gather->compute path --------
// The CRITICAL restore-test-recency check depends on a `lastRunAt` epoch that only the real gather path
// projects from the DownpipeState onto PostureInput. A pure computePosture test over a hand-built
// PostureInput bypasses gatherPostureState and would not exercise that projection. These vectors drive the
// EXACT production path so the projection itself is exercised: a downpipe created via POST /downpipes, a
// real run recorded via /trigger + /complete (so lastRunId is set on the DownpipeState), and a scheduled
// restore test recorded via /restore-test-complete with a backdated `at`. A downpipe with a source binding
// is created; the DO double records the run/test state directly, no seal I/O is invoked.
...
// ---- the off-account-audit-mirror slice reaches the report from DO state ------------------
// The pure builder is graded in validate-posture-compute.ts. What is graded HERE is the wiring: the DO must
// read its own SIEM push config and delivery trail and put them in the PostureInput. A builder nothing
// feeds grades nothing, and the router forwards no part of this slice (it is entirely DO-owned), so this is
// its only wiring proof.
async function routesAuditMirrorSlice(): Promise<void> {
  const slice = { destConfigured: true, breakGlassConfigured: true, operationalConfigured: { public: true, private: false }, tokenFallbackDisabled: true };
  {
    const { stub } = makeScheduler();
    const resp = await fetchDO(stub, "POST", "/posture", { status: slice, authMethod: "access", beaconEnabled: false });
    const { report } = (await resp.json()) as { report: PostureReport };
    const c = byId(report).get("off-account-audit-mirror");
    ok("DO: an estate with no audit-log push destination FAILS off-account-audit-mirror", c?.status === "fail");
    ok("DO: ...at medium", c?.severity === "medium");
  }
  {
    const { stub, storage } = makeScheduler();
    // The stored record shape the DO reads (scheduler-do-limits.ts PushDestinationRecord). The auth secret
    // is a plaintext placeholder on the back-compat floor; the posture path reads the REDACTED view, so it
    // never sees it, which is the point of taking the view rather than the raw record.
    await storage.put("siemPushConfig", { endpoint: "https://siem.example.au/in", format: "splunk-hec", authHeaderName: "Authorization", authHeaderValue: "placeholder", enabled: true, setAt: Date.now(), setBy: "o1@example.au", gen: "gen-1" });
    await storage.put("siemPushTrail", [{ at: "2026-09-09T00:00:00.000Z", ok: true, count: 12, fromSeq: 1, toSeq: 12 }]);
    const resp = await fetchDO(stub, "POST", "/posture", { status: slice, authMethod: "access", beaconEnabled: false });
    const { report } = (await resp.json()) as { report: PostureReport };
    const c = byId(report).get("off-account-audit-mirror");
    ok("DO: a configured, draining push whose last delivery succeeded PASSES", c?.status === "pass");
    ok("DO: ...and the detail says the most recent delivery succeeded", /most recent delivery succeeded/.test(c?.detail ?? ""));
  }
  {
    const { stub, storage } = makeScheduler();
    await storage.put("siemPushConfig", { endpoint: "https://siem.example.au/in", format: "splunk-hec", authHeaderName: "Authorization", authHeaderValue: "placeholder", enabled: true, setAt: Date.now(), setBy: "o1@example.au", gen: "gen-1" });
    // Two attempts: the trail is append-at-end, so the LAST one is the newest and is the one graded. An
    // implementation reading trail[0] would read the older, successful attempt and report a healthy mirror
    // over a failing one, which is why both are seeded rather than just the failure.
    await storage.put("siemPushTrail", [
      { at: "2026-09-08T00:00:00.000Z", ok: true, count: 4, fromSeq: 1, toSeq: 4 },
      { at: "2026-09-09T00:00:00.000Z", ok: false, reason: "endpoint-unreachable" },
    ]);
    const resp = await fetchDO(stub, "POST", "/posture", { status: slice, authMethod: "access", beaconEnabled: false });
    const { report } = (await resp.json()) as { report: PostureReport };
    const c = byId(report).get("off-account-audit-mirror");
    ok("DO: a configured push whose MOST RECENT delivery failed FAILS (the newest attempt is the graded one)", c?.status === "fail");
    ok("DO: ...and the detail says it has not delivered, not that no mirror exists", /has not delivered/.test(c?.detail ?? ""));
  }
}
