...
// counts (optional, ASVS ML-43): when passed, counts.downpipes is bumped on every /downpipes read, so a
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log("\nsource-binding drift (the deploy-dropped-a-source probe):");
    {
      ...
      // ASVS ML-43: the roster is read ONCE per runPreflight() and shared by source-bindings,
    ...
    console.log("\ndestination evidence redaction (NC-5):");
    {
    ...
    console.log("\nkeys are set but do not parse, and an operational recipient is present:");
    {
      ...
      // generic "recipient keys are set but do not parse", with the parse message truncated at 60 chars and
      // no statement of WHICH of the two env vars to re-paste. The load-bearing check is the CLOSED
      // discriminator beside it, which no truncation can cut off.
