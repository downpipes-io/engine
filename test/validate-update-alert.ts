// W3: one-shot "new version available" alert. The cron (src/index.ts runUpdateAlertIfNew) pulls + verifies
...
async function main(): Promise<void> {
  ...
  ok("[W3] severityOf(update-available) === warning", severityOf("update-available") === "warning");
  ok("[W3] isNotifyEvent admits update-available (selectable in a rule)", isNotifyEvent("update-available") === true);

  // ---- once-per-version idempotency ----
  {
    const { stub } = makeDO();
    ok("[W3] first claim for 0.2.0 => alert", (await claim(stub, "0.2.0")) === true);
    ok("[W3] second claim for the SAME 0.2.0 => no alert (deduped, never spammed)", (await claim(stub, "0.2.0")) === false);
    ok("[W3] third claim for the same version still no alert", (await claim(stub, "0.2.0")) === false);
    // A genuinely NEW version alerts once, then dedupes again.
    ok("[W3] a NEW version 0.3.0 => alert once", (await claim(stub, "0.3.0")) === true);
    ok("[W3] repeat 0.3.0 => no alert", (await claim(stub, "0.3.0")) === false);
  }

  // ---- a blank / missing / non-string version never claims (no spurious alert) ----
  {
    const { stub } = makeDO();
    ok("[W3] blank version => no alert", (await claim(stub, "")) === false);
    ok("[W3] whitespace version => no alert", (await claim(stub, "   ")) === false);
    ok("[W3] missing version => no alert", (await claim(stub, undefined)) === false);
    ok("[W3] non-string version => no alert", (await claim(stub, 123)) === false);
    // ...and a real version still alerts after the no-ops (the no-ops recorded nothing).
    ok("[W3] real version after no-ops still alerts", (await claim(stub, "1.0.0")) === true);
  }
  ...
  // ---- the claim records the version (so the dedupe survives a re-read) without disturbing pending/last ----
  {
    ...
    ok("[W3] claim records lastAlertedVersion", rec.lastAlertedVersion === "0.2.0");
    ok("[W3] claim preserves the pending record (does not clobber the lifecycle)", rec.pending !== null && (rec.pending as { toVersion?: string }).toVersion === "v-new");
  }

  // ---- FOLD 1: the canary cron's "rollback needed" escalation for a promoted-but-unsettled bad version ----
  ...
  ok("[FOLD1] severityOf(update-rollback-needed) === critical (page now)", severityOf("update-rollback-needed") === "critical");
  ...
  ok("[FOLD1] update-rollback-needed is a system critical (not rule-selectable), like canary-dead", isNotifyEvent("update-rollback-needed") === isNotifyEvent("canary-dead"));
  {
    const { stub, storage } = makeDO();
    // No pending -> no escalation (the version is already settled; nothing to roll back).
    ok("[FOLD1] rollback-claim with NO pending => no alert", (await claimRollback(stub, "0.2.0")) === false);
    ...
    ok("[FOLD1] first rollback-claim for an unsettled pending => alert", (await claimRollback(stub, "0.2.0")) === true);
    ok("[FOLD1] second rollback-claim for the SAME version => no alert (deduped)", (await claimRollback(stub, "0.2.0")) === false);
    const rec = storage.rawGet<{ pending: unknown; rollbackNeeded?: { recommendedVersion: string } }>("updateLifecycle")!;
    ok("[FOLD1] the rollbackNeeded flag is recorded (the console surfaces the urgent prompt)", rec.rollbackNeeded?.recommendedVersion === "0.2.0");
  }
  {
    ...
    ok("[FOLD1] settling clears pending AND the rollbackNeeded flag", rec.pending === null && (rec.rollbackNeeded ?? null) === null);
  }
  {
    ...
    ok("[FOLD1] the update-available claim preserves the rollbackNeeded flag", rec.rollbackNeeded?.recommendedVersion === "0.2.0" && rec.lastAlertedVersion === "0.2.0");
  }

  console.log(failures === 0 ? "\nW3 UPDATE-ALERT + FOLD-1 VECTORS PASS" : `\n${failures} FAILURE(S)`);
