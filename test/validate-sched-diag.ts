...
//
// Each block proves the SAME two things required of every new recording:
...
//
// The tickets each block makes diagnosable:
...
//   "after the engine rolls back, one downpipe stops backing up entirely" -> the refused dp:
//        record is STAMPED (closed class + the version pair + a count) instead of skipped with a log line.
//   "a backup ran during a declared change freeze" / "the freeze window never applied" -> blackoutResolve
//        stamps the closed class on the downpipe, and config validation REFUSES a degenerate (inert) window.
...
// ---- housekeeping faults that would otherwise pass silently ---------------------------------------
async function testSchedHealth(): Promise<void> {
  // (1) the alarm's opportunistic housekeeping sweep THROWS -> counted, and the alarm still re-arms.
  {
    ...
    ok("a THROWING housekeeping sweep is counted (uncounted, it grows DO storage silently for months)", (health?.sweepFaults?.count ?? 0) >= 1 && (health?.sweepFaults?.lastAt ?? 0) > 0);
  ...
  //     truncated dp: enumeration means downpipes past the cap are NEVER scheduled and read as nonexistent.
  {
    ...
    storage.list = realList;
    const health = storage.rawGet<Record<string, { count: number }>>(SCHED_HEALTH_KEY);
    ok("a TRUNCATED prefix enumeration is counted (uncounted, it silently under-reports the fleet)", (health?.listTruncations?.count ?? 0) === 1);
...
// ---- a rolled-back engine silently skipping a downpipe ---------------------------------------
...
async function testBlackoutResolve(): Promise<void> {
  ...
  // (5) the degenerate window is REFUSED at the write path, so it becomes a counted refusal the operator
  //     sees immediately instead of a freeze that would otherwise silently do nothing for months.
  let refused = false;
  try {
