...
//
// See test/destsim/PULL-ABUSE-BEHAVIOUR.md ("other observations") for the two behaviours this file
// pins that are worth a reader's attention even though neither is scored as a defect: the /metrics
...
async function testAuthGating(): Promise<void> {
  ...
  }
  {
    ...
    // /support/* short-circuits an absent bearer BEFORE any DO round trip, matching /metrics -- 0 DO fetches,
    // rather than paying a DO round trip on every unauthenticated probe (a softer DoS target than /metrics).
