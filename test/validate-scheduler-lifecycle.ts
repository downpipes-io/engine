// SchedulerDO lifecycle vectors (engine-test-018-01 split, first half: TC-01 through TC-09): add/list,
...
// in validate-scheduler-runlog.ts. Behaviour-preserving: every assertion below ran in the original
// main() in this exact order.
...
export async function run(): Promise<void> {
  ...
  // ---- TC-06b: inFlight LEASE reclaims a crashed run instead of wedging forever (ENG-B2) ------
  ...
  // ---- TC-08: runlogIndex monotonicity across multiple downpipes ----------------------
  {
    ...
    // owns the in-flight run and resolves it (completeRun matches the row/runId, ENG-M13).
