...
export default {
  async fetch(req: Request, env: ProbeEnv, ctx: ExecutionContext): Promise<Response> {
    ...
    // which is unavailable used to drop the write silently: on a non-2xx it notes the loss in the
    ...
    if (url.pathname === "/signal-stepup") {
      ...
      // The call site passes the runtime exactly as router.ts:547 and its four twins now do; ?runtime=0 drops
      // it, which is the pre-repair shape and the arm that shows the loss.
    ...
    //
    //   ?hold=bare        void routeAuthChangeAlert(...)                        the pre-repair shape
    //   ?hold=waituntil   ctx.waitUntil(routeAuthChangeAlert(...))              THE SHAPE THE REPAIR LANDED
    ...
    // =========================================================================================================
    // NOTIFY-ALERT ARMS. The destination-change drive above repaired routeEmission for the four
    // destination-change alerts; every OTHER routeXxxAlert call site is repaired by the same helper change but
    // was left unmeasured. These arms drive those other call sites. They add routes; they change nothing the
