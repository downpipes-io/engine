...
// await. The offline validators run the DO serially, so they cannot test this. The apply runs inside
// blockConcurrencyWhile with a re-read and a re-assert that the status is still pending.
//
...
//
// THE DWELL FLOOR (ASVS V2.4.2). approveChange refuses an approve that arrives less than
// APPROVAL_MIN_DWELL_MS after the change's own proposedAt, so an approve fired in the same turn as its
// propose is refused "too-soon" before either concurrent approve reaches the read-modify-write this suite
// exists to race. Each round therefore waits AGE_PAST_DWELL_MS after its own propose before firing that
// round's approve pair (a round's propose is itself built against the config baseline the PRIOR round's
// approve left behind, so rounds stay sequential; proposing every round up front would leave every change
// but the first superseded by the time it is finally approved, and prove nothing about the race). This is a
// real wait, not a storage rewrite: the suite drives the DO only over its own HTTP surface in a genuine
// workerd isolate, which never hands the test process a handle onto the DO's storage the way the in-process
// MockStorage validators do (ageRecord, test/testutil.ts) -- the DO instance is the only thing that ever
...
// time). EVERY round adds exactly one channel.
const ROUNDS = Number(process.env.ROUNDS) || 24;

// AGE_PAST_DWELL_MS clears the dwell floor (ASVS V2.4.2) with margin, mirroring the AGE_PAST_DWELL constant
// the offline validators (validate-dualcontrol.ts, validate-approval-dwell.ts) age a stored record by.
const AGE_PAST_DWELL_MS = APPROVAL_MIN_DWELL_MS + 1_000;
...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < ROUNDS; i++) {
      ...
      // AGE past the dwell floor (ASVS V2.4.2) so the two concurrent approves below reach the
      // read-modify-write this round exists to race, rather than both being refused too-soon.
      await new Promise((resolve) => setTimeout(resolve, AGE_PAST_DWELL_MS));
    ...
    // ASSERTIONS (the apply-exactly-once contract): the apply runs EXACTLY ONCE per change.
