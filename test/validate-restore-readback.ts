...
async function proofReceiptSignature(receipt: RestoreReceipt, signerB64: string): Promise<void> {
  ...
  // recordsSkipped: the count the apply deliberately did not write. The receipt used to state only what
  // LANDED, so a reader auditing a recovery saw "restored 98, allVerified true" and could not tell that
  // records from the archive were still absent from the account.
  //
...
// This is the bug R2 closes: before, a small buffered R2 record re-hashed the SAME in-memory bytes and could
// not notice a write that persisted wrong bytes. Now the buffered path reads the landed object BACK.
