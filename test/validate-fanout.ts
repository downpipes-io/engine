// Prove fan-out 2b: a high-cardinality LEX-ORDERED KV run split across N parallel WORKER ranges and
...
async function main(): Promise<void> {
  ...
  console.log("\nrange cap (H1): planned ranges are sized strictly below the KV watermark-rescan ceiling:");
  {
    ...
    // single range can never fast-forward past the slice budget on a cursor-refused resume and wedge (H1).
  ...
  console.log("\nH1 silent stall: a worker that never progresses (the uncatchable wedge) is STRUCK OUT, fails closed + releases the lease:");
  {
    ...
    // Cover both stall modes the review named: range 0's worker is frozen-but-PRESENT (the rescan wedge --
  ...
  console.log("\nML-25: a /range-done that lands WHILE awaitTick's own fetches are in flight is not reverted by its stale write-back:");
  {
    ...
    // coordinator's own route -- exactly the concurrent worker-initiated call the finding describes.
  ...
  console.log("\nML-25 (bundled gap): a /range-failed arriving after the coordinator has already flipped to MERGE no longer wipes it:");
  {
  ...
  console.log("\nML-25 (bundled gap 2): a range that finishes one tick before its OWN strike ceiling does not strike out an otherwise-healthy run:");
  {
    ...
    // -- the same net harm ML-25 originally described, just reached via a straggler instead of a fresh range.
    const dest = new MemDest();
    const sched = makeScheduler();
    const breakGlassB64 = b64urlEncode(concat(breakGlass.entry.pub.x25519, breakGlass.entry.pub.mlkemEk));
    const { env, instances } = makeFleet({ dest, kv: new MemKV(recs, pageSize), sched: sched.stub, signerPrivateB64, breakGlassB64, knobs: { SCALE_FANOUT_RANGES: "3", SCALE_FANOUT_MIN_RECORDS: "1", SCALE_SHARD_MAX_RECORDS: "3", SCALE_SLICE_SUBREQUESTS: "44" } });
    ...
    // concurrent worker-initiated call the finding describes (its worker is now "gone" per the gate above).
    const rd = await coord.obj!.fetch(new Request("https://runseal.internal/range-done", { method: "POST", body: JSON.stringify({ rangeIndex: 1, shards: [], recordCount: 0, counts: zeroCounts() }), headers: { "content-type": "application/json" } }));
  ...
  console.log("\nbalanced planner (Fix 2b H2): count+stride sizes BALANCED ranges over the WHOLE keyspace, not a front sample:");
  {
    ...
    // LAST range absorbed the whole tail -- the H2 skew (~weeks for 50M). The new count+stride scan sizes 4
  ...
  console.log("\nbalanced planner cap interaction (H1): total/N over the per-range cap splits into MORE ranges, each <= cap:");
  {
  ...
  console.log("\nsliced planning resume (Fix 2b H2): the keys-only scan spans >1 invocation and resumes after the last persisted page:");
  {
  ...
  console.log("\nmin-records un-clamp (Fix 2b H2): a SCAN-path run below the now-KNOWN threshold DOWNGRADES to serial:");
  {
