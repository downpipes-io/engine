...
async function main(): Promise<void> {
  ...
  // isCfSize, not `!== null`: an unmeasurable size answers with a fault CLASS STRING rather than a
  // bare null, so `!== null` would let "schema-drift" through the guard and only the field comparisons would
  ...
  // The invariant is unchanged: no shape may throw, and none may yield a WRONG NUMBER -- the probe still
  // degrades to "unavailable". The failure is no longer a bare null. A null
  // collapsed four answers into one, and the difference is the whole remediation: an auth/scope fault is a
  // token the operator must widen (permanent, actionable), a SCHEMA DRIFT is a Cloudflare-side dataset change
  // no customer action can fix, and a network fault heals itself. Worse, an analytics response the engine
  // could not read was indistinguishable from a source that genuinely holds nothing -- so a broken discovery
  // token showed the customer an EMPTY ACCOUNT and invited them to conclude they had nothing to back up.
