...
async function main(): Promise<void> {
  ...
  // absent record reads the default.
  ...
  // a blank version never claims an update alert (non-string -> "" -> early false).
  ...
  // a blank version never claims a rollback (non-string recommended/to/verdict -> early false).
  ...
  // first pending, no prior lastAlertedVersion -> the absent `: {}` arm.
  ...
  // pending present, no prior rollback flag, no lastAlertedVersion -> first claim alerts.
  ...
  // the SAME recommended version is deduped (already observed) -> no second page.
  ...
  // settle with no lastAlertedVersion -> the absent `: {}` arm; resolves pending + clears the flag.
  ...
  // a rollback claim with a real version but NO pending -> the !pending early false.
  ...
  // a genuinely new version (no rollback flag present) records + alerts.
  ...
  // pending with a lastAlertedVersion present -> the present arm carries it through.
  ...
  // rollback claim while a lastAlertedVersion is present -> the present arm; records the flag.
  ...
  // a new alert version with a rollback flag PRESENT -> the rollbackNeeded present arm.
  ...
  // settle with a lastAlertedVersion present -> the present arm.
  ...
  // ============================================================================================
  // the FIRST "applied" settle adopts the settled semver as the high-water mark (semverMax no-current arm).
  ...
  // a LOWER applied settle never lowers it (semverMax keep-current arm, cmp = -1).
  ...
  // a HIGHER applied settle advances it (semverMax adopt-candidate arm, cmp = 1).
  ...
  // an UNCOMPARABLE applied version keeps the existing mark (semverMax cmp = null fail-safe arm).
  ...
  // an "applied" settle with NO recommendedVersion leaves the mark unchanged (settledVersion === "" arm).
  ...
  // a non-applied (rolled-back) settle never advances the mark.
  ...
  // a cron update-alert claim (returns {shouldAlert}) must CARRY the mark forward, read it back.
  ...
  // a promote carrying a channel freshness claim ADVANCES the persisted watermark (present advance arms),
  ...
  // a rollback-needed claim while pending (returns {shouldAlert}) preserves BOTH; read it back.
  ...
  // and the freshness-watermark advance a settle can carry (a console-only apply records no engine pending).
  // ============================================================================================
  // a CONSOLE applied settle lands in lastConsole + floors.console; the ENGINE's last + floor
  ...
  // a console ROLLED-BACK settle never advances floors.console.
  ...
  // an ENGINE settle that is NOT applied ABORTS a queued console intent with an honest record.
  ...
  // an ENGINE settle that IS applied leaves the queue to the router's console continuation (the
  ...
  // a CONSOLE settle while an engine pending still carries a queue STRIPS the consumed queue.
  ...
  // A settle carrying the channel freshness claim advances the watermark (the console-only
  ...
  // the history ring is BOUNDED (newest kept, capped at 20).
  for (let i = 0; i < 22; i++) {
  ...
  // READ-TIME MIGRATION -- a legacy record carrying only the settledHighWaterMark scalar (no floors
  ...
  // ============================================================================================
  // an engine KEEP decided via the self-check carries confirmationPending:true.
  ...
  // a MISMATCHED recommendedVersion (a different version now confirmed alive) -> clean no-op, the
  ...
  // a blank/missing recommendedVersion -> clean no-op (never a wildcard clear).
  ...
  // the MATCHING recommendedVersion clears the flag, and touches NOTHING else (floors/history sticky,
  ...
  // a REDUNDANT confirm (already cleared) -> clean no-op (confirmationPending is no longer === true).
  ...
  // a settle whose outcome is NOT applied (e.g. rolled-back) never confirms, even naming its version.
  ...
  // ============================================================================================
  // Update provenance: artefactSha384 (the signed channel digest the apply verified) rides
  ...
  // a promote carrying the digest persists it on the pending record.
  ...
  // the settle carrying the digest lands it on the engine's last record AND in the history ring.
  ...
  // a console settle carries its OWN bundle digest into lastConsole (never the engine's).
  ...
  // absent stays absent -- a digest-less settle writes a record without the field.
  ...
  // a non-owner is refused (the clear path routes straight to setDiscoveryToken; the owner gate
  ...
  // a valid token but every account entry is invalid (non-string id / empty id / no id) -> 400.
  ...
  // success, ONE account, as the Access owner (email + sourceIp). Single -> auto-select + engine account.
  ...
  // success, MULTIPLE accounts, as the token break-glass (null email). Name fallback to id; not-single
  ...
  // Owner SET, deduped. A type outside the closed set is REFUSED rather than filtered out with a 200:
  // silently dropping an unknown type would return a success that hides what was dropped, the
  // "I added it and it never appeared" failure shape.
  ...
  // Assert the exact message, not just the status: the offending value and the choice list must both be
  // named in the wording, not merely implied by a 400. The choices are derived from the engine's own
  // exported list rather than restated, so a sixth source type needs no edit here.
  ...
  // getDiscoveryStatus now carries enabledSources (the present `?? []` arm).
  ...
  // non-array sources clears the set, as the token caller (null-email audit arm).
  ...
  // ============================================================================================
  // ACCOUNT SELECTION (setDiscoveryAccounts). The config now lists a1/a2/a3 (from the multi-account discovery above).
  // ============================================================================================
  // non-owner refused (the gated route applies inline with the gate off, so the method's own
  ...
  // owner success, known-id filter + dedupe + a known engine account.
  ...
  // every selected id unknown -> empty selection -> 400.
  ...
  // non-array selected -> empty selection -> 400 (the non-array `: []` arm).
  ...
  // a known selection but an UNKNOWN (string) engineAccountId -> null (the `&&` right-false arm),
  ...
  // a NON-STRING engineAccountId -> null (the `&&` left-false short-circuit).
  ...
  // ============================================================================================
  // owner clear (email-present clear-audit arm).
  ...
  // token-caller clear (null-email clear-audit arm); idempotent.
  ...
  // with NO config, both selection and enable are clean 400s (the no-config `if (!c) throw` arms).
  ...
  // Clear, owner then token-caller (both clear-audit arm sides).
  ...
  // a malformed/blank accountId is dropped silently, nothing stored, no caller required.
  ...
  // NO CALLER AT ALL succeeds (the point of this being unlike setDiscoveryToken/setDiscoveryAccounts
  ...
  // NEVER OVERWRITTEN once set -- the account a running Worker script lives in cannot change under
  ...
  // a fresh DO's first call with via:"update-apply" is honoured (proves the parameter is read, not
  // just defaulted every time -- the no-via call above proved the default, this proves the explicit value).
  ...
  // actorMethod "engine" (not "access"): a structural gate refuses an "access" row that hard-codes
  // sourceIp:null, since "access" means an authenticated human who always has a real address.
