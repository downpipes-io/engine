...
//
// SIZE SPLIT: the in-memory DO doubles, the scheduler builder, the forged-JWT
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // PROOF EXPORT-COMPLETE: the compliance export is COMPLETE past AUDIT_PAGE_MAX (JSON +
    ...
    // AXIS TELEMETRY / AUDIT-TAMPER: the seven cells
    ...
    // it is fine. Measured by plant: stamping an uncovered field on the event slept through all seven
    // audit members, the independent oracle included. Builds its own events, so it perturbs no state.
