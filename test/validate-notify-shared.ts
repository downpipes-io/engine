// Shared fixtures and helpers for the validate-notify suite (split out of validate-notify.ts
// per finding engine-test-009-01 so each area module stays under the size cap). Every helper
// here is byte-identical to the original; the only addition is a shared failure counter so the
// orchestrator can report the aggregate pass/fail across the extracted groups.
