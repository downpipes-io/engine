// Proves that an incompleteness-marker record is STAMPED with its marker KIND
...
async function main(): Promise<void> {
  ...
  // this proves the derivation UPSTREAM of that, in runBackup (seal/pipeline.ts), where a value-shape-based
  // derivation could otherwise collide: two records here carry the IDENTICAL marker-shaped top-level key
  ...
  console.log("marker-part-5: runBackup stamps incompleteMarker from the adapter's markerKind, not the value's shape");
  {
    ...
    // asserts markerKind for it, which is exactly the collision this derivation must avoid.
