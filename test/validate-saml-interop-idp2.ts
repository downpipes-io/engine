// Real-world-IdP interop corpus: ec:InclusiveNamespaces PrefixList + inherited namespaces (ADFS / Shibboleth).
// Split out of validate-saml-interop.ts; the orchestrator imports run() and calls it with the shared harness +
// keys. Every assertion here runs in the original order.
...
export async function run(h: Harness, keys: Keys): Promise<void> {
  ...
  console.log("\nec:InclusiveNamespaces PrefixList + inherited namespaces (ADFS / Shibboleth):\n");
  ...
  // value-only prefix would be dropped) and a genuine assertion would be wrongly rejected.
  {
    ...
    h.ok("CONTROL: an ADFS-style xsi:type assertion signed with InclusiveNamespaces PrefixList=xs verifies", r.ok === true);
    if (r.ok) {
  ...
  // Also at the RESPONSE level: a Response-signed ADFS-style message (Response signature, InclusiveNamespaces
  // PrefixList="xs", unsigned assertion carrying the xsi:type values) verifies via the combined response+assertion path.
  {
    const assertion = buildAssertion({ xsiTypedEmail: true });
    ...
    h.ok("A Response-signed ADFS-style message with PrefixList=xs verifies", r.ok === true);
  }

  // Negative: the SAME ADFS-style assertion, but the digest was computed over the INCLUSIVE form while the
  ...
  // inclusive digest is honoured exactly as declared.
  {
    ...
    const resp = buildResponse([signed]);
    const r = await verifySamlResponse(encodeResponse(resp), baseConn([certPem]), baseCtx());
    h.ok("Negative: an inclusive-form digest with the PrefixList stripped from the document is rejected (digest mismatch)", r.ok === false);
  }

  // Negative: a forged PrefixList listing a DIFFERENT prefix than the one the digest was computed with.
  ...
  // unbound 'bogus' and drops xs -> mismatch -> reject.
  {
    ...
    const resp = buildResponse([signed]);
    const r = await verifySamlResponse(encodeResponse(resp), baseConn([certPem]), baseCtx());
    h.ok("Negative: a forged PrefixList (different prefix than signed) is rejected (digest mismatch)", r.ok === false);
  }

  // Compatibility: an xsi:type assertion signed WITHOUT any InclusiveNamespaces list (the other ADFS/Entra
  ...
  // because the IdP digest and the SP digest both drop xs under pure exclusive rules).
  {
    ...
    const resp = buildResponse([signed]);
    const r = await verifySamlResponse(encodeResponse(resp), baseConn([certPem]), baseCtx());
    h.ok("Compat: an xsi:type assertion signed with NO PrefixList (plain exclusive) still verifies", r.ok === true);
  }
}
