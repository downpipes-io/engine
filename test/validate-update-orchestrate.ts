...
async function main(): Promise<void> {
  ...
  {
    // The per-component floor: a console version BELOW floors.console refuses BEFORE anything runs.
  ...
  {
    // CONFIRM MISMATCH + the rollback deploy ALSO fails -> outcome "rollback-failed".
    //
    // The outcome is never reported as "rolled-back" when the revert did not happen: the console is still
    // serving the unexpected build. The prose can be honest while the enum is not, and the enum is the only
    // part any consumer reads.
  ...
  }
  {
    ...
    // read-back never happened, so the engine cannot say which version is live. Reporting "applied" here would
    // assert a confirmation it does not have. The KEEP behaviour stays the same (one deploy call, no revert);
    // only the claim about it is accurate.
