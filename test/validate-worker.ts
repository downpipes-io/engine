...
// the immutable-headers regression (TC-04), OPTIONS preflight, root path, the
// scheduled() handler swallowing a drive() error without crashing, AND the cron
// orchestration seal HAPPY PATH (XC-H4 / XC-H11 / ENG-H10): scheduled() -> drive() ->
...
//
// This file is a thin orchestrator: the assertions live in the sibling area modules
// (engine-test-022-01 split). It imports each group and CALLS them in the same order the
// monolith ran them, then reports the shared `ok` tally and exits non-zero on any failure.
//   - validate-worker-http.ts    : root/404, CORS, security headers, TC-04, run-now, error handler
...
async function main(): Promise<void> {
  console.log("TC-04 + worker entrypoint validations\n");
