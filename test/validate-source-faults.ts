// Source-side chaos validators (engine-src-fault).
//
...
// asserts the engine's behaviour. The headline question is the analyst-flagged "KV cursor-refusal
// wedge": under a refused cursor, does a run WEDGE, LOSE keys, or fail closed? Plus mid-crawl key
// deletion, a throttle/transient that recovers, an R2 etag change, and a D1 page error mid-stream.
//
...
async function scenarioDeterministicRefusalFailsClosed(): Promise<void> {
  ...
  // FINDING (minor): a raw KV cursor refusal carries no recognised token, so coarseRunError yields the
  // generic class -- the operator gets no actionable "the source cursor was refused" hint.
...
async function scenarioMidCrawlDeletion(): Promise<void> {
  ...
  await finalise(makeSource, dest, driven.cp, driven.allShards, driven.openBuffer);
  const run = await openArchive(dest);
  // WS-P2 (marker record): a mid-crawl-vanished key now seals an honest _vanished MARKER record in its place,
  ...
  // WS-P2: the vanished key seals a _vanished marker, so it counts BOTH as an incompleteness marker
...
// dependency. This is the FIXED behaviour: a KV/R2 binding read throttle/5xx is now named a SOURCE read.
function scenarioClassificationPins(): void {
  ...
  // FIXED: a KV/R2 binding read throttle/5xx whose message carries 'status NNN' is now caught by a
  // source-side branch BEFORE the destination catch-all, so a SOURCE outage is attributed to the SOURCE
  // (the right system) instead of the destination. (The DO's throttle ROUTING already keyed off its own
  // 503/429 regex and parked correctly; this is only the final coarse LABEL on a struck-out run.)
  ...
  // The point of the fix: a source status fault must no longer be attributed to the destination.
  ...
  // D1 read faults were already named by the earlier D1_-prefixed branch (unchanged by the fix).
