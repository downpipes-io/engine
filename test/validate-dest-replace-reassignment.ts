...
//
// THE FIX, this validator drives directly against the real SchedulerDO mixin (DestConfigMixin) and the
...
//
// RED BEFORE GREEN: every section below is written to name what a reader would see AT THE PRE-FIX TREE
// (destinationId undefined, primaryDestinationId(config) undefined -> [undefined] -> resolves the NEW
// default) in its own comment, then asserts the POST-FIX behaviour.
//
...
async function main(): Promise<void> {
  ...
  console.log("-- the rehearsal sequence: 19 deploy-time runs, then the console's first add --");
  {
    ...
    // "add", not "replace". AT THE PRE-FIX TREE, putDest's own "first destination becomes the default"
    // branch would have made archive-2 (the ONLY entry `coll.list` had ever held) the default instead.
    ...
    // THE READ PATH. AT THE PRE-FIX TREE: entry.destinationId is undefined and primaryDestinationId(an
    // UNPINNED downpipe's config) is ALSO undefined, so destinationForRun/destinationsForRun fall to
    // "the current default" -- archive-2, empty, and every drill of the 19 prior runs answered "object
    // missing", byte for byte. Post-fix, the backfill during the seed stamped every one
    ...
    // THE ORPHAN GUARD. AT THE PRE-FIX TREE this fired on archive-2 (the false "only proven copy of 19",
    // quoted verbatim) while the true 19-run holder had no refusal protecting it at all. Post-fix
    // it must protect the ACTUAL holder, "deploy", and free the genuinely-empty archive-2 immediately.
  ...
  //    without a byte moving -- exactly the setDefaultDest path section 3 exercises.
  // ==============================================================================================
  ...
  console.log("-- setDefaultDest repoint: the read path (not just the removal guard) follows the backfill --");
  {
    ...
    // AT THE PRE-FIX TREE (before backfillDefaultRoutedRuns existed): destinationForRun for these 19
    // rows would now resolve archive-2 (today's default), even though "deploy" still holds every byte
    // and archive-2 remains empty -- the read-path half of the SAME defect, on the plain repoint case
    // the priorDefaultIds comment in scheduler-do-limits.ts already names for the removal guard alone.
    ...
    // The removal guard (already fixed by priorDefaultIds before this campaign) and the read path now
    // agree: removing "deploy" is refused for the same 19 runs, from either angle.
    const removeDeploy = await caught(() => dobj.removeDest(DEPLOY_DEST_ID, false, OWNER));
