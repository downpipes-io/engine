...
// AGE_PAST_DWELL (ASVS V2.4.2, approval-dwell): comfortably past the floor, used to back-date a crafted
// record's proposedAt so an approve against it is not itself refused as too-soon.
...
async function main(): Promise<void> {
  ...
  const { sched, OWNER, OWNER2, OPERATOR, subjectOf, call, readLog, setGate, doFetch, ownerCaller, ownerActionKeyCount, destConfig, oidcProposal, listDestinations, listIdpConnections, discoveryStatus, ageOwnerAction } = ctx;
  ...
  try {
    ...
    // ===========================================================================================
    {
      ...
      // proposedAt is back-dated past the ASVS V2.4.2 dwell floor so the approve below is refused for the
      // reason this case is actually about (the DO-executed dispatch fail-closed), not for arriving too soon.
      const a = await putCrafted({ kind: "update-apply", params: UA, status: "pending", routerExecuted: false, proposedAt: new Date(Date.now() - AGE_PAST_DWELL).toISOString() });
      ...
      const b = await putCrafted({ kind: "dest-default", params: { id: "no-such-dest" }, status: "pending", proposedBySubject: null, proposedByGroups: undefined, proposedAt: new Date(Date.now() - AGE_PAST_DWELL).toISOString() });
      ...
      const gRem = await putCrafted({ kind: "dest-remove", params: { id: 123, force: true }, status: "pending", proposedAt: new Date(Date.now() - AGE_PAST_DWELL).toISOString() });
      ...
      const gDef = await putCrafted({ kind: "dest-default", params: { id: 123 }, status: "pending", proposedAt: new Date(Date.now() - AGE_PAST_DWELL).toISOString() });
