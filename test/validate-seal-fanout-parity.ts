...
//
// This proves three properties of the fan-out / sliced population, the ring's own ingestion, and the knob
// resolvers:
//
//   The fan-out / sliced population -- the BIGGEST customers -- must produce rows as complete as inline runs:
//   a fan-out failure completion carries a causeDigest (so a customer's Logpush export can be joined to the
//   failed row), an ok fan-out row carries recordsVanished / incompleteByMarker / incompleteIds /
//   multipartAbortFailed when non-zero (so a fanned-out archive cannot look WHOLE while the same shortfall is
//   reported honestly inline), a sole-destination outage lights the down-destination marker on a DO failure,
//   a "worker stalled" row says WHICH range, and the coordinator's lease-loss posts the lease-lost-abandon
//   ring entry a serial resume does. The seal DO's completion bodies go through ONE rule set
//   (multipartAbortFlag / downDestinationFlag, shared with the inline path); the worker digests its own raw
//   fault at the detection point and reports the 12-hex digest; the coordinator observes lease-lost-abandon,
//   fanout-range-stalled (with the range index) and fanout-report-discarded into the seal-fault ring.
//
//   The seal-fault ring's OWN ingestion must never drop an observation silently: a failed fire-and-forget
//   POST during a scheduler-DO outage, a kind added engine-side but never added to SEAL_FAULT_KINDS, and a
//   malformed count must never read back as an honest 0 (the input a false severe-truncation escalation
//   needs). postSealFault is the one send-side chokepoint: it tallies drops by CLOSED class and flushes them
//   into the ring as "observe-dropped" records; sanitiseSealFault stamps countsMalformed on a record whose
//   count was clamped from a malformed value.
//
//   Every knob resolver outside the slice family must report its own source rather than fail-soft and
//   silently: a typo'd SCALE_SEGMENT_TARGET_BYTES / DEST_THROTTLE_ATTEMPTS / SEAL_VERIFY_SAMPLE /
//   REPL_VERIFY_SEGMENTS / SCALE_FANOUT_RANGES must not resolve to the default with no marker, which would
//   make set-but-invalid indistinguishable from never-set. sealKnobSources reports
//   {source: default|env|clamped|invalid} for every family.
//
...
async function partH(): Promise<void> {
  ...
  // The drop classes are the ONLY two ways an observation never reaches the ring. A malformed count is not a
  // drop: it clamps the observation and stamps countsMalformed:true on the record that lands, which is a
  // different fact carried in a different field. A drop class that cannot be a drop is a promise the pack
  // cannot keep, so the assertion pins the two.
  ...
  //     and it is counted -- flushed into the ring as soon as an observation lands again, so the pack shows
  //     "N observations were dropped" instead of a clean ring that reads as a healthy engine.
  ...
  // (2) UNKNOWN KIND: a kind added engine-side but never added to SEAL_FAULT_KINDS would otherwise be dropped
  //     by the DO's defensive sanitiser with nothing anywhere saying so. It is pre-gated at the SEND side and
  //     converted into a valid observe-dropped record instead; the drifted kind's STRING never rides.
  resetSealFaultDropTally();
  {
    const { stub, calls } = makeScheduler();
  ...
  // (3) MALFORMED COUNT: a NaN `found` would otherwise read back as an honest found:0 -- exactly the input a
  //     severe-truncation escalation fires on. The clamp stays fail-closed, and the record says so.
  {
