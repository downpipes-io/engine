// HTTP entrypoint cases of the validate-worker suite: root/404 banners, CORS preflight + origin
// matching, security headers on every admin response, the immutable-headers regression, run-now
// seal wiring, cache-control, and the last-resort error handler. No network, no deploy.
...
// ---- REGRESSION: immutable-headers -------------------------------------------------
// A live 500 (error-1101) happens when handleAdmin returns a Response that was FORWARDED
...
// index.ts rebuilds the headers into a fresh Headers instance, so the worker MUST
...
export async function run(): Promise<void> {
  ...
  // -----------------------------------------------------------------------------------------
  {
    ...
    // NC-09/NC-17: the root banner carries the base hardening headers.
  ...
  // -----------------------------------------------------------------------------------------
  {
    ...
    // NC-09/NC-17: the 404 carries the base hardening headers too (every emitted response).
    for (const h of BASE_SEC_HEADERS) {
  ...
  // -----------------------------------------------------------------------------------------
  // 7b. THE HEALTH-PROBE CORS EXEMPTION, which the whole CORS-versus-outage fingerprint rests on.
  //
  ...
  // -----------------------------------------------------------------------------------------
  // 8. REGRESSION: handleAdmin returns a Response with IMMUTABLE headers (simulating a
  ...
  //    This is the exact bug that causes a live 500/error-1101 without the copy in index.ts.
  // -----------------------------------------------------------------------------------------
  {
    ...
    ok("immutable-headers regression: worker does NOT throw on immutable-headers response", !threw);
    if (r !== undefined) {
      ok("immutable-headers regression: response arrives (not undefined)", true);
      for (const h of SEC_HEADERS) {
        ok(`immutable-headers regression: security header applied on immutable-origin response: ${h}`, r.headers.has(h));
      }
      ok(
        "immutable-headers regression: CORS applied on immutable-origin response",
        r.headers.get("access-control-allow-origin") === "https://console.example",
      );
    } else {
      // Mark all sub-checks failed if we never got a response.
      for (const h of SEC_HEADERS) {
        ok(`immutable-headers regression: security header applied on immutable-origin response: ${h}`, false);
      }
      ok("immutable-headers regression: CORS applied on immutable-origin response", false);
    }
  }

  // -----------------------------------------------------------------------------------------
  // 8b. RUN-NOW drives the seal: POST /admin/trigger must allocate the run AND drive
  ...
  // -----------------------------------------------------------------------------------------
  {
    ...
    //
    // /diag/dispatch-fault is the SECOND exemption (a support-pack gap): the last-resort catch files
    // this 500 into a bounded DO ring, because a customer's "the console 500s on Notifications" would
    // otherwise leave NO trace the vendor could ever read (the errId lives only in a Workers Logs line the
    // pack structurally cannot carry). It fires AFTER the throw, from the catch -- so the property this assertion
    ...
    // The last-resort catch files the 500 into the bounded dispatch-fault ring (support-pack), so a
    // 500 the customer sees is something the pack can carry. Assert BOTH halves of the contract:
    ...
    //     which is the property "scheduler NOT called on a malformed body" is really protecting.
    ok("dispatch-fault ring: the last-resort 500 was recorded", dispatchFaultBody !== "");
    ok("dispatch-fault ring: the record carries NO byte of the malformed request body", !dispatchFaultBody.includes("not valid json"));
    ok("dispatch-fault ring: the record carries the closed surface + route family + 8-hex errId only", (() => {
