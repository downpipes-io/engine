...
// `config`. The first version of this file built fixtures as `{ source }`, the same wrong shape the code
// under test was casting to, so the test agreed with the bug and passed while every live restore fell back
// to the convention. A fixture invented from the same assumption as the code proves only that they share it.
