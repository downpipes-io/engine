// validate-cf-config-mode-gated.ts -- proves POST /cf-config/mode is a first-class config mutation
// (cf-config-mode-set) routed through gatedConfigMutation, so it dual-gates and auto-snapshots into
// config history like a downpipe upsert, rather than writing inline and escaping the gate, the audit
// trail and config-history attribution. Driven through the REAL router + change-control handlers.
...
// Arm dual control. The mode change queues a pending config change (202) instead of applying inline,
// rather than writing inline regardless of the gate.
