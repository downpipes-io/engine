...
// what happens AFTER a rollover, and it says so: it SIMULATES the storage effect (deletes the oldest keys
// and hand-writes the rollover record) because "driving 10000+ real appends here would be slow". So the
// claim that the simulation is "exactly as the DO's rollOverAudit does" was asserted rather than measured,
// and the boundary itself, where the first entry is actually dropped, was never driven. It is driven here,
// in about two and a half seconds, and the simulation is carried as a REPRODUCTION CONTROL so the two are
// compared rather than trusted.
//
// THE INSTRUMENT HAD TO BE BUILT FIRST, and that is a finding in itself. MockStorage in
// validate-audit-harness.ts ignores list()'s `limit`. rollOverAudit calls list({prefix, limit: dropCount}),
// so under that double a one-entry rollover receives the WHOLE chain and deletes all of it. That is carried
// below as a DIFFERENTIAL CONTROL rather than described, because an instrument that cannot express the
// platform contract cannot measure a boundary that depends on it, and this workspace has a repeat shape of
// a zero that merely looks checked.
