// Prove the console-diagnostics support-pack section end to end at the ENGINE boundary:
//
...
//   2. A well-formed ring rides and is STRUCTURALLY VALUE-FREE: every string field is a frozen union
//      member; a customer sentinel placed in every field / key never appears in the output.
//   3. Caps are enforced: per-kind newest-wins 32, global 128, and a per-kind uncapped true-count rollup.
//   4. Numerics are clamped (count cap 1e6; firstMs/lastMs clamped; engineAttempts clamped); receivedAt is
//      engine-stamped, second-precision; provenance is source:'client-asserted'.
//   5. The section rides ONLY inside a bundle produced with a console POST body: buildSupportBundle folds
...
async function main(): Promise<void> {
  ...
  // Sized here for one reason: this engine DROPS a member it does not admit, silently, so a console that
  // emits a kind the engine has never heard of loses the evidence on arrival and the pack simply does not
  // carry it.
  //
  // The kind vocabulary covers the availability pair (identity-stale-gate, update-channel-unverified) plus
  // eighteen posture kinds. Every posture kind is a BROWSER-SIDE fact this engine cannot hold by construction:
  // a validator that turned an operator away without making a request, a probe whose result was rendered once
  // and stored nowhere, a dual-control refusal the console decided, a resource the browser's own CSP blocked,
  // a store the browser refused to keep anything in, and which renderer the topology map actually got.
  //
  // focus-landing covers WHERE KEYBOARD FOCUS LANDED after a tab activation that is itself a navigation. It is
  // the only member of this vocabulary that can see the accessibility surface at all, and the drop rule above
  // is exactly why it has to live HERE and not on the console alone: without it, a keyboard user trapped on the
  // first tab produces a pack byte-identical to a healthy one.
  //
  // The screen vocabulary carries a config-changes/owner-actions pair, prised out of the `security` bucket
  // both approval inboxes would otherwise fall into, where a frozen-on-skeletons TypeError would coalesce them
  // into one row.
  ...
  // surface carries map-diagnostics: the map's Copy-view-diagnostics block is the console's own remote-
  // support affordance, and a clipboard the browser refuses is exactly how it never reaches support.
  // surface also carries add-operational-key: the targeted break-glass-only -> operational upgrade card's own
  ...
  // buildCheckClass distinguishes `running-unstamped` (THE RUNNING BUNDLE carries no version stamp) from
  // `unstamped` (THE ORIGIN served assets with none): two facts about two different artefacts, kept as
  // separate rows rather than coalesced into one.
  ...
  // fieldClass and anomaly cover the wire values the console COARSENS rather than crashes on, each of which
  // could render a plausible verdict from something it could not read. The loudest is an unrecognised whoami
  // method presented as the break-glass TOKEN FALLBACK posture, a false claim that the customer's security
  // posture is degraded; the most dangerous is an unparseable last-good-run instant, which SKIPS the staleness
  // test entirely so a pipe that has not run in a month reads FRESH. Coalesced into one generic `timestamp`
  // member, support could not say WHICH assurance the customer lost.
  ...
  console.log("\nprojectRecord: set-membership validation + allowlist projection:");
  {
    ...
    // applyClass is the WHOLE discriminator of an apply-outcome row. A receiver that validated the row
    ...
    // SOME ceremony, which is exactly the useless row this discriminator exists to prevent.
  ...
  console.log("\ncaps: per-kind newest-wins 32, global 128, per-kind uncapped true-count rollup:");
  {
  ...
  console.log("\nthe section is request-scoped, not a roster member:");
  {
    ...
    // clientDiagnostics rides on the SupportBundleContext object rather than as a positional third argument: a
    // positional argument cannot represent two channels that each want it, and one object is the only shape
    // that cannot silently drop one. See test/validate-merge-both-channels.ts.
  ...
  console.log("\nthrough the real router (handleAdmin):");
  {
    ...
    // An oversized body 400s: capped before parse, so the receiver never sees it.
