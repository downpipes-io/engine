...
// untouched. Every added surface was PROBED LIVE against a real account before being registered. The
// The in-band count then moved 7 -> 16: nine single-setting writers, each PROVEN by the live round trip
// in test/live-cf-roundtrip.ts (create, capture, damage, verify-the-damage-took, dry run, restore,
// re-read, re-run to convergence) against a real Cloudflare account. Four passed outright; two are
// plan-gated on the proving account and marked UNPROVEN rather than PASS; the rest share the same proven
// writer. `zaraz/default` deliberately has NO writer: Cloudflare's schema documents it GET-only.
//
// NOTE: the total dropped from 216 to 214 when two duplicate surface registrations were removed
// (engine-src-048-01/048-02): "hyperdrive" duplicated "hyperdrive-configs" and "vectorize-indexes"
// duplicated the richer "vectorize". Both removed surfaces were account-scoped + reprovision-tier and
// carried no write() (in-band count unchanged), so account 127->125 and reprovision 46->44. Prose
// across the website, docs and console still cites "216 surfaces" and needs a later (P5) regeneration
// from the canonical line this validator prints.
const DOCUMENTED = {
  total: 313,
  inband: 85,
...
verdictReached(failures, canonical.total);
if (failures > 0) {
