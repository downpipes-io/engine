...
//   6. B7: supplying the operational pair ALSO clears the durable OPERATIONAL_RETIRED marker (best-effort);
...
async function main(): Promise<void> {
  ...
  console.log("\n-- POST /keys/install REFUSES a SILENT re-key when a signer is already present (custody guard) --");
  {
    ...
    // refusal. This is the bug the wave-1b custody agent surfaced (G-P0-003 / G-P0-099).
  ...
  console.log("\n-- R-10: the keys-installed append SURVIVES the Durable Object reset the secret PUTs cause --");
  {
    ...
    // per secret, source "api", trigger "secret", inside the two seconds the install ran -- and a new version
    // RESETS this Worker's Durable Objects. The append that follows therefore races the rollout and throws
    // "Durable Object reset because its code was updated" on the stub the isolate already holds.
    //
    // WHY IT IS GRADED HERE RATHER THAN LIVE. It is a RACE, not a condition: seven live installs across three
    // keyless estates lost exactly one row. So a live cell that drives one install and sees the row land
    // proves nothing either way, and the reproduction is not reliably repeatable on demand. The tolerance is
    // structural and holds or fails in the source, so the reset is INJECTED here and the property is graded
    // deterministically.
    //
  ...
  console.log("\n-- R-10 WIDENED: the rollout's OPAQUE message is ridden out too, and a real fault still is not --");
  {
    // WHY THIS EXISTS, and it is a criticism of the case directly above. That case injects the message the
    // predicate was already written to match, so it could never have failed on the form the runtime actually
    // produced. It passed on the build that shipped the ladder, and the defect went on losing rows live.
    //
    // MEASURED, not assumed. harness-keyless-engine serving artefact c118ede2e5d9 (which is what engine
    // cc631198 builds), `wrangler tail` attached, 8 confirmed re-key installs paced past the limiter. One
    // lost its keys-installed row, and the tail carried both of these inside the SAME request:
    //
    // THE DENOMINATOR ON THE LINE ABOVE DISAGREES WITH ITS OWN COMMIT AND NEITHER IS CORRECTED HERE,
    // because there is nothing left to settle which is right and guessing would be worse than saying so.
    // 82656241 wrote this comment and its own body in the same commit: the body says "Twelve confirmed
    ...
    //
    //   06:04:39.477Z warn  keys-installed audit append failed after a secret write
    //                       (non-reset): internal error; reference = n3rmrlv4r241rk2ob8u84pfq
    //   06:04:39.724Z warn  demo first-run marker clear failed:
    //                       Durable Object reset because its code was updated.
    //
    // "(non-reset)" is retryAfterSelfDeploy's own label for the branch that gives up on attempt ONE without
    // retrying, and the exhaustion message never appeared. So the rollout raised its OPAQUE form on the audit
    // stub and its NAMED form on the marker stub, 162ms apart, and the ladder was never entered.
    //
    ...
    // engine cc631198 and passes here, which is the whole change.
    ...
    ok("the keys-installed row LANDED through the opaque rollout fault (R-10's live loss)", opaque.rows === 1);
    ...
    ok("the live measurement that justifies the widening is recorded at the predicate", auditSrc.includes("n3rmrlv4r241rk2ob8u84pfq"));
  }
