// TC-03: Prove the restore drill (engine/src/admin/drill.ts) against a real in-memory
...
//      non-fallback "integrity check failed" -- matching the MISSING-segment INT-3
//      classification and the restore apply/verify passes. (RED-BEFORE-GREEN: before the
...
async function main(): Promise<void> {
  ...
  // identical damage, and the same class a MISSING segment already gets (INT-3). RED before
  ...
  // ---- SCENARIO 5: windowed cursor rotates FULL decrypt coverage across ticks (INT-1) ----------
  ...
  // ---- SCENARIO 6: tampered manifest SHARD (shard-hash integrity path, RV-CLI drill classification) ----
  ...
  // the restore path classifies accurately. This is the drill-side of the RV-CLI integrity-classification fix.
  ...
  // The load-bearing assertion (RV-CLI): a shard hash mismatch classifies as an INTEGRITY reason, never a
