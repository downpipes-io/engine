...
//
// The module is pure, so every rule below is testable here without an estate, a clock or a notification.
//
...
// Section 4 of the design: nothing here gates work. The only public surface is a verdict, so a caller cannot
...
// ---- the alert, which needed no new machinery -------------------------------------------------------
// The notification step is conditional: it fires only if the dedup is expressible in the
// existing notify machinery, because the remedy for an overdue proof is a person, and on split
// custody a quorum, so an alert that repeats on a cron rhythm trains the operator to ignore it.
//
...
// The steady state: still overdue, no new alert. This is Objection 1's whole constraint and it falls out of
// the existing detector rather than needing a repeat timer.
ok(
...
ok("and reports the recency without a target rather than against one", unstatedBlocks.includes("reported without a target"));
