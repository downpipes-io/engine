...
// declares `check_type` and `endpoint`. Neither field we sent exists, so every attempt was refused with a
// bare 1002 naming nothing, and SIX shapes were guessed across two sittings on two accounts before anyone
// opened the schema, which had been vendored in this repo the whole time. The answer was sitting in a
// file the drift guard already downloads.
//
// dlp-profiles-custom was the same shape: the body wrapped the profile in a `profiles` array when the
// schema says the request body IS the profile. Its recorded refusal was 3314 Forbidden, which is a
// plausible entitlement for a paid Zero Trust product, and it was reached with a body that would have
// failed validation regardless. That is the confounding this campaign has been caught by twice: a wrong
// body stands in front of the real answer, and the real answer is what gets written down.
//
...
if (!existsSync(SCHEMA)) {
  ...
  verdictSkipped("AUTOPROVE BODY SCHEMA CHECK SKIPPED: no vendored OpenAPI schema (set REQUIRE_CF_OPENAPI=1 to require it)");
  process.exit(0);
}
...
verdictReached(failures, checked);
