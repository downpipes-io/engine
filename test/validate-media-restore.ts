...
console.log("\n-- a {_refused} Stream marker is detected (the key that would otherwise launder a refused video into a bogus restore) --");
// stream.ts emits {_refused:...} when a Stream download URL fails the SSRF host allow-list: the video is
// HONESTLY NOT captured. A restore-side isMarkerValue that hand-rolls a subset (_skipped/_unavailable/
// _pending/_truncated) and misses _refused would let this ~80-byte marker slip the guard, get POSTed to
// Stream as a bogus "video", get a new uid, and have the restore report SUCCESS. Asserting it is detected:
...
// The two detectors must mirror each other. isMarkerValue delegates to isIncompleteMarkerValue, so this
...
console.log("\n-- uploader: image 409 where the live id holds a DIFFERENT image surfaces a CONFLICT (not success) --");
{
  ...
  // exists; the blob GET (/images/v1/<id>/blob) returns the DIFFERENT live bytes. Consulting the metadata GET
  // alone would read the id-exists envelope as SUCCESS; reading the blob instead finds the bytes differ from
  // the archived image, and surfaces a conflict.
  let posts = 0;
  let gets = 0;
  const fetchImpl = (async (url: string, init?: { method?: string }) => {
    if ((init?.method ?? "GET") === "POST") {
    ...
    // The metadata GET (mere existence, not proof of identity): the id DOES exist.
  ...
  try {
    ...
    // A 409 confirmed by mere id existence, without comparing bytes, would report success here wrongly.
