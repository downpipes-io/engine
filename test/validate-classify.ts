...
function main(): void {
  ...
  // ---- the same refusal on R2 lands in the right class too -------------------------------------------------
  // R2 answers a lock-bearing PUT to a bucket that is not Object-Lock enabled with "status 501
  // (NotImplemented)". NotImplemented sits in neither vocabulary, so without this case the refusal would
  // read as http-5xx: the operator would be sent to wait out a Cloudflare incident that is not happening
  // and will never clear. R2 is the primary store for this product, so this gap matters most there. AWS's
  // own answer to the SAME combination is the control, asserted alongside R2 so this adds R2 coverage
  // without moving AWS's class.
  ...
  // NARROWING CONTROL: a store answering NotImplemented to a write that asked for NO lock is refusing
  // something else entirely, and nothing establishes it will refuse identically for ever, so 501 does not
  // become permanent as a general rule. It keeps the class it had.
