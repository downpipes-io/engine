...
async function collectCrawl(iter: AsyncIterable<CrawlEvent>): Promise<{ records: SourceRecord[]; marks: string[]; events: CrawlEvent[] }> {
  ...
  for await (const ev of iter) {
    ...
    else if (ev.kind === "mark") marks.push(ev.token); // a mid-crawl vanish is a _vanished RECORD event, not a mark
  }
...
// together. RestoreRequest has only maxRecords (no offset/cursor), so a window that cuts a D1 in half cannot
// write header+some rows, mark the rest outOfWindow, and tell the operator to "re-run with a higher
// maxRecords": a re-run restarts at record 0, so the header's requireFreshTarget would re-run against the
// now-non-empty DB and throw, failing the whole D1 (and the rows would double). Instead, a single D1 that
// does not fully fit the window is NOT partially applied (steered offline), so a re-run never hits the
// non-empty-target throw; the guidance steers a D1 OFFLINE rather than giving the impossible re-run advice.
// KV/R2 windowing is unchanged (idempotent). This proves the target stays untouched and a later full
// restore still works.
async function proveWindowedD1AllOrNothing(ctx: Omit<D1RestoreCtx, "recs" | "r2">, rowsPerPage: number): Promise<void> {
  ...
  ok("the fixture D1 is a multi-record sequence (header + rows + schema)", recs.length >= 5 && decodeD1Record(recs[0]!.value!).kind === "header" && decodeD1Record(recs[recs.length - 1]!.value!).kind === "schema");
  ...
  ok("the apply (window) marker steers the D1 OFFLINE with the not-resumable honesty", appliedMarker !== undefined && /not resumable/i.test(appliedMarker.reason) && /not partially applied/i.test(appliedMarker.reason) && /offline with the downpipe CLI/i.test(appliedMarker.reason));
...
// ---- PROOF 17 (regression): a whole-DB restore whose header refuses a non-empty
// target must write NOTHING AT ALL, not just skip the header. Without this guarantee the apply loop would
// keep attempting every LATER D1 record for that same database once the header's fresh-target guard had
...
// own, so it would still run -- against a table the target already held, never one this restore created --
// and the apply would count it as a genuine restored record on a run whose own headline was "refused,
// target not empty". This covers a live scenario: two of three archived tables already exist (one was
...
async function proveDirtyTargetBlocksAllLaterRecords(ctx: Omit<D1RestoreCtx, "recs" | "r2">): Promise<void> {
  ...
  // exactly that live scenario) but not "orders", which is what this restore is trying to bring back.
  const dirty = new MockD1();
...
async function main(): Promise<void> {
  ...
  // PROOF 13 (an ASVS engine mirror): the single-statement/quote-aware guard on every DDL/schema
...
// proveD1SingleStatementGuard mirrors the Go downpipe CLI's identical decoder guard (d1transcode.go):
// the prefix-only CREATE TABLE/INDEX/TRIGGER/VIEW regexes above only prove a DDL/schema string STARTS
// with the right keyword, not that nothing else follows, so "CREATE TABLE t(x); DROP TABLE other; --"
// would pass them unchanged. Unlike the CLI (which only writes such a string into a .sql file a human
// reviews before running it), a decoded entry here is handed straight to db.prepare() and replayed via
// db.batch() against a LIVE target D1 binding with NO human review step (D1RestoreSink in
// restore-sink.ts), so the same gap is exploitable at strictly higher severity. Each case is driven
...
// decodeTableDDL, schema path via decodeSchemaList), since all four are separate call sites the guard
// has to reach.
//
// The later cases close a gap in the smuggle cases above: those only ever exercise CREATE TABLE/INDEX
// smuggles (allowTriggerBody=false, so the BEGIN/CASE/END tracking branch never actually runs), never a
// CREATE TRIGGER smuggle -- the one entry kind that turns that branch on and the one a bare "begin"/
// "case" alias could fool into hiding a smuggled statement behind a wrongly-still-open body. See
// d1RequireSingleStatement's doc comment for how this was found and verified against a live D1 binding,
// and for the guard and its documented residual.
function proveD1SingleStatementGuard(): void {
  ...
  // statement behind the BEGIN/CASE...END depth tracker. A prefix-only regex would pass this
  // unchanged (it does start with CREATE TABLE/INDEX), so the guard must still refuse it.
  ...
  // ---- trigger-body smuggle: the bare-keyword smuggle cases above (smuggledTable, smuggledIndex)
  // never exercise allowTriggerBody=true -- CREATE TABLE/INDEX never carry a trigger body, so the
  // BEGIN/CASE/END tracking branch never runs for them, and it is ALWAYS the CREATE TRIGGER path (the
  // one that turns it on) that a bare "begin"/"case" alias could fool. Verified directly against a
  // live local D1 binding (wrangler --local, same workerd SQLite engine that backs production D1)
  // that this exact shape passes an untightened guard unmodified AND silently executes the smuggled
  // DROP TABLE via both db.prepare(sql).run() and db.batch([db.prepare(sql)]):
  const triggerSmuggleAlias = "CREATE TRIGGER evil AFTER INSERT ON t\nBEGIN\n  SELECT 1 AS begin;\nEND;\nDROP TABLE payments;\nSELECT 1 AS end;";
  ok(
    "decodeD1Backup refuses a bare 'AS begin'/'AS end' alias trick in a CREATE TRIGGER (live-confirmed bypass)",
  ...
  ok(
    "decodeD1Backup refuses a decorative-trailing-END smuggle shape",
    throwsWith(() => decodeD1Backup(bytesFor({ ...baseFullBody(), schema: [triggerSmuggleDecorative] })), /more than one statement/),
  );

  // ---- trigger-body false positive: the begin/end fixtures above only ever declare begin/end as
  // CREATE TABLE columns (allowTriggerBody=false there, so the tracking branch never runs). A real
  // CREATE TRIGGER body that references a column literally named begin/end via NEW. (the idiomatic way
  // to read the triggering row, e.g. a rostering/shift-scheduling domain) is a distinct case: a naive
  // depth tracker's bare "begin"/"end" detection would run here too and wrongly reject it. Verified
  // directly against real SQLite that both trigger bodies below are valid, single-statement DDL that
  // fires correctly.
