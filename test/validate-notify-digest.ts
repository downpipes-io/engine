...
// validate-notify.ts per finding engine-test-009-01. Covers the daily/weekly success-stream
...
// the digest emission and the bounded "and N more" names tail. Split per finding engine-test-009-08.
...
// R9 instant-ordering: fromAt/toAt are chosen by instant (not lexical string order) and a non-finite
// `at` never wins the window. Split per finding engine-test-009-08.
function testDigestSummaryInstantWindow(): void {
  const base = Date.UTC(2026, 5, 1, 0, 0, 0);
  // R9 REGRESSION: fromAt/toAt must be chosen by INSTANT, not by lexical RFC-3339 string order. We
  ...
  // EMIT each chosen entry's OWN original string (not a re-serialised form).
  {
    ...
    ok(
      "R9 precondition: raw strings sort lexically in a DIFFERENT order than their instants",
    ...
    ok("R9: fromAt is the EARLIEST INSTANT entry's own string (the +10:00 offset)", sm.fromAt === earliestInstant);
    ok("R9: toAt is the LATEST INSTANT entry's own string (the .5Z fractional)", sm.toAt === latestInstant);
    // And the chosen ends are real, comparable instants in the right order.
    ok("R9: chosen window ends are ordered by instant (from <= to)", Date.parse(sm.fromAt) <= Date.parse(sm.toAt));
  }

  // R9 GUARD: a non-finite (unparseable) `at` must NOT win the min/max; it is ignored for the window so
  ...
  // window must come from the two parseable entries, never the garbage string.
  {
    ...
    ok("R9 guard: unparseable `at` never becomes fromAt", sg.fromAt === isoAt(base + 2000));
    ok("R9 guard: unparseable `at` never becomes toAt", sg.toAt === isoAt(base + 8000));
    ok("R9 guard: total still counts the garbage-`at` entry", sg.total === 3);
  }
}
