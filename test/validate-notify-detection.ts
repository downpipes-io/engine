// Detection-layer vectors for the validate-notify suite, split out of validate-notify.ts.
// Covers isAllowedWebhookUrl, classify, shouldAlert and buildAlert (the no-custody surface).
//
// emitAlerts / postWebhook fail-open went with the legacy single-webhook policy. Its two properties
// survive on the path that replaced it and are proven there: deliverPayload never throws
// (validate-notify-fault-evidence.ts, where a thrown fetch and a 410 both come back as a closed
// sub-cause) and it RE-SCREENS the sink host at send time against the metadata, RFC1918 and carrier-NAT
// ranges WITHOUT issuing the POST (validate-notify-routing.ts, which carries the same globalThis.fetch
// spy across so the no-POST half is proven, not inferred from a result shape).
...
// ---- isAllowedWebhookUrl -----------------------------------------------------------
// Split into focused sub-tests: scheme/shape acceptance, userinfo rejection, private-range
// default-deny, and the explicit per-channel override.
...
// ---- classify -----------------------------------------------------------------
// Split into the no-alert/failed-precedence group and the staleness-window group.
...
// ---- shouldAlert (transition + cooldown gate) ------------------------------------
...
// ---- buildAlert (no-custody surface assertion) ----------------------------------
