...
// false-green this contract closes.
//
...
export async function startSyslogEmulator(): Promise<SyslogEmulatorHandle> {
  ...
  // event loop open, so Node drains it and the process exits 0 with the verdict line unprinted. That
  // was the measured false green in test/validate-destsim-selftest.ts: 109 assertions incapable of
  // failing. The pending timers are tracked for the same reason.
