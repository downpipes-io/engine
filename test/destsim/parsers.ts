...
// product cannot disagree about it. They did: the test-send's synthetic event carried 94 zeros and this
// regex wanted 96, which made this parser answer 400 on a payload real Splunk HEC answered 200 on.
...
function assertAuditTarget(v: unknown, where: string): AuditTarget {
  ...
  switch (kind) {
    ...
    case "secret-rotation": {
      assertObjectKeys(v, ["kind", "secretId"], [], w);
      str(v, "secretId", w);
      return v as AuditTarget;
    }
...
export function parseDatadog(raw: string | Buffer, opts?: { contentType?: string }): { events: ParsedDatadogLog[] } {
  ...
  const events = arr.map((item, idx) => {
    ...
    if (status !== expectedStatus) fail(`${where}: status ${JSON.stringify(status)} does not match the outcome-derived value ${JSON.stringify(expectedStatus)} (item 16, HARDENING-2026-07-05.md)`);
...
export function parseTeamsCard(raw: string | Buffer, opts?: { contentType?: string }): { events: ParsedTeamsCard[] } {
  ...
  // `markdown` is optional and, when present, must be the literal false the adapter sets (ASVS V1.3.3):
  // a card that renders its detail as markdown would turn a crafted downpipe name into a live link.
  assertObjectKeys(obj, ["@type", "@context", "themeColor", "summary", "title", "text"], ["markdown"], "teams");
  if ("markdown" in obj && obj.markdown !== false) fail(`teams: markdown, when present, must be false (the detail is untrusted), got ${JSON.stringify(obj.markdown)}`);
