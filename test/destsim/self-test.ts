...
// testSplunkHecObservedContract pins the emulator to the contract CAPTURED from a real Splunk Cloud stack
// (port 8088, /services/collector/event). Without this, a destination configured with
// `Authorization: Bearer <token>` -- which real HEC REFUSES -- would pass the entire suite green, because
// an emulator with no auth check serves any path with 200 {"text":"Success","code":0}. Every row below is
// a status/body pair actually observed.
async function testSplunkHecObservedContract(ok: OkFn): Promise<void> {
  ...
  try {
    ...
    // A path the listener does not serve 404s. This is what makes a misconfigured collector path a FAILURE
    // rather than a silent pass.
    const wrongPath = await post(`Splunk ${HEC_TEST_TOKEN}`, "/ingest");
    ok("hec observed: an unlisted collector path (/ingest) 404s (Splunk does not serve it)", wrongPath.status === 404);
...
async function testSyslogEmulatorRealTls(ok: OkFn): Promise<void> {
  ...
  try {
    ...
    // allowInternalSink is the explicit per-channel opt-in, and it is used here because the emulator IS on
    // loopback: the SSRF screen refuses an internal host by default and is doing its job when it refuses
    // this one. Driving the live handshake needs the same seam a real operator with a private sink would
    // set. validate-egress-host-screen grades the DEFAULT-DENY half, so the screen is not made optional by
    // this line; it is exercised from both sides.
    const r = await deliverSiemSyslog(em.host, em.port, "cef", events, meta, { allowInternalSink: true });
...
async function testSyslogEmulatorFaults(ok: OkFn): Promise<void> {
  ...
  try {
    ...
    __setSyslogConnectForTest(makeRealSyslogConnect());
    const r = await deliverSiemSyslog(em1.host, em1.port, "cef", [realAuditEvent({ seq: 1 })], meta, { allowInternalSink: true });
  ...
  try {
    ...
    const r = await deliverSiemSyslog(em2.host, em2.port, "cef", [realAuditEvent({ seq: 2 })], meta, { timeoutMs: 200, allowInternalSink: true });
    const elapsed = Date.now() - started;
    // The syslog timeout carries its PHASE (handshake / write / flush), because WHERE a stalled
    // receiver black-holes the send is the whole
