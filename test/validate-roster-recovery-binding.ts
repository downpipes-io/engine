...
// PERMANENT credential. On the estate that was measured, 71 of 149 identities could sign in with no role row,
// 67 of them by recovery code, against ONE the roster accounts for.
//
...
//
// Sections F and G cover two guards on the offboarding revoke that a sibling pass drove live and found
// missing: the step-up gate did not cover the route, and a self-targeted revoke was not refused at all.
//
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // AND SAYING SO IS THE POINT. It was written expecting to, and then it passed against the tree
    // WITHOUT the gate, which means something else was already closing it. That something is
    // deleteRole's session-epoch bump: the racer's cookie is dead before the registration is even
    // authorised, so the ceremony never reaches the self-add arm. Leaving the assertion labelled as a
    // proof of the gate would have been a check that cannot fail for the reason it claims.
    //
    ...
    // ===========================================================================================
    {
      ...
      // down at roleTableIsEmpty, well before the roster gate is consulted, and a refusal measured that way
    ...
    //
    // The pass that built sections A to E stated openly that the invite arm was not gated rather
    // than assuming it closed, and it was right. resolveRegistrationAuthorisation evaluates the
    ...
    // ===========================================================================================
    {
      ...
      // the self test. This is the exact case a sibling pass drove live and got 200 on.
