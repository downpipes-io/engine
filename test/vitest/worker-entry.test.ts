...
//
// Raising coverage on the entry + RunSealDO toward the 85/90% targets is a tracked follow-up; this
// file is the load-bearing seam those later tests extend.
...
describe("Worker fetch entrypoint (real workerd)", () => {
  ...
  it("GET /ready carries base hardening headers (proves withBaseSecurity ran)", async () => {
    ...
    // assert without coupling to the full header list (which other tests/B6-x may extend).
