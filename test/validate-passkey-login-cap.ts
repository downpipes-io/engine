// sweepAndBoundLoginChallenges enforces PASSKEY_LOGIN_CHALLENGE_CAP, SERVER-SIDE, driving the
...
async function run(): Promise<void> {
  ...
  // (a) BOUND TO THE CAP: seed cap+50 LIVE login challenges, then sweep -> exactly the cap survives.
  {
    ...
    ok("(a): setup seeded MORE than the cap", countLogin(storage) === over);
    await dobj.sweepAndBoundLoginChallenges(now);
    ok("(a): after the sweep the login-challenge set is bounded to PASSKEY_LOGIN_CHALLENGE_CAP", countLogin(storage) === PASSKEY_LOGIN_CHALLENGE_CAP);
  }
  ...
  // live one survives (expiry is unconditional, independent of the cap).
  {
    ...
    ok("(b): setup seeded live + expired BELOW the cap", countLogin(storage) === liveN + expiredN);
    await dobj.sweepAndBoundLoginChallenges(now);
    ok("(b): EXPIRED login challenges are swept (deleted) regardless of the cap", countLogin(storage) === liveN);
    const survivors = await storage.list<{ expiresAt: number }>({ prefix: LOGIN_PREFIX });
    ok("(b): every survivor is unexpired", [...survivors.values()].every((r) => r.expiresAt > now));
  }
  ...
  // finish. Seed cap+overflow live with strictly increasing createdAt and assert the oldest are gone.
  {
    ...
    await dobj.sweepAndBoundLoginChallenges(now);
    ok("(c): still bounded to the cap when ALL are live", countLogin(storage) === PASSKEY_LOGIN_CHALLENGE_CAP);
    let oldestGone = true;
    let newestKept = true;
    ...
    ok("(c): the OLDEST survivors are evicted first (an old in-flight challenge can die before its finish)", oldestGone);
    ok("(c): the NEWEST challenges (most likely to be completed) are kept", newestKept);
  }
  ...
  // challenge a legitimate burst is mid-ceremony on).
  {
    ...
    await dobj.sweepAndBoundLoginChallenges(now);
    ok("(d): a live set AT the cap is fully preserved (no eviction at/below the cap)", countLogin(storage) === PASSKEY_LOGIN_CHALLENGE_CAP);
  }
  ...
  // and only the login family is bounded.
  {
    ...
    await dobj.sweepAndBoundLoginChallenges(now);
    ok("(d2): the login sweep does NOT touch a (even expired) registration challenge (reg:)", (await storage.get(`${PASSKEY_CHALLENGE_PREFIX}reg:someone@example.com`)) !== undefined);
    ok("(d2): only the login-challenge family is bounded by the sweep", countLogin(storage) === PASSKEY_LOGIN_CHALLENGE_CAP);
  }
