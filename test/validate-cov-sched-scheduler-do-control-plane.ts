...
// control-plane RECOVERY mixin (src/sched/scheduler-do-control-plane.ts), the INFRA-1 layer that builds the
...
async function main(): Promise<void> {
  ...
  // ---- (16) acknowledgeControlPlaneRecovery: the narrow latch-clear that clears ONLY the latch when the
  // plane has organically un-emptied (role table non-empty) under an active latch, and never when there is
  // no owner on record to ask.
