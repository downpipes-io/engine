...
const CASES: Array<[string, SsoFailCode]> = [
  ...
  // ---- HOSTILE INTERPOLATION (adversarial-review Hazard 4): an attacker-chosen token in an interpolated tail must
  ...
  // @Algorithm — the residual the FIRST fix missed (found by the focused re-review). Without the anchor these
  // hijack to `replay` via the injected token.
  ...
  // ---- OAuth2 (oauth2.ts) — the confirmation re-review's flagged inventory gap (fidelity, not a hijack) ----
