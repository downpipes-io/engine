// validate-owner-action-do-reset-refusal.ts -- proves that a FORESEEABLE plain-Error refusal from a replayed
// owner action (e.g. removeDest throwing "no such destination" when the target changed between propose and
// approve, scheduler-do-dest-config.ts:312) is SENTINELLED inside approveOwnerAction's blockConcurrencyWhile,
// not re-thrown. A throw escaping the guard RESETS the Durable Object in workerd (wiping in-memory scheduler
// state -> 500); sentinelling it makes it a clean 4xx with the DO intact. The catch handles only AuthError and
// would otherwise re-throw a plain Error inside the guard (guardThrows()===1). This is the non-AuthError
// sibling of the case already covered by validate-owner-action-dualcontrol-execstate.ts.
