...
//      freshness failure. Found live on the rekey harness estate: a fresh install's first seal against an
//      as-yet-unseeded KV source verified 0 records and read as "verification failed" with zero diagnostic
//      content. Section E below proves runBlindRestoreTest now returns the SAME {nothingToVerify:true,
//      reason:NOTHING_TO_VERIFY_REASON} pair drill.ts already returns for the identical archive, so both
//      restorability-proof routes describe the same state the same way.
//
...
async function proofZeroRecordRun(signer: Signer, recipients: RecipientEntry[], breakGlassIdentity: Uint8Array, operationalPrivateB64: string, signerPrivateB64: string, verifier: Awaited<ReturnType<typeof verifierFrom>>): Promise<void> {
  ...
  // Merkle root, and finds no records. This is the state crud stood in nineteen consecutive times.
  ...
  console.log("\nE. WHAT THE MANUAL POST /admin/restore/verify ROUTE SAYS ABOUT THE SAME ARCHIVE (WIRE-28)");
