...
// reclaimed. The `lost-flight` counter exists to count exactly that, and the ledger says why in its own words:
...
// The remedy already exists in the product and is already used at the sibling site, so this registers with
// `recordCanaryLoss` rather than inventing a second mechanism.
//
...
// distinction matters: below the edge the `lost-flight` key is not present in the aggregate at all, so a
// healthy estate and a lease-losing one are not the same reading with a different number.
//
// THIS SHAPE IS GUARDED WITH A SENTINEL THAT MUST BE REACHED, not an absence or a count: section G drives
...
// so a seed without it is silently discarded and every dose reads "already reclaimed", and a seed that never
// reaches the code can pass a positive dose vacuously. Only a paired negative dose and the sibling control
// failing catch that and turn it into a refusal instead of a false pass.
...
// An instrument that answers the same thing at both doses is not measuring the dose: a temporal sweep that
// reads DECLARED at both trees is exactly that failure, and only a known positive catches it.
