...
// Location. When the caller passes redirect:"manual" it hands back the raw 3xx untouched (what doFetch
// requires); otherwise it simulates a runtime that silently auto-follows the redirect, by answering as the
// "attacker" would -- a plain success carrying different bytes -- so dropping redirect:"manual" is caught the
// same way it would bite in production: a SILENT capture of attacker-controlled content, not a thrown error.
// lastRedirect records the redirect option doFetch sent.
...
console.log("\n-- captureBlob: a single object OVER the single-invocation seal ceiling is SKIPPED (not streamed) -> incompleteness marker --");
{
  ...
  // recordsIncrement surfaces the one object. Streaming it regardless of size would attempt the
  // multi-thousand-subrequest chain and wedge the run.
  ...
  // recordsIncomplete so the completed run reports "N items not fully captured".
