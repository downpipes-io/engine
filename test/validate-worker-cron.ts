// Cron orchestration cases for the validate-worker suite: scheduled() handler resilience to a scheduler
// outage, the seal happy-path reconciliation sequence, the per-run isolation negative controls, the
// guarded error-path /complete, and the post-run alert/expiry/restore-test/digest passes (stocked and
// error-injected fail-open). No network, no deploy.
...
export async function run(): Promise<void> {
  ...
  // -----------------------------------------------------------------------------------------
  // 10. CRON ORCHESTRATION SEAL HAPPY PATH.
  //
  ...
  // resolves the in-flight history row to failed and clears the lease immediately. (Escaping to an
  // index-less catch would complete at index 0, which matches no row, so a misconfigured engine would
  // strand the lease until the 30-minute reclaim; resolving the real row keeps the lease clearing
  // immediately.) The point of THIS test is the wiring/control flow: each due
  ...
  // -----------------------------------------------------------------------------------------
  {
    ...
    const expectedPaths = new Set(["/tick", "/due", "/dest-config", "/trigger", "/complete", "/reconcile-alerts", "/reconcile-replication-alerts", "/expiry/reconcile", "/restore-tests-due", "/canary/due", "/notify/digest-due", "/downpipes", "/cf-config/discovery-due",
      ...
      // run-history retention purge pass (ASVS V14.1.2): the per-tick age-based purge of the run-history
      // ring past RUN_HISTORY_MAX_AGE_DAYS (src/cron/history-retention-pass.ts).
      "/history-retention-pass",
      // per-tick OUTCOME record (scheduler-liveness new-logging): the end-of-invocation false-green detector.
      "/tick-outcome",
      // The single-flight tick lease: drive() acquires it at the top of the tick and releases it at the end,
      ...
      // SIEM audit-log push destination: the drain's per-tick config read
      // (always made, opt-in no-op when unconfigured); this stub answers null, so no further push round-trip.
      "/push-config",
      // OTLP/HTTP metrics push: the drain's per-tick config read (always made, opt-in no-op
      ...
      // The support-pack diagnostic recorder family. These are the
      ...
      //
      // /diag/admin-counters is the cron drain's alarm bell: the rich, structured evidence for each of the
      // gaps it covers rides the cron-health delta above, and the ONE headline fact of each -- a critical page
      // consumed and never delivered, a restore test that did not run, a push-trail write that was lost -- is
      // also raised as a closed admin-counter name, because the pack already carries that aggregate in full. It
      // is raised ONLY on a tick that recorded such a fault (a healthy tick posts nothing here and costs no
      // subrequest), which is why it is a named member of this reviewed surface rather than an unconditional
      // per-tick write.
  ...
  // -----------------------------------------------------------------------------------------
  // 14. The per-downpipe run-loop catch posts a best-effort /complete to clear the lock. That
  //     completion is GUARDED in its own try/catch so a DO blip DURING error handling cannot
  //     reject drive() (which would crash the cron and strand the rest of the due batch). We drive the
  //     REAL scheduled() with a scheduler whose /complete ALWAYS throws while the run itself also fails
  //     (no SIGNER_PRIVATE), and assert the captured drive() promise still RESOLVES.
  //
  ...
  // the first): an unguarded /complete throw would otherwise reject drive() and the second downpipe
