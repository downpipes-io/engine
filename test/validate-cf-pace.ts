...
async function main(): Promise<void> {
  ...
  console.log("\nmakeCfApi(token, fetch, pacer): a media metadata client both PACES a burst and still does a working get():");
  {
    ...
    //
    // THE DRAIN IS WHAT MAKES THIS DETERMINISTIC. THE BUCKET REFILLS ON WALL CLOCK, so every millisecond
    // between (a) taking the burst token and t0 being read is a millisecond of refill. At 5/s a token is
    // back in 200ms, so a machine busy enough to spend 200ms inside step (a) hands (b) a full token, (b)
    // correctly does not wait, and the assertion would fail while the pacer is working exactly as designed.
    //
    // A LOWER bound of 50ms against an expected 200ms wait cannot be broken by the machine being slow
    // AFTER t0; it breaks only when the machine is slow BEFORE t0. Draining immediately before reading the
    // clock closes that window to microseconds without weakening the bound or slowing the test, and it
    // needs no clock seam in production code.
    await pacer.take();
    const t0 = Date.now();
  ...
  // lines explaining why they exist, so a substring check would stay green with the pacer deleted. Verified
  // by deleting it, which is the only way to know a guard like this can fail.
  {
    const src = await import("node:fs").then((fs) => fs.readFileSync(new URL("../src/admin/restore.ts", import.meta.url), "utf8"));
    console.log("\n-- the restore legs are paced (B4) --");
    ok(
