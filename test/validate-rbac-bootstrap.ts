// validate-rbac PROOFs 1..1e (finding engine-test-012-M1): bootstrap-first-caller-as-Owner, the
// emailless/subless denials, the recycled-subject closure (V10.3.3 / V10.5.2) and the legacy
// email-keyed migration. Extracted verbatim from test/validate-rbac.ts; the assertions, order and
...
export async function runBootstrap(ctx: Ctx): Promise<void> {
  ...
  // ---- PROOF 1b (ENG-B1 regression): an emailless but validly-signed Access JWT is DENIED --------
  ...
  // break-glass (the prior privilege-escalation bug). It must be denied (401), never resolved to Owner.
  {
    ...
    ok("ENG-B1: an emailless valid Access JWT is denied (401), not resolved to Owner", r.status === 401);
    ...
    ok("ENG-B1: an emailless Access caller cannot grant a role (denied, never Owner)", gr.status === 401);
  }
  ...
  // ASVS V10.3.3 / V10.5.2 closure proof. A FRESH scheduler keeps it self-contained (its own bootstrap).
  {
    ...
    // And the recycled caller cannot perform an approver-only action: a restore APPLY is refused at the F1
