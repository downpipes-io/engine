...
// reserved-binding guard (with its missing-binding negative control) and the live-adapter dispatch
// for the REST-API source types (stream/images/artifacts). No network, no deploy.
...
export async function run(): Promise<void> {
  ...
  // -----------------------------------------------------------------------------------------
  {
    ...
    // CONFIG_WRAP_KEY/SCIM_BEARER_TOKEN/BEACON_INGEST_KEY must stay in RESERVED_BINDINGS: without that, a
    // secrets source could name one of them and exfiltrate the destination-credential wrap key or a bearer
    // secret via an ordinary backup. Assert each is reserved by name, not just "some binding is reserved",
    // so a future removal of any one of them fails here specifically.
