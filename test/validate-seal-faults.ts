...
function main(): void {
  ...
  {
    // The detection-point kinds are exactly the modes this deliverable covers; the set is closed. The three
    // retention kinds joined the original five seal-lifecycle kinds; the two fan-out kinds (WHICH range
    // wedged, WHY a worker report was discarded) and the ring's own observe-dropped self-report joined them.
    // The two run-diagnosis kinds (config-fault: WHICH binding / secret / token / env var a run died for;
    // run-pressure: the retry / park / strike cost a run paid, on success as well as failure) joined them;
    // both are exercised in depth by test/validate-seal-config-pressure.ts.
    // Eight SEAL + DEST kinds joined them: replica-target-fault (the REAL replica reason, not the two-word
    // {not configured, unreachable} vocabulary), replication-pass (the pass's own execution health),
    // record-skipped-changed (an infra fault masked as content churn), dest-stranding (the bytes the engine
    // strands and never counts), verify-suspect (the ordinal / digest / clean-count a suspect verdict could
    // not carry), seal-mode (the silent fan-out-to-serial downgrade), alert-routing-failed (a critical alert
    // lost with no evidence it was ever attempted) and lock-plane (a scheduler-plane blip misattributed to
    // the destination). They are exercised in depth by test/validate-seal-dest-gap-evidence.ts.
    // Twelve more kinds are exercised in depth by test/validate-seal-refuse-evidence.ts: the
    // checkpoint-corruption trio (checkpoint-invalid, checkpoint-coerced, resume-abandoned), the
    // refuse-to-sign completeness guards (scratch-shard-missing, scratch-hash-mismatch,
    // scratch-preamble-mismatch, merge-count-mismatch, under-crawl, open-shard-count-mismatch,
    // open-batch-missing), the lost terminal verdict (completion-lost) and the silently-normalised absent
    // RUNLOG (runlog-absent). The COUNT is pinned deliberately: a kind added without a recording site and a
    // read path is exactly the "looks closed" failure this guards against.
  ...
  console.log("\nPART F: isWormRefusal -- the orphan-root-reclaim WORM classifier");
  {
    ...
    // can fix in a minute. Classifying it as a WORM refusal told support "locked by design, nothing to fix",
    // and that verdict rides into the support pack through `wormBlocked` (seal/runseal-do.ts).
    //
    // The test mattered more than the classifier here: anyone repairing isWormRefusal would have hit a RED
    // assertion saying the old answer was correct, and the cheapest reading of a red test is that your fix is
    // wrong. A test that encodes a defect defends it.
    //
    // The line above it is the one that proves the precedence is still right: a refusal carrying BOTH lock
