// Confirms the notify pipeline's delivery-feedback evidence: the three feedback confirms -- POST
// /alerts-delivered, POST /recovery-delivered, POST /replication-alerts-delivered -- record their own
// failure in the DO's notifyHealth aggregate (notifyHealth.feedbackFails), so two silent symptoms have
// a durable cause:
//
//   "no second warning while the pipe stayed broken"  -> the OPTIMISTIC cooldown stays written because
//                                                         the confirm that would clear it was lost
//   "PagerDuty never auto-closes after recovery"       -> the owed-resolve marker for a FAILED resolve
//                                                         is never written, so nothing retries it
//
// Each confirm's response is checked, not just awaited: a DO that ANSWERS (a 404 from a skewed route
// chain, a 400, a 500 under load) returns a Response, so an unchecked `await` treats a lost confirm as
// a landed one. All three streams -- alerts, recovery, replication -- record their own failures in the
// same counter, so none of them go somewhere the support pack cannot collect them.
//
// The counter's own bump is checked and retried once, since it is the write most likely to be lost in
// the very fault window it exists to prove.
//
...
// ANSWERS and refuses -- the case a throw-only check would read as success).
...
async function main(): Promise<void> {
  ...
  {
    // A. THROW -- the case a throw-only check already catches (regression guard).
  ...
  {
    // B. 500 -- the DO ANSWERED and refused. An unchecked response would read this as SILENT success.
  ...
  {
    // D. THROW. The recovery-delivered confirm is lost; the failure is recorded, not just logged.
  ...
  {
    // E. 500. The DO answers and refuses; the failure is still recorded.
  ...
  // ---- the counter's OWN write (the diagnostic write most exposed to the fault it exists to prove) -------
  console.log("\n# the bump itself is CHECKED and retried once");
  {
