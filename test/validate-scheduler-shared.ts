// Shared fixtures for the SchedulerDO validator suite (engine-test-018-01 split).
// validate-scheduler.ts was one ~1700-line file with a ~1294-line main(); it is now a thin
// orchestrator that imports and calls a run() from each area module. Every area module shares the
// in-memory doubles, the ok() assertion sink and the config builders defined here, so the split is
// behaviour-preserving: the same assertions run, in the same order, against the same fixtures.
//
...
// set with the EXACT same constant the DO uses (engine-test-018-05): no mirror copy that can diverge.
...
// reach for a lease to read as EXPIRED. Deriving it from the production constant (rather than the inline
// 31 * 60 * 1000 the suites used to repeat) means a change to the lease window flows through automatically
// and the test intent ("expired") is named, not encoded in a literal (engine-test-018-14).
...
// on the header alone therefore drove an estate where NEITHER principal existed: every gate and every
// reserve read authority-lapsed. Three validators sat in exactly that state, and their single-use
// invariants kept passing while they did, because a refused reserve and a spent reserve are the same
// `false`. Seeding through the real setRole (an owner-token break-glass caller, the engine's own internal
// owner authority) plus a real bind keeps the fixture on the production path rather than writing
// role:sub: keys by hand.
...
// index seeded so far). The result is identical to syncDueIndex on a fresh fleet (engine-test-018-06).
