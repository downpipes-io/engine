// Pins the auto-resolve RECOVERY RETRY: a recovery (auto-resolve "incident cleared" close) whose delivery
// FAILS is retried on later reconciliation ticks until it lands, the downpipe relapses, or it ages out -- so
// a transient blip during recovery cannot strand a PagerDuty/JSM/ServiceNow incident open. Without the retry,
// the recovery fires exactly once on the falling edge and a failed delivery is a silent, permanent stranding.
//
// The design keeps a minimal blast radius: the emission path is unchanged (the recovery still fires once
...
// FAILED recovery delivery. The happy path (delivered first time) writes nothing and behaves exactly as a
// one-shot emission -- which is why the existing validate-autoresolve.ts vectors stay green.
//
...
async function main(): Promise<void> {
  ...
  // ---- Scenario B: the HAPPY path is unchanged -- a delivered recovery writes no marker, never retries ----
  {
    ...
    const t3 = await d.reconcile();
    ok("B t3: a delivered recovery is one-shot -- no repeat emission", (t3.recoveries ?? []).length === 0);
  }
  ...
  // ---- Scenario C: a RELAPSE supersedes an owed resolve -- and is NEVER suppressed ----
  {
    ...
    ok("C: the relapse re-alerts immediately (NOT suppressed -- the fix never touches the cooldown lifecycle)", tRelapse.alerts.length === 1 && tRelapse.alerts[0]?.state === "failed");
  ...
  console.log(getFailures() === 0 ? "\nDESTSIM AUTO-RESOLVE RETRY PASS" : `\n${getFailures()} FAILURE(S)`);
  verdictReached(getFailures());
