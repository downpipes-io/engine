// The CROSS-ACCOUNT (confused-deputy) guard on the cf-config and media restore legs.
...
async function main(): Promise<void> {
  ...
  // ONE restore whose cf-config leg AND media leg target TWO
  // DIFFERENT foreign accounts. This is the engine-side proof for a console defect: the console let an
  // operator arm the whole apply by typing only the FIRST warning's account, then stamped
  // confirmDifferentAccountId onto every cross-account leg regardless. The engine must not be fooled by an
  // echo that only ever proves the FIRST account was named -- each leg's confirmation is checked against
  // THAT leg's own target, independently, so confirming the cf-config leg alone must not unlock the media
  // leg's write into a DIFFERENT account nobody echoed.
  ...
  //
  // This proves the engine is an independent authority against a console defect that let an operator type
  // ONLY the first crossAccountWarning's target account and still have BOTH legs stamped confirmed. The
  // section answers whether the ENGINE would catch that on its own if it ever received a
  // request shaped that way -- i.e. is the engine an independent authority, or does it merely trust
  // whatever confirmDifferentAccountId the caller sends. Both legs' targetAccount differ from the archive's
  ...
  {
    // THE EXACT SHAPE THE CONSOLE BUG WOULD HAVE PRODUCED: only the FIRST leg (cf-config) confirmed, the
    // second (media, a DIFFERENT foreign account) left unconfirmed. If the engine trusted the request's
    // OWN echo without checking it against THAT leg's own target, this would proceed. It must not.
    const cf = makeCfDouble();
    const up = makeUploaderDouble();
    const kv = new MockKV();
    const res = (await runRestore(
