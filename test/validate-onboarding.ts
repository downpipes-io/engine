...
async function main(): Promise<void> {
  ...
  // Reference ENGINE_VERSION, never a literal (the 0.1.1 release fixed the update tests the same way;
  // this literal was missed and has failed the chain since the version bump).
  ...
  // ---- Engine-hardening status fields (P9 + P4 + DEF-04) ----
  // P9 (ADMIN_TOKEN_DISABLED): tokenFallbackDisabled mirrors the env flag, default false.
  ...
  // P4 (auditNearCap): absent without a count (honestly not asserted); present and correct with one.
  ...
  // DEF-04 (provenance): absent when unset; surfaced verbatim when set; still no secret value.
