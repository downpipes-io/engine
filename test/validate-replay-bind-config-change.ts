// validate-replay-bind-config-change: THE THIRD REPLAY SITE. A recorded identity re-resolved with no request
// of theirs in hand must never resolve to more authority than a live request from that identity would, and
// on the config-change apply it still could.
//
// 9622c01b closed a resurrection: resolveStoredIdentityAuthority and proposerReplayCaller hand a RECORDED
// EMAIL to roleForCaller, and resolveBoundEntry's bind-on-first-auth is a WRITE that matches a pending grant
// BY EMAIL, binds it onto whatever subject is presented, and deletes the pending row. So an ordinary invite
// issued at a departed person's re-used address re-armed the departed person's act AND consumed the new
// person's grant into the departed subject. That commit set a `replay: true` flag at both sites it found and
// recorded that they "are the only two".
//
// THERE WERE THREE. approveChange builds its replay caller INLINE rather than through a named helper, which
// is why a search for the helper did not find it -- and proposerReplayCaller's own comment calls itself "the
// OWNER-action analogue of approveChange's replayCaller", naming the site nobody then looked at. Every queued
// config change was still exposed: a departed proposer's change, their address re-issued to somebody new,
// and the approve bound the newcomer's grant onto the departed subject and applied the change with it.
//
...
async function run(): Promise<void> {
  ...
  const { call, sched, setGate, ageChange } = ctx;
