// Validates the OTLP/HTTP metrics push feature's PURE + drain-internal logic (mon-otlp): the OTLP
// ResourceMetrics JSON builder (envelope correctness, the canonical metric names, injection
...
async function main(): Promise<void> {
  console.log("shape: OTLP ResourceMetrics envelope correctness");
  {
    ...
    // The windowed counts are honest GAUGE names WITHOUT a _total suffix (a _total name invites rate()/
    ...
    ok("the windowed counts are named recent_* (NOT _total, which would invite rate() over a non-monotonic gauge)", OTLP_METRIC_RECENT_ATTEMPTS === "downpipe_backup_recent_attempts" && OTLP_METRIC_RECENT_SUCCESSES === "downpipe_backup_recent_successes" && OTLP_METRIC_RECENT_FAILURES === "downpipe_backup_recent_failures");
    ok("no metric carries a _total suffix (they are windowed gauges, not lifetime counters)", [...byName.keys()].every((n) => !n.endsWith("_total")));
    ok("the recent_* HELP text states they are windowed gauges, NOT lifetime counters", /windowed gauge/i.test(byName.get(OTLP_METRIC_RECENT_ATTEMPTS)!.description) && /NOT a lifetime counter/i.test(byName.get(OTLP_METRIC_RECENT_ATTEMPTS)!.description));
    ...
    // downpipe_enabled is present for EVERY downpipe (both dp-a enabled + dp-b enabled here).
    ok("downpipe_enabled is present for every downpipe (a decommissioned job is distinguishable from a live one)", byName.get(OTLP_METRIC_ENABLED)!.gauge.dataPoints.length === 2 && byName.get(OTLP_METRIC_ENABLED)!.gauge.dataPoints.every((p) => p.asDouble === 1));
    ...
    // A non-truncated snapshot reports the ACTUAL shaped count and no truncation.
    ok("a within-cap snapshot reports downpipeCount === the input length and truncated:false", shaped.downpipeCount === 2 && shaped.truncated === false && shaped.droppedCount === 0);
  }

  console.log("\nshape: downpipe_enabled reflects a paused/decommissioned downpipe as 0");
  {
    ...
    ok("a live downpipe reports downpipe_enabled 1, a paused one reports 0", byDp.get("live") === 1 && byDp.get("paused") === 0);
  }
  ...
  console.log("\nshape: an all-empty snapshot omits every metric (no metric ships with zero data points)");
  {
    ...
    ok("an empty snapshot reports downpipeCount 0, truncated false", shaped.downpipeCount === 0 && shaped.truncated === false && shaped.droppedCount === 0);
  }

  console.log("\nshape: batch cap covers the known 2500-fleet scale and TRUNCATES HONESTLY beyond it");
  {
    // The cap sits well above the engine's known large-fleet scale (validate-fleet-scale.ts's 2500), so a
    // realistic fleet is NEVER silently truncated.
    ok("OTLP_PUSH_DOWNPIPE_CAP covers the known 2500-fleet scale with margin", OTLP_PUSH_DOWNPIPE_CAP >= 5000);
    ...
    // The OVER-CAP case surfaces the truncation explicitly (never a silent drop) -- the shaped count is the
    // cap, truncated is true, and the dropped count is exact.
    ok("an over-cap snapshot reports downpipeCount === cap (the ACTUAL shaped count, never the pre-cap input length)", shaped.downpipeCount === OTLP_PUSH_DOWNPIPE_CAP);
    ok("an over-cap snapshot flags truncated:true with the exact dropped count (never a silent partial)", shaped.truncated === true && shaped.droppedCount === 25);
    ...
    ok(`a full 2500-fleet snapshot serialises under 10 MB (got ${fleetBody.length} bytes) -- a single POST covers the known scale`, fleetBody.length < 10_000_000);
  }

  console.log("\nshape: buildOtlpResourceMetricsChunks -- a large fleet splits into conservative, independently-sized chunks, never one oversized body");
  {
    // At the engine's known large-fleet scale (2500) with generous label sizes, EVERY chunk
  ...
  console.log("\ndeliverOtlpPush: the auth header carries the secret; the body NEVER does (non-negotiable invariant)");
  {
    ...
    globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      if (String(input).startsWith("https://cloudflare-dns.com/dns-query?")) return new Response(JSON.stringify({ Status: 0, Answer: [{ type: 1, data: "93.184.216.34" }] }), { status: 200 }); // the send-time SSRF resolve screen: answered PUBLIC so the case stays about delivery, and not captured
  ...
  console.log("\nDO: otlpMetricsSnapshot -- the MINIMAL read of existing run/replication state (no recomputed freshness)");
  {
    ...
    // so the snapshot's enabled flag is exercised in both polarities.
    ...
    ok("dp-a (config enabled) surfaces enabled:true in the snapshot", a.enabled === true);
    ...
    ok("dp-c (config disabled) surfaces enabled:false in the snapshot (a paused job is distinguishable)", c.enabled === false);
  ...
  console.log("\nrunOtlpPushPass: full drain integration (opt-in gate; NO cursor -- every enabled tick re-pushes; fail-open; false-green on a genuine fault)");
  {
    ...
    {
      // loadConfigWrapKey sits INSIDE the pass's own try, so a malformed CONFIG_WRAP_KEY throws loudly but
      // that throw lands on THIS pass's false-green path (return false), never escapes runOtlpPushPass
      // uncaught -- and it throws BEFORE any DO round-trip (a scheduler that throws on ANY fetch proves the
      // DO is never reached).
    ...
    }
    {
      ...
      if (String(_url).startsWith("https://cloudflare-dns.com/dns-query?")) return new Response(JSON.stringify({ Status: 0, Answer: [{ type: 1, data: "93.184.216.34" }] }), { status: 200 }); // the send-time SSRF resolve screen: answered PUBLIC so the case stays about delivery, and not captured
    ...
    {
      // End to end: an OVER-CAP fleet must record the ACTUAL shaped count (the cap), NOT the pre-cap snapshot
      // length, and must flag truncated:true -- an unguarded 5000+cap fleet's trail would otherwise read
      // downpipeCount = the full length, ok:true, while only the cap was actually sent. The cap is DELIVERED
      // as several chunked requests, each recorded as its OWN outcome, so the assertions sum across
      // state.recorded rather than expecting one entry for the whole cap.
      const overCap = OTLP_PUSH_DOWNPIPE_CAP + 40;
      const state: FakeState = {
      ...
      globalThis.fetch = (async (input: RequestInfo | URL) => {
        if (String(input).startsWith("https://cloudflare-dns.com/dns-query?")) return new Response(JSON.stringify({ Status: 0, Answer: [{ type: 1, data: "93.184.216.34" }] }), { status: 200 }); // resolve screen: answered PUBLIC, not counted
        requestCount++;
        return new Response("ok", { status: 200 });
      }) as typeof fetch;
      try {
      ...
      ok(`the cap is chunked into ${expectedChunks} requests, each recorded as its own outcome`, requestCount === expectedChunks && state.recorded.length === expectedChunks);
      ok("no single recorded chunk ever exceeds OTLP_PUSH_CHUNK_DOWNPIPES", state.recorded.every((r) => r.downpipeCount <= OTLP_PUSH_CHUNK_DOWNPIPES));
      ok("the recorded downpipe counts SUM to the ACTUAL shaped total (=cap), NEVER the pre-cap snapshot length", state.recorded.reduce((n, r) => n + r.downpipeCount, 0) === OTLP_PUSH_DOWNPIPE_CAP);
      ok("EVERY chunk flags truncated:true on an over-cap push (a large fleet is a LOUD signal, never silent)", state.recorded.every((r) => r.truncated === true));
    }
    {
      // At a large but WITHIN-cap fleet (the engine's known 2500 scale): still several chunked requests
      // (never one oversized body), each an independent ok:true outcome, none truncated.
      const state: FakeState = {
    ...
    {
      // A collector that rejects ONLY the second chunk: per-chunk independence means the OTHER chunks still
      // deliver and record ok:true -- one bad chunk does not fail the whole tick's accounting.
      const state: FakeState = {
    ...
    }
    {
      ...
      // same "real cross-version contract violation" discipline validate-siem-push.ts's P2-2 case uses.
      const state: FakeState = {
