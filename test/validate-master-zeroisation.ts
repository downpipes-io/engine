...
//
// WHY THIS EXISTS. The invariant to assert is that no call site may return with a non-zeroed master.
// Discipline alone (seven `finally` blocks with nothing else enforcing them) is not enough: an eighth
// site added without one would pass every other gate in this repo.
//
// This gate matters because it catches two real holes rather than merely confirming a clean state:
//
...
//
// Four rules, kept honest in count here so a gate cannot lose one without anyone noticing:
...
for (const f of files) {
  ...
  for (const m of text.matchAll(PRODUCER)) {
    ...
    // The zeroise must be in a FINALLY, and it must be THIS binding. A bare `name.fill(0)` matched anywhere
    // later in the file is not enough: the file may hold `clock.master.fill(0)` further down, and a
    // substring match cannot tell `master` from `clock.master`. The lookbehind rejects a qualified
    // reference, so a sibling buffer with a similar name cannot stand in for the one being checked.
...
// rule A never counted it and nothing here checked it. Measured rather than argued: deleting BOTH of that
// path's zeroises left this file printing MASTER-ZEROISATION PASS at exit 0 with the producer count
// unchanged at 6, which is precisely the silent-clean failure this gate exists to make impossible. The
// buffered path really was zeroising at the end of the try and again at the head of the catch rather than in
// a finally, so an early `return` added to that try would have returned with a live master.
//
// The property name must still NAME a master, for the same reason rule A's binding must: this is a rule
// about run masters, not about every 32 random bytes. One match across the 414 source files, and it is
// the real run master.
