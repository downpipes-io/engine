...
//   - retry / throttle-park / RUNLOG-park / strike pressure is invisible until terminal failure unless recorded:
//         the retry counters live in a closure that returns, and the ladders live in the RunSealDO doc that
//         cleanup() deletes. A run that rides out weeks of 503s before finally dying would look exactly like
//         one that failed on a clean estate, and 8 strikes with ONE cause would look exactly like 8 strikes
//         with eight.
...
function partA(): void {
  ...
  // The owner's #1 fear: a wrangler deploy overwrote the bindings, so the configured source binding is gone.
  let caught: unknown;
  try {
...
async function partE(): Promise<void> {
  ...
  ok("it carries the swallowed probe flaps (otherwise silent)", p?.probeFlaps === 3);
...
async function partF(): Promise<void> {
  ...
  ok("its park count survives the doc cleanup (which otherwise erases it)", p?.throttleParks === 22);
