...
/**
 * ageRecord (ASVS V2.4.2, approval-dwell) back-dates a stored dual-control record's raise timestamp
 * (`requestedAt` on a restore/prune approval, `proposedAt` on an owner action or config change) by `ms`,
 * so a bed's same-turn approve lands after APPROVAL_MIN_DWELL_MS without waiting on the real clock.
 *
 * Why this exists. Every approve helper now refuses an approve that arrives inside the dwell floor
 * (reasonCode "too-soon"), and a bed that raises then immediately approves is exactly the shape the floor
 * exists to refuse. validate-dualcontrol.ts's PROOF 9 already back-dates a record's expiresAt through
 * storage.rawGet/rawPut to simulate the clock advancing past the TTL; this is the same trick, generalised
 * over the storage key and the field name so restore, prune, owner-action and config-change beds share
 * ONE implementation rather than four independently-drifting copies.
 *
 * `storage` is the MockStorage the bed's SchedulerDO double already holds (`sched.storage`); rawGet/rawPut
 * bypass the JSON-round-trip clone so the record is read and rewritten as the exact object in the map.
 */
export function ageRecord(storage: { rawGet<T>(key: string): T | undefined; rawPut<T>(key: string, value: T): void }, key: string, field: "requestedAt" | "proposedAt", ms: number): void {
  const rec = storage.rawGet<Record<string, unknown>>(key);
  if (rec === undefined) throw new Error(`ageRecord: no stored record at ${key}`);
  const raw = rec[field];
  if (typeof raw !== "string") throw new Error(`ageRecord: ${key}.${field} is not a string`);
  const at = Date.parse(raw);
  if (!Number.isFinite(at)) throw new Error(`ageRecord: ${key}.${field} does not parse`);
  rec[field] = new Date(at - ms).toISOString().replace(/(\.\d{3})\d*Z$/, "$1Z");
  storage.rawPut(key, rec);
}
