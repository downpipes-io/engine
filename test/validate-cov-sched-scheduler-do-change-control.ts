...
// AGE_PAST_DWELL (ASVS V2.4.2, approval-dwell): back-dates a queued change's proposedAt past
...
async function main(): Promise<void> {
  ...
  try {
    ...
    ageRecord(s.storage, changeKey(idL), "proposedAt", AGE_PAST_DWELL);
    ...
    ageRecord(s.storage, changeKey(idB), "proposedAt", AGE_PAST_DWELL);
    ok("approving B first moves the head (200)", (await call(OPERATOR2, "POST", `/admin/config/changes/${idB}/approve`)).status === 200);
    // Aged so the dwell floor does not itself intercept before the base-moved guard the message + status
    // assertions below depend on.
    ageRecord(s.storage, changeKey(idA), "proposedAt", AGE_PAST_DWELL);
    ...
    ageRecord(s.storage, changeKey(idT), "proposedAt", AGE_PAST_DWELL);
    ...
    ageRecord(s.storage, changeKey(idLEG), "proposedAt", AGE_PAST_DWELL);
    ...
    ageRecord(s.storage, changeKey(idCR), "proposedAt", AGE_PAST_DWELL);
