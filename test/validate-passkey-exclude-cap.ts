// passkeyRegisterBegin must never emit an excludeCredentials list larger than the browser will accept,
// because past that ceiling the browser refuses the whole ceremony and the account can never enrol
// another passkey. Run:
//   node test/validate-passkey-exclude-cap.ts
//
// WHAT WENT WRONG, MEASURED ON A LIVE ESTATE. Three accounts on the probe harness estate held exactly 65
// passkey credentials each. passkeyRegisterBegin put EVERY one of them into excludeCredentials, and
// Chromium refuses at 65 with a DOMException named RangeError ("The `excludeCredentials` attribute exceeds
// the maximum allowed size (64)") thrown before any authenticator is consulted. A dose-response over 0, 1,
// 18, 32, 63, 64, 65, 66 and 100 with a CDP virtual authenticator created a credential at every size up to
// and including 64 and threw at every size from 65 up, with n=0 as the known positive.
//
...
// straight into a passkey enrolment, and it is THAT enrolment which replaces the spent set. With the
// ceremony refused in the browser, every break-glass sign-in spent a code and replaced nothing. probe's
// owner went from 10 unconsumed codes to 8 in one morning across two such sign-ins, with zero
// recovery-codes-generated events between them. An account that empties its bank this way cannot enrol,
// cannot assert a passkey it has lost, and cannot delete a credential to get back under the ceiling
// (/passkey/credentials/delete is step-up gated on the cookie path, and step-up needs the assertion it
// does not have). That is a permanent lockout reached by using the documented break-glass path as intended.
//
...
// ---- (a) the bound itself ------------------------------------------------------------------------
{
  // THE MEASURED NUMBER. If this constant ever drifts above what a browser accepts the whole repair is
  // undone silently, so the number itself is asserted rather than merely used.
