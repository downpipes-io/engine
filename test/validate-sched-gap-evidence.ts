// validate-sched-gap-evidence -- proves the scheduler DO records diagnostic evidence for faults the engine
// detects and would otherwise discard.
//
// Every case in this file is a fault the engine detects and then discards, so the whole test suite is one
// shape, applied repeatedly:
//
//   1. RECORDED ON THE REAL PATH. Not "the recorder works" -- the recorder is CALLED where the fault is,
//      and the aggregate the pack reads comes back non-empty. Evidence counts only once it is IN THE
//      BUNDLE, so every assertion here reads through readSchedDiag (the single GET /sched-diag projection
//      the pack builds from), never a private key.
...
const SENTINELS: readonly string[] = [
  ...
  "example-secret-9f3a2b7c8d1e4f5a6b7c8d9e0f1a2b3c", // a secret
...
// =====================================================================================================
// rejected configuration writes (the 400 that reached only the browser)
// =====================================================================================================
...
// =====================================================================================================
// authorisation refusals (the 403 audited only when it succeeds)
// =====================================================================================================
...
// =====================================================================================================
// the alerting pipeline's own health
// =====================================================================================================
...
// =====================================================================================================
// accepted-but-silently-narrowed writes
// =====================================================================================================
...
// =====================================================================================================
// SIEM audit egress health
// =====================================================================================================
async function g323(): Promise<void> {
  console.log("auditEgress");

  await t("mirrorAuditEvent reports success/failure rather than swallowing it silently", () => {
...
// =====================================================================================================
// capped records that never said they were capped
// =====================================================================================================
...
// =====================================================================================================
// the security-posture evaluation and its swallowed input faults
// =====================================================================================================
...
// =====================================================================================================
// SSO per-connection attribution + the uncounted start/metadata paths
// =====================================================================================================
async function g140(): Promise<void> {
  console.log("sso start/metadata + per-connection ordinal");

  await t("the sso start and metadata paths are CLOSED CODES in the shared vocabulary", () => {
...
// =====================================================================================================
// beacon failure CAUSE class + intermittency (the last-attempt-only record)
// =====================================================================================================
async function g324(): Promise<void> {
  ...
  await t("the CAUSE class rides, so a malformed BEACON_URL is not a network blip", async () => {
    ...
    assert.equal(rec.errorClass, "url-invalid", "the network-level failure carries WHY");
...
async function main(): Promise<void> {
  console.log("validate-sched-gap-evidence: the scheduler-DO fault-ledger evidence checks\n");
  await g139g141();
  await g187g249();
  await g188();
  await g297();
