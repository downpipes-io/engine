...
| Source file | `engine/src/sched/scheduler-do-limits.ts:741` (`RING_CAP`), `engine/src/sched/scheduler-do-records.ts:320` (`DRILL_EVIDENCE_CAP`) |
| Protection level | **L1** |
...
| Retention | Two independent bounds. COUNT: `RING_CAP` = 50 run entries per downpipe (`engine/src/sched/scheduler-do-limits.ts:741` (`RING_CAP`)); the drill-evidence log rolls over above `DRILL_EVIDENCE_CAP` = 500; older entries are evicted automatically. AGE (ASVS V14.1.2, added after this document's first CLOSED claim below was found stale): an entry older than `RUN_HISTORY_MAX_AGE_DAYS` (400 days, `engine/src/sched/history-retention.ts:26-27` (`RUN_HISTORY_MAX_AGE_DAYS`)) is purged regardless of how many entries the ring holds, so a low-traffic downpipe no longer keeps a run's metadata for the life of the account. Enforced by a cron-driven pass (`engine/src/cron/history-retention-pass.ts`) that calls the scheduler DO's own purge route (`POST /history-retention-pass`, `engine/src/sched/scheduler-do-routing.ts:303` (`history-retention-pass`)); an in-flight run, or an entry whose timestamp will not parse, is never purged (fail-safe towards retaining data). Proven by `engine/test/validate-run-history-retention.ts`. |
...
| Retention | 7 days (`engine/src/admin/passkey.ts:153` (`INVITE_TTL_MS`)); the bootstrap invite 24 hours (`engine/src/admin/passkey.ts:174` (`BOOTSTRAP_INVITE_TTL_MS`)). Deleted before the bound email is returned, so an invite authorises at most one registration. |
