...
- **Source network address.** The router reads the edge-injected `CF-Connecting-IP` header and the
  ...
  on every authentication event as non-authority metadata (`engine/src/admin/identity.ts:116`). The
...
or an assertion whose UV flag is clear is rejected as `user_verified`, `engine/src/admin/passkey.ts:277`
and `engine/src/admin/passkey.ts:352`), the connection identity is bound into the signed body, the
