// The bespoke egress-secure sender for the OTLP/HTTP metrics push destination (mon-otlp),
// used by the cron drain (cron/otlp-push-pass.ts). It copies deliverSiemPush's exact send hygiene
...
import { classifyHttpDeliveryStatus, classifyNetworkFailure, type DeliveryFailCode, screenResolvedIfHostname, screenSinkHost, WEBHOOK_TIMEOUT_MS } from "./types.ts";

// OtlpPartialBodyClass is the CLOSED class of what the collector's 200 body actually WAS when the sender went
// looking for partial_success. Without it, EVERY awkward body (empty, oversized, non-JSON,
// non-proto3-shaped) would collapse to `undefined` and the push would be recorded as a fully-clean delivery -- so
// "Datadog shows dropped datapoints but Downpipes says every push delivered clean" was undiagnosable. The
// class rides on the result's `code` (and thence the push trail's `reason`) so the pack can tell a collector
// that CONFIRMED a clean accept from one whose answer we could not read at all:
...
export interface OtlpPushSendResult {
  ...
  // OtlpPartialBodyClass: the collector accepted the push, but its partial_success body could not be
  ...
  // (mirroring `truncated`) instead of reading the 200 as a fully-delivered push. Absent/0 when the collector
  // accepted everything.
...
export async function deliverOtlpPush(url: string, body: string, authHeaderName: string, authHeaderValue: string): Promise<OtlpPushSendResult> {
  ...
  // And what the name RESOLVES to (ASVS V1.3.6), the same screen the SIEM push applies.
  ...
  try {
    ...
    if (ok) {
      ...
      // The body CLASS rides alongside the count: an oversized/unparseable/absent body means we
    ...
    // CLOSED sub-cause (dns / tls / reset / other) at this site; the exception text never leaves.
