...
// this adapter otherwise targets.
...
// /api/global/em/jsonv2 -- always a customer url, so it is SSRF-screened like any other). HTTP
...
export async function deliver(env: Env, channel: NotifyChannel, emission: NotifyEmission, egressAllowlist?: readonly string[]): Promise<ChannelDeliveryResult> {
  // A ServiceNow channel needs an instance URL, a username AND a password, and the one code covered
  // all three. The url/username pair is the ENDPOINT class; the password is the CREDENTIAL class.
  // The url and the username are DIFFERENT missing fields with different fixes. Reporting a blank
  ...
  try {
    ...
    // the key is malformed: there is no usable credential. This is its OWN closed code, NOT
    // no-transport: the two used to be byte-identical in the pack, so "all our ServiceNow alerts stopped after
    // we rotated CONFIG_WRAP_KEY" read as "channel misconfigured" and support sent the customer to re-enter
    // fields that were never wrong. The fix here is the wrap key / re-sealing the credential. Fail-open.
  ...
  const r = await deliverPayload(channel.url, body, channel.allowInternalSink === true, headers, undefined, egressAllowlist);
