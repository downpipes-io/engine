...
//
// Without a named arm per reason, THREE distinct owners would collapse into `email-rejected` -- a subject
// that fails the send primitive's bounds, a body that fails them (both ENGINE-side formatting bugs) and the
// Email Service refusing the send outright (a CUSTOMER-side onboarding gap: an un-onboarded sending domain,
// an unverified sender). Same code, opposite fixes, which would make "our email alerts stopped" impossible
// to diagnose from the pack. Each arm is named instead; the residual email-rejected remains for any reason
// outside the vocabulary, so a future reason can never fall out of the closed set.
export function emailReasonToCode(reason: string | undefined): DeliveryFailCode {
  switch (reason) {
    case "email-not-configured":
      return "email-not-configured";
    // Collapsing these two into ONE code would send support hunting a malformed sender address that does
    // not exist whenever EMAIL_FROM is simply never set, when the customer has just not finished setup.
    // Unset and set-but-wrong are different tickets, so they are different codes.
...
export async function deliver(env: Env, channel: NotifyChannel, emission: NotifyEmission): Promise<ChannelDeliveryResult> {
  ...
  // The channel HAS a transport (email); what it lacks is an ADDRESS LIST. A generic no-transport code
  // would only say "something about this channel is missing" and leave the customer unable to tell which
  // field to fill.
  ...
  // documented E_UPPER_SNAKE code passes through, a NON-CONFORMING code is recorded as the engine-owned
  // E_OTHER placeholder (the fact that the platform sent a code is itself evidence, so it is kept rather
  // than dropped with the code), and the raw platform string never leaves the classifier.
  const platformCode = emailPlatformCodeOf(r.code);
  // Carry the failing recipient's INDEX onto the history row. The address never leaves sendEmail.
  return {
    ok: false,
