...
import { classifyHttpDeliveryStatus, classifyNetworkFailure, screenEgressAllowlist, screenResolvedIfHostname, screenSinkHost, WEBHOOK_TIMEOUT_MS } from "../types.ts";
...
// JSM_SOURCE_MAX bounds the `source` field: a generous ceiling so an
...
// present only when a 202's body carried one.
...
//
// egressAllowlist (ASVS V13.2.4) is checked the same way deliverPayload checks it: before and independent
// of allowInternal, so a private host still needs both the allowlist match and the internal-sink opt-in.
async function deliverJsmRequest(url: string, payload: unknown, allowInternal: boolean, headers: Record<string, string>, egressAllowlist: readonly string[] | undefined): Promise<JsmSendOutcome> {
  ...
  // And what the name RESOLVES to (ASVS V1.3.6), honouring the same per-channel opt-in.
  if ((await screenResolvedIfHostname(url, sinkScreen, allowInternal)).refused) return { ok: false, code: "internal-sink-resolved", sinkScreen };
  ...
  try {
    ...
    // The transport fault is coarsened to its CLOSED sub-cause here; the exception text never leaves.
...
// documented way to learn a 202's real outcome. Bounded by the SAME WEBHOOK_TIMEOUT_MS budget
...
async function pollJsmRequestStatus(baseUrl: string, requestId: string, headers: Record<string, string>, egressAllowlist: readonly string[] | undefined): Promise<boolean | undefined> {
  const url = requestStatusUrlFor(baseUrl, requestId);
  const sinkScreen = screenSinkHost(url);
  if (sinkScreen === "url-invalid" || sinkScreen === "internal-literal") return undefined;
  if (!screenEgressAllowlist(new URL(url).hostname, egressAllowlist)) return undefined;
  if ((await screenResolvedIfHostname(url, sinkScreen, false)).refused) return undefined;
  const controller = new AbortController();
...
// siblings use.
//
// ASYNC-ACCEPTED 202: the Alert API's create/close normally answers
...
export async function deliver(env: Env, channel: NotifyChannel, emission: NotifyEmission, egressAllowlist?: readonly string[]): Promise<ChannelDeliveryResult> {
  // "My JSM channel never delivers" is not diagnosable when the URL and the GenieKey guards emit the same
  // code. Two fields, two codes.
  ...
  try {
    ...
    // key is malformed: there is no usable credential. This is its OWN closed code, distinct from no-transport,
    // so "all our JSM alerts stopped after we rotated CONFIG_WRAP_KEY" reads as a credential problem rather
    // than "channel misconfigured", which would send the customer to re-enter fields that were never wrong.
    // The fix here is the wrap key / re-sealing the credential. Never throw (fail-open).
  ...
  const sent = await deliverJsmRequest(url, payload, allowInternal, headers, egressAllowlist);
  if (!sent.ok) {
  ...
  if (sent.status === 202) {
    ...
    const confirmed = sent.requestId !== undefined ? await pollJsmRequestStatus(channel.url, sent.requestId, headers, egressAllowlist) : undefined;
    // ackOutcome splits the flat unconfirmed:true flag into its two very different meanings. A poll
    // that came back data.success:FALSE is JSM POSITIVELY telling us the alert was NEVER CREATED (a critical page
    // that never appeared) -- an actionable non-delivery that used to be recorded identically to "we could not
    // reach the status endpoint". ok/unconfirmed are UNCHANGED (a create is idempotent-by-alias, and flipping ok
    // would retry-storm a healthy async provider); this is a diagnosis, not a behaviour change.
