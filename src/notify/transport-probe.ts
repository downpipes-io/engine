...
//
// This is not hypothetical. A Splunk Cloud stack serves its HEC ingest port (8088) with Splunk's stock
// self-signed certificate (CN=SplunkServerDefaultCert) while 443 carries a proper CA-issued one. Strict TLS
// refuses before any HTTP is exchanged, so a push destination pointed at that stack cannot deliver at all,
// and the only thing the operator was told was "network-error".
//
...
// Measured against the stack above: 8088 plain opens and 8088 TLS is refused (the certificate case), 443
// plain and TLS both open (a sound chain), and a closed port refuses both (not a certificate case).
//
