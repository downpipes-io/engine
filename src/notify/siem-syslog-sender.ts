// The syslog-over-TLS delivery sink for the SIEM audit-log push destination (the "syslog-TLS sink").
// It is the enterprise auto-parse path: QRadar, Microsoft
...
// throw ERR_MODULE_NOT_FOUND at MODULE-LOAD time and turn the whole gate red. The documented
...
import { canonicaliseBareHost, isInternalSinkHost, WEBHOOK_TIMEOUT_MS } from "./types.ts";
...
export interface SiemSyslogSendResult {
  ok: boolean;
  code?: SyslogFailCode;
  // failedIndex is the BOUNDED, zero-based index of the event whose shaping threw, on a
...
// SyslogFailCode is the CLOSED failure vocabulary of the syslog-TLS sink. It separates three different
// owners that a single collapsed code cannot distinguish: "our runtime has no sockets" (an ENGINE fault:
// the cloudflare:sockets import failed), "your receiver's certificate is not trusted" (a CUSTOMER TLS
// fault: the TLS handshake rejected on socket.opened) and "nothing is listening / your firewall dropped
// us" (a CUSTOMER NETWORK fault: connect() threw). Without the split, "our QRadar feed on 6514 never came
// up" could not be triaged remotely at all. Each cause has its own member, and the timeout carries the
// phase it stalled in.
export type SyslogFailCode =
  ...
  | "syslog-internal-sink-blocked"; // the host classifies as internal/private/loopback/link-local: no socket was opened
...
export const SYSLOG_FAIL_CODES: ReadonlySet<string> = new Set<SyslogFailCode>([
  ...
  "syslog-format-unsupported", "syslog-socket-cap", "syslog-internal-sink-blocked",
]);
...
//
// WHY THE TYPE, AND NOT A RUNTIME GUARD FURTHER DOWN. buildSyslogRecord used to take the full PushFormat and
// read `format === "leef" ? shapeLeef(...) : shapeCef(...)`, so every one of the other six formats fell into
// the CEF arm and was DELIVERED AS CEF with nothing recording the substitution. A destination configured
// splunk-hec over syslog-tls passed both config validators and then shipped ArcSight CEF to a Splunk HEC
// parser. Narrowing the parameter makes that fallback a compile error: a new PushFormat member cannot reach
// this file at all unless someone deliberately adds it to SYSLOG_TLS_FORMATS and writes its shaper arm.
...
// facility*8 + the per-event severity. HOSTNAME/APP-NAME are our fixed identifiers.
...
// MAX_CONCURRENT_SOCKETS caps concurrently-open outbound sockets per isolate (cap 6).
...
// opts.allowInternalSink relaxes the internal-host screen below and is likewise validator-only: the drain
// (cron/siem-push-pass.ts) never sets it on the production path.
export async function deliverSiemSyslog(host: string, port: number, format: PushFormat, events: AuditEvent[], meta: PushCursorMeta, opts?: { timeoutMs?: number; allowInternalSink?: boolean }): Promise<SiemSyslogSendResult> {
  const capped = events.slice(0, SIEM_PUSH_BATCH_CAP);
  if (capped.length === 0) return { ok: true };

  // SSRF SCREEN AT SEND TIME, the same isInternalSinkHost classifier the notify webhook, the IdP test and
  // the destination endpoint all use. A stored host of 169.254.169.254 or 10.0.0.5 must never get a real
  // TCP/TLS connection attempt carrying the estate's audit stream, so this screen is the control that stops
  // it: whether workerd's own connect() refuses private space is not something this codebase has measured,
  // and an application-layer sink must not rest on an unmeasured platform behaviour. It runs BEFORE the
  // shaping, the concurrency slot and the connect, so a refusal costs nothing and opens nothing.
  //
  // THE PRODUCT CONSEQUENCE, stated plainly: an on-prem SIEM reachable only on a private address cannot be
  // fed by this sink. It never could. The engine runs at the Cloudflare edge with no tunnel into a customer
  // network, so an unscreened connection attempt would fail later and less honestly. The refusal is a named
  // non-delivery the push trail shows.
  if (opts?.allowInternalSink !== true && isInternalSinkHost(canonicaliseBareHost(host))) return { ok: false, code: "syslog-internal-sink-blocked" };
  ...
  try {
    ...
    // bounded INDEX so support can point at it remotely instead of "syslog-shape-failed, forever".
  ...
  try {
    ...
    // HARD TIMEOUT: the bound must hold regardless of runtime behaviour,
    ...
    // distinguishing a black-holed handshake from a stuck flush rather than collapsing both to `timeout`,
    // and it splits a TLS-trust rejection (handshake) from a mid-stream write failure.
    ...
    const writeSequence = (async (): Promise<SiemSyslogSendResult> => {
      try {
        ...
        // receiver certificate, the single most common "our QRadar feed never came up" cause and the one the
        // old collapsed syslog-connect-failed could never name.
