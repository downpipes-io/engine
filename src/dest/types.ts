// DestIoSnapshot lives here, in the types module, rather than in dest-io.ts: dest-io.ts imports
// admin/diag-writer.ts, which reaches router-helpers.ts and from there the whole scheduler-do mixin web,
// so importing it from dest-io.ts would pull the Durable Object into a types leaf that has no business
// depending on it. A pure interface belongs with the other types either way.
/**
 ...
 * The message is byte-identical to the plain Error it replaces, so every existing catch, log line and
 ...
 *
 * WHY. Classifying a cleanup delete's failure from the bare fact of a throw calls every one of them "denied"
 * -- a PERMISSION fact that has not been established. A 503 SlowDown, a 500, a 429 and a dropped socket are
 * not a denial, and telling support the customer's retention is permanently broken because one request was
 * unlucky is the same defect, inverted, as hiding a real least-privilege denial behind "the store was
 * flaky". The status is already in hand at the throw site; only the type carries it forward.
 *
...
// on put either). Measured against a real R2 bucket over its S3 endpoint, with this repo's own
// signer: GET /<bucket>/?object-lock= answers 404 ObjectLockConfigurationNotFoundError, and a PUT
// carrying x-amz-object-lock-mode: COMPLIANCE answers 501 NotImplemented, "Header
// 'x-amz-object-lock-mode' with value 'COMPLIANCE' not implemented". The S3 endpoint is therefore
// NOT an escape hatch: following it means a 501 on every write and an archive holding nothing.
/**
...
export interface WormStatus {
  ...
  defaultDays?: number;
  // unknownReason names WHY the probe could not confirm enforcement. A compliance customer's
...
/**
 * WORM_UNKNOWN_REASONS is the CLOSED vocabulary behind a "cannot confirm" WORM probe.
...
export interface Destination {
  ...
  // destination instance's lifetime: the op, the HTTP status, the closed S3 <Code>, the fault class,
  ...
  // destIo reports the bounded DEGRADATION counters of this destination instance: the branches a
