// D1 RESTORE FAULT CLASSIFICATION -- the closed vocabulary and the typed error a D1 restore raises,
...
// ---------------------------------------------------------------------------------------------------------
// THE RESTORE METADATA A SINK SILENTLY SHEDS
//
...
// past, and the binding refuses a put whose expiration is not at least a minute ahead. Draining the key to
// failures under the generic destination-access reason would lose the customer's data on the ordinary
// operation of restoring an old backup and name the wrong cause while doing it, so the key restores without
// the lapsed expiration instead and the loss is counted here.
//
