...
// using the vocabulary is the first step toward a destination module that starts honouring it. I broke
// that gate when I created this file, and the comment is reworded rather than the gate widened.
 ...
 * the store would not keep.
 *
 * The two primitives are real. The conclusion was not. The mapping is exact, because the two vocabularies
 * name the same two guarantees: governance means a privileged principal CAN lift the retention, which is
 * an UNLOCKED Azure policy; compliance means nobody can shorten or remove it, which is a LOCKED one. And
 * nothing is being guessed at all, because the mode is a REQUEST HEADER chosen per write rather than a
 * property of the container that has to be inferred. The legal hold, which genuinely has no counterpart
 * here, is simply never set.
 *
 * This is the second refusal in this file to fall to the same test in the same week, after the GCS one
 * below, and the pair is the lesson: BOTH were written from a belief about a provider rather than from a
 * measurement, both were wrong, and in both cases the engine's own live capability probe already reached
 * the right answer and the hardcoded rule on top of it only broke the working case. An Azure container
 * with version-level immutability enabled now answers the probe, has its policy accepted, and enforces it;
 * a container without it is refused by the probe's verdict, which is the same rule every other store is
 * held to rather than a special case for this one.
 *
 ...
 *
 * So the engine's existing PROBE already reached the right answer in both directions, and a hardcoded
 * refusal on top of it only broke the working one. The general lesson is worth more than the fix: a live
 * capability probe beats a rule written from a belief about a provider, and the probe was here first.
 *
 ...
 *
 * So this changes no outcome. It changes the SENTENCE. The object-lock refusal used to tell the operator
 * to "create a new bucket with Object Lock enabled", which is Amazon's spelling of the setting. Google
 * Cloud DOES have the setting, under its own name, PER-OBJECT RETENTION, and only at bucket-create time,
 * which is what the measurement above establishes; the old sentence sent the operator looking for an
 * "Object Lock" checkbox that Google Cloud's console does not have. A remedy the operator cannot follow
 * is worse than a plain refusal, and this one was ALSO wrong about the capability, not only its name.
 * The per-provider remedies now live in one place, dest/worm-remedy.ts, for every surface that offers one.
 */
