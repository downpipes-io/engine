...
import { isInternalSinkHost } from "../notify/types.ts";
...
// Re-export the addressing style so importers of this module (e.g. sched/scheduler-do.ts) can use the
// "../dest/s3.ts" import path.
 ...
 * multipart part) so a black-holed S3-compatible endpoint can never hang a pass indefinitely. It is
 * GENEROUS: a large backup segment shares this exact primitive (a multipart part up to PART_SIZE, or a
 * small buffered object up to MULTIPART_THRESHOLD), so the bound must survive a real-world
 * slow-but-progressing upload, not just the fast common case -- 120s is well beyond a typical
...
export class S3Destination implements Destination {
  // allowInternalEndpoint relaxes the two endpoint screens (the https/loopback check at construction and the
  // internal-host screen in metered()). Readonly and set once from opts, so nothing can flip it after the
  // destination is built.
  private readonly allowInternalEndpoint: boolean;
  ...
  // pacer is the OPTIONAL adaptive destination throttle. When set, every outbound request takes a
  ...
  // fetchTimeoutMs is the bound metered() applies to every outbound fetch. Defaults to the
  ...
  // own lifecycle/abort policy must reap. It is a DIAGNOSTIC counter only (an integer, never a key/id/value);
  // putMultipart increments it in the abort-failed branch it otherwise swallows, and the seal driver reads
  // it via multipartAbortFailures() to surface the signal on the run row.
  private multipartAbortFailedCount = 0;

  // faults is the bounded, closed-vocabulary destination-fault ring: every failing op notes its
  ...
  // io is the bounded DEGRADATION counter set: the swallowed-by-design branches that make a run that
  ...
  // surface stays the 5 required identity fields plus opts, rather than growing an ever-longer positional
  // parameter list. An omitted opts (and any omitted field) is byte-identical to before; a future
  // destination knob has a named home, not a positional tail.
  constructor(endpoint: string, bucket: string, region: string, accessKeyID: string, secretKey: string, opts?: { now?: () => Date; meter?: Meter; worm?: WormPolicy; pacer?: DestPacer; sessionToken?: string; addressing?: Addressing; storageClass?: string; fetchTimeoutMs?: number; allowInternalEndpoint?: boolean }) {
    // allowInternalEndpoint is the TEST SEAM described on requireHttpsEndpoint and on the wire screen in
    // metered() below. It relaxes BOTH, and only a validator ever sets it: dest/factory.ts (every env and
    // console destination) and cron/siem-push-pass.ts's production caller both omit it, so a deployed
    // destination is screened on every request it makes.
    this.allowInternalEndpoint = opts?.allowInternalEndpoint === true;
    ...
    requireHttpsEndpoint(endpoint, { allowInternalEndpoint: this.allowInternalEndpoint });
  ...
  private async metered(input: URL, init: RequestInit, destOp: DestOp): Promise<Response> {
    ...
    // (PUT/POST multipart parts, DELETE); an untagged spend would still count the total.
    const method = (init.method ?? "GET").toUpperCase();
    // SSRF screen at the wire. This screens the URL every request actually goes to, not merely the endpoint
    // an operator submitted at intake, so this method never signs the destination credential onto a request
    // to an internal/private/loopback/link-local address regardless of how a destination record reached it.
    // This is the egress class carrying the destination credential and the customer's archive bytes, so the
    // screen sits at the one chokepoint every request passes through rather than only at intake.
    //
    // This screen is what actually stops the request: the Workers runtime's own routing behaviour for a
    // private address is not relied on. It reads the URL that is about to go on the wire (never the stored
    // field), so a vhost prefix or any other rebuild of the URL is screened as sent.
    //
    // It runs BEFORE the meter spend and the pacer token: a request that never leaves is not a subrequest,
    // and must not be billed as one or consume a pace slot.
    if (!this.allowInternalEndpoint && isInternalSinkHost(input.hostname)) {
      // Record the refusal so a held guard is visible rather than silent. The class only; the host is
      // caller-chosen and never rides into the fault ring.
      this.faults.noteGuardRefusal("endpoint-internal-refused");
      throw new Error(`s3 request refused: the destination endpoint resolves to an internal/private/loopback/link-local address (${method})`);
    }
    const costOp: CfOp = method === "GET" || method === "HEAD" ? "r2ClassB" : "r2ClassA";
    this.meter?.spend(1, costOp);
    // Pace BELOW the destination's tolerance: take a token before the request, then fold the
    ...
    // Bound the request with a finite, generous AbortController timeout: a black-holed
    ...
    try {
      ...
      // Note the DEGRADATION this response represents, at the one chokepoint every request passes
      ...
      // Note the TRANSPORT fault before rethrowing. The recorded record carries the op, the closed
      ...
      if (timedOut) {
        ...
        // multipart step RETRIES it away, so a run can succeed having eaten several of these. Count it.
  ...
  private url(key: string): string {
    ...
    // encodePath REFUSES a key with a "." or ".." path segment (the traversal guard). The refusal is a
    // hard error the run row coarsens away, so support could not tell a specific dot-segmented key from the
    // destination itself stopping the run. Record the closed class (never the key) and rethrow unchanged:
    // the guard's behaviour is untouched, only its visibility is improved.
    try {
  ...
  // re-sending the credentialed SigV4 request to a third-party endpoint. Any
  ...
  // this destination instance's lifetime, so the seal driver can stamp `multipartAbortFailed` on the run
  // row and the invisible stranded-part-storage fault becomes visible. An integer only, never a
  // key/id/value. Implements the optional Destination.multipartAbortFailures accessor.
  ...
  // instance's lifetime. Implements the optional Destination.destFaults accessor; the seal driver
  ...
  // destIo is the bounded DEGRADATION snapshot of this instance: the counters for the branches a
  ...
  // errorBodyOf reads a FAILED write's error body once, bounded regardless of whether the
  ...
  async put(key: string, body: Uint8Array): Promise<void> {
    ...
    if (resp.status >= 300 && resp.status < 400) {
      ...
      this.faults.noteRedirect(resp); // the region hint / unrecognised-target class (never the Location URL)
  ...
  // ifMatch is never silently dropped from the signed headers and the request. An
  ...
  async putConditional(key: string, body: Uint8Array, opts: { ifMatch?: string; ifNoneMatch?: string }): Promise<PutConditionalResult> {
    ...
    // the credentialed SigV4 request to a third-party endpoint.
    ...
    // in the FAULT ring. It IS counted as a degradation: one conflict is routine, but a rising count on
    // the RUNLOG means two engines fighting over one log, which a successful run would otherwise hide.
    if (resp.status === 412) {
    ...
    // A conforming store returns an ETag on a successful conditional PUT. A store that does NOT is
  ...
  // putStream writes a streamed body (a large sealed segment, plaintext <=1 GiB). A streaming
  // request body emits chunked Transfer-Encoding with no
  ...
  async putStream(key: string, body: ReadableStream<Uint8Array>, size?: number): Promise<void> {
    if (size !== undefined && size > MAX_STREAM_SEGMENT_BYTES) {
      // The engine's OWN ceiling refused the write. The run row coarsens the message away, so support
  ...
  private async uploadAllParts(key: string, body: ReadableStream<Uint8Array>, bound: number, uploadId: string): Promise<string[]> {
    ...
    for (;;) {
      ...
      total += value.length;
      if (total > bound) {
        // A sealed stream overran its declared bound -- an engine-side invariant, so the MAGNITUDE is
  ...
  private async multipartStep<T>(label: string, fn: () => Promise<T>): Promise<T> {
    ...
    for (let attempt = 0; attempt < MULTIPART_RETRY_ATTEMPTS; attempt++) {
      if (attempt > 0) {
        ...
        // throttling destination is not re-hit on a blind 150ms backoff. Capped at MULTIPART_MAX_BACKOFF_MS
        // so a large Retry-After cannot stall the slice wall budget; a throttle needing longer is carried
        // by the run's resume.
        const expo = Math.floor(Math.random() * 150 * 2 ** (attempt - 1));
        const ra = retryAfterMsOf(lastErr);
        // Count the RE-ISSUE. A retry that eventually succeeds leaves no trace anywhere today, yet a
      ...
      try {
        ...
        lastErr = e;
        // A STAMPED Object-Lock refusal is thrown on immediately, above the message regex, because R2
        // refuses a lock-bearing CreateMultipartUpload to a bucket that is not Object-Lock enabled with a
        // 501, which /status (5\d\d|429)/ would otherwise match and retry three times against a property
        // the bucket can only be created with. Rethrowing HERE also keeps the stamp: the
  ...
  //
  // req.op is the closed destination op this request belongs to, so a transport fault or a refused
  ...
  // default of "". The conditional-write guard in putConditional uses !== undefined,
  ...
  // off-site replication seg/ sync uses); list() above accumulates the whole result and is for
