...
export interface RestoreSink {
  ...
  // D1 restore FILTERED OUT of the apply. A subset restore silently drops the indexes and triggers of
...
export class KVRestoreSink implements RestoreSink {
  ...
  async put(name: string, value: Uint8Array, opts?: RestorePutOptions): Promise<void> {
    ...
    // The descriptor CARRIED an expiration and it is unusable (not a number, non-positive, NaN): the key
    // is restored with NO TTL and will never expire, which would otherwise leave a receipt claiming full
    // fidelity anyway. Count the shed at the drop site (a closed field kind; the unusable VALUE is never
    // carried) so support can tell the customer to re-set the TTLs on evidence rather than on a guess.
...
export class R2RestoreSink implements RestoreSink {
  ...
  async getStreamForVerify(name: string): Promise<ReadableStream<Uint8Array>> {
    ...
    // These two cases are not the same fault, so they are kept apart rather than coarsened into ONE reason.
    // An ABSENT object means the bucket accepted the write and does not have it (the write was eaten); a
    // present object with NO
...
/**
 * unwiredSecretSinks counts the bound secrets that have NO runtime write path. Secrets Store bindings are
 * read-only at runtime, so a secret whose operator never wired a `put` CANNOT be restored in-account, and without
 * this check that is discovered only DURING the disaster, on the restore receipt, when it is far too late to do
 * anything about it.
 *
...
export class D1RestoreSink implements RestoreSink {
  ...
  // How many non-table schema objects (indexes / triggers / views) a table-SUBSET restore FILTERED OUT.
  ...
  /** schemaFilteredCount is the standing count of schema objects this subset restore dropped. Read by the
  ...
  private async applySchema(schema: string[]): Promise<void> {
    ...
    // A table-SUBSET restore silently DROPS the indexes and triggers of the tables it did not create (and,
    // as the note above records, a view's INSTEAD OF trigger with them), so an app can break after a restore
    // that reported complete success. The COUNT (schemaObjectsFiltered) makes this observable, and it carries
    // no DDL text and no name.
  ...
  private async requireFreshTarget(): Promise<void> {
    ...
    if (userTables.length > 0) {
      // A bare "restore keeps refusing 'target not empty'" gives support NOTHING to act on -- not even how many
      // tables are in the way. The refusal carries the RESIDUAL TABLE COUNT (a clamped int), so the operator
      // can be told what they are about to drop. Table NAMES are the customer's own schema labels
  ...
  private async runBatches(stmts: D1PreparedStatement[]): Promise<void> {
    ...
    for (let i = 0; i < stmts.length; i += D1_INSERT_BATCH) {
      ...
      try {
        ...
        // A D1 restore is applied over several NON-ATOMIC batches, so a mid-apply fault leaves a
        // PARTIALLY-LOADED database, and a bare "partial restore" says nothing about WHERE it stopped or
...
function r2PutOptions(opts?: RestorePutOptions): R2PutOptions | undefined {
  ...
  if (r2.httpMetadata && Object.keys(r2.httpMetadata).length > 0) {
    ...
    if (typeof h.cacheExpiry === "string") {
      ...
      // The stored cacheExpiry did not parse, so the object is restored WITHOUT it and behaves
      // differently at the edge; without a count this would sit silently behind a receipt that claims the
      // record restored. Count the shed (the closed field kind only; the unparseable value is never carried).
