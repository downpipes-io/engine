...
// This module is that single classifier, consumed by withRetry, the RUNLOG read-modify-write, the buffered
// seal loop, and the inline-throttle routing in seal/runstate.ts. A throttle/transient fault is RETRYABLE; an auth/permanent
...
//
// Reading the status first matters because CfApiError's message is built as the joined Cloudflare error text
// `|| \`HTTP ${status}\`` (cf-config-core.ts callEnvelope), so the status reaches the message ONLY when
// Cloudflare returned no error text at all. Whenever Cloudflare answers with prose, which is the common case
// for a 4xx, the status is absent from the message, so classifying on the message alone falls to NETWORK_RE
// over Cloudflare's own wording, then to the fail-loud default -- both wrong, and both reachable through
// withRetry: a 503 whose prose reads "Service temporarily unavailable" matches nothing and is NOT retried, so
// a Cloudflare blip fails a backup that should ride it out; a 400 whose prose happens to contain "connection"
// or "network" matches the network arm and IS retried, six attempts deep, per surface.
//
 ...
 * branch decided it; keeping the two as one decision order, rather than two copies held in step by hand,
 * means an arm like the Object-Lock override below is added once and never risks landing in one copy and
 * not the other.
 */
 ...
 * destination-fault diagnosis: "permanent" alone cannot be trusted, because the classifier's DEFAULT
...
/**
 * DEST_DOWN_REASONS is the CLOSED vocabulary for "why is this destination down". The replication
