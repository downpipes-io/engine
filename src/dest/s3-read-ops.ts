 ...
 * idempotent delete) and the signing credentials. get/exists/delete/list/objectLockStatus are free
 * functions over this seam, keeping s3.ts small: a caller of these ops never needs the whole destination
 * class.
 *
 * Implemented by S3Destination itself, and by the in-memory doubles the destination tests build.
 */
export interface ReadIO {
  ...
  // faults is the destination's bounded, closed-vocabulary fault ring. Every read/probe op notes a
  ...
  // io is the destination's bounded DEGRADATION counter set. exists() notes into it when a HEAD
...
// cap regardless of what Content-Length claims, so a misconfigured or hostile store cannot exhaust
...
export async function get(io: ReadIO, key: string): Promise<GetResult | null> {
  ...
  if (resp.status >= 300 && resp.status < 400) {
    ...
    // A 301/307 from an S3 endpoint almost always means the bucket lives in ANOTHER REGION, and the
  ...
  if (!resp.ok) {
    ...
    // Read the (already-failed) error body ONCE -- bounded, and used ONLY to extract the closed <Code> token,
    // never surfaced -- and throw a DISTINCT `thaw-needed` fault so the verify-at-seal coarse mapper (and the
    // restore path) render a legible "object is in a cold storage class" reason instead of an indistinguishable
    // access outage. This never weakens the read: a genuine 403/5xx without that Code still throws the generic
    // status error.
    const errBody = await readS3ErrorBody(resp);
    // Note the fault identity BEFORE the throw: the op, the status, the closed <Code> and the
  ...
  if (!etag) {
    io.faults.noteAnomaly("no-etag-on-get"); // a 200 GET with no ETag -- the store cannot satisfy the conditional-write contract
  ...
  // a misconfigured or hostile store cannot exhaust isolate memory either way.
...
export async function exists(io: ReadIO, key: string): Promise<boolean> {
  ...
  // THE dedup-defeating fault: a 403 (the credential lost s3:HeadObject) or a 5xx (the store is
...
export async function headStatus(io: ReadIO, key: string): Promise<number> {
  ...
  if (resp.status >= 300 && resp.status < 400) {
    ...
    io.faults.noteRedirect(resp); // the region hint / unrecognised-target class (never the Location URL)
  ...
  // noted in the fault ring. A HEAD carries no body, so there is no <Code> to read.
...
export async function del(io: ReadIO, key: string): Promise<void> {
  ...
  if (resp.status !== 204 && resp.status !== 404 && !resp.ok) {
    // Note the identity before throwing: a delete refused by an Object-Lock retention window
    // (a locked bucket the prune can never reclaim) reads completely differently from a 403 policy denial,
    // though the thrown message text alone is the same shape for both ("DELETE key: status NNN").
    io.faults.noteResponse("delete", resp, await readS3ErrorBody(resp));
    // TAGGED with the status: the message stays byte-identical, so every catch and log line matching on
    // the message text keeps working, while a caller that reads the status can tell a permission denial
    // (401/403) from any other refusal.
 ...
 * sync) walks page by page rather than accumulating the whole store like list() above. list() is
 * the accumulating loop over this: every page is built, signed and classified the same way
 * regardless of which caller drives it, so only the result shape differs between the two.
 *
...
export async function listPage(io: ReadIO, prefix: string, cursor?: string, pageOrdinal?: number): Promise<ListPage> {
  ...
  // Bounded regardless of Content-Length: every store pages a listing at 1000 keys, so a
  ...
  // If the store says the listing IS truncated yet gives NO continuation token, the walk
  // below STOPS and every caller believes it saw the whole keyspace -- so a replication seg/ sync copies
...
export async function objectLockStatus(io: ReadIO): Promise<WormStatus> {
  try {
    ...
    // the reason is lost. Note the identity in the fault ring first so a WORM claim that could not be
    ...
    if (resp.status >= 300 && resp.status < 400) {
      ...
      io.faults.noteRedirect(resp);
      // A credentialed probe that was REDIRECTED (a proxy interposing, or a bucket in another region).
    ...
    if (!resp.ok) {
      ...
      // The store ANSWERED and refused. A 401/403 is the one-line IAM fix (the credential lacks
    ...
    // Bounded regardless of Content-Length: GetObjectLockConfiguration's response is always a
    ...
    // Naming the cause as "network" rather than "denied" matters: a transport fault is self-healing,
    // while a permission denial needs an IAM fix, and conflating them would send a compliance customer
    // chasing an IAM change for a passing blip.
 ...
 * member that names the remedy. PURE; the status is an integer, and nothing else about the response is
