 ...
 * in cleartext. The check is on the scheme and host only, parsed once, so a typo fails loud
 ...
 * endpoint-not-https.
 *
 * The loopback allowance is a test seam, not a configuration: http://localhost, http://127.0.0.1
 * and http://[::1] are refused like any other http endpoint unless the caller explicitly passes
 * opts.allowInternalEndpoint. No production caller passes it: dest/factory.ts builds every env and
 * console destination without it, and cron/siem-push-pass.ts forwards it only from the delivery
 * opts a validator supplies. The only callers that want the allowance are the validators driving a
 * node:http emulator on 127.0.0.1 (test/validate-destination-format-coverage.ts's S3 drop emulator,
 * and this file's own contract validator). In a deployed configuration the exemption is
 * unreachable, which is the point.
 *
 * All three throws below are TYPED (DestBuildError) rather than plain Errors, because
 * destBuildFaultOf classifies on the TYPE and never on the message text (deliberately: the message
 * carries the endpoint and the bucket, which vary per call, so it cannot double as a stable
 * classifier). varName is deliberately NOT set: the same helper serves the env destination
 * (DEST_ENDPOINT) and the console-set one (the stored `endpoint` field), and naming the wrong one
 * would be a false fact.
 */
export function requireHttpsEndpoint(endpoint: string, opts?: { allowInternalEndpoint?: boolean }): void {
  let u: URL;
  try {
    u = new URL(endpoint);
  } catch {
  ...
  // The loopback hosts are the literal spellings only (localhost, 127.0.0.1, [::1]), so 127.0.0.2 or a name
  // that merely resolves to loopback is still refused, and even these three are refused unless the caller
  // asked for the seam.
  const isLocalLoopback = u.protocol === "http:" && (u.hostname === "localhost" || u.hostname === "127.0.0.1" || u.hostname === "[::1]");
  if (isLocalLoopback && opts?.allowInternalEndpoint === true) return;
