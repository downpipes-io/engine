// fault-log.ts -- the bounded, closed-vocabulary record of WHY a destination op failed.
//
...
/**
 * DEST_ANOMALIES: the destination behaviours that are NOT a failed op with a
 * status -- so the fault ring above cannot hold them -- and that would otherwise either die as a coarse
 * run-error class or, worse, go SILENTLY TOLERATED. Each member names the BEHAVIOUR, never the key, page, header or size:
 *
 *   list-truncated-no-cursor      (DATA LOSS) a ListObjectsV2 page said IsTruncated=true and gave NO
 ...
 *   no-upload-id                  a multipart-initiate 200 whose body carried no <UploadId> (a
 *                                 namespaced-XML or stripping proxy): every large-segment write fails.
 *   no-etag-on-part               a multipart UploadPart 200 with no ETag header.
 *   no-etag-on-get                a GET 200 with no ETag: the store cannot satisfy the conditional-write
 *                                 contract the RUNLOG depends on.
 *   no-etag-on-conditional-put    a conditional PUT 200 with no ETag. This one is SILENTLY TOLERATED
 *                                 (the caller falls back to a body compare), so RUNLOG concurrency
 *                                 degrades with nothing recorded anywhere.
 *   error-in-200-body             the store answered 2xx and embedded an <Error> document.
 *   segment-over-limit            the engine's own segment ceiling refused a write. overBoundBytes
 ...
 *                                 generic destination throw and "did a stored destination aim the archive at
 *                                 the metadata IP?" has no answer in the pack. The HOST never rides: it is
 *                                 caller-chosen, so only the class and its count are kept.
 *   redirect-wrong-region         a credentialed request was answered with a 3xx whose Location names an
 ...
 *   redirect-target-unrecognised  a 3xx whose Location is NOT an AWS regional endpoint (a corporate proxy,
 ...
 * to the engine-guard members (the two size guards, the key-shape guard and the wire-time SSRF screen), so
 * only those can be noted with a byte magnitude alongside the count.
 */
 ...
 * S3 endpoint, and RETURNS that token (or undefined when the target is not one). This is the redaction
...
/** How many multipart aborts FAILED on this destination instance, and the class of the most recent. */
...
export interface DestFaultSnapshot {
  ...
  /** {closed anomaly -> count}. Absent when the store behaved conformantly. */
  anomalies?: Record<string, number>;
  /** The region token of the redirect target, when it named an AWS regional endpoint. Never a URL. */
  redirectRegionHint?: string;
  /** The CLAMPED size that tripped an engine guard (segment-over-limit / sealed-bound-exceeded). */
  overBoundBytes?: number;
  /** How many list pages the walk reached before the store truncated it without a cursor. */
  listPagesReached?: number;
  /** The multipart aborts that THEMSELVES failed, leaving stranded part-storage the customer is billed
...
export class DestFaultLog {
  ...
  // The anomaly counters: a bounded {closed member -> count} tally plus the three
  ...
  private overBytes = 0;
  private listPages = 0;
  // The failed-abort tally + the class of the most recent one. Two integers and a closed enum.
  private abortFailures = 0;
  private lastAbortClass: AbortFailureClass | undefined;

  /**
   * Notes a multipart abort that ITSELF failed, leaving stranded part storage. Counts only: the object
  ...
  /**
   * Notes a credentialed request the store answered with a redirect. The Location is READ only to
  ...
  /**
   * Notes an engine GUARD refusal with its MAGNITUDE: the size that tripped a ceiling, so support can
   ...
   */
  noteGuardRefusal(a: Extract<DestAnomaly, "segment-over-limit" | "sealed-bound-exceeded" | "key-shape-refused" | "endpoint-internal-refused">, bytes?: number): void {
    this.noteAnomaly(a);
  ...
  /**
   * Notes a listing the store TRUNCATED WITHOUT A CURSOR: the walk stops believing it enumerated the
