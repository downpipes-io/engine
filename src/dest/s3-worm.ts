...
export function parseObjectLockConfig(xml: string): WormStatus {
  // A 200 whose body is NOT an ObjectLockConfiguration is a store (or, far more often, a PROXY) mangling
  // or stripping the document -- a completely different remedy from a permission denial or a network blip, so
  // this names the cause rather than collapsing it into a bare "unknown". Never carry a byte of the body.
