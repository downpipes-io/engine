...
// protocol, its own authentication scheme, its own object model and its own listing document. Pointing the
// S3-compatible destination at a real Azure endpoint answers 403 AuthenticationFailed on the first call,
// and no configuration makes that work.
//
...
//
//   (IMMUTABILITY is not listed here: Azure's two primitives map cleanly onto one mode plus one window.
//   Governance is an UNLOCKED policy and compliance is a LOCKED one, and the mode is a header chosen per
//   write rather than something inferred. See dest/azure-worm.ts for the correspondence.)
//
...
export class AzureBlobDestination implements Destination {
  ...
  // pacer is the OPTIONAL adaptive destination throttle, on the same terms the S3 client holds one:
  ...
  // io is the bounded DEGRADATION counter set, the same one S3Destination and R2Destination hold. It
  ...
  private async send(method: string, path: string, query: Record<string, string> | undefined, headers: Record<string, string>, body?: BodyInit, contentLength?: number): Promise<Response> {
    ...
    // Pace BELOW the store's tolerance, at the same point in the sequence the S3 client does it: the
    ...
    try {
      ...
      // and rethrow a NAMED error: a bare AbortError names neither the bound nor the request, which would
      // make a timed-out Azure write and a cancelled one read identically in a log.
      if (timedOut) {
  ...
  async putConditional(key: string, body: Uint8Array, opts: { ifMatch?: string; ifNoneMatch?: string }): Promise<PutConditionalResult> {
    ...
    // same thing to the caller. Neither is a fault, and BOTH are counted as a degradation: one
  ...
  async exists(key: string): Promise<boolean> {
    ...
    // THE dedup-defeating fault, and it reaches Azure by the same route it reaches S3: a 403 (the
   ...
   *
   * IT DOES NOT HARDCODE `{ enabled: false }`. Hardcoding it would be wrong in the worst possible direction
   * now that the write path carries Azure's policy headers: the question WormStatus asks is whether this
   * container enforces the policies written to it, and a hardcoded false would report "no immutability
   * here" about containers that are enforcing it, on the very destinations a compliance customer bought it
   * for.
   *
  ...
  // destIo is the bounded DEGRADATION snapshot of this instance, implementing the optional
