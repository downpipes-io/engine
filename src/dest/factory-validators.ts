 ...
 * immutability policy is the one setting where a silent drop is worse than a refusal. A rejected policy used
 * to leave the destination stored, verified and answered 200 with NO Object-Lock at all: the operator asked
 * for compliance-mode WORM, was told the destination was saved, and had a bucket with no lock on it. Nothing
 * downstream could tell that apart from a destination nobody ever asked to lock.
 *
