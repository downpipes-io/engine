// AWS STS AssumeRole mints SHORT-LIVED credentials so a destination need not store a long-lived
...
function parseAssumeRoleResponse(xml: string): TempCredentials {
  ...
  if (!accessKeyId || !secretAccessKey || !sessionToken) {
    // A 200 whose document carried no credentials (a proxy or appliance in the path). Typed with the
...
export async function assumeRole(params: AssumeRoleParams, principal: { accessKeyId: string; secretAccessKey: string }): Promise<TempCredentials> {
  ...
  // Validate the region BEFORE it is ever interpolated into a host.
  if (!isValidStsRegion(region)) {
    // The "auto" region (the R2 convention) under an STS policy. Typed so the standing record names the
  ...
  try {
    resp = await fetch(u, {
      ...
      body: ab(body),
      // Never follow a 3xx and re-send the signed AssumeRole request to a redirect target.
      redirect: "manual",
    });
  } catch (e) {
    // The request never got an answer (DNS, a blocked egress, an intercepting appliance). Recorded as the
  ...
  // A non-ok response is reported by STATUS ONLY; the body can carry an error document, but never
  ...
  if (!resp.ok) {
    // The AssumeRole refusal body is deliberately never read as-is (it can carry a credential on other
    // paths), so an AccessDenied on the role's TRUST POLICY, an ExpiredToken PRINCIPAL, a mismatched
    // ExternalId and a DELETED role would all be the same "status 403" to support without this. Read the
    // (already-failed, bounded) ERROR document ONCE and use it ONLY to SELECT a member of the closed STS_FAILURE_CLASSES
