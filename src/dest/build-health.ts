// DESTINATION BUILD HEALTH: a destination that cannot be constructed is not a standing state until this
// tracks it, and an STS AssumeRole failure cause is otherwise a black box.
//
...
/**
 * STS_FAILURE_CLASSES is the CLOSED vocabulary for WHY an AssumeRole was refused. Each names a different
...
/**
 * classifyStsErrorBody reduces an STS ERROR DOCUMENT to a CLOSED StsFailureClass. It is the redaction
...
/**
 * DestBuildHealth is the STANDING record. The field a coarse run error can never carry is failingSinceAt:
...
export interface DestBuildHealth {
  ...
  readonly stsClasses: Record<string, number>; // closed STS class -> count
}
