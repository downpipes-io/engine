...
export class R2Destination implements Destination {
  ...
  // io is the bounded DEGRADATION counter set. The native binding has no pacer, no fetch bound and no
  ...
  // faults is the bounded closed-vocabulary evidence ring. The native binding never speaks HTTP,
  // so it records no per-op status faults; what it CAN record is the engine's own guard refusals, which
  ...
  // ifMatch is never silently dropped from onlyIf. An empty-string etagMatches is
  ...
  async putConditional(key: string, body: Uint8Array, opts: { ifMatch?: string; ifNoneMatch?: string }): Promise<PutConditionalResult> {
    ...
    // degradation: the SAME signal the S3 driver's 412 carries, so a RUNLOG being fought over by two
  ...
  // destIo is the bounded DEGRADATION snapshot of this instance. Implements the optional
  // Destination.destIo accessor; clamped integers only.
  destIo(): DestIoSnapshot {
    return this.io.snapshot();
  }

  // destFaults is the bounded fault/anomaly snapshot on this driver: the engine guard refusals this
  ...
  async putStream(key: string, body: ReadableStream<Uint8Array>, size?: number): Promise<void> {
    if (size !== undefined && size > MAX_STREAM_SEGMENT_BYTES) {
      // The engine's own segment ceiling refused the write. Closed class + the CLAMPED size; never the key.
  ...
  // sync uses it to walk the content-addressed store page by page. One .list = one metered
  ...
  //
  // THE S3 ENDPOINT IS NOT A WAY ROUND IT, AND THIS COMMENT USED TO SAY IT WAS. It read "R2 DOES
  // support S3 Object-Lock through its S3-compatible API", and four operator-facing surfaces published
  // "reach the bucket by its S3 endpoint" as the remedy on the strength of it. types.ts said the
  // opposite 58 lines from its own WORM_UNKNOWN_REASONS list, and types.ts was right.
  //
  ...
  async objectLockStatus(): Promise<WormStatus> {
    // This "unknown" is STRUCTURAL, not a fault -- the native R2Bucket binding exposes no Object-Lock
    // read API, so this destination can never confirm enforcement no matter how the credential is scoped. It
    // must not be indistinguishable from a denied probe or a network blip, which would send operators chasing
    // an IAM fix that cannot exist. The reason names the limit; it names no remedy beside it, because the
    // S3-endpoint route is refused 501 by R2 (measured above) and would break every write.
