...
// requireConfig is the required-env helper. This module keeps the entry module (src/index.ts) a thin
// handler and imports nothing from it, so there is no cycle.
...
// RETENTION EVIDENCE. The retention pass is the data-lifecycle subsystem whose evidence answers "storage
// keeps growing despite retention.enforce=true" and "retention deleted my run X": every skip, deferral and
// applied prune builds ONE bounded, redaction-safe record per tick, posted to the scheduler DO
// (recordRetentionPass). The record's vocabulary, shape, classifiers and sanitiser live in the LEAF module
// cron/retention-record.ts so the DO and the pack can share them without dragging this pass's import graph
// (which reaches the seal, dest and admin layers) into the DO base and closing a cycle. doURL comes from the
// do-url.ts leaf rather than admin/router.ts, which would close a cycle of exactly this kind.
...
// alters the caller's path. A dropped record is not silent: the loss is counted as "retention-record" in the
// droppedWrites aggregate and lands when the DO recovers, rather than leaving the pack showing the previous
// pass as the latest one. An engine whose DO predates the slot answers not-found, which recordDiagWrite
// counts as a drop: that is the honest reading, since the pack's retention section is stale on such an
// engine either way.
...
//
// 3-2-1 durability: the prune is REPLICA-COVERAGE-AWARE. The same-tick "replicate before prune"
...
//
// PAUSE MEANS PAUSE. A PAUSED downpipe is not pruned: the selection below tests both `retention !==
// undefined` AND `config.enabled`, because a downpipe whose run trees kept being deleted on the cron's own
// cadence while paused would have its LAST remaining backup become eligible the moment its window elapsed --
// the most expensive reading of the switch. `enabled: false` is called "Paused" on every console surface an
// operator reads, which is the customer-facing word for "stop doing things to my data", and the documented
// account-exit tells a departing customer to pause every downpipe and then stop watching. The two controls
// must not invert in danger: DELETING a downpipe takes it out of GET /downpipes so it is never selected here
// again, while a PAUSED downpipe stays selected unless this pass excludes it explicitly, so pausing must be
// at least as safe as deleting.
//
...
export async function runRetentionPrunes(env: Env, scheduler: DurableObjectStub): Promise<void> {
  ...
  // The pass evidence record: filled as the pass walks, posted at whichever exit it takes.
  ...
  // per-destination fold can attribute the stand-down to the bucket it would have pruned.
  ...
  // 3-2-1 durability: read the per-destination replication state ONCE for the whole pass so the
  ...
  try {
    ...
    // The pass evidence record: posted AFTER every destination has reported, so one bounded
...
// (primaryDestinationId). Keying off the bare legacy destinationId here was a bug: a
// console-created downpipe that pins a NON-DEFAULT primary via destinationIds[] (with no legacy
// destinationId) was bucketed under "" (the default RUNLOG), where its runs never appear, so its
// over-cap runs were never pruned. Resolving the primary the same way the seal path does puts every
// downpipe in the bucket whose RUNLOG actually holds its runs. A legacy single-destinationId downpipe
// resolves to the SAME key as before (primaryDestinationId returns destinationId when there is no
// list), so its behaviour is unchanged. Exported so the retention validator can assert the key
// without standing up a DO. Replicas are not bucketed here: retention prunes the primary archive's
// RUNLOG, and a replica is an additional copy of finalised runs, not a separate retention scope.
...
// /replication => { byDownpipe }), the holdsIndex map the prune gate reads. Fail-open: an
...
// The `unreadable` boolean rides the pass evidence record: a pass that gated on NO coverage proof
...
// THE SIBLING OF fetchReplication'S OWN FAULT CHECK, and it needs the same r.ok. SchedulerDO.fetch
...
// then ASSERTS the replication state was read and was empty, when it was never read at all. Support
...
// replicaCoverageFor computes the ReplicaCoverage for one downpipe from its CONFIGURED replica
...
// so a run below maxReplicaHoldsFrom is treated as uncovered and RETAINED. A replica with no recorded
// floor contributes +Infinity (proves nothing below its
...
async function runRetentionPrunesForDest(env: Env, infra: RetentionPruneInfra, key: string, group: DownpipeState[], now: Date): Promise<void> {
  // Every exit below stamps ONE destination outcome on the pass record: a closed skip code, or no code
  ...
  for (const state of group) {
    try {
      ...
      // record so a half-applied or perpetually-throwing prune is diagnosable from the pack.
      // A PruneApplyError's wormBlocked boolean rides the row: a WORM/Object-Lock delete refusal is
...
interface RetentionPruneInfra {
  ...
  lock: RunlogLock;
  // The per-downpipe per-destination replication state (holdsIndex per destination), read ONCE for
  ...
  // The pass EVIDENCE accumulator: every destination skip and per-downpipe outcome is stamped
  ...
  // per-destination fold can attribute it. Set by the per-destination loop; "" at construction.
  destKey: string;
}
...
async function runOneRetentionPrune(
  ...
  // The replica-coverage gate for THIS downpipe, computed from its configured replicas and their
  ...
  // postPruneObservations writes the PLANNER's fault rows onto the bounded seal-fault ring: the abstain
  ...
  // operator-facing TEXT never rides the record: classifyDeferral coarsens it to a closed class, so a
  ...
  if (retention.enforce !== true) {
    ...
    // often exactly this: enforce was never set. The record says so in counts.
  ...
  // (supersede commits first) but the deletes are half-done, so storage keeps growing with no trace unless the
  // progress counts made so far are recorded rather than thrown away with the exception. Record what the
  // apply had actually committed (plus whether WORM/Object-Lock REFUSED the delete, an irreducible remainder
  // rather than a transient flake) and then RE-THROW unchanged, so the fail-open retry behaviour above is
  // unaffected.
...
// auditWriteFailures so an applied-but-unaudited prune is not invisible.
...
export async function emitVolumeRegressionAlerts(env: Env, scheduler: DurableObjectStub, regressed: VolumeRegressionAlert[]): Promise<void> {
  try {
    ...
    for (const dpId of newlyRegressed) {
      ...
      // The DO's reconcile above ALREADY LATCHED this downpipe as alerted (that is what makes the alert
      // edge-triggered: it re-arms only when the volume recovers), so a routing failure here would lose the
      // warning for the WHOLE regression episode if the delivered boolean were discarded. Capture and latch
      // it, so the pack can say the page was consumed and never delivered instead of showing an unexplained
      // silence.
...
// DELIBERATELY returns NO sizeOf lookup, so planPrune reports reclaimableBytes
...
// production path supplies no body-reading sizeOf.
