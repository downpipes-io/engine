...
// classifyOtlpConfigFault coarsens a config-step throw into a closed PushPassFailCode, the OTLP twin of
// classifyPushConfigFault. The message, the DO status and the unwrap error text never
// leave this function.
...
export interface OtlpPushDeliveryResult {
  ...
  truncated: boolean;
  // droppedCount sizes the truncation: how many downpipes the OTLP_PUSH_DOWNPIPE_CAP left out of this push,
  // so support can answer the question the customer asks -- "how many of our downpipes are missing from the
  // dashboards?" -- once the fleet outgrows the cap. 0 when the whole (post-cap) snapshot fits.
  droppedCount: number;
  ...
  // a silent full success. Absent when the collector accepted everything.
...
async function recordOutcome(
  ...
  try {
    ...
    // The twin of the SIEM trail write: if the response were not checked, a lost trail row would read as a
    // recorded one and the metrics feed's trail would go quietly stale while the pass reports success.
...
// recordOtlpPassFault records a PRE-DELIVERY fault on the OTLP push trail, the twin of
// cron/siem-push-pass.ts's recordPassFault: without it, a malformed wrap key, an unreadable DO config or
// an unreadable metrics snapshot would take an early `return false` that writes NOTHING, so "our
// Grafana/collector stopped receiving metrics" would arrive with an empty trail. The DO drops an outcome
// whose gen does not match the live config, so a fault raised before the config resolved re-reads the raw
// record for its gen alone (an absent record means OTLP push is not configured: nothing to record; an
// unreachable DO means the trail itself is unwritable). `reason` is a closed PushPassFailCode, never a
// message, status line or endpoint. `cause` is the RAW throw behind the closed reason. It is reduced to a
// 12-hex one-way digest here and nowhere else, so the trail row can be joined to the customer's own
// Workers Logs line for the same fault.
  ...
  try {
    ...
    for (const chunk of chunks) {
      ...
      await recordOutcome(scheduler, {
        ...
        // Size the truncation. The boolean says "you are missing downpipes"; this says how many.
