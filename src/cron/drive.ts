...
// guarded so a fault degrades to "skip this pass this tick", never a crashed cron invocation. This
// module imports the pass helpers from the cron/* siblings (not from index.ts), so there is no cycle.
// drive() itself is a thin orchestrator over the named pass sub-functions, each implemented in its own
// cron/* pass sibling (reconcileAndDue + runSealLoop in seal-loop-pass.ts; the alert / replication-alert
// / source-drift / expiry passes in alert-passes.ts; runRestoreTestPass in restore-test-pass.ts;
// runDiscoveryPass in discovery-pass.ts); the retention/canary/replication/digest/update passes delegate
// to their own cron/* helpers.

import { CRON_CADENCE_MS } from "./cron-cadence.ts";
...
import { runHistoryRetentionPurge } from "./history-retention-pass.ts";
...
// The cron cadence is passed to the DO so it can derive the tick GAP: how many ticks are MISSING
// between the previous recorded tick and this one. Without it, a hole in the tick ring (a DO-unreachable
// tick, which records nothing at all) is indistinguishable from a cron that never fired.
//
// The cadence is imported from cron-cadence.ts, the single source pinned to the wrangler literal, rather
// than re-stated locally, because a hand-copy that outlives a cron retune would make this arithmetic
// report ticks as missing that were never scheduled.

// drive is the cron entry the scheduled() handler calls. It takes a DO-level SINGLE-FLIGHT tick lease
...
export async function drive(env: Env): Promise<void> {
  ...
  // Acquire the single-flight tick lease before doing any work. Only an EXPLICIT acquired:false (a tick
...
// runScheduledTick is the body of one cron invocation, run under drive()'s single-flight tick lease.
...
// digest flush and the new-version alert). Replication runs BEFORE retention so a trailing replica
// catches up on its backlog before the primary prunes the source data. Each pass is a guarded sub-function so
// a fault degrades to "skip this pass this tick", never a crashed cron invocation. It is kept separate from
// drive() so the tick lease can wrap it in one place.
async function runScheduledTick(env: Env, scheduler: ReturnType<typeof schedulerStub>): Promise<void> {
  ...
  if (due === null) {
    // The scheduler DO was unreachable at the /tick or /due preamble, so this invocation does NOTHING
  ...
  // ONE budget for the whole cron invocation: every due downpipe's inline
  ...
  // Accumulate the per-tick OUTCOME. passErrors counts every pass that
  ...
  // the outcome to a CLOSED pass name, so "no alerts / no restore tests / no exports for days"
  ...
  // retention prune below: retention prunes the PRIMARY (it deletes a superseded run-tree + its
  ...
  // Retention prune: classification-driven retention with automatic deletion. For each
  ...
  // replication so a trailing replica catches up on its backlog before the primary prunes the
  ...
  // Run-history retention purge. The per-downpipe run-history ring is
  // already bounded by COUNT (RING_CAP), but nothing bounded it by AGE, so a low-traffic downpipe could
  // keep a run's metadata for as long as the account exists. This purges entries older than
  // RUN_HISTORY_MAX_AGE_MS (sched/history-retention.ts) from every downpipe's ring in one DO round trip.
  // It is independent of the archive-retention pass above (a different asset class entirely: operational
  // run metadata in the DO, not customer backup bytes in the destination), so it runs alongside it rather
  // than gated on it. Guarded like every other pass: a DO hiccup degrades to "no purge this tick".
  await guarded("history-retention", "run-history retention purge", () => runHistoryRetentionPurge(scheduler));

  // Orphan reconcile DRY-RUN INVENTORY (report-only). For every destination bucket
  ...
  await guarded("canary", "canary flight", async () => {
    // runCanaryIfDue returns the one-shot rollbackNeeded claim (for the on-demand canary-tick route); the
    // cron ignores it here, so discard it to keep the guarded() Promise<void> contract.
  ...
  // Control-plane recovery. The HEALTH pass runs first (cheap: one DO read; it only probes a
  ...
  // recorded even on an unattended account, answering whether a deploy silently dropped the account's source
  // bindings. Cheap + fail-open.
  ...
  // Notification digest flush (the daily/weekly success-stream summary). This
  ...
  // New-version alert: pull + verify the signed update channel and, when it advertises a NEWER
  ...
  // Record the per-tick OUTCOME LAST: the DO appends a clamped, redaction-safe
  ...
  try {
    ...
    // The tick-outcome POST itself failed, so this tick leaves a HOLE in the ring. Note it in the
  ...
  // The invocation's HEALTH evidence, posted LAST: which passes ran and
  ...
  await recordCronHealth(scheduler, CRON_CADENCE_MS);
}
