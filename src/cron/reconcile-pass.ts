...
// DELETES NOTHING and SALVAGES NOTHING - the enforced GC + salvage ride the sharded-RUNLOG keystone and
// are a separate, multi-day project not yet built.
//
// ADDITIVE + REVERSIBLE: the whole pass is gated behind the ORPHAN_RECONCILE env flag (DEFAULT OFF), so
// deploying this code changes NOTHING - no extra reads, no log lines - until an operator opts in. Turning
// it on yields the dry-run inventory a rollout reviews first. Rollback = unset the flag (or
// stop calling runOrphanReconcile). It mirrors the retention prune pass's per-destination infra
// (loadSigner -> verifier, zeroised on every exit; buildDestination per bucket; fetchDestConfig), and is
...
export async function runOrphanReconcile(env: Env, scheduler: DurableObjectStub): Promise<void> {
  ...
  // The pass is OPT-IN, so an empty reconcile section in the pack is AMBIGUOUS -- "the operator never
  ...
  try {
    ...
    // Every bail below leaves the LAST persisted signal standing, and a months-old signal is
  ...
  try {
    ...
    // No SIGNER_PRIVATE means the pass can never verify a RUNLOG, so it bails EVERY tick -- silently, and
  ...
  try {
    ...
    for (const destKey of destKeys) {
      try {
        ...
        // THIS destination's inventory (and therefore its signal) is now stale, while the others may be
...
async function inventoryOneDestination(
  ...
  try {
    ...
    if (destKey && !destCfg) {
      noteReconcileSkip("dest-not-configured"); // a downpipe pins a destination whose config record is gone
    ...
    noteReconcileSkip("dest-build-failed"); // the destination could not be BUILT (credential, endpoint, wrap key)
  ...
  if (!sigObj || !(await verifyRunlogSig(runlog.body, sigObj.body, verifier))) {
    // These two are split: a .sig object that is ABSENT (an operator deleted it, or a bucket lifecycle rule
    // expired it) needs a re-sign; a signature that FAILED verification is a signer rotation or TAMPER, and the
    // operator must not proceed. The pass still abstains and still deletes nothing either way -- only the
  ...
  // with the topology change, or a wrong-source read mass-mis-classifies live committed runs.
...
async function persistReconcileSignal(scheduler: DurableObjectStub, signal: ReconcileSignal): Promise<void> {
  try {
    ...
    // Leaving the response unread would let a REFUSED persist pass for a written signal -- and the pack would
    // then show the PREVIOUS signal as if this tick had confirmed it.
...
function emitInventory(inv: ReconcileInventory): void {
  ...
  if (inv.deferred !== undefined) {
    ...
    // The circuit-breaker abstain reads as free text, which can describe the customer's topology, so it can
    // never ride in a pack. Record the BOOLEAN instead -- the fact the breaker tripped is the diagnosis; the
    // sentence is not.
