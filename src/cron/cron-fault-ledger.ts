// THE CRON FAULT LEDGER -- the cron subsystem's closed-vocabulary evidence sink.
//
// WHY IT EXISTS. The cron invocation is the engine's heartbeat: it dispatches every backup, probes every
// destination, refreshes cf-config discovery, pushes the SIEM feed, drives the auto-heal and emits the
// beacon. Every one of those passes is guarded so a fault degrades to "skip this pass this tick" rather than
// crashing the invocation. Left unrecorded, that guard would make faults invisible: a bare `passErrors`
// integer per tick cannot say WHICH of ~20 passes failed or why; a tick ring can go silent with no record at
// all when the scheduler DO is unreachable, so a flapping DO and a cron that never fired would read
// identically; a destination probe whose catch drops the underlying error before any sink cannot separate an
// expired STS credential from a DNS fault from a deleted bucket, all of which would otherwise read as "all
// destinations unreachable"; a discovery cache can stop refreshing with nothing to show it; an auto-heal can
// latch forever with no recorded deferral; a beacon can fail with no class; and, the data-loss-grade case, a
// SIEM push can substitute a timestamp, truncate a field, cap a batch or reset the cursor to zero with no
// counter anywhere. This ledger gives each of those a closed, bounded, no-custody record instead.
//
...
//
// THE OUTAGE THAT CANNOT REPORT ITSELF. When the scheduler DO is unreachable, drive() bails BEFORE it can
// record anything -- the sink for the record is the thing that is down. So the tick-record failure is
...
// CRON_PASS_NAMES is the CLOSED set of passes the cron invocation drives. Naming each pass here is what lets
// a failure be attributed to it: elsewhere a pass that bails in its own guard bumps only one anonymous
// passErrors integer, and three passes (the fleet drill, deploy-observe and the health pass, which swallows
// its own throw) never move it at all -- so without this list, days with no alerts, no restore tests or no
// exports could not be attributed to a pass, let alone a cause.
export const CRON_PASS_NAMES = [
  ...
  "history-retention", // the run-history ring's age-based purge
...
// stale cache this tick. Without it, each of these reasons is either swallowed or folded into an anonymous
// passErrors bump, while the capture path fail-safes to ALL every surface in the registry every run when it
// cannot tell what is stale -- slow and expensive, with no recorded cause.
...
// BEACON_FAIL_CLASSES is the CLOSED vocabulary for a failed beacon emission. Without it, the throw path
...
// swallowed or Workers-Logs-only. Backups that stop after a wipe and never resume take this shape.
...
// RESUME_APPLY_REASON_CODES is the CLOSED projection of the DO's resume-apply refusal reason. The DO
...
// SIEM_SHAPING_FALLBACKS is the CLOSED vocabulary of the SIEM push's SILENT fallbacks. None of these has a
// counter or a marker anywhere else.
...
// ---- notify, updates, control-plane export, push, restore-test and reconcile faults ------------------------
//
...
// /cron-health) and ONE redaction chokepoint (applyCronHealth) rather than one each.
...
// catch. It keys BOTH the undelivered-critical latch and the dropped-emission map.
...
// export. Without it, perDest carries ok:false and NOTHING else, so "one destination's recovery artefact is
...
// RESTORE_TEST_SKIP_CLASSES is the CLOSED vocabulary for a scheduled restore test that did NOT RUN.
// The pack shows lastRestoreTestAt going stale and restoreTestStalled going true; on its own it cannot say
// WHY, and the four causes need four different answers.
//   pre-drill-fault     - the drill threw BEFORE it started (an unresolvable destination, an unusable wrap
//                         key): the whole recording machinery is downstream of the throw, so nothing is
//                         written at all -- a drill that never gets tested for months
...
// RECONCILE_SKIP_REASONS is the CLOSED vocabulary for an orphan-reconcile pass that recorded no signal.
 ...
 * It reads the fault ONLY to select a member and RETURNS that member: the endpoint, the bucket, the
...
// The failover destination probe REUSES the DEST_PROBE_REASONS vocabulary and the bounded
...
// failover WRITE probe in cron/seal-dispatch.ts is simply a caller of that record. Its catch discards the S3
// error before any sink, so without a bridge, "backups failing: all destinations unreachable" could not
// separate an expired STS credential (rotate it) from a deleted bucket (recreate it) from a DNS fault
// (wait). classifyDestProbeReason below is that bridge, and the probe rows ride the existing
...
//
// This is a FIFO admission bound, not an LRU one: the guard below refuses a destination the map has never
// seen, so what it keeps is the FIRST n probed in the invocation, not the most recent, and nothing here
// evicts anything. The bound matches the product's own per-destination cap in the export health record and
// the DO map, rather than an assumption about fleet size -- nothing in the product bounds how many
// destinations an estate may hold. The refusal itself is COUNTED and posted, because a destination refused
// here never reaches the DO at all: it is invisible to every cut downstream, and a failing destination
// missing from this record would otherwise read as a destination that probed fine.
...
/**
 * classifyBeaconFail coarsens a beacon emission fault into a closed BeaconFailClass.
 *
 ...
 * closed code. The DO's reason can embed storage paths, so this is the boundary at which the text is
...
/**
 * classifyDestProbeReason coarsens a failed destination WRITE probe into a closed DestProbeReason --
...
export interface CronHealthDelta {
  ...
  // down). Carried across ticks in the isolate-local tally and reported by the next HEALTHY tick.
  ...
  // healthy run. The customer durably holds FEWER copies than they configured, with zero warning.
  danglingDestinationRefs?: number;
  // notify: the alerts that were CONSUMED and never delivered, and the history rows that were
  ...
  // updates: the update channel's own verification state, and the confirm-clear that keeps failing.
  updates?: { checked?: boolean; channelVerified?: boolean; verifyFailures?: number; confirmClearFailures?: number };
  // cpExport: the control-plane export pass's TRUNCATION, its per-destination fail classes, its
  ...
  // push: the delivery-trail writes that were themselves DROPPED. A lost trail write on a SUCCESSFUL
  // delivery strands the cursor, so the next tick re-sends the same batch, producing duplicate events downstream.
  push?: { siemTrailWriteFailures?: number; otlpTrailWriteFailures?: number };
  // restoreTest: per-downpipe non-execution (closed class), the whole-pass budget deferral, and the
  // on-demand fleet-drill's own batch health.
  restoreTest?: {
  ...
  // reconcile: the flag echo, the closed skip reason, the RUNLOG signature verdict split, and the
...
/**
 * noteCronPass records ONE pass's outcome. A completed pass resets its consecutive-failure run; a
...
/**
 * noteTickRecordFailure notes that a tick could not record its outcome at all: either the scheduler
...
/** noteDiscoverySkip records why the cf-config discovery pass did not refresh a stale cache. */
...
/**
 * noteBeaconEnv records WHICH of the three beacon env vars are present. Presence booleans only: a
 * half-configured beacon (one var typo'd) is otherwise indistinguishable from a deliberately-off one, so
 * opting in and never seeing the vendor assurance view light up would have nothing behind it. The VALUES
...
/** noteBeaconFail records the closed class of a failed beacon emission. */
...
/** noteBeaconStateWriteFailure records that recordBeaconState swallowed its OWN failure. */
...
/** noteAutoHealAttempt records that the auto-heal pass ran at all this tick. */
export function noteAutoHealAttempt(): void {
  ledger.autoHeal = { ...(ledger.autoHeal ?? {}), attempted: true };
}

/** noteAutoHealDeferral records a STALL (not a refusal): latched, no progress, the reason otherwise swallowed. */
...
/** noteResumeApplyRefusal records the CLOSED projection of the DO's free-text resume-apply reason. */
...
/** noteSiemShapingFallback counts ONE silent SIEM shaping / cursor fallback. */
...
/**
 * noteDestProbeFault records that the failover WRITE probe found ONE destination unwritable, with the
...
/**
 * noteDanglingDestinationRef counts a fan-out destinationId with NO config record: the run is skipped
...
// ---- notify, updates, control-plane export, push, restore-test and reconcile recorders ----------------------
//
 ...
 * latched) and then NOT delivered to a single channel. This is the shape behind nobody being paged when
 * backups stop, a destination dies or a bad update needs rolling back: the claim-then-deliver pattern
 * spends the once-per-event budget BEFORE it checks routeNotification's delivered boolean (which several call
 * sites ignore outright), so the page is permanently lost -- and without this record the only trace would be
 * an aged delivered:false row inside a 20-of-1000 notify history window that a later pack no longer carries.
 *
...
/** noteNotifyHistoryAppendFailure records a notify history append that was DROPPED: the alert may well
 ...
 * the batch is discarded, its pending seqs are cleared, and NO history row is written at all. */
...
/** noteDroppedEmission records a WHOLE notify emission lost because the routing pass threw. Without this it
 * becomes only an anonymous passSkips++ whose own bump can silently fail; this names the EVENT that was lost. */
...
/** noteUpdateChannelCheck records the update channel's verification state this tick: a channel that
 * stops VERIFYING (a signing-key rotation, or tamper) makes the update-alert pass return silently -- no
 * alert, no evidence -- so without this a critical update going unannounced would have nothing behind it. */
...
/** noteUpdateConfirmClearFailure records a confirm-clear attempt that failed: the applied update shows
...
/** noteCpExportAttempt records that the control-plane export pass RAN at all this tick. */
export function noteCpExportAttempt(): void {
  ledger.cpExport = { ...(ledger.cpExport ?? {}), attempted: true };
}

/** noteCpExportTruncated records that the export loop BROKE OUT on the shared budget, leaving the
...
/** noteCpExportDestFail records WHY one destination refused the recovery artefact: without it, perDest carries
 * ok:false with no cause, so a WORM refusal, a rotated wrap key and a network blip would be one signal. */
 ...
 * sealed-posture bucket: the readable roster/topology stays in the destination, and without this the debt is
...
/** noteCpExportRecordWriteFailure records that the export-health record write ITSELF was lost: the
...
/** noteCpExportPassThrew records that the export pass threw and BYPASSED recordExportHealth entirely,
 * so the last-written record is a previous, healthier tick's. */
export function noteCpExportPassThrew(): void {
  ledger.cpExport = { ...(ledger.cpExport ?? {}), attempted: true, passThrew: true };
}

/** notePushTrailWriteFailure records a push delivery-trail write that was DROPPED. On a SUCCESSFUL
 * delivery this strands the cursor, so the next tick re-sends the same batch, which is why a SIEM can end up
 * receiving duplicate events with nothing recorded behind it. */
 ...
 * The downpipe id is the customer's OWN label (the class the pack already carries in downpipes[]);
...
/** noteRestoreTestPassDeferred records that the WHOLE restore-test pass yielded on the shared budget.
 * A single deferral is routine; a run of them is why restore tests can read as overdue fleet-wide. */
...
/** noteFleetDrillBatch records the on-demand fleet-drill's batch health: without it a stuck drill run
...
/** noteReconcileEnabled echoes the ORPHAN_RECONCILE flag. The pass is opt-in, so an EMPTY reconcile
 * section is otherwise ambiguous: "the operator never turned it on" and "it is on and has been failing for
 * weeks" would look identical. This is the boolean that separates them. */
 ...
 * so a MONTHS-OLD signal is attributable rather than indistinguishable from a fresh one. */
...
/** noteRunlogSigVerdict SPLITS the merged RUNLOG-signature abstain: a DELETED .sig object (an operator
...
/** noteReconcileCircuitBreaker records that the inventory's circuit breaker TRIPPED. */
...
/**
 * assertSiemBatchCapSound is the STANDING GUARD on the latent data-loss path here: the SIEM shapers slice
...
/** One pass's durable health. Counts + closed class + timestamps; never a message. */
...
export interface CronHealth {
  ...
  // ---- all bounded: closed enums, counts, booleans, clamped labels ----------------------------------------
  /** The alerts CONSUMED and never delivered (by closed event), the one-shot claims spent on
  ...
  /** The update channel's verification state, and the confirm-clear that keeps failing. */
  readonly updates: {
  ...
  /** The control-plane export's truncation, per-destination fail classes, plaintext debt and lost writes. */
  readonly cpExport: {
    readonly lastAttemptAt?: string;
  ...
  /** The push delivery-trail writes that were themselves dropped (a stranded cursor => duplicates). */
  readonly push: { readonly siemTrailWriteFailures: number; readonly otlpTrailWriteFailures: number; readonly lastAt?: string };
  /** Per-downpipe restore-test NON-EXECUTION, the pass's budget deferrals, the fleet-drill's health. */
  ...
  /** The reconcile flag echo, the closed skip reasons, and the SPLIT RUNLOG-signature verdict. */
  readonly reconcile: {
 ...
 * @param cronIntervalMs - the expected tick interval, used to derive the tick gap.
...
export function applyCronHealth(prior: CronHealth | undefined, delta: unknown, now: number, cronIntervalMs: number): CronHealth {
  ...
  // ---- notify, updates, control-plane export, push, restore-test and reconcile ------------------------
 ...
 * diag:destprobefaults record. Only FAILING destinations produce a row, so a healthy fleet posts
