...
// format without a network call or a Durable Object. The envelopes are the documented wire shapes:
// raw-json is the pull feed's own body, POSTed instead of GET-returned; ndjson is one raw audit event
// object per line (the
...
//
// INJECTION SAFETY (the #1 adversarial target, FORMATS-AND-TRANSPORT.md). CEF/LEEF/GELF and the eventual
...
// capBatch slices a batch to SIEM_PUSH_BATCH_CAP and COUNTS the truncation. Every shaper goes through
...
export function shapeSplunkHec(events: AuditEvent[], _meta: PushCursorMeta): ShapedPush {
  ...
  const lines = capped.map((e) => {
    ...
    // A malformed ts would otherwise silently become "now", so the SIEM's own event ordering is wrong and
    // nothing anywhere says so. Count the substitution (a count; never the event, the ts or the actor).
...
function targetIdName(t: AuditTarget): { id?: string; name?: string } {
  switch (t.kind) {
    ...
    case "idpconnection":
      return { id: t.connId };
    // A refused authentication ceremony. The id slot is the connection ONLY when one resolved; an
    // unresolved probe carries no id at all rather than an attacker-chosen path segment. The ceremony names
    // the slot so a SIEM rule can key on it without parsing the message.
    case "authn-attempt":
      return t.connId !== undefined ? { id: t.connId, name: t.ceremony } : { name: t.ceremony };
    case "credential-cleanup":
      return { id: t.itemId };
    case "secret-rotation":
      // The closed cadence id (a fixed, non-secret vocabulary), redaction-safe as the id slot.
      return { id: t.secretId };
    case "dest-change":
...
function wireParts(e: AuditEvent): WireParts {
  const parsed = Date.parse(e.ts);
  // The same silent timestamp substitution guard as the SIEM wire path above, on the CEF/LEEF/GELF wire path.
  if (!Number.isFinite(parsed)) noteSiemShapingFallback("timestamp-substituted");
...
// denied is mid, a failed is high. Kept deliberately simple and documented (FORMATS-AND-TRANSPORT.md).
...
function trunc(s: string, n: number): string {
  ...
  // A truncated field would otherwise be invisible -- and a truncated HASH field silently fails the
  // customer's own chain-verification script, which reads as "your audit log is tampered". Count it (a
  // count only).
...
// shapeCef renders one ArcSight CEF line per event (FORMATS-AND-TRANSPORT.md). The 7 header fields are all
...
// shapeLeef renders one IBM LEEF 2.0 line per event (FORMATS-AND-TRANSPORT.md). The 6 header fields include
...
//
// A PLACEHOLDER STILL HAS TO BE WELL FORMED: a hand-typed run of zeros can easily land at the wrong width
// when the hash field is 96 hex characters wide, and being documented as "never verified against the real
// chain" makes a short placeholder easy to miss. GENESIS_PREV_HASH is the audit chain's own zero
// placeholder, built from SHA384_HEX_LEN, so the width here is derived rather than typed by hand. That
// matters on the wire: real Splunk HEC does not check the digest, but our own destsim emulator does, so a
// malformed placeholder would pass one target and fail the other with nothing to say which was right.
