...
// safety net). This module keeps src/index.ts a thin entry handler: index.ts re-exports flushDigests
// (the behaviour-neutral test seam) and imports runCanaryIfDue for the manual canary-fly-now runtime.
// This module imports nothing from index.ts, so there is no cycle.

// doURL is imported from its DEFINITION module (router-helpers.ts), not the router.ts re-export, so this
// cron module does not depend on the big admin router. That keeps the graph acyclic now that the harness
// canary-tick spoke (router-test-fault.ts) imports runCanaryIfDue from here and is itself imported by
// router.ts (router.ts -> router-test-fault -> notify-passes would otherwise close a cycle via doURL).
import { doURL } from "../do-url.ts";
import { checkUpdates } from "../admin/updates.ts";
import { runCanaryCycle } from "../canary/cycle.ts";
// HARNESS-ONLY: a no-op unless HARNESS_TEST_FAULTS is set (see src/admin/test-faults.ts), so a production
// hourly canary tick is byte-identical.
import { consumeHourlyCanaryFault } from "../admin/test-faults.ts";
...
// one-line changelog summary + "review it in the console", and NEVER a token, url or hash. It is
...
export async function runUpdateAlertIfNew(env: Env, scheduler: DurableObjectStub): Promise<void> {
  ...
  // A channel that is CONFIGURED and does not VERIFY is the tamper / signing-key-rotation case, and a pass that
  // simply returns leaves no alert and no local evidence, so "we were never told a critical update existed" has
  ...
  // The claim above is SPENT -- the DO has recorded this version as alerted, so it will never fire again -- so
  // the delivered boolean this returns must be checked: a routing failure would otherwise consume the one-shot
  // budget and deliver nothing, permanently. Latch the undelivered alert (closed event name only).
...
async function deliverDigestBatch(env: Env, scheduler: DurableObjectStub, batch: DigestBatch, at: string, egressAllowlist: string[] | undefined): Promise<number[]> {
  ...
  // A digest whose channel has since been DELETED or DISABLED cannot be delivered. Its seqs are
  ...
  // so the notify history shows the digest as undeliverable rather than silently dropped with only an
  // internal-only tally (noteDigestNoTransport) behind it.
  let records: DeliveryRecord[];
  if (channel?.enabled) {
    const r = await deliverToChannel(env, channel, emission, egressAllowlist);
  ...
  // CHECK the append: a non-2xx must not pass for a write (the response is read), or a delivered
...
export async function flushDigests(env: Env, scheduler: DurableObjectStub): Promise<void> {
  ...
  const { batches, egressAllowlist } = (await dueResp.json()) as { batches: DigestBatch[]; egressAllowlist?: string[] };
  if (batches.length === 0) return;
  const cleared: number[] = [];
  for (const batch of batches) {
    try {
      const seqs = await deliverDigestBatch(env, scheduler, batch, at, egressAllowlist);
...
export async function routeNotification(env: Env, scheduler: DurableObjectStub, emission: NotifyEmission): Promise<boolean> {
  try {
    ...
    const records: DeliveryRecord[] = await deliverEmission(env, emission, now, egressAllowlist);
    // Record the per-channel outcomes so the DO appends history. The append is AWAITED and CHECKED: an
    // isolate that exits the moment routeNotification returns must not drop the row for an alert that WAS
    // delivered, since the pack's answer to "was our critical alert delivered?" is that row. The append is one
    // DO round trip on a path that has already made several, and a failure is COUNTED rather than logged
    // into the void. It never throws into the caller: the delivery has already happened above, so a failed
    // record write degrades to "fewer history rows", never a thrown cron.
    try {
      const rec = await scheduler.fetch(doURL("/notify/record"), {
        method: "POST",
        body: JSON.stringify({ emission, records }),
    ...
    // A whole notify pass skipped because a DO fetch threw. Record it
    ...
    // passSkips alone is ANONYMOUS (and its own bump can silently fail). Name the EVENT that was lost whole, so
...
export async function runCanaryIfDue(env: Env, scheduler: DurableObjectStub): Promise<{ shouldAlert: boolean }> {
  ...
  // HARNESS-ONLY (no-op unless HARNESS_TEST_FAULTS is set): consume an hourly-canary-unhealthy fault and force
  // THIS flight's results unhealthy, so the REAL escalateRollbackIfNeeded below finds a promoted-but-unsettled
  // live bad build and claims the one-shot rollbackNeeded. Single-shot: a later tick (fault gone) flies
  // healthy. It NEVER deploys (the engine holds no deploy credential), so the never-auto-deploy invariant
  // remains untouched.
  if (await consumeHourlyCanaryFault(env, scheduler)) {
    for (let i = 0; i < results.length; i++) {
      results[i] = { ...results[i]!, status: "dead", deadReason: "harness test-fault: the hourly canary was forced unhealthy (hourly-canary-unhealthy)" };
    }
  }
...
async function escalateRollbackIfNeeded(env: Env, scheduler: DurableObjectStub, results: CanaryCheckResult[], at: string): Promise<{ claimed: boolean }> {
  ...
  try {
    ...
    if (pending && pending.recommendedVersion === ENGINE_VERSION) {
      ...
      const { shouldAlert } = (await claimResp.json()) as { shouldAlert: boolean };
      if (shouldAlert) {
        // The WORST case of the claim-then-deliver pattern. The one-shot rollback-needed claim is spent
        // above, so a routing failure here means the owner is NEVER paged about a bad update that is live --
        // so the delivered boolean must be latched, never discarded.
        const delivered = await routeNotification(env, scheduler, {
...
export async function confirmUpdateIfSettled(scheduler: DurableObjectStub, results: CanaryCheckResult[]): Promise<void> {
  ...
  try {
    ...
    if (last && last.outcome === "applied" && last.confirmationPending === true && typeof last.recommendedVersion === "string" && last.recommendedVersion === ENGINE_VERSION) {
      // An applied update that shows "verification pending" FOREVER means this clear is failing, tick after
...
async function notifyCanaryTransitions(
  ...
  for (const t of transitions) {
    if (t.transitioned === "dead") {
      // The DO has already DEDUPED this transition (a persistent death pages once), so an undelivered
      // canary-dead is a page that will never be re-attempted, leaving the destination dead with nobody told.
      // Latch the delivered boolean rather than discard it.
      const delivered = await routeNotification(env, scheduler, {
        event: "canary-dead",
