...
export async function runBeaconEmitPass(env: Env, scheduler: ReturnType<typeof schedulerStub>): Promise<void> {
  // Record WHICH of the three env vars are present, as booleans. The gate below is all-or-nothing, so a
  ...
  // beacon-config.ts because the posture screen has to answer the same question and previously answered it
  // from two of the three vars, so it reported this estate as phoning home while this function returned here.
  ...
  try {
    ...
    // ASVS V1.3.6 / V13.2.4: BEACON_URL is operator-supplied and this POST carries the ingest bearer, so the
    // target is screened at send time like every other egress: https only, never an internal or metadata
    // host, and a redirect is a failure rather than somewhere to follow the bearer to. The refusal is thrown
    // with the wording classifyBeaconFail maps to "url-invalid", the permanent-operator-error class, so the
    // support pack reads it as "fix BEACON_URL" and not as a network blip.
    const target = new URL("/beacon", base);
    if (target.protocol !== "https:" || isInternalSinkHost(target.hostname)) {
      throw new Error("invalid base url: BEACON_URL must be https and must not name a private, loopback, link-local or metadata host");
    }
    ...
    // The CLASS rides with the attempt, so the DO's beacon record (which the pack carries) can say
    ...
    //
    // A throw path that persisted ok:false with NO status and NO class would make a malformed BEACON_URL (a
    ...
    // The SAME closed class also rides into the DO's beacon record, so the pack can tell a
...
async function recordBeaconState(scheduler: ReturnType<typeof schedulerStub>, ok: boolean, status: number | undefined, errorClass?: BeaconFailClass): Promise<void> {
  try {
    await scheduler.fetch(doURL("/beacon-state"), {
      ...
      // re-validates it against the same closed set at its own boundary.
      body: JSON.stringify({ ok, ...(typeof status === "number" ? { status } : {}), ...(errorClass !== undefined ? { errorClass } : {}) }),
    });
  } catch {
    // A swallowed failure here would let beacon.lastOk / lastAt go
    // silently STALE and the pack would present a weeks-old beacon verdict as current. Count the loss (a
    // count, nothing else) so a stale beacon field is attributable rather than misleading. Still fail-open:
    // never let beacon bookkeeping affect a backup.
