...
// regression (test/validate-fleet-scale.ts) exercises a 2500-downpipe account, so the cap sits
// at 2x that (5000) to cover it with margin and never silently drop a downpipe at any realistic fleet size.
...
// buildOtlpResourceMetricsChunks (one body per chunk), so the wire shape for a given set of
...
// validator); the cron drain itself sends buildOtlpResourceMetricsChunks's per-chunk bodies instead
...
// chunks, each rendered as its OWN complete, independently-POSTable OTLP ResourceMetrics body: the
