...
// pass this tick", never a crashed cron.

import { envFlagEnabled } from "../admin/auth.ts";
import { envRecipientFingerprint, envSignerFingerprint } from "../admin/key-vintages.ts";
...
// secretRotationForward (ASVS V13.3.4) resolves, on the Worker side (the DO holds no env of its own), which
// of the seven cadenced secrets' env bindings are present and the current fingerprint of each PUBLIC-key
// binding the engine already derives for another purpose (key-vintages.ts), so the scheduler DO's reconcile
// can seed/refresh the credential-lifecycle registry's rotation rows. Bearer/destination credentials never
// get a fingerprint entry here (no-custody): only presence is reported for them.
async function secretRotationForward(env: Env): Promise<{ presentBindings: Record<string, boolean>; fingerprints: Record<string, string | null>; adminTokenDisabled: boolean }> {
  const bound = (v: string | undefined): boolean => typeof v === "string" && v.trim() !== "";
  return {
    presentBindings: {
      SIGNER_PRIVATE: bound(env.SIGNER_PRIVATE),
      BREAK_GLASS_PUBLIC: bound(env.BREAK_GLASS_PUBLIC),
      OPERATIONAL_PRIVATE: bound(env.OPERATIONAL_PRIVATE),
      ADMIN_TOKEN: bound(env.ADMIN_TOKEN),
      SCIM_BEARER_TOKEN: bound(env.SCIM_BEARER_TOKEN),
      DISCOVERY_API_TOKEN: bound(env.DISCOVERY_API_TOKEN),
      DEST_SECRET_ACCESS_KEY: bound(env.DEST_SECRET_ACCESS_KEY),
    },
    fingerprints: {
      SIGNER_PRIVATE: await envSignerFingerprint(env.SIGNER_PRIVATE),
      BREAK_GLASS_PUBLIC: await envRecipientFingerprint(env.BREAK_GLASS_PUBLIC),
      OPERATIONAL_PUBLIC: await envRecipientFingerprint(env.OPERATIONAL_PUBLIC),
    },
    adminTokenDisabled: envFlagEnabled(env.ADMIN_TOKEN_DISABLED),
  };
}

// ---- the delivery-FEEDBACK confirms (a support-pack gap) --------------------------
//
// The three feedback confirms below (/alerts-delivered, /recovery-delivered, /replication-alerts-delivered)
// are the writes that CLOSE the loop after an alert is routed: they clear the OPTIMISTIC cooldown of a
...
// cooldown stays written) and "PagerDuty never auto-closed after recovery" (the owed resolve is dropped).
//
// TWO PROPERTIES THIS GUARANTEES:
//   1. A THROW is not the only failure caught. A DO that ANSWERS -- a 404 after a partial deploy that skews
//      the route chain, a 400, a 500 under load -- returns a Response, so `await scheduler.fetch(...)`
//      resolves happily, and treating that as success would make the single most likely real-world failure
//      of these routes invisible to BOTH the cooldown logic and the pack. The response is CHECKED (resp.ok),
//      so a non-2xx is counted exactly like a transport throw.
//   2. All three streams count. Bumping the notifyHealth aggregate from only one stream, while the other two
//      log to Workers Logs (which the pack cannot collect), would make notifyHealth.feedbackFails a lossy
//      sample of one stream's throw-only path rather than a TRUE count of the whole feedback surface.
// The counter's own write is not fire-and-forget either: a bare `.catch(() => {})` bump would itself be the
// write most likely to be lost in the very fault window it exists to prove (same DO, same tick), so it is
...
export async function runAlertPass(env: Env, scheduler: ReturnType<typeof schedulerStub>): Promise<boolean> {
  ...
  try {
    ...
    if (pendingTransitionIds.length > 0) {
      ...
      // the delivery accounting reconciled, and the pipe is silently stuck in cooldown. confirmFeedback
      // counts it in notifyHealth.feedbackFails, so the pack can name the cause instead of showing a healthy
      // notify pipeline through the silence.
    ...
    // AUTO-RESOLVE: the DO returns one emission per downpipe that just crossed
    ...
    // downpipe with an owed resolve whose prior delivery failed. Route each through the
    ...
    //
    // TWO-PHASE RETRY: the delivery is not fire-and-forget. Each recovery's outcome is
    ...
    // ages out. A transient blip during recovery therefore cannot strand an incident open. The feed-
    ...
    if (deliveredRecoveryIds.length > 0 || failedRecoveries.length > 0) {
      ...
      // for ever ("PagerDuty never auto-closed after recovery"). confirmFeedback records this stream in
      // either the throw or the non-2xx case, so the loss is never silent.
      await confirmFeedback(scheduler, "/recovery-delivered", { deliveredIds: deliveredRecoveryIds, failed: failedRecoveries });
    }
    return true;
  } catch (e) {
    // The pass failed. Classify the throw HERE, where the exception is in hand -- the driver only
...
export async function runReplicationAlertPass(env: Env, scheduler: ReturnType<typeof schedulerStub>): Promise<boolean> {
  ...
  try {
    ...
    if (pendingTransitionIds.length > 0) {
      ...
      // leaves a degraded fan-out silently cooled with no second warning. confirmFeedback records this stream
      // in either the throw or the non-2xx case, so the loss is never silent.
      await confirmFeedback(scheduler, "/replication-alerts-delivered", { failedTransitionIds });
    }

    // AUTO-RESOLVE: route the replication-degraded / run-at-risk-eviction
...
export async function runExpiryPass(env: Env, scheduler: ReturnType<typeof schedulerStub>): Promise<boolean> {
  ...
  // cron. routeNotification is itself fully fail-open. The DO returns no emissions when nothing is
  // tracked and none of the seven cadenced secrets (secret-rotation.ts) are configured; each configured
  // one costs a handful of storage reads/writes to keep its tracked row in step, seeded from the
  // presentBindings + fingerprints this pass resolves and forwards below (the DO holds no env of its
  // own). The detail is the item label + days-remaining only (redaction-safe; never a secret), and the
  // event is account-level so downpipeId/downpipeName are null.
  try {
    const forward = await secretRotationForward(env);
    const expResp = await scheduler.fetch(doURL("/expiry/reconcile"), { method: "POST", body: JSON.stringify(forward), headers: { "content-type": "application/json" } });
