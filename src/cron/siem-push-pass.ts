...
// drain cannot tell. So the guarantee this pass provides is at-least-once TO ACCEPTANCE. Confirming the events
// were indexed needs the sink's own acknowledgement protocol (for HEC, a request channel plus an ack poll),
// which is state that has to survive the tick that sent the batch; that is a separate, costed piece of
// design, deliberately not half-built here, because an
// ack path that advanced the cursor anyway would be worse than this honest limit. OPT-IN: no destination
...
export interface PushDeliveryResult {
  ...
  status?: number;
  reason?: string;
  // causeDigest: the 12-hex one-way digest of the RAW fault behind `reason`, the SAME construction the
...
// record on the DO delivery trail. Without these, EVERY pre-delivery fault
// (a rotated/malformed CONFIG_WRAP_KEY, an unreadable DO, an unreadable cursor or audit export, an
// unreadable metrics snapshot) would take an early `return false` that writes NOTHING to the trail: the
// customer's "our SIEM/metrics push silently stopped" ticket would arrive with an empty trail, indistinguishable
// from a drain that was never enabled. Each member is a closed code, never a message, status line, endpoint or
// secret; the coarsening happens HERE, at the recording site, so the pack's push sections carry a class the
// bot can reason over. "shape-or-deliver-fault" is kept as a member of this list too.
...
//
// A single fixed string, "s3-put-failed", for every possible cause on the S3-drop sink would leave "our SIEM
// S3-drop retries forever" reaching support with no way to tell the customer whether it is THEIR bucket policy,
// THEIR expired STS session, an Object-Lock refusal or OUR signing. The string is SPLIT into the same closed
// class set the destination probes already use (dest/classify.ts DEST_DOWN_REASONS + the fault log's closed S3
// <Code> allow-list), so the row names an operator action. Still a closed enum: the S3 error body is never
// carried.
...
/**
 * classifyPushS3Fault coarsens a failed S3-drop PUT into a closed PushS3FailCode. It reads the driver's
...
//
// CALLING shapeNdjson UNCONDITIONALLY, ignoring the format entirely, would write seven of the eight formats a
// customer could choose to the bucket as NDJSON with nothing saying so: the delivery would succeed, the
// cursor would advance, and the trail would record a successful push of the chosen format, with all eight
// storing byte-identical objects. The format therefore has to be THREADED IN here rather
// than defaulted, and the census in test/validate-destination-format-coverage.ts reads the object back off
// a real destination and refuses any pair whose stored bytes parse only under a format other than the one
// configured, so the substitution cannot return silently.
async function putSiemBatchToS3(s3: ResolvedS3Target, format: PushFormat, events: AuditEvent[], meta: PushCursorMeta, opts?: PushDeliveryOpts): Promise<PushDeliveryResult> {
  ...
  // is what turns a fixed "s3-put-failed" into a class an operator can act on.
  const dest = new S3Destination(s3.endpoint, s3.bucket, s3.region, s3.accessKeyId, s3.secretAccessKey, {
    ...
    ...(opts?.allowInternalSink === true ? { allowInternalEndpoint: true } : {}),
  });
  try {
    ...
    // one-way 12-hex prefix of that same raw text: it identifies WHICH fault without carrying any of it.
...
// PushDeliveryOpts is the ONE delivery-time test seam on this path, and it exists so the destination-format
// census (test/validate-destination-format-coverage.ts) can drive the REAL sender against a node:http S3 drop
// emulator and a node:tls syslog emulator, both of which listen on 127.0.0.1. Both sinks screen an
// internal host (the s3 leg at S3Destination's wire chokepoint, the syslog leg in deliverSiemSyslog), which
// is exactly what an emulator on loopback trips. The drain's own call site below passes NOTHING, so a
// deployed estate is screened; a validator that wants a loopback emulator has to ask for it in one place.
...
// one request per event (deliverGelfHttp: Graylog's GELF HTTP input drops a batched body by
// default). It NEVER throws.
export async function deliverResolvedPush(cfg: ResolvedPushConfig, events: AuditEvent[], meta: PushCursorMeta, opts?: PushDeliveryOpts): Promise<PushDeliveryResult> {
  if (cfg.sink === "s3") {
    if (!cfg.s3) return { ok: false, reason: "s3-target-missing" };
    return putSiemBatchToS3(cfg.s3, cfg.format, events, meta, opts);
  }
  if (cfg.sink === "syslog-tls") {
    if (!cfg.syslog) return { ok: false, reason: "syslog-target-missing" };
    return toPushDeliveryResultSyslog(await deliverSiemSyslog(cfg.syslog.host, cfg.syslog.port, cfg.format, events, meta, opts?.allowInternalSink === true ? { allowInternalSink: true } : undefined));
  }
  if (cfg.format === "gelf") {
  ...
  // refusal that must not advance the cursor past audit events Splunk has thrown away. Every other format is
  // sent with this off, so its result is unaffected.
...
async function recordOutcome(
  ...
  try {
    ...
    // The response must be READ: an unread non-2xx would pass for a recorded outcome, and that is not a
    // cosmetic loss. The cursor authority lives in this record, so a dropped write on a SUCCESSFUL delivery
    // would leave the cursor where it was and the NEXT tick would re-send the same batch -- the "our SIEM is
    // receiving duplicate events" ticket, with an anonymous passErrors integer as its only trace. Count the
    // lost trail write.
...
// recordPassFault records a PRE-DELIVERY fault on the SIEM push trail: the drain never
...
// `cause` is the RAW throw behind the closed reason, when there was one. It is reduced to a 12-hex
...
export async function runSiemPushPass(env: Env, scheduler: ReturnType<typeof schedulerStub>): Promise<boolean> {
  ...
  try {
    ...
    // A lastPushedSeq that is not a number (a DO shape drift after a partial deploy, or a storage
  ...
  // LATENT DATA LOSS RISK. The drain asks for SIEM_PUSH_BATCH_CAP events and the shapers
  // independently slice to SIEM_PUSH_BATCH_CAP. Those are the SAME constant today, so nothing is lost -- but
  // the cursor below must advance to the last event the SHAPER actually sent, not the last event the DRAIN
  // returned. Parting those two limits (one edit, in either file) would shape the batch TAIL away while
  ...
  try {
    ...
    await recordOutcome(scheduler, {
      ...
      // The S3 leg is the one delivery path with a RAW throw, so it is the one that can carry a join key.
