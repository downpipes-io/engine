...
// recordSealError ATTRIBUTES one pre-run dispatch fault to the downpipe that hit it. Without it, a
// throw that escapes runDownpipe leaves NOTHING per-downpipe: the /complete below is a runId-"" history no-op,
// so the pack shows a run-history GAP and one anonymous sealErrors integer, and "this downpipe silently
// stopped producing runs" cannot be told from "its destination record was deleted" or "the DO was down".
//
...
async function recordSealError(env: Env, downpipeId: string, cls: SealErrorClass): Promise<void> {
  // The LOSS of this record is itself evidence: the `seal-error` droppedWrites kind exists for it, wired
  // through recordDiagWrite below, so a DO that refuses or is unreachable does not take the downpipe's only
  // cause with it and leave the pack showing a run-history GAP with nothing to explain it -- the state
  // ("this downpipe silently stopped producing runs") this recorder exists to end.
  //
  // The DO being the sink for this write does not mean a DO outage cannot record its own outage: that is
  // precisely what the dropped-write protocol solves. recordDiagWrite counts the loss in an isolate-local
  // tally and folds it into the aggregate on the next write that SUCCEEDS, so the gap lands the moment the
  // DO comes back -- which is when the pack is generated. It stays fire-and-forget and never throws
  // (recordDiagWrite swallows both a transport throw and a non-2xx), so the loop's error path is unchanged.
  // Only the closed class and the customer's own downpipe id cross the wire.
...
// before it will START another due downpipe. A crowded tick (many due
...
//
// Deriving this from FINALISE_RESERVE (rather than a bare literal) keeps this "a few multiples" by
// construction, so the two constants cannot drift apart. A thinner margin (close to 1.5x
// FINALISE_RESERVE) is exactly wide enough for the failover probe(s) and the /trigger round-trip to eat
// before a non-resumable source (secrets, D1, cf-config; see seal/slice.ts crawlNonResumable) seals even
// its first record, so shouldYield() (remaining <= FINALISE_RESERVE) can trip immediately and the source
// throws "source too large for one slice" -- a false diagnosis for a source that is not large at all,
// just unlucky in tick order: a small Secrets downpipe can fail with that exact message on a scheduled
// tick sandwiched between other due downpipes, while an on-demand run (full budget) and every other
// scheduled tick succeed.
...
// processes the due set OLDEST-DUE-FIRST (fairness) so a crowded tick that runs out of
...
export async function runSealLoop(env: Env, due: DownpipeState[], budget: ReturnType<typeof budgetFromEnv>): Promise<{ dispatched: number; coalesced: number; carried: number; sealErrors: number }> {
  // FAIRNESS: process the due downpipes OLDEST-DUE-FIRST so a crowded tick that
  ...
  // PER-INVOCATION failover-probe cache: many downpipes on a fan-out fleet share the
  ...
  for (; i < ordered.length; i++) {
    ...
    // BUDGET EARLY-EXIT: stop dispatching new downpipes once there is no longer
    ...
    try {
      ...
      //
      // The fault is ATTRIBUTED. The class comes off the tag runDownpipe's sites attach (never
      ...
      // it is the one the pack would otherwise see only as an unexplained stall.
