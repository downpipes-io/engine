// cf-config discovery refresh pass for the cron driver (cron/drive.ts): probes ONE stale
// auto-mode cf-config downpipe per tick to refresh its surface set, gated on ample shared
// budget so it never starves backups/replication.
...
export async function runDiscoveryPass(env: Env, scheduler: ReturnType<typeof schedulerStub>, budget: ReturnType<typeof budgetFromEnv>): Promise<boolean> {
  ...
  // tick so its capture runs back up only the surfaces in USE (not one GET per all every surface in the registry every
  // run). Bounded to one downpipe per tick (each probe is ~200 GETs) and gated on ample shared budget,
  ...
  // costs one read.
  try {
    // EVERY exit from this pass records WHY. The discovery cache silently never refreshing (a
    // missing token, an account narrowed out of scope, chronic budget starvation, a CF-API 403) is the whole
    // "cf-config backups got slow and expensive" ticket -- capture fail-safes to ALL every surface in the registry every run
    ...
    // Cross-account confused-deputy re-check (ASVS V4): `selected` can narrow AFTER this
