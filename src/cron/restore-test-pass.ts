...
// pass/fail notification; recordRestoreTestOutcome is the two-place best-effort recorder. This module
// keeps src/index.ts a thin handler: index.ts re-exports runScheduledRestoreTest (the behaviour-neutral
// test seam) so the scheduled-restore-test validator that imports it from ../src/index.ts keeps working.
// This module imports the notification routing from cron/notify-passes.ts and the destination resolver
// from cron/seal-dispatch.ts (not from index.ts), so there is no cycle.
...
export async function runScheduledRestoreTest(env: Env, scheduler: DurableObjectStub, state: DownpipeState): Promise<void> {
  ...
  // The SCHEDULED drill is the one that runs unattended, so it is the one whose failures nobody reads.
...
async function markRestoreTestStart(scheduler: DurableObjectStub, id: string, at: number): Promise<void> {
  try {
    ...
    // The response must be read: a lost marker means a tick-KILLED drill leaves no "attempted" trace at
...
async function recordRestoreTestOutcome(
  ...
  try {
    ...
    // The response must be read, so a non-2xx does not pass silently for a recorded outcome.
    if (!resp.ok) noteRestoreTestSkip(downpipeId, "record-failed");
  } catch (e) {
    // BOTH outcome writes can fail, leaving a week-long hole in the evidence behind a perfectly healthy
  ...
  try {
    ...
    if (!resp.ok) noteRestoreTestSkip(downpipeId, "record-failed"); // a capability/validation refusal eats the evidence row
  } catch (e) {
    noteRestoreTestSkip(downpipeId, "record-failed"); // the evidence row for a drill that RAN is lost
...
export async function runRestoreTestPass(env: Env, scheduler: ReturnType<typeof schedulerStub>, budget: ReturnType<typeof budgetFromEnv>): Promise<boolean> {
  ...
  if (budget.shouldYield()) {
    // One deferral is routine (backups outrank drills, which is correct). A RUN of them is chronic
  ...
  try {
    ...
    for (const state of testsDue) {
      if (ran >= cap || budget.shouldYield()) {
        // A DUE downpipe that never gets its turn this tick. Correct (the herd is capped on purpose), and
      ...
      try {
        ...
        // A PRE-DRILL throw (an unresolvable destination, an unusable CONFIG_WRAP_KEY) lands BEFORE every
    ...
    // Classify the throw here; the driver only sees the false return.
...
async function runFleetDrillPass(env: Env, scheduler: ReturnType<typeof schedulerStub>, budget: ReturnType<typeof budgetFromEnv>, remainingCap: number): Promise<void> {
  ...
  try {
    ...
    // The fleet-drill campaign moved a BATCH. Without this, a campaign that is stuck (dispatching members that
    // never drill) moves no counter anywhere, so "the fleet drill has been running for a week" has no
    ...
    for (const state of batch) {
      ...
      try {
        ...
        noteRestoreTestSkip(state.config.id, "pre-drill-fault"); // same pre-drill hole, on the fleet path
    ...
    // The fleet drill never moves passErrors on its own -- a drill campaign could fail every tick for
    // days while the pack shows a clean cron. It is folded into the restore-tests pass health, which is the
    // pass the operator recognises ("no restore tests are running").
    noteCronPass("restore-tests", false, e);
    noteFleetDrillBatch(true); // the campaign's OWN batch health (a stuck campaign, named as such)
