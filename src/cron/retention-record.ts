...
// that coarsen free text into an enum, and the pure sanitiser that is the single redaction chokepoint.
//
...
export const RETENTION_OUTCOMES = [
  ...
  // PAUSE MEANS PAUSE: the downpipe is PAUSED (config.enabled false), so the cron pass did not plan and
  // did not delete. The row exists rather than the downpipe simply being absent, because "retention
  // deliberately left this alone because you paused it" and "retention silently did nothing" are two
  // different sentences and support must be able to tell them apart. An ATTENDED prune (the
  // dual-controlled route) is unaffected and still deletes.
  "paused",
] as const;
...
// perpetually deferring downpipe ("retention never deletes anything") is then visible as a class, not a
// string.
...
export interface RetentionDownpipeOutcome {
  ...
  // opaque id class destinations[] already carries. Without it the per-destination fold
  ...
  // the fault ring; here it reaches the per-destination fold so the console can name the posture.
...
export interface RetentionPassRecord {
  readonly at: number;
  // attended marks a record posted by the attended prune route (admin/router-retention-prune.ts), which
  // prunes ONE downpipe on an operator's action rather than the whole fleet on the cron. The DO folds an
  // attended record into the per-destination sidecar exactly like a cron record, but never lets it
  // supersede the latest CRON pass slot: a single-downpipe pass must not read as a whole tick.
  ...
  readonly replicationUnreadable: boolean; // the coverage gate ran on NO proof, so every replicated run was held
...
export function classifyPruneError(e: unknown): RetentionErrorClass {
  ...
  // `\bput\b`, not `put ` with a trailing space, so the pattern matches both a native binding's "put:
  // <reason>" and an S3 leg's "PUT seg/0001: status 400" while excluding "input" (the p there is preceded
  // by a word character, so the boundary fails). A message that falls past every branch here reaches
  // `other`, so this is the one label that says a destination write is where to look.
