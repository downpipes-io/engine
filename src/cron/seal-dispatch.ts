...
// pinned-or-default destination; sealRun drives the sliced seal. This keeps src/index.ts a thin entry
// handler: index.ts re-exports destinationReachable + selectSealDestination (the behaviour-neutral test
// seams) so the validators that import them from ../src/index.ts keep working. This module imports
// nothing from index.ts, so there is no cycle.
...
export async function runDownpipe(env: Env, state: DownpipeState, budget?: SliceBudget, probeCache?: Map<string, boolean>): Promise<RunDispatchOutcome> {
  ...
  // failover probe is metered against the shared tick budget and memoised per-tick.
  //
  // BOTH pre-run steps below can THROW before a run index exists,
...
//
// METERED: without this, the probe's put (and the best-effort delete) are real platform subrequests that,
// on a multi-dest fleet, would be invisible to the tick budget and could blow the per-invocation
// subrequest cap before the budget noticed. The optional meter counts each attempted subrequest against
// the SAME shared tick budget the seal loop spends, so a fleet's failover probing stays accounted for (the
// caller pairs this with a per-tick probe cache so a shared destination is probed once per tick).
export async function destinationReachable(dest: Awaited<ReturnType<typeof buildDestination>>, meter?: SliceBudget, destinationId?: string): Promise<boolean> {
  ...
  try {
    ...
    // Classifying the fault HERE, at the only site that ever holds it, into the closed DEST_PROBE_REASONS
    // vocabulary the destination record already speaks is what lets "backups failing: all destinations
    // unreachable" separate an expired STS credential (rotate it) from a deleted bucket (recreate it) from
    // a DNS fault (wait) -- three tickets with completely different remedies that a bare dropped S3 error
    // cannot distinguish. The message, the endpoint, the bucket and the key stay in the driver.
...
export async function selectSealDestination(
  ...
  for (const id of ids) {
    ...
    if (!cfg) {
      // A fan-out destinationId with NO config record. The run proceeds happily to its remaining
    ...
    // Reusing a probe result already taken THIS tick for this destination: a fan-out
    // fleet where many downpipes share destinations probes each destination once per invocation, not
    // once per downpipe, so the failover probing does not scale with the fleet size and stays inside
