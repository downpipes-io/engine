...
export function digestWindowElapsed(period: DigestPeriod, oldestAtMs: number, nowMs: number): boolean {
  if (!Number.isFinite(oldestAtMs) || !Number.isFinite(nowMs)) {
    // This is the CONSERVATIVE suppression. An unparseable window bound means the digest does NOT flush -- an
    // alert the customer expected is never sent. Suppressing conservatively is right (a guessed flush could
    // spam), but it must be VISIBLE rather than leaving "our digest just stopped arriving" evidence-free.
    // Counted, never the timestamp itself.
...
export function summariseDigest(entries: PendingDigestEntry[]): DigestSummary {
  ...
  for (const e of entries) {
    ...
    if (!Number.isFinite(t)) {
      noteTimestampParseFailure(); // this entry is silently EXCLUDED from the digest's oldest/newest span
      continue;
    }
    if (fromAt === "" || t < fromMs) {
      fromMs = t;
...
export function groupDigestDue(entries: PendingDigestEntry[], nowMs: number): DigestBatch[] {
  ...
  for (const channelId of [...byChannel.keys()].sort()) {
    ...
    for (const e of group) {
      ...
      const t = Date.parse(e.at);
      if (!Number.isFinite(t)) noteTimestampParseFailure(); // silently excluded from the summary's window
