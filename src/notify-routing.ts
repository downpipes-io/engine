...
import {
  ...
  type NotifyEvent,
  type Severity,
  type ResolvedSinkVerdict, type SinkScreenVerdict,
...
export interface NotifyHistoryEntry {
  ...
  // response. recovered rides for a PagerDuty resolve emission so the pack can
  // correlate a resolve with its trigger (pagerduty-resolve-dedupkey). All redaction-safe.
  deliveryCode?: DeliveryFailCode;
  platformCode?: string;
  recovered?: boolean;
  // The send-time sink-screen verdict for a url-bearing channel. Unlike
  ...
  test?: true;
  // unconfirmed: true on a delivered:true entry whose sink only
  ...
  unconfirmed?: boolean;
  // ackOutcome is the CLOSED verdict of the async confirmation step, splitting the flat
  ...
  // On an email-recipients-invalid failure, the POSITION of the offending entry in the channel's address
  // list. ONE typo'd address kills delivery to EVERY recipient of that channel (the send is a single call);
  // without this index, support cannot name the bad entry and the customer must eyeball the list and guess.
  // An index is an integer, never an address, so it names the field without naming a person.
...
// ChannelResult's failure carries the CLOSED webhook-reject code alongside the operator-facing SENTENCE.
// The sentence is what the operator reads and what the pack can never record (it quotes the
// submitted URL's shape and is free text); the code is the SAME decision as a closed enum member, so the DO
// can count it. "I can't save my SIEM/webhook channel" is a common ticket, and a validation loop that only
// ever answers a 400 to the browser leaves the pack blind to it. A count per code answers it in one line:
// they keep pasting an http:// URL, or their sink is on a private address and they have not ticked the
// internal-sink override. Additive: every existing consumer reads `reason` and is unchanged.
...
function parseChannelSecret(raw: unknown): SecretParseResult {
  ...
  // stored UNWRAPPED: treating only an EXACT "" as keep-secret would let a whitespace-only value like "   "
  // fall through to the plaintext branch below and persist verbatim even with CONFIG_WRAP_KEY set.
 ...
 * the length test, so with CONFIG_WRAP_KEY set -- the ordinary deployment -- the 512-character bound is never
 * evaluated inside parseChannelSecret at all: without a separate check here, an oversized token stores as
 * ciphertext while the same token with no wrap key hits a clean 400, so the two postures would disagree
 ...
 * arm and is exempt from a length rule that is about plaintext. (An isWrappedSecret call after that test
 * would be an always-false condition, since the value is a string by then, so it is deliberately omitted.)
 *
...
// when true (exactOptionalPropertyTypes-safe). egressAllowlist (ASVS V13.2.4) is the account's current
// configured allowlist; an absent value (no caller supplied one) permits every host.
function validateUrlKind(kind: "webhook" | "slack", name: string, url: unknown, allowInternalSink: boolean, egressAllowlist: readonly string[] | undefined): ChannelResult {
  const v = isAllowedWebhookUrl(url, { allowInternalSink, ...(egressAllowlist !== undefined ? { egressAllowlist } : {}) });
...
function validateTeamsKind(name: string, c: Record<string, unknown>, allowInternalSink: boolean, recipientsValidator: RecipientsValidator, egressAllowlist: readonly string[] | undefined): ChannelResult {
  if (c.url !== undefined && c.url !== null && c.url !== "") {
    const v = isAllowedWebhookUrl(c.url, { allowInternalSink, ...(egressAllowlist !== undefined ? { egressAllowlist } : {}) });
    if (!v.ok) return { ok: false, reason: v.reason, code: v.code };
...
function validateJsmKind(name: string, url: unknown, apiKeyRaw: unknown, allowInternalSink: boolean, egressAllowlist: readonly string[] | undefined): ChannelResult {
  const v = isAllowedWebhookUrl(url, { allowInternalSink, ...(egressAllowlist !== undefined ? { egressAllowlist } : {}) });
  if (!v.ok) return { ok: false, reason: v.reason, code: v.code };
...
function validateServicenowKind(name: string, url: unknown, usernameRaw: unknown, apiKeyRaw: unknown, allowInternalSink: boolean, egressAllowlist: readonly string[] | undefined): ChannelResult {
  const v = isAllowedWebhookUrl(url, { allowInternalSink, ...(egressAllowlist !== undefined ? { egressAllowlist } : {}) });
  if (!v.ok) return { ok: false, reason: v.reason, code: v.code };
...
// egressAllowlist (ASVS V13.2.4) is the account's CURRENT configured outbound allowlist. The caller (the
// DO's addNotifyChannel) reads its own stored OrgPolicy and passes it here; when it is omitted, every
// url-bearing kind's isAllowedWebhookUrl call permits every host (screenEgressAllowlist's unset behaviour).
export function validateChannel(raw: unknown, recipientsValidator: RecipientsValidator, egressAllowlist?: readonly string[]): ChannelResult {
  ...
  // allowInternalSink: the per-channel SSRF override. Only a literal boolean true opts in;
  ...
  if (kind === "webhook" || kind === "slack") return validateUrlKind(kind, trimmedName, c.url, allowInternalSink, egressAllowlist);
  if (kind === "teams") return validateTeamsKind(trimmedName, c, allowInternalSink, recipientsValidator, egressAllowlist);
  if (kind === "pagerduty") return validatePagerdutyKind(trimmedName, c.routingKey);
  if (kind === "jsm") return validateJsmKind(trimmedName, c.url, c.apiKey, allowInternalSink, egressAllowlist);
  if (kind === "servicenow") return validateServicenowKind(trimmedName, c.url, c.username, c.apiKey, allowInternalSink, egressAllowlist);
...
// (a hand-maintained copy previously dropped backup-volume-regression + source-detached, so a rule
// could not select them and the POST 400'd "unknown event").
...
//
// egressAllowlist (ASVS V13.2.4) is the account's CURRENT configured allowlist (the caller reads it from
// the DO once per resolve/digest-due round trip, never per channel). It is threaded to every url-bearing
// adapter except pagerduty (a fixed provider endpoint the customer never configured, so it is never
// subject to a list governing what THEY chose to reach) and email (no url, no fetch).
export async function deliverToChannel(env: Env, channel: NotifyChannel, emission: NotifyEmission, egressAllowlist?: readonly string[]): Promise<ChannelDeliveryResult> {
  try {
    switch (channel.kind) {
      case "webhook":
        return await webhookChannel.deliver(channel, emission, egressAllowlist);
      case "slack":
        return await slackChannel.deliver(channel, emission, egressAllowlist);
      case "pagerduty":
        return await pagerdutyChannel.deliver(channel, emission);
      case "teams":
        return await teamsChannel.deliver(env, channel, emission, egressAllowlist);
      case "email":
        return await emailChannel.deliver(env, channel, emission);
      case "jsm":
        return await jsmChannel.deliver(env, channel, emission, egressAllowlist);
      case "servicenow":
        return await servicenowChannel.deliver(env, channel, emission, egressAllowlist);
    }
  } catch {
    // A thrown delivery path must never escape into a backup or the cron; degrade to not-delivered.
    //
    // Reporting this as `network-error` would be wrong: that is a CUSTOMER-side transport class, so an
    // ADAPTER CODE REGRESSION -- our bug -- would read in the delivery history as an intermittent network
    // fault and send support chasing the customer's firewall. Every adapter already swallows its own
    // transport errors (deliverPayload and sendEmail never throw), so a throw that reaches HERE is by
    // construction an engine fault, and adapter-exception says exactly that. The path is otherwise
    // unchanged: still fail-open, still
    // not-delivered, still no message recorded.
...
export interface DeliveryRecord {
  ...
  platformCode?: string;
  recovered?: boolean;
  // The send-time sink-screen verdict for a url-bearing channel, carried on
  // success AND failure (a delivered `hostname` sink is the rebind-exposed class). Dropped for email/pagerduty.
  sinkScreen?: SinkScreenVerdict;
  resolvedScreen?: ResolvedSinkVerdict;
  // unconfirmed: JSM/Opsgenie's async-accepted (202) outcome, carried through into the history
  // entry so a bare 202 is never recorded as an indistinguishable plain "delivered".
  unconfirmed?: boolean;
  // ackOutcome: the CLOSED async-confirmation verdict, carried through /notify/record into the
  ...
  ackOutcome?: AckOutcome;
  // Which recipient in the channel's address list was rejected (an integer POSITION, never the address).
  recipientIndex?: number;
}
...
export async function deliverEmission(env: Env, emission: NotifyEmission, channels: NotifyChannel[], egressAllowlist?: readonly string[]): Promise<DeliveryRecord[]> {
  const records: DeliveryRecord[] = [];
  for (const ch of channels) {
    const r = await deliverToChannel(env, ch, emission, egressAllowlist);
    records.push({
      channelId: ch.id,
      channelKind: ch.kind,
      delivered: r.ok,
      ...
      ...(r.resolvedScreen !== undefined ? { resolvedScreen: r.resolvedScreen } : {}),
      // unconfirmed rides on a delivered:true JSM/Opsgenie async-accepted (202) outcome.
      ...(r.unconfirmed === true ? { unconfirmed: true } : {}),
      // ackOutcome: the closed async-confirmation verdict alongside it, so a POSITIVELY-failed
      // async create is distinguishable from an unobtainable confirmation.
      ...(r.ackOutcome !== undefined ? { ackOutcome: r.ackOutcome } : {}),
      // The failing recipient's INDEX, on a failure only. The address stopped inside sendEmail.
...
//
// It is shared rather than copied (the seal path's routed alert calls into it too) because the failure
// handling here is the load-bearing part: it distinguishes "no channel was configured" (an honest
// nothing-to-deliver, not a lost alert) from a routing throw, and it AWAITS the history write so support
// can tell "the alert never went out" from "it went out and only its record was lost". A second copy
// would drift on exactly those distinctions.
//
...
export async function routeEngineNotification(
  ...
  try {
    ...
    const { now, egressAllowlist } = (await resolveResp.json()) as { now: NotifyChannel[]; digestedCount: number; emission: NotifyEmission | null; egressAllowlist?: string[] };
    ...
    const records = await deliverEmission(env, emission, now, egressAllowlist);
