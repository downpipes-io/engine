...
/**
 * HybridVerifyVerdict is hybridVerify's boolean, un-collapsed.
 *
 ...
 *   ed no,   ML-DSA ok   -> ed25519-only-mismatch: the mirror image. The ML-DSA half verified these
...
export async function hybridVerifyDetailed(v: HybridVerifier, message: Uint8Array, signature: Uint8Array): Promise<HybridVerifyVerdict> {
  try {
    if (signature.length !== ED25519_SIG_LEN + MLDSA_SIG_LEN) {
      // A signature OBJECT of the wrong length is a MALFORMED signature file (a truncated / half-written
    ...
    // DO NOT SHORT-CIRCUIT ON THE CLASSICAL HALF. Both halves sign the SAME message bytes, so the half
