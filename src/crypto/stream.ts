...
/**
 * Returns the next chunk counter, or throws when advancing would wrap the 8-byte index field and re-use an
 * earlier chunk's nonce under the same key. Ported from the Go reference's advanceChunk
 * (internal/crypto/stream.go:310), including its message, and for the same reason: it is the ONE place the
 * seal paths step the counter, so every path inherits the no-wrap, no-reuse invariant rather than relying on
 * a caller's own windowing to stay under it.
 *
 * The bound it enforces is unreachable in production today (a segment is capped at MAX_SEGMENT_CHUNKS =
 * 16384 by the caller's windowing in src/seal/record.ts, and 2^64 chunks of 64 KiB is 2^80 bytes), which is
 * exactly why it had gone unwritten on this side: section 10.3's assurance argument is that the TypeScript
 * sealer is a byte-for-byte port of the Go reference, and a chokepoint present on one side only is a hole in
 * that argument, not in today's archives.
 *
 * @param counter - the current zero-based chunk index.
 * @returns counter + 1.
 * @throws Error when counter is already at STREAM_MAX_CHUNK_INDEX.
 */
export function advanceChunk(counter: bigint): bigint {
  if (counter >= STREAM_MAX_CHUNK_INDEX) throw new Error(`stream would exceed the safe chunk count: chunk index ${counter} wraps the per-stream nonce counter`);
  return counter + 1n;
}
...
export async function openStream(fileKey: Uint8Array, sealed: Uint8Array, aad?: Uint8Array, maxChunks = 0): Promise<Uint8Array> {
  // Every abort below carries its locus on the fault ledger -- the chunk index, byte counts, and whether the
  // object was zero-byte, partially written, or authentically corrupt -- rather than surfacing as a bare
  // Error (or, for the GCM arm, a bare DOMException OperationError) with no locus at all. Ints only: never
  // the object key or a byte.
  ...
  return new ReadableStream<Uint8Array>({
    async pull(controller): Promise<void> {
      try {
        ...
        // The STREAMING open is the path a multi-GiB restore actually takes, and its aborts would otherwise
        // cross two async stream boundaries before anything could see them. Record the locus HERE, where the chunk
