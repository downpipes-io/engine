...
export async function openCapsule(wraps: CapsuleWrap[], priv: HybridRecipientPrivate, aad: Uint8Array): Promise<Uint8Array> {
  ...
  for (const w of wraps) {
    ...
    if (master.length !== 32) {
      // The capsule DEM authenticated and yielded a master of the wrong size. That is a corrupt capsule,
      // not a wrong identity, though it can otherwise look identical to a tamper. Ints only.
  ...
  // No wrap matches the identity held, which means the operator is holding the WRONG recipient identity for
  // this archive (a rotation, a restore from another tenant's run, a break-glass key where an operational one
  // is needed). It is NOT tamper and NOT a destination fault, though it can otherwise read as "integrity check
  // failed" and send support chasing corruption when the customer simply holds the wrong key. The two
  // FINGERPRINTS are of PUBLIC material (the class the pack already carries under keys.*), which
