...
// U64_MAX is the largest value the eight-byte big-endian encoding can carry (2^64 - 1), the same ceiling
// the Go reference names as streamMaxChunkIndex (internal/crypto/stream.go:28).
const U64_MAX = 0xffff_ffff_ffff_ffffn;

/**
 * Encodes a 64-bit unsigned integer as 8 big-endian bytes.
 *
 * DataView.setBigUint64 does NOT range-check: it truncates modulo 2^64 (measured on Node 22:
 * setBigUint64(0, 2n ** 64n) writes eight zero bytes, byte-identical to the encoding of 0n, and -1n writes
 * ffffffffffffffff). The only u64be caller in the seal path is the STREAM chunk nonce (stream.ts
 * chunkNonce), so a value that wrapped would re-emit a nonce this key had already used, which is the one
 * AES-GCM failure there is no recovery from. The Go reference refuses at the counter rather than wrapping;
 * this is the same refusal at the encoder, so both the seal and the open path inherit it whatever they
 * step. Every current caller passes a byte count derived from a JS number (< 2^53), so nothing in
 * production is near the bound and no archive byte moves.
 *
 * @param n - the value to encode.
 * @returns the 8-byte big-endian encoding.
 * @throws RangeError when n is negative or above 2^64 - 1, rather than silently wrapping it.
 */
export function u64be(n: bigint): Uint8Array<ArrayBuffer> {
  if (n < 0n || n > U64_MAX) throw new RangeError(`u64be: ${n} is outside the unsigned 64-bit range; DataView.setBigUint64 would wrap it modulo 2^64`);
 ...
 *
 * Every character pair is validated before decoding, closing a silent-coercion hole: `parseInt(pair, 16)`
 * alone returns NaN for a non-hex pair, and assigning NaN into a Uint8Array COERCES IT TO ZERO. Without the
 * validation, a corrupted or truncated segment object key ("seg/zz/zzzz...seg") would decode SILENTLY to a
 * run of zero bytes and flow on as a perfectly well-formed 48-byte segment id, which would then simply fail
 * to match anything -- reported to the operator as an ordinary "object missing". A malformed key and a
 * genuinely deleted object would be byte-identical in the pack, and the only fault with a fix (re-derive the
 * key) would be indistinguishable from the one without. Validating first means the reader's catch can class
 * it as malformed-object-key instead, and the pack can say which it is.
 *
...
export function hexDecode(s: string): Uint8Array<ArrayBuffer> {
  ...
  for (let i = 0; i < out.length; i++) {
    ...
    // corrupted object key like "seg/0g/..." is classed malformed rather than decoding to a
    // wrong-but-well-formed id. The message names the fault, never the value (the string can BE a key).
...
export function constantTimeEqual(a: Uint8Array, b: Uint8Array): boolean {
  ...
  // when a platform primitive runs without its owning object as `this`. That detached form passed the Node
  // test harness (which has no timingSafeEqual and takes the XOR fallback below) but broke EVERY secret
  // comparison in the live runtime: the admin token, the passkey assertion, the session MAC and the
  // recovery codes all route through here, so a detached call locked the whole engine out of auth.
