...
// @noble/post-quantum.
//
// The library is NOT constant-time-guaranteed, so what the engine does with it matters, and
// the honest answer depends on the posture. This comment used to claim "the engine
// encapsulates and signs only, so the trusted surface in production is encapsulate + sign",
// and that is true only in the break-glass-only posture. In the DEFAULT posture the engine
// holds OPERATIONAL_PRIVATE and decapsulates with it routinely: the restore drill, the hourly
// canary, verify-at-seal, in-console restore, the retention prune and the control-plane
// auto-heal all do. Decapsulation with a LONG-LIVED key is precisely the shape a timing side
// channel targets, so the caveat is load-bearing there rather than theoretical.
//
// Two things bound it, and neither is the false claim it replaced. An attacker needs to
// measure the engine's own timing from inside the customer's Cloudflare account, and anyone
// with that position already has the environment the key sits in. And break-glass-only removes
// the exposure by removing the key rather than by mitigating it. The Rust/WASM escalation
// remains the documented fallback if a constant-time implementation is required.
