...
export interface EmailResult {
  ...
  reason?: string;
  code?: string;
  // On an email-recipients-invalid failure, this carries the POSITION of the offending entry in the channel's
  // address list. It is an integer, never an address, and it is what lets support say "your third recipient is
  // the problem" instead of "one of your five addresses is wrong, find it".
...
//
// The rejection carries the recipientIndex -- the POSITION in the list of the entry that failed, never the
// address. One typo'd address in a five-recipient channel kills delivery to ALL FIVE (the send is one call),
// and without a position support could not name the bad entry: the customer had to eyeball five addresses and
// guess. An index is enough to point straight at it, and it is not customer data.
...
function validateEmailMessage(env: Env, msg: EmailMessage): { ok: true; payload: EmailPayload } | { ok: false; reason: string; recipientIndex?: number } {
  ...
  // Carry the failing entry's INDEX through, so the channel adapter can record it on the history row. The
  // address itself stops here and goes no further.
...
export async function sendEmail(env: Env, msg: EmailMessage): Promise<EmailResult> {
  try {
    ...
    // order to record the E_OTHER placeholder rather than silently dropping the fact.
