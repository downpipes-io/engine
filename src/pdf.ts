...
function coverContent(report: Report): string {
  ...
  // Descriptor under the title, wrapped to the column rather than painted as a single unwrapped run, so
  // a longer descriptor cannot overhang the right margin or collide with the rule below: the rule and the
  // panel below it move by however many lines it actually takes.
  ...
  out += textRun(left, footY, "Generated in your own account, and never built from the contents of your backups.", CAPTION_SIZE, MUTED, false);
  return out;
}
...
// ---- report-render health ------------------------------------------------------------------------------
//
// A lastRestoreTestAt that is not a finite epoch (a legacy string, a corrupt row, a NaN) would otherwise
// throw a RangeError out of `new Date(Number(r.lastRestoreTestAt)).toISOString()` deep inside PDF assembly,
// failing the customer's whole compliance-report download with no evidence anywhere of why, so the report
// they need for an audit could not be produced from a single corrupt row. The guard below renders "unknown"
// instead of dying, so one corrupt row cannot take the whole document down; the coercion is COUNTED, so a
// silently-degraded cell is not a silent fallback of its own. The tally is isolate-local (the renderer is a
// pure leaf with no env and no scheduler stub, exactly like sources/source-fault-ledger.ts) and the router
// drains it after each render and files it against the admin-counter aggregate.
...
// failure. Before this, a break-glass deferral printed as bare "fail" in the signed PDF (the same
// conflation the console fixed for lastRestoreTestDeferred elsewhere), which reads as "tested and failed"
// next to a compliance-stamp "Proven" cell that may correctly say the downpipe's recoverability IS proven --
// an auditor would otherwise see a contradictory row.
function restoreOutcomeLabel(r: Record<string, unknown>): string {
  ...
  // reads, so the distinction must survive into the signed PDF rather than collapsing to a bare "pass"
  // for this exact state. It says what was not established, not that anything failed.
  ...
  return [
    { kind: "heading", text: "Stated recoverability-proof interval" },
    {
      ...
      text:
        cadenceDays > 0
          ...
          : "This estate has not stated an interval for proving recoverability, so the recency below is reported without a target.",
    },
    { kind: "heading", text: "Per-downpipe restore-test recency" },
    {
      ...
      table: {
        ...
        // These widths are set deliberately rather than derived by proportional scaling: fittedColWidths
        // would shrink Proven to 146.7pt where the phrase needs 150.6pt at 9.5pt, wrapping "attended
        // verification, YYYY-MM-DD" mid-phrase. test/validate-reports.ts asserts that phrase reads as one
...
function slaComplianceBlocks(data: Record<string, unknown>): Block[] {
  ...
  return [
    ...
    {
      kind: "para",
      text: "Expected is the cadence's contractual minimum over the Window column, not the report's full period: a downpipe created (or, where no creation time was recorded, first observed running) partway through the period has its window clamped forward to that start.",
    },
  ];
}
...
function formatPeriod(period: { fromSeconds: number; toSeconds: number }): string {
  // Guarded for the same reason as the restore-test cell: a non-finite period bound (a malformed
