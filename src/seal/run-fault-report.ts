...
export function beginRunFaults(): void {
  ...
  // The seal-path observation ledger is cleared on the same run boundary and for the
...
function destSnapshots(dest: Destination | undefined): { faults?: unknown; io?: unknown } {
  ...
  // no failing op at all (a listing truncated without a cursor, a missing ETag on a conditional PUT, an engine
  // guard refusal). The anomaly half matters precisely because the run still reports ok, so gating the post on
  // total alone would drop exactly the evidence this exists to carry.
...
export async function reportRunFaults(scheduler: DurableObjectStub, downpipeId: string, dest?: Destination): Promise<void> {
  ...
  try {
    ...
    // The seal-path evidence detected deep in the PURE core (a record skipped as changed-mid-crawl and WHY,
    // destination bytes the engine stranded, the run's requested-vs-effective seal mode, the RUNLOG lock
    // plane's own faults) comes from modules that take a Destination and a budget, not a DO stub, so they NOTE
    // into the ledger and this -- the one place a crawl already ends -- posts it to the seal-fault ring.
...
async function postRunObservations(scheduler: DurableObjectStub, downpipeId: string, obs: RunObservations): Promise<void> {
  ...
  const at = Date.now();
  // One record per closed skip CAUSE -- the distinction between genuine content churn and a persistent
  ...
  // One record per stranding CLASS, with an integer COUNT (never a boolean).
  ...
  // The run's requested-vs-effective seal mode (the silent fan-out-to-serial downgrade).
  ...
  // The REFUSE-TO-SIGN completeness guards. These runs produce NO ARCHIVE, and without this the two counts
  // that size the shortfall (found vs expected) would exist only inside a raw thrown message that coarsens to
  // a generic failed run row. One record per guard that fired, each with its closed kind and its two integers.
  ...
  // The destination's RUNLOG is ABSENT while this downpipe has prior runs on it, so the signed root chains onto
  // NOTHING (prevRunId=null) and the history silently restarts -- what a downpipe repointed at a re-created or
  // wrong bucket looks like, and otherwise indistinguishable from a first run.
  ...
  // The RUNLOG lock / CAS plane counters. A non-zero count here means the SCHEDULER plane flaked, not the
