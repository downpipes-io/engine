...
// RunCheckpoint is the durable between-invocation state of a sliced run:
...
//
// partialRecord is the OPTIONAL mid-record-resume state: a single streamed-content
...
export interface CheckpointCounts {
  ...
  // KV key / R2 object DELETED between the list page and the GET -- a live-source race, SPEC 12.5). The
  // adapters skip these silently by design; counting them lets a run report "N objects vanished mid-crawl"
  ...
  recordsIncomplete: number; // sealed INCOMPLETENESS sentinels (_truncated/_unavailable/_skipped/_pending/_refused): a record landed but it is a marker, not the real bytes, so the archive is intentionally short of the live source
  ...
  // incompleteIds is the PER-KIND ATTRIBUTION companion to incompleteByMarker: for each marker kind,
...
// slices (mid-record resume). recordIndex is the id the partial record will commit
...
export interface RunCheckpoint {
  ...
  sliceCount: number;
  // The mid-record-resume prefix, or null when no record is part-sealed. It holds a
...
async function wrapKey(signerPrivateB64: string): Promise<CryptoKey> {
  ...
  // A SIGNER_PRIVATE that is present but MALFORMED (a truncated paste, a wrong encoding) strands every
...
// The two ways a master unwrap can fail are two DIFFERENT investigations, and a bare
// "checkpoint-unwrap-failed" row cannot tell them apart. The AEAD FAILING means the wrap key
...
export function validateCheckpoint(c: unknown): RunCheckpoint {
  ...
  // recordsIncomplete may be absent on a checkpoint persisted by an
  // in-flight run whose document predates this field. Default it to 0 rather than rejecting,
  // so an older checkpoint never strands a resuming run; the next slice's increments then accrue from 0.
  if (typeof counts.recordsIncomplete !== "number") counts.recordsIncomplete = 0;
  // recordsVanished may likewise be absent on an older checkpoint document.
  // Default it to 0 rather than rejecting (the same
  // forward-compatible discipline as recordsIncomplete), so an older document never strands a resuming run.
  if (typeof counts.recordsVanished !== "number") counts.recordsVanished = 0;
  // incompleteByMarker (the PER-MARKER breakdown) may likewise be absent on a checkpoint
  // persisted by an in-flight run whose document predates it. DEFAULT it to {} rather than
  // rejecting (the same forward-compatible discipline as recordsIncomplete above), so an older document never
  ...
  // incompleteIds (the PER-KIND attribution) may likewise be absent on a checkpoint persisted
  // by an in-flight run whose document predates incompleteByMarker. DEFAULT it to {} (identical forward-compatible
  // discipline), so an older document never strands a resuming run; the next slice's attribution then accrues from an
  ...
  // partialRecord may likewise be absent on a checkpoint persisted by an in-flight run whose
  // document predates it. DEFAULT it to null rather than rejecting (the same
  // forward-compatible discipline as recordsIncomplete), so an older document never strands a resuming
...
// are replaced by their AES-256-GCM wraps (NC-1): both embed the last-/part-committed record
...
export async function openOpenShardBatch(signerPrivateB64: string, runId: string, seq: number, wrapped: { iv: string; ct: string }): Promise<Record<string, unknown>[]> {
  ...
  const c = new ArrayBuffer(ct.length);
  new Uint8Array(c).set(ct);
  // A FAILED open-shard batch AEAD is a distinct resume fault from a failed MASTER unwrap (the carried
  // manifest lines cannot be re-read, so the final shard would be short), and it needs its own closed
  // sub-code so it is not indistinguishable from that other fault in the pack. The message is preserved verbatim.
  let plain: Uint8Array;
  try {
