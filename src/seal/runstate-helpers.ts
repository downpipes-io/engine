...
// worker-side seal entry points share. The seal DO and sealRunSliced/sealRunBuffered
...
import { maybeHeaderFaultDestination } from "../admin/test-faults.ts";
...
// It lives HERE (not in runstate.ts) because the seal DO's SLICED and FAN-OUT completion bodies must
// stamp exactly the same flag the inline path does -- one rule, not two, decides what a completion carries.
export function multipartAbortFlag(dest: Destination | undefined): { multipartAbortFailed: true } | Record<string, never> {
  ...
  // The run row's boolean says THAT parts were stranded, never HOW MANY, which alone cannot answer "why does the
  // archive bucket grow far faster than the data?". Record the COUNT alongside the (unchanged) boolean, in the
  // seal-fault ring's stranding class.
  if (n > 0) noteStranded("multipart-parts", n);
  return n > 0 ? { multipartAbortFailed: true } : {};
}

// downDestinationFlag names the destination PROVEN down by a failed run, so the scheduler records a
// lastOk:false reachability heartbeat and the map's per-destination indicator lights up for a SOLE-destination
// outage too, not just a fan-out replica. This keeps the sliced and fan-out completion paths at parity with the
// inline path, so the biggest customers' runs are never the ones whose destination outage fails to light the
// marker. Only on a DESTINATION-ACCESS classification (a source
...
// OPEN_PREFIX namespaces the RunSealDO's append-only OPEN-SHARD batches: the AES-256-GCM-wrapped
...
// A Durable Object storage list() returns at most one page (the platform caps it at ~1000
...
// shards, far beyond any real run), mirroring the scheduler DO's listAllByPrefix.
...
// The throttle ladder: a destination/source THROTTLE (503 SlowDown / 429) is handled on its OWN
...
export async function runSealVerification(ctx: SealVerifyContext, runId: string, plaintextBytes: number): Promise<SealVerification | undefined> {
  ...
  if (!verifyAtSealEnabled(env)) {
    // An ABSENT verdict would otherwise be ambiguous between "verification is switched OFF for this deployment"
    // and "this run predates the verdict field". Record the OFF posture once per run, so a suspect-free pack is
    // never mistaken for a verified one. A closed enum; nothing else rides.
  ...
  if (v.status === "suspect") {
    // The suspect verdict itself is already on the run row (status/tier/coarse reason). A diagnosis still needs:
    // WHICH of 900 shards, HOW MANY records verified clean before the fault (sampled is zeroed on a failure),
    // whether two recurring reader faults are the SAME fault (no digest), and what transiently broke on a
    // recovered verdict. Ordinals, counts, a hex digest and closed enums only -- never a shard id, an object key
    // or a reader exception.
    ...
    // The CRITICAL suspect-archive alert's routing is FAIL-OPEN, and it can die before or around the notify
    // history write, which would otherwise leave NO evidence that an attempt was even made. Record the failure,
    // and separate the two cases support cannot otherwise tell apart: the alert never went out at all
    // (routingFailed) vs it WAS delivered and only its history row was lost.
...
// verifyModeOf maps the verdict's TIER to the closed per-run verify mode, so the pack can say which
...
// requireEnv is the run path's env-var gate. An absent var is a TYPED config fault carrying the
...
// DESTINATION the run is sealing to (per-destination keying): finalisers writing different
...
export async function cfConfigToken(
  ...
  // Cross-account confused-deputy guard (ASVS V4): this is the run
  ...
  if (typeof accountId === "string" && accountId !== "") {
    ...
    if (!accountInDiscoveryScope(accountId, cfg)) {
      // TYPED so the pack can say "this run failed because its account is outside the discovery scope"
...
export async function sliceDepsFromEnv(env: Env, config: DownpipeConfig, budget: SliceBudget, opts: SliceDepsOptions = {}): Promise<SliceDeps> {
  ...
  // HARNESS-ONLY (HARNESS_TEST_FAULTS): consume a single armed dest-header-corrupt fault and, when one fired,
  // wrap the destination so THIS run's archive writes fault on a malformed/unexpected response header.
  // A no-op on every production/dev/uat/internal engine (the gate short-circuits before any DO
  // round-trip), so the seal path stays byte-identical there. Every seal entry funnels through here (inline
  // slice, seal-DO resume, fan-out worker/coordinator), and this builder is seal-only (restore/drill/verify
  // build their destination straight from the factory), so the fault can only ever ride a real seal WRITE.
  const dest = await maybeHeaderFaultDestination(env, await buildDestination(env, budget, opts.destOverride ?? null));
...
// Fan-out knob defaults + bounds. FAIL-SAFE OFF: SCALE_FANOUT_RANGES defaults to 1 (the serial
...
// sealLadderKnobSources reports the SOURCE of the fault-ladder + fan-out knobs. Every one of them
