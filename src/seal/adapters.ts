...
import { chaosKVBinding, chaosSourceFetch } from "../sources/chaos-fault.ts";
...
function mediaMetadataApi(env: Env, token: string, sourceType: string): ReturnType<typeof makeCfApi> {
  return makeCfApi(token, chaosSourceFetch(env, sourceType), accountPacer(env));
}
...
// SELF-IDENTITY. A backup only self-identifies -- and a roster rebuild after a deploy wipe only
...
/**
 * selfIdentityClass classifies ONE downpipe's archive self-identity from its stored config. PURE (no
...
/** selfIdentityDegraded is the boolean the roster carries: true when this downpipe cannot be
...
export function buildAdapter(env: Env, state: DownpipeState, cfConfigToken?: string): SourceAdapter {
  ...
  const guard = (b: string) => {
    ...
    // The refusal is TYPED, so the run path can record
  ...
  if (s.type === "secrets") {
    ...
    const bound: BoundSecret[] = list.map((sec) => {
      ...
      // A Secrets Store binding exposes get(); a plaintext env secret is the string itself. ANYTHING
      // ELSE (a KV namespace, an R2 bucket, a D1 database, a service binding accidentally bound under the
      // secret's name) must never be coerced with String(store) -- that would seal "[object Object]" as the
      // secret's value, every run would report a clean ok, and the corruption would surface only at restore, in
      // the disaster. REFUSE the record instead: a typed config fault names the binding (an operator label,
      // never the value) and the run fails loudly, so the pack carries a config-fault seal-fault record with
      // the closed code secret-binding-wrong-type rather than months of silently corrupt backups.
  ...
  if (s.type === "cf-config") {
    ...
    return new CloudflareConfigSource(cfConfigToken, s.accountId, s.zoneId, undefined, chaosSourceFetch(env, "cf-config"), undefined, accountPacer(env));
  }
  if (s.type === "workers") {
    ...
    return new WorkersSource(s.accountId, makeWorkersCfApi(cfConfigToken, chaosSourceFetch(env, "workers"), accountPacer(env)));
  }
  if (s.type === "stream") {
  ...
  // The owner's #1 fear (a wrangler deploy that wipes source bindings) lands HERE: the throw's
  // TYPE carries the binding NAME so the failed run names it.
  ...
  switch (s.type) {
    case "kv":
      return new KVSource(chaosKVBinding(env, binding as KVNamespace), s.namespaceId ?? s.binding!);
    case "r2":
