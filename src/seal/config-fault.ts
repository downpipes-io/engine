// CONFIG-FAULT EVIDENCE (support-pack mode run-config-fault-unnamed). Every run of a downpipe can
// start failing after a wrangler deploy, a token rotation or a discovery-scope change (the owner's confirmed
// #1 fear: a deploy that wipes source bindings). Today the failed run row carries only a COARSE class --
...
//   - secret-binding-wrong-type: a secrets source names a binding that IS present but is NEITHER a
...
//                               the secret's value on EVERY run and report a clean ok; the corruption would
//                               surface only in the disaster, at restore. The build REFUSES the
//                               record (this typed fault) rather than sealing the coercion, so the run fails
