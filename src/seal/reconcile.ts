...
//
// SCOPE: this module implements Phase A-D of the classifier, the per-class inventory, the mass-orphan
// CIRCUIT-BREAKER, and a per-pass READ-BUDGET. The enforced GC (the segment-GC protected-set subtraction,
// Phase E/F) and the salvage path both depend on a sharded-RUNLOG keystone and a topology-aware committed
// accessor and are not implemented here. Because this module proposes NO deletion at all, it is
...
// fraction is the signature of a wrong-source / version-skew read of the committed set). Both return
...
export interface OrphanRun {
  ...
  // reader-binding pre-flight; the report-only inventory does not act on it.
...
export interface ReconcileInventory {
  ...
  readonly circuitBreakerTripped: boolean; // true when the mass-orphan guard abstained the pass
  ...
  // distinguishes: a root whose SIGNATURE did not verify (or that could not be read as a trustworthy root at
  // all) is a TAMPER/corruption signal, while a signed-but-INCOMPLETE tree is a crashed finalise. Support could
  // not tell them apart without bucket access. Counts only; optional so a caller-built inventory (validators)
...
export interface ReconcilePolicy {
  ...
  readonly maxOrphanFraction?: number; // default 0.5; above this (orphans/physical) the pass abstains (the wrong-source-read guard)
  readonly maxClassifyPerPass?: number; // default 32; bounds the per-pass attest READ load - the rest defer to pendingClassify
}
...
// ULID_TIME_BYTES is the 48-bit (6-byte) millisecond timestamp prefix of a 16-byte binary ULID.
...
/**
 * The mass-orphan CIRCUIT-BREAKER (the catastrophic-case guard). A correct steady state has FEW
...
export interface ReconcileInventoryInput {
  ...
  // RUNLOG topology, which under a sharded-RUNLOG must ship in lockstep with the topology change.
...
export async function planReconcileInventory(input: ReconcileInventoryInput): Promise<ReconcileInventory> {
  ...
  // The two causes behind a "broken" verdict, counted at the classification site where the probe's
  ...
  for (const runId of orphanIds) {
    ...
    // Phase D read-budget: bound the per-pass probe (attest) READ load; the rest defer to the
...
/** The high-level three-way summary the operator reads first: the committed runs, the orphaned-but-RECOVERABLE
 * backups (salvageable + stale: each has a valid
...
export interface ReconcileSignal {
  ...
  readonly circuitBreakerTripped: boolean; // the mass-orphan wrong-source-read guard fired
  readonly deferred?: "circuit-breaker" | "runlog-unverifiable"; // set iff the whole pass abstained
  // ---- the distinctions the inventory already makes, folded away by the three-way summary -----
...
export function reconcileSignalFor(
  ...
  return {
    ...
    // Carry the inventory's own distinctions instead of folding them into two totals. All counts.
