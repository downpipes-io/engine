...
// read (_vanished -- a mid-crawl live-source race). The stream/images/artifacts/
...
// completion and the notification can say "completed with N items not fully captured".
//
// Classification is an ADAPTER ASSERTION, not a content-shape guess. Each emitter above sets
...
// field (ShardRecord.incompleteMarker), never by re-parsing decrypted bytes. Classifying by content shape
// alone is unreliable: a real customer value that happens to be a small JSON object carrying one of the
// MARKER_KEYS names (e.g. `{"_pending":false,"orderId":42}`) is indistinguishable from a genuine sentinel
// by shape, which would silently drop it on restore. incompleteMarkerKind/isIncompleteMarkerValue below
// remain a content-shape classifier ONLY (used by the format-level round-trip test,
// validate-incomplete-marker.ts); they are NOT consulted by the seal-time stamp or the restore-time
// write-skip gate, and must never be reintroduced as either one's authority.
//
...
// The caller pairs this value with `markerKind: "_vanished"` on the yielded SourceRecord (the adapter's
...
// IncompleteIds is the PER-KIND ATTRIBUTION companion to IncompleteByMarker: for each marker kind a
...
// have that shape. NOT AUTHORITATIVE: a real customer value can coincidentally have this exact
...
// ShardRecord.incompleteMarker) call this. It is kept as a pure content-shape classifier for the
...
// isIncompleteMarkerValue is the BOOLEAN view over incompleteMarkerKind: the same NOT-AUTHORITATIVE caveat
// applies. media-restore.ts isMarkerValue delegates here but is not the restore-side write-skip gate (see
// restore-apply.ts/restore-plan.ts, which read ShardRecord.incompleteMarker instead); this stays available
// for tests/tooling that want a pure content-shape check.
