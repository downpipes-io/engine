...
// path. The default 3 attempts / 150 ms (≈ 600 ms total) cannot
...
// throttleRetryKnobSources reports the SOURCE of the destination-throttle retry knobs. The
...
// isTransient is the default withRetry predicate. It DELEGATES to the shared destination-fault
...
// Keeping this one named classifier means which faults retry is decided in exactly one place.
...
//
// RETRY PRESSURE: every retry this loop actually TAKES is ticked into the per-isolate retry meter
