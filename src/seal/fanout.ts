...
// A SERIAL verdict carries WHY. A downpipe with fan-out ENABLED that silently downgrades to serial
// still takes days on a run the customer believes is parallel, and the pack would otherwise carry nothing at
// all (the fan-out knobs are deliberately omitted from sealKnobs and the run row has no mode field). The
// reason is a member of the CLOSED FanoutDowngradeReason vocabulary; it is absent when fan-out was simply
// never asked for.
...
export function addCheckpointCounts(a: CheckpointCounts, b: CheckpointCounts): CheckpointCounts {
  ...
  // incompleteIds (the per-kind ATTRIBUTION) is UNIONed the same way -- each kind's ids merged, deduped
  ...
  return {
    ...
    recordsVanished: a.recordsVanished + b.recordsVanished, // sum the mid-crawl vanished counts across workers
...
// ScratchShardError TYPES a refused scratch-shard read with the CLOSED kind the seal-fault ring
...
export async function mergeStep(deps: SliceDeps, cp: RunCheckpoint, master: Uint8Array, st: MergeState, carriedLines: Record<string, unknown>[], lock?: RunlogLock): Promise<MergeStepResult> {
  ...
  while (nextScratch < st.scratchShards.length) {
    ...
    // OBSERVE the refusal with the counts the merge already holds -- how many scratch shards merged
    // CLEANLY before this one, and how many the run declared -- then re-throw UNCHANGED. The refuse-to-sign
    // decision is untouched; this is the evidence that a run which produces NO ARCHIVE would otherwise lose entirely.
  ...
  if (globalRecordIndex !== cp.counts.records) {
    // The merged-vs-declared delta is what SIZES the shortfall.
  ...
  if (st.expectedRecordTotal !== undefined) {
    ...
    if (encountered < st.expectedRecordTotal) {
      // Encountered-vs-counted is the ONLY signal that separates the accepted false positive (a benign bulk
      // DELETION between the count pass and the crawl) from a real under-crawl. Both fail the run closed; only
      // the delta tells support which one it was, and it used to die with the exception.
  ...
  // restarts the history chain with prevRunId=null. Recorded evidence.
...
export async function deleteScratchShards(deps: SliceDeps, scratchShards: ShardEntry[]): Promise<void> {
  for (const s of scratchShards) {
    try {
      ...
      //
      // On a WORM / Object-Lock bucket this catch fires for EVERY scratch shard of EVERY fan-out run, so
      // the archive bucket grows far faster than the data -- permanently, and with no evidence anywhere unless
      // it is counted (a failed cleanup must never fail a sealed run, so the swallow stays; the count is what
      // makes "why is my bucket so big?" answerable).
