// CHECKPOINT-FAULT TYPES for support-pack mode checkpoint-corruption-unattributed. The LEAF that
// lets a resume fault name ITSELF instead of collapsing to a generic "run failed" / "checkpoint unwrap
// failed" in the pack.
//
// THE TICKET THIS EXISTS FOR: "a huge backup restarts from scratch every night and never completes."
// Nothing anywhere said (a) that a corrupt checkpoint was being ABANDONED, (b) WHICH field of it failed
// validation (a signer rotation, a tampered doc and a corrupted DO write are three different
// investigations), or (c) that N slices of real, durable progress were thrown away when it was.
//
