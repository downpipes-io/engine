// SEAL-FAULT OBSERVE POSTER (support-pack mode seal-fault-observe-dropped). Every seal-fault
...
// hold a drifted kind, and the DO re-validates + clamps everything through sanitiseSealFault regardless.
// Every field is already a closed enum, an id or an integer.
export interface SealFaultPost {
  ...
  rangeIndex?: number;
  discardClass?: string;
  // config-fault: the CLOSED prerequisite code + the offending binding / env-var NAME (an operator
  // label; the DO's sanitiser re-gates it to the bare identifier charset, so a value can never ride here).
  configCode?: string;
  bindingName?: string;
  // run-pressure: counts + closed classes for the retry / park / strike pressure one run spent.
  retries?: number;
  ...
  outcome?: "ok" | "failed";
  // Covers replica-target-fault, replication-pass, record-skipped-changed, dest-stranding, verify-suspect,
  // seal-mode, alert-routing-failed and lock-plane. Every field is already a closed enum, a customer-owned
  // opaque id, a hex handle or an integer; the DO's sanitiser re-gates every one of them regardless (this
  // module adds no trust).
  ...
  // Covers checkpoint-invalid / checkpoint-unwrap-failed / checkpoint-coerced / resume-abandoned, the
  // fan-out and open-shard completeness guards (which reuse found/expected/ordinal above), completion-lost
  // (which reuses outcome/causeDigest/attemptClasses above) and runlog-absent. Every field is a closed enum,
 ...
 * own ingestion losses. STRICTLY BEST-EFFORT and fail-open: it never throws, so a routing/persist hiccup
 * degrades to a counted "no observation" and can never affect the run. It reads/mutates no archive, RUNLOG
 * or seal path.
 *
