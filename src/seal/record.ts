...
//
// A value past the single-segment ceiling (16384 STREAM chunks, 1 GiB of plaintext per
// segment) is sealed as a CHAINED multi-segment record: the value is windowed, each
// window addressed and sealed as its own content-addressed segment exactly as the Go
// conformance writer chains (per-window segId over the window's own bytes, per-window
// file key, chunk indices restarting at 0 per segment), and the record line carries the
// ordered segments list with the WHOLE-record plaintext
...
export interface RecordSealDeps {
  ...
  // segmentTargetBytes caps one segment's plaintext (default and maximum: the 16384-chunk
  // ceiling). Validators lower it to exercise real chains without gigabyte fixtures,
  // which is legal: a segment is bounded at AT MOST 16384 chunks.
...
// resumes it. A skipped record is EXCLUDED from the manifest entirely; a partial record is
...
// two fresh opens. A short or long read means the object changed or vanished behind its etag pin, so the
...
async function sealSingleStreamRecord(deps: RecordSealDeps, r: SourceRecord, counts: SealCounts): Promise<SealRecordOutcome> {
  ...
  // Each withRetry on the data path names the subsystem it waits on (a closed enum), so the run's
  ...
  if (addr.size !== declared) {
    // The record is skipped and EXCLUDED from the archive, so a restore is legitimately missing it. An
    // aggregate skip count alone cannot distinguish a persistent infra fault from ordinary content churn, so
    // record the specific cause (a size that moved between the list and the seal).
...
// content-addressed segments. Bounded windows need a ranged open; a source that cannot
...
//
// MID-RECORD RESUME. When the slice budget runs low mid-record, the seal stops after the current
...
async function sealChainedStreamRecord(deps: RecordSealDeps, r: SourceRecord, target: number, counts: SealCounts, resumeFrom?: ResumeFrom): Promise<SealRecordOutcome> {
  ...
  if (resumeFrom !== undefined) {
    ...
    if (etag === undefined || resumeFrom.etag === undefined || etag !== resumeFrom.etag || openStreamedRange === undefined) {
      // Distinguish "the source has NO etag at all" (a legacy source that can never be resumed, so its
    ...
    try {
      ...
      // The resume's prefix re-hash THREW -- a 403 the token lost mid-crawl, a timeout, a 412. This is an
      // INFRA fault, and recording it as ordinary churn would let a persistent, fixable source-access
      // problem stay invisible behind an apparently healthy run.
  ...
  for (let offset = startOffset; offset < declared; offset += target) {
    ...
    const attempt = await withRetry(
      ...
      { subsystem: "source-api" }, // a windowed re-read waits on the SOURCE, not the destination
    );
    if (attempt.addr.size !== len) {
      // A window came back a different length than the chain expected: the object moved under the pin.
...
// the run, record and salt, so a secrets segment is always new by construction and the exists probe is
// skipped (one subrequest saved per secret; an address collision would need a salt collision).
