// RUN-PRESSURE EVIDENCE. A run that ultimately SUCCEEDS after riding out weeks of edge-of-budget 503/429
// retries and throttle parks leaves no trace: the retry counters live in a closure that returns, and the
// throttle/strike ladders live in the RunSealDO's transient doc, which cleanup() deletes the moment the run
// resolves. So "backups suddenly started failing" is diagnosed with no history of the pressure that preceded
// it, and on terminal failure the row cannot say whether the 8 strikes had ONE consistent cause or eight
// different ones, nor which subsystem was throttling.
//
// This module holds the pure half of that evidence: the CLOSED vocabularies, the pure classifiers that map a fault to a
...
/**
 * sealAttemptClass coarsens ONE caught slice fault into a CLOSED attempt class. It is the ONLY thing
 ...
 * ladder already holds ("runs take 5 hours some days" is undiagnosable without it). Reads the message
