...
// (fail-open), and posts the "ok" completion. The archive is COMPLETE from finaliseRun
...
async function completeInline(env: Env, scheduler: DurableObjectStub, deps: SliceDeps, state: DownpipeState, trig: RunTrigger, cp: RunCheckpoint, master: Uint8Array, newShards: ShardEntry[], openShardLines: Record<string, unknown>[], destinationId?: string): Promise<void> {
  // An inline run finalises in one invocation, so the open-shard buffer lives only in memory
  ...
  try {
    const resp = await scheduler.fetch(doURL("/complete"), {
      ...
      body: JSON.stringify({
        ...
        ...(Object.keys(cp.counts.incompleteByMarker).length > 0 ? { incompleteByMarker: cp.counts.incompleteByMarker } : {}), // per-marker breakdown (which kinds); only carried when non-empty
        ...(Object.keys(cp.counts.incompleteIds).length > 0 ? { incompleteIds: cp.counts.incompleteIds } : {}), // per-kind attribution (which surface); only carried when non-empty
    ...
    // A REFUSED completion (a non-2xx) never throws, so this arm would otherwise never see it and the run's
    // SUCCESS would drop in silence -- the archive is complete and the pack would show the row as "abandoned".
    ...
    // The archive is COMPLETE and the run is a SUCCESS, and without this its verdict simply evaporates: the
    // row resolves later as a generic "abandoned" and the pack contradicts a destination that restores fine.
    await observeCompletionLost(scheduler, state.config.id, trig.runId, "ok");
  }
}

// observeCompletionLost records a terminal outcome the engine COMPUTED and then FAILED TO DELIVER (the
...
// false backup failure or re-run a fully-sealed downpipe under a fresh master. The cost of that discipline is
// that the verdict can simply VANISH -- the row then resolves via lease reclaim as a generic "abandoned" for a
...
// refusedHandoff turns a REFUSED seal-DO handoff into a throw whose class is CLOSED. The DO answers
// a refusal with a FIXED, engine-authored error body; classifyHandoffRefusal reads it ONLY to select a member
// of HANDOFF_REFUSAL_CLASSES and returns that member, so the body never rides. A bare status form ("... : status
// 500") would fall through coarseRunError's generic status catch-all to "destination access error", which would
// blame the customer's bucket -- and light the map's down-destination indicator -- for a fault entirely inside
...
// of leaving the run dangling in-flight until the 30-minute lease reclaim.
async function handoffToDO(env: Env, state: DownpipeState, b: SliceBudget, cp: RunCheckpoint, newShards: ShardEntry[], openShardLines: Record<string, unknown>[], destOverride?: RuntimeDestConfig | null, destinationId?: string): Promise<void> {
  ...
  // The inline slice's leftover open-shard buffer (openShardLines) rides the handoff in memory
...
// handoffToCoordinator hands a fanned-out run to the per-downpipe COORDINATOR DO: it wraps the ONE
...
export async function sealRunSliced(env: Env, state: DownpipeState, trig: { runId: string; index: number; prevRunId: string | null }, opts: SealRunOptions = {}): Promise<void> {
  ...
  // be park-and-resumed on the DO ladder. They start at the initial (slice-0) checkpoint and
  ...
  // The inline slice's leftover open-shard buffer, tracked beside cp/resumeShards so a
  ...
  try {
    ...
    // Fan-out (opt-in via SCALE_FANOUT_RANGES): a high-cardinality LEX-ORDERED KV run is sealed by N
    ...
    // The REQUESTED vs EFFECTIVE seal mode. A downpipe with fan-out enabled that silently downgrades to
    // serial still takes days on a backup the customer believes is parallel, and without this the pack carries
    // NOTHING (the fan-out knobs are deliberately omitted from sealKnobs and the run row has no mode field).
    // Closed enums and small integers only; no key-space sample rides.
    ...
    if (decision.mode !== "serial") {
      ...
      // count-then-stride plan (no ranges yet). Either way the coordinator owns the run from here.
    ...
    // A THROTTLE (503/429) on this INLINE run is NOT a hard failure -- the inline path has no
    ...
    //
    // CLASSIFY THE ERROR, NOT ITS MESSAGE. This was the one call site in the engine that handed the shared
    // classifier `m` instead of `e`, and the object it discarded is where the throttle evidence lives: the
    // Retry-After tag a CF 429 carries (seal/retry.ts rateLimitError) and the status a CfApiError carries as
    // a field. Both of those are gone from a plain string. A Cloudflare 429 that arrived WITH error text
    // (`why` is the joined CF text, and only falls back to "HTTP <status>" when CF sent none) therefore read
    // as NOT a throttle and failed the inline run hard, instead of parking it for the patient ladder that
    // would have resumed the same run when the rate-limit window passed.
    ...
    try {
      await scheduler.fetch(doURL("/complete"), {
        method: "POST",
        // When the seal faults on the DESTINATION (a single-dest downpipe never probes/fails over, so a
        // sole-destination outage would otherwise surface ONLY as a failed run while the map reads healthy),
      ...
      // The run FAILED, the engine knows why, and the verdict did not land. The row would otherwise resolve
      // as a generic "abandoned" via lease reclaim, which reads as "we lost track of it" rather than "it failed
      // for THIS reason". Record the computed verdict + its closed class so the real cause survives the lost POST.
...
// postBufferedOk verifies the just-written archive (fail-open) and posts the "ok"
...
// never touches them.
//
// DELIBERATELY NOT master-threaded (an opkey-reduction decision). The other three
...
async function postBufferedOk(env: Env, scheduler: DurableObjectStub, dest: Destination, state: DownpipeState, trig: RunTrigger, summary: RunSummary, destinationId?: string): Promise<void> {
  ...
  const resp = await scheduler.fetch(doURL("/complete"), {
    ...
    body: JSON.stringify({
      ...
      ...(summary.recordsVanished > 0 ? { recordsVanished: summary.recordsVanished } : {}), // KV/R2 objects deleted between list and read (mid-crawl); only when non-zero (mirrors the sliced path)
      recordsIncomplete: summary.recordsIncomplete, // records sealed as incompleteness sentinels (not the real bytes)
      ...(Object.keys(summary.incompleteByMarker).length > 0 ? { incompleteByMarker: summary.incompleteByMarker } : {}), // per-marker breakdown (which kinds); only carried when non-empty
      ...(Object.keys(summary.incompleteIds).length > 0 ? { incompleteIds: summary.incompleteIds } : {}), // per-kind attribution (which surface); only carried when non-empty
      ...(sealVerification !== undefined ? { sealVerification } : {}),
  ...
  // A REFUSED ok completion for a FULLY SEALED buffered archive never throws, so the caller's catch would
  // never see it either, and the verdict would simply vanish.
  if (!resp.ok) await observeCompletionLost(scheduler, state.config.id, trig.runId, "ok");
}
...
async function sealRunBuffered(env: Env, state: DownpipeState, trig: { runId: string; index: number; prevRunId: string | null }, opts: SealRunOptions = {}): Promise<void> {
  ...
  // The sealed summary of a run that COMPLETED. It is held OUTSIDE the try/catch because posting the ok
  // completion must NOT sit inside the failure catch: postBufferedOk performs a verify + a network POST, and a
  // fault in either caught below the try would BOOK THE RUN AS FAILED for an archive that is fully sealed,
  // signed and in the bucket. That is the false-failure half of the "run shows failed but the archive restores
  // fine" case, and it is a correctness fault, not just a reporting one. The seal and the completion are
  // two separate phases with two separate failure meanings.
  ...
  beginRunFaults();
  try {
    // Liveness preflight (buffered path): prove each binding-backed source's resource still exists
    ...
    try {
      await scheduler.fetch(doURL("/complete"), {
        method: "POST",
        // Name the sole destination down on a destination-access fault (see the sliced path above).
      ...
      log("error", `run ${state.config.id} (${trig.runId}) failure completion did not land: ${await redactedRunError((ce as Error).message)}`);
      // As on the sliced path -- the computed FAILED verdict did not land, so record it on the ring.
      await observeCompletionLost(scheduler, state.config.id, trig.runId, "failed", { cause, error: e });
    }
  } finally {
    // The per-run master and the per-run signer load are ended on EVERY exit, here in the function's only
    // `finally`, matching the sliced path's discipline and what validate-master-zeroisation asserts: zeroising
    // only at the end of the try and the head of the catch would leave an early `return` in the try able to
    // return with a live master. Zeroised FIRST, so a throw from the drain below cannot strand them.
    clock.master.fill(0);
  ...
  // The archive is SEALED. Completing it is a SEPARATE phase whose failure is a completion fault, never a
  ...
  if (sealed !== undefined) {
    ...
    // VERIFICATION -- which notes into the format/crypto integrity ledger. Drain again so that evidence still
    // reaches the pack, since the verify runs after the try's own drain rather than inside it. The
    // reporter never throws, posts nothing when clean, and the ledgers it drains are already empty on a healthy
    // run, so this costs a warm isolate nothing.
    await reportRunFaults(scheduler, state.config.id, dest);
  }
}
