...
// The sliced seal: a run that cannot fit one invocation's subrequest and
...
// The open-shard-spanning shard reduction pushes this ceiling from ~22M to ~500M records (unreachable on the
...
// PAD10_LIMIT bounds the per-RANGE local record index of a fan-out WORKER. A composite recordId
...
interface SliceCtx {
  ...
  randomNonce: () => Uint8Array;
  shardMax: number;
  // rangeIndex is this slice's fan-out worker tag, copied from cp.rangeIndex: undefined on a
...
interface SliceState {
  ...
  // Mid-record resume: when this slice left a streamed record part-sealed, its prefix is recorded
  ...
  // True once an in-crawl shardMax flush fired this slice, which means the PRIOR open-shard
...
async function flushShard(ctx: SliceCtx, st: SliceState): Promise<void> {
  ...
  st.shardIndex++;
  st.lines = [];
  // A flush consumed the open-shard buffer (which was seeded with the carried prior batches),
...
async function sealOne(ctx: SliceCtx, st: SliceState, record: SourceRecord): Promise<void> {
  ...
  // The sentinel KIND, when this record IS a marker, comes from the source
  ...
  if (markerKind !== undefined) {
    ...
    // A _vanished marker is ALSO the distinct "mid-crawl deletion" signal. It seals + counts like any
    ...
    // Attribution (all source types): record WHICH surface/object was short. safeMarkerAttribution is the
...
async function crawlResumable(ctx: SliceCtx, st: SliceState): Promise<boolean> {
  ...
  for await (const ev of ctx.deps.source.crawlFrom(selector, ctx.cp.cursor, ctx.deps.budget)) {
    if (ev.kind === "record") {
      ...
      // A record left mid-record-partial STOPS the crawl HERE, before its trailing mark is consumed,
...
// changed so the owner can update its append-only wrapped batches:
...
//
// Open-shard spanning: the open shard is NOT closed at every slice boundary. priorOpenLines is
...
export async function runSlice(deps: SliceDeps, cp: RunCheckpoint, master: Uint8Array, priorOpenLines: Record<string, unknown>[] = []): Promise<SliceResult> {
  ...
  const ctx: SliceCtx = {
    ...
    sealDeps: {
      ...
      meter: deps.budget,
      // The mid-record yield signal IS the slice's own shouldYield (the same boundary the crawl
  ...
  const st: SliceState = {
    // Seed the open-shard buffer with the carried prior lines, so the in-crawl shardMax flush
  ...
  // INVARIANT: a mid-record partial ALWAYS stops the crawl (crawlResumable returns false the moment
  ...
  // The unconditional end-of-slice flush is REMOVED. The open shard is NOT closed at the slice
  ...
  const checkpoint: RunCheckpoint = {
    ...
    // Carry forward THIS slice's mid-record partial (null when no record is part-sealed). The ...cp
    ...
    // The current open-shard buffer size (all carried prior lines plus this slice's, minus any
  ...
  // Describe the open-buffer change for the storage owner. A flush this slice consumed the prior
...
// the archive. It first seals the carried OPEN shard (openShardLines = the lines
...
export async function finaliseRun(deps: SliceDeps, cp: RunCheckpoint, master: Uint8Array, allShards: ShardEntry[], openShardLines: Record<string, unknown>[], lock?: RunlogLock): Promise<{ rootWritten: true; shards: number }> {
  ...
  // Seed the signed root's freshness.prevRunId from the destination-local RUNLOG (the SAME source
  ...
  // A prior run SUCCEEDED (the scheduler's prevRunId) yet this destination's RUNLOG has no entry to
  ...
  // The recovery bundle makes the destination self-sufficient.
  ...
  // the SAME destination-local prev (localPrev, not cp.prevRunId), so root and entry agree.
...
// error. It lives here so the worker entry and the seal DO share one vocabulary.
export function coarseRunError(m: string): string {
  ...
  // key on a status/code/keyword in the message) can ever match it, and it used to fall all the way through
  // to the generic "run failed", losing the destination attribution entirely. The wrapper's prefix is fixed
  // and self-describing (R2Destination is the only producer of it), so this is checked FIRST, ahead of even
  // the RUNLOG Layer 1d handling below: a native fault ON the RUNLOG object must read as a destination fault,
  // never as "runlog write contended" (an operator retrying CAS contention expects it to clear; an auth/
  // capability fault never will). Mapped onto the SAME "destination access error" class an HTTP-attributed
  // destination fault gets -- no new vocabulary member, and the run row still names the destination as the
  // cause rather than the undifferentiated "run failed".
  ...
  // A finalise that REFUSED to sign a truncated archive (the completeness guard: fewer shards
  ...
  // ---- The LOUD, SELF-DESCRIBING refusals below are distinguished from a generic "run failed" (or, worse, from a
  // MISATTRIBUTED "destination access error"). Each of these is an engine refusal that already knows exactly
  // what went wrong; the vocabulary below is what carries that knowledge into the pack's failed run row. The
  // classes are CLOSED and carry no operator data: the raw messages can embed a record name or an object key,
  // so they stay digest-only (causeDigest), and only these fixed classes ride.
  //
  // A SEAL-DO HANDOFF REFUSAL is checked first, and the ordering is load-bearing: its message ends in
  // "status 500", which the generic status catch-all at the bottom would otherwise classify as a DESTINATION
  // ACCESS ERROR -- so a fault entirely inside the engine's own control plane would blame the customer's
  // bucket and light the map's down-destination indicator. The sub-class is lifted out of the message
  ...
  // The binding is present but the underlying resource (KV namespace / R2 bucket / D1 database)
...
// redactedRunError is the LOG form of a run fault: the coarse class plus a short
