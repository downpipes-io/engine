...
interface CompleteOptions {
  ...
  // destination-attributed signals the inline path stamps: the stranded-multipart-parts boolean
...
interface RunDoc {
  ...
  // is persisted. The in-memory RunCheckpoint with the plaintext cursor exists
  ...
  // runlogAttempt is the SEPARATE patient ladder for RUNLOG write CONTENTION: when another
  ...
  destinationId?: string;
  // Open-shard spanning: the half-open live range [openBatchSeqStart, openBatchSeqEnd) of
  ...
  // coordinatorId is set ONLY on a fan-out WORKER doc: the per-downpipe DO id of the COORDINATOR
  ...
  // pressure accumulates the retry / park / strike cost this run has already paid, so it SURVIVES
...
// ABSENT = a SCAN plan where the namespace is larger than the front sample, so the coordinator
...
// RangeFailed is the WORKER -> COORDINATOR terminal-failure report (POST /range-failed). It carries the same
// evidence set an INLINE failure does, because the fan-out population is otherwise the hardest to diagnose:
//   - reason      : the coarse, closed failure class the worker's ladder resolved (coarseRunError).
...
export interface CoordinatorDoc {
  ...
  // The PLAN phase (the SLICED, keys-only balanced planner). planScan is the bounded-memory
  ...
  throttleAttempt?: number;
  // The no-progress strike ladder (await phase). progress[i] is range i's worker-liveness FINGERPRINT
...
export interface StartSliceRun {
  ...
  shards: ShardEntry[];
  // The inline first slice's leftover open-shard buffer (plaintext lines, in-memory transit on a
...
// The per-downpipe seal Durable Object, which dispatches sealing to per-downpipe workers rather than
// running it inside the scheduler DO. A run too large for one invocation is handed here after its
...
export class RunSealDO {
    ...
    try {
      ...
      switch (`${req.method} ${url.pathname}`) {
        case "POST /start": {
          ...
          // The inline slice's leftover open-shard buffer rides the handoff in memory; wrap it into
          // an open-shard batch (seq 0) HERE, at the storage boundary, exactly as the cursor is wrapped.
          // Its length must equal the checkpoint's recorded open-shard count, or the handoff is
          ...
          if (openLines.length !== (cp.openShard?.count ?? 0)) {
            // The handoff is internally inconsistent, so the run dies here, before a single DO slice runs.
            // The two counts that size the refusal are recorded, so the pack shows more than a bare
          ...
          // ones, batched into one bulk delete. The enumeration is PAGED with a startAfter cursor:
          ...
          if (staleKeys.length > 0) {
            ...
            // shard-truncation-stalekeys). The cleanup itself stays integrity-load-bearing;
        ...
        case "GET /status": {
          ...
          return json({
            ...
            attempt: doc.attempt,
            // throttleAttempt completes the worker-liveness fingerprint a fan-out COORDINATOR polls:
      ...
      // The body carries a fixed class, never the raw message (which can embed a storage key or
  ...
  async alarm(): Promise<void> {
    ...
    // Fan-out dispatch: a coordinator doc (kind:"coordinator") drives the spawn/await/merge state
    // machine; a worker doc (coordinatorId set) seals its range and reports /range-done; everything else is
    // the ordinary SERIAL alarm below.
    ...
    // the orphan run-tree it wrote before the RUNLOG append failed. Stays undefined
    ...
    try {
      ...
      dest = deps.dest;
      try {
        // The stored cursor is unwrapped only here, inside the invocation. A failure to unwrap the
        // checkpoint under the CURRENT signer is OBSERVED (signer-rotation-strands-runs) then re-thrown so the
        // existing strike ladder still fails the run loudly; the observe never changes that outcome.
        ...
        // Zeroise the per-invocation signer load on every path (sealRunBuffered discipline; the
      ...
      // Name the missing prerequisite BEFORE the ladders redact the cause. A config fault (a wiped source
      ...
      // routing below is otherwise untouched.
      await this.observeConfigFault(scheduler, e, doc.checkpoint.downpipeId, doc.checkpoint.runId);
      // RUNLOG CONTENTION: a typed RunlogContendedError means another finaliser is writing the
      ...
      // than shedding ~1/3 of a trigger storm to terminal failure. Checked FIRST so a contended write is
      ...
      // Append this strike's coarse class to the run's ordered, capped list before the ladder charges it,
      // so a terminal failure can say whether the strikes had one consistent cause (a rotated credential) or
  ...
  // identity fields) then RE-THROWS unchanged, so the existing strike ladder resolves the run normally.
  // The observe is best-effort + fail-open, so it can never turn a strand into anything worse.
  private async openCheckpointForResume(scheduler: DurableObjectStub, signerKey: string, doc: RunDoc): Promise<{ cp: RunCheckpoint; master: Uint8Array }> {
    // The SILENT COERCIONS, checked first. validateCheckpoint DEFAULTS a missing counter rather than rejecting it
    // (so an engine upgrade never strands an in-flight run) -- but that makes a LEGACY doc (the counter predates
    // the field: benign) and a CORRUPTED doc (the counter came back a non-number and is being silently coerced:
    // a miscounted archive) look identical. The inspector is PURE and runs BEFORE the in-place defaulting; it reads
    ...
    try {
      ...
      // This resume is DEAD, and the run will restart from ZERO next cadence. Record three things: WHICH field
      // of the checkpoint was malformed (a corrupted / tampered DO write), or WHICH unwrap
  ...
  // the DO side re-validates + clamps + drops an out-of-vocabulary kind. The fire-and-forget POST's own
  // failure is not silent: postSealFault tallies a dropped observation (transport / unknown-kind) and
  // flushes it into the ring as an "observe-dropped" record, so a clean ring stays distinguishable from
  // a ring whose writes are being lost.
  ...
  // observeConfigFault records WHICH prerequisite a run died for: a source binding a deploy wiped, a
  ...
  // moment the run resolves. It fires on BOTH outcomes: a run that succeeded after riding out a
  // multi-hour throttle is exactly the evidence the "backups suddenly started failing" ticket needs, and it is
  // evidence that would otherwise vanish with the doc cleanup(). A run that spent nothing records nothing, so the
  ...
  private async runSliceStep(signerKey: string, deps: SliceDeps, budget: SliceBudget, doc: RunDoc, cp: RunCheckpoint, master: Uint8Array): Promise<RunCheckpoint | null> {
    // Liveness preflight: on the FIRST slice of a run (no records committed yet), prove the bound
    ...
    if (hasLivenessProbe(deps.source)) {
      if (cp.sliceCount === 0 && cp.cursor === null) {
        ...
        // Resume slice: re-probe so a source resource DELETED between slices (a namespace,
        // bucket or database dropped mid-run -- the "deleted mid-process" case) is named
        ...
        try {
          ...
          // The SWALLOW is deliberate (a probe blip must not make a long run fragile), but left uncounted it would
          // be SILENT: a source that is flapping for hours would show up nowhere until a run finally dies of it.
          // Count the flap on the run's pressure (a bounded integer, never the probe's error) so "backups suddenly
          // started failing" carries the run-up. Counting cannot change the swallow.
          doc.pressure = withProbeFlap(doc.pressure);
        }
      }
    }
    // Load the carried open-shard buffer (the wrapped batches in the live range) and seed
    // runSlice with it, so the in-crawl shardMax flush accounts for records sealed in earlier slices and
    // triggers at the full 5000-record shard threshold rather than at one slice's own record budget.
    ...
    // exactly where the run-fatal transport class and crawl stage live. reportRunFaults never throws
    ...
    // Reconcile the open-shard batches with the slice's outcome. A flush (openReset) consumed the
    ...
    // Drain this slice's retries out of the isolate meter and carry the run's accumulated pressure into
  ...
  // page limit. A single un-paged storage.list() returns at most one page (~1000 keys), so a
  ...
  // scheduler DO's listAllByPrefix.
  ...
  // loadOpenLines reads the live open-shard batches [seqStart, seqEnd), unwraps each (the AEAD is
  ...
  private async loadOpenLines(signerKey: string, runId: string, seqStart: number, seqEnd: number, downpipeId?: string): Promise<Record<string, unknown>[]> {
    ...
    for (let seq = seqStart; seq < seqEnd; seq++) {
      ...
      if (wrapped === undefined) {
        // DO storage lost a wrapped open-shard batch from INSIDE the live range, so the records it held
        // can never be sealed and the run refuses to sign. The two integers that size the refusal
        // (which sequence went missing, where the live range ends) are recorded on the ring, not just in the thrown message.
  ...
  // (fail-open: a suspect verdict flags the row + alerts but the run still completes
  ...
  private async finaliseAndComplete(scheduler: DurableObjectStub, deps: SliceDeps, doc: RunDoc, cp: RunCheckpoint, master: Uint8Array, signerKey: string): Promise<void> {
    // Sort by NUMERIC id: the signed root lists shards in seal order, and a paged DO-storage
    ...
    // Completeness guard: the run flushed exactly cp.nextShardIndex shards (sequential ids
    ...
    if (cp.nextShardIndex > 0 && shards.length < cp.nextShardIndex) {
      ...
      // throw. This only records the two counts the completeness guard
    ...
    // Load the carried OPEN shard (the records buffered across slices but not yet flushed) and
    ...
    if (openLines.length !== expectedOpen) {
      // The OPEN-shard half of the completeness guard. The serial shard guard above already observes its
      // found/expected into the ring; this observes the carried buffer that spans slices, so a run that
      // refuses to sign a short archive for this reason still leaves evidence behind.
    ...
    // PER-DESTINATION lock keying: key the RUNLOG lock by the destination this run sealed to
    ...
    // runs AFTER that window and notes into the same ledger: the RUNLOG lock/CAS plane counters and the
    // absent-RUNLOG chain restart are both detected here; without this drain, on the sliced path they would be
    // cleared by the next run's begin before ever reaching the pack. Never throws; posts nothing for a clean finalise.
    ...
    // A run that SUCCEEDED under pressure is the one whose evidence would otherwise be lost entirely (cleanup()
    // is one line away). Record it before the doc is deleted. A clean run records nothing.
  ...
  private async handleThrottle(scheduler: DurableObjectStub, doc: RunDoc, m: string, dest?: Destination): Promise<void> {
    ...
    // Name WHO asked the run to slow down (destination / source API / scheduler). The classifier reads the
    ...
    if (throttleAttempt >= maxYields) {
      ...
      try {
        ...
        // The run rode the WHOLE parking window and was abandoned for a sustained outage. Losing that
      ...
      // The run is being abandoned after riding the WHOLE parking window. Record how long it waited and on
  ...
  // handleContention rides RUNLOG write CONTENTION on its OWN patient ladder, mirroring
  ...
  // checkpoint. Striking such a run toward terminal failure would misclassify ordinary contention as a fault:
  // in a large concurrent storm against one destination it would shed a big share of runs loud, orphaning
  // committed bytes for no reason. So contention rides runlogAttempt, which NEVER charges the
  ...
  private async handleContention(scheduler: DurableObjectStub, doc: RunDoc, m: string, dest?: SliceDeps["dest"]): Promise<void> {
    ...
    if (runlogAttempt >= maxYields) {
      ...
      await this.observeSealFault(scheduler, {
        ...
        // The closed refusal class (worm-locked / access-denied / transient), the LIST-failed sentinel (so
        // reclaimed:0 is not ambiguous between "nothing was there" and "we could not look"), and the count
        ...
        // The strand CLASS, which is the diagnosis, and the two cases are not the same fault. A delete
      ...
      // A run given up after the whole RUNLOG parking window. Record the park count (the scheduler is the
  ...
  private async reclaimOrphanRunTree(dest: SliceDeps["dest"] | undefined, runId: string): Promise<{ reclaimed: number; wormBlocked: boolean; refusalClass?: WormRefusalClass; listFailed?: boolean; stranded: number }> {
    ...
    let stranded = 0;
    // refusalClass distinguishes worm-locked / access-denied / transient rather than collapsing them into one
    // wormBlocked boolean: folding "access denied" and "status 403" into the WORM arm would report a ROTATED or
    // UNDER-SCOPED credential to support as "locked by design, nothing to fix" when the real fault is a key the
    // operator could fix in a minute. The strictest class
    ...
    try {
      ...
      // Without this, an empty reclaim would read IDENTICALLY whether nothing existed or the LIST call failed --
      // so a permanently-leaking run-tree would look like a clean one. listFailed is that distinction.
  ...
  private async handleHardFault(scheduler: DurableObjectStub, doc: RunDoc, m: string, dest?: Destination): Promise<void> {
    doc.attempt += 1;
    // The raw message can embed a record name; the log line carries the coarse
    ...
    if (doc.attempt >= MAX_SLICE_FAILURES) {
      ...
      try {
        ...
        log("error", `sliced run ${doc.checkpoint.runId} failure completion did not land: ${await redactedRunError((ce as Error).message)}`);
        // The run struck out, the engine knows WHY, and the verdict did not land -- so the row resolves via
      ...
      // The run has struck out. Record the whole pressure history (retries, parks, the ordered strike
  ...
  private async complete(scheduler: DurableObjectStub, cp: RunCheckpoint | StoredRunCheckpoint, status: "ok" | "failed", opts: CompleteOptions = {}): Promise<void> {
    ...
    // The coarse failure class the row records. Hoisted because the down-destination attribution keys on
    // it: only a DESTINATION-ACCESS class may name a bucket down, never a source or config fault.
      ...
      body: JSON.stringify({
        ...
        ...(status === "ok"
          ? {
              ...
              ...(cp.counts.recordsVanished > 0 ? { recordsVanished: cp.counts.recordsVanished } : {}), // in-scope objects deleted between list and read (mid-crawl); only when non-zero
              recordsIncomplete: cp.counts.recordsIncomplete, // records sealed as incompleteness sentinels (not the real bytes)
              ...(Object.keys(cp.counts.incompleteByMarker).length > 0 ? { incompleteByMarker: cp.counts.incompleteByMarker } : {}), // per-marker breakdown (which kinds); only when non-empty
              ...(Object.keys(cp.counts.incompleteIds).length > 0 ? { incompleteIds: cp.counts.incompleteIds } : {}), // per-kind attribution (which surface); only when non-empty
              ...
              // The inline ok body stamps the stranded-multipart-parts boolean; a SLICED run that
          ...
          : {
              ...
              // A sliced run whose seal faulted ON THE DESTINATION names that destination down, so
    ...
    // A REFUSED completion (a non-2xx: the scheduler DO answered and rejected it) does not throw here (a
    // completion blip must never re-run a sealed downpipe under a fresh master), so none of the fail-soft catch
    // arms around this call ever see it. Left unrecorded, the row would later resolve as a generic "abandoned"
    // with no way to tell whether the archive actually restores fine, so the verdict is recorded on the ring
  ...
  // ---- Fan-out: COORDINATOR + WORKER roles -----------------------------------------------------
  ...
  // phase and spawns its workers now. A SCAN plan (body.ranges absent: the run is
  // larger than the front sample) persists a PLAN-phase doc and lets the alarm run the sliced,
  ...
  private async handleStartFanout(req: Request): Promise<Response> {
    ...
    if (body.ranges !== undefined && body.ranges.length > 0) {
      ...
      const doc: CoordinatorDoc = {
        ...
        phase: "await",
        attempt: 0,
        progress: ranges.map(() => null), // per-range last-polled liveness fingerprint
        rangeStrikes: ranges.map(() => 0), // per-range consecutive no-progress tick count
  ...
  // rather than leave a half-spawned run reporting ok. (One worker per range is the current model; a
  // bounded-worker work queue draining the ranges would be the natural next step -- see fanout.ts FANOUT_MAX_RANGES.)
  ...
  private async handleRangeDone(req: Request): Promise<Response> {
    ...
    const scheduler = schedulerStub(this.env);
    // OBSERVE every DISCARDED worker report (support-pack mode fanout-report-discarded). Each discard
    // below is CORRECT -- it is what makes /range-done idempotent -- but without this, a fan-out run that hangs
    // (or whose counts look short) because reports are being dropped would leave NO trace anywhere. Recording the
    // closed class + the range index makes it diagnosable. Best-effort + fail-open: recording it never changes the discard.
  ...
  // spawning just the failed range under the shared master -- is not implemented; this fails the whole run closed instead.)
  private async handleRangeFailed(req: Request): Promise<Response> {
    ...
    if (doc?.kind !== "coordinator" || doc.phase !== "await") {
      ...
      // a run that completed ok while a worker was reporting a terminal range failure is worth seeing.
    ...
    try {
      // The FAILED fan-out row reads like a failed inline row. The worker's 12-hex causeDigest
      ...
      // destination down; a stranded multipart abort is flagged. Each value is RE-GATED here at the
  ...
  private async workerAlarm(doc: RunDoc): Promise<void> {
    ...
    // signals an inline failure does. Undefined when the invocation faulted before the destination was
  ...
  private async rangeFinalise(deps: SliceDeps, doc: RunDoc, cp: RunCheckpoint, master: Uint8Array, signerKey: string): Promise<void> {
    ...
    const expectedOpen = cp.openShard?.count ?? 0;
    // The WORKER's completeness guards give a fan-out range the same observability the serial finalise has. A
    // range that refuses to report itself fails the WHOLE run (the coordinator can never merge), and without
    // these, both refusals would leave only a raw message that coarsened to a generic failed row.
    const scheduler = schedulerStub(this.env);
    if (openLines.length !== expectedOpen) {
  ...
  private async reportRangeFailed(doc: RunDoc, reason: string, m: string, dest?: Destination): Promise<void> {
    try {
      // The worker computes the 12-hex causeDigest of the RAW fault HERE, at the detection point (the raw
  ...
  private async coordinatorAlarm(doc: CoordinatorDoc): Promise<void> {
    ...
    // merge failure to it, exactly as the serial alarm does.
    let dest: Destination | undefined;
    try {
      if (!(await this.heartbeatBase(scheduler, doc.base))) {
        ...
        // OBSERVE the clean abandon (support-pack mode lease-lost-abandon), matching what a SERIAL resume already
        // posts: without this, the fan-out population -- the biggest customers -- would have lease-reclaim events
        // that leave no record at all. Best-effort + fail-open: recording it never changes the abandon.
      ...
      if (doc.phase === "plan") {
        await this.planTick(scheduler, doc); // one sliced count/stride step, then spawn or downgrade
        return;
      }
      if (doc.phase === "await") {
        await this.awaitTick(scheduler, doc); // poll worker progress, strike a stall, re-arm/fail-closed
        return;
      }
      const signerKey = requireEnv(this.env.SIGNER_PRIVATE, "SIGNER_PRIVATE");
      const budget = budgetFromEnv(this.env);
      ...
      try {
        ...
        try {
          ...
          // The merge is where a fan-out run REFUSES TO SIGN -- a missing / corrupted scratch shard, a
          ...
          try {
            ...
            // The merged archive is COMPLETE -- root signed, bundle written, RUNLOG appended -- and the ok
            ...
            // re-throw so the ladder behaves normally.
  ...
  // planTick advances the SLICED, keys-only balanced plan by one budget's worth of list pages.
  ...
  // await ladder (an unreachable worker strikes the run closed) rather than leaving the coordinator inert.
  ...
  // RunDoc -- reusing the SAME wrapped master and run identity -- so the next alarm runs the ordinary serial
  ...
  // awaitTick is the coordinator's WAITING step. Without it, re-arming the heartbeat unconditionally would let
  // a worker stalled in an UNCATCHABLE wedge (a CPU/subrequest hard-kill loop -- the KV watermark-rescan wedge --
  // that never trips the worker's own strike ladder) leave the coordinator awaiting forever while its heartbeat
  // kept the lease owned:true, with no recovery backstop (unlike a dead serial worker, which stops heartbeating
  // so the lease expires and is reclaimed). So each tick polls every not-yet-done range's worker for a liveness
  // fingerprint and STRIKES any range whose worker persisted NOTHING since the last poll. A range frozen for
  // MAX_SLICE_FAILURES consecutive ticks fails the WHOLE run closed (mirroring the serial strike-out): a failed completion (empty runId =>
  ...
  private async awaitTick(scheduler: DurableObjectStub, doc: CoordinatorDoc): Promise<void> {
    ...
    if (stalled >= 0) {
      ...
      // OBSERVE WHICH range wedged (support-pack mode fanout-range-stalled). The completion row can only
  ...
  private async completeFanout(scheduler: DurableObjectStub, doc: CoordinatorDoc, dest: Destination | undefined, sealVerification?: SealVerification): Promise<void> {
    const resp = await scheduler.fetch(doURL("/complete"), {
      ...
      body: JSON.stringify({
        ...
        // recordsVanished (in-scope objects deleted mid-crawl), the per-marker incompleteness breakdown and its
        // per-kind attribution ride on the fan-out completion exactly as they do inline, each carried only when
        // non-zero / non-empty and matching the inline body byte for byte -- so a fanned-out run (the biggest
        // customers' runs) reports the same shortfalls honestly instead of looking artificially whole.
    ...
    // A REFUSED ok completion for a MERGED archive (root signed, RUNLOG appended, bytes in the bucket) is
    // recorded the same way the serial path records one, on the population that can least afford to lose it.
    if (!resp.ok) {
      await this.observeSealFault(scheduler, { kind: "completion-lost", at: Date.now(), downpipeId: doc.base.downpipeId, runId: doc.base.runId, outcome: "ok" });
    }
  }
  ...
  private async coordinatorFault(scheduler: DurableObjectStub, doc: CoordinatorDoc, m: string, dest?: Destination): Promise<void> {
    if (m.includes("RUNLOG") || /(status|HTTP) (503|429)/.test(m)) {
      ...
      if (throttleAttempt >= maxYields) {
        ...
        try {
          // Stamp the 12-hex causeDigest of the raw fault on the failed row (never the message itself), so a
          ...
          // The fan-out population is exactly the one whose lost verdicts were never recoverable.
    ...
    if (doc.attempt >= MAX_SLICE_FAILURES) {
      ...
      try {
        // Coarse class + causeDigest + down-destination attribution + the stranded-parts flag,
        // the same four signals an inline strike-out posts. A merge that struck out on the destination lights
        ...
        // As above: the merge struck out with a known coarse class and the verdict evaporated.
        await this.observeSealFault(scheduler, { kind: "completion-lost", at: Date.now(), downpipeId: doc.base.downpipeId, runId: doc.base.runId, outcome: "failed", causeDigest: await causeDigest(m), attemptClasses: [sealAttemptClass(new Error(m))] });
      }
      await this.cleanup();
      return;
