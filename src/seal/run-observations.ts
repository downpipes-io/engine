...
// COMPLETENESS_MAX bounds the refuse-to-sign note list. A run refuses ONCE (the guard throws), so one
// entry is the real case; the cap is the abuse ceiling.
const COMPLETENESS_MAX = 8;

// CompletenessKind is the subset of SEAL_FAULT_KINDS a REFUSE-TO-SIGN completeness guard raises. Each
...
/** One refuse-to-sign observation: a closed kind, two counts, and (for a scratch-shard fault) the
...
export interface RunObservations {
  /** {closed skip cause -> count} for records skipped as changed-mid-crawl. */
  skipsByCause: Partial<Record<SkipCause, number>>;
  /** Up to 25 one-way `h:<12 hex>` handles of the skipped records. Never an object name. */
  skipHandles: string[];
  /** Partial (mid-record) seals ABANDONED, whose already-written segments are orphaned. */
  partialsAbandoned: number;
  /** {closed stranding class -> count} of destination objects/parts the engine stranded. */
  strandsByClass: Partial<Record<StrandClass, number>>;
  /** The seal mode the downpipe REQUESTED and the mode the run actually ran in. */
  ...
  /** The RUNLOG lock / CAS plane counters. A blip here is a SCHEDULER-plane fault, not a destination one. */
  lock: { acquireFaults: number; windowExhausted: number; casExhausted: number; sigPublishSkipped: number; releaseFaults: number };
  /** The REFUSE-TO-SIGN completeness guards' counts. A run that produced no archive at all carries these
   * integers here rather than only inside a raw thrown message that coarsens to a generic failed run row. */
  completeness: CompletenessNote[];
  /** The destination's RUNLOG was ABSENT while this downpipe has prior runs on it, so the signed root
   * chains onto nothing (prevRunId=null) and the history silently restarts. `priorRuns` is the run index
...
/**
 * noteSkippedChanged records ONE record skipped as changed-mid-crawl. The CAUSE is what makes a
...
/** notePartialAbandoned records a multi-GiB partial seal the slice ABANDONED: its already-written
...
/** noteStranded records destination bytes the engine stranded: a scratch shard whose delete failed, a
...
/** noteSealMode records the run's REQUESTED vs EFFECTIVE seal mode: the silent fan-out-to-serial
...
/** noteLockPlane bumps one RUNLOG lock / CAS plane counter. These faults coarsen to the DESTINATION
...
/**
 * noteCompleteness records ONE refuse-to-sign completeness guard. The guard's DECISION is unchanged --
 ...
 * the signed root then carries prevRunId=null and the history chain silently restarts, which is what a
