...
// Verify-at-seal: right after a run seals successfully (archive +
...
// covers EVERY record (full coverage), not just a strided sample. Below this the metered
...
// re-read, so a very large run's at-seal verify cannot trip the platform subrequest cap re-reading
...
// object can briefly read as a 404 or a stale prior version, so a verify
...
// verifyAtSealOnce uses at lines 291/301-303 and maxAttempts at 179) so the support pack can surface the
...
// sealVerifyKnobSources reports the SOURCE of every verify-at-seal knob. resolveSealVerifyKnobs
...
// causeDigest is the 8-hex FNV-1a CORRELATION handle of a RAW failure text. It is the SAME idiom the
...
function storeOver(dest: Destination): ObjectStore {
  return {
    ...
    // Expose the destination's LIST so attestKeyless's >900-shard sampled branch can cross-check
...
async function verifyAtSealOnce(env: Env, dest: Destination, runId: string, plaintextBytes: number, master?: Uint8Array): Promise<SealVerification> {
  ...
  // Tier-0 (ALWAYS): keyless attestation of signature, completeness and anti-rollback freshness.
  ...
  if (tier0.fault !== null) {
    // The coarse reason is unchanged; the DIGEST rides beside it so two recurring Tier-0 faults can be
  ...
  // Full coverage for a small run: when the plaintext is within SEAL_VERIFY_FULL_BYTES the
...
async function tier0Attest(store: ObjectStore, runId: string, verifier: HybridVerifier, shardCheck: "full" | { sampleAbove: number; sample: number }): Promise<{ fault: { reason: string; digest: string } | null; shardCount?: number }> {
  // tag pairs a coarse reason with the correlation digest of the RAW attestation text. The raw text is
...
async function decryptVerify(env: Env, store: ObjectStore, runId: string, verifier: HybridVerifier, sampleN: number, fullCoverage: boolean, at: number, master?: Uint8Array): Promise<SealVerification> {
  ...
  // `sampled` is ZEROED on a suspect verdict, so a 50,000-record run's failure could not say whether it
  ...
  try {
    ...
    // The ORDINAL (an integer index, never a key), the records that verified CLEAN first, and the
