...
export interface Env {
  ...
  // The per-downpipe seal Durable Object: a run too large for one
  ...
  CPU_MS?: string;
  // Fan-out knobs, all optional and FAIL-SAFE OFF. SCALE_FANOUT_RANGES is the maximum number of
  ...
  // SCALE_FANOUT_SAMPLE_CAP is the DIRECT-PLAN threshold: a run whose whole keyspace fits this
  ...
  // ORPHAN_RECONCILE gates the orphan-reconcile DRY-RUN INVENTORY cron pass (cron/reconcile-pass.ts).
  // DEFAULT OFF: the pass is a no-op unless this is set to an affirmative
  ...
  // holds: once the row count exceeds it, the DO rolls over the oldest rows first, the
  ...
  // Verify-at-seal. After a run seals successfully, the engine reads the
  ...
  // segment bytes rather than copying them blind, mirroring the VERIFY_AT_SEAL idiom. It catches
  ...
  // lower-assurance fallback bearer token.
  ...
  // even if ADMIN_TOKEN is configured. An Enterprise tenant that has stood up Access can set this
  ...
  // DEMO_MODE, when set to a truthy string ("1"/"true"/"yes"/"on"), enables the DEMO-ONLY reset surface
  // (POST /admin/demo/reset), which wipes the scheduler Durable Object back to first-run so a throwaway
  // demo deployment can be reset and re-walked repeatedly (e.g. to test upgrades/new versions). It is a
  // DESTRUCTIVE capability, so it is HARD-GATED: when DEMO_MODE is absent/falsey the route 404s BEFORE any
  // auth, production never sets it, so production has no reset surface at all. When set, only the
  // ADMIN_TOKEN break-glass bearer may invoke it (decoupled from the wipeable passkey/role identity). It
  // is set ONLY on the demo engine (wrangler.demo.toml), NEVER on a real deployment. See src/admin/router.ts.
  DEMO_MODE?: string;

  // HARNESS_TEST_FAULTS, when set to a truthy string ("1"/"true"/"yes"/"on"), enables the HARNESS-ONLY
  // engine test-fault-hook (the owner-gated /admin/test-fault/* surface, src/admin/router-test-fault.ts), so
  // the self-update fault-provocation journeys can provoke engine-INTERNAL faults that
  // page.route and the harness-sink cannot reach: a settle canary forced DEAD, an auto-rollback deploy forced
  // to THROW, an unhealthy hourly canary, and a dropped source binding. It is HARD-GATED exactly like
  // DEMO_MODE: when absent/falsey the /admin/test-fault/* routes 404 BEFORE any auth, so production, dev, uat
  // and internal engines have no fault surface at all, and every injection seam (settle verdict, settle
  // binding diff, hourly canary) short-circuits before any Durable Object read, so the path is byte-identical.
  // When set, the surface is additionally Owner-gated, and each fault is single-shot, self-clearing, and
  // affects only the NEXT matching update op on that one estate. It is set ONLY on the harness `channel`
  // estate, NEVER on a real deployment. See src/admin/test-faults.ts + src/sched/scheduler-do-test-fault.ts.
  HARNESS_TEST_FAULTS?: string;

  // Optional release provenance the console's provenance card surfaces. These are public,
  ...
  // API cross-origin (e.g. https://downpipe-console.provii.au). Only this exact origin is
  // allowed; no wildcard.
  ...
  // for that exact host). Set it explicitly to the shared registrable parent (e.g. "provii.au") ONLY
  ...
  // Object plaintext floor" (design ENTERPRISE-UX-BLUEPRINT §2.4) to an encrypted floor whose
  ...
  // Destination backpressure pacing (dest/pace.ts). DEST_RATE_PER_SEC is the steady request rate
  ...
  // DEST_THROTTLE_MAX_YIELDS bounds the park-and-resume window (seal/runstate.ts): how many
  ...
  // The deeper in-slice retry budget for the destination THROTTLE/transient class (seal/retry.ts
  // DEST_THROTTLE_RETRY). DEST_THROTTLE_ATTEMPTS / DEST_THROTTLE_BASE_MS let a store with a
  ...
  // CONTROL_PLANE_EXPORT_SEALING_DISABLED: sealing is ON BY DEFAULT -- the cron writes the SEALED
  ...
  // (fail-closed; enable only after the OA-3 sandbox experiment); "off" skips the read-back.
  UPDATE_READBACK_MODE?: string;
  // UPDATE_CHANNEL_MAX_AGE_DAYS (OPT-IN, default OFF) is the freshness backstop: when set to a positive
