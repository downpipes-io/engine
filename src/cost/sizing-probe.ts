...
// estimate shows a real number with no typing. Analytics-first
...
// metadata-listing fallback or drop the variant when that work is scheduled.
...
// empty"). That is a completely different fact from an honestly unavailable one: collapsing the two into
// identical evidence (a 0 and a basis) would let a broken discovery token, or a Cloudflare schema drift,
// show the customer an empty account and invite them to conclude they had nothing to back up.
//
...
export async function estimateSourceSize(source: SizingSource, deps: SizingDeps): Promise<SizeEstimate> {
  // Every exit notes its closed outcome class. The measured-zero arm is the one that matters: an
