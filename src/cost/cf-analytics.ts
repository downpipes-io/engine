...
// BEFORE the first backup runs, without reading a single value (analytics-first by design). It runs
// in the customer's own account (no-custody is inherent) and
// reads only sizes and counts, never content.
//
...
import { CF_GRAPHQL_URL as CF_GRAPHQL } from "../lib/outbound.ts";
...
// CfAnalyticsFault is WHY a size could not be measured. A bare null collapsed four different
...
export function makeCfAnalytics(token: string, fetchImpl: typeof fetch = fetch): CfAnalytics {
  // query POSTs a GraphQL request and returns the `data` payload, or the CLOSED fault class. It reads
  ...
  // missing or wrong-shaped is itself a SCHEMA DRIFT, not an empty source -- an important distinction.
