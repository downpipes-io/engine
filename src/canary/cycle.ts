...
async function openAndVerify(ctx: CycleCtx, prefix: string, baseDest: Destination, signer: Signer, master: Uint8Array): Promise<Step<Run>> {
  // A break-glass-only engine holds no in-account read-back key, and used to stop here: four aspects
  // reported skip and the flight finished ailing "no-read-back-key", forever, on every flight. That was
  // a false alarm dressed as a posture note -- the canary seals its OWN synthetic corpus and still holds
  // that flight's per-run master, which opens this cell and nothing else. So the read-back now runs in
  // every posture, and the ailing-by-posture branch is gone.
  //
...
async function decryptAndCompare(ctx: CycleCtx, opened: Run, baseDest: Destination): Promise<Step<{ plaintext: Uint8Array; corpus: CanaryRecord }[]>> {
  ...
  try {
    ...
    // destination blip a data-integrity DEATH and fire the evacuation alert, and now that the
    // break-glass-only posture reaches this code at all (it used to stop before the read-back) that
    // would be an hourly false alarm on estates that were previously immune to any dead verdict.
