...
//
// The prose nets alone got one of those actions exactly backwards. reader.ts raises
// freshnessUnverifiableError when the anti-rollback check COULD NOT RUN, and its message reads
// "freshness: ...", which the first net below matches. So a destination-side fault reading one
// destination's own _RECOVERY/RUNLOG was reported as a data-integrity DEATH: it paged an operator to
// evacuate, and it rolled back a healthy engine version, over a run whose bytes nothing had questioned.
// That category exists precisely so a caller can tell "the check found a rollback" from "the check could
// not run"; this caller threw the whole error away and kept its text.
//
// A check that could not run is an availability fault about ONE destination, so it is ailing. A check
// that RAN and found a rollback, and any integrity failure, stays a death. format-unsupported is also a
// death: the canary reads back an archive this same build has just written, so a build that cannot read
// its own writer is not a transport problem.
