...
//
// A coarse "unreachable" | "other" split would stamp every ailing flight "unreachable" regardless of what
// actually happened, making an expired credential, a WORM refusal, a throttling store and a black-holed
// endpoint indistinguishable and sending triage to the wrong side of the problem. The actionable split
// lives one directory away in dest/classify.ts and is shared with the cron fault ledger and the SIEM push
// pass.
//
...
//   probe-mismatch  the destination ANSWERED and returned the wrong bytes. That is not a "down" reason at
//                   all, and calling it unreachable was the most misleading case of the old vocabulary: the
//                   store is up and is giving back something other than what it was given.
...
// like deadReason.
//
// "no-read-back-key" is deliberately absent: a break-glass-only engine that could not decrypt its own
// canary cell would finalise every flight ailing by posture, forever. The flight reads back with its own
// per-run master (canary/cycle.ts), so this failure mode has no producer, and the type omits it rather
// than carrying a promised-but-empty class, matching the precedent in admin/support-sections-runs.ts.
...
// (the bird fell dead, or a dead bird recovered) at a point in time. The canary pages exactly ONCE per transition (a persistent death does not
...
export interface CanaryView {
  ...
  // destination it named is deleted. resolveEffectiveDests then DROPS it silently and the canary simply
  // stops proving that destination. Without this field the console has no way to say so: the pin
  // vanishes from `dests` and the aggregate keeps reading healthy for the destinations that remain, so an
  // operator who deliberately pinned a cold-archive bucket can lose that proof and see nothing. An
  // all-dangling case leaves a fault-ledger entry, but a partial dangle leaves no trace at all, and
  // neither reaches a screen.
  //
