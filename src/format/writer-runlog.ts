...
export function parseRunlog(bytes: Uint8Array): RunlogEntry[] {
  ...
  // A RUNLOG that will not parse is the CORRUPTION half of the tamper-vs-corruption question: it throws a
  // message naming the failing field, and support needs to say WHICH LINE of the document is bad, not just
  // that the whole account's seals are failing. The line ORDINAL (an int) and the
  // document's line COUNT are the locus; the closed parse-field kind is the mode. The line's CONTENT -- which
  // is a customer's run ids, downpipe ids and timestamps -- never rides, and the throw is unchanged.
