...
// across many invocations without ever materialising the leaf list.
//
...
export class MerkleFrontier {
  ...
  // All three refusals are SEAL-CHECKPOINT CORRUPTION on resume: left unclassified, they would kill a
  // multi-invocation run on every retry while the pack shows only a coarse run failure. The corrupt DO state
  // is transient (the next resume re-reads it), so support cannot otherwise see remotely that the CHECKPOINT,
  // not the source, not the destination, is the thing failing, nor whether the drift is one leaf or
  // wholesale. The ints below say exactly that; the serialised frontier blob itself never rides.
