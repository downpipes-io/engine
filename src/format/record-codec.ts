...
export function segmentChunkBound(recordId: string, chunkRange: [number, number]): number {
  ...
  // Each refusal below files a chunk-range locator carrying the bare ordinals into the integrity fault
  // ledger before throwing, rather than coarsening to an undifferentiated "integrity check failed". The
  // chunk ordinals are bare ints and the record id is NOT carried (it is a customer key: the caller's
  // digest, not the name).
  if (first !== 0) {
...
export function capStream(max: number, recordId: string): TransformStream<Uint8Array, Uint8Array> {
  ...
  return new TransformStream<Uint8Array, Uint8Array>({
    transform(chunk, controller): void {
      seen += chunk.length;
      if (seen > max) {
        // An inflate running past the DECLARED plaintext size is either a corrupt gzip member or a
        // decompression bomb; the two byte counts filed below are the whole answer, rather than an
        // undifferentiated "integrity check failed".
 ...
 * malformed-object-key locator and a shard-hash fail stage into the integrity fault ledger first, so a
 * damaged KEY is distinguishable from a missing OBJECT (see the note in the body). The decode error is
 * rethrown unchanged rather than wrapped.
 */
export function segIDFromObject(object: string): Uint8Array {
  const base = object.slice(object.lastIndexOf("/") + 1).replace(/\.seg$/, "");
  // hexDecode rejects a non-hex pair rather than coercing it to a zero byte (crypto/bytes.ts), so a
  // corrupted object key cannot silently produce a well-formed-looking all-zero segment id that then
  // matches nothing and reports to the operator as an ordinary "object missing". The two are
  // distinguishable: malformed-object-key means the KEY is damaged (re-derive it); absent means the
  // OBJECT is gone.
  let raw: Uint8Array;
  try {
    raw = hexDecode(base);
