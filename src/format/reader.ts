...
export class Run {
  ...
  restoreRecordStream(rec: ShardRecord): ReadableStream<Uint8Array> {
    ...
    // module-level helper). A generator error (a missing object, a GCM failure, a
    ...
    // The codec + final-hash pipeline is finaliseStream.
...
// concatenated: each chunk flows straight downstream. Extracted from restoreRecordStream so the
// per-segment decrypt loop is independently testable.
...
// failure aborts the restore. Extracted from restoreRecordStream.
...
// It is an explicit parameter rather than something inferred, because getting it wrong is silent: the first
// version of the ownership fix below zeroised unconditionally, and the negative cases in
// validate-restore-from-master.ts then wiped the caller's valid master before the positive cases ran.
async function openRunWith(store: ObjectStore, runID: string, verifier: HybridVerifier, opts: { verifyFreshness?: boolean; checkRecoveryBundle?: boolean } & FreshnessOptions, acquireMaster: AcquireMaster, ownsMaster: boolean): Promise<Run> {
  // runID is interpolated raw into the object-key templates below; for an S3-style
  ...
  // Recovery-bundle verification runs first, before the run-specific chain, so a
  ...
  // Canonical-numeric gate (SPEC 11.3). Runs after the signature check so that a
  ...
  // that fails verification, a freshness refusal), and before this catch existed each of those returned with
  // a live master and no reference left to reach it. On success ownership passes to the Run, whose dispose()
  // ends it; a finally here would therefore be wrong, because it would wipe the master the caller just
  // received.
  try {
    ...
    if (opts.verifyFreshness) {
      ...
      // rollbackDetected is unconditional, never masked by allowStale (a chain anomaly or a
      ...
      //
      // The !checked arm exists because this line once gated on two fields that a shared failure helper set
      // to "clean" for the five worst inputs, and with allowStale on (which every engine caller passes) an
      // absent or forgery-signed RUNLOG opened the run. A future path that establishes nothing and forgets
      // the unconditional signal is refused here rather than opened. Restated because the two are easy to
      // conflate: allowStale is an acknowledgement about the AGE of a run, never about the integrity of the
      // document that dates it.
      //
      // It is raised FIRST and with its own category, and it is no longer redundant. It still
      // refuses on exactly the states it refused on before -- every unchecked result also carries
      // rollbackDetected true, so nothing that used to refuse now opens -- but the category it throws tells
      // the restore side WHICH KIND of refusal this is. A check that could not run is an unknown about ONE
      // destination's own `_RECOVERY/RUNLOG` (which appendRunlog writes and signs per destination), so the
      // 3-2-1 walk may try a replica whose check CAN run; a check that ran and found a rollback is a finding
      // about the run and stays terminal on every destination. Both refuse here. See integrity-error.ts.
...
// structural format violations the Go reader's openShards tail enforces. Extracted from openRun so the
// per-shard open loop is independently testable.
...
// the signed root. Extracted from openRun so the completeness and hash checks are independently testable.
