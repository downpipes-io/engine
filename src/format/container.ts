...
function unframe(magic: Uint8Array, b: Uint8Array): Uint8Array {
  // All three refusals are CONTAINER FRAMING faults, distinct from a record whose bytes were tampered
  // with, so each is classified separately rather than folded into one generic "integrity check failed"
  // reason. A short container is a TRUNCATED object (the store ate the write); a bad magic is the wrong
  // object entirely; an unsupported version is an engine ROLLBACK reading a newer archive. Three different
  // fixes. The receivedBytes int is the truncation magnitude; no key and no byte of content ever rides.
