...
export interface KeylessAttestation {
  ...
  reason: string | null;
  // WHAT complete:true actually claims. An unqualified `complete` is three different strengths of claim:
  // every shard read back and hashed (full); a strided sample plus the per-shard PRESENCE pass, so a
  // durably missing shard outside the sample is still caught (sampled); or a strided sample with NO presence
  // pass at all, because the store cannot list() (sampled-no-presence) -- in which case a MISSING shard
  // outside the sample reads as complete, and the post-incident question "why did the verify say complete when
  // a shard was gone?" had no answer anywhere. Absent when the completeness pass did not run (the signature
  // failed, or the manifest is structurally invalid), which is honest rather than a fabricated "full".
...
function coarseKeylessReason(e: unknown, runID: string, ctx: string): string {
  ...
  let reason: string;
  // The TYPED category is read before any prose net: a version refusal can be worded "formatVersion ... is
  // missing the major.minor.patch components", and the `is missing` net below is a substring match, so an
  // archive written by the engine our own update channel recommends would otherwise be attested with the
  // reason "object missing". That is worse than an inaccurate sentence: REASON_OBJECT_MISSING is in the
  // replica-fallback set, so the answer would point the caller at a 3-2-1 replica that holds the same signed
  // run and the same label and could only refuse again. Reading the category the gate already set removes
 ...
 *   downpipe. minRunlogIndex is an out-of-band account-global RUNLOG high-water mark (e.g. the
 ...
 *   allowStale; omitted, behaviour is unaffected. shardCheck defaults to
 ...
 *   so a VERY large run does not trip the platform subrequest cap re-reading thousands of shards:
...
export async function attestKeyless(store: ObjectStore, runID: string, verifier: HybridVerifier, opts: { allowStale?: boolean; shardCheck?: "full" | { sampleAbove: number; sample: number }; minRunlogIndex?: number } = {}): Promise<KeylessAttestation> {
  ...
  // Same shared-chokepoint shape gate as openRun (reader.ts) -- runID feeds the identical
  ...
  // The coverage mode this completeness pass ACTUALLY achieved, decided below and returned beside the
  ...
  try {
    ...
    if (full) {
      ...
      // Bounded form for a very large run: the count cross-check first (NO read; the root signature
      ...
      // PRESENCE pass: the strided GET sample below only spot-checks min(sample,N) shards, so a
      ...
      // WHICH of the two sampled modes this is matters: with list() the presence pass runs and a missing
      // shard outside the strided sample is still caught (sampled). WITHOUT it the pass is skipped and
      // complete:true becomes a WEAKER claim -- only min(sample,N) shards are looked at, so a durably
      // missing shard elsewhere in the run reads as complete. Recording the coverage mode as a closed enum
      // on the attestation and a row in the run's integrity evidence means the post-incident question
      // ("why did the at-seal verify say complete when a shard was gone?") always has an answer.
  ...
  // Record the DEGRADED coverage modes into the run's integrity evidence (noteAttestCoverage is a no-op
  ...
  try {
    ...
    // rollbackDetected is unconditional (never masked by allowStale), so a chain anomaly or a
