...
//
// The old classifier was a brittle keyword regex over the raw reader message. Several genuine
// integrity/completeness failures the reader throws (e.g. "recovered N records, the root declares M",
// a codec/preamble structural mismatch) did NOT match the regex and DEFAULTED to the catch-all
// "recovery check failed" reason, which IS in the replica-fallback set: a truncated / incomplete /
// corrupt archive could therefore wrongly fall back to a replica instead of failing loud. Making the
// reader raise this typed error removes the guessing: any RunIntegrityError is NON-fallback by
// construction, regardless of its message.
