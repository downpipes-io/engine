...
//     "RUNLOG absent": support tells the customer the object is gone while it is visibly in the bucket;
...
//     failed", and each one sends the customer down a completely different path;
//   - the throw site NAMES the failing shard / record / segment and the declared-vs-recovered counts, and all
//     of it is discarded, so a ONE-RECORD hash mismatch and a WHOLESALE truncation look identical;
//   - five untyped throws in the verify paths misclassify an INTEGRITY fault as AVAILABILITY, so the pack
//     records a class that is not merely coarse but WRONG, and the 3-2-1 replica fallback engages against a
//     damaged signature object;
//   - a record with neither value nor stream is defaulted to ZERO BYTES, sealed, signed and reported green:
//     months later the restore yields nothing and no record anywhere says the engine held empty content at
//     capture time;
...
//     no leg, so support cannot locate the offending object without re-running with instrumentation.
//
...
// ---------------------------------------------------------------------------------------------------------
// the destination READ fault behind an "absent/missing" verdict
// ---------------------------------------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------------------------------------
// WHICH SPEC 8.3 verification stage failed
// ---------------------------------------------------------------------------------------------------------
...
/**
 * CLASSIFIED_BY records HOW a failure class was reached: a TYPED throw is authoritative; a
...
// ---------------------------------------------------------------------------------------------------------
// WHERE it failed (the locator), and the silent zero-byte seal
// ---------------------------------------------------------------------------------------------------------
...
export const INTEGRITY_FAULT_KINDS = [
  ...
  "defaulted-empty-record", // a record arrived with NEITHER a value NOR a stream and was sealed as ZERO BYTES
  // the SEAL CHECKPOINT (the Merkle frontier) deserialised CORRUPT on resume, so a huge multi-invocation
...
// ---------------------------------------------------------------------------------------------------------
// key and recipient PROVISIONING faults
// ---------------------------------------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------------------------------------
// the STREAMING failure locus
// ---------------------------------------------------------------------------------------------------------

/**
 * STREAM_LEGS names which side of the pipe stopped. It is the first question of every large-object ticket
 * ("is my SOURCE flaky or is my DESTINATION flaky?") and the coarse run failure alone does not answer it.
 *
...
export const STREAM_FAULT_CLASSES = [
  ...
  "gcm-auth-fail", // the AEAD tag did not authenticate: THE bytes changed under us (a bare OperationError otherwise)
...
// ---------------------------------------------------------------------------------------------------------
// the ANTI-ROLLBACK / RUNLOG anomaly, with forensic evidence
//
 ...
 * produce and the vocabulary the ledger will accept are the same list rather than two lists kept in agreement by convention.
 */
...
// ---------------------------------------------------------------------------------------------------------
// the WRITER's loud refusals, coarsened to a generic "run failed"
//
...
// a CUSTOMER FAULT the generic run failure cannot name: an emptied KV namespace or a de-scoped API token makes the
 ...
 * fault a coarse run row cannot name on its own, while the stream-wiring members are engine invariants. Getting
 * that wrong sends support down the opposite path, because the coarse run row looks the same either way.
 *
...
// ---------------------------------------------------------------------------------------------------------
// the DEGRADED attestation coverage mode behind an unqualified complete:true
//
...
export interface IntegrityFaultSnapshot {
  ...
  defaultedEmptyRecords: number; // how many records were sealed as zero bytes because they arrived empty
  ...
  runlogAnomalies: RunlogAnomaly[]; // capped forensic rows behind a rollbackDetected / corrupt RUNLOG
  writerRefusals: Record<string, number>; // closed WriterRefusalKind -> count
  attestCoverage: string; // the DEGRADED coverage mode of the last attestation ("" when full / undegraded)
}
...
/**
 * noteFetchFault records a destination READ fault behind an absent/missing verdict. Never throws: the
...
/**
 * noteFailStage records WHICH SPEC 8.3 verification stage failed and whether the class was reached from
 * a TYPED throw or a keyword guess.
 *
...
/**
 * noteLocator records WHICH item failed and by how much. The digest MUST already be a 12-hex one-way
 ...
 * neither a value nor a stream, and files a locator carrying only a one-way digest of its name.
 *
...
/**
 * noteCryptoFault records a key / recipient PROVISIONING fault. Fingerprints are of PUBLIC material
...
/**
 * noteStreamFault records the LOCUS of a streaming abort: which leg, which structural mode, which
...
/**
 * noteRunlogAnomaly records the FORENSICS of an anti-rollback / RUNLOG anomaly: which anomaly, the two
...
/**
 * noteWriterRefusal counts ONE loud writer refusal that the run row would otherwise coarsen to a
...
/**
 * noteAttestCoverage records the attestation's coverage mode when it is DEGRADED -- i.e. anything other
...
export function isEmptyIntegritySnapshot(s: IntegrityFaultSnapshot): boolean {
  return (
    ...
    // The runlog anomalies, writer refusals and attestation coverage mode are each evidence in their own
    // right and can be the ONLY thing a snapshot carries -- a degraded attestation coverage mode in
    // particular rides a run that otherwise reports a clean, complete verdict, which is the entire point of
    // recording it. Omitting them here would silently drop the post.
