...
// identically). By design the full specification and the reader source are NOT in the
...
// scripts/e2e-writer-reader.sh, the one place a real archive and a real built Go reader both exist), so a
// wrong custody-share count against a quorum split, or a headline line missing --apply, is caught before
// it reaches a customer relying on this text mid-disaster. Change this text only alongside a run of that
// gate.
//
...
export async function verifyBundle(store: ObjectStoreReader, verifier: HybridVerifier): Promise<void> {
  // Two distinct faults are kept apart here rather than mis-reported as one:
  //   - the two store.get calls can throw for a read-side 403 / 429 / cold-storage reason, which is
  //     reported as the bundle being missing, not corrupt;
  //   - b64urlDecode on a TRUNCATED or half-written SHA384SUMS.sig throws a PLAIN Error whose message
  //     matches no integrity keyword, so left uncaught classifyRestoreFailure would coarsen it to the
  //     AVAILABILITY class. That would engage the 3-2-1 replica fallback and tell the customer to check
  //     their bucket credentials, while the primary destination actually holds a DAMAGED SIGNATURE OBJECT.
  //     That is not a coarse class, it is a WRONG one. Typing it as a RunIntegrityError makes it
  //     non-fallback by construction, which is the guarantee integrity-error.ts exists to hold.
