...
// share, so buildArchive's per-record loop stays a short orchestrator.
...
// straight to the destination in a second pass via a single streamed PUT, never holding it whole. The
// sealed segment is NOT placed in the object map.
async function sealStreamRecord(ctx: SealContext, recordId: string, r: WriteRecord, dest: Destination | undefined): Promise<Record<string, unknown>> {
  const { p, cak } = ctx;
  // Both refusals are LOUD and SELF-DESCRIBING here and then collapse into a generic "run failed" by the
...
async function sealBufferedRecord(ctx: SealContext, recordId: string, r: WriteRecord, out: Map<string, Uint8Array>): Promise<Record<string, unknown>> {
  ...
  // THE SILENT ZERO-BYTE SEAL. A record that arrives with NEITHER a value NOR a stream is defaulted to
...
export async function buildArchive(p: WriteParams, opts: { dest?: Destination } = {}): Promise<Map<string, Uint8Array>> {
  ...
  //
  // This refusal is almost always a CUSTOMER fault the pack could not name -- an emptied KV namespace, a
  ...
  // shard line + Merkle leaf. The two seal paths are sealStreamRecord and sealBufferedRecord.
  ...
  // Shard manifest, signed root, RUNLOG anchor and recovery bundle.
...
// anchor (unless the pipeline writes it conditionally itself), and the recovery bundle. Split out so
// buildArchive stays a short sequence.
async function finaliseArchive(p: WriteParams, out: Map<string, Uint8Array>, recordLines: unknown[], leaves: Uint8Array[]): Promise<void> {
  ...
  // RUNLOG conditionally across runs, it sets skipRunlog and handles this.
  if (!p.skipRunlog) {
