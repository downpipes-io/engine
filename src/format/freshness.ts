 ...
 *
 * Hardcoding this false on every early return would invert the signal by severity: a forged RUNLOG
 * signature and a deleted RUNLOG would both report "no rollback" while a benign below-pin index
 * reports one. Driven against openRun with a real hybrid signature and allowStale on, a tampered
 * _RECOVERY/RUNLOG.sig or an absent RUNLOG would then OPEN the run while only the benign pin miss
 * refuses, making deletion of the document the quietest way through. The Go reader
 * (internal/format/freshness.go) uses the same two fields for the same reason, so the two
...
export interface FreshnessResult {
  ...
  checked: boolean;
  // WHY the RUNLOG read failed, when it did. The reason string says "RUNLOG absent" for a 403 after a
...
export async function checkRunlogFreshness(store: ObjectStore, runId: string, root: RootManifest, verifier: HybridVerifier, opts: FreshnessOptions = {}): Promise<FreshnessResult> {
  ...
  try {
    ...
    // Discarding this exception without classifying it would lose the real cause: the store's error names
    // whether it is a 403 after a credential rotation, a 429, a lifecycle move to a restore-required tier, or a
    // network fault, and without that distinction the pack says "RUNLOG absent" for all of them and support
    // cannot tell access-denied from not-found from throttle. classifyFetchFault reads the message ONLY to
    // select the two closed enums and returns them; the message -- which embeds the object key and the
    // endpoint host -- is never stored, never logged and never returned.
  ...
  // The one-way digest of the RUNLOG BYTES, so every anomaly row below can be JOINED to the document
  ...
  if (!(await hybridVerify(verifier, runlogBytes, sig))) {
    noteFailStage("freshness-signature");
    // A RUNLOG whose own signature will not verify is either a TAMPER or a destination-side bit-flip, and the
    // digest is what lets the customer prove which: a bit-flip changes the bytes, a re-signed forgery does not.
    noteRunlogAnomaly({ kind: "sig-invalid", ...digest });
    return fail("RUNLOG signature did not verify");
  }

  // A RUNLOG line that will not parse (a corrupt index, a missing runId, a destination-side bit-flip)
  ...
  // The three RUNLOG-INDEX disagreements are one stage (the document does not agree with the signed
  ...
  if (anomaly) {
    // Unconditional: a chain anomaly is proof the log was corrupted or rewritten, never a
    // legitimate state, so rollbackDetected is true regardless of allowStale.
    // The anomaly REASON interpolates the customer's run and downpipe ids, so it can never be recorded.
    // The closed stage can, and it is the actionable half: a rewritten chain is a security event, not a
    // scheduling one.
    noteFailStage("freshness-chain");
    // THE forensic row: a possible ROLLBACK ATTACK needs evidence recorded -- without it support cannot tell
    // a hostile rewrite from a corrupt object, which are opposite tickets (one is an incident, one is a
    // bucket). The CLOSED kind says which anomaly the detector matched,
  ...
  if (opts.minRunlogIndex !== undefined && runlogMax < opts.minRunlogIndex) {
    // Unconditional, exactly like the chain-anomaly branch above: this is the ONE check
    ...
    noteFailStage("freshness-chain");
    // The REPLAY case, and the two indices are the whole finding: indexA is the max this document
...
// detectChainAnomaly inspects the signed RUNLOG for the chain anomalies of SPEC 10
// (per-downpipe prevRunId linearity, not index contiguity),
...
//     integrity anchor, so the checks sort by index first (SPEC 10).
//
...
//
// Returns the CLOSED KIND and the two DISAGREEING INDICES beside the reason. The reason is
// returned to the caller (it names the customer's own runs, which is exactly why it can
...
function detectChainAnomaly(entries: RunlogEntry[]): ChainAnomaly | null {
  ...
  // FINALISE time, so concurrent runs legitimately append out of allocation order (a
  // fleet running many downpipes concurrently produces exactly that within its first hour of operation). The whole-document
  // signature is the integrity anchor; reordering without re-signing is impossible. The
  // checks below run over a copy sorted by index; the one index fact that IS a
  // corruption signal is a DUPLICATE (the account-global counter never reissues one).
...
// ChainAnomaly is the detector's structured verdict: the CLOSED kind and the two disagreeing indices
...
// runlogDigest is the 12-hex one-way join key over the RUNLOG BYTES: the same causeDigest discipline the
...
/**
 * The account-global runlogCounter (allocated by
