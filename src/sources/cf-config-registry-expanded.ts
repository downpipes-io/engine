// Part 2 of the Cloudflare configuration surface registry: the source-expansion surfaces. Kept in a
// separate module to keep each file a readable size; the surfaces and their EXACT order are unchanged.
// cf-config-registry.ts concatenates the core list with this one to form CF_CONFIG_SURFACES. See
// cf-config-registry.ts for the full restoreTier / fail-open contract.
...
// Support-pack fault evidence. These surfaces are the registry's only NESTED reads: a per-item expand (one
// queue's consumers, one waiting room's rules) and a per-SUB-PART read (one bucket's lifecycle/lock/CORS).
// Both classes of failure are invisible remotely: a sub-part that fails archives as `null`, byte-identical
// to "not configured" (so a LOST object-lock/WORM posture is unprovable), and a per-item throw voids the
// whole surface with only the bare surface id in the pack. Each site below records a COARSE, redaction-safe
// fact (closed reason class + a bounded attribution id whose customer-owned item name is reduced to a
// one-way handle) before it degrades. The CF error text never leaves the site.
import { classifySourceFaultReason, faultItemId, recordIncompleteFault, recordShapeAnomaly } from "./source-fault-ledger.ts";

// subPartFault records ONE failed sub-resource read inside a surface. The surface still archives (the
...
// itemFault records ONE failed nested per-item read that is about to VOID the whole surface, so the
...
export const CF_CONFIG_SURFACES_EXPANDED: CfConfigSurface[] = [
  ...
  // `dex.api.parameter.invalid`, and per_page=50 is accepted. Same shape as the rulesets ceiling, with the
  // same consequence if missed: the surface cannot capture at all.
  //
  // An account without DEX entitlement fails earlier with an entitlement error, before the per_page value is
  // ever evaluated, so the rejection above surfaces only on an entitled account.
  // listEnvelope, not listCapped: DEX answers `{dex_tests: [...]}`, and a plain list reader would archive the
  // ENVELOPE as the single item, leaving a restore nothing to rebuild from. Same shape as the gateway-lists
  // surface, where a count is what gets archived and the values are not.
  //
  // The pagination ceiling still applies underneath: DEX refuses per_page=1000 with
  // `dex.api.parameter.invalid`. Both faults sit on the same surface, and an unentitled account's earlier
  // entitlement error would mask either one.
  ...
  // ---- NEW: custom per-item-expand surfaces (cf. expandRulesets); no write() => capture-only/out-of-band restore ----
  {
    ...
    read: async (api, ids, meter) => {
      ...
      for (const q of queues) {
        ...
        if (typeof qid !== "string") { recordShapeAnomaly("cf-config/queues:list-item"); out.push(q); continue; } // an unrecognised list item is dropped through un-expanded
        meter?.spend(1);
        try {
          out.push({ ...q, consumers: await api.get(`/accounts/${ids.accountId}/queues/${qid}/consumers`) });
        } catch (e) {
          // One queue's consumers read failing voids the WHOLE queues surface. Attribute the item first.
  ...
  },
  {
    ...
    read: async (api, ids, meter) => {
      ...
      for (const ns of namespaces) {
        ...
        if (typeof nsName !== "string") { recordShapeAnomaly("cf-config/workers-for-platforms:namespace"); out.push(ns); continue; } // an unrecognised namespace is never expanded
        try {
          meter?.spend(1);
          const list = await api.get(`/accounts/${ids.accountId}/workers/dispatch/namespaces/${encodeURIComponent(nsName)}/scripts`);
          // A script inventory that is neither an array nor {scripts:[...]} coerces to an EMPTY list here,
          ...
          for (const s of arr as Array<{ script_name?: string; name?: string; id?: string }>) {
            ...
            // A FAILED settings read archives as `settings: null`, byte-identical to a script with no
          ...
          // This namespace's whole script inventory is lost; the surface still archives, so nothing else
  ...
  },
  {
    ...
    read: async (api, ids, meter) => {
      ...
      for (const r of rooms) {
        if (typeof r?.id !== "string") { recordShapeAnomaly("cf-config/waiting-room-rules:room"); continue; } // an id-less room is DROPPED entirely (not even pushed through)
        meter?.spend(1);
        try {
          out.push({ waitingRoomId: r.id, rules: await api.get(`/zones/${ids.zoneId}/waiting_rooms/${r.id}/rules`) });
        } catch (e) {
          await itemFault("cf-config/waiting-room-rules:rules", r.id, e); // one room's rules read voids the surface
          throw e;
        }
      }
      return out;
  ...
  },
  {
    ...
    read: async (api, ids, meter) => {
      ...
      const out: unknown[] = [];
      for (const r of rooms) {
        if (typeof r?.id !== "string") { recordShapeAnomaly("cf-config/waiting-room-events:room"); continue; }
        meter?.spend(1);
        try {
          out.push({ waitingRoomId: r.id, events: await api.get(`/zones/${ids.zoneId}/waiting_rooms/${r.id}/events`) });
        } catch (e) {
          await itemFault("cf-config/waiting-room-events:events", r.id, e);
          throw e;
        }
      }
      return out;
  ...
  },
  {
    ...
    read: async (api, ids, meter) => {
      ...
      for (const s of sites) {
        ...
        if (typeof rsid !== "string") { recordShapeAnomaly("cf-config/web-analytics-sites:site"); out.push(s); continue; }
        meter?.spend(1);
        try {
          out.push({ ...s, rules: await api.get(`/accounts/${ids.accountId}/rum/v2/${rsid}/rules`) });
        } catch (e) {
          await itemFault("cf-config/web-analytics-sites:rules", rsid, e);
          throw e;
        }
      }
      return out;
  ...
  },
  {
    ...
    read: async (api, ids, meter) => {
      ...
      for (const l of lists) {
        if (typeof l?.id !== "string") { recordShapeAnomaly("cf-config/account-rule-list-items:list"); continue; } // an id-less list is dropped, items and all
        try {
          out.push({ listId: l.id, name: l.name, items: await paginate(api, `/accounts/${ids.accountId}/rules/lists/${l.id}/items`, meter) });
        } catch (e) {
          await itemFault("cf-config/account-rule-list-items:items", l.id, e);
          throw e;
        }
      }
      return out;
  ...
  },
  {
    ...
    read: async (api, ids, meter) => {
      ...
      for (const s of snips) {
        ...
        if (typeof name !== "string") { recordShapeAnomaly("cf-config/snippets:list-item"); out.push(s); continue; }
        meter?.spend(1);
        let content: unknown = null;
        // A failed content read archives as `content: null`, byte-identical to a snippet that genuinely has
        // no content, so "the restored snippet has null content" is unprovable at capture time. Record the fault.
  ...
  // ===== additional config/metadata surfaces =====
  // NOTE: no separate "hyperdrive" surface is defined here: it would read the same
  // /accounts/{}/hyperdrive/configs endpoint with the same scope and restore tier as "hyperdrive-configs", so
  // it would capture an identical record. The richer-labelled "hyperdrive-configs" (see cf-config-registry.ts)
  // is the one used; its catalogue label/category are carried below.
    ...
    read: async (api, ids, meter) => {
      ...
      for (const ix of indexes) {
        ...
        if (typeof name !== "string") {
          recordShapeAnomaly("cf-config/vectorize:list-item");
          out.push(ix);
          continue;
        }
        meter?.spend(1);
        ...
        try {
          ...
          await subPartFault("cf-config/vectorize:info", name, e); // a failed info read is indistinguishable from an index with no info
          info = null;
        }
        out.push({ ...ix, info });
      }
  ...
  },
  {
    ...
    read: async (api, ids, meter) => {
      ...
      for (const b of buckets) {
        ...
        if (typeof name !== "string") {
          recordShapeAnomaly("cf-config/r2-bucket-config:bucket");
          out.push(b);
          continue;
        }
        const cfg: Record<string, unknown> = { bucket: name };
        // Every one of these per-part reads fails to `null`, which is BYTE-IDENTICAL to "this bucket has no
        // lifecycle / no object-lock / no CORS". For a backup product that is the worst possible ambiguity:
        // "why did the restored bucket lose its object-lock (WORM) config?" cannot be answered, at capture
        // time or afterwards, because the archive proves nothing. Each catch records the failing
        // "<surface>:<part>" with the bucket reduced to a one-way handle, so the
