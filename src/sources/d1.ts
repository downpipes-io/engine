...
//
// CONSISTENCY: the whole export must read ONE point-in-time snapshot ACROSS slices. D1
...
export class D1Source implements SourceAdapter, ResumableSource, LivenessProbe {
  ...
  // probeLiveness proves the bound database still EXISTS with a constant SELECT 1 (reads no table
  ...
  async *crawlFrom(_selector: Selector, token: string | null, meter?: Meter): AsyncIterable<CrawlEvent> {
    ...
    // A RESUME whose token carries no bookmark cannot re-open the snapshot the earlier slices read.
