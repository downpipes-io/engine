...
export function writeSingleSetting(pathFor: (ids: { accountId: string; zoneId?: string }) => string, label: string): NonNullable<CfConfigSurface["write"]> {
  ...
  // re-derived by a reader guessing at it. Seven proven surfaces are served by this factory, and until it
  // published its verb, its path and its label there was nothing about them a reader could ask, so
  // src/sources/cf-config-writer-fingerprint.ts had to fall back to this shared body for all seven and could
  // not tell a change to one surface's endpoint from a comment edit here.
...
export function writeSingleObject(
  ...
  // The writer DECLARES how it writes, so a harness can ask instead of parsing this file. The echo
  // harness needs the verb and needs to know this is the whole-object shape; the first version of that
  // probe hardcoded PATCH, tested surfaces declared PUT with the wrong verb, and reported their 405s as
  // defects. A fact the code already holds should never be re-derived by a reader guessing at it.
  return Object.assign(fn, {
    cfWriteKind: "object" as const,
    cfWriteMethod: method,
    cfWritePath: pathFor,
    // The field rules go out with the rest. A harness that re-derived them would test a body this writer
    // never sends, which is exactly what happened when the echo harness applied the GLOBAL strip and kept
    // reporting three surfaces as refusing after their per-surface corrections had fixed them.
    ...
    // function. A restore proof that survived an edit to this argument would be a proof of a writer that no
    // longer exists, which is what the fingerprint in cf-config-writer-fingerprint.ts is there to refuse.
