...
// 56 settings on the probe account. Ten settings that exist as their own endpoints are NOT in that
...
export const CF_CONFIG_SURFACES_GAPS2: CfConfigSurface[] = [
  ...
  { id: "device-policy-default", scope: "account", restoreTier: "idempotent", read: one((i) => `/accounts/${i.accountId}/devices/policy`), write: writeSingleObject((i) => `/accounts/${i.accountId}/devices/policy`, "PATCH", "device-policy-default", {
    // Cloudflare NAMED each of these in turn, one per refused request, until the body was accepted:
    // policy_id/default/gateway_unique_id are server-assigned, the DEFAULT policy is always enabled so
    // `enabled` cannot be set on it, and fallback_domains is a separate sub-resource with its own surface
    // and its own endpoint. Found by the echo check, which caught this surface refusing its own read
    // output within a minute of it being recorded as proven.
