...
// the only surface-specific write code. The registry (cf-config-surfaces.ts) imports these write functions;
// this module imports the core types/paginate and the shared helpers, so there is no cycle.
//
...
//
// A natural-key COLLISION (two live items sharing one natural key) is NOT conservative in the way it can
// look: "at worst one item is treated as missing and CREATED, never a wrong UPDATE of an unrelated item"
// holds only while the snapshot's server ids are still live. Once they are stale, which is
// precisely the restore-into-a-rebuilt-account case natural() exists to serve, keep-the-first binds a
// snapshot item to whichever twin Cloudflare happened to return first. Reproduced against the real
// registry: two Zero Trust posture rules both named "Disk Encryption", one Windows and one macOS, a
// snapshot with stale ids, and the writer issues PUT .../live-A carrying the macOS criteria onto the
// WINDOWS rule, then creates a duplicate, then reports success. Every Windows device fails posture and
// loses access. Which of the two outcomes you get depends on Cloudflare's list ordering, which is not
// ours to control.
//
...
//
// A LIVE item the snapshot does not contain produces no op, but it still needs a change entry: silence
// there is the defect, not the additive behaviour. A restore run to recover FROM an attack would otherwise
// leave an attacker-added DNS record, page rule, or a firewall rule with mode "whitelist" in place and
// still report success. It matters most on the allow-list surfaces, where the added entry IS the attack.
//
// So it reports, and does not delete. Deleting a live extra by default would be the worse defect: a
...
function buildListOps(spec: ListWriteSpec, liveById: Map<string, Record<string, unknown>>, liveByNat: Map<string, Record<string, unknown>>, snapshot: Array<Record<string, unknown>>, live: Array<Record<string, unknown>>, ambiguousNat: Set<string>): { changes: ConfigChange[]; ops: ListOp[]; skippedAmbiguous: ConfigWriteResult["skipped"] } {
  ...
  for (const snap of snapshot) {
    ...
    if (lostNested.length > 0) {
      ...
      // be reviewed, which is why every removal is named rather than tallied.
  ...
  // Every live item no snapshot item claimed is reported as a "remove" change, which summariseDiff
...
// in the snapshot" and the writer refuses the restore it exists to perform.
//
...
// at every depth, so a Gateway list entry's created_at never affects the comparison. Convergence for a
// nested surface depends on `readLive`: the writer must read live WITH the same sub-read its own read
// uses, or live carries no `items` key at all and the snapshot's extra key makes every list differ forever.
//
...
export function normaliseItem(spec: ListWriteSpec, item: Record<string, unknown>): Record<string, unknown> {
  ...
  // nestedCollections, to normalise a request BODY. Requiring the function would throw "spec.serverId is
  // not a function" for every generated writer at once, which is at least a loud failure rather than a
  // quiet one.
  // Drop the item's OWN address field, matching by the FIELD the spec resolves the address from rather than
  // by value. Matching BY VALUE deletes every field that happens to hold the same string. That is harmless
  // while the address is always an opaque server id: a
  // uuid does not collide with a url or a description. It stops being harmless the moment a surface is
  // addressed by a human-chosen field. A custom page asset whose description equalled its name would have
  // had the description silently dropped from the update body.
  //
  // It also made the offline drift guard blind to two endpoints, because its synthetic item sets several
  // fields to one placeholder, so resolving the address to that placeholder erased the whole body and no
  // request was issued at all. A guard going quiet is the worse half of the same bug.
  //
  // So delete the FIELD the address was resolved from, which the spec knows, rather than every field that
  // looks like it.
...
//
// writeListSpec never deletes an ITEM: applyListOps only creates and updates, and live-only items are
// reported without queueing an op. But an update sends the whole item, so a collection nested INSIDE an
// item is replaced wholesale, and any entry the snapshot lacks is deleted with it -- the same failure
// shape one level down, and why gateway-lists has no writer: PUT /gateway/lists/{id} carrying four items
// leaves exactly those four, confirmed against the live API.
//
...
async function applyListOps(spec: ListWriteSpec, api: CfApi, ids: Ids, ops: ListOp[], meter?: Meter): Promise<{ applied: number; skipped: ConfigWriteResult["skipped"] }> {
  ...
  for (const op of ops) {
    try {
      ...
      // Classify the refusal HERE, at the only site that ever holds the exception. The 120-char text
...
// key cannot collide, which is the property `indexLive` depends on and the one every writer must state
// explicitly. `kind` is deliberately NOT in the key: kind is immutable
...
async function diffOneRuleset(api: CfApi, rulesBase: string, labelOf: string, snap: Record<string, unknown>, liveById: Map<string, Record<string, unknown>>, liveByPhase: Map<string, Record<string, unknown>>, dryRun: boolean, changes: ConfigChange[], ops: RulesetOp[], skipped: ConfigWriteResult["skipped"], meter?: Meter): Promise<void> {
  ...
  // prune, never wholesale-replace a collection. A ruleset's rule list is a collection, and the previous
  // comment here ("its own idempotent unit, not a whole-account overwrite") argued the blast radius was
  // small rather than that the deletion was allowed. It is not allowed at either size.
  //
  // The realistic case is not exotic. An engineer adds a WAF rule on Tuesday, a restore runs on Wednesday
  // from Monday's snapshot, and the rule disappears with the run reporting success. The same shape is how
  // an attacker's added rule would have been silently reverted, which is the one case where deleting is
  // what you wanted; the operator still has to be the one who decides that, out of band.
  //
  // Matching is on IDENTITY, not on content. Content matching is wrong: an EDITED rule has different
  // content on each side, so an ordinary change would read as "the live rule is not in the snapshot" and
  // the writer would refuse every restore it exists to perform. Identity has to survive an edit, which is
  // exactly what content cannot do.
  //
  ...
  if (liveOnly.length > 0) {
    // NAME each one. A restore that quietly does something to a rule the operator cannot see is a product
    // failure even when the write itself is correct. Reporting "3 rules became 2" is exactly that failure:
    // it is a count, and a count cannot be reviewed.
...
export function writeRulesets(scope: CfScope): NonNullable<CfConfigSurface["write"]> {
  ...
  return async (api, ids, data, opts, meter) => {
    ...
    // index, so this threw on every account and the WRITE path of both ruleset surfaces could not run at
    // all. expandRulesets (the READ) was fixed for the same reason; this half was missed because nothing
    // exercised it: the auto-prove sweep only covers the generated writers, and these two are hand-written
    // and predate it. They were carried as proven on a proof that never touched this line.
