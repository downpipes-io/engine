...
export class SecretsSource implements SourceAdapter {
  ...
  async *crawl(selector: Selector, meter?: Meter): AsyncIterable<SourceRecord> {
    for (const s of this.secrets) {
      ...
      // ONE of N bound secrets being deleted (or its store being unreachable) throws here and kills
      // the WHOLE secrets run; a coarse class alone leaves WHICH secret sank it, and that every
      // other secret read fine, invisible remotely. Record the failing secret's identity as a one-way
