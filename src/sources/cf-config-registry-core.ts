// Part 1 of the Cloudflare configuration surface registry: zone DNS/rules/WAF/traffic/TLS/email/logs
// and the first set of account settings/access/data-service config. This lives in its own module,
// separate from cf-config-registry.ts, to keep each module a readable size; the surfaces and their
...
export const CF_CONFIG_SURFACES_CORE: CfConfigSurface[] = [
      ...
      if (!opts.dryRun) {
        for (const it of toApply) {
          try {
            ...
            // An un-settable zone setting (an ACM-gated cipher, a plan-gated feature) skips itself.
  ...
  // NOTE: no separate "vectorize-indexes" surface exists here; it would duplicate the richer "vectorize"
  // surface below: both read /accounts/{}/vectorize/v2/indexes, but "vectorize" additionally captures each
  // index's /info, so "vectorize-indexes" would be a strict subset. The "vectorize" surface is kept.
];
