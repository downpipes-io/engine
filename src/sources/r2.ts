...
function parseToken(token: string): R2Token {
  // Both failure modes here are RUN-FATAL (JSON.parse throws; a token without a string `after` throws).
  // recordResumeFatal names the resume stage so a bucket crawl wedged on a corrupt token is diagnosable
  // rather than showing only "the run failed" on every tick. The token is never read into the record; the
  // throws are byte-unchanged.
  let t: R2Token;
  try {
...
export class R2Source implements SourceAdapter, ResumableSource, LivenessProbe {
  ...
  // probeLiveness proves the bound bucket still EXISTS with one cheap object list (no value read)
        ...
        if (obj.size > ceiling) {
          ...
          yield {
            ...
            record: {
              ...
              markerKind: "_skipped",
              markerReason: "size-cap", // WHY this object is short -- the buffered-capture ceiling, not a fault
            },
          };
          ...
          if (body === null) {
            // Vanished mid-crawl: the object was listed but DELETED before its value could be read.
