...
// READ is fully paginated (every page accumulated; see paginate() in cf-config-surfaces.ts), so
...
// parseCfConfigToken classifies a corrupt cf-config resume token into the CLOSED defect vocabulary
...
// NOT_ATTEMPTED_MARKER_REASON is the FIXED reason a not-attempted "_unavailable" marker carries. It
...
export class CloudflareConfigSource implements SourceAdapter, ResumableSource {
  ...
  async *crawlFrom(selector: Selector, token: string | null, meter?: Meter): AsyncIterable<CrawlEvent> {
    ...
    // A bare JSON.parse would let a corrupt checkpoint throw a raw SyntaxError that matches no coarse
    // error branch, so a wedged cf-config downpipe would read as a plain "run failed" and support would have
    // no way to see that the run is stuck on resume-state corruption (remedy: clear the resume token).
    // Classify it instead.
    ...
    // clear error instead of returning an empty crawl. The identity watermark is a valid resume point (it
    // precedes the first surface), so it is exempt from this check.
    ...
    // fatal fault from a real error rather than inventing one: every surface failing is nearly always
    ...
    // notAttempted: the surfaces the CALLER left out of the include set because a cached probe could
    ...
    for (const surface of applicable) {
      ...
      try {
        ...
        lastFault = e;
        // Record WHY this surface is short, into the closed reason vocabulary, at the exact site that
    ...
    if (after === null && attempted > 0 && succeeded === 0) {
      // The run row would otherwise carry ONE coarse "run failed" class with no status and no stage, so
  ...
  // never fails the whole config crawl. markerKind is the adapter's own assertion that this
  // value IS one of the two sentinels below, never left for the seal/restore paths to guess from shape.
  // markerReason rides beside markerKind: the CLOSED reason class the seal folds into the run row's
  ...
  private surfaceErrorValue(surface: CfConfigSurface, e: unknown): { value: Uint8Array; ok: boolean; markerKind: MarkerKey; markerReason: SourceFaultReason } {
    if (e instanceof CfPaginationTruncated) {
      // A list surface that exceeded the pagination max-pages guard is NOT "unavailable",
  ...
  async estimate(selector: Selector): Promise<{ records: number; bytes: number }> {
    ...
    // A notAttempted surface yields a marker RECORD too, so it counts here for the same reason -- the
