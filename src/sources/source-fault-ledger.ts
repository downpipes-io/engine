// SOURCE FAULT LEDGER: coarsened, redaction-safe evidence for source faults that the archive cannot carry.
//
...
//   - the closed REASON class behind each incompleteness sentinel it seals, plus the truncation
//     magnitude (pages read / records accumulated) and a stuck render's age;
//   - a fault that never becomes a record at all: a failed SUB-PART read inside a surface, a failed
//     PER-ITEM read nested inside a surface read, a tolerant-parse drop;
//   - the run-fatal throw's HTTP status class and pipeline stage.
//
...
// It is deliberately a CLASS, not the status: the numeric CF error-code list and the message never
...
// SOURCE_FAULT_STAGES is the CLOSED vocabulary for WHERE in the crawl a fatal fault happened, which
...
// There is no `sub-read` stage: no adapter has a sub-read FATAL to record it at. Every nested read the
...
// correctly, as `item-read`. Adding this stage would mean INVENTING a new run-fatal throw at a level
// that absorbs its faults into per-item markers, which is a change to what fails a backup, not a
// recorder. A stage no throw site can select tells support a distinction the crawl never makes.
...
// point-in-time guarantee it claims. This is a DATA-INTEGRITY signal, not a failure signal: all three
...
// RESUME_TOKEN_DEFECTS is the CLOSED vocabulary for HOW a resume token was corrupt. A wedged
...
// grounds. These refusals are anonymous: the refusal is archive-sealed behind a "_refused" marker count,
...
// It is the missing attribution behind "our runs suddenly take 10x longer and trip rate limits": an
...
// D1_DEFECT_CLASSES is the CLOSED vocabulary for a D1 decode / validation refusal. Every one of
...
// There is no `ddl-injection-rejected` member here: this ledger rides on a BACKUP run's row, and the backup
// crawl never parses DDL: d1.ts reads the schema text out of the database and encodes it verbatim. The
...
// and as the D1 defect class `multi-statement-ddl`. The two members here are the
...
// RETRY_AFTER_SECONDS_MAX clamps the platform's Retry-After ask to a day, so a hostile or garbled
...
// SourceFatalFault is the run-fatal throw's coarsened identity: which source type, which transport
...
export interface SourceFaultLedger {
  // incompleteReasons: per marker kind, the closed reason class -> count map. The per-kind COUNTS
  ...
  // gives a failing SUB-PART (for example "r2-bucket-config/lock") or a failing nested ITEM (for example
  ...
  // recognise. This is the ONLY signal that a silent API-shape drift is voiding coverage while every
  ...
  // truncation: the WORST truncation magnitude seen this run: how many pages were read and how many
  ...
  // a video stuck at percentComplete 40 for three weeks reads as a large age here, where a fresh
  // upload reads as minutes. Clamped.
  oldestPendingAgeMs?: number;
  // fatal: the run-fatal fault, when the crawl threw rather than degraded. Last write wins (a crawl
  // only ever dies once).
  fatal?: SourceFatalFault;
  // snapshotConsistency: whether the D1 crawl actually held its point-in-time guarantee. Absent for a
  ...
  // _rowid_ probe threw. The fallback is correct for a genuine WITHOUT ROWID table, but a TRANSIENTLY
  ...
  // resumeTokenDefects: the closed defect class -> count map for corrupt resume tokens. The COUNT is
  ...
  // cfSelectorMode: WHICH cf-config surface selector this run actually used. Absent for a non-cf-config
  ...
  // d1Defects: the closed defect class -> count map for a D1 decode/validation refusal. Worth a first-
  ...
  // throttle429Count: every 429 the crawl absorbed, INCLUDING the ones a retry swallowed. This is the
  ...
  // retryExhaustedCount: how many times the retry ladder gave up entirely.
  retryExhaustedCount: number;
  // maxRetryAfterSeconds: the LARGEST Retry-After the platform asked for, clamped. A rising figure is
  // how mounting back-pressure reads before it becomes an outage.
  maxRetryAfterSeconds?: number;
  // securityRefusals: the closed security-refusal kind -> count map. Counts only; the refused host,
...
// recordShapeAnomaly is the writer for a TOLERANT-PARSE fallback: the response did not have the shape
...
// recordResumeFatal is the RESUME-STAGE fatal. A resume token the adapter cannot parse is run-fatal --
...
// recordSourceFatal is the writer for a RUN-FATAL source failure: the crawl is about to throw, and the
...
// recordSnapshotConsistency is the writer for the D1 point-in-time verdict. Called by the reader at the
...
// its _rowid_ probe threw. The probe's own error is DISCARDED at the call site (it is a legitimate
...
// recordResumeTokenDefect is the writer for a corrupt resume token. `sourceType` is the adapter's own
...
// recordThrottle is the writer for ONE absorbed 429. It is called INSIDE the retried function, so it
...
// recordRetryExhausted is the writer for a retry ladder that gave up: the fault the customer finally
...
// recordSecurityRefusal is the writer for a refusal made on SECURITY grounds. The KIND is the whole
...
/**
 * noteCfSelectorMode records WHICH cf-config surface selector this run used. Idempotent per run; the
...
/**
 * recordD1Defect records ONE D1 decode/validation defect: the closed class, and a one-way HANDLE of
...
export function mergeSourceFaultLedgers(a: SourceFaultLedger, b: SourceFaultLedger): SourceFaultLedger {
  ...
  // Worst-wins: one re-anchored slice colours the whole multi-slice run.
 ...
 * It reads the message ONLY to match the ENGINE'S OWN throw literals (d1-format.ts, d1-reader.ts --
...
// path. It is the answer to "token rotated: expired, under-scoped, or is Cloudflare down?".
