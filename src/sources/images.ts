...
// ImagesToken is the resume cursor: the v2 list `continuation_token` for the page to resume on
...
export class ImagesSource implements SourceAdapter, ResumableSource {
  ...
  async *crawlFrom(selector: Selector, token: string | null, meter?: Meter): AsyncIterable<CrawlEvent> {
    ...
    if (!variantsDone && inScope("_variants", selector)) {
      ...
      try {
        ...
        // The account variant-DEFINITIONS read failed. Failing OPEN (swallowed) here would make a run
        // whose variant config could NOT be read look identical to an account with no variants -- an invisible
        // shortfall. Emit an honest "_unavailable" marker in the _variants slot instead, so the seal counts it
        // (recordsIncomplete) and the run reports the gap. Mark variantsDone so a RESUME of THIS run does not
        // re-attempt it (symmetric with the success path); a LATER fresh crawl retries the read. Reason coarse+capped.
    ...
    for (let page = 0; page < IMAGES_MAX_PAGES; page++) {
      ...
      try {
        ...
        // Record the CLOSED transport verdict before the rewrap below throws away the HTTP status. The
      ...
      // A v2 list whose `images` field is not an array coerces to an EMPTY page here, so on a Cloudflare
      ...
      for (const img of images) {
        ...
        if (id === undefined) { recordShapeAnomaly("images:list-item"); continue; } // an id-less image is silently dropped from the archive
    ...
    // WHY (the page cap) and the shortfall MAGNITUDE (pages read, images accumulated), so "is our library
  ...
  private async *blob(id: string, enc: (s: string) => Uint8Array, meter?: Meter): AsyncIterable<SourceRecord> {
    ...
    try {
      ...
      if (cap.skip) {
        // The size ceiling, not a fault. The image id is customer-owned, so it is attributed by handle.
      ...
      // WHY this image's bytes are missing (auth / not-found / rate-limited / server-error), which otherwise
      // lives ONLY in the archive-sealed marker payload.
