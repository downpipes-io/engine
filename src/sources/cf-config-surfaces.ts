...
//
// REGISTRAR IS CAPTURED, and the sentence here used to say the opposite: "Registrar is excluded
// (needs the Global API Key)". It is not excluded and it does not need that key. registrar-domains
// and registrar-registrations have been in the registry with ordinary read() builders on the
// discovery token (cf-config-registry-gaps.ts:83), the console catalogue labels them, the generated
// surface table in the docs listed them as captured, and a live sweep of all 313 surfaces has both
// answering 200 to an ordinary discovery token. What IS out of reach is narrower and is recorded
// beside the surface itself: the registry-side transfer locks and authorisation codes, which are
// registrar state rather than Cloudflare configuration and are not replayable by any API.
//
