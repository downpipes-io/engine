...
// the namespace's repos); each per-repo content walk and each per-blob fetch is fail-open (a
...
// the platform subrequest cap and wedging the run. The validator's plain Meter has no shouldYield,
// so wantsYield() returns false there and the walk runs whole.
...
//
// RESUMABILITY: the content walk is deterministic in its record order (the commit log order, then
...
// ArtifactsToken is the resume cursor: `afterRecName`, the "<ns>/<repo>" of the last repo
...
//
// The per-repo content walk is itself bounded by ARTIFACTS_MAX_OBJECTS but a
...
// repo boundary); a present-but-malformed cursor throws rather than silently restart.
function parseArtifactsToken(token: string): ArtifactsToken {
  // A raw JSON.parse SyntaxError matches no coarse-error branch, so a wedged downpipe (every slice
  // failing on the same corrupt token) reads as a generic "run failed" with no route to its known remedy.
...
export class ArtifactsSource implements SourceAdapter, ResumableSource {
  ...
  //
  // The per-repo content walk ALSO checkpoints WITHIN a repo. A single repo's content walk can exceed
  ...
  async *crawlFrom(selector: Selector, token: string | null, meter?: Meter): AsyncIterable<CrawlEvent> {
    ...
    for (const ns of namespaces) {
      ...
      // A namespace the API returned WITHOUT a recognised name field is skipped ENTIRELY -- every repo,
      ...
      try {
        ...
        // Fail-open WITH AN HONEST MARKER: a namespace whose repo list cannot be read (a 5xx
        ...
        // The CLOSED reason class for this namespace's shortfall (auth / not-found / server-error /
      ...
      for (const repo of repos as Array<{ name?: string; id?: string }>) {
        ...
        // content walk yields records AND intra-repo {mark}s: a {mark} carrying an `inRepo` cursor is
  ...
  // emit yields ONE content record through the resume-skip + intra-repo checkpoint discipline. While
  ...
  //
  // It yields CrawlEvent (records + intra-repo {mark}s) and honours `skipContentUntil`: on a mid-repo
  ...
  private async *repoContent(target: RepoTarget, enc: Encoder, meter: Meter | undefined, prevRepoRecName: string, skipContentUntil?: string): AsyncIterable<CrawlEvent> {
    ...
    try {
      ...
      const logReason = classifySourceFaultReason(e); // an unreadable commit log costs the WHOLE history
    ...
    // The log can parse into a shape not recognised (neither an array nor {commits:[...]}, or no
    // commit carrying a hash/sha/id). commitHashes returns [] for BOTH that and a genuinely empty repo, and the
    // walk then returns silently: an ENTIRE commit history absent from the archive with a clean-looking run.
    ...
    for (const commitHash of commits) {
      ...
      try {
        ...
        const commitReason = classifySourceFaultReason(e);
  ...
  private truncatedMarker(recName: string, enc: Encoder): SourceRecord {
    // WHY the repo is short (the object-walk cap) and by HOW MUCH (objects walked), so the monorepo
  ...
  // a blob record before the content watermark is suppressed by emit() (the slice already sealed it),
  ...
  private async *blob(ctx: WalkCtx, hash: string): AsyncIterable<CrawlEvent> {
    ...
    try {
      ...
      if (cap.skip) {
        yield* this.emit(ctx, { sourceType: "artifacts", name, value: enc(JSON.stringify({ _skipped: cap.skip.reason, size: cap.skip.size })), markerKind: "_skipped", markerReason: "size-cap" }); // the size ceiling
      } else if (cap.stream) {
      ...
      yield* this.emit(ctx, { sourceType: "artifacts", name, value: enc(JSON.stringify({ _unavailable: e instanceof Error ? e.message : String(e) })), markerKind: "_unavailable", markerReason: classifySourceFaultReason(e) });
    }
  }
  ...
  // can be told apart from a genuinely empty repo: entries present but no hash extracted is API-shape
