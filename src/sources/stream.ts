...
// to the platform subrequest cap and wedging the run. The validator's plain Meter has no shouldYield,
// so wantsYield() returns false there and the video is captured whole.
...
// isCloudflareStreamUrl is the host allow-list for the MP4 download URL (an SSRF guard).
...
// StreamToken is the resume cursor: the page `start` (the `created` value the current list
...
//
// A single video captured WITH content emits several records (metadata,
...
// mid-video cursor is validated only when present (an absent cursor = resume at the video boundary);
// a present-but-malformed cursor throws rather than silently restart.
function parseStreamToken(token: string): StreamToken {
  // Closed defect class per corruption mode; the token bytes are never recorded.
  let t: StreamToken;
  try {
...
// VideoCtx is the per-video record-emission context. One video emits several records (metadata,
...
interface VideoCtx {
  ...
  // createdMs is the video's upload time as epoch ms, when the list reported a
...
export class StreamSource implements SourceAdapter, ResumableSource {
  ...
  async *crawlFrom(selector: Selector, token: string | null, meter?: Meter): AsyncIterable<CrawlEvent> {
    ...
    for (let page = 0; page < STREAM_MAX_PAGES; page++) {
      ...
      try {
        ...
        // Record the CLOSED transport verdict + stage BEFORE the rewrap below discards the HTTP status.
      ...
      // A list result that is not an array coerces to an EMPTY page, so a Cloudflare response-shape change
      ...
      for (const v of videos) {
        ...
        if (uid === undefined) { recordShapeAnomaly("stream:list-item"); continue; } // a uid-less video is silently dropped
      ...
      if (fresh === 0) {
        ...
        // WHY this library is short (the inclusive `created` cursor cannot advance past a timestamp bucket
    ...
    recordIncompleteFault("_truncated", "page-cap", { id: "stream:list", pagesRead: STREAM_MAX_PAGES, recordsAccumulated: captured }); // the shortfall magnitude
    yield {
      kind: "record",
      record: {
        sourceType: "stream",
  ...
  // emit yields ONE of a video's records through the resume-skip + intra-video checkpoint discipline.
  ...
  // through emit() so the watermark check advances (mirrors the artifacts blob replay short-circuit).
  ...
  //
  // Replay short-circuit: on a mid-video resume, a "<uid>/video.mp4" at or before the content watermark
  ...
  private async *video(uid: string, ctx?: VideoCtx, meter?: Meter): AsyncIterable<SourceRecord> {
    ...
    try {
      ...
      if (status === "ready" && url !== undefined) {
        ...
        // the host against the allow-list BEFORE fetching (an SSRF guard): a compromised
        ...
        if (!isCloudflareStreamUrl(url)) {
          ...
          // The reason class only (never the host, which rides in the archive-sealed marker payload).
          recordIncompleteFault("_refused", "redirect-refused", { id: await faultItemId("stream:video", uid) });
          // The same refusal, additionally counted as a SECURITY event. The reason above explains
        ...
        if (cap.skip) {
          recordIncompleteFault("_skipped", "size-cap", { id: await faultItemId("stream:video", uid) }); // the buffered-capture ceiling
      ...
      // Carry the render's AGE (clamped ms since upload), the only signal that separates "uploaded a
      ...
      recordIncompleteFault("_unavailable", reason, { id: await faultItemId("stream:video", uid) });
  ...
  //
  // Replay short-circuit: on a mid-video resume the caption LIST is re-read (cheap, and it re-derives
  ...
  private async *captions(uid: string, ctx?: VideoCtx, meter?: Meter): AsyncIterable<SourceRecord> {
    ...
    try {
      ...
      // A non-array caption list coerces to NO TRACKS, so on a response-shape change every video's captions
      ...
      // The caption LIST read failed (after the api layer's own retry). Failing open here (returning
      // silently) would make a video whose caption inventory could NOT be read look identical to a video
      // with no captions -- an invisible shortfall. Emit an honest per-video "_unavailable" marker instead, so
      // the seal counts it (recordsIncomplete) and the run reports "N items not fully captured" rather than a
      // clean ok. It flows through the caller's emit() like the video's other records (resume-suppressed while
      // replaying, intra-video marked), so it never breaks the checkpoint. A later run re-reads the list and
      // captures the tracks if the fault was transient. Reason is coarse + capped (no free-text past the cap).
      const listReason = classifySourceFaultReason(e); // WHY the caption inventory is short
    ...
    for (const t of tracks) {
      ...
      try {
        ...
        if (cap.skip) {
          recordIncompleteFault("_skipped", "size-cap", { id: await faultItemId("stream:captions", uid) });
        ...
        const trackReason = classifySourceFaultReason(e);
