...
// page-following paginator (page-based and cursor-based), and the read-builders one()/list()/
// expandRulesets() the registry's read functions are made of. The registry (cf-config-surfaces.ts) and
// the write specs (cf-config-write.ts) import from here; this module imports only the shared leaf
// (cf-config-shared.ts) plus Meter/withRetry, so there is no cycle.
...
export interface ConfigWriteResult {
  ...
  applied: number;
  // Each skip carries a CLOSED class alongside its human `reason`. The reason is the raw (prefix-
  // stripped, 120-char) Cloudflare message and stays exactly where it belongs -- in the live HTTP response
...
export function makeCfApi(token: string, fetchImpl: typeof fetch = fetch, pacer?: CfPacer): CfApi {
  const callEnvelope = (method: string, path: string, body?: unknown): Promise<{ result: unknown; result_info?: CfResultInfo }> =>
    withRetry(async () => {
      ...
      const init: RequestInit = {
        method,
        redirect: "manual", // a 3xx from the Cloudflare API is a failed call, never followed with the bearer
      ...
      // a cursor envelope (`{items, next}`). Treating those as failures throws away real data: a scim-users
      // endpoint carrying a real user would archive as "_unavailable", which is silent because fail-open
      // renders it as a marker. Accept the body as the result and record the anomaly so a non-standard
      // shape stays visible rather than becoming invisible.
      ...
      // `success:true` and puts its list at `data`. Testing `success === undefined` alone misses the
      // third. `"result" in b` distinguishes an absent key from a present-but-null one, which is a real
      // and different case: a null result on a list endpoint is a shape anomaly, not an empty list.
      ...
      //
      // An account with ZERO custom page assets never runs the delete leg, because the create is refused
      // first; on an account where the create succeeds, a round trip that treats the 204 as a failure reports
      // "CREATE-REFUSED: DELETE ... HTTP 204", which is a success being read as a failure.
      //
  ...
  return {
    ...
    getPage: async (path) => {
      ...
      // A LIST endpoint that answers with a NULL/ABSENT result is not an empty list (that is `[]`), it is
      // a response shape not recognised, and the coercion below turns it into a clean empty surface
...
export const RULESETS_PER_PAGE = 50; // Cloudflare refuses per_page > 50 on the ruleset index.
...
// dns settings): a list-shaped surface must use list() so it pages.
...
//
// A surface's OWN read() must be exercised through the paginator, not through a plain request: reading
// with too large a page size does not degrade, it fails outright, so a surface driven only by a plain
// request stays capable of capturing nothing while still LOOKING registered.
...
// surface whose ceiling is below that default fails before it can diff, so its writer can never work no
// matter how correct the create body is.
//
...
//
// Some list endpoints answer with bare identifiers only (e.g. /rum/site_info/site_tag/list answers
// ["<site_tag>", ...] and nothing else), so a surface reading the list directly would archive a list of
// opaque ids while the real content (for a rum-site surface, the host, the site token and the snippet
// configuration) lives at the per-item endpoint and is never captured; a restore from an archive like that
...
//
// NOT COVERED BY THE DRIFT GUARD. That guard drives each read with a synthetic list whose item is an
// OBJECT, so the pass-through branch below is what runs and the per-item endpoint is never recorded in the
// baseline. Shaping the fixture to return a primitive just for this would be fitting the guard to one
// surface; this endpoint is exercised by the live sweep and not by the offline one.
...
//
// Some list endpoints (Gateway lists among them) return {id, name, type, count} and NO items: the values
// live at a per-item endpoint (e.g. /gateway/lists/{id}/items). Reading only the list endpoint captures
// "this list has 3 entries" and none of the three, which for a Zero Trust allowlist or blocklist is the
// entire security-relevant content, and nothing reports a shortfall because a count is a perfectly valid
...
// A caller that expects the Cloudflare envelope shape and throws on a 200 without `success` would archive
// both as "_unavailable" markers while they hold real data.
//
...
export function expandRulesets(scope: CfScope): CfConfigSurface["read"] {
  return async (api, ids, meter) => {
    ...
    // index with "query parameter error: per_page cannot be greater than 50", so the default 1000 makes
    // BOTH ruleset surfaces throw and archive as "_unavailable" markers. That is the modern WAF
    // configuration in its entirety, and it is the most valuable thing the product captures. The ceiling is 50.
    ...
    for (const rs of index) {
      if (typeof rs?.id !== "string") {
        // A ruleset the index returned WITHOUT an id is silently dropped here. If Cloudflare ever renames
      ...
      try {
        ...
        // ONE ruleset failing (an Enterprise-phase ruleset the plan does not entitle, a 403 on a deleted
        ...
        // the closed reason class. Per-item fail-open would be a separate behaviour change.
