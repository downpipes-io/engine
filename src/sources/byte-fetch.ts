...
export function makeByteFetcher(token: string, fetchImpl: typeof fetch = fetch, pacer?: CfPacer): ByteFetcher {
  // withRetry only ever surfaces the TERMINAL outcome, so every 429 a retry successfully swallowed is
    ...
    withRetry(async () => {
      ...
      for (let hop = 0; ; hop++) {
        ...
        if (resp.status === 429) {
          ...
          // Count the throttle BEFORE the throw, so a 429 the retry ladder goes on to absorb is still
        ...
        if (resp.status === 0 || (resp.status >= 300 && resp.status < 400)) {
          ...
          // A redirect on a byte-fetch target is a SECURITY refusal, not a transport hiccup: it is an
      ...
      // The retry ladder gave up. Counted separately from the absorbed 429s above, so "throttled but
