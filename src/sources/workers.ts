...
// all-aspects-failed guard can record WHY the run is fatal instead of a coarse class with no status.
...
function failFor(path: string, status: number, why: string, headers: Headers): Error {
  ...
  // Carry the HTTP STATUS on the thrown error (a bare integer, alongside the message the callers already
  ...
  // "my Images/Stream/Workers backup fails after a token rotation" case. The status is a coarse, redaction-
...
function buildRawFetcher(
  ...
  return (path) =>
    withRetry(async () => {
      await pacer?.take();
      const resp = await fetchImpl(`${CF_API_BASE}${path}`, { method: "GET", headers: auth, redirect: "manual" });
      if (!resp.ok) {
...
export class WorkersSource implements SourceAdapter, ResumableSource {
  ...
  private async *yieldAspect(name: string, selector: Selector, meter: Meter | undefined, counter: AspectCounter, produce: () => Promise<Uint8Array>): AsyncIterable<SourceRecord> {
    ...
    // markerReason is the CLOSED reason class for the marker below; without it the pack could say a
    ...
    // markerKind is set ONLY on the fail-open catch path below, at the exact point the adapter
    ...
    try {
      ...
      counter.lastFault = e; // the all-aspects-failed guard classifies the run-fatal status from a REAL fault
      // Attribute the failing aspect. The aspect ("content"/"settings"/"versions"/"schedules") is a
  ...
  private async *yieldScript(id: string, selector: Selector, meter: Meter | undefined, counter: AspectCounter): AsyncIterable<SourceRecord> {
    ...
    yield* this.yieldAspect(versionsName, selector, meter, counter, async () => {
      ...
      // Neither an array nor {items:[...]} coerces to an EMPTY inventory, so on a Cloudflare response-shape
    ...
    yield* this.yieldAspect(schedulesName, selector, meter, counter, async () => {
      ...
      // Same silent coercion as versions above, but the consequence is data loss on restore -- a
  ...
  // cap is ~1000, ~3 calls/script): the run spans invocations instead of being killed with no progress.
  async *crawlFrom(selector: Selector, token: string | null, meter?: Meter): AsyncIterable<CrawlEvent> {
    ...
    try {
      ...
      // Record the transport verdict BEFORE the rewrap below discards it. The rewrapped message is a
    ...
    if (after === null && counter.attempted > 0 && counter.succeeded === 0) {
      // The list SUCCEEDED and every read failed, which is a narrower diagnosis than "the token is bad":
