...
function parseToken(token: string): KVToken {
  // Record the CLOSED defect class so a token-corruption wedge is legible on the run row instead of
...
export class KVSource implements SourceAdapter, ResumableSource, LivenessProbe, KeySampler {
  ...
  private async readRecord(k: { name: string; expiration?: number }, meter?: Meter): Promise<SourceRecord | null> {
    ...
    if (typeof this.kv.getWithMetadata === "function") {
      ...
      // This fallback silently DEGRADES the capture -- the key's metadata and TTL are simply not read, and
  ...
  // rangeBound (fan-out) cuts a listed page at the range's INCLUSIVE stopAt: it returns the keys
  ...
  async *crawlFrom(selector: Selector, token: string | null, meter?: Meter): AsyncIterable<CrawlEvent> {
    ...
    // Seed the start watermark from the range's EXCLUSIVE startAfter (skip keys <= startAfter),
    ...
    do {
      ...
      if (ended) return; // the first key past the range's stopAt ends the crawl (no further mark/page)
      cursor = page.list_complete ? undefined : page.cursor;
  ...
  private async *yieldPage(keys: { name: string; expiration?: number }[], selector: Selector, skipThrough: string | null, pageInCursor: string | undefined, meter?: Meter): AsyncIterable<CrawlEvent> {
    for (const k of keys) {
      ...
      if (rec === null) {
        // Vanished mid-crawl (SPEC 12.5): the key was listed but DELETED before its value could be
  ...
  async estimate(selector: Selector): Promise<{ records: number; bytes: number }> {
    const listPrefix = selector.include.length === 1 ? selector.include[0] : undefined;
    // Honour the range bound so a per-range estimate sizes only that partition (the fan-out
  ...
  // sampleKeys (fan-out) lists in-scope keys -- metadata only, NEVER a value read -- up to `cap`, in
  ...
  // listKeysFrom is the SLICED, keys-only planning scan (see the KeySampler doc): it yields one
