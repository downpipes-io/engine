...
export const SERVER_STAMPED = new Set([
  ...
  // restore's diff carried it back and Cloudflare answered 10004 malformedParams to the whole request. The
  // write then failed entirely, INCLUDING the harness's own attempt to undo its damage, which left Web
  // Analytics switched on for a real zone until it was turned off by hand. The field the endpoint reports
  // and refuses to accept is the dangerous kind: it looks like configuration and behaves like an id.
  "site_tag",
  // READ-ONLY METADATA ABOUT the setting rather than the setting. Cloudflare stamps `editable` on every
  // zone-settings response and correctly ignores any attempt to write it. The singleton PROBER has known
  // this since its first run, when flipping `editable` made ten surfaces look unexercisable; the WRITER
  // did not, so it could still send the field in a diff. The two now agree.
...
const VOLATILE_KEYS = new Set([
  ...
  //
  // It was invisible until the sweep was pointed at an account with real configuration: the proving account
  // has no Zaraz, so the surface read empty and the writer had nothing to disagree with. `zaraz-config` was
  // carried as PROVEN on that evidence. This sits in VOLATILE_KEYS rather than SERVER_STAMPED for the same
  // reason as `created`/`updated`: it is to be ignored in a DIFF, not stripped from a request body.
  "debugKey",
]);
...
// and VOLATILE_KEYS (timestamps and similar churn). Before this was unified, SERVER_STAMPED fields were
// stripped before the write but still surfaced in the diff preview as spurious "change" rows (e.g.
// "version: 1 -> 3") that a restore would never apply, misleading the operator about what would change.
// Deriving both the diff and the equality check from one set keeps the preview and the write in agreement.
