...
export interface SourceRecord {
  ...
  // markerKind: set by the adapter itself, ONLY at the exact call site that builds a synthetic
  ...
  // markerReason: the reason class for the marker this record carries, set by
  ...
  // Never inferred from the value's bytes: like markerKind it is an adapter ASSERTION.
...
export interface Selector {
  ...
  // range optionally narrows a LEX-ORDERED resumable crawl (KV first; R2 stretch) to one
  ...
  // notAttempted names the record ids the CALLER deliberately left out of `include` because a
...
// KeySampler is the optional key-only planning scan a LEX-ORDERED source exposes so the
...
export interface KeySampler {
  ...
  // listKeysFrom is the SLICED, keys-only planning scan the coordinator drives to COUNT then
