...
export async function probeCfConfig(
  ...
  // WHY each unavailable surface is unavailable, and WHICH surfaces truncated. The id lists alone
...
/**
 * sanitiseCfConfigDiscovery is the DO-side REDACTION CHOKEPOINT for a posted discovery result. Storing the
 * probe's object VERBATIM onto DownpipeState (and from there into the pack) is safe only because the sole
 * writer is the engine's own probe. Because the record carries a class map, that implicit trust is made
 * explicit here: every surface id must be a member of the CLOSED registry vocabulary, every
...
//
// THE SILENT DROP. "empty" and "gated" are evidence of ABSENCE: the account does not use the
...
// that this run is not capturing. Simply filtering those ids out of `include` would let the crawl skip
// them BEFORE its attempted counter, producing no record, no marker and no run-level incompleteness
// count: a backup that quietly did not capture something. They stay out of
...
export function effectiveCfConfigSelector(
  ...
  if (resolveCfConfigMode(source) === "manual") {
    noteCfSelectorMode("configured"); // the operator's explicit selection
  ...
  if (!fresh || discovery === undefined) {
    // THE COST EXPLOSION. An auto-mode downpipe whose discovery cache is missing or STALE (>25h,
