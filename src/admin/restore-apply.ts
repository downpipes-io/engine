...
import { type MediaFaultClass, type MediaUploader, mediaFaultOf, validateMediaBytesType } from "./media-restore.ts";
...
async function applyDataRecords(run: Run, window: Array<{ rec: ShardRecord; resolved: Resolved }>, bufferedMaxBytes: number, runId: string, failures: RestoreFailure[], receiptRecords: RestoreReceiptRecord[], outOfBand: RestoreSkipped[]): Promise<{ recordsVerified: number; recordsRestored: number; bytesRestored: number; aborted: boolean; d1Fault?: D1FaultEvidence }> {
  ...
  for (const { rec, resolved } of window) {
    ...
    // surfaced out of band. This is the restore-side counterpart the capture-side _vanished marker requires,
    // and it also covers the R2 over-ceiling _skipped marker, which must never be restored as garbage
    ...
    // adapter assertion and writes back like any other real record.
    ...
    try {
      ...
      // A fresh-target REFUSAL writes NOTHING; coarsening it into the generic partial-restore
      // sentence would tell the operator their database MAY BE INCONSISTENT when it is untouched, and would
      // cost the fault ring its d1-target-not-empty class (which keys on the sink's own literal). Carry the
      // sink's engine-owned refusal message for that one class; every other D1 fault keeps the partial-restore
...
export async function runApply(env: Env, body: RestoreRequest, run: Run, state: RestorePlanState, bufferedMaxBytes: number, runId: string, isLatest: boolean, cfApiFactory: (token: string) => CfApi, mediaUploaderFactory: (token: string) => MediaUploader, destFallback?: RestoreDestFallback): Promise<RestoreResult> {
  ...
  // Clear any metadata-shed tally an earlier operation left in this warm isolate, so the shed counts
  ...
  //
  // AND SO DOES A SURFACE THE API MERELY REFUSED. The write path is fail-open per
  // ITEM: an item Cloudflare rejects skips itself and the rest of the surface still applies, which is right,
  // and each refusal is classified at the skip site (cf-config-write.ts, via classifyCfWriteSkip). A refusal
  // reaches `configApplied`, but counting it there alone is not enough: left uncounted elsewhere, `ok` and
  // `complete` stay true, and it is absent from `result.skipped`, so the receipt's recordsSkipped misses it.
  //
  // THE TWO WAYS A TOKEN CAN BE WRONG NEED THE SAME TREATMENT; THE SAFER-LOOKING ONE IS THE RISKIER ONE.
  // Supplying NO token leaves the surface out of band (restore-plan.ts), where it is reported and counted.
  // Supplying a token that is merely TOO NARROW puts the surface in the plan, where every item is attempted
  // and refused 401/403. Left uncounted, an apply that wrote nothing would return ok:true,
  // allVerified:true, no failures, no skipped count, and a receipt that hashes exactly as a clean one. That is
  // the restore path, reachable from the console's own restore form, and the receipt is what the customer
  // keeps as evidence, so an uncounted failure would not merely be silent, it would be attested.
  //
  // The apply must still not ABORT, deliberately: by the time a refusal is known the other surfaces have
  // already applied, and throwing that away would help nobody. The surfaces that applied keep their applied
  // counts, the surface that did not is a named failure carrying the class and the remedy, ok and complete are
  // false, and the receipt carries the surface and the classes.
  ...
  // The per-CLASS media fault tally (and, on a conflict, the archived/live digest pair) distinguishes an
  // over-the-cap video, a transient Cloudflare blip and an id occupied by DIFFERENT live bytes, so "some videos
  // restored, some failed" can be diagnosed remotely rather than collapsing to one anonymous {failures: n}
  // integer. Counts keyed by the closed MEDIA_FAULT_CLASSES; the digests are hashes of the customer's own bytes
  // (the receipt's existing join-key idiom), so a conflict dispute can be adjudicated. Never a media byte, an
  // id or a CF body.
  ...
  for (const { rec, type, id } of mediaWindow) {
    try {
      ...
      // ASVS V5.2.2: sniff the decrypted bytes' OWN magic number before any upload. Throws a MediaFault
      // (cls "type-mismatch", caught below) when the bytes are recognisably the OTHER media kind than this
      // record's captured type; otherwise returns the sniffed content type (undefined when unrecognised).
      const contentType = validateMediaBytesType(type, bytes);
      const uploader = mediaUploaderFactory(body.mediaRestore!.token);
      const res = type === "images"
        ? await uploader.uploadImage(body.mediaRestore!.accountId, id, bytes, contentType)
        : await uploader.uploadStreamVideo(body.mediaRestore!.accountId, id, bytes, contentType);
      ...
      if (!res.verified) {
        ...
        // durable restore failure. The two must not collapse into an identical verified:false.
