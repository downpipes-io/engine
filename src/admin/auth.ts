...
//
// BREAK-GLASS DISPOSAL: the ADMIN_TOKEN is meant for a ONE-TIME bootstrap of the first Owner. Once
...
// This module-level cache holds the Cloudflare Access JWKS for the same-isolate lifetime, which deviates
// from the usual rule against global mutable state across requests. It is safe because the cached value
// carries no secret (RSA public keys only), is only ever written after the URL is SSRF-pinned to
// *.cloudflareaccess.com, is read-only after each write, and is bounded by a one-hour TTL. The alternative
// (a per-request fetch) would add an HTTPS round-trip to every gated request for no security gain.
...
async function fetchCertsCached(url: string): Promise<{ keys: { kid: string; kty: string; n: string; e: string }[] }> {
  ...
  // SSRF defence in depth: the JWKS this returns GATES Access auth, so pin the fetch to
...
// break-glass compare is currently throttled for this caller. Unlike Access (an IdP-backed
...
// the verified JWT exp, so the router can resolve the caller's role, surface session context and
...
export async function authorise(
  ...
  if (assertion && env.CF_ACCESS_TEAM_DOMAIN && env.CF_ACCESS_AUD) {
    ...
    if (!res.ok) {
      // OBSERVE: classify WHY the signed Access assertion was refused to a closed signal (aud / issuer /
    ...
    // OBSERVE: this is the case where Cloudflare Access lets the caller in but the console reports denied.
    // A service token (signed for the application audience but carrying common_name, not email) or an IdP
    // misconfigured to omit the email claim lands HERE, verified but unusable. It is recorded under the
    // same closed name the native-IdP path emits, so the pack has one emailless-principal counter across
    // both front doors. Best-effort; it never alters the fail-closed deny below.
    if (!res.email) {
      if (onAuthDeny) onAuthDeny("emailless-assertion");
      return { ok: false };
    }
    // SECURITY: an authenticated Access caller MUST also carry a stable
    ...
    // since it is mutable and a recycled address could inherit a prior member's role, so both must be
    // present for the access method.
    // OBSERVE: as above, but for the subject-only failure mode (a validly-signed assertion with no `sub`).
    // Recorded under the same closed name the native-IdP path uses, so one counter covers both front doors.
    if (!res.subject) {
      if (onAuthDeny) onAuthDeny("subject-unusable");
      return { ok: false };
    }
    // OBSERVE: the assertion VERIFIED and is about to be admitted, and the bounder threw away one or
    // more of the claims it carried. This is the case where a user is in the right AD group but gets viewer:
    ...
    // bare-token break-glass has no IdP identity). subject is the stable principal the role table
    ...
    return {
      ...
      ...(typeof res.iat === "number" ? { authIat: res.iat } : {}),
  ...
  if (verifyPasskeySession) {
    ...
    if (sessionToken !== null) {
      ...
      // OBSERVE: distinguishes the two real causes behind "users keep getting logged out mid-shift" so each
      // can be counted (the deny is byte-identical either way, and the response stays a generic 401 - the
      // verifier deliberately tells the edge nothing finer, which is the anti-oracle property this preserves).
      // A cookie that does not verify at all is the expiry/bad-MAC/tamper class; a cookie that verifies but
      // carries no email is a corrupt session record, a different fix entirely.
      if (res === null) {
        if (onAuthDeny) onAuthDeny("session-verify-failed");
        return { ok: false };
      }
      // A TOKEN SESSION: the bootstrap bearer exchanged for a minted session. It carries no email by
      // construction, and it is subject to the same two switches the bare bearer is: the env flag and the
      // in-app retire latch both end it (the DO refuses a retired one at verify too; this is the env half,
      // which the DO cannot see). It is never downgraded to the bearer path.
      if (res.method === "token") {
        ...
        return { ok: true, method: "token", email: null, viaCookie: true };
      }
      if (!res.email) {
  ...
  // Hardening: when the operator has disabled the token fallback, only verified Access or a valid
  ...
  // OBSERVE: each bare-token deny below records its OWN closed class, so "our automation suddenly gets
  // denied" can be told apart from "this engine was never given a token" or "the operator turned the
  // fallback off". Only the BRANCH is named - never the presented bearer.
  if (envFlagEnabled(env.ADMIN_TOKEN_DISABLED)) {
    if (onAuthDeny) onAuthDeny("admin-token-denied-disabled");
    return { ok: false };
  }
  ...
  if (breakGlassRetired && (await breakGlassRetired())) {
    // OBSERVE: a token refused because the Owner RETIRED it in-app. Distinct from the env flag above and
    // from break-glass-check-unavailable (a DO outage failing closed), which the router records separately.
    if (onAuthDeny) onAuthDeny("admin-token-denied-retired");
    return { ok: false };
  }
  // ANTI-BRUTE-FORCE THROTTLE: consulted HERE, immediately before the compare, so a
  ...
  if (!(await tokenEqual(presented, token))) {
    // OBSERVE: a bearer WAS presented and compared, and did not match: a stale/rotated/wrong token in
