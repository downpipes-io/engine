...
// The per-check builders live in posture-checks.ts; each
...
// The input-shape types live in posture-types.ts. Re-exported here so every importer keeps importing
// them from "./posture.ts".
export type {
...
export const CHECK_SEVERITY: Record<string, PostureSeverity> = {
  ...
  // off-account-audit-mirror (medium, ISO 27001 A.8.15): whether the audit trail is mirrored to a holder
  // outside this Cloudflare account (the outbound SIEM audit-log push). It is the COMPENSATING control the
  // engine's own architecture disclosure names for three separate residuals (the scheduler's privilege
  // concentration, in-account audit integrity, and signer-key forgery on a full account compromise), and
  // nothing graded whether it was on. Medium, not high: it compensates for residuals rather than being a
  // primary control, and an estate whose threat model excludes its own account holder risk-accepts it here.
  // Registered here alongside the push() call, so the check's real severity is never separated from its
  // registration. See the builder in posture-checks.ts.
  "off-account-audit-mirror": "medium",
  ...
  // still matches the set recorded in the last SIGNED control-plane export (posture-checks.ts). Registered
  // here at its real severity: isKnownCheckId gates acceptPostureRisk on membership here, so an owner can
  // attest, accept or override this check -- the exact one that catches an attacker-added seal recipient --
  // rated at its true weight rather than push()'s `?? "low"` fallback.
