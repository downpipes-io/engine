...
import { jsonError, jsonResponse, ownerActionGate, ownerActionQueuedResponse, recordAudit, recordAuditAfterSelfDeploy, recordBookkeepingAfterSelfDeploy, recordVerifiedEngineAccountAfterSelfDeploy, requireAllUpdateApproval, updateNeedsDualControl, validateDeployToken } from "./router-core.ts";
...
// dual-control gate: one approval gates the whole live apply, so the gate takes the most cautious
...
export interface ConsoleApplyArgs {
  ...
  consoleFloor?: string; // the console's floor (floors.console)
  dryRun: boolean;
  // The verified document's freshness claim; an APPLIED console outcome advances the watermark (a
...
//   applied      -> a component:"console" settled record (advancing floors.console + the freshness watermark)
...
export async function executeConsoleApply(args: ConsoleApplyArgs): Promise<ConsoleApplyResult> {
  ...
  if (!bytes) {
    ...
    if (!args.dryRun) {
      // The CONSOLE component's own download guard: recording it as this closed class (not only an audit row)
      // matters because the pack's configEvents excerpt strips the audit detail, and without the guard class
      // "the console update never shows up" reads to support as an unclassed denial. Same closed class as the
      // engine leg's download guard: the component is not what is being asked.
  ...
  // applied-unconfirmed and rollback-failed MUST be recorded here. Without them this branch simply does
  // not fire, so the console's two worst states would persist NO lifecycle record at all -- the pack would
  // carry whatever the prior apply left behind, which is the same class of lie the wider enum exists to
  ...
  if (applyLanded || r.outcome === "rolled-back" || r.outcome === "rollback-failed") {
    // This write is what the console reads to offer a rollback; when it is dropped the
...
export async function recordConsoleQueueResolution(scheduler: DurableObjectStub, actor: RouteActor, queuedVersion: string, reason: string): Promise<ConsoleApplyResult> {
  // A QUEUED intent whose closure record is dropped stays queued forever in the pack.
  await recordDiagWrite(scheduler, "update-settled", () => scheduler.fetch(doURL("/update-settled"), {
    method: "POST",
...
export function consoleRollbackTargetFrom(rec: { lastConsole?: { outcome?: string; fromVersion?: string } | null }): string {
  ...
  // applied-unconfirmed and rollback-failed keep the SAME fromVersion convention (the known-good prior),
...
export async function executeConsoleRollback(args: { env: Env; scheduler: DurableObjectStub; actor: RouteActor; accountId: string; token: string }): Promise<ConsoleRollbackResult> {
  ...
  if (r.outcome === "reverted" || r.outcome === "reverted-unverified") {
    ...
    // A rollback whose record is dropped leaves the fleet on a version nothing recorded.
    await recordDiagWrite(scheduler, "update-settled", () => scheduler.fetch(doURL("/update-settled"), {
      method: "POST",
...
// floors and passed the freshness gate. Everything else (dual control, token, deploys, records) runs
...
export async function handleComponentApply(a: ComponentApplyArgs): Promise<Response> {
  ...
  // OPT-IN DUAL CONTROL: ONE approval gates the WHOLE live apply, whatever the component set. The
  ...
  if (!a.dryRun && acting.length > 0 && updateNeedsDualControl(combined, await requireAllUpdateApproval(scheduler))) {
  ...
  if (!a.dryRun && !validateDeployToken(a.token)) {
    // The multi-component apply leg the console drives for a console component must record both a guard
    // counter and the audit event; without them, a refused "Update now" on a console-bearing release is
    // invisible in the pack, the same gap as the legacy engine-only leg's twin.
  ...
  if (wantsEngine) {
    if (engineIsCurrent) {
      ...
      if (p.outcome === "promoted" && p.fromVersion && p.toVersion) {
        ...
        //
        // recordDiagWrite CATCHES, so a dropped pending-record write never throws here -- and a
        ...
        // a reset during the audit write would throw out of this route and into the apply route's outer
        // catch, which answers "could not start the update right now; nothing was changed" WHILE THE ENGINE
        // IS LIVE ON THE NEW VERSION. That sentence being false is the exact failure this discipline guards
        // against.
        ...
        // See the legacy apply leg's identical comment in router-updates.ts on licence binding at claim
        // time. Best-effort, never checked.
        await recordVerifiedEngineAccountAfterSelfDeploy(env, a.accountId, "update-apply");
        if (!pendingWritten || !audited) {
          // The legacy leg's own honesty branch: the engine is LIVE ON THE NEW VERSION and the record
        ...
        // The sibling of router-updates.ts's legacy promote arm: a promote refusal on this leg must file both
        // an update-fault and a high-water guard class, not only an audit row -- the same downgrade the
        // console refuses on the engine-only apply, driven through the components-aware apply the console
        // uses whenever a release carries a console component, must not reach the pack as an unclassed denial.
