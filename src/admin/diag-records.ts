...
export const METRICS_SCRAPE_OUTCOMES = [
  ...
  "credential-expiry-unreadable", // the metrics grant's stored expiresAt does not PARSE, so it can never expire and every reader agrees it is live. The scrape is refused FAIL-CLOSED so the corruption is named rather than masked
...
// ---------------------------------------------------------------------------------------------------------
// THE DIAGNOSTIC RECORDERS' OWN DROPPED WRITES (the shared pattern)
//
...
export const DROPPED_WRITE_KINDS = [
  ...
  "seal-error", // a per-downpipe pre-run seal fault could not be attributed: the downpipe's silence has no cause
  "metrics-health", // a /metrics scrape outcome could not be recorded: the scrape surface's own health is blind
  "webauthn-fault", // a structural WebAuthn fault could not be recorded: a total-lockout window shows no WebAuthn evidence
  "restore-fault", // a restore/drill/verify fault row could not be recorded: the pack shows a restore that failed with NO per-record or per-phase attribution
  "admin-counter", // an admin diagnostic counter bump could not be recorded: a licence source-flip, a reclaimed apply lease or a silent row exclusion left no trace
  "dest-build-health", // a destination BUILD outcome could not be recorded: the pack cannot say the destination has been unbuildable since a given moment, nor WHICH knob broke it -- so the owner's #1 fear (a redeploy that wiped DEST_* / CONFIG_WRAP_KEY) reads as an ordinary run failure
  ...
  // in-flight lease. A dropped post leaves the downpipe dangling in flight until the 30-minute lease expires,
  // so the pack shows a STALL (downpipes[].stalled) with no cause -- indistinguishable from a genuinely
  // wedged seal. The DO is by definition unreachable at fault time (that is usually WHY the post failed), so
  // the pending-tally + flush-on-recovery protocol is exactly right: the loss lands the moment the DO comes
  // back, which is when the pack is generated.
  "run-completion",
  ...
  "run-faults",
  // The 10 remaining best-effort ADMIN-ROUTER DO writes: each is an `await`ed fetch whose response is never
  // checked, or a bare `.catch(() => {})`. Their losses are counted by surface, because each loss makes a
  // DIFFERENT pack section quietly incomplete: an unverified audit chain reads as intact, a discovery refresh
  // reads as "nothing found", a key-ceremony event never joins the audit trail, a restore receipt never
  // anchors, and the deploy keystone (the owner's #1 fear: a deploy that wiped source bindings) simply never
  // appears -- indistinguishable from "no redeploy happened".
  ...
  // The demo fresh-first-run marker CLEAR after a landed key install. A demo reset cannot delete
  ...
  // pasted short: the run row still shows a coarse failure, which is exactly what this aggregate answers.
  "integrity-faults",
  ...
  // the heal for the owner's #1 fear (a deploy that wiped the source bindings) was TRIED and FAILED -- which
  ...
  "recovery-refusal",
  "test-outcome", // a wiring-check (Test button) outcome was lost, so the pack again cannot say whether the operator's test passed
  // The RETENTION pass record (cron/retention-pass.ts recordRetentionPass, POST /retention-record). A dropped
  // post means the pack's `retention` section shows the PREVIOUS pass as the latest one, so a prune that
  // deleted too much, or that silently stopped pruning, reads as a pass that never happened. Retention is the
  // one place a wrong answer is unrecoverable in both directions, which is why its record matters most among
  // this aggregate's members.
  "retention-record",
] as const;
...
export const RESTORE_FAULT_CLASSES = [
  ...
  // readback-failed's two halves, which a single coarse class would throw away. The bucket ATE the write (the
  // get came back absent) is a different fault from the bucket returning a BODYLESS object -- one is a lost
  // write, the other a store/proxy that is mangling reads -- and support cannot ask the right next question
  // without the split.
  ...
  // The BREAK-GLASS-ONLY posture's honest refusal. Coarsening it into "not-configured" would make a drill
  // that CANNOT be exercised in-account by design read shape-identical to a drill that genuinely FAILED,
  // sending support hunting a fault that does not exist. This class says instead "the engine holds no
  // in-account read-back key; recovery is exercised offline with the break-glass key".
  "posture-unexercisable",
  // --- the D1 decode / validation refusals, which would otherwise land in `other` with no locus at all ---
  // The three route to COMPLETELY different diagnoses, and telling them apart is the whole point: support
  // could not distinguish a tampered archive from a writer bug, and could not spot that an unknown format hint
  // means ENGINE VERSION SKEW (the archive was written by a NEWER engine than the one restoring it, e.g. after
  // a rollback) -- a one-line diagnosis that would otherwise stay invisible.
...
export function classifyRestoreFaultClass(reason: string | undefined): RestoreFaultClass {
  ...
  // The break-glass-only refusal is NOT a failure and is never recorded as one (see the class).
...
export const ADMIN_COUNTER_NAMES = [
  ...
  "report-data-read-unavailable", // a report-data DO read FAULTED or answered non-2xx: without this guard it would be cast straight into the report body and signed over garbage
  // --- destination configuration an operator SET and the engine silently DISCARDED ---
  // Each names the FIELD, never its value. The env-knob half of this is DERIVED at pack-build time from env
  ...
  "ingest-pull-rate-limited", // a per-clientId cap on SUCCESSFUL /support/audit-feed pulls was hit (see support-ingest.ts's INGEST_PULL_RATE_LIMIT_MAX_PER_WINDOW): the collector is polling faster than the generous per-credential budget allows
  ...
  // The gate already decides between these five (classifyIngestCredentialCheck), and the /metrics scrape
  // carries the same five through to METRICS_SCRAPE_OUTCOMES. See admin/support-ingest.ts's
  // INGEST_PULL_REFUSAL_COUNTERS for the full reasoning.
  ...
  "ingest-pull-refused-credential-expiry-unreadable", // the stored grant's expiresAt does not PARSE, so it can never expire and the gate authenticates forever. Re-mint, and treat a corrupt stored record as its own incident
  ...
  "report-pdf-unparseable-ts", // the PDF renderer met a timestamp it could not render and coerces it to "unknown" rather than throwing: without this guard a RangeError deep inside PDF assembly would 500 the whole DOWNLOAD, so an auditor's report could not be produced at all and nothing would say why
  ...
  // "Our integration script keeps getting 'invalid runId'": a rejection reaches only the immediate HTTP
  // caller and nothing else persists it, so without a counter here support cannot say whether the ids arrive
  // truncated, case-mangled or out of range -- three different fixes on the customer's side. The KIND is the
  // counter; the candidate string (which could be any value a caller chose) is classified at the edge and
  // DISCARDED there.
  ...
  // There is no `stored-expiry-unknown-kind` counter, because no read-time skip on kind exists to count.
  // reconcileExpiry walks every stored row; laddersFor is TOTAL (an unknown kind takes the default 30-day
  // ladder rather than being dropped); daysRemaining is computed from the expiry alone. So a row with a
  // garbled kind is still counted, still laddered and still warned about. The only kind check in the system
  // (isExpiryKind) is at WRITE time, and its refusal already has a producer: stored-expiry-write-rejected below.
  ...
  "notify-critical-alert-undelivered", // a ONE-SHOT critical alert was CONSUMED (its claim spent, its dedupe latched) and delivered to NO channel. The page is permanently lost -- nobody was told backups had stopped
  "notify-history-append-failed", // a notify history append was DROPPED, so a delivered alert can leave no row at all: "was our critical alert delivered?" is unanswerable from the pack in exactly the case that matters
  "update-channel-verify-failed", // the signed update channel stopped VERIFYING (a signing-key rotation, or tamper) and the alert pass returns silently: "we were never told a critical update existed"
  "cp-export-dest-failed", // a destination REFUSED the signed control-plane recovery artefact, so ITS recovery generation is stale while export health looks green (the closed cause is in cronHealth.cpExport.perDestFail)
  "cp-export-record-write-failed", // the export-health record write ITSELF was lost, so the pack reads a stale export health as if it were this tick's verdict
  "cp-export-plaintext-purge-pending", // readable PLAINTEXT control-plane exports could not be purged from a sealed-posture bucket, so the roster/topology stays readable in the destination and nothing said so
  "push-trail-write-failed", // a SIEM/OTLP delivery-trail write was dropped. On a SUCCESSFUL delivery that strands the cursor, so the next tick re-sends the same batch: the duplicate-events ticket
  "restore-test-not-run", // a scheduled restore test did NOT run (a pre-drill throw, chronic budget starvation, a lost outcome write). "Never tested for months", with a perfectly healthy-looking engine behind it
  "reconcile-pass-skipped", // the orphan-reconcile pass recorded no signal this tick, so its last signal may be months old -- and a stale signal is indistinguishable from a fresh one
...
//
// THE CAP IS RIGHT, AND THE REFUSAL MUST NOT BE SILENT. This is not a ring: past the cap it
...
export function applyIntegrityFaultsCounted(prior: IntegrityFaults | undefined, downpipeId: string, snap: unknown, now: number): { record: IntegrityFaults; refusedSubjects: number } {
  ...
  // weaker one -- that is the whole point ("this run's complete:true was the degraded kind").
...
//
// THE NUMBER IS NOT A PREFERENCE. Two caps here bound the SAME subject space: `recordControlPlaneExportHealth`
// slices perDest at 64 and DECLARES the drop, and this cap must match it. Nothing caps how many destinations
// an estate may hold (only destinationIds PER DOWNPIPE, at 20), so this 64 is the only statement the product
// makes about the size of a destination fleet. This record answers "all destinations unreachable" by naming a
// cause for EACH one, so the cap must not bite before the record does its job: destinations past it read as
// ones that probed fine, so a cap set too low turns a total outage into one that reads as a partial one.
...
//
// The attach pipeline's post-write verification is the owner's #1 fear made into a guard: after PATCHing the
...
// PATCH raced ours and one of the two writes was a silent lost update). Each alarm reaches only an HTTP
// response, read by exactly one operator in one browser tab, once, unless it is recorded here: without this
// record the pack holds two healthy-looking sources-attached audit rows and no trace of the alarm, its timing,
// or the race -- while a source quietly stops backing up.
//
// NO-CUSTODY: binding NAMES only, the operator-label class the pack already ships in sourcesDetached.
...
//
// The self-update pipeline is the owner's #1 fear (a deploy that wipes source bindings), so a single coarse
// `reason` string is not enough to diagnose it. Without these distinctions:
//   - a canary that fails AND whose auto-rollback ALSO fails leaves no durable proof (the engine limps on a
//     bad version and the pack says only "reverted");
//   - a truncated channel.json reads as "signature did not verify" (verifyChannel would otherwise collapse
//     sig-fail / json-parse / shape-invalid into ONE null), sending support on a key-pinning chase;
//   - six distinct artefact-download failures (404 vs redirect vs oversize vs empty body vs network) would
//     land as the single fixed audit detail "artefact-download-failed";
//   - a CF API shape change makes the provenance read-back report "unavailable" fleet-wide with no fixture;
//   - the post-update binding diff computes the EXACT names of the dropped source bindings, and only the
//     COUNT would otherwise persist: support could prove bindings were dropped and could not say WHICH to
//     re-attach;
//   - and the lifecycle records themselves (pending / settled / superseded / the ramp pending) are DO writes
//     that must be checked, or an engine could be LIVE on a new version with no record of it at all.
//
// ONE bounded ring answers all of this, because it is one pipeline and support reads its faults together.
//
...
export const UPDATE_FAIL_STEPS = [
  ...
  "bookkeeping", // the LIFECYCLE RECORD write itself: the deploy happened, the record of it did not
] as const;
...
export const UPDATE_CAUSE_CLASSES = [
  // --- channel: the three-way collapse verifyChannel would otherwise report as one wrong reason ---
  ...
  // --- transport: the download modes that would otherwise collapse to one fixed audit detail ---
  ...
  "binding-uncarryable", // the live script has a binding TYPE the pipeline will not risk dropping: it refuses rather than upload blind (the owner's #1 fear, honoured)
  ...
  "readback-unknown-shape", // the CF API answered a shape the read-back parser does not recognise: provenance reads "unavailable" fleet-wide (a bounded shape descriptor rides)
  ...
  // The post-update binding diff finds source bindings the update DROPPED. The count alone cannot say WHICH
  // to re-attach -- the owner's #1 fear, half-instrumented without the names -- so the names ride on the row.
  "sources-dropped",
...
/**
 * The bounded read-back SHAPE descriptor. Rather than carry the CF response's top-level key NAMES, this
 * carries a strictly stronger-redacted form that answers the same question -- how many top-level keys the
 * body had and whether the three the parser needs were present -- with NO strings at all, so not even a
 * Cloudflare key name (let alone a value) can ride.
 */
...
export const ADMIN_REFUSAL_SURFACES = [
  ...
  // ---- THE GOVERNANCE, PEOPLE AND NOTIFY WRITES, NONE OF WHICH IS RECORDED ON REFUSAL ----------------------
  //
  ...
  // reload is simply gone, so three states produce an identical pack -- refused last week, never attempted,
  // applied and later turned off. This aggregate is the ENGINE-SIDE ring with count + lastAt that tells them
  // apart: the engine remembers across sessions, reloads and tabs, which is exactly what a browser cannot.
  //
  ...
  "recovery-codes-regenerate", // POST /auth/recovery-codes/regenerate -- recovery codes are the account's last-resort credential, so a refusal here that leaves no trace is the surface where silence costs the most
  "break-glass-retire", // POST /policy/retire-break-glass-token
  "restore-proof", // a restorability PROOF (a blind verify or a keyless attest) refused BEFORE it ran. It runs no restore, writes no evidence and stamps no downpipe, so a week of refused proofs and a week in which nobody drilled leave the pack IDENTICAL: a stale lastRestoreProvenAt and nothing else
] as const;
...
export const ADMIN_REFUSAL_REASONS = [
  ...
  "reserved", // the write named a RESERVED engine binding/name (the confused-deputy guard): security-significant, and would be indistinguishable from a typo without this class
...
//
// ON WHY THERE IS NO `change-control` REASON HERE.
//
...
// nothing can honestly write is dead evidence wearing a discriminator's clothes.
//
...
export function classifyUpdateFailStep(e: unknown): { step: UpdateFailStep; cause: UpdateCauseClass } {
  ...
  // --- THE DIGEST CHECK is tested FIRST. Without this arm, a downloaded artefact that does not hash to what
  // the SIGNED channel declares would misreport as a Cloudflare upload fault ({version-post, other}): CDN
  // corruption or TAMPER, the pipeline changed NOTHING, and Cloudflare had not even been called yet -- three
  // wrong facts in one row, in the owner's #1 fear domain. Nothing else can produce these sentences
  // (verifyAndGuard's own three verify arms), and a later arm must never be able to swallow them.
...
// a failure, so it carries no signal here. This records the failed ATTEMPT, which is a real availability fact
// about a real operator action.)
// ---------------------------------------------------------------------------------------------------------
...
//
// The update, rollback, ramp and licence-activation routes each end in an outermost
...
//
// This is the DISASTER-RECOVERY path: the estate is gone, the customer is rebuilding it from a signed export
// and their offline recovery kit, and the support pack may be the ONLY artefact that survives to explain what
// happened. Without a record here, every refusal on that path reaches only the operator's browser: the
// import's signature refusal, the recovery-kit download's 500/502, the apply-staged 409 on a corrupt staged
// artefact, the reconcile's rejection.
//
// And the SHARPEST edge: a THROWN crypto fault (a corrupt kit signer.pub that will not import, an
// unparseable signature blob) and a genuine signature MISMATCH (a modified export -- tamper) both fall
// through to the SAME `sigOk = false` and the SAME "the export signature did not verify" refusal unless they
// are told apart. Those are opposite diagnoses: one says "your kit file is damaged, get another copy", the
// other says "someone has altered your recovery artefact". "Estate import says signature invalid but the kit
// is correct" is exactly the ticket that cannot be answered when they are byte-identical.
//
...
export const RECOVERY_REFUSAL_SURFACES = [
  ...
  // POST /control-plane/acknowledge-recovery, the acknowledge-only latch clear.
...
// RECOVERY_REFUSAL_CLASSES is WHY. The first four are the crypto verdict, and they are the pair-of-pairs this
// whole vocabulary turns on. They are NOT a structural guess about the signature STRING: they are the closed
// verdict hybridVerifyDetailed (crypto/sign.ts) already computes, so the four worlds a bare `false` would
// otherwise collapse into stay four distinct rows.
//
...
//                      key. The export is fine. A structural check alone cannot tell this from tamper, because
//                      a right-length corrupt key sails through a length check and fails only inside the
//                      verify -- so this class is what stops that inversion.
...
//                      signer rotation. Without this class the verify would short-circuit on the classical half
//                      and never run the second one, scoring it `signature` and telling the customer their
//                      disaster-recovery artefact had been TAMPERED WITH when the remedy is "take another copy
//                      of your kit". Tamper cannot reach this class: altering the export breaks BOTH halves.
...
export const RECOVERY_REFUSAL_CLASSES = [
  ...
  "verifier-invalid", // the VERIFIER would not import: the kit's signer.pub is corrupt, or this engine's own SIGNER_PRIVATE will not load. The KEY is damaged, NOT the export -- the opposite remedy from `signature`, which a bare boolean verdict cannot tell apart from this
  ...
  // The two ways the acknowledge-only latch clear refuses. Separate classes, because the operator's next step
  // differs completely and because the console maps a class to the stable code a customer reads out. Neither
  // is reachable by a break-glass caller (the route refuses the bare token upstream of both), so neither can
  // ever coalesce with a reconcile refusal.
  "ack-role-table-empty", // the estate's role table is empty, so clearing the latch would remove the only explanation for every caller resolving to viewer (the deadlock guard). NOT a wiped-plane refusal
