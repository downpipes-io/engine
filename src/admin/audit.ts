...
export async function auditHash(event: AuditEvent): Promise<string> {
  ...
  //
  // actorSubject is included in the hashed object only when it is a string. A legacy entry recorded
  // before subject-keying existed has no actorSubject, so the field stays absent from `hashed` and its
  // digest matches the stored hash exactly, keeping every such entry's hash intact and the chain
  // verifiable alongside newer entries. A new entry stamps a string subject (or omits it for the
  // token/engine actor, which also leaves it out), so the hash commits to the subject when there is
  ...
  // Fold the IdP acr/amr/auth_time advisory signal into the digest only when present (it rides only on
  // an idp-sign-in whose IdP asserted it). An event without it -- every entry recorded before this signal
  // existed, and every non-sign-in entry -- hashes exactly as it always has, so the chain verifies across
  // both. canonicalJSON sorts keys, so adding the key when present does not disturb the order of the others.
...
export async function buildEvent(draft: AuditDraft, seq: number, ts: string, prev: AuditEvent | null): Promise<AuditEvent> {
  ...
  const event: AuditEvent = {
    ...
    target: draft.target,
    // Carry the OPTIONAL advisory IdP acr/amr/auth_time when the draft supplied it (idp-sign-in only),
...
function describeTarget(t: AuditTarget): string {
  switch (t.kind) {
    ...
    case "authn-attempt": {
      // A REFUSED authentication ceremony. The connection is named only when it resolved, and `attempts` says
      // how many failures this one row stands for, so a CSV reader is never left reading a coalesced row as a
      // single event. Nothing here is representable but the closed ceremony/kind enums, the operator-chosen
      // connection slug and an integer.
      const via = t.connKind !== undefined && t.connId !== undefined ? ` via ${t.connKind} connection ${t.connId}` : "";
      const n = t.attempts > 1 ? ` (${t.attempts} attempts)` : "";
      return `failed ${t.ceremony}${via}${n}`;
    }
    ...
    case "secret-rotation":
      // The closed cadence id of the attested-rotated secret; never a value.
      return `secret rotation confirmed ${t.secretId}`;
