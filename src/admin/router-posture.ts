// router-posture.ts -- the security-centre / posture computation + the contract-section-6 reports:
// the env-derived presence slice, the live WORM capability probe, the destination write-probe,
// computePosture-via-DO (which also routes any regression), and the report assembly/sign/PDF path.
// It imports the shared primitives from router-core + the alert router from router-notify, and never
// imports router.ts (no cycle).
...
import { consumeReportDataFault } from "./test-faults.ts";
...
> {
  ...
  try {
    ...
    // CLASSIFY THE DELETE FAILURE; DO NOT INFER IT FROM THE FACT OF A THROW.
    //
    // dest.delete throws on ANY unusable status as well as on a transport fault, so a bare
    // `catch { deleteProbe = "denied" }` would land a 503 SlowDown, a 500, a 429 throttle and a dropped socket
    // all as "denied" -- a PERMISSION fact the probe has not established -- filing a durable pack row telling
    // support this customer's retention is permanently broken when their IAM allow-list is correct. In the
    // other direction it would make a genuine least-privilege denial waveable-away as a blip. Both directions
    // are wrong, from one row.
    //
    // del() carries the status on the response and throws it TAGGED (DestStatusError), so the verdict
    ...
    // THE DISCRIMINATOR objectLockStatus ALREADY COMPUTES IS CARRIED, NOT DISCARDED.
    //
    //   objectLockUnknownReason  objectLockStatus already returns a CLOSED WormUnknownReason (WORM_UNKNOWN_REASONS)
    //                            saying WHY it could not confirm; reading only status.enabled and discarding the
    //                            rest would collapse two different intents into a byte-identical row: "denied"
    //                            (the least-privilege key that may PutObject but not
    //                            GetBucketObjectLockConfiguration -- and, by the same allow-list, not
    //                            DeleteObject) and "not-implemented" (a store with NO Object-Lock API, where a
    //                            refused delete is the customer's own deny-delete policy). One is a one-line
    ...
    //                            with no default rule (dest/types.ts), so "enforced" alone cannot stand in for
    //                            "locked"; carry the presence of an actual default retention rule as a boolean.
    //
    ...
    //
    // AzureBlobDestination.sasExpiry() carries a doc comment saying it is "for a surface that warns before
    // the day comes", and this probe is that caller: without a caller, the warning surface the method's own
    // comment describes would not exist. A capability with no caller reads exactly like a shipped feature
    // when it is not one.
    //
...
// why this parameter exists at all: without it, this caller throws away routePostureRegressions's returned
// promise, and the posture-regression alert becomes the one alert in the engine with nothing keeping it alive.
export async function computePostureViaDO(env: Env, scheduler: DurableObjectStub, caller: Caller | null, keepAlive?: (task: Promise<unknown>) => void): Promise<PostureReport> {
  ...
  //
  // THE PROMISE IS KEPT INSTEAD OF DROPPED. routeEmission RETURNS its two writes (the alert-emit counter and
  // the /notify/record post that IS the notify-history row) rather than firing them with a bare void, so a
  // call site handing the returned promise to a keep-alive really covers the durable record. Every other
  // routeXxxAlert call site hands it over the same way; a bare `void routePostureRegressions(...)` here would
  // leave nothing for a keep-alive to cover.
  //
  // Under real workerd, with the DO dispatch a tick late, a GET /admin/posture that reports a regression can
  // otherwise lose the notification entirely: the emission is abandoned at its FIRST await (the channel
  // resolve), so the resolve is issued but the webhook delivery never happens, and nobody is told at all. A
  // posture regression is a previously-passing security control that has started failing, so the alert that
  // goes missing is the one that says the account's protection has degraded.
  //
...
export async function handleReport(env: Env, scheduler: DurableObjectStub, caller: Caller, kindRaw: string, params: URLSearchParams): Promise<Response> {
  ...
  if (params.get("format") === "pdf") {
    // The render is GUARDED and its health is RECORDED: an unrenderable value such as a non-numeric
    // lastRestoreTestAt could otherwise throw a RangeError deep inside PDF assembly, so the customer's
    // compliance-report DOWNLOAD 500s -- the report an auditor is waiting for cannot be produced at all, and
    // nothing anywhere says why. The renderer coerces an unrenderable timestamp to "unknown" and COUNTS it (a
    // degraded cell must not become a new silent fallback), and a render that throws for any other reason is
    // counted too, so a 500 on this route is never evidence-free. Both counters ride the pack's existing
...
export async function buildReportBody(env: Env, scheduler: DurableObjectStub, caller: Caller, kind: ReportKind, period: ReportPeriod, now: number, framework?: string): Promise<Report> {
  ...
  if (kind === "change-requests") {
    ...
    // This report is SIGNED, so a row the projector quietly dropped becomes a signed statement that the
...
// assertReportData is the guard on the three report-data DO reads: without it, a DO that answers 500 (a
// storage incident, a contract drift) would have its ERROR BODY cast straight into the report's data shape,
// filled with undefined -- and the engine would SIGN it. A signed compliance report showing "no restore
// tests" / "no changes recorded" is materially worse than no report: it is a confident false statement, and
// the customer's auditor reads it as fact.
//
// It fails LOUDLY instead (the route's caller turns the throw into a 5xx, so the console shows an error
// rather than an empty report) and the failure is COUNTED durably, so a report read that is intermittently
// faulting is visible in the pack even when the retry succeeded. Best-effort counter; never a message or a body.
//
// HARNESS-ONLY: a report-data-read-fail fault, if armed, is consumed here and forces this
// call down the SAME throw-to-5xx branch a real non-ok DO read takes, single-shot, regardless of whether
// resp itself is ok -- proving the guard, not shortcutting it. Gated on testFaultsEnabled first inside
// consumeReportDataFault, so a production/dev/uat/internal engine never touches the fault store.
async function assertReportData(env: Env, scheduler: DurableObjectStub, resp: Response): Promise<void> {
  const forced = await consumeReportDataFault(env, scheduler);
  if (resp.ok && !forced) return;
