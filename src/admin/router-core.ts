// router-core.ts holds the cross-cutting primitives every admin route shares: the JSON
// response/refusal helpers, the auth-signal log lines, the per-route capability gate, the per-caller
// and per-IP rate limiters, the step-up re-auth gate, and the router-executed owner-action dual-control
// gate. The caller-header builder and the first-class audit appenders (including the self-redeploy-retry
// variants) live in router-audit.ts and are re-exported from here so importers that reference them by
// name still resolve. router.ts imports the spokes, and no spoke imports router.ts.
...
import { readSessionCookie, STEPUP_FRESH_MS } from "./session.ts";
...
// fetch runtime (a unit test) falls back to a bare void.
//
...
// jsonRefusal is jsonError PLUS a CLOSED refusal class the console can act on.
//
...
//
// HONEST SCOPE: this is a SHAPE check ONLY (character set + length). It does NOT, and CANNOT from inside
...
// the authn-success / authn-failure signal; they are coarse and carry NO token, NO Access
...
// review and any log-shipping pipeline can see refused sign-ins. It logs the attempted
...
// rejected). It emits through the structured logger so the line lands in Workers Logs with
...
// resolved. It is observability rather than a first-class audit event because it fires on every
// authenticated request that resolves a caller, not because the vocabulary lacks somewhere to put it: the
// closed union carries the `authn-failure` action and the `authn-attempt` target, which the ceremonies
// inside the DO write on the failure side. It logs the auth method and the
...
// structured logger so the line lands in Workers Logs with the JSON shape, and it does not
// alter any response.
export function logAuthSuccess(method: AuthMethod, role: Role): void {
  log("info", `${AUTH_LOG_PREFIX} success method=${method} role=${role}`);
}

// logAuthCeremonySuccess records a COMPLETED sign-in ceremony that minted a session: the passkey
// login/finish and register/finish. The boundary line above fires only on the session's NEXT request, so
// without this the most common human sign-in on an estate not fronted by Access had no record at the moment
// it happened, while the OIDC, SAML and recovery-code sign-ins each write one. Method and ceremony only:
// no email, no credential id, no session value.
export function logAuthCeremonySuccess(method: AuthMethod, ceremony: "login/finish" | "register/finish" | "session/bootstrap"): void {
  log("info", `${AUTH_LOG_PREFIX} success method=${method} ceremony=${ceremony}`);
}

// logSurfaceAuth records the bearer authentication of a MACHINE surface: the SCIM endpoint, the
// /metrics scrape and the /support/* pulls, whose bearers are not sessions and never reach logAuthSuccess.
// "ok" is an info line; any other outcome is the closed refusal class the surface already computes (never
// the presented value). The same shape as the boundary lines so one query finds every authentication.
export function logSurfaceAuth(surface: "scim" | "metrics" | "support-pull", outcome: string): void {
  if (outcome === "ok") log("info", `${AUTH_LOG_PREFIX} success method=token surface=${surface}`);
  else log("error", `${AUTH_LOG_PREFIX} failure method=token surface=${surface} reason=${outcome}`);
}


// logCapabilityDenied records ONE capability-gate 403 (authorization failures, including capability-gate
// denials, are logged) as a coarse console.error line on the same Layer 2 operational stream the authn
// lines above use. It is called from inside gate() itself, the ONE per-route capability check every
// admin route shares, so all of gate()'s call sites are covered without any of them having to remember
// to log -- the same "instrument the one seam" reasoning noteAdminWriteRefusal already uses, applied at
// the seam gate() actually sits at rather than the hub's route table, which names only a small fraction
// of gate()'s many call sites.
//
// It logs the REQUIRED capability and the caller's ROLE only: never the caller's email, subject or
// source IP, matching the no-custody discipline logAuthFailure already holds. A GRANT (callerCan true)
// logs nothing -- a working capability check succeeding on every request is not a security event, and a
// line for it would drown the ones that are (the same noise discipline noteAdminWriteRefusal states for
// a 2xx write).
//
// This is a Layer 2 log line, not a Layer 1 audit-chain entry or a DO-side counter: it costs no DO round
// trip (gate() is synchronous and has neither), and it carries the SAME posture as an unauthenticated 401
// -- a per-refusal row into the tamper-evident chain would amplify against the customer's own audit
// storage (the chain's entry cap rolls over faster, and every row re-crosses the mirror line, the pull
// feed and any configured push destination) for evidence that is materially the same fact each time:
// this caller's role does not hold this capability. Workers Logs has no such amplification path (Layer 2
// leaves the account only if the operator adds their own Logpush job), so a per-denial line there is the
// cheap, sufficient control the 401 carve-out itself draws the line at.
export function logCapabilityDenied(required: Capability, have: Role): void {
  log("error", `authz denied capability=${required} have=${have}`);
}

// gate is the per-route CAPABILITY check. It returns a 403 JSON
...
export function gate(caller: Caller, required: Capability): Response | null {
  ...
  logCapabilityDenied(required, caller.role);
...
// caller gets their own bucket, keyed on the STABLE SUBJECT rather than on method or email, across all
// six auth methods (access, passkey, oidc, saml, token, recovery): email is not a stable identity across
// methods, and keying by method alone would put every non-token caller into one shared budget, letting
// one operator's console automation 429 every other operator's mutating routes and the break-glass
// fallback with them. The bare-token break-glass has no stable identity at all, so it shares one "token"
// bucket: it is the all-or-nothing operator credential, and giving it a single bucket caps an automated
// misuse of the shared token without ever letting a per-IP signal leak in. Keying on the verified caller
// (never the source IP) is the design point: a per-IP limiter would fail closed across a shared NAT/egress
// and harm legitimate customers. Caller.subject is null ONLY for the bare-token break-glass (see its
// declaration), so it is the exact discriminator this function keys on, the same subject-over-email axis
// the rest of this engine authorises on.
...
// fail-closed harm a limiter on this path must avoid. The limiter narrows abuse on the happy
...
// rateLimitedResponse is the ONE producer of a REQUEST-FLOOD 429 on this engine (the per-caller and
// per-IP limiters). Every such 429 body is RFC 9457 problem+json ({ type, title, status, error }) served
// as application/problem+json, with a whole-second Retry-After and the advisory IETF RateLimit +
// RateLimit-Policy headers beside it. The dual-control approve routes answer their OWN, differently-shaped
// 429 (TooSoonRefusal, scheduler-do.ts) for a different question entirely -- not too many requests, but too
// soon after the request this ONE approve is reviewing -- so it is not built here and does not share this
// shape.
//
...
export async function rateLimited(scheduler: DurableObjectStub, caller: Caller): Promise<Response | null> {
  ...
  try {
    ...
    //
    // That fail-open is DELIBERATE. A DO answering a shape with no boolean verdict
    ...
    // The log line dies with the Workers-Logs retention window, so the same
...
//
// The ONE necessary exception to "each string here MUST match a real POST dispatch case": the two
...
// dispatchedPostSubs()'s literal `case "POST ..."` parse; validate-session.ts's structural check reads
...
// Set. Adding the bare "/restore" string here would also gate the harmless, high-volume dry-run path
...
// the approval and before any byte is written -- mirroring the exception above exactly, one static sub
...
export const STEPUP_SUBS: ReadonlySet<string> = new Set([
  ...
  // /signin-factors/revoke is the strictly larger sibling of /passkey/credentials/delete above: it removes
  // EVERY credential for an address plus the recovery record plus every pending invite, in one call, rather
  // than a single credential. It is the one route in this engine that destroys recovery codes, which cannot be
  // re-derived, so the act it performs is irreversible in a way no other member here is.
  //
  // The membership test is exact string equality on `sub`, which is why this route has to be enumerated
  // explicitly: nothing about the route's shape or its capability would pull it in automatically. Access and
  // bare-token callers stay exempt inside requireStepUp exactly as they are for every other member, so this
  // gate applies to the cookie-borne path only.
  "/signin-factors/revoke",
  // DETECTION CONFIG: the alert rules and the channels they
  ...
  // POST /config/signin-context-policy turns the
  ...
  // per-prefix. /config/approval-policy was considered as the stronger candidate and is refuted: the DO
  ...
  // who can disarm outright is the bare-token break-glass owner, who is exempt from step-up anyway. Gating
  ...
  // Approving a prune request is the dangerous
  ...
  // support-credential-mint does not get reliable protection from dual control by default:
  // it is absent from HIGH_BLAST_ALWAYS_GATED (owner-action.ts) and requireConfigApproval
  // defaults false (scheduler-do-org-policy.ts), so ownerActionGate answers "off" and the mint runs inline on
  ...
  // with at least one factor. A stale ambient session that survived a compromise must not be able to sign the
  // legitimate operator's other tabs out unchallenged, so both self-scoped terminations are gated too.
  "/sessions/terminate-others",
  "/sessions/terminate",
  // Emailing a custody share transits the engine on its way to a
...
// requireStepUp gates a SENSITIVE action behind a fresh re-authentication, so a STALE
// ambient session cannot perform it. The bare-token break-glass (all-or-nothing, email-less, holding no
// per-user attribute) is the ONE exempt method. A Cloudflare Access caller is judged on the verified JWT's
// issued-at, the way the control plane's requireFreshActor already does: an assertion minted within
// STEPUP_FRESH_MS is a recent factor and passes; an older one, or one with no finite iat, is refused with a
// 401 { stepUpRequired:true, reauth:"access" } that tells the console to send the operator back through
// Access rather than open the passkey sheet (an Access session has no passkey to assert with). For a cookie
// method it asks the DO (which holds the signing key) whether the session was recently authenticated OR a
// valid single-use step-up token is presented (the DO consumes it). Returns null when satisfied (proceed),
// or a 401 { stepUpRequired:true } the console catches to run the re-auth ceremony and retry. FAILS CLOSED:
// if the DO check is unavailable, the action is denied.
// `actor` is the caller or verdict (method plus, for Access, authIat), or a bare method for a cookie caller.
// `purpose` is the one named exception to the DO's method-aware check: "enrol-passkey" lets a
// fresh OIDC/SAML session satisfy the gate on the passkey self-add route only, because an SSO member with no
// passkey yet has nothing to assert with. Every other caller omits it and an IdP session must assert a passkey.
export async function requireStepUp(req: Request, scheduler: DurableObjectStub, actor: AuthMethod | { method: AuthMethod; authIat?: number }, runtime?: KeepAlive, purpose?: "enrol-passkey" | "end-own-session"): Promise<Response | null> {
  const method = typeof actor === "string" ? actor : actor.method;
  if (method === "token") return null;
  if (method === "access") {
    const authIat = typeof actor === "string" ? undefined : actor.authIat;
    if (typeof authIat === "number" && Number.isFinite(authIat) && Date.now() - authIat * 1000 < STEPUP_FRESH_MS) return null;
    return new Response(JSON.stringify({ error: "step-up required", stepUpRequired: true, reauth: "access" }), { status: 401, headers: { "content-type": "application/json" } });
  }
  ...
  try {
    const resp = await scheduler.fetch(doURL("/stepup/check"), {
      method: "POST",
      body: JSON.stringify({ token, stepUpToken, ...(purpose !== undefined ? { purpose } : {}) }),
      headers: { "content-type": "application/json" },
    });
    const out = (await resp.json()) as { satisfied?: unknown };
    if (out.satisfied === true) return null;
    // THE ANSWERED OUTAGE: stepUpCheck's contract is total: it returns { satisfied: boolean } on EVERY path it takes. But
    ...
    // to the 401 the step-up ceremony OPENS with, so an outage like this is invisible in the response AND in the pack.
    //
    ...
    // Fail-closed on a DO outage returns the SAME 401 "step-up required" as a caller who simply has not
    // stepped up. So an outage is indistinguishable from the ordinary, healthy prompt: from the response and
    // the pack alone, "every sensitive action keeps demanding a passkey and never accepts it" looks the same
    // whether the cause is an outage or a caller who has not stepped up yet.
    //
...
// authRateLimited is the PER-IP pre-check for the UNAUTHENTICATED /admin/auth/* ceremony routes.
...
export async function authRateLimited(scheduler: DurableObjectStub, req: Request, runtime?: KeepAlive): Promise<Response | null> {
  ...
  // edge-guaranteed accept is documented in docs/security/identity-sessions-and-files.md.
  ...
  try {
    ...
    const verdict = (await resp.json()) as { allowed?: boolean; retryAfterMs?: number };
    // FAIL CLOSED on anything but an explicit admit. This is the UNAUTHENTICATED sign-in surface
    ...
    // THE ANSWERED OUTAGE: scheduler.fetch() does NOT throw on an HTTP error status -- the DO's outer
    ...
    // above returns the same 429 the genuine over-cap returns, and NOTHING is recorded. During an outage like
    // this the pack looks byte-identical to a healthy quiet day, even though nobody could sign in.
    //
    // Three states, three rows: a genuine over-cap records auth-ratelimited (the DO's own auto-signal on
    ...
    // Records that the unauthenticated auth limiter FAILED CLOSED on a
...
// updateNeedsDualControl is the risk-class rule for the update-apply / update-settle / update-ramp dual
...
// dangerous activities" principle. The riskClass passed in is the NORMALISED one
...
//
// alwaysGate IS THE CUSTOMER TAKING THAT DECISION BACK (the org policy requireAllUpdateApproval).
// The risk class rides on the VENDOR-SIGNED manifest, so without this flag the party who decides
// whether the customer's second-owner gate is consulted is the vendor: a holder of the release signer
// private key could declare a consequential release "routine" and the gate would never be asked.
// With alwaysGate true the risk class stops being an excuse and every
// live apply consults the gate.
//
// IT CREATES NO GATE. Consulting the gate is not the same as being stopped by it: ownerActionGate still
// answers "proceed" when dual control is off for the account, so an estate with the master gate off is
// unchanged by this flag. A dry-run is never gated either, because the callers check dryRun before asking.
export function updateNeedsDualControl(riskClass: RiskClass, alwaysGate: boolean): boolean {
  if (alwaysGate) return true;
  return riskClass === "migration" || riskClass === "breaking";
}


// requireAllUpdateApproval reads the customer's "gate every engine update" org policy from the DO, for the
// four update paths that decide whether to consult the second-owner gate.
//
// IT FAILS CLOSED, and the reasoning is worth stating because the sibling reads in this file default to the
// permissive answer. An unreadable policy means the DO did not answer, and the very next thing all four
// callers do on a live apply is ask that same DO to record or consume an owner action, which cannot succeed
// either. So the choice is not "gate or do not gate", it is "report an honest refusal from the gate, or ship
// arbitrary engine code because the store that holds the customer's control was unreachable". A customer who
// has required a second owner on every update is entitled to the first. An estate that never set the flag is
// unaffected in the only case that matters, because with the master gate off the gate answers "proceed".
export async function requireAllUpdateApproval(scheduler: DurableObjectStub): Promise<boolean> {
  try {
    const resp = await scheduler.fetch(doURL("/config/update-approval-policy"), { method: "GET" });
    const out = (await resp.json()) as { requireAllUpdateApproval?: unknown };
    if (typeof out.requireAllUpdateApproval === "boolean") return out.requireAllUpdateApproval;
    log("error", "update-approval policy read returned no boolean; treating every update as gated");
    return true;
  } catch (e) {
    log("error", `update-approval policy unavailable, treating every update as gated: ${(e as Error).message.slice(0, 140)}`);
    return true;
  }
}
...
export function resolveRollbackTarget(rec: { pending: null | { fromVersion?: string }; last: null | { outcome?: string; fromVersion?: string } }): string {
  ...
  //
  // The three HONEST outcomes carry the SAME fromVersion convention (the known-good), and each is a
...
// recordAuditAfterSelfDeploy, observeAttachAfterSelfDeploy, stampRestoreProven) live in the
// sibling router-audit.ts; they are re-exported below so importers that reference them by name on
// router-core still resolve. ownerActionGate above imports callerHeaders from that spoke.
// recordBookkeepingAfterSelfDeploy gives the update-lifecycle bookkeeping writes
// the same self-redeploy retry the audit append already has.
export {
  callerHeaders,
