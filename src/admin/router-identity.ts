// router-identity.ts -- the post-auth identity + IdP-connection-management routes (the dispatch-split,
// MOVE-ONLY): the DEMO_MODE reset, the whoami identity echo, the role-table read, and the native-IdP
// (OIDC/OAuth2/SAML) CONNECTION MANAGEMENT surface (NOT the pre-auth login flow, which is router-idp-web).
// Each case body was MOVED VERBATIM from handleAdmin's switch in router.ts; the only change is that the
// shared locals are read off the RouterCtx the hub builds once (after authorise() + resolveCaller), so
// the per-route capability gate runs here exactly as it did inline, unmoved relative to its handler. This
// spoke imports the shared primitives from router-core/router-helpers and never imports router.ts (no cycle).
...
import { IDP_SECRET_AAD, loadConfigWrapKey, maybeWrapConfigSecret } from "./config-secret.ts";
...
// fireInBackground is the module-scope twin of handlePasskey's fireInBackground closure,
...
// the CRYPTO VERDICT -> REFUSAL CLASS map, and the operator sentence that goes with each.
//
// A pure length check on the SIGNATURE STRING has no access to the KEY at all, so it cannot tell a
// recovery-kit signer.pub that is the correct length and CORRUPT (bit rot, a partially restored file, the
// wrong same-size key) from a genuinely ALTERED export: both fail inside the total verify and would report as
// `signature`, telling the support engineer "your recovery artefact was tampered with" when the truth is "your
// kit file is damaged, take another copy" -- the precise inversion this map exists to avoid.
//
...
// recordIdpSetupSignal counts ONE closed IdP setup-time outcome in the bounded auth-signal aggregate: a failed
// "test connection" probe, or a connection proposal REFUSED by the pre-write validators. Both would otherwise
// vanish (the probe persists nothing and a validator refusal 400s before any storage write), which is why "we
// tried to add Okta SSO and it never saves" can arrive with a pack that shows no IdP at all -- indistinguishable
...
function recordIdpTestOutcome(scheduler: DurableObjectStub, result: unknown, runtime?: AdminRuntime): void {
  ...
  // "the SSO test failed with some cert error." The signal above is a COUNTER (how often an IdP test has
...
// that post-reset runs reuse indices against (a deterministic demo-reset chain fork -- two
...
// so only the operator presenting the break-glass token may. Checking ONLY tokenEqual on that direct path
// would silently skip the SAME disablement predicate authorise() enforces on its own token path
// (envFlagEnabled(ADMIN_TOKEN_DISABLED) and the DO's break-glass-retired latch) -- so a token the Owner has
// disabled or retired, believing the shared secret fully neutralised, would keep authenticating here forever,
// letting anyone who ever held it (plus any historically-valid signed export) permanently re-escalate to
// Owner. Defined ONCE and called from all three cases so a future fourth break-glass route cannot repeat an
// incomplete raw-tokenEqual copy. Returns the 401 Response to short-circuit with,
...
async function requireLiveBreakGlassToken(req: Request, env: Env, scheduler: DurableObjectStub, runtime?: AdminRuntime): Promise<Response | null> {
  ...
  if (envFlagEnabled(env.ADMIN_TOKEN_DISABLED) || (await breakGlassRetiredViaDO(scheduler))) {
    // The bearer was BYTE-CORRECT and the fallback is disabled or RETIRED, so this is a presentation of
...
// lockoutRefusalIfLastSignInPath (the lockout guard) refuses deleting or disabling the LAST enabled IdP
...
async function lockoutRefusalIfLastSignInPath(env: Env, scheduler: DurableObjectStub, connId: string, runtime?: AdminRuntime): Promise<Response | null> {
  ...
  try {
    ...
    // THE GUARD THAT PREVENTS AN ACCOUNT LOCKING ITSELF OUT IS RUNNING BLIND HERE. It fails OPEN by design
...
export async function handleIdentity(ctx: RouterCtx): Promise<Response | null> {
  ...
  switch (`${req.method} ${sub}`) {
    ...
    case "POST /demo/reset": {
      ...
      // Reaching here is itself a DEMO_MODE-gated path (the early guard 404s the route
      ...
      // The DO wipe resets runlogCounter to 0, but the destination bucket SURVIVES. Without also
    ...
    // sits at the line-budget ceiling; see that file's header for why, and the case below for the precedent).
    ...
    case "GET /control-plane/export-download": {
      ...
      if (denied) return denied;
      // The operator is trying to OBTAIN a fresh recovery artefact here, so every failure below is recorded
      // rather than left as a bare HTTP error with no durable trace: "the recovery-kit download 502s" must not
      // arrive at support with nothing in the pack, on the one path where the pack may be the only artefact
      // that survives.
    ...
    case "POST /control-plane/import": {
      ...
      try {
        ...
        // THE KIT KEY is damaged, not the export. A signer.pub that will not even PARSE (the wrong
      ...
      // THE SHARPEST EDGE HERE: a LENGTH CHECK ON THE SIGNATURE STRING cannot see the key at all, so an
      // operator-supplied signer.pub that is the correct 2624 bytes and CORRUPT would sail through it, fail
      // inside the total verify, and be recorded as `signature`: byte-identical to a genuinely ALTERED export.
      // "Estate import says signature invalid but the kit is correct" is exactly that failure mode, and a
      // length check would answer it with the OPPOSITE of the truth ("your recovery artefact was tampered
      // with").
      //
      // hybridVerifyDetailed computes the honest verdict and this route keeps it. FIVE worlds, five rows: a
      ...
      if (verdict !== "ok") {
        // The SAME closed class that goes into this engine's own refusal latch rides in the 400 body, so the
        // console can stamp a distinct code on it rather than folding a tampered export, a wrong kit and a
        // damaged .sig into one generic code and one coalesced pack row.
      ...
      // accountIdAbsent is the state in which a PRE-ACCOUNT-DISCOVERY export (built before the engine ever
      // knew its own account id) silently imports EVERYTHING DISABLED -- sameControlPlaneAccount fails safe on
      // a null on either side, so the import "succeeds" and not one downpipe runs. Without naming this state,
      // the customer's estate would be back on the screen with no backup running and nothing saying why.
      ...
      if (!resp.ok) {
        ...
        // The 400 leg carries the closed class; the 403 leg does not need one (the console already has
    ...
    case "POST /control-plane/import-sealed": {
      ...
      // not a signature of the plaintext -- none was ever written for it (each generation writes ONE artefact + ONE
      // signature; a sealed generation's plaintext .json/.json.sig siblings are purged). Same
    ...
    case "POST /control-plane/restore": {
      ...
      // The same key-damaged vs export-tampered split as the estate import above. On THIS path the
    ...
    case "POST /control-plane/apply-staged": {
      ...
      if (!isControlPlaneExport(staged.export) || typeof staged.signature !== "string" || staged.signature.length === 0) {
        // "apply-staged keeps 409ing on a corrupt staged artefact" is the WEDGE this class names: the auto-heal
      ...
      if (stagedVerdict !== "ok") {
        // The STAGED artefact's signature blob not decoding is the permanent WEDGE (the auto-heal parked
    ...
    case "GET /whoami": {
      // WHOAMI PAYLOAD CONSISTENCY, checked at the one place the echo is built.
      //
      ...
      // Combine the verified verdict (method/email/exp) with the DO-resolved role +
      ...
      // For a COOKIE-BORNE session, issue/refresh the double-submit CSRF token the console echoes
    ...
    case "GET /roles": {
      // The member + role table. Gated on roles.read. This capability is declared in the
    ...
    case "POST /idp/connections": {
      ...
      // THE CLIENT SECRET IS SEALED HERE, in the Worker, before it crosses into the Durable Object: the same
      // ingress discipline every other console-set secret already had, and this was the last class without
      // it (the reasoning is at IDP_SECRET_AAD in config-secret.ts). With no CONFIG_WRAP_KEY bound
      // maybeWrapConfigSecret returns the plaintext unchanged, so the back-compat floor is untouched.
      const wrapped = typeof body.secret === "string" && body.secret.length > 0 ? await maybeWrapConfigSecret(loadConfigWrapKey(env.CONFIG_WRAP_KEY), body.secret, IDP_SECRET_AAD) : undefined;
      const forward = wrapped === undefined ? body : { ...body, secret: wrapped };
      const created = await scheduler.fetch(doURL("/idp/conn/create"), { method: "POST", body: JSON.stringify(forward), headers: callerHeaders(caller) });
      // A proposal REFUSED by the pure pre-write validators returns {ok:false, reason} and writes NOTHING
      ...
      // Real-time alert on the sign-in trust surface. Fire-and-forget + fail-open (never affects the op).
    ...
    case "POST /idp/connections/delete": {
      ...
      if (!parsed.ok) return jsonError("request body must be valid JSON", 400);
      // Refuse if this delete would remove the last sign-in path with no other way back in.
    ...
    case "POST /idp/connections/enabled": {
      ...
      if (!parsed.ok) return jsonError("request body must be valid JSON", 400);
      // Refuse if DISABLING this connection would remove the last sign-in path with no other way back in.
    ...
    // POST /idp/connections/cert is the ZERO-DOWNTIME SAML signing-cert ROLLOVER edit: {connId, addCerts?,
    ...
    // It is the same governance class as the other IdP-management routes - the gate
    ...
    case "POST /idp/test": {
      ...
      recordIdpTestOutcome(scheduler, result, runtime); // the probe's closed fail class (never the submitted URL)
    ...
    case "POST /idp/test-saved": {
      ...
      recordIdpTestOutcome(scheduler, result, runtime); // same closed fail class on the re-verify path
      return jsonResponse(result);
    }
    default:
      return null;
