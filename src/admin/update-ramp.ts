// OPT-IN GRADUAL RAMP (default OFF). A normal apply stays the atomic dry-run -> promote -> canary-gate
...
// URLs need *.workers.dev, which downpipes bans; this is the honest, feasible substitute). The flow:
...
// never throws out.
//
// TWO-PHASE: exactly like update-apply.ts's promote/settle split, the canary/self-check gate
...
//
// Moved out of update-apply.ts (file-size remediation). Behaviour is
...
export interface RampInput {
  ...
  settledHighWaterMark?: string; // anti-rollback floor (highest version ever settled); see GuardInput
  // knownRollbackTarget/knownToVersion, threaded straight through to verifyAndGuard: the
...
export async function startGradualRamp(driver: DeployDriver, input: RampInput): Promise<RampResult> {
  ...
  // runs in verifyAndGuard so a guard change lands in one place. The ramp-specific
  ...
  // The shared guard phase and the ramp-specific deploy phase are split so each reads
  // separately.
...
// new version and ramp `percentage`% of traffic to it. It does NOT fly the canary: this
...
// driver.rampVersion is guaranteed present (the caller refused a driver without it). Never throws.
...
//                 ramped slice.
//   rollback-failed-still-split  the ramped version did not pass AND the rollback to 100% prior FAILED.
...
export async function settleAfterRamp(driver: Pick<DeployDriver, "deployVersion">, gate: HealthGate, args: RampSettleArgs): Promise<RampSettleResult> {
  ...
  // RAMP-PATH STRICTNESS, preserved across an earlier incident-fix to decideKeep. That fix
  ...
  try {
    ...
    // A live traffic split still routes to the suspect version even when the enum says "rolled-back".
