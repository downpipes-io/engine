...
import { authRateLimited, logSurfaceAuth } from "./router-core.ts";
...
// approval for config changes" is on (see isScimOffboardCaller's comment in identity.ts: queuing this
// caller like a human proposer would produce a stuck state no approver could ever clear, since no login
// flow can bind this synthetic subject for the queue's approve-time replay to re-resolve authority from).
...
// (see isScimOffboardCaller in identity.ts). It never blocks or fails the SCIM response, and the DO
// drops any out-of-vocabulary name; only the closed event name crosses the wire (never the bearer / email).
//
// The write is CHECKED rather than a bare .catch(() => {}): a scheduler DO that is unavailable (or
// answering non-2xx) would otherwise drop every SCIM signal SILENTLY -- leaving the pack showing a perfectly
// quiet SCIM surface during the exact window in which leavers are not being deprovisioned. A dropped write is
// counted in the droppedWrites aggregate (recordDiagWrite), so an INCOMPLETE pack says so instead of reading
// as clean. It still never throws, never blocks and never alters the SCIM response.
// waitUntil (R-111 follow-up): scim.ts sits OUTSIDE the admin router tree (it is its own
...
// approval queue entirely regardless of the "Require Config Approval" setting - so a 202 (queued)
...
// ScimOffboardError carries the offboarding DO's refusal as STRUCTURE rather than prose: the HTTP status
...
// scimOffboardSignal maps a caught offboarding failure to its CLOSED signal name. The last-Owner guard keeps
...
// STRUCTURED FIELD and never the operator sentence.
//
// WHY A STRUCTURED FIELD RATHER THAN THE OPERATOR SENTENCE, AND WHY THAT MATTERS MORE THAN TIDINESS.
// Matching `text.includes("would remove the last Owner")` (LAST_OWNER_REFUSAL verbatim) would catch only one of
// the guard's TWO sentences: deleteRole refuses with DUAL_CONTROL_FLOOR_REFUSAL whenever the raised
// two-Owner floor fires, and that sentence does not contain the substring. So a SCIM offboard refused by the
// dual-control floor would be filed as a generic 4xx, hiding the signal that would explain it for the case an
// operator is most likely to meet, since arming dual control is what raises the floor. The same miss would
// await any caller of revokeSignInFactors, whose two refusal sentences are its own by design.
//
// It would also make an operator-facing sentence load-bearing in a different module: improving that copy, in
// any of the three places it is written, would silently break a signal with no test naming the dependency. The
// code is a closed identifier a copy edit cannot reach.
//
...
// revokeSignInFactorsFirst issues the offboarding REVOKE ahead of the role delete.
//
...
async function offboard(env: Env, email: string, waitUntil?: (task: Promise<unknown>) => void): Promise<boolean> {
  ...
  if (resp.status !== 200) {
    ...
    // maps it to a SCIM error rather than a false 204. The throw carries the DO's HTTP STATUS and a
    // structured last-Owner flag, so the dispatcher buckets the refusal on the status CLASS instead of
    // matching the DO's prose (a brittle string test that would silently misfile every refusal the day the
    // wording changes). The status is an int and the flag a boolean; the DO's text never reaches the aggregate.
  ...
  try {
    ...
    // The DO answered 200 but its body did not parse, so we do NOT know whether a member was removed.
    // Coercing to removed=false would SILENTLY SUPPRESS the offboarding alert - precisely the
    // alert a takeover/insider scenario depends on. Record the contract fault and continue CONSERVATIVELY
    // (removed=true): a spurious "member removed" alert is far safer than a missed real one.
    recordScimSignal(env, "scim-response-unparseable", waitUntil);
    removed = true;
  }
...
export async function handleScim(req: Request, env: Env, waitUntil?: (task: Promise<unknown>) => void): Promise<Response> {
  ...
  if (!(await scimBearerOk(req, configured))) {
    ...
    logSurfaceAuth("scim", "bearer-rejected"); // V16.3.1: a per-attempt line beside the bounded counter
    return scimError(401, "invalid or missing SCIM bearer token");
  }
  logSurfaceAuth("scim", "ok");
  ...
  if (path.startsWith(usersPrefix)) {
    ...
    // Each id-shape rejection records its OWN closed class. These branches 400/404 a connector FOREVER
    // (the leaver is never removed); without a class per rejection, support could not tell a
    // wrong-id-attribute connector from a healthy-but-idle one. The offending id NEVER rides - only the class.
    ...
    // A connector deactivating via an unsupported method (e.g. PUT with active:false) 405s forever.
  ...
  // The connector is calling a SCIM route this facade does not implement (it expects a fuller provider).
...
// offboard() bypasses that gate entirely for this caller (queuing it instead would produce an unapprovable
// stuck state, see identity.ts's isScimOffboardCaller). The one remaining thrown case in
...
async function handleScimDelete(env: Env, email: string, waitUntil?: (task: Promise<unknown>) => void): Promise<Response> {
  try {
    ...
    // The last-Owner guard (a leaver cannot be removed because they are the sole Owner) keeps its own
    // bucket; every other refusal is bucketed by the DO's HTTP STATUS CLASS (4xx contract drift vs 5xx DO
    // fault) rather than by matching the DO's prose. See scimOffboardSignal.
...
async function handleScimPatch(req: Request, env: Env, email: string, waitUntil?: (task: Promise<unknown>) => void): Promise<Response> {
  ...
  // The two shape rejections below are the Entra "unrecognised PatchOp" failure - a connector whose every
  // deprovision 400s forever while the pack shows a healthy SCIM surface. Only the rejected-shape CLASS is
  // recorded; the PatchOp body (which carries the member's id / attributes) never leaves this frame.
  ...
  try {
    ...
    const message = (e as Error).message;
    // As in the DELETE path, the last-Owner guard keeps its own bucket and everything else is
