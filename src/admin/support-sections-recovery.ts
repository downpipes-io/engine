...
// export pass's per-destination health, and the export-lag pointer). support.ts assembles the bundle
// from these gatherers, each keeping its own redaction contract (see the per-function comments).
...
// refusal reason added there rides into the pack automatically instead of being silently dropped (the
// failure mode a hand-listed copy would produce, e.g. "sealed-no-op-key"). validate-support-autoheal-codes
// pins the two together.
const AUTOHEAL_REFUSAL_CODES: ReadonlySet<string> = new Set(AUTO_HEAL_REFUSAL_CODES);
// The sub-cause + skip vocabularies are bound to their canonical tuples for the SAME reason the refusal
// codes are -- a hand-listed copy is exactly how "empty-recipient-set" (an engine that can NEVER write a
// recovery artefact) would be silently dropped at this boundary while the recorder happily writes it. A
// hand-listed EXPORT_SKIP_CODES set could go stale relative to its own type; binding it closes that.
...
// The per-destination export-fault classes, and the recovery-refusal vocabularies, are taken from the
...
// fetchRecoveryRefusals reads the bounded manual-recovery refusal aggregate. It is fetched as a
...
async function fetchRecoveryRefusals(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  try {
    ...
    return {
      ...
      // are opposite diagnoses and must not read as the same refusal.
...
export async function fetchRecoveryStatus(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  {
    // The MANUAL-path refusals ride in this same section (see fetchRecoveryRefusals). They are gathered
    ...
    // An out-of-vocabulary probe class must not drop the WHOLE amnesiaProbe block, which would make a real
    // self-diagnosis -- "the plane was wiped and here is how I know" -- vanish on a class rename. The block
    // rides with its timestamp and a drift placeholder in place of the unrecognised class; the raw value is
    ...
    const dep = j.deploy && typeof j.deploy.engineVersion === "string"
      ...
      : null;
    // The candidate-scan census is clamped at the pack boundary too (defence in depth: a DO record written
    ...
    // The ESTATE-IMPORT OUTCOME. accountIdAbsent otherwise rides only the import's one-shot HTTP response,
    // which is gone by the time a pack is built, so it must be captured here or it is lost for good.
    // Two states share the same disabled roster and the same audit trail, and without this field the same pack:
    //
    ...
    const exp = j.exportHealth && typeof j.exportHealth === "object"
      ? {
          ...
          // The per-destination REASON. "Some copies of your configuration are stale or missing -- but
          // WHICH destination, and why?" ok:false alone says neither. A WORM lock (permanent, needs a new key
          // scheme), a rotated CONFIG_WRAP_KEY (the credential cannot be opened at all) and a network blip
          // (self-healing) are three different causes behind one boolean without this field, and this record is
          // what the self-backup posture check reads to decide whether the customer's environment is
          // recoverable at all.
          ...
          // perDestTotal / perDestTruncated: the TRUE uncapped destination count, and whether the record
    ...
    return {
      ...
      // A refusal whose CODE is not (yet) in the pack's allowlist must not lose the code entirely, leaving
      ...
      // The SUB-CAUSE. refusedCode "signature" covers a truncated .sig, a rotated signer, a MIXED signer
      // rotation and a genuine tamper -- four incidents, one code, four different responses. refusedCode
      // "unreadable" covers a JSON parse fault, a shape drift and a version pin. Without refusedSubCause the
      // sealed-open failure files as "shape-check", erasing even "this engine is not a recipient of the
      // capsule" -- a state in which no retry can EVER work and recovery must go offline with the break-glass
      // identity.
...
// fetchExportState pulls the control-plane EXPORT-LAG pointer: the config version the LAST signed
