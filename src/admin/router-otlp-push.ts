...
// (the redacted view), POST /otlp-push (set/replace, owner + dual-control) and POST /otlp-push/delete
// (owner, no dual control). Mirrors router-push.ts EXACTLY, minus
...
// fireInBackground mirrors the identical helper in router-identity.ts: it schedules background work via
// waitUntil when a runtime is available, or fires it without awaiting otherwise.
...
function validateOtlpPushSetShape(body: {
  ...
  // Screen the auth header VALUE at config time, matching router-push.ts: a supplied value with a
...
export async function handleOtlpPush(ctx: RouterCtx): Promise<Response | null> {
  ...
  switch (`${req.method} ${sub}`) {
    ...
    // This is the twin of GET /push and is gated the same way for the same reasons: no
    ...
    case "POST /otlp-push": {
      ...
      if (!validated.ok) {
        // Record the refusal, as for SIEM push, so a rejected attempt does not go unlogged.
