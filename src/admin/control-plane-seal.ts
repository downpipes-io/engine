...
export interface SealedControlPlaneExport {
  ...
  //
  // TRADE-OFF, STATED PLAINLY (adversarial review): bodyHash is a HASH COMMITMENT sitting in the
...
//
// Auto-heal is gated on CONFIG_RECIPIENT_PRIVATE, not the operational key: naming the parameter below for the
// operational key would be wrong and misleading, since it could lead a reader to conclude a break-glass-only
// estate cannot self-heal its configuration after a Durable Object wipe. The break-glass-only switch does not
// remove CONFIG_RECIPIENT_PRIVATE (router-keys.ts deletes only the two OPERATIONAL secrets), so auto-heal runs
// in either posture.
//
...
// body. Pure + testable.
//
// The parameter takes env.CONFIG_RECIPIENT_PRIVATE (control-plane-pass.ts:593,624), never the operational
// key; see openSealedControlPlaneExport above for why naming it as the operational key would mislead a
// reader about what auto-heal can do.
