...
//     droppedWrites aggregate rather than vanishing: absence of evidence must never read as absence of
//     problems;
...
/**
 * recordUnwrapFault records ONE destination-credential unwrap failure at the moment of use, so the pack
...
/**
 * recordBindingAlarm records ONE post-write binding-safety alarm. The alarm itself is UNCHANGED: the
...
/**
 * recordUpdateFaults records the already-classified rows of ONE update-pipeline
...
/**
 * recordAdminRefusal records ONE refused admin write as a closed (surface, reason) pair. The refusal
...
//
// A refused save must not die in a dismissed toast. Nineteen call sites across seven spokes make each write
// refusal, and recording at each site would work exactly until the twentieth route was added, so it is
// recorded at the ONE seam every admin write's answer already passes through: the hub's dispatch loop (router.ts).
//
...
// DELETE are different tickets, and folding them into one key would recreate the coalescing this design
// exists to avoid.
const ADMIN_WRITE_SURFACES: Readonly<Record<string, AdminRefusalSurface>> = {
  // THE KEY CEREMONY. Without a recorded surface here, every refusal on these three routes -- a one-shot
  // scoped token that will not mint, a posture the engine will not switch, a refused Cloudflare secret write --
  // is a 4xx into a browser and nothing else, and the key ceremony is the one surface where "I tried three
  // times last week and gave up" ends with an account that cannot decrypt its own archives. They belong in the
  // FROZEN TABLE like every other admin write: the hub already sees them (handleKeys returns through the same
  // dispatch loop). break-glass-only is the POSTURE SWITCH, which the key-rotate surface names explicitly
  // ("rotating a key, or the break-glass posture switch").
