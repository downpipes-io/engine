...
// string, never a value. This is the audit-domain logic for engine-observed presence tracking;
// audit.ts re-exports every symbol here so importers see one public surface.
...
export interface StatusSnapshot {
  ...
  // same-software-version redeploy, e.g. one that silently drops a live-attached source binding (owner-#1),
...
export function diffStatus(prior: StatusSnapshot | null, obs: StatusObservation): { drafts: AuditDraft[]; next: StatusSnapshot } {
  ...
  for (const field of TRACKED_PRESENCE_FIELDS) {
    ...
    // The presence LOSS (true -> false) closes an asymmetry at the heart of a "backups stopped three
    // weeks ago" ticket: the engine recorded a secret APPEARING but never a secret VANISHING, so a deploy that
  ...
  // binding (owner-#1) is now recorded, not only a software upgrade. The deploy-id arm requires BOTH ids
