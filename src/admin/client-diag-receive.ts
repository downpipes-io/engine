// client-diag-receive.ts is the REQUEST-SCOPED receiver for the console-diagnostics support-pack section.
...
//
// INVARIANT I1 (REQUEST-SCOPED ONLY): the projected section is folded into ONLY the bundle produced by the
...
// Set-membership allowlists (the engine re-validation authority) are private to this module so the ONE
// applier that gates on them owns them: no second list can be built against the vocabulary, and each Set is
// DERIVED (new Set(ARRAY)) from the one canonical tuple, so a member added to the vocabulary is admitted here
// by construction and the two cannot drift.
...
// finite count -- a repeat increments it, D7).
...
// clampAttempts projects the client-asserted engineAttempts denominator (D4: the signal reasons over a
...
export function projectRecord(input: unknown): ClientDiagnosticRecord | null {
  ...
  // and refuses to say what, on which ceremony, or whether the capability was even present: a refused
  // identity.key download and an unrelated null-deref would otherwise reduce to the same
  ...
  // These discriminators are each the WHOLE point of the row that carries them, and
  ...
  // These discriminators are projected under the same rule as every field above: each one
  ...
  // channelReasonClass says the channel is not verified and refuses to say why, which is what a six-week silent outage
  // looks like. Both are validated above, so a present-but-drifted value has already failed the record closed.
  ...
  // These discriminators follow the same rule. An admin-write with no adminOp
...
// receivedAtSecond stamps the engine-side receipt time floored to SECOND precision (I4). The client clock is
...
// vendor-pull asymmetry, D9). A PRESENT-but-empty ring yields a section with records:[] (D9: "collected,
