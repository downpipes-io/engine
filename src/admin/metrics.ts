...
// tripping it up on every unattended scrape.
//
...
import { logSurfaceAuth } from "./router-core.ts";

// MONITORING_SURFACES is the closed set of monitoring/observability PULL surfaces the engine exposes. It is
// parsed externally by a coverage-map script (the platform<->chaos-ledger drift detector) as a new
// REALITY family, exactly the way NOTIFY_EVENT_NAMES/ChannelKind/REPORT_KINDS are already parsed out of
// notify/types.ts and admin/reports.ts. A future monitoring surface (e.g. an OTLP metrics-push target, see
// the plan doc's remaining build units) is added here so it shows up as an honest UNMAPPED drift until a
// chaos scenario covers it, rather than shipping invisibly to the harness.
...
export async function gatherMetricsState(scheduler: DurableObjectStub, faults?: { shapeFallback: boolean }): Promise<GatheredDownpipe[]> {
  ...
  // The DO answered 2xx but a body was NOT the shape this renderer expects, so it was coerced to empty.
...
//
// It returns the CLOSED reason alongside the verdict. The client-facing 401 is byte-identical (no
...
// during an outage is exactly the meta-failure this design guards against. Only the closed outcome enum + two booleans
...
export async function handleMetricsRoute(req: Request, scheduler: DurableObjectStub): Promise<Response> {
  ...
  logSurfaceAuth("metrics", auth); // V16.3.1: every scrape's authentication, pass or refuse, as a per-attempt line
  if (auth !== "ok") {
    // The refusal is UNCHANGED: a bare 401 with no detail. Only the closed reason is recorded.
    //
  ...
  try {
    ...
    // Prometheus's own `up` goes to 0 (the honest signal). That behaviour is unchanged; it is also
