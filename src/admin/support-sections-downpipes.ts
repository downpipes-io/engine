...
// SOURCE_FAULT_STATUS_CLASS_SET gates the cf-config discovery's per-surface UNAVAILABILITY class.
...
// drift placeholder rather than to silence -- the raw string never rides either way.
...
// BLACKOUT_RESOLVE_CLASSES mirrors the closed BlackoutResolveClass vocabulary (sched/types.ts). The row
...
// SupportDownpipeState is the FULL roster row shape GET /downpipes returns. It lives here rather than in
// support.ts (which is at its module-size budget) because this is the downpipe-row domain; support.ts imports
// it back as the type of the roster it maps over.
export type SupportDownpipeState = {
  ...
  inFlight: boolean;
  // The monotone sealed-run counter + the per-destination anchor map the engine persists on the downpipe
  ...
  // lastRestoreOom: the restore-subsystem OOM-risk marker the scheduled restore test
  ...
  // restoreTestStartedAt: the drill in-flight marker; present +
  ...
  // lastSealVerify carries the redaction-safe verdict whole; tier0Cause
  // says WHY a verified Tier-0 verdict skipped the decrypt sample (break-glass / sample-off / too-large),
  // and attempts/recovered expose a verdict that self-healed after a
  ...
  // restorability assurance the engine already persists on the downpipe: the offline-restorability
  ...
  // so these ride in declared and copied here.
  ...
  // cfConfigDiscovery is the cached cf-config surface-discovery result the engine already
  ...
  // the closed cron-resolution class; the cron/timezone STRINGS never ride (no-custody), only the class + when.
  nextRunAt?: number;
  inFlightSince?: number;
  cronResolve?: { class: string; at: number };
  // blackoutResolve: the closed outcome of applying this downpipe's blackout (change-freeze) windows at
...
// projectDownpipeDiagnostics returns the extra per-downpipe fields the pack carries. Spread into the
...
export function projectDownpipeDiagnostics(d: SupportDownpipeRow, refusals: Map<string, StateRefusedRow>): Record<string, unknown> {
  ...
  // blackoutResolve: the CLOSED outcome of applying this downpipe's declared blackout (change-freeze)
  ...
  // operator believes they have a freeze that does not exist). Neither state is visible anywhere else. The class + the
  ...
  // selfIdentity: whether this downpipe's stored source config still carries the NATIVE ids a roster
  ...
  // plane loss the downpipe cannot be reconstructed -- a posture that is un-reconstructable and
  ...
  // stateRefused: this downpipe's persisted dp: record was REFUSED at read time by the schema guard
...
// the roster-level twin of the per-row boolean. 0 is meaningful (a fully reconstructable fleet).
...
//
// THE DOUBLE-COUNT GUARD. Reading the raw destinationIds array (blanks dropped, nothing else) here while the
// anchor map beside it is stamped from allDestinationIds -- primary + DE-DUPLICATED replicas, which is what the
// run path actually writes to -- would let a config carrying the same destination id twice produce a configured
// list of length 2 for ONE destination, so that destination would appear TWICE in neverReportedIds and
// neverReportedCount would read 2 where the truth is 1. The count is the number the bot's threshold and the
// pack's severity both read, so a duplicate id in one config would inflate the finding on every consumer at
// once. Using ONE resolver on both sides of the join -- allDestinationIds, the same function the run path and
// nextReplAnchors use -- keeps the configured set and the anchor keys from drifting apart. The cap still bounds
// the tail (the roster section carries an over-wide fan-out).
...
// projectReplication emits the replication block: what HAS reported, what the downpipe was CONFIGURED to
...
// which the console can see and the pack alone cannot. Absent entirely when the downpipe has neither a
// configured fan-out nor a heartbeat: an honest absence, not an empty claim.
//
// `readable` IS THE SECOND ARGUMENT AND IT IS LOAD-BEARING. The never-reported claim is a fact about
// HEARTBEATS, and it can only be made by code that actually READ them. When the /replication read faults the
// section degrades to an empty map, and the difference (configured minus reported) then names EVERY configured
// destination -- so one transient DO blip would make the pack assert, of a destination that holds every run,
// that it has never held anything. That row is not merely undiscriminated: it is FALSE, and it is the row a
// support engineer would escalate on. On an unreadable read the block still carries the intended fan-out
// (which comes off the downpipe's own config and is always readable) and makes NO claim about what has reported.
// THE AGE OF THE SILENCE IS THE WHOLE DIAGNOSIS (see NO-SHIP-DECISIONS.md). A destination configured five
// minutes ago has reported no heartbeat. So has a
// destination that has held no copy of any backup since March. The rows are byte-identical, so an alarm over
// row absence fires on the customer who has done nothing wrong, and a bot that cries wolf devalues every true
// finding it reports. neverReportedSealedRunsSince is the discriminator: how many backups have SUCCEEDED
// since this destination entered the fan-out and still produced no copy of anything. It is a COUNT (sealedRuns
// minus the destination's anchor), never a timestamp of a customer event. 0 = "no backup has succeeded since
// you added it": nothing is owed, nothing is wrong. It is the MAXIMUM across the never-reported destinations,
// because the worst one is the one the customer needs told about.
//
...
// The per-destination replication heartbeats, the configured-destination roster (null when that read
// faulted, so the dangling check is SKIPPED rather than producing false positives), the per-downpipe pre-run
// seal-dispatch faults, the downpipes whose freshness rule cannot arm, and the state-refusal index -- each
// already fetched by the bundle (no extra DO round trip).
export interface ProjectRowOpts {
  ...
  // freshnessTruncation: the downpipes the freshness-fault map REFUSED at its cap, and whether
...
// verdicts the engine persisted on the downpipe, and the fleet-level signals joined onto it. It lives here
// rather than in support.ts (which is at its module-size budget), for the same reason as
// projectDownpipeDiagnostics: this is the downpipe-row domain. Every field is a closed enum / boolean / clamped int /
...
export function projectDownpipeRow(d: SupportDownpipeState, opts: ProjectRowOpts): Record<string, unknown> {
  ...
  return {
    ...
    // over-narrow/typo'd selector vs all-vanished) and a discovery account/zone mismatch, both otherwise
    ...
    // Config shape COUNTS: a silently dropped form row (a secret binding, a schedule blackout
    ...
    // An out-of-vocabulary class does not drop the WHOLE cronResolve block (which would read as "this
    ...
    // A reason code the pack's allowlist has not learned yet does not VANISH: without this, a downpipe whose
    // restore tests have failed for weeks would read as though it never reported a cause at all.
    ...
    // lastRestoreOom: the restore-subsystem OOM-risk marker -- the largest single
    ...
    // Verify-at-seal verdict: redaction-safe (status/tier/sampled/at + coarse reason).
    ...(d.lastSealVerify ? { lastSealVerify: d.lastSealVerify } : {}),
    // Restorability assurance. restoreProven = the offline-restorability proof: WHEN it last passed,
    ...
    ...(Array.isArray(d.recoverySamples) && d.recoverySamples.length > 0
      ...
      : {}),
    // cf-config surface discovery: the last probe's present/empty/gated/unavailable surface-id sets,
    ...
    ...(d.cfConfigDiscovery
      ? {
          cfConfigDiscovery: {
            ...
            // unavailableByClass: the closed STATUS CLASS behind the bare `unavailable` id list. A token
            ...
            // truncated: the surfaces whose probe hit a PAGINATION truncation. A truncated read is
      ...
      : {}),
    // Per-destination replication heartbeats: WHICH destination(s) are failing for this downpipe.
    ...
    //
    // THE INTENDED FAN-OUT RIDES WITH IT; without it the row would be unreadable. A destination that has
    ...
    // which the pack would otherwise carry nowhere. So "P is configured to fan out to D2 and D2 has never once
    // replicated" (escalate: that copy of the backup does not exist) and "P was never configured to fan out to
    // D2 at all" (entirely legitimate; D2 belongs to some other downpipe) would produce a BYTE-IDENTICAL row.
    // The console can tell them apart because the browser holds cfg.destinationIds; the engine holds the same
    // config right here, at pack-build time, and projects it too.
    //
    // Emitting the key only when reported.length > 0 would also erase the worst case: a downpipe on which NOT
    // ONE destination has ever reported would drop the replication key entirely, hiding the most severe
    // instance of the gap hardest. The key is instead emitted whenever the downpipe has a configured fan-out,
    // reported or not. Destination ids are already an accepted class on this exact row (they ride in destinations[].id
    ...
    // danglingDestinationRef: this downpipe pins a destination id absent from the roster, so
    ...
    // lastSealError: the PRE-RUN seal-dispatch fault, attributed to THIS downpipe. The throw lands
    ...
    // freshnessComputable: FALSE means the shared staleness rule can never ARM for this downpipe (a
    ...
    //
    // freshnessComputableUnknown: THE THIRD STATE. The fault map REFUSES a new downpipe once full rather than
    // evicting an old one, so past the cap a downpipe with an unarmable staleness rule is ABSENT from the map,
    // and an absent row carries no field -- which would read as "the staleness rule is fine". The count in
    // schedDiag.capTruncations says some row was dropped but cannot say which, so a clean assertion sitting
    // here would sit beside an unnamed caveat in a different section. The refusal instead names its subject,
    // so this row says the one thing that is actually true of it: whether this downpipe's staleness rule can
    // arm is unknown.
    //
    ...
    ...(freshnessIndex.has(d.config.id)
      ...
      : freshnessTruncation.named.has(d.config.id) || freshnessTruncation.incomplete
        ...
        : {}),
    // blackoutResolve / selfIdentityDegraded + selfIdentityClass / stateRefused:
...
// ---- recoveryRto -------------------------------------------------------------------------------------------
// The RTO estimator's own SELF-ASSESSMENT never otherwise reaches the pack: GET /rto exists on the DO and no
// other gatherer reads it, so its fields (including the two below) reach the pack only through this section.
//
// The two facts it adds are the ones support could not otherwise answer:
//
//   rejectedSamples -- "basedOnDrills says 2 but we ran 15 restore tests". The other 13 carried a non-finite
//   duration or zero verified bytes and are filtered out of the throughput maths with NO record. The FILTER is
//   correct (a zero-duration sample carries no throughput signal and would corrupt the estimate); its SILENCE
//   is the gap, because the pack shows recoverySamples.count and basedOnDrills side by side and the
//   discrepancy reads as a bug in one of them.
//
...
//   archive"). Without it, a fleet that ran 15 drills and has no usable sample would read IDENTICALLY to one
//   that never drilled.
//
