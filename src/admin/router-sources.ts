...
// path matchers + the denied-role guard. This spoke imports no other router spoke (and never router.ts),
...
//
// Every real binding on the estate answers typeof "function" for the twelve method names this file's
// duck-type ladder checks (get/put/list/head/createMultipartUpload/getWithMetadata/prepare/batch/exec/
// idFromName/send/fetch) for EXACTLY one coherent capability group and nothing else (SRC_KV: get/put/
// list/getWithMetadata only; SRC_R2 and the destination DEST_R2: get/put/list/head/createMultipartUpload
// only; SRC_D1: prepare/batch/exec only; the Durable Object bindings SCHEDULER/RATELIMIT_DO/RUNSEAL:
// get/idFromName only, and nothing else). The Secrets Store binding is the one exception: it answers
// typeof "function" for ALL TWELVE, spanning R2 + KV + D1 + Durable-Object + email-send at once. Its own
// property list (Object.getOwnPropertyNames on the VALUE, not the env bag) is empty -- every one of
// those twelve answers comes from a generic, property-name-agnostic capability stub, not a real, narrow
// method set. That is why the exclusion below (`fns(o, ["idFromName"])`, meant to skip a Durable Object
// namespace binding) would otherwise discard it: the Secrets Store value answers "yes" to idFromName
// exactly like it answers "yes" to everything else, so that exclusion must never run before the
// generic-stub check gets a look.
//
// A binding that matches more than one of these mutually exclusive capability groups is, by every
// binding actually measured on a live estate, not really any of them: it is the generic-stub shape only
// the Secrets Store binding exhibits here. That is checked FIRST, before the DO/email/service exclusions
// get a chance to misfire on it, and only a stub that also answers "get" is routed to secrets (still
// excluding null and every primitive; a plain string/number/boolean var or secret is still never
// auto-offered). A binding matching exactly one group is unaffected and reaches the same
...
export function enumerateBoundSources(env: Env): { kv: string[]; r2: string[]; d1: string[]; secrets: string[] } {
  ...
  for (const name of Object.getOwnPropertyNames(bag)) {
    ...
    // The generic-capability-stub shape (see the header above): a real binding on this estate never
    // matches more than one of these five groups. Three or more is a wide, deliberately conservative
    // margin above the two actually needed to identify the stub, so a binding that legitimately overlaps
    // two groups by coincidence is not swept in by this check alone.
    //
    ...
    // exactly like the Secrets Store binding. test/validate-rpc-binding-tripwire.ts fails the build the
    // moment such a binding is declared in wrangler.toml, so that assumption cannot go stale silently;
    // see that file for why, and src/admin/rpc-proxy-bindings-reverified.ts for how to clear it.
...
export async function resolveEngineAccount(env: Env, scheduler: DurableObjectStub): Promise<string | null> {
  try {
    ...
    // A DO fault here silently demotes the engine's own account id to the env var -- or to null, which
...
// Read-only and best-effort: it reads Cloudflare storage analytics (no value reads) for
...
export async function handleEstateSize(env: Env, scheduler: DurableObjectStub): Promise<Response> {
  ...
  try {
    ...
    if (accountId === null || token === null) {
      // The probe never runs at all here -- there is no account id, or no discovery token -- so no sizing
      // outcome is drained and nothing else records that the estimate was impossible. The console renders
      // this honest-empty estate exactly like an empty account, so the counter below is what tells the two
      // apart: it is evidence of a missing token, not evidence of an empty account. The response shape is
      // unchanged.
    ...
    // Drain the probe's closed outcome classes and record them. An unavailable size and a MEASURED zero both
    // render as a 0 to the console, so without this a broken discovery token (or a Cloudflare analytics
    // schema drift, which no customer action can fix) would show the customer an EMPTY ACCOUNT and invite
    // them to conclude they have nothing worth backing up. The response to the console is byte-unchanged;
    // this only records what the probe already computed.
    ...
    // The honest-empty estate the console renders on ANY failure still needs an explanation: record whatever
    // the probe managed to classify before it threw, so an available:false is not the end of the trail.
    // Best-effort, and deliberately AFTER the response shape is decided, so observing the fault can never
    // change what the operator sees.
    await recordCostSizing(scheduler, drainSizingOutcomes());
    // The probe THREW, so the per-source classes may be empty (it may not have reached a single source) and
    // the whole projection collapses to the same mute zero the empty-account case renders. The counter says
...
// and bounds the size so a hostile url cannot exhaust memory. The BYTES are still verified against the
// channel's pinned sha384 by the state machine before any deploy, this only fetches. Returns null on any
// problem (bad url, non-https, non-2xx/redirect, empty, or over the cap).
...
/**
 * fetchArtefactBytesDetailed distinguishes fetchArtefactBytes's causes of failure. A bare null would collapse
 * a CDN 404 (the signed channel names an artefact that does not exist), a 5xx (wait), a refused redirect (a
 * signed update being steered elsewhere -- a security signal), an empty body, an over-cap bundle and a
 * network fault into one fixed audit detail, "artefact-download-failed", even though the causes differ.
 *
...
export async function fetchArtefactBytesDetailed(url: string): Promise<{ ok: true; bytes: Uint8Array } | { ok: false; cause: UpdateCauseClass; httpStatus: number }> {
  ...
  // An artefact URL comes from the fetched channel document, so it is pinned to the channel's own host
  // before anything is requested from it; a signed channel that named another host would otherwise be
  // enough to make the engine fetch from it.
  if (u.hostname.toLowerCase() !== UPDATE_CHANNEL_HOST) return { ok: false, cause: "url-config", httpStatus: 0 };
  try {
...
//
// A SERVED FALLBACK IS NOT SILENT. The walk records each refusal and stamps a RestoreDestFallback onto any
// result it did not get from the first choice, so a restore served by the third copy after two refusals is
// distinguishable from one served by the first; without this, no result shape would carry a destination
// identity and the fault ring projects only from a FAILED outcome. The op also RECEIVES the refusals so
// far, which is how the apply path carries the same fact into the signed receipt core (a receipt is what a
// customer keeps to prove what they recovered and from where).
export async function withRunDestFallback<T extends { ok: boolean }>(
  ...
  // Tracks whether the run carried RECORDED destination id(s) (a string candidate, not the [undefined]
  // default fallback), and whether ANY of them still resolves to a configured destination. When the run
  // recorded destinations but NONE are configured any more, every read fell through to the env default and
  // the only honest cause is "the destination(s) holding this run were removed", not a generic missing object.
  ...
  // Every recorded candidate exhausted on an AVAILABILITY reason AND none of the recorded ids still resolves
  // to a configured destination -> the run's copy lived on a removed destination. Re-label the generic
  // availability reason (e.g. "object missing") with the actionable removed-origin cause so the operator is
  // told to re-add the destination or re-point the restore (the `explicit` override seam), not left chasing
  // a missing object that is structurally unaddressable. This never masks an integrity failure (those
  // already returned above, are excluded from isReplicaFallbackReason, and can never reach here).
...
// the per-account product lister) lives in the leaf ./router-sources-discovery.ts, keeping this spoke
// under its structural budget. Re-exported here so callers importing them by name from
// router-sources.ts (or via router.ts) are unaffected.
export {
  cfApi,
  DISCOVERY_LIST_CAP,
  listAccountProducts,
