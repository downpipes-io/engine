// RTO (Recovery Time Objective) estimate, the recovery-time companion to RPO: RTO is surfaced via the
// live GET /admin/rto route alongside the RPO/freshness signal. This is a PURE
...
export interface RtoEstimate {
  ...
  // The samples the estimate SILENTLY EXCLUDED, and WHY the confidence is what it is.
  //
  // "basedOnDrills says 2 but we ran 15 restore tests" was unanswerable: 13 samples carried a non-finite
  // duration or zero verified bytes and were filtered out of the throughput maths with no record. The FILTER is
  // right (a zero-duration sample carries no throughput signal and would corrupt the estimate); its silence was
  // not, because the pack carries recoverySamples.count and basedOnDrills side by side and the DISCREPANCY
  // between them looked like a bug in one of them.
  rejectedSamples?: { count: number; lastReason: RtoRejectReason }; // present iff at least one sample was excluded
  // And "why is RTO confidence stuck at low despite frequent drills": the 3-value confidence enum ERASES the
  // cause, and the three causes need three different answers ("we cannot size your archive" / "run more than
  // one drill" / "your drills only sample a sliver of the archive"). Present iff the estimate is degraded.
...
export function estimateRto(input: RtoDownpipeInput): RtoEstimate {
  ...
  // and a sample that verified NOTHING are different faults with different fixes, and both used to vanish.
  let rejectedCount = 0;
