// Posture INPUT-SHAPE types (extracted from posture.ts for file-size hygiene and to break an import cycle).
// These are the redaction-safe observable slices computePosture reads. They
...
export interface PostureDownpipeInput {
  ...
  // stays an offender, however affirmative an older proof was (otherwise a stale proof could sign a false
  // PASS into the compliance pack -- see the check for the full note).
  ...
  // key or value. This gate depends on lastRunId actually being projected from DownpipeState, since an
  // empty gate would leave the critical check always passing however stale the tests were; the recency
  // itself is graded off lastRestoreTestAt/Ok above, which the DO does project.
  lastRunId?: string;
...
//                       (providerForEndpoint). It decides the REMEDY, since each provider's mechanism
//                       differs: Azure's is version-level immutability, Google Cloud's is a bucket
//                       created with per-object retention, and R2 has no mechanism at all by any route
//                       (dest/worm-remedy.ts carries the 501 measurement). Honestly absent when no
//                       destination could be resolved, in which case the check falls back to the S3
//                       wording.
...
export interface PostureExpiryInput {
  ...
  // so the scheduler-DO builder can map ExpiryStatus.state through without a narrowing cast. This also
  // exempts an ephemeral pending-cleanup item from the FAIL (a hygiene nudge, not an expiry).
...
export interface PostureInput {
  ...
  // auditMirror (OPTIONAL) is the OFF-ACCOUNT AUDIT MIRROR observable: whether an outbound SIEM audit-log
  // push destination is configured, whether it is draining, and how its last delivery attempt went. All
  // four fields are DO-owned (the push config record and its bounded delivery trail); none of them is a
  // secret, an endpoint or a credential -- two booleans, an optional outcome and an optional timestamp.
  // ABSENT (an older caller) => the check is not raised, rather than a fabricated "no mirror" verdict.
  //
  // lastAttemptOk/lastAttemptAt describe the MOST RECENT attempt only. A configured mirror with no attempt
  // on record yet (both absent) is the freshly-configured state, which the check passes with its strong
  // claim withdrawn: nothing has been observed reaching the mirror, so nothing may be claimed about it.
  auditMirror?: { configured: boolean; enabled: boolean; lastAttemptOk?: boolean; lastAttemptAt?: string };
  ...
  // zero-day cadence, or every estate that has not chosen one would show as permanently overdue. One
  // integer, no secret.
  ...
  // updateIntegrity (OPTIONAL, DP-0/DP-D) is the update-lifecycle observable for the two supply-chain
...
// posture-checks.ts, which would close the two madge cycles admin/posture-checks.ts > admin/posture-checks-
// immutability.ts and admin/posture.ts > admin/posture-checks.ts > admin/posture-checks-immutability.ts.
// posture-checks.ts re-exports both so its importers keep importing from there.
