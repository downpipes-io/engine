...
// `drops` is the bounded, redaction-safe tally. Every `return true` below is a group the provider
...
async function resolveGroups(conn: Oauth2Connection, authHeaders: Record<string, string>, doFetch: typeof fetch, drops: ClaimDropTally): Promise<string[]> {
  ...
  for (const url of conn.groupsUrls ?? []) {
    // The cap stops the walk here too, so a whole membership endpoint can go unread. Same fact, same name.
    if (out.length >= GROUPS_MAX) {
      drops.add(claimDropCounter("oauth2", "groups-list-capped"));
      break;
    }
    ...
    for (const entry of res.value) {
      ...
      if (typeof slug === "string" && typeof orgLogin === "string") {
        ...
        if (s.length > 0 && o.length > 0) {
          // NO `break` on a full list. addGroup returns false the moment the push FILLS the list, and
          // breaking here would skip the cap check at the top of the loop -- so 260 teams capped at 200 would
          // record nothing at all. Falling through to the next iteration lets the top-of-loop guard see that
          // entries REMAIN and record the drop. When the list fills exactly and no entry remains, nothing was
          // dropped and nothing is recorded: no false signal.
      ...
      //
      // NO `break` here either. This is the branch a GENERIC provider (the documented hatch for a flat
      // membership list, idpconn-validators.ts validates conn.groupsPath as a live connection field) walks.
      // addGroup returns false the moment the push FILLS the list, WITHOUT recording a drop, and breaking on
      // that would skip the top-of-loop cap guard -- the sole producer of claim-drop-oauth2-groups-list-capped
      // on this walk. Without the fall-through, 260 asserted groups capped at 200 would record nothing at all,
      // byte-identical to an IdP that asserted no groups: "the user is in the right group and gets viewer"
      // producing the same empty row as "your IdP sent us nothing". Falling through lets the top-of-loop guard
      // see that entries REMAIN and record the drop; when the list fills exactly and no entry remains, nothing
      // was dropped and nothing is recorded.
...
export async function completeOauth2Login(
  ...
  // The tally the bounders fill as they drop. The DO's shared IdP-callback recorder reads
  // principal.claimDrops and bumps each closed name into the admin counters, so a dropped claim on this
  // provider is visible there rather than reading as an IdP that sent no groups at all.
