...
export interface RestoreRequest {
  ...
  exclude?: string[];
  maxRecords?: number;
  // recordName is the FIRST-CLASS single-record path: when set, the restore
  ...
  //
  // confirmDifferentAccountId is the CROSS-ACCOUNT confirmation (the confused-deputy guard). A
  ...
  // (see approvals.ts resolveCfConfigSurfaces), so an approval authorises a concrete surface set
  ...
  //
  // confirmDifferentAccountId is the CROSS-ACCOUNT confirmation, the media-leg twin of the
...
// CrossAccountWarning (the confused-deputy guard) names a restore LEG (cf-config or media)
...
export interface RestorePlan {
  ...
  // cfConfigSurfaces: the RESOLVED cf-config surface allow-list this plan was computed against, in
  ...
  // client-side to show the operator the hash an approval binds to; adding the surface set to the binding
  // without stating it here would leave the mirror unable to reproduce it, because the DEFAULT is the proven
  // set and the console has no copy of that. Hashing account + zone only would put a different hash on
  // screen from the one the approval bound to, so a requester and an approver could not compare them.
  //
  // Stating the resolved list here is deliberately not "let the console re-derive the rule". A second
  // implementation of resolveCfConfigSurfaces would have to track PROVEN_WRITE_SURFACES forever, and one
  // rule with two implementations is how the in-band split drifted across three repos before the catalogue
  // single-sourced it. The console hashes what this says; there is nothing for it to get wrong.
  //
  // An older engine omits it, and an older engine also does not bind surfaces, so a console that falls back
  // to omitting them still matches. The fallback is correct by construction rather than by luck.
  ...
  // crossAccountWarnings: the cf-config / media legs whose target account is NOT provably the
  ...
  // The cf-config zone guard. Present only when a zone-scoped surface would be written into a zone
...
// RESTORE_FAILURE_CLASSES -- WHY one record of a restore did not land, as a closed class tagged AT THE
...
export interface RestoreFailure {
  name: string;
  reason: string;
  // The closed class, tagged at the fault site. Optional on the type only so a failure built by an older
...
//
// WHY A cf-config SURFACE IS ON THE RECEIPT AT ALL. It was not, and that was the defect. The apply path fed
// the receipt from the data loop and the media loop only, so a restore whose cf-config leg wrote NOTHING --
// because the caller's second, edit-scoped Cloudflare token was merely too NARROW, and every item came back
// 401/403 -- produced a receipt with zero records, allVerified VACUOUSLY true, and a digest identical to a
// clean one. The receipt is the artefact a customer keeps as evidence of a recovery, so the failure was not
// merely silent, it was attested. A surface record is the fix: the receipt can now state that the surface was
// attempted and refused, and the digest moves when it was.
// THE ID MAP, AND WHY IT IS ON THE RECEIPT RATHER THAN ONLY ON THE SCREEN. Cloudflare Stream TRANSCODES every
// upload, so a restored video lands on a NEW uid and every reference the customer holds to the archived uid
// has to be repointed. That mapping used to ride ONLY on RestoreResult.mediaRestored, the ephemeral API
// response the console renders. The receipt is the artefact the customer keeps and the one the "Download
// receipt" button writes, and it did not carry the map at all, so after a recovery of many videos the durable
// record could not say which archived uid became which live one. The documentation said the receipt reported
// it, which was true of the screen and false of the file.
//
...
//
// It exists because a fallback used to be invisible. withRunDestFallback discarded the refused attempt and
// returned the replica's result unchanged, no result shape carried a destination identity, and the fault
// ring projects only from a FAILED outcome -- so a restore served from the third copy after the first two
// refused looked identical to one served from the first. The engine's own diagnostic vocabulary named the
// gap ("dest-fallback": which destination refused, before the fallback masked it) and nothing ever wrote
// the row. A customer restoring from a replica has learnt something about their primary that they can only
// act on if they are told it.
//
...
export interface BlindRestoreTest {
  ...
  destFallback?: RestoreDestFallback;
  reason?: string;
  // nothingToVerify: true iff ok is false SOLELY because the run holds no records (run.records.length
...
// CfConfigSkipClass is the closed vocabulary of WHY one cf-config item did not apply on a restore.
...
export type CfConfigSkipClass =
  ...
  | "no-live-phase"
  | "live-only-rules"
  // "read-current-failed" is deliberately absent: it is not a member of CF_WRITE_SKIP_CLASSES and no
  // code path in this repo or any consumer produces it. A closed vocabulary that advertises a class the
  // product cannot emit is a false claim to whoever reads a support pack. If the read-current case is
  // implemented, add it to CF_WRITE_SKIP_CLASSES first and this union follows, which is the direction the
  // drift assertion in validate-cron-source-diag.ts enforces.
  | "rate-limited"
  | "api-unavailable"
  | "other";
...
export interface RestoreResult {
  ...
  // skipReasonCounts is the CLOSED {class: count} map behind `skipped`. The bare integer could not
  ...
  // conflict DIGEST PAIRS. Without this, "some videos restored, some failed" reads as one anonymous {failures: n}:
  // an over-the-200MB-cap video (a platform limit, recover it out of band), a transient Cloudflare blip (retry) and
  // an id occupied by DIFFERENT live bytes (the operator must choose to overwrite or remap) would be indistinguishable.
  // The digests are SHA-384 hashes of the customer's OWN bytes -- the same irreversible join-key idiom the restore
  // receipt already carries -- so a "that image WAS ours" conflict can be adjudicated rather than argued. Honestly
  // ABSENT when no media record failed. Counts + closed class keys + hex digests only; never bytes, ids or a body.
  mediaFaults?: Record<string, number>;
  mediaConflictDigests?: Array<{ archivedSha384: string; liveSha384: string }>;
  // D1 partial apply: the evidence that LOCALISES a half-applied D1 restore -- the worst silent
  ...
  // The restore descriptor fields a sink SHED (a KV expiration that was not a usable number, an R2
...
// PointInTimeRun is the resolution of "the run to restore from AS OF timestamp T":
...
export interface PointInTimeRun {
  ...
  // How many SUCCESSFUL runs were EXCLUDED from this resolution because their startedAt could not be
