...
// 150 bits of entropy, which clears the ASVS V6.5.2 / V11.5.1 112-bit floor for a lookup secret while staying
// writable. Each char is one CSPRNG byte masked to 5 bits, no modulo bias. NOTE: a length change here
// invalidates any codes issued at the old length (they no longer match the length gate), so it must be
// paired with a recovery-code REGENERATION; passkey holders are unaffected (codes are the lost-passkey
// break-glass).
...
export interface RecoveryRecord {
  ...
  // keyProof is the KEY-CONTINUITY witness: a MAC over a fixed, domain-separated label under the SAME key the hashes above were computed with, so a
...
// RECOVERY_CODE_CONFUSABLES is the Crockford base32 confusable MAP, and it is the reason I/L/O/U are absent
// from the alphabet in the first place: a code is READ OFF PAPER and typed back, so the characters that look
// alike have to resolve to the one the code really holds.
//
// Stripping them instead would be wrong: a customer who
// types O for 0 or I for 1 -- the single typo the alphabet exists to tolerate -- would have the character DELETED,
// which makes the normalised code one short, which fails the length check, which produces the same "that
// code did not work" as a wrong code, at the one moment the customer is locked out.
//
// Refusing the input instead would be wrong here and is not what this is: a presented code is not stored and
// carries no bound to enforce, and refusing a stray hyphen or space would break every customer who types the
// code in the grouped form it is DISPLAYED in. Mapping widens nothing -- each presented string still resolves
// to exactly one canonical code, and a minted code contains no I/L/O/U, so generation is unchanged.
//
// U maps to nothing: Crockford excludes it to avoid accidental obscenities, not for visual ambiguity, so
// there is no character it could have been meant to be. It stays stripped.
...
// fakeRecoveryRecord (an anti-enumeration helper) builds a throwaway RecoveryRecord
...
// not an HMAC pass, so the caller (dummyRecoveryRecord) does not run a full extra round of signing before
// verifyCode starts - the no-record path does the SAME crypto work as the real-record path, not
// double it.
...
export interface VerifyResult {
  ...
  // corruptSlots is HOW MANY of the record's stored slots had an UNDECODABLE salt or hash (a support-pack signal).
...
export async function verifyCode(rawKey: Uint8Array, record: RecoveryRecord, presented: unknown): Promise<VerifyResult> {
  ...
  // loop's timing stays uniform, and then simply never match); without it, a corrupt record and a mistyped
  // code produce the identical generic 401.
  let corruptSlots = 0;
