...
export interface AccessResult {
  ...
  exp?: number;
  // iat is the verified JWT issued-at (epoch seconds): the instant Access minted this assertion, which is
  // the closest the engine can get to "when did this person last authenticate to Access". It is threaded
  // out so the step-up gate (ASVS V7.5.1) can treat a fresh Access assertion as a recent factor and refuse
  // a stale one, the same rule the control plane applies (requireFreshActor). Absent when the token has no
  // finite iat, and the gate then fails closed.
  iat?: number;
  ...
  reason?: string;
  // claimDrops is the bounded set of CLOSED counter names naming which claims this token asserted and
...
//
// `drops` is the bounded out-parameter that makes the drop VISIBLE. Every branch below discards
...
// JWKS_REASON are the FIXED internal literals fetchAndImportKey returns when the Access key path itself
...
// accessDenySignal maps a verifyAccessJWT failure REASON to a CLOSED support-signal name, so
...
//
// The classification splits the catch-all cf-access-verify-failed into its actionable sub-causes (skew / alg /
// typ / signature / malformed), which are different tickets with different fixes; cf-access-verify-failed
// remains as the residual bucket. The cf-access-jwks-* family covers the Access KEY PATH being unavailable at all.
...
export function accessDenySignal(reason: string): string {
  ...
  // --- the JWKS availability family: fixed literals, no interpolation ---
...
async function fetchAndImportKey(opts: AccessOptions, issuer: string, kid: string): Promise<CryptoKey | { reason: string }> {
  // The JWKS fetch can THROW straight out of verifyAccessJWT (a cloudflareaccess.com blip, a DNS/egress
  // fault, a non-2xx, a JWKS whose shape changed, or an SSRF-pin refusal on a bad CF_ACCESS_TEAM_DOMAIN), which
  // would otherwise unwind authorise() into a 500 with NOTHING reaching accessDenySignal: an Access outage would
  // be indistinguishable from no traffic in the pack. Catch it here and coarsen it to a FIXED literal, so it becomes an ordinary
  ...
  // importKey REJECTS (a DOMException) on a malformed / non-RSA-shaped JWK entry -- a JWKS the shape guard
  // admitted (it is an object with a keys array) but whose selected key is unusable, and that rejection would
  // otherwise escape as a 500. Coarsen it to the fixed import-failed literal: every token signed under that kid is
...
function extractSignedClaims(payload: AccessPayload, issuer: string): AccessResult {
  ...
  // A PRESENT but unusable idp hint is a drop, not an absence. An absent claim is not recorded (that is
