// SOURCE-DISCOVERY HEALTH: the closed vocabularies, the pure classifier, and the pure applier that is the
// single redaction chokepoint for the discovery aggregate.
//
// GET /sources/discover is the engine's answer to "what can this account back up?", and the console renders
// the whole Add-a-source form from it. It is FAIL-OPEN PER PRODUCT (router-sources-discovery.ts): a token
// missing the R2 scope degrades the R2 listing to a coarse error string and the route still answers 200 with
// an EMPTY r2 array. The operator sees a form with no buckets in it. Nothing is recorded anywhere, so
...
// and "the account is empty" are not the same observation, and this module is what keeps them apart.
//
...
// classifier READS a Cloudflare error string ONLY to SELECT an enum member and RETURNS that enum: the text
// never passes through. No account id, no account name, no bucket, no namespace, no database, no secret
// name, no zone, no token and no endpoint can reach a record -- the router hands this module the classified
// OUTCOME, never the listing.
...
// distance between the first two is the reason this module exists:
...
// ---------------------------------------------------------------------------------------------------------
// THE TOKEN-SET VERDICT
//
// POST /sources/discovery-token answers a flat 400 for every kind of refusal, so the browser's own row
// collapses to {discovery-connect, sources, refused} whether the cause was a typo'd token, a scope-less
// token, an expired token, a Cloudflare outage or a verify that never got an answer. Support cannot remotely
// distinguish a scope gap from an invalid token from that row alone.
//
...
//
// NO-CUSTODY: this classifier follows the same pattern as classifyDiscoveryError. It READS the engine's own
...
// DISCOVERY_TOKEN_SET_FAIL_CLASSES is WHY a pasted discovery token was refused. The first two are the pair
// that matters most, and they have OPPOSITE remedies:
//
...
//   no-expiry           the token verified and can list accounts, but it carries NO expiry, or one further out
//                       than DISCOVERY_TOKEN_MAX_DAYS. A standing account-read credential with no end date is
//                       refused; the remedy is "recreate the token with an Expiry of 90 days or less". The
//                       engine registers the expiry it accepts in the credential lifecycle
//                       registry, so the operator is reminded before it lapses.
export const DISCOVERY_TOKEN_SET_FAIL_CLASSES = ["token-invalid", "scope-insufficient", "cf-api-error", "verify-timeout", "no-expiry"] as const;

// DISCOVERY_TOKEN_MAX_DAYS bounds how far out a discovery token's expiry may sit when it is set. Ninety days
// matches the rotation cadence the security documentation commits to for this credential.
export const DISCOVERY_TOKEN_MAX_DAYS = 90;
 ...
 * valid and may not do that". Telling the two apart matters because their remedies are opposites.
 *
...
export function classifyTokenSetFailure(errors: readonly unknown[], shapeRejected: boolean): DiscoveryTokenSetFailClass {
  ...
  // A token that AUTHENTICATED, listed cleanly and saw no account has a SCOPE problem, not a bad token:
  // Cloudflare answered a clean 200 with an empty result, but the engine still refuses it with the same flat
  // 400 as a typo'd token.
...
export interface DiscoveryObservation {
  ...
  // blackout and a single membership-scoped account are opposite problems ("my D1 databases never show up"
  // versus "half our accounts show no resources"), and the fold alone cannot tell them apart. These fields are
  // COUNTS OF ACCOUNTS by verdict class, per product, computed BEFORE the fold. No account id and no account
  // name is here or can be: the router hands this module numbers.
  readonly accountsByProduct?: Partial<Record<DiscoveryProduct, DiscoveryAccountTally>>;
  // How many of the scanned accounts had AT LEAST ONE degraded product.
...
export interface DiscoveryHealth {
  ...
  readonly accountsByProduct?: Partial<Record<DiscoveryProduct, DiscoveryAccountTally>>; // per-account tallies by product; see DiscoveryObservation
  readonly accountsDegraded?: number; // accounts with at least one degraded product on the last scan
...
export function applyDiscoveryHealth(prior: DiscoveryHealth | undefined, obs: unknown, now: number): DiscoveryHealth {
  ...
  // The per-account tallies go through the SAME chokepoint discipline as everything above -- the product key
...
// ---------------------------------------------------------------------------------------------------------
// ATTACH / RE-ATTACH HEALTH: the highest-severity failure mode in this product, with no record of it failing.
//
// POST /sources/attach and POST /sources/reattach-missing are the two writes that put a source binding onto
// the engine, and reattach-missing is the HEAL for the product's highest-severity risk -- a bare
...
// re-attach saw -- because the plan can also fail by SILENCE: a downpipe whose source has no binding name is
// presented in NONE of the plan's four lists (see roster-reattach.ts MalformedSource), and a CONFLICTING
// claim blocks a binding out of the heal entirely. Both leave the heal reporting success while a downpipe
// stays broken.
//
...
// `detach` is the third, and it is easy to conflate with `attach`: an attach and a detach are the SAME POST to
// the same route, told apart only by which of the two typed argument lists the console filled in, so a refused
// Detach must be classed as its own op rather than folded into Attach. Their failures are not interchangeable
// either: a refused attach means a source was never protected; a refused DETACH means a source the operator
// believes they have removed is STILL BOUND and still being read.
...
// ---- WHICH PHASE of the binding change refused, decided where it is KNOWN --------------------------------
//
// "Attach now / Detach / Re-attach all refuses with an error" sits in this product's highest-severity risk
// area, and a binding-change refusal reaches the console as an identical 400 whichever of three very
...
//
// A CLASSIFIER OVER A MESSAGE IS A GUESS: matching on text alone cannot separate a harness refusal ("safety
// check failed: existing binding X would be dropped; refusing to write") from a Cloudflare write failure with
// a prose body -- both carry no status and no network word, so both would land in `other`, collapsing THE
// PRODUCT WORKED and CLOUDFLARE BROKE into one row. Text matching is unsafe the other way too: the token
// pre-flight's own message contains the word "permission", so a naive classifier would file the engine's OWN
// "this token cannot use Workers KV" as a Cloudflare `auth` rejection.
//
// THESE PHASES DO NOT NEED TO GUESS: each one knows what it refused.
...
// plus the four a phase can establish for itself when Cloudflare did not answer it at all.
//
// The capability pre-flight is why the second group exists. A single BOOLEAN per probe cannot tell "Cloudflare
// said 403 on this capability" (a PROVEN fact: the token lacks it) from "the probe did not answer" (a
// Cloudflare 5xx, a 429, a socket reset: NOTHING about the token was established). Collapsing both to the same
// false value would force both into token-scope, telling support the engine had PROVED the token lacks a
// capability when nothing was proven, sending them to re-mint a perfectly good deploy token while a genuine
// Cloudflare outage sits unexamined in its own logs. A member whose NAME asserts a fact must only be written
// by code that established that fact.
//
...
export const ATTACH_FAULT_CLASSES = [
  ...ATTACH_REFUSAL_CLASSES, // safety-prove-failed | token-scope | token-window | cf-write-failed | verify-unread
  ...
  "binding-alarm", // the POST-WRITE safety harness raised: the write landed and the verify says the bindings are not what was intended. The single most serious member (it is the product's highest-severity risk actually happening, mid-heal)
 ...
 *
 * THE ORDER OF PRECEDENCE IS THE POINT. A TAG beats the text, every time, because a tag was written by the phase
 * that established the fact and the text is a sentence someone wrote for a human. Message matching alone cannot
 * separate the safety harness saying "existing binding X would be dropped; refusing to write" (the product
 * working, nothing written, nothing at risk) from Cloudflare failing the settings PATCH with a prose body (the
 * product not working) -- both carry no status and no network word, so both would land in `other`. It would also
 * read the word "permission" out of the engine's OWN token pre-flight message and file a token-scope refusal as
 * a Cloudflare auth failure.
 *
 * The message is still read for the UNTAGGED remainder, and there it does what tags cannot: it SELECTS a
 ...
 * @param bindingAlarm - true when the caller has already recognised a TAGGED post-write safety alarm,
...
// ---------------------------------------------------------------------------------------------------------
// THE ATTACH REFUSAL HISTORY (the richest diagnostics in the product, otherwise thrown away with the browser tab)
//
...
//
// The ring below keeps a bounded, append-only history of {at, stage, cause, missingCaps} instead. It does NOT
...
export interface AttachObservation {
  ...
  // The RICH refusal evidence, when the throw carried a tag. Absent on a success and on an untagged
...
export interface AttachHealth {
  ...
  // The bounded, APPEND-ONLY refusal history (newest last). It deliberately does not coalesce: the
...
export function applyAttachHealth(prior: AttachHealth | undefined, obs: unknown, now: number): AttachHealth {
  ...
  // A FAILURE IS DECIDED BY THE PRESENCE OF THE FIELD, NOT BY THE RECOGNISABILITY OF ITS VALUE.
  //
  // Treating an out-of-vocabulary class as a fall-through success would make a FAILED attach carrying a class
  // this fold does not recognise come out BYTE-IDENTICAL to a genuinely successful one, silently inflating the
  // success count of the heal that repairs this product's highest-severity risk.
  //
  ...
  // Append the RICH refusal, when the observation carried one. Every field is re-gated against its
