...
// and in-product source-attach routes. Each case reads the shared locals off the RouterCtx the hub builds
// once, so the downpipe.read / keys.ceremony gate on each route runs inline exactly as the route requires.
// Imports shared primitives + the source/attach helpers + the cf-config catalogue/probe; never imports
// router.ts.
...
import { discoveryTokenExpiryVerdict, readTokenExpiry } from "./cf-api.ts";
...
// classifyDiscoveryOutcomes reduces every account's listing to ONE closed per-product verdict, folding
...
// It also counts the accounts. The fold above is lossy in the one direction that matters for a complaint
// like "half our accounts show no resources": folded to the worst, ten accounts denied on D1 and ONE
...
function classifyDiscoveryOutcomes(listings: readonly AccountListing[]): {
  ...
  // An empty account must not MASK a populated one. SEVERITY ranks empty(1) above ok(0), so ONE genuinely
  // empty spare account would fold nine populated accounts to lastOutcome.r2 = "empty" -- the pack asserting
  // "no R2 anywhere" while the wizard shows the customer their buckets. That is a FALSE row, not a missing
  // one, and it is worse. `empty` is reported only for a product NO account had data for; if any account
  // listed rows, the product is not empty and the per-account tally says how many were.
...
// boundSourceCount reduces the engine's OWN bound-source listing to a single COUNT for the discovery record. The
...
// audit rows (attach/detach), the credential-lifecycle row for the spent ephemeral token, and the
// engine's own account id, PROVEN by changeBindings' own read
  ...
  switch (`${req.method} ${sub}`) {
    ...
    case "GET /sources/discover": {
      ...
      try {
        ...
        // tokenPresent:false is a LIE when the token is merely UNREADABLE. "Discovery says no token but
        // we set one" is this branch, and nothing in the pack can tell the two apart.
      ...
      if (token === null) {
        // NO TOKEN. The account-wide tier is never even attempted, so the form shows only the engine's
      ...
      // RECORD WHAT THE FORM ACTUALLY SAW. This route is fail-open per product: every listing above can
      ...
      const observation: DiscoveryObservation = {
        ...
        // The per-account tallies ride BESIDE the folded verdict, so "every account is blacked out on D1"
    ...
    case "POST /downpipes/cf-config/rediscover": {
      ...
      // Cross-account confused-deputy re-check (ASVS V4): addDownpipe enforces this at create
      ...
      // CHECKED. The probe result is returned to the console either way, but a persist that is DROPPED
    ...
    case "POST /sources/discovery-token": {
      ...
      // EVERY EXIT FROM THIS ROUTE SAYS WHICH ONE IT WAS. "Verify and save always fails" is the most common
      // onboarding complaint, and five distinct faults wear that one sentence: a typo'd token, a scope-less
      // token, an expired token, a Cloudflare outage and a verify that never got an answer would otherwise all
      // COALESCE into a flat 400, and the console cannot subdivide a 400. THIS ROUTE MAKES THE CLOUDFLARE CALL
      // AND SEES THE STATUS, so the class is a fact here and an inference anywhere else.
      //
      // The HTTP answers below are exact, message for message; the classifier reads the engine's own
      // `HTTP <status>` literal solely to SELECT a member (the Cloudflare message body, the token and the
      // account ids are read, discarded, and cannot leave).
      ...
      if (resolved.accounts.length === 0) {
        // The refusal this route decides. A token Cloudflare answered 403 for (valid credential, missing
        ...
        // got an answer. The engine refuses a zero-account token, so `verified-zero-accounts` is never a state
        // the console can observe directly: this row is where that fact lives.
      ...
      // list empty?" -- and that is the discover half, a different record with a different remedy.
      // V13.2.1: the token is a STANDING backend credential, so it must be term-limited. Its public id and
      // expiry are read off the same verify endpoint (never the value); no expiry, an unreadable one, or one
      // further out than DISCOVERY_TOKEN_MAX_DAYS is refused with the closed class no-expiry, and nothing
      // reaches the DO. The facts that pass ride to the DO so the credential lifecycle registry can remind
      // the operator before the token lapses.
      const expiry = await readTokenExpiry(token, resolved.accounts[0]!.id, fetch);
      const expiryProblem = discoveryTokenExpiryVerdict(expiry.expiresOn, DISCOVERY_TOKEN_MAX_DAYS);
      if (expiryProblem !== null) {
        fireInBackground(runtime, recordDiscoveryTokenSet(scheduler, { outcome: "refused", failClass: "no-expiry", accountsSeen: resolved.accounts.length }));
        return new Response(JSON.stringify({ error: `token refused: ${expiryProblem}` }), { status: 400, headers: { "content-type": "application/json" } });
      }
      ...
      return scheduler.fetch(doURL("/sources/discovery-token"), {
        method: "POST",
        body: JSON.stringify({ token: sealed, accountsSeen: resolved.accounts, ...(expiry.tokenId !== undefined ? { tokenId: expiry.tokenId } : {}), expiresOn: expiry.expiresOn }),
        headers: callerHeaders(caller),
      });
    }
    ...
    case "POST /sources/attach": {
      ...
      // WHICH OPERATION THIS IS can only be decided here. An attach and a detach are the SAME POST
      // to this one route, told apart only by which of the two typed lists the console filled in, so a refused
      // detach must never be recorded as a refused attach. Support cases name all three actions ("Attach now / Detach /
      ...
      if (!validateDeployToken(tok)) {
        // A refusal on this route is recorded, not only returned as a response. A token that is not
        // token-shaped means the write never left the engine, which is a completely different failure from a
      ...
      if (account instanceof Response) {
        ...
        // No source can EVER be attached in this state; recording it here gives the refusal a durable trace.
        await recordAttachHealth(scheduler, { op, fault: "account-unknown" });
        return account;
      }
      // The window-check BLIND SPOT. Cloudflare's token-verify endpoint could not be reached, so the
      ...
      try {
        ...
        // A post-write safety alarm must not be visible only in this HTTP response: the alarm that names a
        // dropped binding -- or proves a concurrent writer's lost update -- would otherwise die with the
        // operator's browser tab, leaving a healthy-looking sources-attached row beside a source that has
        // quietly stopped backing up. Persist the closed kind + the binding labels; the refusal itself is
        // unchanged.
        // The alarm is narrow BY DESIGN (it fires only on a tagged post-write safety fault), so an ORDINARY
        // failure -- a 403 on the token, a Cloudflare 5xx, a network reset -- would otherwise record NOTHING.
        // The attach record carries the classified cause of EVERY failure, with the tagged alarm outranking the
        ...
        // A TAGGED refusal carries the rich evidence (which stage, which cause, which capability is
    ...
    case "POST /sources/reattach-missing": {
      ...
      // The plan's OWN SHAPE, recorded on every re-attach whatever its outcome. The two counts that
      ...
      const noteReattach = async (fault?: AttachFault, e?: unknown): Promise<void> => {
        // The re-attach runs the SAME pipeline, so it can be refused by the same guards -- and its
      ...
      // A CONFLICTING claim is two downpipes naming the SAME binding as DIFFERENT Cloudflare resources.
      ...
      //
      // The signal is recorded here, before the branch splits, so it also covers the state that would otherwise
      // stay silent: planRosterReattach deletes only the CONTESTED binding names from toAttach, so the ordinary
      // shape -- three rebuildable sources plus one contested binding -- has toAttach.length > 0 AND
      // conflicting.length > 0, falls through to the write below, and answers 200 with an attached[] list. That
      // is precisely the state the signal's own vocabulary describes ("the affected sources stay unprotected
      // while the heal REPORTS SUCCESS"). Recording it here means a heal that attaches three of four and leaves
      // the fourth contested still gets a row for it.
      ...
      if (plan.toAttach.length === 0) {
        ...
        // binding is conflicting or malformed is a heal that can never run, and answering 200 with an
        // all-clear-shaped body would hide that. Recorded as plan-refused so the pack can tell the two apart.
      ...
      try {
        ...
        // The re-attach path runs the SAME post-write safety harness, so it can raise the same alarms.
        const alarm = await recordBindingAlarmFrom(scheduler, e);
        // "reattach-missing keeps failing after the deploy wiped bindings" is the owner's #1 fear with the heal
        // broken for it. The classified cause of every failure lands here.
...
// recordBindingAlarmFrom persists a TAGGED post-write binding-safety alarm into the DO's bounded ring.
...
// Returns TRUE when the throw WAS a tagged post-write alarm, so the attach record can class it as
// "binding-alarm" -- the one class that means the write went through and the engine's bindings are now not
// what was intended, which is the owner's #1 fear actually happening, mid-heal.
async function recordBindingAlarmFrom(scheduler: DurableObjectStub, e: unknown): Promise<boolean> {
  const tag = bindingAlarmOf(e);
  if (tag === null) {
    // THE VERIFY THAT NEVER RAN. verifyAfter's alarms all mean "the post-write check ran and
    ...
    // owner's #1 fear domain, with the safety harness blind. `postwrite-reread-failed` names exactly this case,
    // and this branch is its only producer, so the bindingAlarms ring -- the ONE place a support engineer looks
    // to ask "did a settings write break this engine's bindings" -- would otherwise read clean for it.
    //
