...
export const SSO_FAIL_CODES = [
  ...
  "groups-overflow", // the IdP WITHHELD the groups claim because the user is in too many groups (the Entra >200-group overage). Its own code because the remediation is unique and the consequence is a SILENT AUTHORIZATION DOWNGRADE: without this code the refusal falls to "other", which reads as an unclassified oddity rather than "configure App Roles or a groups filter". This is the "our admins lost console admin overnight" ticket.
  // The two SSO paths that are otherwise entirely UNCOUNTED, because the aggregate is written only by the
  // CALLBACK/ACS wrapper. Both are recorded with an EXPLICIT code (they are known by their PATH, not by
  // parsing a message), so neither is produced by classifySsoFailure below -- they are members of the closed
  // code set so they can ride the same aggregate, the same by-kind axis and the same pack projection.
  "start-failure", // the sign-in died at /start: the connection is disabled, deleted, or the AuthnRequest could not be built. "Users clicking our old SSO link get an error page" -- and the pack showed zero SSO failures.
...
const RULES: Array<{ re: RegExp; code: SsoFailCode }> = [
  ...
  // because falling through to "other" would hide a silent authorization downgrade behind an unclassified bucket.
...
// classifySsoStartFailure is the START-path sibling of classifySsoFailure. The SSO-failure aggregate is
...
// ---------------------------------------------------------------------------------------------------------
// THE SSO FAILURE SUB-CAUSE (the remediation-bearing half of an "SSO broke overnight" ticket)
//
// The 10 SSO_FAIL_CODES above name the STAGE that refused (issuer / key / signature / replay / malformed).
// They do not name the CAUSE, and the cause is what the operator has to fix. Support cannot tell:
//   - an EXPIRED Entra client secret (the token endpoint answered `invalid_client`) from any other token-
//     endpoint failure. The answer WAS in the token response and was thrown away. This is the single most
//     common "SSO broke overnight and we changed nothing" ticket.
//   - a kid-not-found after a JWKS rotation from a duplicate-kid document that refuses the whole JWKS.
//   - state-missing (no cookie) from state-replayed (a double-submitted callback) from state-expired.
// And two SAML classes are ACTIVELY MISCLASSIFIED by the coarse code: an IdP-initiated POST at an
// SP-initiated-only connection files under `replay` (it is a CONFIG toggle, not an attack), and the IdP's own
// non-Success StatusCode -- the one field that says "your IdP DECLINED this user" (unassigned app, MFA
// refused) -- was never even READ, so it fell into `other`.
//
...
// the closed set below and is then DISCARDED. The two MARKS the reject sites embed (`[oauth:<code>]` and
...
export const SSO_SUB_CODES = [
  ...
  // --- SAML: the IdP's OWN StatusCode (never read before this: it is the field that says the IdP DECLINED) ---
  ...
  // --- SAML: the config-toggle and out-of-scope refusals (previously misfiled as replay / malformed) ---
  "idp-initiated-disallowed", // an IdP-initiated POST (an IdP "tile" launch) at an SP-initiated-only connection. A CONFIG TOGGLE, not a replay attack -- the misroute this sub-code exists to correct
  "encrypted-assertion-unsupported", // the IdP is ENCRYPTING assertions and this build is sign-only. A precise, actionable refusal that used to read as "malformed"
 ...
 * for the `sso-sub-*` auth-signal counters. PURE. The reason is read transiently and NEVER
...
//
// THE GAP: every SSO failure the pack carries was classified BY THE DURABLE OBJECT -- which means only failures
// that REACHED the DO were ever counted. Four legs never do:
//   - the ACS form / callback query would not parse (a proxy that mangled the cross-origin POST);
//   - the DO round-trip from the ACS / callback / metadata handler faulted (an engine availability fault, which
//     the user experiences as "clicking Sign in with Okta bounces straight back");
//   - the SP metadata endpoint could not render, so the customer's IdP cannot even IMPORT the connection;
//   - and, the single most diagnostic artefact of them all, the IdP's OWN `?error=` code on the callback --
//     the field in which the IdP says WHY it declined -- which was read, used to choose a generic redirect,
//     and then thrown away.
// So the pack shows ZERO SSO failures through an outage the customer swears was full of them.
//
