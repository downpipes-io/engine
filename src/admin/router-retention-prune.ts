...
// tick. Without this route the console's only answer is "prune offline with the reader instead" -- a
// terminal instruction, which the house no-customer-CLI rule forbids (see the editor-retention-section.ts
// hint this route lets us shorten).
//
// This route reuses the same shape the attended-verification session (router-attest.ts) and the
// break-glass restore panel (router-restore.ts POST /restore/capsule) already establish for this exact
// problem: the engine never asks for the break-glass PRIVATE. It serves each candidate run's NON-SECRET
// master capsule (POST /retention-prune/candidate), the browser recovers each run's 32-byte master locally
// (keydecap.ts openCapsule, entirely client-side), and the browser hands back ONLY those per-run masters,
// batched, on POST /retention-prune/apply. The masters are used to build a SegEnumerator for exactly one
// call and are never stored; every recovered master this router touches is zeroed before the response is
// built, on every exit path (see the try/finally around the plan phase below).
//
...
// (openRunWithMaster).
//
// THE ABSTAIN GAP. planPrune's own ABSTAIN invariant only protects a segment referenced by a RETAINED run:
// if a SUPERSEDED run cannot be opened, buildSupersededSegRefs silently EXCLUDES it from the superseded
// reference set (fail-safe on its own downpipe-wide view) -- but a segment that run shares with a DIFFERENT
// superseded run that DID open is then computed as an orphan and deleted, even though the excluded run's
// own RUNLOG entry stays "active" (never superseded) and its manifest still references that now-missing
// segment. A four-run fixture (two superseded runs sharing one content-addressed segment, one master
// omitted from the batch) demonstrates the failure: the segment would be deleted while its still-active
// manifest referenced it. planPrune itself is NOT changed (it stays the cron's unmodified,
// exhaustively-tested planner); instead this route requires the operator's batch to open EVERY candidate
// run (retained AND superseded) before planPrune is ever called at all (precheckCandidateCoverage below).
// A batch that cannot open every candidate is refused outright, naming every run that failed, whether the
// master was omitted or simply wrong -- so planPrune's own internal enumeration can never hit a run this
// route has not already proven readable, and the shared-segment scenario above cannot arise: either every
// superseded run opens (nothing is silently excluded) or the whole call refuses before any delete.
//
// DUAL CONTROL. retention.enforce is armed via plain downpipe.write (dual-controlled only when the opt-in
// requireConfigApproval gate is on, which is OFF by default), and a restore apply -- overwriting live data,
// but leaving the archive untouched -- unconditionally requires a SECOND authorised identity's approval
// (router-restore.ts, admin/approvals.ts). A prune apply deletes the archive itself, a LESS reversible act,
// so a single identity plus step-up is not enough: this route requires its OWN maker != checker approval
// (admin/prune-approvals.ts, mirroring the RestoreApproval/planHash pattern) bound to a keyless planHash
// over the downpipe's CURRENT retained/superseded split, before ANY real delete. Preview (previewOnly:true,
// or a downpipe whose stored enforce is not true) needs no approval: it can never delete. See
// prune-approvals.ts's header for the single-owner-estate posture (no carve-out: identical to restore's, a
// sole owner is blocked until a second identity exists).
//
...
// data-plane action, so it gets the platform's data-plane dual-control precedent instead, per the dual
...
//
// THE BOUND IS RIGHT, AND A NAIVE READING OF IT IS WRONG (MEASURED, validate-retention-prune-cap-honesty).
// The naive reading says "a downpipe with more active runs than this simply cannot be pruned from this
// route in one call; the cron path (once an operational key is added) has no such bound, so this is a
// console-usability limit, not a correctness one." Both halves fail on the posture this route exists for.
//
...
//     every candidate, and precheckCandidateCoverage (deliberately) refuses any batch that does not.
//     Driven over every batch size from 1 to N at N = 201: every one of them refuses.
...
// RETENTION_MAX_KEEP_RUNS (10000), so raising this to match would defeat its own purpose. The refusal says
// which population it counted and names the remedy that actually lowers it: POST /keys/add-operational,
// the console's "add operational key" card, which restores the scheduled pass that has no such bound --
// another control in this product, named in the refusal itself.
...
> {
  ...
  // Replica coverage, read fresh (best-effort/conservative on a hiccup, exactly as the cron reads it):
  ...
  // replUnreadable is carried so the attended pass record can say honestly, exactly as the cron
...
// plan is computed, closing the shared-segment gap the adversarial review's four-run fixture proved: a
// superseded run planPrune's own downpipe-wide view would otherwise silently exclude (and so never protect
// a segment it shares with a sibling superseded run that DID open) can no longer reach planPrune at all.
...
// retention-pass.ts recordRetentionPass), so an attended prune reaches the same retention telemetry a
// cron pass feeds rather than only fault observations. The record is marked `attended` and scoped to the
// ONE downpipe this call actually pruned
  ...
  switch (`${req.method} ${sub}`) {
    ...
    case "POST /retention-prune/apply": {
      ...
      if (wantsRealApply) {
        ...
        // The ATOMIC gate-to-work reservation (ASVS 2.1.6 parity): flips approved -> applying so a
      ...
      // RESERVATION-LEAK GUARD: from the moment reserve() above succeeds, this call HOLDS a dual-control
      // reservation that MUST be settled exactly once, on EVERY exit path, including a throw. settleAction
      // is the single source of truth for what the outer finally does; it defaults to the SAFE choice
      // (release, so a genuinely-approved plan is never stuck) and is flipped to "consume" at the ONE point
      // below where the apply genuinely, fully succeeded. The WHOLE reserve-to-consume window (planPrune
      // included) sits inside the guarded region below, including planPrune's OWN call (listRunTree ->
      // dest.list(), a real, reachable R2 failure mode): a throw there must not leave the reservation stuck
      // "applying" for the 30-minute PRUNE_APPLY_LEASE_MS lease to self-heal, which would refuse an
      // immediate honest retry of an already-approved plan.
      ...
      try {
        ...
        // The counts each attended pass-record row below carries follow the same planned-or-committed
        ...
        if (!wantsRealApply) {
          ...
          // falsely say enforcement is off.
        ...
        try {
          ...
          try {
            ...
            if (e instanceof PruneApplyError) {
              ...
              // which this route knows and the cron's catch site does not.
              await postAttendedPassRecord(scheduler, now.getTime(), target, {
                id: downpipeId,
                destKey,
                outcome: "error",
