...
// group->role mapping, the composable custom roles and the audit-trail reads plus the owner-only intent
// marker. Each case body reads the shared locals off the RouterCtx the hub builds once, so the
// roles.write / access.policy / keys.ceremony gate on each route runs here exactly as it would inline.
// Imports shared primitives + the role-invite helpers; never imports router.ts.
...
// record ever carries about them (a single `filtered` boolean): the VALUES are an actor e-mail, an action,
  ...
  switch (`${req.method} ${sub}`) {
    ...
    case "POST /roles": {
      ...
      if (roleResp.status === 200) {
        ...
        if (entry) {
          // The invite send is fail-open BY DESIGN (email is observability, never a control). Without
          // recording the skip class, the closed { sent:false, reason } this computes would be discarded, so
          // "my new admin never received the invite" would have no evidence anywhere: the grant COMMITTED,
          // the person HAS access, and nobody could tell whether the engine even tried to email them. The
          // grant path below is unchanged (still fail-open); recording the skip class closes that gap.
          // Never the grantee's address or the provider's rejection text.
          ...
          // The grant MINTED a registration invite (so this person has no passkey and cannot sign in
    ...
    // roles.read enforces "any authenticated role may read it": the DO arm this forwards to is
    // listGroupRoles() with no caller argument (scheduler-do-routing.ts), so without a gate here the
    // callerHeaders on this line would be decoded by nothing. roles.read is the capability that says
    // exactly what the sentence says, and it is the one GET /roles already gates on (router-identity.ts)
    // over the sibling table in the same read family. It sits in the viewer floor, so every built-in holds
    // it and a custom role cannot subtract it: this refuses nobody and turns "any authenticated role" from
    // prose into the check.
    ...
    // Gated the same way as GET /group-roles above, and for the same reasons: the DO arm is
    ...
    // ---- audit: reads gate on audit.read, declared in the contract and shown in the console's
    // roles-builder (a creator can tick or untick it). The three gates below make audit.read a real, live
    // authorisation check rather than a phantom one (proven directly against a hand-built caller lacking
    // it in validate-custom-roles.ts). PRECISE CLAIM: every built-in role holds audit.read from the viewer
    // floor up, and identity-rbac.ts's read floor is additionally folded, unconditionally, into every
    // custom role's resolved capability set (resolveAuthority, scheduler-do-rbac.ts:530-533, "custom roles
    // are additive"), so no custom role composable through the product today can actually be excluded from
    // it -- these gates are correct and future-proofing (they bite the moment a narrower caller becomes
    // constructible), not a live boundary against any caller reachable now. The intent marker is
    // Owner-only.
    case "GET /audit": {
    ...
    case "GET /audit/export": {
      ...
      if (denied) return denied;
      //
      // RECORD THE ATTEMPT. This forward is the ONLY place that sees both ways an export can fail -- the
      // DO answering a non-2xx (the whole-log scan died), and the round-trip throwing (the DO is unreachable)
      // -- so recording both here is what stops "we cannot get our audit log out" leaving no trace anywhere
      // in the engine. Without it a customer could not PROVE what happened, on the one artefact whose entire
      // purpose is proof; and the failure is by construction the one fact the exported log itself can never
      // carry. The record is three closed enums and a boolean: never the filter, never the chain, never a
      // byte of the log.
    ...
    case "POST /audit/intent": {
      ...
      // result or a value. When the operator runs the key ceremony or is guided through an
