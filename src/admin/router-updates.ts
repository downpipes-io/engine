...
// forwarders that sit beside them (the dispatch-split, MOVE-ONLY): GET /downpipes|updates|history|runs/at|
// rto|replication|licence and the two-phase update apply/settle/status/rollback/ramp/ramp-settle lifecycle
// (the gradual ramp is ALSO two-phase -- POST /update/ramp never flies the canary, POST
// /update/ramp/settle, a genuinely separate request, does). POST /update/apply and POST
// /update/ramp both refuse (409, pendingUpdateRefusal) while a PRIOR apply/ramp's verification is still open,
// so the documented "finish the ramp via apply" step can never reach the deploy machinery mid-split. Each
// case body is MOVED VERBATIM from handleAdmin's switch in router.ts; the only change is reading the shared
// locals off the RouterCtx the hub builds once, so the per-route capability gate runs here exactly as it does
// inline. Imports the shared primitives + the update-apply orchestration; never imports router.ts.
...
import { callerHeaders, gate, jsonError, jsonResponse, ownerActionGate, ownerActionQueuedResponse, rateLimited, recordAudit, recordAuditAfterSelfDeploy, recordBookkeepingAfterSelfDeploy, recordVerifiedEngineAccountAfterSelfDeploy, requireAllUpdateApproval, updateNeedsDualControl, validateDeployToken } from "./router-core.ts";
...
// HARNESS-ONLY test-fault injection (owner-gated engine test-fault-hook). Every helper is a no-op unless
// HARNESS_TEST_FAULTS is set (see src/admin/test-faults.ts), so a production settle is byte-identical.
import { consumeDropSourceBindingFault, consumeSettleFault, deployFailingDriver, forcedDeadGate } from "./test-faults.ts";
...
// resolveTrustedRampTarget supplies verifyAndGuard's record-rollback-target step with the
...
export async function handleUpdates(ctx: RouterCtx): Promise<Response | null> {
  ...
  switch (`${req.method} ${sub}`) {
    // GET /downpipes returns listDownpipes() VERBATIM
    ...
    case "GET /history": {
      ...
      //
      // This route documents a floor it did not hold: router-keys.ts gates GET /keys/vintages on
      // downpipe.read and says it is "the same cap as GET /history and GET /runs/at, the sibling
      // run-history reads". Of the two siblings that sentence names, /runs/at gates on downpipe.read and
      // /history gates on nothing, so the sentence is true of the intent and false of the code. This route also strictly dominates /runs/at, which resolves
    ...
    case "GET /runs/at": {
      // POINT-IN-TIME run resolution: GET /admin/runs/at?downpipe=&at=<rfc3339> -> the resolved
      ...
      const denied = gate(caller, "downpipe.read");
      if (denied) return denied;
      // This route is the recovery TIMELINE's resolver -- an operator picking the moment to restore
      // from. A bare forward with no catch would let a DO fault escape to the Worker's last-resort handler,
      // leaving the console showing a generic failure with nothing anywhere naming the point-in-time lookup
      // as the thing that broke, so it catches that fault here. It is read-only, so its stage can only ever
      // be do-read (nothing is changed here, and unlike its siblings that claim is unconditionally true).
      let resp: Response;
    ...
    case "GET /rto": {
      // RTO ESTIMATE: GET /admin/rto[?id=<downpipeId>] -> the per-downpipe + fleet recovery-time
    ...
    case "GET /replication": {
      ...
      //
      // Redaction-safe is a claim about the BODY only, not about the caller. This is per-downpipe state
      // keyed on the downpipe id, so it takes the same downpipe.read floor as the sibling forwarder it
      // names above; the wording "same thin authenticated forwarder as /history" is true of the gate as
      // well as of the shape. No caller's answer changes today, for the reason set out on GET /downpipes.
      const denied = gate(caller, "downpipe.read");
      if (denied) return denied;
    ...
    case "POST /licence": {
      ...
      // `stage` is the LAST CHECKPOINT PASSED, advanced as the route makes progress. The outer catch
      ...
      try {
        ...
        if (body.token === null) {
          ...
          await recordDiagWrite(scheduler, "licence-refusal", () => scheduler.fetch(doURL("/expiry/observe-licence"), { method: "POST", body: JSON.stringify({ notAfter: cleared.notAfter ?? null }) })); // a dropped expiry observe means the licence-expiry warning never fires and nothing says why
        ...
        if (token === "" || token.length > 20000 || !/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/.test(token)) {
          ...
          await recordDiagWrite(scheduler, "licence-refusal", () => scheduler.fetch(doURL("/licence-activation-refusal"), { method: "POST", body: JSON.stringify({ reasonCode: "malformed-token" }), headers: { "content-type": "application/json" } })); // a dropped refusal record leaves "it just says community" with no recorded reason
        ...
        if (!verified.valid) {
          ...
          await recordDiagWrite(scheduler, "licence-refusal", () => scheduler.fetch(doURL("/licence-activation-refusal"), { method: "POST", body: JSON.stringify({ reasonCode: verified.reasonCode ?? "body-malformed" }), headers: { "content-type": "application/json" } })); // as above -- the refusal reason is the whole diagnostic value
        ...
        await recordDiagWrite(scheduler, "licence-refusal", () => scheduler.fetch(doURL("/expiry/observe-licence"), { method: "POST", body: JSON.stringify({ notAfter: active.notAfter ?? null }) })); // a dropped observe silently disarms the expiry cockpit for this licence
    ...
    case "POST /update/apply": {
      ...
      const limited = await rateLimited(scheduler, caller);
      if (limited) return limited;
      // See the licence route above. On THIS route the stage matters most: the outer catch asserts
      ...
      try {
        ...
        const token = typeof body.token === "string" ? body.token.trim() : "";
        // PRIMARY guard: refuse before any other work when a prior apply/ramp is still awaiting
        ...
        if (pendingBlock) {
          // noteUpdateGuardRefusal, not recordUpdateRefusal alone. The audit event is stripped of its
          // `detail` by the pack's configEvents excerpt, so an audit-only record would reach support as an
          // unclassed "update-refused / denied" row. The closed counter is what makes the guard legible in
          // the pack.
          await noteUpdateGuardRefusal(scheduler, caller, sourceIp, "pending-open");
          return pendingBlock;
        }
        // Pending is confirmed clear above, so any live split remaining now can only be an
        // already-settled ramp (or an unrecognised one, still refused by verifyAndGuard). Resolve the
        ...
        if (accountId === null) {
          // Record the refusal so a diagnoser sees "the owner tried to
          // update but the engine account was never marked", instead of a bare 400 that leaves no trace.
          // The audit event alone does not do that: update-refused is allowlisted for the configEvents excerpt,
          // but the excerpt strips the target's `detail`, so the class would be lost there and the pack would carry
          // an unclassed denial. The counter carries the class. Best-effort; the guard behaviour is unchanged.
        ...
        stage = "channel-fetch";
        // Collect the channel-metadata DEGRADATIONS the resolver would otherwise drop log-only (a
        ...
        // A resolve/URL fault would 400 BEFORE any audit, leaving the
        // failed update attempt with no trace in the pack. Record it (update-refused) before the 400.
        if ("error" in art) {
          // The CHANNEL leg of the update-fault ring: without it, a customer who "cannot update" because the
          // signed channel will not verify would produce a pack in which the update pipeline was never
          // touched. The three verify causes are three different remediations (a wrong signer is a
          // key-pinning problem, a truncated CDN object is not, a shape drift is OUR bug), and they otherwise
          // all arrive at the operator as one sentence and at the pack as nothing. Component `channel`,
          ...
          // The audit detail carries the closed guard class rather than the resolver's own sentence, which
          // the pack's excerpt drops anyway; the class rides the counter too.
        ...
        // Read the persisted anti-rollback floor + freshness watermark, then REFUSE a replayed/stale
        // descriptor before any download or deploy. The high-water mark is threaded into the apply
        // input and enforced in verifyAndGuard (even with allowDowngrade).
        //
        // The floor read is a SCHEDULER DO READ and is BRACKETED back to do-read for exactly that reason.
        // readUpdateFloor has no catch of its own (deliberately: a DO read failure must become this route's
        // clean fail-closed "nothing was changed" 400), so a scheduler DO outage here THROWS into the outer
        // catch below. Left inside the channel-fetch window, it would record update-apply|channel-fetch --
        // the same row a DNS/TLS fault on the signed channel records -- and the pack's own legend would then
        // send the support engineer to check the vendor's CDN and the customer's egress while the real fault
        // is the customer's own scheduler DO. It is not a narrow window either: every other call in the
        // channel-fetch span is throw-proof (loadVerifiedChannel returns {error}, fetchArtefactBytesDetailed
        // returns a union, destinationConfigured catches both arms), so the DO read would be a large share of
        // the real rows landing under the channel's label if it stayed there. Two different owners: two
        // different stages.
        stage = "do-read";
        ...
        if (!dl.ok) {
          // The audit detail stays the fixed coarse class (unchanged), and the CLOSED MODE + the numeric
          // status ride the update-fault ring: a CDN 404 (the signed channel names an artefact that is not
        ...
        // DESTINATION GATE: a LIVE apply (never dry-run/preview) with NO
        ...
        // OPT-IN DUAL CONTROL (router-executed): going LIVE (shipping arbitrary engine code) takes a
        ...
        // approver even when dual control is ON (the common patch case). A DRY-RUN changes nothing, so
        ...
        // The channel declares a risk class this build does not know (or none at all), and it is coerced.
        ...
        // described by a token nobody recognises. Record it, since neither would otherwise be visible anywhere.
        ...
        // The customer's own "gate every update" policy overrides the vendor's declared risk
        // class. Read only on a LIVE apply: a dry-run is never gated, so a dry-run must not pay a DO round
        // trip to learn a fact it will not use.
        if (!dryRun && updateNeedsDualControl(riskClass, await requireAllUpdateApproval(scheduler))) {
        ...
        if (!dryRun && !validateDeployToken(token)) {
          // THE "UPDATE NOW" BUTTON'S OWN GUARD, and it recorded nothing at all -- not a counter, not even the
        ...
        if (p.outcome === "promoted" && p.fromVersion && p.toVersion) {
          // The promote above JUST redeployed the engine (a live version cutover), the same
          ...
          // The signed channel digest verifyAndGuard just enforced rides the pending record AND the
          ...
          // Persisting the verified engine account here matters: the promote above just proved this accountId
          ...
          if (!pendingWritten || !audited) {
            // The engine is LIVE ON THE NEW VERSION and the record of it did not persist. /update/status
          ...
          // The refusal reason is a 200-char sentence already coarsened by msg(), which cannot say WHICH STEP
          // failed on its own -- and every step has a different remediation (a token scope on read-settings,
          ...
          // The anti-rollback floor is the one update guard that refuses a SIGNED, VERIFIED channel, and
          ...
          // Without a producer here, a channel repeatedly trying to push the engine backwards would look, in
          // the pack, exactly like a channel nobody had asked for an update from.
          //
    ...
    case "POST /update/settle": {
      ...
      const limited = await rateLimited(scheduler, caller);
      if (limited) return limited;
      // The settle runs the CANARY and, on a bad verdict, the AUTO-ROLLBACK -- both real deploys. Its
      ...
      try {
        ...
        // A pending record carrying `percentage` came from the opt-in gradual ramp, not the
        ...
        // The KEEP dual-control gate below applies only to migration/breaking releases. The risk was
        ...
        // HARNESS-ONLY (no-op unless HARNESS_TEST_FAULTS): consume a verdict-affecting settle fault ONCE, so
        // the wraps below force this settle's REAL canary/rollback path down the branch the journey provokes.
        // A drop-source-binding fault is a DIFFERENT kind and is deliberately LEFT armed for the KEPT-settle
        // binding diff below to consume, so one settle can carry both a verdict fault and a binding fault.
        const settleFault = await consumeSettleFault(env, scheduler);
        ...
        // POST-SWAP TOLERANCE: this request runs on the JUST-PROMOTED isolate, where
        ...
        // HARNESS-ONLY: force the decision canary DEAD when a settle fault is armed, so probeSettleVerdict
        // flies a REAL dead verdict (a dead verdict is never retried) and decideKeep rolls back, producing the
        // genuine settle trace the oracle reads. No-op unless a fault fired (settleFault is null).
        const decisionGate = settleFault ? forcedDeadGate(makeHealthGate(env, scheduler, pending.recommendedVersion)) : makeHealthGate(env, scheduler, pending.recommendedVersion);
        ...
        // NOISE. This route does NOT bump update-settle-inconclusive: bumping on any non-alive/non-dead
        // verdict here would fire on the documented POST-SWAP TOLERANCE case, where
        // decideKeep KEEPS, this settle records the outcome "applied" and CLEARS the pending -- a routine,
        // CONCLUDED settle, not the counter's documented meaning (posture-counters.ts) of "the settle could
        // not decide and THE RAMP STAYS ARMED". A counter that fires on the commonest healthy settle in the
        ...
        // flight on a KEPT build is legible as what it is. The counter's one producer is the ramp settle's
        // inconclusive branch (router-updates-ramp.ts), the one path that genuinely leaves the pending armed.
        ...
        // the self-check + the decideKeep branch -- would otherwise live only in the response (probe.attempts),
        // which races the isolate swap and can be lost, leaving a rollback impossible to diagnose after the
        // fact (or from a support pack). It is (a) LOGGED here in real time (visible on `wrangler tail`),
        ...
        // STALE-PENDING GUARD (TTL), HEALTH-AWARE. A pending verification older than the window was
        ...
        if (decision.keep && updateNeedsDualControl(pendingRisk, await requireAllUpdateApproval(scheduler))) {
          // KEEP of a migration/breaking release is gated behind a second owner (a routine release keeps
        ...
        const scriptName = typeof env.WORKER_NAME === "string" && env.WORKER_NAME.trim() !== "" ? env.WORKER_NAME.trim() : "downpipe-engine";
        // HARNESS-ONLY: wrap the deploy driver so the AUTO-ROLLBACK deployVersion THROWS (the compound
        // rollback-deploy-fail leg), so settleAfterPromote returns the honest "rollback-failed" with the engine
        // still on the bad build. No-op unless that specific fault fired; every read method still passes
        // through, so the superseded/split guards below read the real deployment unchanged.
        const driver = settleFault === "rollback-deploy-fail" ? deployFailingDriver(makeCfDeployDriver({ token, accountId, scriptName })) : makeCfDeployDriver({ token, accountId, scriptName });
        ...
        // Persisting the verified engine account here matters too: the read just above (currentLiveVersions
        ...
        // (router-updates.ts:533). A settle is a SEPARATE request from apply: on an engine still running the
        // pre-upgrade code, the apply leg's own call does not exist there and cannot fire, so this settle
        // request, arriving after the swap, is the first request guaranteed to run on the new code, and
        // must not rely on the apply leg alone. Best-effort and
        ...
        // PERSISTENCE-FIRST HONESTY: the decision is already final here (the probe
        ...
        // so a status re-read and the cron's own read can see it, rather than leaving it response-only.
        // Only-when-true keeps the record clean; every rollback and every canary-confirmed keep omits it.
        ...
        // A "rolled-back" outcome above just redeployed fromVersion (settleAfterPromote's own
        ...
        // harmless immediate success when nothing actually reset). This is unconditional rather than only
        // correcting the record when the rollback deploy step itself fails: a successful settle is ALSO
        // recorded via the race-safe path, not just a plain write that could be lost to the very self-redeploy
        // it is racing, and it also carries the audit trail, not only the bookkeeping record. Both writes
        // carry the full settle DECISION TRACE (selfCheckOk, settleTrace), and the audit detail carries the WHOLE decision (verdict +
        ...
        if (!settledWritten || !audited) {
          // The SETTLE half. A settle that did not persist leaves the console unable to offer a rollback
        ...
        if (s.outcome === "applied" && Array.isArray(pending.sourceBindingsBefore) && pending.sourceBindingsBefore.length > 0) {
          ...
          // HARNESS-ONLY (no-op unless HARNESS_TEST_FAULTS): consume a drop-source-binding fault and REMOVE the
          // named binding from the post-update set, so the REAL diff below reports it in droppedSources exactly
          // as a genuinely dropped binding would (the owner's #1 fear, provoked on a harness estate only).
          const droppedByFault = await consumeDropSourceBindingFault(env, scheduler);
          if (droppedByFault !== null) postNames.delete(droppedByFault);
          const droppedSources = pending.sourceBindingsBefore.filter((n) => !postNames.has(n));
          if (droppedSources.length > 0) {
            // THE OWNER'S #1 FEAR. The diff computes the EXACT names; recording only a COUNT on the audit row
            // while the names ride the HTTP response and are gone would let support PROVE bindings were
            // dropped without being able to say WHICH to re-attach. Binding names are the operator's own
            // labels, the identical redaction class the pack already ships in sourcesDetached
        ...
        if (queuedConsole !== null && typeof queuedConsole === "object" && typeof queuedConsole.version === "string") {
          ...
          if (s.outcome === "applied") {
            ...
            if ("error" in artNow) {
              // The SETTLE-TIME channel re-verify. It fails with a half-applied release (the engine is on
              // the new version, the console is not), which is the update state a customer is most likely to
              // ring about, so the channel's cause is recorded here too: same row, same closed cause.
    ...
    case "GET /update/status": {
      ...
      // destinationConfigured (ADDITIVE): the SAME authoritative read the live-apply gate
      // uses, so the console can replace the apply control with the "add a destination first" line (and
      // the eventual confirmation can arrive) WITHOUT first attempting an apply. UpdateRecord otherwise
      // carries no destination field at all, so it is added here rather than duplicated onto a second endpoint.
    ...
    // ---- the STANDALONE one-click rollback (revert to the recorded known-good version) --------------
    ...
    // takes), so by design it needs NO second-owner approval (dual control gates only the consequential
    // apply/keep direction). Owner-gated (keys.ceremony) + rate-limited; the one-shot deploy token is required + never
    ...
    // ---- the OPT-IN gradual ramp (start + settle) live in router-updates-ramp.ts
