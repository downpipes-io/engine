...
// separation: the engine emails ONLY shares of the 256-bit wrapping key; the ciphertext (the
...
// it.
//
// THE HTML PART IS RENDERED BY THE ENGINE'S OWN THEME (email-theme.ts) AND NO LONGER BY A LOCAL COPY, for
// three reasons, each of which was measured on the rendered artefact rather than read off the source.
//
// 1. IT CARRIED A VENDOR SIGN-OFF ON MAIL THE VENDOR DOES NOT SEND. This message goes out from the
//    CUSTOMER'S engine, over the CUSTOMER'S own sending domain, to a custodian the customer chose, and it
//    was signed "Tim O'Connor / downpipes (Maelstrom AI Pty Ltd)". email-theme.ts states the rule for
//    exactly this class of mail in its own header ("NO VENDOR IDENTITY ... must never carry the licensing
//    pack's sign-off"), and it matters more here than anywhere else in the product: the whole custody
//    design turns on Maelstrom receiving nothing, and a recovery-share email signed by the vendor invites
//    a custodian to read vendor involvement into a key ceremony the vendor has no part in. The footer is
...
// 3. IT WAS THE ONLY DOWNPIPES EMAIL THAT DID NOT FIT A PHONE. The card was a fixed 480px table inside
//    20px of body padding, with no narrow-screen rule, so at a 360px viewport the document measured 500px
//    wide and scrolled sideways; every other email in the product measured within its viewport at the same
//    width. It also had no dark-scheme rules, no charset, no lang and no color-scheme declaration, so it
//    rendered as a light card in a dark client while its siblings adapted.
...
export async function handleCustody(ctx: RouterCtx): Promise<Response | null> {
  ...
  switch (`${req.method} ${sub}`) {
    ...
    // touches this route, so captured shares stay useless. Body-only, no attachment.
