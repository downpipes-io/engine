// The capability + composable-custom-role layer of the engine governance backbone (D3 RBAC), split out of
...
// surface and layers Caller/callerCan/WhoAmI on top).
//
...
function validateCustomRoleCapabilities(
  ...
  for (const c of proposal.capabilities) {
    ...
    // `escalation` tags the two refusals that are security events rather than validation ones: an attempted
    // privilege escalation through a custom role -- an access-admin composing a role that grants a
    // capability they do not themselves hold -- needs to be distinguishable from an ordinary validation
    // failure, so a caller can tell "someone attempted to escalate" apart from any other rejected proposal.
...
// owner cap.
// capabilitiesOfCustomRole re-applies the owner-reserved bar at READ time, so a stored custom role that
// somehow HOLDS an owner-reserved capability can never confer it. The clamp is right, but a stored role can
// only hold an owner-reserved capability if something wrote one there, and the write path refuses to -- so a
// drop here means the record was tampered with, and dropping it silently would destroy the only evidence of
// that. This counts the drop (never the capability, the role name or the label).
