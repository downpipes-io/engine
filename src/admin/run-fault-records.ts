// RUN-FAULT RECORDS (the SOURCE + DESTINATION evidence sink).
//
// WHY THIS MODULE EXISTS. Two per-run fault ledgers record evidence at the exact fault sites:
...
// Neither ledger is useful on its own: draining it into memory without posting it on is the same as not
// recording it. Evidence that does not reach the bundle does not exist: a fault path is not diagnosable until
// a remote diagnosis can read its evidence from the support pack.
//
...
//
// THE PIN IS REAL: the type annotations below pin the two together, so a member added to dest/classify.ts's
// union and forgotten here fails the typecheck. The gate below DROPS a fault record whose arm or class is not
// in these sets (sanitiseDestFaults `continue`s the whole record), so a forgotten member means a destination
// fault silently absent from the support pack, the same shape of defect as a recorder writing into a void.
//
...
// D1_TABLE_HANDLE_SHAPE is the STRICTER gate for the D1 defect locus. The generic shape above admits a
...
/** The run-fatal source fault: the transport verdict + the crawl stage it died at. */
...
export interface SourceFaultsEntry {
  ...
  // cfSelectorMode: WHICH cf-config surface selector the run actually used. Only the stale fail-safe is
  ...
  // d1Defects / d1DefectTables: the closed D1 decode-defect class -> count, plus the one-way HANDLES of
...
/** One destination fault SHAPE: the op, the coarse status, the allow-listed S3 code, the arm. */
...
/** One downpipe's most recent destination fault + degradation snapshot. */
export interface DestFaultsEntry {
  at: number;
  total?: number;
  overflow?: number;
  faults?: DestFaultRow[];
  /** {closed anomaly -> count}. A non-conformant store, a silently-truncated listing,
   * an engine guard refusal. Counters only. */
  anomalies?: Record<string, number>;
  /** The region token of a redirect target (never the Location URL). */
  redirectRegionHint?: string;
  /** The clamped size that tripped an engine ceiling. */
  overBoundBytes?: number;
  /** Pages the listing walk reached before the store truncated it without a cursor. */
  listPagesReached?: number;
  /** The multipart aborts that THEMSELVES failed, stranding part storage the customer is billed for --
...
export function sanitiseSourceFaults(raw: unknown, now: number): SourceFaultsEntry | null {
  ...
  // incompleteReasons: {closed marker kind -> {closed reason -> count}}.
  ...
  // shapeAnomalies: the tolerant-parse drops -- the ONLY trace that a silent Cloudflare API shape
  ...
  // truncation: the SHORTFALL MAGNITUDE. "Is our monorepo fully backed up?" is answerable only with
  ...
  // fatal: the run-fatal throw's transport verdict + crawl stage. This is the "my Images backup fails
  ...
  // snapshotConsistency: a DATA-INTEGRITY verdict that rides on runs which report OK. "pinned" is the
  ...
  // resumeTokenDefects: the closed defect class + a COUNT, which is what separates a one-off from a
  ...
  // d1Defects: the closed defect class + a COUNT. The three classes route to COMPLETELY different
  ...
  // cfSelectorMode: like snapshotConsistency, the HEALTHY values are deliberately NOT recorded
  ...
  // throttle pressure: the absorbed 429s are the EARLY WARNING -- a chronically throttled account
  ...
  // securityRefusals: the KIND is the whole record. The refused host / URL / DDL text is exactly the
...
export function sanitiseDestFaults(faults: unknown, io: unknown, now: number): DestFaultsEntry | null {
  ...
  // The same posted object, read for the ANOMALY half of the snapshot. Typed separately
  ...
  // anomalies: the destination behaviours that are NOT a failing op with a status -- a
  ...
  // redirectRegionHint: the ONE field derived from a store-controlled header. It is re-gated to the bare
  ...
  // strandedAborts: re-gated DO-side against the closed ABORT_FAILURE_CLASSES with a clamped count, so
  ...
  // io: the DEGRADATION counters of a destination that is making a GREEN run slow and expensive --
