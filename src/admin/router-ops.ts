// router-ops.ts -- the operations-and-alerting routes (the dispatch-split, MOVE-ONLY): the drill-evidence
// log, the notification channels/rules/history/test, the credential-expiry tracker,
// the Access-only posture surface and the in-app break-glass-token retire. Each case body was
// MOVED VERBATIM from handleAdmin's switch in router.ts; the only change is reading the shared locals off
// the RouterCtx the hub builds once, so the notify.config / expiry.config / access.policy / keys.ceremony
// gate on each route runs here exactly as it did inline. Imports shared primitives + the notify channel
// delivery; never imports router.ts.
...
// DO (a whitespace-only apiKey forwarded untrimmed would otherwise be stored as UNWRAPPED plaintext even
// with CONFIG_WRAP_KEY set; stripping it here agrees with parseChannelSecret's trimmed-empty rule at the
// DO). A NON-string apiKey is left in place so the DO's parseChannelSecret still rejects it
...
export async function handleOps(ctx: RouterCtx): Promise<Response | null> {
  ...
  switch (`${req.method} ${sub}`) {
    // ---- drill-evidence (build-contract 1.4.7): evidence log for drills + rehearsals --------
    ...
    // This is the route that names the rule the other eight follow.
    //
    ...
    case "POST /notify/test": {
      ...
      // ALSO recorded on the history ring flagged test:true, so a green test leaves a durable trace that
      // the channel was verified; rules and digests never read history, so the test entry can never
      // trigger routing.
      const denied = gate(caller, "notify.config");
      if (denied) return denied;
      ...
      // ASVS V13.2.4: a test-send goes through the SAME allowlist screen a real delivery does (the account's
      // current list, read fresh so a test genuinely proves what a live alert would do).
      const eaResp = await scheduler.fetch(doURL("/config/egress-allowlist"), { method: "GET" });
      const { egressAllowlist } = (await eaResp.json()) as { egressAllowlist?: string[] };
      ...
      const result = await deliverToChannel(env, channel, emission, egressAllowlist);
      // "My Slack test keeps failing." The delivery FAILURE CLASS the channel adapter computed (a 401 on
      ...
      //
      // THE CODE RIDES AS ITSELF, NOT AS `reason`. ChannelDeliveryResult.code is already a member of the
      ...
      // webhook URL, the recipient or the provider's text (the standard placeholder discipline).
      fireInBackground(
        runtime,
      ...
      // recorded them on a failed delivery; the test row, the ONE durable artefact of a Test
    ...
    // discards (the arm is listExpiryStatuses(), no caller argument). That is the /history sentence from
    // 1d857310 a second time, one layer down. posture.read for the reason on GET /notify/rules and on the
    // same kind of evidence: support-sections-config.ts fetches doURL("/expiry") twice into the
    // posture.read-gated pack, once for the licence expiry status and once for the whole registry. It is a
    // viewer-floor capability, so "any authenticated role" stays exactly true after this and is now the
    // thing the code says rather than the thing two comments claimed.
    case "GET /expiry": {
      const denied = gate(caller, "posture.read");
      if (denied) return denied;
    ...
    // ---- access policy: surface the Access-only (token-fallback) posture ---
    ...
    case "POST /policy/require-access": {
      ...
      // Lockout pre-flight. Disabling the token (ADMIN_TOKEN_DISABLED) or deleting the secret is an
      ...
      try {
        ...
        // Failing safe is right; failing SILENTLY is not. An operator who holds a passkey is told they
      ...
      return jsonResponse({
        ...
        // The second-factor breakdown + the pre-flight verdict. secondFactor is the four named factors
        ...
        secondFactor: {
          ...
          // enum; never an email, a count or a code). recoveryReady reflects the underlying conditions rather
          // than latching once true, so an operator who is told a factor they believe they have is not in
          // place needs to know why: "every code has been used" and "your codes cannot verify any more" have
          // different remedies.
          recoveryReadyReason: lockout.recoveryReadyReason,
          // passkeyOwnerEvidence is REPORTED, NEVER OR'd. passkeyOwnerEnrolled above still counts credential
          // RECORDS, so a set of orphaned virtual-authenticator keys with no working credential behind them
          // can still keep secondFactorPresent true. The honest fix needs evidence
