...
// happens when a person decides to do it. Without this, the only thing that says when it is due is a
// preference in one browser: console attend.ts stores the cadence and the reminder toggle under localStorage,
// both off by default, so the nudge does not follow the operator to another machine, private browsing drops
// it, and clearing site data resets it. The estate itself knows nothing about its own proof rhythm.
//
// This module is a PURE function with no storage and no alerting, so it can be tested independently of the
// estate-wide setting and of anything that notifies on it.
//
 ...
 * "overdue". That keeps an unset cadence behaviourally identical to having none configured at all.
 */
 ...
 *
 * It reports rather than decides. Nothing here gates work: a posture whose proof obligation blocks work
 * would be a worse default than the one it replaces, so the caller's job is to display the overdue and
 * never-verified sets and, at most, notify on a transition.
 */
