// Posture check builders (kept in their own module for file-size hygiene, apart from posture.ts).
...
//
// DURABLE KEYED CREDIT: the attended-verification proof threads into this check so a break-glass-only
// estate can pass on a REAL keyed verify rather than a scheduled drill it can never pass.
...
// older proof is. (Crediting the durable proof unconditionally -- computing the graded time as
// maxDefined(ordinary recency, durable proof) with no further check -- would sign a FALSE PASS into the
// compliance pack whenever a genuine fresh failure followed an older proof on an ORDINARY operational
// estate, precisely the estates this feature was never meant to touch. gradedOkAt's genuineNotOk gate below
// is what makes the credit safe.) It also credits ONLY the full keyed method, never a blind-test proof
// (which carries no sample rate or scope, so a one-record test and a full-run test are indistinguishable
// once stamped) and never a keyless-attest proof (no decryption, so it does not satisfy a decrypt-verify
// check).
...
// seal-verification (critical, recoverability): verify-at-seal (ENG-RST-01) reads each run's archive
...
// buildImmutability lives in its own module: it is the one check with a dependency none of its neighbours
// have (dest/worm-remedy.ts, the per-provider WORM vocabulary), and that vocabulary would otherwise push
// this file over the 1000-line budget. Re-exported here so every importer and every citation pinned at
...
// off-account-audit-mirror (medium, ISO 27001 A.8.15 / A.8.16): is the engine's audit trail mirrored to a
// holder the account cannot edit?
//
// WHY IT IS GRADED AT ALL. The engine's architecture disclosure names ONE mitigation for THREE separately
// listed residuals: the SchedulerDO's privilege concentration, the tamper-evident audit log's in-account
// integrity, and signer-key forgery on a full account compromise. Each answers "an optional,
// customer-configured off-account mirror". The mirror ships as a first-party control (the outbound SIEM
// audit-log push, src/admin/router-push.ts), and nothing graded whether it was on: an estate with no mirror
// scored identically to one pushing every audit row to an external SIEM, so the mitigation existed only for
// the customer who happened to find it.
//
// WHY MEDIUM. It is a compensating control for residuals, not a primary control, and an estate whose threat
// model does not include its own account holder is entitled to decline it. Registered in CHECK_SEVERITY
// (posture.ts) beside dest-cred-encryption, so an Owner can risk-accept it rather than carry a permanent red.
//
// FOUR READINGS, and the last two are the ones a boolean "is it configured" would have got wrong:
//   - no destination at all                 FAIL: nothing leaves the account
//   - configured but NOT draining           FAIL: a destination that pushes nothing mirrors nothing
//   - configured, draining, last push FAILED FAIL: the mirror is stale, which is the state an operator most
//                                            needs told, because the config screen still says "configured"
//   - configured, draining, nothing sent yet PASS, with the strong claim withdrawn: the drain runs on the
//                                            cron tick, so a mirror set up a minute ago has no delivery on
//                                            record and must not be failed for it, nor claimed to be receiving
// Redaction-safe: the input is two booleans, an outcome and a date. The endpoint, the vendor and the auth
// secret never enter this check.
export function buildAuditMirror(input: PostureInput): CheckDraft | null {
  const m = input.auditMirror;
  if (m === undefined) return null;
  const delivered = m.lastAttemptOk === true;
  const failedLast = m.lastAttemptOk === false;
  const failed = !m.configured || !m.enabled || failedLast;
  const compensates =
    "An in-account storage holder can re-chain the audit log; an external holder of the pushed seq/prevHash/hash detects that, and it is the same mirror that compensates for the scheduler's privilege concentration and for signer-key forgery on a full account compromise.";
  return {
    id: "off-account-audit-mirror",
    title: "Off-account audit mirror",
    control: "ISO 27001 A.8.15",
    auto: failed ? "fail" : "pass",
    how: "Reads whether an outbound audit-log push destination is configured and draining, and the outcome of its most recent delivery attempt. It reads no endpoint and no credential.",
    detail: !m.configured
      ? `No off-account audit mirror is configured, so every copy of the audit trail is inside this Cloudflare account. ${compensates}`
      : !m.enabled
        ? `An audit-log push destination is configured but draining is off, so no audit row leaves the account. ${compensates}`
        : failedLast
          ? `The audit-log push destination has not delivered: its most recent attempt failed${m.lastAttemptAt ? ` (${m.lastAttemptAt.slice(0, 10)})` : ""}, so the off-account copy is behind the in-account trail.`
          : delivered
            ? `Audit rows are pushed to an off-account destination and the most recent delivery succeeded${m.lastAttemptAt ? ` (${m.lastAttemptAt.slice(0, 10)})` : ""}.`
            : "An audit-log push destination is configured and draining, but no delivery has been recorded yet, so nothing has been observed reaching the mirror.",
    remediation: !m.configured
      ? "Configure an audit-log push destination on the console's Settings screen (any SIEM that accepts an HTTPS feed, an S3 drop or syslog over TLS), so a holder outside this account keeps the audit chain."
      : !m.enabled
        ? "Turn draining on for the configured audit-log push destination on the Settings screen."
        : failedLast
          ? "Check the push destination's delivery trail on the Settings screen and fix the endpoint, credential or network path it names."
          : "No action required; the audit trail is mirrored off-account.",
  };
}
...
// so attended verification is what replaces unattended proof, and without it the only thing that knows
// when it is due is a preference in one browser.
//
...
// of the backup, and nothing here gates any work.
...
// update-apply-provenance (low, SLSA v1 / NIST SSDF PS.3, DP-0/DP-D): whether the most recent APPLIED
...
export function buildUpdateVersionDrift(input: PostureInput): CheckDraft | null {
  ...
  // The acceptable set: the settled expectation, plus every id the update
