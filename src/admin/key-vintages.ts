// key-vintages.ts -- the KEYLESS, MANIFEST-AUTHORITATIVE key-vintage inventory (product fix G-P0-098, and the
// surfacing half of G-P0-099). It answers "which archive vintages does each key open, and which runs are
...
// value stamped into every root the current signer signs. Absent/malformed is null, never a throw. Exported
// for the secret-rotation cadence seed (cron/alert-passes.ts runExpiryPass), which reports it to the
// scheduler DO the same way this module reports it for the key-vintage inventory: the DO holds no env, so
// only the Worker side can derive it.
export async function envSignerFingerprint(b64: string | undefined): Promise<string | null> {
  if (typeof b64 !== "string" || b64.trim() === "") return null;
  try {
