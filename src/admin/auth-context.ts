 ...
 *
 * The amr list cap is a drop that must be tallied on its own: capping at AMR_MAX_COUNT still returns a
 * DEFINED `amr` (the first 16 entries), so a drop test of `amr === undefined && input.amr !== undefined`
 * alone can never fire for it. An amr of 20 entries with "mfa" at index 18 would then be silently truncated,
 * producing the same row as a fully usable amr and leaving no record that an entry was lost, on the exact
 * question the counter exists to answer: "prove this session was MFA'd". claim-drop-oidc-token-amr covers an
 * entry that is "over-long, OR PAST THE LIST CAP", so this out-parameter is what lets buildAuthContext book
 * the list-cap case as well as the per-entry one.
 *
...
//
// `drops` is the bounded out-parameter naming the advisory claims that are ASSERTED and DROPPED. The
...
// constructor is shared by the OIDC and SAML paths, so hard-coding a single boundary here would file a SAML
// drop under a boundary the assertion never crossed -- a row asserting a fact the code never established. The
// SAML caller passes "saml"; the OIDC caller passes "oidc-token".
