...
// The phase-refusal vocabulary lives in discovery-health.ts, the leaf that owns every attach fault class.
...
function validateAdditions(byName: Map<string, LiveBinding>, additions: AttachSource[], removeSet: Set<string>): { fresh: LiveBinding[]; added: string[] } {
  ...
  for (const s of additions) {
    ...
    // name (or detach the existing one first), which is nothing like a token problem. Message unchanged.
...
export function planChange(
  ...
  // refused rather than risk rewriting a stranger's worker. That is a completely different concern from "my
  // token doesn't work", so this is tagged and reported distinctly from a plain token failure.
...
//
// verifyAfter's alarms are the owner's #1 fear caught in the act: a settings PATCH that did NOT preserve a
...
export function asAttachRefusal(cls: AttachRefusalClass, e: unknown): unknown {
  ...
  // with it, which outranks every phase class there is: burying the owner's #1 fear actually happening under a
