// Support-pack section gatherers: the fault-ledger sections -- ten DO records that recorders
// write at the fault site, and which each of these sections reads and delivers into the bundle. A fault
// belongs in the BUNDLE, so each of these is the delivery half of a recorder that already fires on the fault path:
//
//   - unwrapFaults    WHEN the runtime CONFIG_WRAP_KEY unwrap started failing, and which of the four
...
//   - bindingAlarms   the post-write binding-safety alarms (the owner's #1 fear: a deploy that drops a
//                             source binding), with the operator's own binding labels.
//   - updateFaults    the self-update pipeline's failing STEP + closed cause, the
...
//   - adminRefusals   the Worker-EDGE refusal trail (surface x reason). gate-unavailable is the one to
...
//   - integrityFaults the seal/verify/restore core's fault LOCUS --
...
//   - dispatchFaults  the Worker's four last-resort catches, including the SELF-REFERENTIAL one that
//                             covers /support/diagnostics itself.
//   - cronHealth      every cron pass's outcome by closed name + error class,
...
//   - destProbeFaults per destination, the closed reason the write-probe refused it -- so "all
//                             destinations unreachable" finally names a cause per destination.
//   - destBuildHealth failingSinceAt -- "every backup has been failing to even BUILD its
//                             destination since Tuesday, because DEST_ENDPOINT is gone".
//   - costSizing      the sizing probe's outcome by source type, including measured-zero (a 0 presented
...
import {
  ...
  // The format-layer vocabularies, re-declared DO-side in diag-records.ts and pinned member-for-member
...
// The closed sets below, each re-gated at the PACK boundary as well as at the DO applier (defence in depth).
...
// #1 fear, named rather than left as a bare count on an HTTP response nobody kept), and `readbackShape` (a bounded descriptor
...
// ---- adminRefusals (the Worker EDGE half) ---------------------------------------------------------
// A durable refusal trail for the admin writes, rather than an HTTP response only the customer holds,
// and which they may not have kept. The key is "<surface>:<reason>" over TWO closed sets, so the key space is bounded by
// their product. `restore-apply:gate-unavailable` is the DR-hour headline: a restore refused "not approved"
// BECAUSE the approval gate could not be READ is an ENGINE fault, distinct from a genuinely missing approval.
// Disjoint from the DO-side schedAdminRefusals (which records what the DO
...
export async function fetchIntegrityFaults(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  ...
  for (const [id, v] of Object.entries(typeof j === "object" && j !== null ? j : {}).slice(0, INTEGRITY_FAULTS_DOWNPIPE_CAP)) {
    ...
    // runlogAnomalies: the ANTI-ROLLBACK / RUNLOG chain forensics. The detector's own `reason` string
    ...
    // writerRefusals: the writer's three LOUD refusals, distinct from a generic run failure.
    ...
    // attestCoverage: what the attestation's complete:true actually CLAIMS. sampled-no-presence is the
...
// ---- dispatchFaults -------------------------------------------------------------------------------
// The Worker's four last-resort catches, each recorded rather than only producing a 500. The non-admin
...
// ---- cronHealth ------------------------------------------------
// The cron plane's own health, rather than one anonymous scheduler.ticks.passErrors
...
export async function fetchCronHealth(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  ...
  //
  // notify: the CRITICAL alerts that are CLAIMED and never delivered. `claimSpent` is the one to
  ...
  // updates: the update channel's own VERIFICATION verdict, checked on every tick the channel is
  ...
  // cpExport: the control-plane export pass. `truncated` + destsSkippedByBudget matter because the
  ...
  // push: the delivery-TRAIL writes that are themselves dropped. This is the duplicate-events ticket:
  ...
  // restoreTest: the drills that did NOT RUN. The pre-drill throw lands BEFORE all the recording
  // machinery, so without this the "we have not been tested for months" ticket would carry nothing at all. Keyed by the
  ...
  // reconcile: the flag ECHO first -- an empty reconcile section would otherwise be ambiguous, because "never turned
  // on" and "on and bailing for weeks" would read identically. The RUNLOG-signature verdict is SPLIT because
...
// destination since Tuesday, because DEST_ENDPOINT is gone" (the owner's #1 fear, fully instrumented).
...
// ---- adminRouteErrors ------------------------------------------------------------------------------
// The update / rollback / ramp / licence routes' outermost catches record the exception rather than only
// returning a FIXED sentence -- "nothing was changed" -- so "update apply always fails with a generic message"
// arrives with a server-side trace: the locus, the cause, and the fact that it happened.
//
...
//                 Different owners, distinguished rather than folded into one generic message.
//
//                 The do-read / channel-fetch split is load-bearing: the anti-rollback floor read (a DO read,
//                 uncaught by design so it fails the route closed) must sit OUTSIDE the channel-fetch window,
//                 because a scheduler DO outage recorded as "channel-fetch" would send the support engineer to
//                 check a CDN that was never involved. The floor read is bracketed to do-read in both the apply and the ramp.
