...
import { approvalDwellRefusal, type GovernanceDecision } from "./approvals.ts";
...
// direct escape. owner-action.ts states the same asymmetry.
...
//
// THREE KINDS HAVE NO DO-METHOD RE-CHECK OF THEIR OWN: addDownpipe re-checks only the narrower
// scheduledtest.config; removeDownpipe and setCfConfigMode are dispatched with no caller at all. The map is
// consumed at approve by BOTH halves of the maker-checker pair -- canApproveChange for the CHECKER, and
// approveChange's maker floor for the PROPOSER's live re-resolved authority -- so it is the map, not the
// delegated method, that carries those three at apply time.
...
// subject (iss+"|"+sub for Access, passkeySubject(email) for passkey) against the
...
export interface PendingConfigChange {
  ...
  // (their email, their subject, their propose-time groups) and appends an ordinary human-attributed audit
  // entry; without an IP carried on the record, that replay-time entry would have no source address to log,
  // even though the engine captured one at propose time. It is NOT bound into contentHash: the apply
  // re-resolves no authority from it, so it is provenance, not a decision input. It never reaches the support
  // pack (the audit excerpt strips sourceIp from every event by design); it rides only in the customer's own
  // audit chain, exactly as a direct, ungated mutation's does.
  ...
  supersededAt?: string;
  // lastAttempt records the record's last failed apply attempt, so a change that will not apply is not left
  // sitting in the inbox marked "pending" with no record of what was tried or why it did not go through.
  // outcomeClass is a CLOSED enum (sched-fault-ledger GOVERNANCE_OUTCOME_CLASSES) that
...
//   - maker != checker on the STABLE SUBJECT axis (the authority axis), the SAME axis restore dual control
//     compares (approvals.ts: approverSubject !== requesterSubject): the approver's subject must differ from
//     proposedBySubject. The display-email comparison is kept as a belt-and-suspenders floor (a
//     canonicalised-email self-approval is also refused), so a pre-subject legacy record with a null
//     proposedBySubject still cannot be self-approved by email;
//   - the approver must hold the same write capability the original mutation requires;
//   - approval-dwell: the approve must arrive at least APPROVAL_MIN_DWELL_MS after the
//     change was proposed, or no checker could have reviewed it. `now` is the DO's own clock, passed in
//     (never re-read here) so this stays pure and the caller can pin it in a proof.
...
// synchronous identity/state/capability/dwell gate; both run inside the DO's read-modify-write so the
...
export function canApproveChange(
  ...
  // Only reached once every other refusal above has cleared, so a change that is terminal/unauthorised is
  // classified as THAT, never as too-soon. proposedAt is the record's own raise instant.
  const dwell = approvalDwellRefusal(record.proposedAt, now);
...
//
// WHY THIS REDACTION EXISTS, and it is a CAPABILITY BOUNDARY, not merely tidiness. A NotifyChannel carries
...
// instead, because the pending-change inbox is a governance queue that any config reader may watch. Without
// this redaction, a queued notify-channel-set would hand that same credential to every downpipe.read holder:
// per ROLE_CAPABILITIES, viewer, restore-operator and access-admin hold downpipe.read and hold NO
// notify.config, so those three roles would see on the inbox route the exact credential the channels route
...
// PendingChangeStatus) and nothing anywhere deletes a record, so without scrubbing, a rejected or
// superseded notify-channel-set would keep the submitted credential indefinitely, an exposure that grows
// with time. The scrub runs in the SAME storage.put as each terminal status, so there is no window, and it
// reuses the projection's own field list so there is ONE definition of what counts as a secret here.
//
...
// echoes are covered together because the redaction rule applies to every caller-facing echo, not only the
// listing: an approver arming a change and an approver vetoing one are both reading, and neither needs the
// credential to decide.
