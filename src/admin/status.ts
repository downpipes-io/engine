...
export interface StatusReport {
  ...
  // inviteSenderConfigured: can this engine EMAIL a new member their set-up link at all? "The new
  // members never received a set-up link" has two completely different answers, and the pack cannot tell
  // them apart without this: a deployment that deliberately configures no invite sender (the Owner hands the
  // link over by hand, which is a legitimate posture) and one whose sender is MISCONFIGURED (the send is
  // attempted and refused). This boolean is the first half of telling them apart; the role-grant-invite-
  // undeliverable counter and the invite-email auth signals are the second. Presence only: the sender
  // address never rides.
  ...
  // statusSource tells a bundle consumer whether this report's DESTINATION facts reflect the LIVE
  ...
  // destAmbiguous names the ONE unconfigured-looking state that is not unconfigured at all. The customer
  ...
  ready: boolean;
  // tokenFallbackDisabled reflects ADMIN_TOKEN_DISABLED: true when the shared-token fallback is
  ...
  // auditNearCap is true when the retained audit-log count is at or above
  ...
  // Release provenance for the console's provenance card: a public artefact digest and a
  ...
  //
  // artefactSha384 is the engine's SELF-REPORTED build-stamped digest of its own deployable
  ...
  // cfVersionId / cfVersionTag are the deployed Cloudflare Worker version's immutable identity
  ...
  // cfAccountId is the engine's own Cloudflare account id, so the console can send it with a self-serve
  // licence claim (POST /control-plane/licence/claim) and the control plane can bind the licence to it.
  // This is the CUSTOMER'S OWN
  // account id, not a vendor secret and not backup data, so surfacing it here (unlike everything else this
  // route deliberately withholds, see the module header) costs nothing: the browser reading this
  // response is already inside that same Cloudflare account's console.
  //
  ...
  // writing to it, so a success already proves the account). No customer deployment writes CF_ACCOUNT_ID
  // directly (not wrangler.toml, not the deploy script), so a self-serve customer depends on this
  // fallback: without it, status would omit the field on every read and the console would never send an
  // account id with any claim.
  //
...
// alone. A single options object keeps the parameter list within GUARDRAILS §6 (max-4-params) rather
...
export function resolveDestKind(env: Env, consoleDestSet: boolean | undefined, consoleDestHost: string | undefined): { destKind: DestProvider | null; destConfigured: boolean; destAmbiguous: boolean } {
  ...
  // The factory rejects the ambiguous (r2 && s3) case, so status reports not-ready, not a guess, and says
  // so explicitly. destConfigured stays false (the engine genuinely cannot pick a destination and no
  // backup can run), so every existing consumer is byte-identical; destAmbiguous carries the CAUSE, which
  // turns "configure a destination" into "set DEST_KIND".
...
// the count arrives strictly after buildStatus has run. Before this, the router discarded that response and
// the count was never supplied by anyone: auditNearCap was absent from every status body this engine has
// ever served, and the console rendered "this engine build does not report audit-log capacity" for ever,
// which is the export-before-rollover warning never firing on any estate.
//
...
export function buildStatus(env: Env, downpipeCount: number, opts: BuildStatusOptions = {}): StatusReport {
  ...
  return {
    ...
    destConfigured,
    destKind,
    // Whether the destination facts came from a LIVE DO read (the caller supplied consoleDestSet)
    ...
    // Reflect the token-fallback hardening so the console can show "Access only".
