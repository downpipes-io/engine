...
// callback) and the SAML 2.0 SP flow (metadata / start / the cross-origin ACS POST), plus their __Host- txn
// cookies, the relative-only returnTo guard and the generic no-oracle failure redirect. These run BEFORE the
// main authorise() gate. This spoke imports only the shared primitives (router-core, for the same per-IP
// authRateLimited pre-throttle /admin/auth/* uses) and never router.ts, so it stays acyclic.
...
import { sessionSetCookie , readSessionCookie } from "./session.ts";
...
// fireInBackground: see router-identity.ts's identical helper. This whole file is pre-auth (like
// router-auth-flow.ts's handlePasskey), so runtime is threaded as a plain parameter down the call chain
// rather than read off a RouterCtx, which does not exist yet at this point.
...
// recordSsoSubCause counts the SUB-CAUSE of a failed interactive sign-in in the bounded auth-signal
...
// to this edge. The sub-cause is classified HERE, at the last point that sees the reason: classifySsoSubCode
// reads it TRANSIENTLY, selects one member of the closed
...
export async function handleSaml(req: Request, env: Env, sub: string, runtime?: AdminRuntime): Promise<Response> {
  ...
  // PER-IP pre-throttle (the same authRateLimited ceremony limiter as /admin/auth/* uses): this whole
...
async function handleSamlMetadata(scheduler: DurableObjectStub, consoleOrigin: string, connId: string, runtime?: AdminRuntime): Promise<Response> {
  ...
  try {
    ...
    if (out.ok !== true || typeof out.metadata !== "string") {
      // Without this signal, "our IdP cannot import your SP metadata URL" leaves no server-side evidence at all.
...
async function handleSamlAcs(req: Request, env: Env, scheduler: DurableObjectStub, consoleOrigin: string, connId: string, runtime?: AdminRuntime): Promise<Response> {
  ...
  if (samlResponse.length === 0 || relayState.length === 0) {
    // The cross-origin form POST arrived without an assertion (or without the server-minted RelayState):
    // a proxy mangled it, or it is not an ACS POST at all. It never becomes a DO request, so this edge
    // signal is the only record of it.
  ...
  try {
    const resp = await scheduler.fetch(doURL("/idp/saml/acs"), { method: "POST", body: JSON.stringify({ connId, samlResponse, relayState, acsUrl, browserBind, ...(sourceIp !== null ? { sourceIp , ...(readSessionCookie(req) ? { priorToken: readSessionCookie(req) } : {}) } : {}) }), headers: { "content-type": "application/json" } });
    ...
    if (out.ok !== true || typeof out.token !== "string") {
      // Count the SAML sub-cause (the IdP's own StatusCode class, an encrypted assertion, an IdP-initiated
    ...
    // Same fire-and-forget unusual-context notify as the OIDC callback (delivery never affects sign-in).
    ...
    // The ACS leg could not reach the DO (or its answer was not JSON). The user sees the same generic
...
async function handleOidcProviders(scheduler: DurableObjectStub, runtime?: AdminRuntime): Promise<Response> {
  try {
    ...
    // An EMPTY provider list is served, so every SSO button vanishes from the sign-in page and
...
async function handleOidcStart(req: Request, scheduler: DurableObjectStub, consoleOrigin: string, connId: string, runtime?: AdminRuntime): Promise<Response> {
  ...
  try {
    ...
    if (out.ok !== true || typeof out.authorizeUrl !== "string" || typeof out.txnId !== "string") {
      // The sign-in died at /start, so it will NEVER reach the DO's callback wrapper - the only
      // thing that writes the SSO-failure aggregate. Classify the DO's reason to a CLOSED name (the reason
      // interpolates discovery errors, connIds and endpoint hosts, so it is read transiently and discarded -
      // only the name is recorded) and count it in the bounded auth-signal aggregate.
...
async function handleOidcCallback(req: Request, env: Env, scheduler: DurableObjectStub, consoleOrigin: string, connId: string, runtime?: AdminRuntime): Promise<Response> {
  ...
  if (idpError !== null || code.length === 0 || state.length === 0 || txnId.length === 0) {
    // The IdP's OWN `?error=` code is the single most diagnostic artefact in the whole flow -- it is the
    // IdP saying WHY it declined (an expired client secret answers invalid_client; a conditional-access policy
    // answers access_denied; an IdP outage answers temporarily_unavailable). It is read here, used to pick a
    // generic redirect, and recorded before being discarded: mapped through a CLOSED registry set,
  ...
  const sourceIp = req.headers.get("CF-Connecting-IP");
  try {
    // configWrapKey rides on this ONE request, and this is the site that decides it should.
    //
    // WHY IT HAS TO. The stored IdP client secret is an at-rest IDP_SECRET_AAD envelope, and the token
    // exchange that needs it opened happens INSIDE the scheduler Durable Object, which holds no env:
    // BaseSchedulerDO's constructor takes only `state`, and widening it changes the one type surface ~30
    // mixins declare `this` against. So the key is forwarded rather than bound. It is request-scoped and
    // stored nowhere: the DO reads it into a local, unwraps once and drops it, and neither side writes a
    // request body to the log.
    //
    // WHAT IT COSTS, stated rather than glossed. The property CONFIG_WRAP_KEY buys is that a read of Durable
    // Object storage alone discloses no credential, and that is untouched: the key is never written to
    // storage. The channel is the Worker-to-DO fetch, which only code already holding env can construct, and
    // which also carries the RAW client secret on the create path. The residual is that a compromise able to
    // observe this internal request sees a key that opens every config secret in the account rather than one
    // connection's client secret. Removing it means widening the DO constructor.
    const resp = await scheduler.fetch(doURL("/idp/oidc/callback"), {
      method: "POST",
      body: JSON.stringify({
        connId, code, state, txnId,
        ...(iss !== null ? { iss, ...(readSessionCookie(req) ? { priorToken: readSessionCookie(req) } : {}) } : {}),
        ...(sourceIp !== null ? { sourceIp } : {}),
        userAgent: req.headers.get("user-agent"),
        ...(typeof env.CONFIG_WRAP_KEY === "string" && env.CONFIG_WRAP_KEY.trim() !== "" ? { configWrapKey: env.CONFIG_WRAP_KEY } : {}),
      }),
      headers: { "content-type": "application/json" },
    });
    const out = (await resp.json()) as { ok?: boolean; token?: string; returnTo?: string; newSignInContext?: boolean; reason?: unknown };
    if (out.ok !== true || typeof out.token !== "string") {
      // Count the OIDC/OAuth2 sub-cause (the provider's own invalid_client / invalid_grant, a kid-not-found
      // after a JWKS rotation, which single-use artefact failed) before discarding the reason.
      recordSsoSubCause(scheduler, out.reason, runtime);
      return oidcFailRedirect(consoleOrigin, true);
    }
    // The DO flagged a materially NEW coarse sign-in context (opt-in). Fire the redaction-safe notify
    ...
    // The callback leg could not reach the DO (or its answer was not JSON): an engine availability
