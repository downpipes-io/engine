...
// two different counters -- collapsing them into "update-refused" is the failure mode this design
// exists to remove.
//
...
// ---- the claim/group BOUNDING DROPS at every parse boundary ----------------------------------------------
//
...
export const CLAIM_DROP_COUNTER_NAMES = [
  ...
  // boundGroupList all use GROUPS_MAX 200 / GROUP_NAME_MAX 256), so decodeCaller has nothing left to drop. A
  // member keyed on a hand-built header no production producer can emit would be coverage-shaped nothing that
  // a support engineer could never see, and the SAML front door tallies its own group drops directly.
  ...
  // an over-long name, a control-char name and every group past GROUPS_MAX -- and completeOauth2Login returns
  // a principal with no claimDrops at all, so the DO's shared recorder never fires on this path. A GitHub org
  // with 260 teams whose mapped team falls past the cap is exactly "the user is in the right group and gets
  // viewer", and without a tally it produces the SAME empty row as a provider that asserted no groups.
  ...
  // the SAML assertion (saml/assertion.ts): the advisory context. Without a tally on buildAuthContext, an
  // over-length AuthnContextClassRef or an unusable AuthnInstant drops in silence and reads exactly like an
  // IdP asserting neither. The auth-time drop is recorded in assertion.ts, at the raw attribute, because
  // parseSamlInstantMs folds an unreadable instant to null and null reaches the shared constructor as
  // `undefined` -- the same input as an IdP that asserted no AuthnInstant at all.
  ...
  // the SAML GROUPS attribute. SAML groups ride VERBATIM out of the assertion (collectIdentity returns every
  // value of the configured groups attribute, unbounded) and are first bounded by the DO's boundGroupList
  // inside the native session mint. An Entra or Okta SAML user in three hundred groups whose ONE mapped group
  // falls past the cap is "the user is in the right AD group and gets viewer", and without a tally naming the
  // KIND and the boundary the pack cannot say which of the three bounds threw it away. Produced at the session
  // mint, where the SAML principal's groups are actually bounded.
...
//
// The boundary is a fact about WHERE the drop happened, so it must be passed in rather than assumed:
// buildAuthContext is shared by the OIDC and SAML paths, so hard-coding "oidc-token" would file a SAML drop
...
// ---- the FIRE-AND-FORGET security alert that can silently never fire ---------------------------------------
//
...
// THE CLASS IS THE ALERT'S MEANING, NOT THE NOTIFY EVENT. Deriving the class from the coarse notify EVENT
// would coalesce it, because three of the events are shared by unrelated alerts: "an attacker added an owner"
// (role-change), "recovery codes were regenerated" (auth-credential-change) and "the backup destination was
// repointed" (dest-change) would all compose the SAME name under bumpAdminCounter's closed-name sum, so a
// support engineer holding the pack could not tell which alert had never reached a human -- the whole question
// this counter exists to answer.
//
// Each class below is passed DOWN from the call site that knows what it is alerting about, so the name says
// what happened. The events stay separate (the customer's notify rules still key on them); only the evidence
// improves.
export const ALERT_EMISSION_COUNTER_NAMES = [
  ...
  // credential-change: an admin credential was revoked (a passkey removed) or added (a passkey self-add)
  ...
  // in. These stay two separate classes rather than one "recovery-abuse", because they are opposite facts --
  // someone is IN versus someone is being KEPT OUT -- the customer's notify rules key on the two different
  // EVENTS, so the no-channel remedy differs, and folding them into one name would assert a fact the code
  // never established: a lawful owner break-glass with no channel wired filed as "abuse".
  ...
  // ---- the REJECTED stage is enumerated for TWO classes ONLY, because two classes are all that can produce
  // it. A "rejected" row means the DO's parseEmission REFUSED the emission at the input boundary, so it was
  // never matched against a single rule: no channel was consulted, and NO rule change can ever fix it. That is
  // the opposite remedy from no-channel ("wire a rule"), so filing it AS no-channel would send support to a
  // rule the customer already had.
  //
...
// "rejected" is NOT the product of every class: see the note in ALERT_EMISSION_COUNTER_NAMES. The composed
...
// THERE IS NO EVENT->CLASS MAP, AND THAT ABSENCE IS DELIBERATE. Deriving the class from the notify EVENT would
// be a lossy read: the events are a NOTIFICATION-ROUTING vocabulary (what a customer subscribes a channel to),
// not a taxonomy of what happened, and three of them are shared by unrelated alerts, so deriving the class
// from the event would make the counters coalesce on exactly the pairs that must be told apart.
//
// The class is instead an ARGUMENT, passed from the call site that knows the meaning of the alert it is firing
...
//
// NOISE is preserved structurally: only the routeXxxAlert wrappers -- which fire ONLY on security events --
// reach the counter at all. An OPERATIONAL notification (backup-success, canary-recovered, update-available)
// goes through routeNotification, never touches this, and a customer who has deliberately wired no channel for
// one can never trip a fault counter.
//
// There is no "source-detached" class here: a detached SOURCE BINDING is not a member being deprovisioned, and
// the event is emitted only from the cron alert pass, through routeNotification -- a DIFFERENT transport that
// never calls noteEmissionFailure. The REAL offboard events (a member removed in the console, a SCIM leaver
// removal) go through the offboard class named here, kept distinct from the owner-added grant.

// ---- the REFUSED / INCONCLUSIVE update, settle, ramp and rollback attempts --------------------------------
//
...
export const UPDATE_REFUSAL_COUNTER_NAMES = [
  ...
  // THE APPLY LEG'S OWN GUARDS. The four names below sit behind a recordUpdateRefusal audit event, and the
  // pack's configEvents excerpt deliberately drops the audit target's `detail` -- so without these names, every
  // one of them would reach a support engineer as the same unclassed "update-refused / denied" row. "We clicked
  // Update now three times last night and it just errored" can then have four different causes with four
  // different remedies: settle the open verification, mark the engine's own Cloudflare account, the signed
  // channel would not resolve an artefact, or configure a destination.
  ...
  // NO MEMBER for an unrecognised component split. parseComponentsRequest refuses a `components` array
...
// ---- the SILENT DEGRADATION of the update safety machinery -------------------------------------------------
//
...
export const UPDATE_DEGRADATION_COUNTER_NAMES = [
  // PRESENT-BUT-UNUSABLE versus HONESTLY-ABSENT are split into two pairs of names, because the recorder must
  // sit BEFORE the point where the product erases the difference. loadVerifiedChannel sanitises the signed
  // claim to well-typed-or-absent, so a TYPE-malformed claim (a sequence shipped as the string "12", an
  // issuedAt shipped as an epoch number) is already undefined by the time checkChannelFreshness runs: a
  // recorder placed there could only ever see ABSENT, filing that absence under the MALFORMED name and leaving
  // the far worse state -- no watermark yet (the whole fleet today), a type-malformed issuedAt, the opt-in
  // max-age guard ON, and therefore running on nothing while a stale descriptor applies -- recording NOTHING AT
  // ALL and reading exactly like a fully-protected apply. The -malformed/-unparseable pair is bumped AT THE
  // ERASURE (the sanitiser), unconditionally, so it fires with or without a watermark; the -absent pair is
  // bumped by the check, and only for a claim the descriptor genuinely never made.
  ...
  // The FLEET-WIDE key fault is not a fact about any stored row. CONFIG_WRAP_KEY is present and is not 32 bytes
  // (an operator pasted the wrong secret), so loadConfigWrapKey throws BEFORE the config plane is read. No
  // destination credential in the ACCOUNT can be opened, and the gate has learnt nothing about whether a
  // console-set default even exists, so this stays its own name rather than "the stored credential would not
  // open" plus "the console-set default could not be resolved" -- two claims about a row nobody fetched, on
  // the poll path.
  "update-degraded-destgate-wrapkey-malformed",
  // A single self-check boolean would collapse two OPPOSITE diagnoses: the wrong build is live (roll back)
  // versus the engine could not PROVE what is live (do not touch it). Splitting the cause is the whole point.
  //
  // There is no third member for "the DO plane is down": that state has NO REACHABLE PRODUCER, because a catch
  // around runPreflight cannot throw by construction, and asserting it would claim a fact the code never
  // establishes. With every scheduler fetch rejecting, the real gate emits -preflight, because a preflight that
  // proved nothing is exactly what that member says. See update-gate.ts.
  ...
  // A single "decrypt-failed" counter would fold three causes with three OPPOSITE remedies into one, asserting
  // a decrypt failure in two cases where no decryption was attempted. The discriminator is already on the
  // thrown error (a closed, tagged cause), so these keep it rather than discard it.
...
/** The closed self-check cause: the two states a single boolean would erase. Both have a real producer in makeHealthGate.selfCheck. */
...
// ---- the CONTROLS SUPPORT CANNOT OTHERWISE PROVE ARE IN FORCE ----------------------------------------------
//
// Two counters, two questions the pack cannot otherwise answer, and both are POSTURE in the exact sense this
// group is about: the customer believes something is happening (a new member is being emailed their set-up
// link; every human action is attributed to a source IP) and it is not, and nothing else says so.
export const GRANT_POSTURE_COUNTER_NAMES = [
  // A role grant MINTED a registration invite (so the granted person has no passkey and CANNOT sign in without
  // the link), THE ENGINE HAS AN INVITE SENDER CONFIGURED, and the send still did not land. The grant
  ...
  //
  // BOTH conditions are required, and the second one is why this counter is readable at all. Bumping on every
  // minted invite that was not sent would be TRUE OF EVERY GRANT in an org that deliberately configures no
  // invite sender and hands links over out of band -- a legitimate posture. Requiring both keeps a fault-named
  // counter from climbing for ever on a healthy engine, in the one section whose contract is that any non-zero
  // value means something silently went wrong.
  //
  ...
  // An audit entry was appended for an ATTRIBUTED HUMAN actor (the engine identified a subject or an email)
  // with NO source IP captured, AT THE MOMENT THE ROW IS WRITTEN. The console labels such rows honestly ("not
  // recorded") and the pack's audit excerpt strips sourceIp from every event by design, so the customer's "why
  // do some of our audit rows have no source IP?" cannot be answered from the pack alone: a capture path that
  // is STILL failing (a proxy stripping the header, a path that never threaded it) and one that was fixed long
  // ago read as the same silence without this counter. It counts only what this engine appends from here on --
  // it does not count, and cannot count, the rows already on the chain -- so a NON-ZERO count is a capture
  // failure that is happening now, and lastAt says when it last did. The historical population is read off the
  // audit rows themselves. The IP itself still never rides: this counter exists precisely so that it never
  // has to.
  //
  // THE GOVERNED REPLAY IS NOT A CAPTURE FAULT. Approving a change or an owner action REPLAYS the proposer's
  // mutation as the proposer. Left unhandled, the replay caller would carry no source IP, so the replayed row
  // would be attributed + human + sourceIp null: this counter's exact shape, climbing on every approved change
  // under dual control at every customer who has turned the gate ON, on a healthy engine. The engine HAS the
  // address (the propose request carries it, and its own propose row records it), so it is carried onto the
  // replay caller via PendingConfigChange.proposedBySourceIp / PendingOwnerAction.proposedBySourceIp. A
  // deliberate drop of a captured address must never be read as a failure to capture it.
  //
  // ATTRIBUTED is load-bearing. Gating on the actor METHOD alone would count every FAILED PASSKEY LOGIN: that
  // audit draft carries actorMethod "passkey" with a null subject, a null email and a hard-coded null sourceIp,
  // because "passkey" there names the MECHANISM and there is no person and no session to take an IP from. A
  // healthy engine with three fumbled logins this week would then produce {count:3, lastAt:now},
  // BYTE-IDENTICAL to the ongoing-capture-failure row this counter exists to tell apart, with every cancelled
  // WebAuthn prompt keeping lastAt recent for ever. The question is asked about rows that NAME A PERSON, and
  // that is the only population counted.
  "audit-human-event-missing-source-ip",
] as const;

// ---- the whole posture family, in the order it is spread into ADMIN_COUNTER_NAMES -------------------------
//
// Six names in this family are NOT declared here: they already exist in ADMIN_COUNTER_NAMES. This module
