...
// is purely structural (a file-size split); update-apply.ts
...
export interface ArtefactMeta {
  ...
  // minEngineVersion is the compat floor: the OLDEST engine this release may be applied ONTO. When set
...
// rampVersion (OPT-IN) splits LIVE traffic between a new version and the prior one by percentage (the
...
export interface DeployDriver {
  ...
  // OA-3 sandbox experiment is what graduates the gate from warn to enforce.
...
// OA-3 sandbox experiment proves the platform returns module bytes unchanged.
...
export interface HealthGate {
  ...
  // The SAME flight as flyNow(), plus the one fact its CanaryLiveness cannot carry -- whether the cycle
...
//   ailing | pending -> KEEP (confirmation pending). The settle runs on the JUST-PROMOTED isolate, where
//            the code swap has reset every Durable Object, and BOTH the canary flight AND the self-check
//            round-trip that DO -- so in the post-swap window they routinely fail to COMPLETE. That is NOT
//            evidence the build is bad; it is the health check sitting inside its own blast radius, and
//            rolling back on it would roll back perfectly healthy updates on every swap. So a non-dead
//            verdict does not roll back: the verified new version is kept and the HOURLY canary -- which
//            runs from well clear of the swap window -- is the real health gate (it auto-rolls-back and
//            alerts if the build proves genuinely unhealthy; the operator's one-click rollback covers the
//            meantime). The self-check result is RECORDED for diagnosis (it shapes the reason and the
//            confidence) but NEVER gates: a check that cannot run in this window must not decide a rollback.
...
export interface GuardMessages {
  // Anti-rollback floor wording: below the settled high-water mark, or uncomparable to it.
  ...
  // liveSplit: the live deployment is currently split across more than one version (typically an
...
export interface GuardInput {
  ...
  // settledHighWaterMark is the MONOTONIC highest engine version this customer has ever SUCCESSFULLY
  ...
  dryRun?: boolean;
  // knownRollbackTarget/knownToVersion are the caller-supplied, PERSISTED-lifecycle rollback
...
// guard, verify-artefact (the bytes match the signed channel's hash), then record-the-rollback-target
// (split-aware, see below). It appends each step to `steps` via `log` and returns a typed GuardResult.
// Extracted so a guard change lands in ONE place; the caller-specific
...
export async function verifyAndGuard(
  ...
  // Anti-rollback floor: refuse a target BELOW the highest version ever successfully settled, the
  ...
  // Compat guard: refuse a release whose minEngineVersion is newer than the running engine.
  ...
  // Record the rollback target (the current live version) before any change. When the driver
  ...
  // lifecycle already vouches for, is trusted instead). router-updates.ts additionally
  // refuses a second apply/ramp while a prior one's verification is still open (the primary guard against
  // a repeat call reaching planAndPromote/startGradualRamp mid-verification);
  ...
  try {
    if (typeof driver.currentLiveVersions === "function") {
      ...
      if (single !== null) {
        ...
        // A split is not automatically fatal. If it is EXACTLY the shape of the one prior
