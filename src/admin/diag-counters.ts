...
/**
 * noteInvalidRunId counts ONE restore/verify runId rejection under its closed malformation kind. The
...
export async function recordTestOutcome(
  ...
  entry: {
    ...
    platformCode?: string;
    // The notify-channel surface's closed DeliveryFailCode. THIS TYPE IS A WIRE CONTRACT: the field is
    // serialised straight into the DO body below, so a member that is not declared here is silently dropped
    // in the hop and the recorder never sees it.
    deliveryCode?: string;
  },
): Promise<void> {
  try {
 ...
 *
 * `deleteProbe` and `objectLock` are the destination probe's OWN closed verdicts, passed through
...
export async function noteTestOutcome(
  ...
  result: {
    ...
    // token. It is classified HERE through emailPlatformCodeOf and the raw value never leaves this function,
    // so the caller may hand over whatever the Email Service returned.
    ...
    // DELIVERY_FAIL_CODES by the recorder. It must NOT go through `reason`: classifyTestFailure's text arm
    // coarsens http-bad-request, http-gone, http-4xx and email-platform-rejected all to "rejected", which would
    // destroy the discriminator the channel layer had already computed.
  ...
  // is not verified, E_RATE_LIMITED = a platform throttle: three different remedies), and dropping it would
  // land all three in the pack as {"ok":false,"reasonClass":"other"}. emailPlatformCodeOf gates it to the
  ...
  await recordTestOutcome(scheduler, {
    ...
    // The CLEANUP DELETE's own status class. Inferring the probe's delete verdict from the bare fact of a
    // throw would read a 503, a 429 and a dropped socket all as "denied" -- a permission fact none of them
    // establishes. deleteProbe names what was measured and this carries the status behind it.
    ...
    // Object-Lock API) would be the same row with opposite remedies; without the second, "enforced" would stand
    // in for "locked". The DO recorder re-gates both against their closed sets and admits them only where they
    // mean something.
