...
//
// SEQUENCING CONTRACT (design MULTI-COMPONENT-UPDATES s6): when a release ships BOTH components, the
...
const CONSOLE_GUARD_MESSAGES: GuardMessages = {
  ...
  // liveSplit: a console deploy is always atomic (100% cutover; the console never ramps, see
...
export interface ConsoleApplyInput {
  ...
  consoleFloor?: string; // floors.console, the console's own anti-rollback floor
...
// applied-unconfirmed and rollback-failed are the console arm's two honest outcomes for states a plain
// "applied" or "rolled-back" cannot honestly claim: a promote whose live-version confirmation could not be
// read (so "applied" would be claiming a proof it never obtained), and a rollback whose own deploy failed
// (so "rolled-back" would be claiming a revert that did not happen, while the customer keeps being served
// the bad console build).
...
//      monotonic floors.console (which even allowDowngrade never crosses) plus the document-level
...
export async function applyConsoleComponent(driver: AssetsDeployDriver, input: ConsoleApplyInput): Promise<ConsoleApplyResult> {
  ...
  try {
    ...
    // "applied" is a claim of PROOF -- the promote landed AND the live version was read back and matched.
    // Here the read-back never happened, so the engine cannot say which version is live, and reporting
    // "applied" anyway would assert a confirmation it does not have.
  ...
  try {
    ...
    // The console rollback FAILED here, so reporting "rolled-back" would be wrong: the customer is still
    // being served the unexpected console build.
