...
export interface ReattachPlan {
  ...
  // MALFORMED sources: a downpipe whose source is binding-backed (kv/r2/d1/secrets) and whose config
...
// `malformed` is a third arm the caller can otherwise miss entirely: a binding-backed source that
...
export function planRosterReattach(downpipes: Pick<DownpipeConfig, "id" | "source">[], liveBindingNames: ReadonlySet<string>): ReattachPlan {
  ...
  // The sources the plan can present NOTHING for -- a binding-backed source with no binding name.
