// The "config as a source" model: downpipes versions its OWN governance configuration so an operator
...
//
// This file is the CHAIN + VERIFY layer. The SNAPSHOT shapes + projection live in ./config-snapshot.ts
// (ConfigSnapshot etc., snapshotConfig, serialiseSnapshot) and the plain-English diff lives in
// ./config-diff.ts (diffConfig, summarise), keeping each layer a readable size; this file re-exports
// every symbol from both so callers keep importing them by name from config-history.ts.
//
...
//   - digest    "edhmac256:" + b64url(HMAC-SHA-256(sessionKey, canonical(chain-bound fields))): the
...
export interface ConfigVersion {
  ...
  digest: string; // "edhmac256:..." HMAC-SHA-256 over the chain-bound fields, keyed by the in-DO key
  //                 (a record carrying the legacy "edhmac384:" label over the SAME HMAC-SHA-256 bytes
  //                 verifies too; configDigestMatches accepts both)
...
// labels the format uses. "edhmac256" reads as "engine-DO HMAC-SHA-256", naming the primitive precisely:
// the MAC is HMAC-SHA-256 under a 256-bit key, distinct from the 384-bit content hash it signs over. A
// published cryptographic inventory needs to say that plainly, without a caveat paragraph warning a
// reviewer that grepping for 384 lands on a SHA-256 MAC.
const DIGEST_PREFIX = "edhmac256:";

// LEGACY_DIGEST_PREFIX is a label some existing records carry. The MAC bytes under it are computed
// identically (same key, same canonical body, same HMAC-SHA-256), so a stored record stays verifiable
// regardless of which label it carries. configDigestMatches accepts either label and compares the MAC
// body, so an existing config history verifies intact after an engine update rather than reporting a
// chain break the operator cannot tell from tampering. New records carry DIGEST_PREFIX from the first write.
const LEGACY_DIGEST_PREFIX = "edhmac384:";

// digestBody strips whichever accepted label a digest carries and returns the base64url MAC body, or null
// when the string carries no label this build knows. The label is not a secret, so testing it with
// startsWith leaks nothing; the constant-time comparison in configDigestMatches covers the MAC body, which
// is the part an attacker would have to guess.
function digestBody(digest: string): string | null {
  if (digest.startsWith(DIGEST_PREFIX)) return digest.slice(DIGEST_PREFIX.length);
  if (digest.startsWith(LEGACY_DIGEST_PREFIX)) return digest.slice(LEGACY_DIGEST_PREFIX.length);
  return null;
}

/**
 * configDigestMatches compares a freshly recomputed digest against a stored one, accepting either the
 * current or the legacy label on the STORED side, and comparing the MAC bodies in constant time
 * (constantTimeEqual returns false on differing lengths, so a truncated body cannot match).
 *
 * A digest carrying no known label never matches: an unlabelled or foreign-labelled string is not a digest
 * this build can verify, and reading it as one would let a forged record pass on a label nobody recognises.
 *
 * @param recomputed - the digest recomputed under the in-DO key (always the current label).
 * @param stored - the digest as persisted on the record (current or legacy label).
 * @returns true when both carry a known label and their MAC bodies are equal.
 */
export function configDigestMatches(recomputed: string, stored: string): boolean {
  const a = digestBody(recomputed);
  const b = digestBody(stored);
  if (a === null || b === null) return false;
  return constantTimeEqual(utf8(a), utf8(b));
}
...
export interface ConfigChainVerdict {
  ...
  // causeClass: WHICH check failed. "Broken at version 41" is one answer for four different
...
async function digestBreakCause(successor: ConfigVersion | undefined, v: ConfigVersion, rawKey: Uint8Array): Promise<ChainBreakCause> {
  ...
  if (!configDigestMatches(successorDigest, successor.digest)) return "digest-unvouched"; // the witness is broken too
...
export async function verifyConfigChain(
  ...
  for (let i = 0; i < versions.length; i++) {
    ...
    // Compare the HMAC digest in constant time (over the label-stripped body) to avoid a timing
    // side-channel, matching session.ts/recovery.ts/saml dsig. configDigestMatches accepts the pre-rename
    // "edhmac384:" label on the STORED side, so a history written by an older engine still verifies.
    //
    // A DIGEST FAILURE DOES NOT ESTABLISH THAT THE BODY IS INTACT.
    //
    ...
    // its unkeyed contentHash. That version then PASSES the recompute and FAILS the digest, so reporting the
    // digest failure as proof of intactness would tell support "the body is intact and verifies cleanly" while
    // the body was the attacker's.
    //
    ...
    if (!configDigestMatches(recomputedDigest, v.digest)) {
