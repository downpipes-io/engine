...
//
// THE PROBLEM THIS SOLVES: the pack's restore evidence is OUTCOME-PLUS-COUNTS. A failed restore-apply keystone says HOW MANY
...
const PHASE_OF_CLASS: Record<RestoreFaultClass, RestoreFaultPhase> = {
  ...
  // The break-glass-only refusal is a PLAN-phase honest refusal (nothing was opened, nothing written).
  "posture-unexercisable": "plan",
  // A D1 decode refusal is an ARCHIVE-readability verdict reached before any write, so it is a `verify`
...
export async function recordRestoreOutcome(scheduler: DurableObjectStub, op: RestoreFaultOp, out: RestoreOutcomeView): Promise<void> {
  ...
  // THE CONFUSED-DEPUTY ATTEMPT. A restore that names one of the ENGINE'S OWN reserved bindings as a
  ...
  // whole restore before any write. The refusal is correct and complete, and recording it answers
  // "did anyone probe reserved bindings via restore?" where nothing else would. The engine's own reason literal
  // is the discriminator (it is engine-set, never caller text), and the binding NAME never rides.
  try {
...
//
// Projecting a failed drill (manual or scheduled) through restoreFaultRows alone would give ONE row carrying a
// closed class and nothing else: no blast radius (1 corrupt record or 500?), no failing index (which object did
// the bucket's lifecycle rule expire?), no missing-object name, and no way to tell "a deploy wiped
// SIGNER_PRIVATE" from "the archive is corrupt". It would also file the BREAK-GLASS-ONLY posture -- an honest
// refusal, not a failure -- under the same class as a genuine one.
//
// drillFaultRows is the drill's own projector. It reuses the SAME bounded ring, the SAME closed vocabularies
