...
// apply machinery, plus the blind restore test and keyless attestation (the dispatch-split spoke for
// restore). Reads its shared locals off the RouterCtx the hub builds once, so the role gate, the maker !=
// checker dual-control gate and every status code run here exactly as they do for every other route.
// Imports shared primitives and the restore core; never imports router.ts.
...
//
// Swallows every failure, and the consequence of that is stated here rather than glossed over. It is called
// from the read-only dry-run path, so a DO hiccup must not turn a preview into an error. But an unrecorded
// anchor is not harmless: requestRestore refuses a request whose plan has no recorded dry run, so a preview
// whose anchor did not land cannot be requested against and the operator has to run the dry run again. That
// is the deliberate direction: the request route's re-plan recomputes fidelityWarnings and discards them
// (only isLatest/plannedWrites/bytes are forwarded), so falling back to the request instant would disclose
// nothing to anyone. A refusal an operator can clear with one read-only call is the honest failure.
...
export async function buildSourceBindingMap(scheduler: DurableObjectStub): Promise<Map<string, string>> {
  ...
  try {
    ...
    // The source lives on `config`: GET /downpipes returns the DO's listDownpipes(), an array of
    // DownpipeState, and sched/types.ts declares `config: DownpipeConfig` with the source nested inside it.
    // `source` is still accepted at the top level too, so a caller or fixture using the flat shape keeps
    // working.
    ...
    for (const dp of downpipes) {
      ...
      if (s.type === "kv") {
        ...
        // bucketName. Without it, a source carrying only a binding produces no r2: entry at all, resolveSink
        // misses and falls back to the R2_<bucket> convention, and every record skips with "target binding
        // not present": a restore that completes and writes nothing.
        //
        // The key is the BINDING because that is what the archive record actually carries in these cases.
...
// as a base64url(32 bytes) value on a DISTINCT TRANSPORT HEADER. It is NEVER a field on RestoreRequest:
...
export async function handleRestore(ctx: RouterCtx): Promise<Response | null> {
  ...
  switch (`${req.method} ${sub}`) {
    // ---- restore: dry-run any role; APPLY requires Approver/Owner (the hard rule below) ------
    case "POST /restore": {
      ...
      // Reject a runId that is not a canonical ULID before it reaches any object-key template. A
      // dot-segment payload (e.g. "../../evil-bucket") would otherwise flow unchecked into
      // `run/${runId}/...` and, for an S3-style destination, collapse the signed request path outside
      // the configured bucket via new URL()'s RFC-3986 normalisation (dest/sigv4.ts).
      // The rejection reaches only the immediate HTTP caller, so a customer's integration script can 400
      ...
      // The optional browser-supplied per-run master rides a distinct transport header (never on the
      ...
      // RESTORE_BUFFERED_MAX_BYTES is resolved deep inside the restore core (restore-sinks.ts), which holds
      // no DO stub and must not grow one; this route is where the knob's usability is answerable with a
      // recorder in hand, and it is the real entry point for the operator complaint "I set the knob and
      // records still buffer". A boolean predicate over env: the unusable value itself never travels. An
      // absent knob is the default, not a fault, so an ordinary deployment records nothing here.
      if (bufferedRestoreMaxBytesInvalid(env)) fireInBackground(runtime, bumpAdminCounter(scheduler, "restore-buffered-max-invalid"));
      // Enforced server-side: any apply (confirm:true) requires Approver or
      ...
      // limiter must NEVER mask a security-relevant denial: for the apply path the role gate (and
      ...
      if (body.confirm === true) {
        ...
        // 1) The capability gate first (before rate-limiting): an apply (confirm:true) requires
        ...
        // the denied-apply audit and a security-relevant denial is never masked by a 429. The maker !=
        ...
        // 1a) Step-up: a stale ambient session must not write customer data back without a fresh re-auth.
        // POST /restore is one static sub carrying both the read-only dry-run (this branch never reaches;
        // confirm is not true) and the actual apply, so it cannot be a literal STEPUP_SUBS member (a Set
        // keyed on `sub` cannot see the body's `confirm` field) -- this mirrors the dynamic-<id>-approve
        // pattern exactly, one static sub gated on its parsed dangerous action instead of one dynamic id
        // (see router-core.ts's comment on STEPUP_SUBS). Called here, after the role gate and before the
        // dual-control reservation, so an unauthorised or unfresh apply is refused before any DO state is
        // touched or any byte written; the token method stays exempt inside requireStepUp, and an Access
        // caller is judged on the assertion's issued-at there, exactly as everywhere else.
        const stepUp = await requireStepUp(req, scheduler, caller, runtime);
        ...
        // 2) Dual control: the apply additionally requires a second authorised identity's approval,
        // bound to this plan hash, with maker != checker. The DO is the authority; ask it to reserve a
        // usable approval before touching live data -- gateRestore atomically checks usability and flips
        // the record to "applying" in one Durable Object read-modify-write, closing the TOCTOU race a
        // read-only gate would leave open, where N concurrent identical applies could all observe
        // usable:true and all run before any of them consumed. A self-approval is already refused at
        // approve time, and the reservation re-checks approver != requester, so a caller cannot approve
        // their own apply. No usable approval -> 403 { error: "restore not approved", planHash } (a
        // capability gate the console routes to "awaiting approval", not a mystery failure), recorded as
        // a denied apply; nothing was reserved, so there is nothing to release.
        ...
        if (!gateBody.usable) {
          // "Our approved restore said not-approved" is one of two opposite things: either the approval
          // genuinely is not there (the operator must go and get one), or the gate could not be read -- a
          // DO fault -- and the apply was refused fail-closed while a perfectly valid approval sat in
          // storage. The response is byte-identical in both cases (deliberately: the gate stays fail-closed
          // and must never oracle), but recordAdminRefusal below distinguishes them by reason class, so the
          // DR-hour question "is my approval missing, or is your engine broken?" has an answer.
        ...
        // A usable approval that still carries an appliedAt is proof that an earlier apply reserved this
        // approval, died mid-write (the Worker was killed: no /restore/release, no /restore/consume, no
        // restore-apply audit event -- nothing at all) and had its reservation reclaimed by the lease
        // (approvals.ts effectiveStatus). releaseRestore deletes appliedAt on every clean release and
        // consumeApproval terminates the record, so this state is reachable only through a crash. This
        // retry is about to overwrite the stale appliedAt, so record it now, before the reserve below
        // overwrites it: it is the only trace that a half-written apply ever happened, exactly the evidence
        // support needs for "my KV namespace is half restored and I have no idea why". Best-effort; it
        // never gates the retry.
        ...
        // 2b) Reserve: the gate above is read-only, so two concurrent applies for the same plan hash both
        // observe usable:true and both would otherwise reach the write below. Immediately adjacent to the
        // write -- nothing else runs in between -- this is the actual atomic single-use transition: the DO
        // flips approved -> applying in one read-modify-write, so of two racing requests only one can
        ...
        try {
          ...
          // A throw out of the restore core propagates past every line below -- the recordAudit at (4), the
          // receipt anchor at (5) -- so an apply that dies here would otherwise leave no outcome row
          // anywhere: the pack would show a consumed/released approval and nothing else, and support could
          // not tell whether the engine had started writing. Record both the closed fault row (op apply,
          // phase write, class apply-threw, carrying the 8-hex errId that joins to the Workers-Logs line)
          // and a failed restore-apply audit event, which rides the existing keystone projection. Both are
          // best-effort and neither swallows the throw: the caller still gets its 500.
          ...
          // result is undefined only when the call above threw (propagating out uncaught); either way the
          // reservation must not be left dangling, so an unset result also releases.
          //
          // This is the dual-control accounting write. A dropped consume means a single-use approval is
          // never terminated (it may authorise a second apply -- the single-use guarantee a dual-control
          // attestation rests on is then not proven); a dropped release strands the reservation until the
          // lease is reclaimed, so the operator's retry is refused with no cause anywhere. recordDiagWrite
          // counts a failed consume/release rather than letting it vanish silently, and the DO's own
          // approvalFaults ring records the reclaim from the other side. The path stays best-effort and
        ...
        // 3a) Project the finished apply into the bounded restoreFaults ring -- which record failed, in
        // which phase, in which closed failure mode (integrity abort vs a WORM-refused write vs an object
        ...
        await recordAudit(scheduler, caller, sourceIp, "restore-apply", result.ok ? "success" : "failed", {
          ...
          // Name the destination the bytes were actually read from at apply, so a divergence from the
          // reviewed destinationId (hash-bound; see restorePlanHash) is visible on the final event too.
        ...
        if (result.receipt) {
          try {
            // Enrich the receipt anchor with a REDACTION-SAFE apply SUMMARY so the support pack can see
            ...
            // The cf-config restore's per-item skip CLASSES, summed across every surface. Each skip site
            ...
            // The per-CLASS breakdown of the failures. `failures: 3` alone is an integer with no cause, and a
            ...
            // surface refused the write, and one whose video would not re-upload are the same evidence at
            // that resolution even though three of those four are not even about the archive. The class is
            // the one the fault site TAGGED; the per-record `reason` prose and the record names never ride.
            // An untagged failure counts honestly as sink-write rather than being dropped, so the classes
            // always sum to `failures`.
            ...
            const summary = {
              ...
              // The media apply's per-CLASS failure counts and any conflict digest pairs, exactly as the
              ...
              // The D1 partial-apply LOCALISATION (closed error class + failed batch index / batch total,
              ...
              // The restore descriptor fields a sink SHED by design (an unusable KV expiration, an
            ...
            await recordAudit(scheduler, caller, sourceIp, "restore-verified", result.receipt.summary.allVerified ? "success" : "failed", {
              ...
              ...summary,
              // configSkipReasons: fold the per-surface cf-config skip classes the apply already
            ...
            // The receipt phase. If the apply succeeds and its proof-of-correct-restore does not land, the
            // receipt's SHA-384 is never bound into the tamper-evident chain, so the one artefact an auditor
            // asks for after a restore ("prove what you wrote back, and that it verified") does not exist,
            // with only this log line in a Workers Logs stream the vendor cannot pull. The restore is not
            // failed and nothing about the apply changes; the anchor's loss is recorded as a fault row.
            // `other` is the honest class (the throw is a DO/audit-write fault, not one of the named restore
            // failure modes) and errId is the existing irreversible join key; the message never rides.
          ...
          // Emits the restore-applied notification: this is the one irreversible operation in the product,
          // so a customer who selects it on a rule must see it fire, exactly as a restore test passing does.
          //
          // The detail states the shortfall rather than only the total, for the same reason the receipt and
          // the console outcome do. "Restored 98 of 100" with no cause reads as success, and a notification
          // is the surface where an operator is least likely to go looking for one.
          //
      ...
      // Dry-run path (confirm omitted or false), gated on restore.dryrun, so restore.dryrun is a real,
      // live authorisation check rather than a phantom one that is merely declared in the contract and
      // shown in the console's roles-builder (proven directly against a hand-built caller lacking it in
      // validate-custom-roles.ts). Precise claim: every built-in role holds restore.dryrun from the
      // viewer floor up, and identity-rbac.ts's read floor is additionally folded, unconditionally, into
      // every custom role's resolved capability set (resolveAuthority, scheduler-do-rbac.ts, "custom
      // roles are additive"), so no custom role composable through the product today can actually be
      ...
      // A dry-run refusal (an oversized in-account window, a reserved target binding, a run that could not
      // be opened, every record skipped for a missing binding) needs to be visible in the support pack, not
      // silently absent while the customer stays blocked. A clean dry-run records nothing; a refusal names
      // its closed class and phase.
      await recordRestoreOutcome(scheduler, "dry-run", result);
      // This preview is what an operator reads and then asks a second person to approve, and the plan above
      // has just published the applyDeadline its fidelity warnings were computed against. Record the instant
      // so the approval's TTL starts HERE rather than at the request, which is what makes that deadline true:
      // without it the last instant an apply could still be writing would be
      // `request + RESTORE_APPLY_DEADLINE_MS`, later than this plan's deadline by however long the operator
      // spends reading it, and a KV expiration lapsing in that gap would be dropped by the sink with nothing
      // in the approved plan naming it. Put-if-absent in the DO, so the anchor is the OLDEST preview still in
      ...
      // an error. A missing anchor is not a silent loss of the property either: requestRestore does not fall
      // back to its own re-plan instant, it refuses the request, because the re-plan's fidelity warnings are
      // discarded and so the fallback would disclose nothing to anyone. An anchor that did not land costs the
      // operator one more dry run, loudly.
      //
    ...
    // ---- serve a chosen run's master capsule so the browser can decap its per-run master -----------
    ...
    // ---- dual control: request -> approve (maker != checker) -> apply ----------------
    case "POST /restore/request": {
      ...
      //
      // The blast-radius cues (isLatest/plannedWrites/bytes) are recomputed server-side from the runId
      // and selectors below, never forwarded from the client: an Operator+ raising a request by direct
      // API could otherwise show an approver benign cues (e.g. isLatest:true/plannedWrites:0) while the
      // hash-bound plan restores a large or stale run. The bound hash already binds only the real
      // decision fields and the apply already recomputes from the archive, so this never alters what is
      // restored; but the approver inbox and the restore-request audit event are the maker-checker
      // signal, so they must show the engine's truth, not the requester's claim.
      ...
      // Same ULID-shape gate as the other restore routes (see the comment on POST /restore).
      // Same closed-kind counter as the sibling routes (see POST /restore).
      ...
      // Decode the OPTIONAL browser-supplied per-run master from its transport header (never on the
      ...
      if (md instanceof Response) return md;
      const master = md.master;
      // Raising a restore request gates on restore.request (operator/restore-operator/approver/owner).
      ...
      const sourceBindings = await buildSourceBindingMap(scheduler);
      // The cues are computed WITHOUT the caller's destinationId (explicit passed as undefined),
      ...
      return scheduler.fetch(doURL("/restore/request"), {
        ...
        body: JSON.stringify({
          ...
          // Record WHICH destination the requester named (display/audit only; the cues above are
    ...
    case "POST /restore/approve": {
      ...
      // state rule, and records a restore-approve event with both identities. Gates on restore.approve
      // (restore-operator/approver/owner); the DO re-checks and refuses the bare-token fallback.
    ...
    case "POST /restore/reject": {
      ...
      const limited = await rateLimited(scheduler, caller);
      if (limited) return limited;
      // The checker's CLOSED rejection reason, gated at the edge as well as in the DO. A value outside the
    ...
    case "GET /restore/approvals": {
      // The pending-approval inbox. A caller who can approve (restore.approve: approver/owner, plus
      // restore-operator) sees every request; anyone else who raised a request still sees their own. Any
      // authenticated role may read it (so a requester can track their own); the DO filters by the
      // forwarded caller and the approver flag.
    ...
    case "POST /restore/verify": {
      ...
      // A restorability PROOF that is refused before it ever runs. The pack carries the stale
      ...
      // are locked out of their own proof, the other is a fleet nobody is drilling. The reason class is
      // what separates them: forbidden (a role/capability that cannot verify) from rate-limited (the
      ...
      // Pass the live scheduler so runBlindRestoreTest can pin minRunlogIndex to its account-global
      // runlogCounter, closing the same whole-document RUNLOG replay its sibling /restore/attest below
      // closes -- this endpoint persists the same stampRestoreProven compliance stamp on a pass, so it is
      // an equally-reachable, equally-valuable sink to close (same restore.verify capability, same
      ...
      // A FAILED restorability proof is the highest-value diagnostic in the product ("can I actually get
      // my data back?"), so the ring names the phase and the per-record class (WHICH record failed to
      // decrypt, and whether the fault was integrity or availability), not just a coarse restore-test
      // reason code on the downpipe. A PASS records nothing.
    ...
    case "POST /restore/attest": {
      ...
      // Pass the live scheduler so runKeylessAttest can pin minRunlogIndex to its account-global
      ...
      // The keyless attestation runs even in the break-glass-only posture, so a FAILED one is often the
      // only recovery signal an engine can produce at all. Its reason lands in the ring as a closed class.
