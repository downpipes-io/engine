...
export async function openVerifiedRun(env: Env, runId: string, destOverride?: RuntimeDestConfig | null, minRunlogIndex?: number): Promise<Run> {
  ...
  const store: ObjectStore = {
    get: async (k: string) => {
      ...
      if (!r) {
        // INT-3: distinguish a missing SEGMENT (the run's content-addressed bulk data, which the signed
...
// including the INT-3 seg/ completeness distinction, and the same freshness options), differing in ONLY two
...
export async function openRunFromMaster(env: Env, runId: string, master: Uint8Array, destOverride?: RuntimeDestConfig | null, minRunlogIndex?: number): Promise<Run> {
  ...
  const store: ObjectStore = {
    get: async (k: string) => {
      const r = await dest.get(k);
      if (!r) {
        // INT-3: a gone SEGMENT (`seg/`) is a completeness shortfall of a present, signed run -- a tamper-
