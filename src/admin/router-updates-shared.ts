...
// place (size hygiene; the ramp halves live in router-updates-ramp.ts and must not import
// router-updates.ts back, the same no-cycle rule router-updates-components.ts follows).
...
// readUpdateFloor reads the persisted ANTI-ROLLBACK floor (settledHighWaterMark) + the FRESHNESS watermark
// (lastChannelSeq/lastChannelIssuedAt) from the update-lifecycle record, plus the opt-in max-age, so the
...
// freshnessRefusal enforces replay protection on the resolved channel descriptor. It returns a jsonError
...
export function freshnessRefusal(art: RecommendedArtefact, freshness: FreshnessState, onRefused?: () => void, onDegraded?: (names: readonly string[]) => void): Response | null {
  // The erased-claim flags ride with the claim. The resolver already COUNTED a malformed claim (at the
  ...
  if (!verdict.ok) {
    // THE ONE THAT MATTERS. A freshness/replay refusal means the channel served an artefact that is
    // back-dated or below the sequence high-water mark -- a REPLAYED update, which is a possible attack
    // signal -- and it is the ONLY guard in this family that emits no audit event at all: it is a bare
    // 400 into the browser.
    onRefused?.();
...
// pendingUpdateRefusal (the PRIMARY guard) refuses a NEW apply/ramp while a PRIOR apply/ramp's
...
// recordUpdateRefusal records a FAILED self-apply update attempt as an `update-refused` audit event
...
// not wrap it).
//
// It is NO LONGER CALLED DIRECTLY BY ANY GUARD. Calling it directly and stopping there would mean every
// apply/ramp-start guard's refusal reaches a support engineer as the same unclassed "update-refused /
// denied" row, since the pack's configEvents excerpt deliberately drops the audit target's `detail`.
// noteUpdateGuardRefusal below is the ONE entry point (it calls this, plus the closed counter that
// actually carries the class into the pack), so the audit half can never ship without the pack-visible half.
...
// noteUpdateGuardRefusal is the ONE call every update-family guard makes when it refuses.
// recordUpdateRefusal above is private to this module and has exactly one caller: this function. A guard
// cannot ship the audit half without the pack half.
//
...
// noteUpdateAttempt counts the ATTEMPT itself, which the pending record cannot: `updates.last` keeps
...
// noteUpdateDegradations counts the SILENT WEAKENINGS of the update safety machinery: a replay claim
