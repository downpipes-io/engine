...
// ARE the sign-in flow). This spoke imports the shared primitives (router-core), the alert routers
// (router-notify) and the session resolvers (router-session); none of them imports it, and it does not
// import router.ts (no cycle).
...
import { attemptedMethod, authRateLimited, logAuthFailure, requireStepUp, logAuthCeremonySuccess } from "./router-core.ts";
...
import { consumePasskeySessionMintFault } from "./test-faults.ts";
...
export async function handleRecovery(req: Request, env: Env, runtime?: AdminRuntime): Promise<Response> {
  // fireInBackground: both auth-signal writes below fire as the LAST action on their branch before an
  // immediate return, with no await in between to give the detached promise a scheduling window -- a bare
  // `void` here can lose the write live. runtime is undefined only for a direct call with no fetch runtime
  // (a unit test), which falls back to the bare-void behaviour.
  const fireInBackground = (task: Promise<unknown>): void => {
    if (runtime?.waitUntil) runtime.waitUntil(task);
    else void task;
  };
  const scheduler = schedulerStub(env);
  // PER-IP pre-throttle (same fail-closed per-IP ceremony limiter as the passkey routes), BEFORE the body
  ...
  const resp = await scheduler.fetch(doURL("/recovery/recover"), {
    method: "POST",
    body: JSON.stringify({ email: body.email, code: body.code, ...(ip !== null ? { ip } : {}), ...(readSessionCookie(req) ? { priorToken: readSessionCookie(req) } : {}), userAgent: req.headers.get("user-agent") }),
  ...
  if (result.ok !== true) {
    ...
    // "every recovery code we try is rejected" otherwise leaves NOTHING in the pack -- the log line above goes
    // to Workers Logs, which remote support structurally cannot read. Count the rejection class only; the
    // presented code, the email and the IP never ride, and the 401 the caller sees stays generic.
  ...
  //
  // enrolPasskey IS FORWARDED FROM THE DO, never hardcoded here: the authority computes the answer, and a
  // layer in front republishing a constant instead would mean that the day the authority learns to say no,
  // the wire keeps saying yes. It is read DEFENSIVELY (an absent or non-boolean field yields false rather
  // than true), because the only cost of a false negative here is that the console asks the user to enrol
  // from the Keys screen instead of prompting them, and the cost of a false positive is offering a button
  // the registration route will refuse.
...
export async function handleRegenerate(req: Request, env: Env, runtime?: AdminRuntime): Promise<Response> {
  ...
  // Records WHICH branch refused (unset CONSOLE_ORIGIN vs a foreign Origin vs no Origin at all): this is the
  // last-way-back-in path, so "regenerate recovery codes always 403s" would otherwise leave no trace in the
  // pack whatsoever. The 403 and its body are unchanged.
  ...
  // (or a recent authentication). The bare token is exempt inside requireStepUp and an Access caller is judged
  // on the assertion's issued-at; a recovery sign-in is itself recent-enough (freshness) so a break-glass
  // operator is never locked out of this.
  const stepUp = await requireStepUp(req, scheduler, verdict, runtime);
  ...
  if (result.ok !== true || !Array.isArray(result.codes)) {
    // The regenerate failed engine-side. Without this, the console keeps nagging the operator to regenerate and
    // the regenerate keeps failing, with the DO's reason discarded into a bare 500 and recorded nowhere.
...
// handleConfirmStaged serves POST /admin/auth/recovery-codes/confirm: the console fires this the moment
// the operator ticks "I have saved my recovery codes" and
...
async function handleLogout(req: Request, env: Env, runtime?: AdminRuntime): Promise<Response> {
  // fireInBackground: see handleRecovery's identical helper above.
  // Load-bearing here in particular -- see the session-termination comment below.
  const fireInBackground = (task: Promise<unknown>): void => {
    if (runtime?.waitUntil) runtime.waitUntil(task);
    else void task;
  };
  // A csrf-origin-unset here is especially misleading in the field ("I click sign out and nothing happens"),
  // so the branch is named. The signal is best-effort and must never block the logout: schedulerStub can
  // THROW synchronously on an unbound env, so the resolve is contained here exactly as the router's own
  // onAuthDeny sink contains it.
  ...
  if (token !== null) {
    try {
      ...
      // THE ONE THAT MATTERS MOST ON THIS LIST. The cookie is cleared unconditionally (correctly: a
...
async function resolveRegisterAuth(
  ...
  // invitee has no session yet: that is the whole point of an invite), so reading it only inside the
  // authorised arm would forward no IP on that path: the DO would read sourceIp = null, and every completed
  // invite enrolment would append an audit entry that is ATTRIBUTED (it names the enrolled member) with a
  // human actor method and NO SOURCE IP -- exactly the shape the audit-source-ip-gap counter exists to
  // detect. An organisation onboarding three people this week, on an engine whose capture path works
  // perfectly, would then produce {count:3, lastAt:<recent>} in adminCounters, a section whose whole contract
  // is that a non-zero value means a surface silently went wrong. That reading is byte-identical to a proxy
  // stripping the header, so the counter could not answer the one question it exists for, with both halves
  // of its discriminator dead: lastAt recent when nothing is wrong, and the count climbing on the commonest
  // healthy event in the estate.
  //
  // Threading the edge IP on every register path makes the counter mean what its name says: it now moves only
  // when the engine had an actor and genuinely could not capture an address.
  ...
  // The "enrol-passkey" purpose is what lets a FRESH OIDC/SAML session pass here (ASVS V6.8.4): an SSO member
  // enrolling their first passkey has no passkey to assert with, and this is the only route that says so. An
  // Access self-add is gated the same way and judged on the assertion's issued-at (ASVS V7.5.1).
  if (sub === "register/finish" && verdict.ok && verdict.method !== "token") {
    const stepUp = await requireStepUp(req, scheduler, verdict, runtime, "enrol-passkey");
...
export async function handlePasskey(req: Request, env: Env, sub: string, runtime?: AdminRuntime): Promise<Response> {
  // fireInBackground keeps a diagnostic write alive past the response, the same way ctx.waitUntil already
  // does for every other background write in this codebase (index.ts's sealNow/canaryNow). Every call below
  // fires as the LAST action before its enclosing branch returns, with no further await to give it a
  // scheduling window: a bare `void` here is not merely theoretically racy, since the isolate can be
  // reclaimed before the write to passkey-session-mint-failed completes. runtime is undefined only for a
  // direct handleAdmin call with no fetch runtime (a unit test), which falls back to the bare-void behaviour
  // unchanged.
  const fireInBackground = (task: Promise<unknown>): void => {
    if (runtime?.waitUntil) runtime.waitUntil(task);
    else void task;
  ...
  // CONFIRM a staged recovery-code set, also AUTHENTICATED and scoped to the caller's own email. See
  // handleConfirmStaged.
  ...
  // yet, so this one bucket is keyed on the source IP (CF-Connecting-IP). It FAILS CLOSED, which is the
  // OPPOSITE of the per-caller limiter and is the whole point: authRateLimited has three fail-closed arms (a non-admit
  // verdict, an answered-but-broken verdict, and an unreachable limiter) precisely because there is no
  // verified operator here whose availability is worth trading for an open brute-force window. It sits AFTER the
  ...
  if (oc === null) {
    // CONSOLE_ORIGIN is unset or unparseable, so EVERY passkey ceremony 501s and nobody can enrol or sign
    // in. This is a deploy-configuration fault that reads to the user as "sign-in is broken", and without
    // this the pack would carry no trace of it at all.
  ...
  if (isFinish) {
    ...
    const setCookie = await sessionCookieForFinish(env, scheduler, text, readSessionCookie(req), { sourceIp: req.headers.get("CF-Connecting-IP"), userAgent: req.headers.get("user-agent") });
    // V16.3.1: a proven ceremony that minted a session is logged at the moment it happens; a finish that did
    // not verify sets no cookie and is already on the audit chain as an authn-failure row.
    if (setCookie !== null) logAuthCeremonySuccess("passkey", sub === "register/finish" ? "register/finish" : "login/finish");
    // Count the ceremony OUTCOME in the bounded auth-signal aggregate, so a lockout is visible in the pack
    // instead of only in Workers Logs. recordPasskeyOutcome allowlists the DO's coarse reason
    ...
    // V6.3.7: a passkey SELF-ADD on an already-enrolled account is a credential change exactly as a revoke is,
    // and the revoke already fires auth-credential-change (router-account-session.ts). Gated on the DO's own
    // finish body, never the client's, and on the DO's proven enrolmentPath: a bootstrap (first Owner) or an
    // invite redemption is an onboarding, not a change to an existing account's sign-in details. Fired after
    // the response is fully determined, through the runtime-aware fireInBackground, so it can never delay or
    // fail the ceremony. The detail names the account and nothing about the credential.
    if (sub === "register/finish" && setCookie !== null) {
      type FinishBody = { ok?: unknown; email?: unknown; enrolmentPath?: unknown };
      let enrolled: FinishBody | null;
      try {
        enrolled = JSON.parse(text) as FinishBody;
      } catch {
        enrolled = null;
      }
      if (enrolled !== null && enrolled.ok === true && typeof enrolled.email === "string" && enrolled.email !== "" && enrolled.enrolmentPath === "self-add") {
        fireInBackground(routeAuthChangeAlert(env, scheduler, "auth-credential-change", "credential-change", `A new passkey credential was enrolled for ${enrolled.email}.`));
      }
    }
  ...
  // options. The status stays the DO's (200 for a verification verdict). A BEGIN that refuses is its own
  // lockout class (a spent/absent invite, a forbidden enrolment, a challenge store that cannot write), so
...
// recordPasskeyOutcome counts a passkey ceremony outcome as a CLOSED auth-signal. It parses the DO's
...
export async function sessionCookieForFinish(env: Env, scheduler: DurableObjectStub, finishBodyText: string, priorToken?: string | null, client?: { sourceIp?: string | null; userAgent?: string | null }): Promise<string | null> {
  let parsed: unknown;
  try {
  ...
  try {
    // HARNESS-ONLY: a passkey-session-mint-fail fault, if armed, forces this call down the SAME
    // catch below a real DO hiccup takes, single-shot -- the ceremony is genuinely verified (o.ok===true just
    // above, off the DO's own finish body) and the mint alone is forced to fail, so recordPasskeyOutcome's
    // caller records passkey-session-mint-failed rather than the failure vanishing. Gated on testFaultsEnabled
    // first inside consumePasskeySessionMintFault, so a production/dev/uat/internal engine never touches the
    // fault store.
    if (await consumePasskeySessionMintFault(env, scheduler)) throw new Error("harness test-fault: passkey session mint forced to fail (passkey-session-mint-fail)");
    const resp = await scheduler.fetch(doURL("/passkey/session/issue"), {
      method: "POST",
      // priorToken (V7.2.4): the session cookie this browser presented, forwarded so the DO ends it as it mints.
      body: JSON.stringify({ email, ...(typeof priorToken === "string" && priorToken !== "" ? { priorToken } : {}), ...(client !== undefined ? { client } : {}) }),
      headers: { "content-type": "application/json" },
    });
...
export async function sendBootstrapLink(env: Env, scheduler: DurableObjectStub, req: Request, runtime?: AdminRuntime): Promise<Response> {
  ...
  const skip = (reason: string): Response => {
    ...
    // "the first-Owner set-up email never arrives" is the most common bootstrap ticket, and this route
    // answers a NO-ORACLE 200 on every skip reason BY DESIGN (an anonymous presser must not learn whether an
    // Owner exists). The response below is unchanged; the reason is also RECORDED as a closed signal, so
    // support can tell "no EMAIL binding" from "the bootstrap path is closed because an Owner already
    // exists" from "the DO mint faulted", rather than relying on a Workers Logs line the vendor structurally
    // cannot read. Best-effort; never the address or the link.
  ...
  try {
    ...
    // The PROVIDER rejected a correctly-configured send. This is the diagnosis that most often ends the
    // ticket ("your sender domain is not onboarded"), so it is recorded as a closed signal rather than
    // reaching only the operator's tail log. The provider's message is READ only by the log line above and
    // is never recorded.
