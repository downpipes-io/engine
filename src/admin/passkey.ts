...
export interface PasskeyCred {
  ...
  // THE USABILITY WITNESS. Every other field on this
  // record is unchanged by the destruction of the authenticator holding the private half, which is why
  // `passkeyOwnerEnrolled` can read true on seven credentials that cannot produce an assertion. These two
...
// ids: every size up to and including 64 creates a credential, and 65 and above throws
// `DOMException` named `RangeError`, message "The `excludeCredentials` attribute exceeds the maximum
// allowed size (64)." The n=0 known positive creates a credential, so the rig can succeed and its
// failures therefore mean something. Firefox and WebKit are INCONCLUSIVE in that rig (they answer
...
// place to lower.
//
// WHY AN UNBOUNDED LIST IS A LOCKOUT RISK, NOT A BLEMISH. If excludeCredentials carried EVERY credential
// an email holds, an account that has accumulated 65 or more would have every registration ceremony throw
// in the browser before a single byte reaches this engine. That alone is only an inconvenience for someone
// holding a working passkey. The harm is that re-enrolment is ALSO the step that mints a fresh recovery-code
// set (passkeyRegisterFinish -> generateRecoveryFor): a break-glass recovery sign-in consumes one code and is
// then routed straight into enrolling a fresh passkey, and it is THAT enrolment which replaces the spent
// set. With the ceremony refused, each break-glass sign-in would spend a code and replace nothing, so the
// bank drains monotonically toward zero with no way back. An account that empties its bank in this state
// cannot enrol (the ceremony throws), cannot sign in with a passkey it has lost, and cannot delete a
// credential to get back under the ceiling, because /passkey/credentials/delete is step-up gated on the
// cookie path and step-up needs the very assertion it does not have. That is a permanent lockout reachable
// through the documented break-glass path exactly as intended, which is why the list is bounded below.
//
...
//
// MEASURED, Chromium 149.0.7827.55, CDP virtual authenticator on a local origin, dose-response over
// allowCredentials sizes 0, 1, 32, 63, 64, 65, 66, 100, 128 and 256. Sizes 1 through 64 complete the
// assertion; 65 and above throws DOMException named `RangeError`, "The `allowCredentials` attribute
// exceeds the maximum allowed size (64)." The rig reproduces the excludeCredentials result (first throw at
// 65), and the verdict does not move when the genuine credential is placed LAST rather than first: the
// browser refuses on list SIZE, before any authenticator is consulted.
//
// WHY AN UNBOUNDED allowCredentials IS A SECOND, DISTINCT LOCKOUT RISK. If stepUpBegin emitted an
// account's ENTIRE credential list here, step-up gates recovery-code regeneration, self-add passkey, the
// key ceremony, posture risk-accept, restore-approve -- and /passkey/credentials/delete, which is the ONLY
// way to reduce a credential count. So an account at 65 could not step up, and could not delete a
// credential to get back under 65, because deleting requires the step-up it can no longer pass. The count
// moves in one direction only. Bounding excludeCredentials alone lets such an account enrol again, but
// every enrolment adds a credential, so without also bounding this list, re-enrolling walks the account
// further from the ceiling it needs to re-cross.
//
