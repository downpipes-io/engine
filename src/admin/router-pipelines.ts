...
// drill rehearsal. Each case body reads the shared locals (including the injected `runtime` that
// carries sealNow / canaryNow) off the RouterCtx the hub builds once, so the per-route capability gate runs
// inline. Imports shared primitives + the drill core; never imports router.ts.
...
export async function handlePipelines(ctx: RouterCtx): Promise<Response | null> {
  ...
  switch (`${req.method} ${sub}`) {
    ...
    case "POST /downpipes": {
      ...
      // The audit row says "failed" and never why. The closed reason class rides alongside it, so a
    ...
    // This route's floor is set by naming its sibling: GET /downpipes gates on downpipe.read, so "the
    // same read class as GET /downpipes itself" resolves to a capability and this route takes it.
    // The forward carries no callerHeaders, so the DO arm (roster-hygiene) has no principal to check and
    // the router is the only possible home. downpipe.read is a viewer-floor capability, so this refuses
    // nobody: it makes "every authenticated role reads the list, so it reads this too" checkable against
    // the route it points at instead of against the role table nobody re-reads.
    ...
    case "POST /trigger": {
      ...
      if (denied) {
        // "Run now does nothing". For a caller whose role does not hold run.trigger that is precisely
        // what it does -- and without this, nothing anywhere said so.
      ...
      // bytes rather than stranding the downpipe in flight and reporting a fake success. The
      ...
      if (!tr.ok) {
        // A run that was never ALLOCATED produces no run row at all, so "Run now fails every time" has no
        // engine artefact whatsoever without this -- the pack simply shows a downpipe that has not run.
    ...
    case "GET /canary":
      ...
      // write and not a secret. It gates on NOTHING beyond authentication: no capability in this engine
      // names canary state specifically. Sibling reads each derive their capability from a route that
      // already discloses the same rows (the posture.read support pack) or from a sibling that names one,
      // and this route has neither. Its write siblings gate run.trigger and owner, both above the viewer
      // floor, so matching one would refuse readers rather than pin the floor. Authentication is therefore
      // the whole of what stands in front of it, and it is recorded as such in
      // test/validate-admin-route-manifest.ts (class authenticated-only).
    ...
    case "POST /drill": {
      ...
      // A FAILED drill leaves only a coarse reason on the downpipe's restore-test stamp without this. Project it into
      ...
      // This is the DRILL projector (not the generic one): it carries the blast radius (how many records failed),
    ...
    case "POST /drill-all": {
      // Start an ON-DEMAND bulk restore-test campaign ("drill the whole fleet now"). Same authority
    ...
    case "GET /drill-all":
      ...
      // a small failed-id sample), no secret. It gates on nothing beyond authentication, exactly like GET
      // /canary, for the same reasons: matching the write sibling would be wrong here too (POST /drill-all
      // gates drill.run, which viewer and access-admin do not hold).
