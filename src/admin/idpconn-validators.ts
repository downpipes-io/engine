...
// structural size budget. The validateOidc / validateOauth2 / validateSaml functions, the validateSecretRef
// helper, the shared string-coercion primitives and the bounds constants live here. idpconn.ts keeps the
// public discriminated-union types and the
...
// the trust root, and a rollover appends one to the pinned array; without rejectDuplicates, an append of a
// cert already in the array collapses to the same array and answers ok:true, telling the operator the new
// certificate is trusted when nothing has changed, so the cutover would break at the IdP's switch. Nobody
...
function validateOauth2OptionalFields(p: IdpConnectionProposal, conn: Omit<Oauth2Connection, "createdBy" | "createdAt">): { ok: true } | { ok: false; reason: string } {
  ...
  // PRESENT-AND-UNUSABLE IS A REFUSAL, NOT A DROP. A bare truthiness guard here would let an over-length or
  // control-character-bearing path fall through silently, saving the connection with the mapping ABSENT.
  // validateOidcClaims above refuses on exactly this sentinel for its own claim fields, and groupsPath is
  // authorisation, so the OAuth2 and SAML branches hold the same discipline rather than dropping the field.
...
export function validateSamlCerts(raw: unknown): { ok: true; certs: string[] } | { ok: false; reason: string } {
  // OVER CAPACITY IS NOT A SHAPE FAULT. strArray returns a single null for "not an array", "an entry is not
  // a bounded string" AND "more entries than maxItems", so without this branch an array of perfectly valid
  // PEMs that is merely one too long falls to the generic sentence below and tells the operator their
  // certificates are not PEM X.509 certificates, when the verdict is on LENGTH alone (returned before any
  // entry is read).
  //
  ...
  // rejectDuplicates: without it, an append of an ALREADY-PINNED certificate collapses to the existing array
  // and answers ok:true, telling the operator a rollover cert is now trusted when nothing has been added.
  // Named separately from the generic array failure so the operator is told which mistake they made.
...
export function validateSaml(p: IdpConnectionProposal, base: Base): ValidateResult {
  ...
  // PRESENT-AND-UNUSABLE IS A REFUSAL, NOT A DROP (see the same discipline on the OAuth2 paths above).
  // Without this check, an over-CLAIM_NAME_MAX groupsAttr would store the connection ok:true with NO group
  // mapping, so the operator's groups-to-roles rules would silently not apply and every user would land with
  // no groups. The console omits a blank box rather than sending "", so refusing an empty value breaks no
  // caller it has.
...
// isConnId is re-exported for the dispatcher's id check.
export { isConnId };
