// Identity and role types for the engine governance backbone (D1 whoami, D3 RBAC). The
...
export type AuthVerdict =
  // authIat is present on the ACCESS method only: the verified Access JWT's issued-at (epoch seconds), read by
  // the step-up gate so a stale Access assertion is refused on a sensitive action (ASVS V7.5.1). Never carried
  // on the passkey, oidc, saml or token verdicts, whose freshness the DO judges from the session cookie.
  // viaCookie marks a method "token" verdict that arrived as an EXCHANGED SESSION COOKIE rather than the bare
  // bearer (V7.2.2): the CSRF Origin guard applies to it like any other ambient cookie, and the bootstrap
  // exchange route refuses it (a session cannot be exchanged for another).
  | { ok: true; method: AuthMethod; email: string | null; subject?: string; exp?: number; authIat?: number; groups?: string[]; identityProvider?: string; connId?: string; viaCookie?: true }
...
export interface Caller {
  ...
  // authIat is the verified Access JWT's issued-at (epoch seconds), present on an ACCESS caller only. It is a
  // ROUTER-LOCAL field the step-up gate reads (ASVS V7.5.1): encodeCaller does not forward it to the DO, and
  // no DO path reads it, because the DO judges freshness from the session cookie it signs, which an Access
...
export interface WhoAmI {
  ...
  // it server-side against the readable __Host-downpipes_csrf cookie (ML-02, defence in depth on top of the
