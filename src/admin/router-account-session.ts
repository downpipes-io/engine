...
// ceremony. Each case body reads the shared locals off the RouterCtx the hub builds once. The hub
// already ran authorise(); per-route checks stay inline. Imports shared primitives + session helpers; never
...
//
// The refusal is RECORDED as its closed sub-class (header missing / cookie missing / value mismatch).
// Those are three different tickets -- a console that never read whoami, a browser that blocked or partitioned
// the cookie, and a genuine cross-site attempt -- and without this the pack carries no evidence for any of
// them, so "terminate-sessions always 403s" has nothing to point at. Best-effort and fire-and-forget: the 403 is already
...
function csrfBlock(caller: Caller, req: Request, scheduler: DurableObjectStub, runtime?: AdminRuntime): Response | null {
  ...
  // Both signals below fire as the last action before this function's
  // caller immediately returns the 403, with no intervening await -- the same shape that can otherwise lose the
  ...
  // The sub-class above says HOW the double-submit check failed; this says WHICH SURFACE was hit. The
...
export async function handleAccountSession(ctx: RouterCtx): Promise<Response | null> {
  ...
  switch (`${req.method} ${sub}`) {
    ...
    case "GET /passkey/credentials/all": {
      ...
      // records, so that residue accumulates and a measured sweep found five on one estate.
      //
    ...
    // V7.5.2: the caller's OWN live sessions, and ending ONE of them by id. The list is metadata only (no token
    // leaves the scheduler) and flags the session that presented this cookie; terminate takes a sid that must
    // belong to the caller (a foreign id is a 404), is CSRF-guarded and step-up gated like terminate-others.
    case "GET /sessions": {
      const resp = await scheduler.fetch(doURL("/passkey/session/list"), { method: "GET", headers: { ...callerHeaders(caller), "x-downpipe-session-token": readSessionCookie(req) ?? "" } });
      const out = (await resp.json()) as { ok?: boolean; sessions?: unknown[] };
      if (!out.ok) return new Response(JSON.stringify({ error: "no first-party session to manage on this auth method" }), { status: 400, headers: { "content-type": "application/json" } });
      return new Response(JSON.stringify({ sessions: out.sessions ?? [] }), { headers: { "content-type": "application/json", "cache-control": "no-store" } });
    }
    case "POST /sessions/terminate": {
      const csrf = csrfBlock(caller, req, scheduler, runtime);
      if (csrf) return csrf;
      const limited = await rateLimited(scheduler, caller);
      if (limited) return limited;
      const body = (await req.json().catch(() => ({}))) as { sid?: unknown };
      const resp = await scheduler.fetch(doURL("/passkey/session/terminate-one"), { method: "POST", body: JSON.stringify({ sid: body.sid }), headers: callerHeaders(caller) });
      const out = (await resp.json()) as { ok?: boolean; reason?: string };
      if (out.ok === true) return new Response(JSON.stringify({ ok: true }), { headers: { "content-type": "application/json" } });
      if (out.reason === "not-found") return new Response(JSON.stringify({ error: "no such session" }), { status: 404, headers: { "content-type": "application/json" } });
    ...
    case "POST /sessions/terminate-others": {
      ...
      if (!out.ok || !out.token) {
        // THE THIRD GUARD, AND THE ONE THE DO CAN NEVER SEE. This 400 is decided HERE, in the router, so it
        // never enters the DO's AuthError funnel and no ledger row for it exists on either side otherwise.
        // On the console it reads as {admin-write, access-security, terminate-other-sessions, refused-validation},
    ...
    case "POST /stepup/begin": {
      ...
      if (oc === null) {
        // Fired as the LAST action before return, the same shape that can otherwise lose the passkey ceremony
        ...
        const w = recordAuthSignalEdge(scheduler, "passkey-not-configured"); // no CONSOLE_ORIGIN -> every step-up 501s
        if (runtime?.waitUntil) runtime.waitUntil(w);
        else void w;
    ...
    case "POST /stepup/finish": {
      ...
      // OBSERVE: "the console keeps refusing my sensitive action". A failed step-up leaves no trace
      // anywhere without this, so support cannot tell a broken step-up from a user who never attempted one. The DO body is
