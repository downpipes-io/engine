...
// destCredHash is a fingerprint over the
...
// verifyControlPlaneSignatureDetailed returns the closed verdict verifyControlPlaneSignature discards.
// TOTAL: it never throws, and it returns the closed HybridVerifyVerdict.
//
// WHY THIS EXISTS. verifyControlPlaneSignature returns a bare `false` for four completely different worlds,
// and a caller that renders ONE refusal for all of them -- on the disaster-recovery path, where the customer
// is rebuilding a wiped estate -- gives remedies that are the OPPOSITE of each other:
//
...
//
// A pure length check on the SIGNATURE STRING cannot see the key at all, so it reports a right-length
// CORRUPT signer.pub as tamper ("the export was modified") when the truth is "your kit file is damaged, take
// another copy" -- this closed verdict keeps the four causes distinguishable so the manual recovery routes
...
export function parseControlPlaneArtefactVersion(key: string): number | null {
  ...
  // A SEALED generation (`…-<iso>.sealed.json`) also ends with `.json`, but it is NOT a plaintext
...
export interface StagedControlPlane {
  ...
  // Resume-apply OUTCOME diagnostics. When the tolerant auto-heal resume applies the slice, it records
...
// support pack can surface WHY the auto-heal refused (owner-#1 "did my control plane recover?") without the
...
export const AUTO_HEAL_REFUSAL_CODES = [
  ...
  // `resume-failed` is deliberately NOT a member. A refusal code recorded here LATCHES the auto-heal off --
  // the health pass reads `if (status.refused !== null) return;` and steps back to the manual break-glass
  // path -- and a resume slice that did not apply is deliberately RETRIED on the next tick, precisely so a
  // transient DO fault cannot strand a customer's recovery in manual. Writing this code would CHANGE RECOVERY
  // BEHAVIOUR (one blip = no more auto-heal, ever), so no site writes it. The fact itself is not lost: the
  // resume refusal is projected to its own closed class at the site that holds it (cron-fault-ledger
  // noteResumeApplyRefusal) and rides in cronHealth, where it does not disarm anything.
] as const;
...
export interface ControlPlaneRecoveryRecord {
  ...
  // subCause is the closed WHY behind the code, and scan is what the candidate pass actually saw in the
...
// a brand-new account) are indistinguishable without this enum: a support engineer could not otherwise tell
// "healthy new account" from "amnesia the latch could not confirm". Redaction-safe (a closed enum).
...
// ControlPlaneDeployObservation is the DETERMINISTIC per-tick deploy-identity record. The cron
...
// ControlPlaneExportHealth is the DO-stored outcome of the last control-plane export pass. It records whether the pass wrote a
...
export interface ControlPlaneExportHealth {
  ...
  // `reason` is the CLOSED cause a destination refused the recovery artefact (only on ok:false). Without
