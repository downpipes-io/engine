...
// The god-module split (MOVE-ONLY): handleAdmin (the central method+path dispatch) stays here; the
...
import {
  ...
  rateLimited,
  requireStepUp,
  STEPUP_SUBS, logAuthCeremonySuccess } from "./router-core.ts";
...
import { handleTestFault } from "./router-test-fault.ts";
import { handleUpdates } from "./router-updates.ts";
import { originFailClass, readSessionCookie, sessionSetCookie } from "./session.ts";
...
//
// OWN-SURFACE DEVIATIONS (§12): the /admin surface deliberately does NOT carry a `/v0/`
...
// HERE IS NOT EVIDENCE A ROUTE DOES NOT EXIST. That is not a caveat for tidiness: a route such as POST
// /admin/keys/add-operational can be missing from this list while the removal route below sits in it
// labelled "irreversible", so a statement built on this list's completeness inherits a false premise
// rather than a wrong sentence.
//
// COVERAGE, MEASURED RATHER THAN CHARACTERISED: 51 CATALOGUED OF 172 DISPATCHED. Both numbers are asserted
...
// describes. A prose fraction such as "roughly a third" is the one form of this claim that can never be
// wrong and never be checked, which is exactly why the count here is measured, not characterised. A further
// 5 lines below carry routes that are real but are NOT `case` labels (the pre-switch surfaces: the health
// probe, the two /auth/* recovery routes, and the dynamic-id config-change approve/reject pair); those are
// declared in that validator, so a catalogue line naming a route the dispatch does not have is red too.
...
//   POST /admin/keys/break-glass-only  remove BOTH operational worker secrets (Owner; keys.ceremony). The deleted KEY MATERIAL is unrecoverable and archives sealed to it never regain an operational recipient (the recipient set is baked in at seal time and is never rewrapped), but the POSTURE is reversible: POST /admin/keys/add-operational above installs a fresh pair and clears the OPERATIONAL_RETIRED marker. GUARDED: refuses with 409 {discardGuard} when removing the operational key would strand an archive it alone opens (or when the impact cannot be fully determined), unless confirmDiscardStranded is set (the deliberate, warned path)
...
//   POST /admin/auth/recovery-codes/confirm  promote a STAGED recovery-code set (minted on a self-add enrolment over an existing live set) to live, the actual invalidation of the old set; a no-op when nothing is staged (authenticated inside; self-service)
...
export async function handleAdmin(req: Request, env: Env, runtime?: AdminRuntime, slide?: { cookie: string | null }): Promise<Response> {
  ...
  if (url.pathname.startsWith("/admin/auth/")) {
    // THE HUB'S REFUSAL RECORDER, HOISTED OVER THE PRE-AUTH EDGE. The frozen route table holds
  ...
  // HARNESS-ONLY test-fault surface (/admin/test-fault/*): it EXISTS only when HARNESS_TEST_FAULTS is set (a
  // harness estate). In PRODUCTION (and dev/uat/internal) it 404s HERE, before any auth or DO round-trip,
  // exactly as the demo reset above, so no fault surface exists to find or invoke. When the flag IS set the
  // request falls through to authorise() and the Owner-gated handleTestFault spoke below (which re-checks the
  // flag and the Owner role). The four injection seams are separately gated at the point of consumption, so a
  // no-flag engine is byte-identical whether or not this file changed.
  if (url.pathname.startsWith("/admin/test-fault/") && !envFlagEnabled(env.HARNESS_TEST_FAULTS)) {
    return new Response("not found", { status: 404 });
  }
  // Authorise ONCE and carry the identity, instead of collapsing to a boolean. Precedence is
  ...
  // Record WHY authorise() denied, as a closed signal in the DO's bounded auth-signal aggregate, so the
  ...
  //
  // The write is CHECKED, not a bare .catch(() => {}): during a DO outage a bare catch would drop every
  // auth-deny signal SILENTLY and the pack would show a QUIET authSignals aggregate for the exact window the
  // customer reports as a total lockout -- absence of evidence reading as absence of problems.
  ...
  // The twin of onAuthDeny for the ADMITTED path. A verified Access assertion whose group claim is
  // bounded away signs the user IN -- with the wrong role -- and without this sink records nothing at all, so
  // "present but dropped" would be indistinguishable from "the IdP sent nothing". The sink bumps a CLOSED
  ...
  // throttled approvals lookup ended an operator's session in the middle of a restore, and the harness banked
  // rate limits as auth failures. Retry-After makes the difference actionable rather than merely visible.
  //
  ...
  if (!verdict.ok) {
    ...
    // There is DELIBERATELY no counter here. A caller presenting NO credential is not a signal that matches
    // no deny class and leaves no durable trace: authorise() records admin-token-denied-empty-bearer for
    // exactly that caller (auth.ts), and it is a pack-visible member of this vocabulary. Every other refusal
    // -- a retired token replayed, an expired cookie, a stale automation bearer, an emailless assertion, a
    // rate-limited sweep -- ALREADY has its own discriminating row. A counter firing unconditionally on every
    // !verdict.ok would be the arithmetic SUM of those rows: it would separate no pair of states.
    //
    // Worse, it would fire on LEGITIMATE states. Every 12-hour passkey session that expires, every signed-out
    // browser that touches an admin route, every unconfigured automation trips it, so on a healthy tenant it
    // would be permanently and monotonically non-zero. A reviewer asking "were we probed last Tuesday?" could
    // not tell a probe from a dozen sessions timing out: same count, same day-ring shape. A false signal on a
    // hot path is worse than no signal, because it costs a support engineer the time an honest silence would not.
    return new Response("unauthorised", { status: 401 });
  }
  // SECURITY, defence in depth: an authenticated EMAIL-BEARING caller (Access OR passkey) must
  ...
  if (verdict.method !== "token" && !canonicalEmail(verdict.email)) {
    ...
    logAuthFailure(verdict.method, "session verified but carried no usable email");
    return new Response("unauthorised", { status: 401 });
  }
  ...
  // POST /admin/session/bootstrap (ASVS V7.2.2): the ONE route that takes the bare ADMIN_TOKEN bearer and hands
  // back a dynamically minted session, so the static secret is presented once and the console discards it.
  // It refuses anything that is not the bare bearer: a cookie-borne token session cannot be re-exchanged, and
  // no other method has a secret to exchange. The DO refuses the mint while the token is retired in-app.
  if (req.method === "POST" && url.pathname === "/admin/session/bootstrap") {
    if (verdict.method !== "token" || verdict.viaCookie === true) {
      logAuthFailure(verdict.method, "session bootstrap needs the bare ADMIN_TOKEN bearer");
      return new Response(JSON.stringify({ error: "the session exchange takes the bare ADMIN_TOKEN bearer only" }), { status: 401, headers: { "content-type": "application/json" } });
    }
    const issued = await scheduler.fetch(doURL("/passkey/session/issue"), {
      method: "POST",
      body: JSON.stringify({ method: "token", priorToken: readSessionCookie(req), client: { sourceIp: req.headers.get("CF-Connecting-IP"), userAgent: req.headers.get("user-agent") } }),
      headers: { "content-type": "application/json" },
    });
    const out = (await issued.json().catch(() => null)) as { ok?: unknown; token?: unknown } | null;
    if (out?.ok !== true || typeof out.token !== "string") {
      logAuthFailure("token", "session bootstrap refused: the token is retired or the mint failed");
      return new Response(JSON.stringify({ error: "the bootstrap token cannot be exchanged for a session (retired, or the scheduler refused the mint)" }), { status: 401, headers: { "content-type": "application/json" } });
    }
    logAuthCeremonySuccess("token", "session/bootstrap");
    return new Response(JSON.stringify({ ok: true, method: "token" }), { status: 200, headers: { "content-type": "application/json", "set-cookie": sessionSetCookie(out.token) } });
  }
  // Resolve the caller's role ONCE. The token fallback is the documented break-glass and
  ...
  // resolution rather than after it: the role resolution is not a pure read. On a fresh Access-fenced
  ...
  // RECORD WHICH CREDENTIAL PATH THIS REQUEST AUTHENTICATED ON. The engine classifies it
  // (caller.method is the closed access | passkey | oidc | saml | token set). Without recording it,
  // "was this estate behind Access when the change was made, and how long did it run on the shared break-glass
  // token?" is unanswerable from the pack: the only disclosure would be an amber console chip that dies with
  // the browser tab, and the audit excerpt would carry no authn events at all.
  //
  ...
  if ((isCookieBorneMethod(caller.method) || verdict.viaCookie === true) && req.method !== "GET" && req.method !== "OPTIONS") {
    // Classify WHICH of the three branches refused, not merely THAT one did. originFailClass returns
    ...
    if (originFault !== null) {
      ...
      // Record the bounded auth-signal so a "console mutations are being 403'd" is diagnosable from the pack.
      // Fire-and-forget: a diagnostic write must never delay or fail the 403; the DO drops any out-of-vocabulary
      // name. The write is CHECKED, so a dropped signal is counted in droppedWrites rather than leaving the pack looking
      // like nobody's console mutations were being 403'd at all.
      //
      // The name is the CLOSED SUB-CLASS, because "every console save 403s" has two opposite fixes.
  ...
  // AuditAction/AuditTarget union carries both the `authn-failure` action and the `authn-attempt` target, so
  // the reason a router-side SUCCESS is a log line rather than a chain row is not a union limitation: it
  // fires on EVERY authenticated request that resolves a caller, and a row per resolved caller would bury
  // the privileged mutations the chain exists to hold. The FAILURE side is a first-class audit event, written
  // at the ceremony that refused it inside the DO, not here. No token, no email and no session value
  ...
  // The header's own FAULT, classified at the only place it can be. A change reference that arrives
  ...
  // bare token only and judges an Access caller on the assertion's issued-at (ASVS V7.5.1); a caller lacking
  // the route's capability still hits its own 403 on retry.
  if (req.method === "POST" && STEPUP_SUBS.has(sub)) {
    // V7.5.2: the two self-scoped session endings accept a fresh sign-in of any method as the second factor
    // (they reach no session but the caller's own); every other gated route holds the method-aware rule.
    const purpose = sub === "/sessions/terminate-others" || sub === "/sessions/terminate" ? "end-own-session" : undefined;
    const stepUp = await requireStepUp(req, scheduler, caller, runtime, purpose);
  ...
  // This is the same router-gates-coarse / DO-enforces-specific split as the restore approve path.
  {
    ...
    if (changeAction !== null) {
      const denied = gate(caller, "downpipe.read");
      if (denied) return denied;
      // STEP-UP (ASVS V7.5.1): the <id> here is a per-request ULID path segment, so this route can
  ...
  // enforces the specific dual-control logic. A mutating POST, so it is rate-limited per caller after the gate.
  {
    ...
    if (ownerAction !== null) {
      // THE OWNER GATE. This block RETURNS BEFORE the dispatch loop below (its path carries a dynamic
      ...
      // STEP-UP (ASVS V7.5.1): same structural reasoning as the config-change block above - the <id>
  ...
  // POST-AUTH DISPATCH (the god-module split): the central method+path switch was MOVED, body-for-body,
  // into the route-group spokes imported above; the
  ...
  const dispatch = [
    handleIdentity,
    // A one-route spoke (see its own file header for why it was not grown into handleIdentity above, which
    // sits at the line-budget ceiling).
    handleControlPlaneRecoveryAck,
    // Another one-route spoke, same reason (see its own file header) -- GET /control-plane/status
    ...
    // HARNESS-ONLY: returns null unless HARNESS_TEST_FAULTS is set AND the path is /test-fault/*. In
    // production the prefix already 404d at the pre-auth edge above, so this never matches there.
    handleTestFault,
    handleUpdates,
    handleStatus,
  ...
  for (const handle of dispatch) {
    ...
    if (resp !== null) {
      // THE ONE SEAM EVERY ADMIN WRITE'S ANSWER PASSES THROUGH. Recorded here, at the hub, rather than at
