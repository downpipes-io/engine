...
// scheduler is OPTIONAL: when supplied (the real router-restore.ts /restore/verify caller always has
...
export async function runBlindRestoreTest(env: Env, body: RestoreRequest, destOverride?: RuntimeDestConfig | null, scheduler?: DurableObjectStub): Promise<BlindRestoreTest> {
  ...
  try {
    ...
    try {
      ...
      // A run holding no records (an empty source, never a selector emptied by the guarded
...
// -> scheduler-do-scheduling.ts schedulerSignals) as the out-of-band anti-rollback pin: it is
...
// scheduler is OPTIONAL: when supplied (the real router-restore.ts caller always has one in scope),
