...
// cron classifies into, so the pack's gate and the classifier cannot drift apart.
...
//
// The last attempt alone cannot tell an INTERMITTENT beacon from a dead one -- "ok, fail, ok, fail" and
...
// "internal-error" is LOAD-BEARING: without it, both licence backstop catches report an ENGINE regression as
// "body-malformed", i.e. as a CORRUPT CUSTOMER TOKEN, sending support to chase a token that was always fine.
// Omitting it here would silently DROP the code at the pack boundary -- the exact out-of-vocabulary drop this
// gate exists to make visible -- and the fix would regress invisibly. This set and the DO's own
// LICENCE_REFUSAL_REASON_CODES must both carry "internal-error", or the drop happens one layer earlier and
// costs the bounded refusal ring its entry too. Both sets also carry "supersedes-current-term" (the
// term-regression refusal on POST /admin/licence: a signed, unexpired token whose term ends earlier than the
// active licence). Neither is a verify outcome, so neither is a member of the LicenceReasonCode union, exactly
// like "malformed-token".
...
export async function fetchConfigIntegrity(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  ...
  // unavailableProbes: each sub-read below is independent + best-effort, and a FAULTED sub-read would otherwise
  ...
  // only above zero -- so a corrupt marker answering 200 would land in the same silence as a clean account. The
  // DO says `coerced` on the wire, and those land here too, which is the whole point of this list.
  ...
  try {
    ...
    //
    // WHY THIS MATTERS, IN TWO PARTS. `CONFIG_HISTORY_CAP` is 2,000 and the rollover that enforces it is
    // CORRECT: it prunes, and the chain still verifies afterwards. So `intact` stays true, no key is rotated,
    // no tail is deleted and no body is missing -- and the condition below would OMIT THE WHOLE BLOCK once an
    // estate is driven past the cap, even though the bundle before and after the rollover reports identical
    // section summaries over a materially different set of retained versions. Second, `earliestId` is already
    // in this payload, already computed by the verifier, so a reader who opens the block has something to
    // derive from only if this projection reads it.
    //
    // THE AUDIT CHAIN KEEPS THIS RULE. `verifyAudit` derives `rolledOver` from `earliestSeq > 1`
    // and `status.ts` carries `auditNearCap` and `auditRolledOverCount`, the latter documented as how many
    // entries the rollover has ALREADY DESTROYED. `configNearCap` and `configRolledOver` have ZERO occurrences
    // in `engine/src`, so the config chain is the one place this convention is not carried -- the lone hold-out,
    // on a chain whose entries are SIGNED VERSIONS a customer may ask to roll back to.
    //
    ...
    // empty while it has ever rolled over. A stored config-rollover record would still be the better answer and
    // is a DO change this projection cannot make; what it can do is stop the pack answering as though the
    ...
    // The BODY-RETENTION scan. The block above is about the chain's HEADERS; this is about whether the
    ...
    //
    // headTruncated IS THE ARM THAT MATTERS MOST. The keyed verify
    // recomputes over the RETAINED versions, so deleting the NEWEST ones leaves it reading intact:true -- and on
    // an otherwise-healthy estate every other arm of this condition is false, so without it the block would be
    // OMITTED ENTIRELY. A truncated config history would then produce a configIntegrity section with nothing in
    // it: the pack's own headline reads clean, and the latch in schedDiag.chainBreaks is the only thing in the
    // bundle that knows.
    if (v.intact !== true || v.signingKeyRotated === true || v.headTruncated === true || bodyGap > 0 || rolledOver) {
      out.history = {
        ...
        // the key set it allows names these three fields, and this is the narrower emit that satisfies it.
        ...
        // WHAT THE KEY-FREE EVIDENCE SAYS. The keyed verify stops at the first break, so a rotated or
        // deleted in-DO key (one owner button: terminate-all-sessions) fails version 1's digest and the block
        // would say signingKeyRotated with no further comment -- IDENTICAL whether the rest of the chain is
        // pristine or the attacker's. The content hashes, the parent links and the id contiguity need no key; unkeyedIntact
        ...
        // verifyFaulted: the chain could not be CHECKED at all, which is a different and worse statement
...
export async function fetchLicenceActivationRefusals(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  try {
    ...
    return {
      ...
      // An out-of-vocabulary reason code does not vanish, leaving "N activations were refused" with no
    ...
    // can tell "no activation was ever refused" from "could not read the refusal tally".
...
export async function fetchLicenceExpiryTracker(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  try {
    ...
    // A read fault is NOT honest absence (no drift). Flag it so a diagnoser can distinguish the two.
...
// fetchExpiryRegistry projects the WHOLE credential-expiry registry into the pack. status.expiryWarnings
...
export async function fetchSourcesDetached(env: Env, scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  try {
    ...
    // The plan's OMISSIONS, which no reader is otherwise shown. A CONFLICTING binding name (two
    ...
    // NONE of the plan's lists at all -- "downpipe X is silently omitted from the heal plan". Both leave a
    ...
    // Flag it so a diagnoser does not read "no sources are detached" when the computation could not run.
...
// fetchOwnerActionQueue projects the dual-control owner-action queue AGGREGATE. The configEvents
...
// waiting for a second owner" is otherwise undiagnosable. This reads the caller-independent DO aggregate: how many
...
// destination ids. It is the roster a downpipe's configured destinationId / destinationIds is
// cross-checked against to detect a DANGLING reference (a pin to a destination since removed, which fails the
// run loudly but is otherwise never counted or attributed in the pack). Ids only (server-minted, the same class the
...
export async function fetchRosterIntegrity(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  try {
    ...
    // The IN-RECORD truncation markers. The DO bounds its own lists, and a bounded list that does not
    // SAY it is bounded is an evidence bug: `ghosts` reads as the complete set of undeletable rows when it is
    ...
    // A read fault is NOT honest absence (a clean roster). Flag it so the two are distinguishable.
...
// delete probe was permitted. Re-gated at the pack boundary so an out-of-vocabulary value is DROPPED.
const OBJECT_LOCK_STATES: ReadonlySet<string> = new Set(["enforced", "not-enforced", "unknown"]);
// The closed chain-break cause vocabulary, re-gated at the pack boundary: the config-history block carries
...
// DEST_CONFIG_ANOMALY_SET re-gates the env-derived destination-config anomalies at the pack boundary.
...
export async function fetchDestResolution(env: Env, scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  ...
  // destinationsUnavailable: this section is MIXED-source -- the effectiveRatePerSec/rateKnobInvalid
  ...
  // from an account with no destinations). Keep the env-derived rate knobs but flag the DO-read fault
    ...
    destinations = (Array.isArray(j.destinations) ? j.destinations : []).slice(0, 32).flatMap((d) => {
      ...
      try {
        ...
        return [{
          ...
          // rather than a bare `typeof === "string"` check: an OPERATOR-CONTROLLED field must not ride into
          // the confidential pack ungated, so a value stored before the validator existed (or by a drifted
          // writer) is never carried verbatim. It is a CLOSED ENUM like every other projected field: a member
          // rides, anything else collapses to UNKNOWN_CODE -- which is itself the diagnosis (an unrecognised
          // class means the write path is silently falling back to STANDARD).
          ...(gateClosed(d.storageClass, STORAGE_CLASS_SET) !== undefined ? { storageClass: gateClosed(d.storageClass, STORAGE_CLASS_SET) } : {}),
          // PER-DESTINATION immutability posture. The engine persists and returns all three on
          // every destination row, so the dangerous immutability SHADOW -- a WORM
          // policy CONFIGURED against a bucket that does not actually ENFORCE Object-Lock -- is visible for
          // every destination, not only the DEFAULT one (which also carries the single live wormPosture probe).
          // wormPolicyConfigured is presence-only (the policy's mode/days stay off this
  ...
  // configAnomalies: every destination-config INTENT the engine silently discards, derived purely from
  ...
  // unsupported storage class and an invalid addressing style all fall back in silence. Closed enum
...
export async function buildKeysHealth(env: Env): Promise<Record<string, unknown>> {
  ...
  // buildKeyMaterialHealth widens the malformed-key coverage from the 3 slots above to the SIGNER plus
...
// ---------------------------------------------------------------------------------------------------------
// SOURCE-DISCOVERY HEALTH -- the setup form's own honesty.
//
...
// cap cuts them off. The customer configures their backup estate against whichever one it was, and a scope-gap
// or a truncation means the resources they most needed to protect are never even offered. Without this
// section, the empty form and the empty account read as the same observation remotely.
//
// This projects the recorded verdicts. Read it as: lastOutcome names each product's CURRENT verdict; a
// "denied" or "truncated" is a form that lies by omission; emptyObservations is how many times the console
...
// listings themselves (buckets, namespaces, databases, secret names, zones, account ids and names) are
// classified at the fault site and never leave it, and every key/value here is re-gated against its closed set
...
// The token-set vocabularies, bounding the projection to exactly what the recorder can write.
...
export async function fetchDiscoveryHealth(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  ...
  // THE TOKEN-SET VERDICT IS GATHERED BEFORE THE h === null EARLY RETURN BELOW, WHICH IS THE
  // WHOLE POINT. A refused token means no discover EVER RAN on this engine -- that is what "Verify and save
  // always fails" means -- so the discovery record is null, and without this the section would return {} and
  // the pack would say nothing at all about the most common onboarding ticket. The one fact support needs is
  ...
  // The ATTACH record is gathered even when discovery has NEVER run -- an engine whose bindings were
  ...
  // The per-account tallies, re-gated a THIRD time on the way into the sealed pack (closed product key,
  ...
  return {
    ...
    // attach: the ATTACH / RE-ATTACH record rides inside this same section, because it answers the
    ...
    // lastTokenSet: the verdict on the token the whole form depends on. Absent when no token was ever
...
// fetchAttachHealth projects the attach / re-attach record. It answers two tickets the pack otherwise cannot
// touch:
//
...
//        what was intended. That alarm otherwise goes only into an HTTP response.
//   "reattach-missing keeps failing after the deploy wiped bindings" -> faults.reattach with no matching
//        successes.reattach. This is the owner's stated #1 fear WITH ITS HEAL BROKEN, and it otherwise records
//        nothing.
//
...
// account id, the script name, the Cloudflare message and the binding names are classified at the fault site
// and never leave it; every key/value is re-gated against its closed set here too (defence in depth).
async function fetchAttachHealth(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  ...
  // refusals: the bounded, append-only REFUSAL HISTORY. This is the half of the attach story the coarse
