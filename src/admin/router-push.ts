// router-push.ts -- the outbound SIEM audit-log push destination admin routes:
// GET /push (the redacted view), POST /push (set/replace, owner + dual-control), POST /push/delete (owner,
...
// happen in the DO, by design), sharing the exact shape + send path the cron drain uses.
...
import { canonicaliseBareHost, isAllowedWebhookUrl, isInternalSinkHost } from "../notify.ts";
...
// fireInBackground mirrors the identical helper in router-identity.ts.
...
function validatePushSetShape(body: {
  ...
  // before the submission can be queued as a pending dual-control approval: without this check, a mismatched
  // combination would validate and ship CEF under whatever format was chosen.
  ...
  if (sink === "s3") {
    ...
    // Addressing and storage class are forwarded as bare strings and the DO then runs validateAddressing /
    // validateStorageClass, both of which drop an unrecognised value; without a refusal here the push
    // destination would save 200 addressed and tiered differently from what the operator chose. The
    // destination surface carries the refusal twin for exactly these two fields (router-destinations.ts);
    // this is the same pair on the audit-export surface, with the same rules, read from the same
    // addressingRejected / validateStorageClass functions, so the two surfaces cannot drift apart.
  ...
  if (sink === "syslog-tls") {
    ...
    // SSRF screen at the config boundary. The s3 sink's endpoint goes through isAllowedWebhookUrl above and
    // the http sink's through the same call below, both of which screen the host. The host here is a bare
    // hostname, not a URL, so the classifier is called directly on it (isAllowedWebhookUrl would reject a
    // non-URL outright); this stops a host such as a cloud metadata address from being stored and later
    // reached by notify/siem-syslog-sender.ts opening a TLS socket to it. The sender re-screens at send time,
    // so a record stored before this screen existed is still refused.
    if (isInternalSinkHost(canonicaliseBareHost(host))) {
      return { ok: false, response: jsonError("push destination syslog host resolves to a private/loopback/link-local address (incl. cloud metadata); these are refused. This engine runs at the Cloudflare edge and cannot reach a host on your private network.", 400) };
    }
    // ABSENT means the 6514 default. A PRESENT port outside 1..65535, or a fractional one, is REFUSED rather
    // than replaced by 6514: silently substituting the default would tell the operator the push destination
    // was saved while their audit stream went to a port they never chose, which on an audit-export surface is
    // a silently misdirected security log.
  ...
  // boolean is REFUSED rather than coerced: coercing would accept the JSON string "true" as truthy and store
  // it as the OPPOSITE of what the caller asked for, byte-identical in status and message to a body that
  // never mentioned the flag. A silently inverted boolean is not a truncated string: it is a value the caller
  // believes they set, and this flag decides WHERE THE CREDENTIAL RIDES -- spliced into the endpoint URL, or
  // sent as a header -- and it relaxes the forbidden-header-name check below. Nothing legitimate sends a
  // non-boolean here; the console's push form emits a real boolean from a checkbox.
  ...
  // Screen the auth header VALUE at config time: an absent/empty value is the keep-secret signal (unchanged),
  // but a SUPPLIED value carrying a control char (CR/LF header injection) or exceeding the length cap is
  // rejected with a clear 400 here -- BEFORE it is sealed -- rather than surfacing later as an opaque
...
export async function handlePush(ctx: RouterCtx): Promise<Response | null> {
  ...
  switch (`${req.method} ${sub}`) {
    ...
    // The redaction above is a claim about the BODY only; the caller is checked here instead. The forward
    // carries no callerHeaders, so the DO's getSiemPushView() has no principal and the router is the only
    // place a caller check can happen. posture.read is the floor this data already sits behind:
    // support-sections-push.ts fetches doURL("/push") into the posture.read-gated support pack, so gating at
    // that same floor refuses no reachable principal while keeping "any authenticated role" a checked
    // guarantee.
    case "GET /push": {
    ...
    case "POST /push": {
      ...
      if (!validated.ok) {
        // Recording this closed (surface, reason) pair means a support pack shows that a submission was
        // rejected and why, rather than holding no record that the customer had even tried. Never the
        // submitted endpoint, header name or secret.
    ...
    case "POST /push/test": {
      ...
      // noteTestOutcome records the STATUS CLASS so a push-test failure's diagnosis outlives the browser tab
