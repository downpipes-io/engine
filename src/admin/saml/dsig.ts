...
// The attack-shape ledger. dsig is where the two WRAPPING shapes that need a signature to be interesting
// are refused -- a duplicate ID the Reference resolves to twice, and a second ds:Signature -- and both would
// otherwise coarsen into the same `signature` counter as an ordinary cert rotation.
...
export interface VerifyXmlSignatureOptions {
  ...
  // REJECT a signature made under a cert that is outside its notBefore..notAfter window (a freshness
...
export async function verifyXmlSignature(
  ...
  // 7. SIGNING-CERT VALIDITY WINDOW. Only reached for an otherwise-VALID signature (so the window
...
// profile). It captures whether the c14n keeps comments and the InclusiveNamespaces PrefixList the
...
function walkTransforms(
  ...
  // The InclusiveNamespaces PrefixList carried on the exc-c14n# Transform. Captured from the c14n
...
async function verifyReferenceDigest(
  ...
  const digestOpts: C14nOptions = {
    ...
    // The InclusiveNamespaces PrefixList: force-render the listed in-scope ancestor prefixes on the
