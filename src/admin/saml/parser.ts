...
// recorded as its own distinguishable signal rather than coarsened into one indistinguishable `malformed`
// counter.
...
function parseElement(
  ...
  for (;;) {
    ...
    if (ch === 0x3c) {
      ...
      if (n1 === 0x21) {
        ...
        if (st.text.startsWith("<![CDATA[", st.pos)) {
          ...
          // `&<![CDATA[lt;]]>` is not well-formed XML even though the merged bytes read '&lt;'). This is
          ...
          // on its own. (Re-validating a slice of pendingText itself at this point would instead reintroduce
          // O(n^2) cost on many chained CDATA sections, because slicing a string that is also repeatedly grown
          // by "+=" forces a full re-flatten of the accumulated-so-far run at every boundary. Validating the
          // span against st.text - which is never mutated - avoids that entirely.)
    ...
    // afterwards. Checking each span once against st.text, instead of re-slicing the ever-growing
...
export function parseXml(input: string | Uint8Array, limits?: ParseLimits): ParseResult {
  ...
  if (byteLen > maxBytes) {
    // A document over the byte ceiling is a RESOURCE shape, not a syntax one: without this signal, a
    // resource-exhaustion probe would be indistinguishable from a genuinely enormous assertion, since both
    // land in `malformed`.
  ...
  if (!gate.ok) {
    ...
    // billion-laughs probe, distinct from a pretty-printer glitch (which would otherwise also land under
    // `malformed`). This is the single most unambiguous attack signal the SAML surface can produce.
  ...
  if (!rootRes.ok) {
    // The depth / node ceilings are the parser's resource guards. Separate them from a syntax error, for
  ...
  if (st.pos < st.text.length) {
    ...
    if (st.text.charCodeAt(st.pos) === 0x3c) {
      ...
      // recorded here, at the site, where no ordering rule can turn it into a formatting glitch.
