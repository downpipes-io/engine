...
// The SAML signal ledger. Every branch below reports SUCCESS to the caller while quietly dropping something
// the operator configured -- which is precisely why no failure record exists for any of them, and why "my
// email-bound role never applies after SSO" is otherwise undiagnosable.
...
export interface SamlPrincipal {
  ...
  groups: string[];
  nameId: string;
  // claimDrops: the CLOSED counter names of the ADVISORY claims this assertion asserted and the engine
...
// this Status==Success precondition (a deliberate defence-in-depth control). It decodes the
...
// SAML_STATUS_CLASSES is the CLOSED coarse class of a non-Success samlp:StatusCode. The StatusCode is the
// one field in the whole Response that says WHY the IdP refused; without reading it, a declined
...
export function consumeAssertion(assertion: XmlElement, conn: SamlConnection, ctx: AssertionContext): ConsumeAssertionResult {
  ...
  // The bounded tally the advisory bounder fills as it drops. Closed counter names only.
  ...
  if (authnStmt !== null) {
    ...
    // The attribute is OPTIONAL, so ABSENT is legitimately "no bound". But PRESENT-and-unparseable would
    // otherwise be read as the same thing -- and the consequence is a security regression that reports success:
    // the native session is no longer capped at the IdP's session end, so it OUTLIVES it. Absent stays silent
    // (it is not a fault); present-but-unreadable is recorded. The attribute VALUE never rides, only the fact.
    ...
    // RECORD THE DROP WHERE THE FACT IS ESTABLISHED. parseSamlInstantMs folds an unreadable instant to null,
    // and null reaches buildAuthContext as `undefined` -- the SAME input as an IdP that asserts no AuthnInstant
    // at all -- so buildAuthContext can only ever see ABSENT, and without this presence test the one state
    // claim-drop-saml-auth-time is named for ("an AuthnInstant was asserted and would not parse") would record
    // nothing. The only thing that could reach the member is a NEGATIVE-epoch instant, which is a different
    // fact. The presence test belongs here, where the raw attribute is still in hand. The attribute VALUE never
    // rides, only the closed counter name.
    if (authnInstantRaw !== null && authnInstantMs === null) claimDrops.add(claimDropCounter("saml", "auth-time"));
    // Without a tally passed to buildAuthContext, an over-length AuthnContextClassRef or an AuthnInstant that
    // would not parse is dropped in silence and reads exactly like an IdP asserting neither -- and "prove this
    // session was MFA'd / when did they actually authenticate?" has no honest answer either way. The boundary is
    // passed explicitly ("saml"), because this constructor is shared with the OIDC path and defaulting it would
    // file the drop under a boundary this assertion never crossed. The dropped VALUE never rides.
...
function confirmBearer(
  ...
  // WHICH binding blocked the last examined bearer confirmation. The refusal below is a SEVEN-WAY BLEND
  ...
  // @InResponseTo mismatch is a genuine replay/injection signal. The distinction is made HERE, at the site that
  // knows, so the three fixes stay distinguishable rather than collapsing into one message. Track the LAST
  // blocker (a closed token, not a value) and carry it as the
    ...
    if (canon !== null) {
      if (ctx.emailVerifiedPolicy === "trust-idp") {
        ...
        if (verified) {
          ...
          // A GOOD email is being DROPPED here and the sign-in still succeeds, so nothing anywhere records
...
function collectAttributes(assertion: XmlElement, inAssertion: Map<string, string>): Map<string, string[]> {
  ...
  for (const stmt of findChildrenInSamlNs(assertion, "AttributeStatement", inAssertion)) {
    ...
    for (const attrEl of findChildrenInSamlNs(stmt, "Attribute", inStmt)) {
      ...
      // The SAME Attribute @Name asserted MORE THAN ONCE. A conforming IdP emits each attribute once; a
