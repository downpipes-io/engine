...
// The SSO START-path signal ledger. recordSsoFail is written ONLY by the ACS (callback) wrapper, so a
...
export async function buildRedirectUrl(conn: SamlConnection, p: RedirectParams): Promise<string> {
  // The stored idpSsoUrl is the start path's first killer, so it is checked FIRST -- fail fast, before any
  ...
  // The DEFLATE leg. A CompressionStream fault (or a runtime without "deflate-raw") means NO redirect URL
