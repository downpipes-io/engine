...
//   - certValidity: a cert's notBefore..notAfter window (epoch ms) for the verify-time freshness check.
...
// pinned on a connection), so none of this reads a secret. Behaviour is byte-identical to the original
// response.ts: the code here was MOVED verbatim, not changed.
//
...
// verify path (dsig.ts) to REJECT a signature made under a cert outside its validity window. It is
