...
// The attack-shape ledger. Recorded AT the refusal site, which knows the shape; never inferred from the
...
export async function verifySamlResponse(
  ...
  if (assertions.length === 0) {
    // A DECLINED sign-in is exactly the case that carries no Assertion (the IdP refused the user, so it
    // minted none). Checking Status only after this point would leave every decline reporting the anonymous
    // "Response carries no saml:Assertion", filed under `malformed`, which reads as "your IdP is sending us
    // rubbish" when the truth is "your IdP declined this user" (an unassigned enterprise app, a
    // blocked account, an MFA the user would not complete). Check Status FIRST when there is no assertion, so
    // the decline is named. A Success response with no assertion keeps its exact original reason.
  ...
  if (assertions.length > 1) {
    // A second Assertion beside the signed one is the textbook XSW envelope. Recorded at the site (which
  ...
  // The refusal carries the CLOSED class of the IdP's own StatusCode (samlStatusClass: one of
  // responder / requester / authnfailed / nopassive / other, derived ONLY from the fixed spec URIs, never from
  // the free-prose StatusMessage). This is the single field that distinguishes "the IdP DECLINED this user"
  // (an unassigned enterprise app, a blocked account, a refused MFA) from an IdP-side outage. The mark is a
  // closed token; the client-facing redirect stays generic, so no detail is oracled back to the caller.
  if (!assertStatusSuccess(responseRoot)) return fail(`SAML Response status is not Success [saml-status:${samlStatusClass(responseRoot)}]`);
  ...
  // The health of the pinned set, computed from the values this loop ALREADY has rather than thrown away.
  // `parseReason` is overwritten by every bad PEM and discarded entirely the moment ANY cert parses, so a
  // corrupt certificate pasted during a rollover is silently skipped for weeks -- until the good one lapses and
  // every sign-in stops. These are counts, booleans and closed key classes ONLY: the PEM, the SPKI bytes, the
  // subject and the parse reason never cross into the ledger.
  ...
  noteIdpCertObservation({
    ...
    curves,
    nearestNotAfter,
    // An unreadable window makes dsig.ts skip the freshness check for that cert (fail-OPEN by design, and
    // correct -- a DER quirk must not lock out a working IdP). That it FIRED is the diagnostic nobody had.
    windowUnenforced: usableCerts.some((c) => c.validity === undefined),
    // Every pinned cert failed to parse: this connection cannot verify a single sign-in. SSO is DEAD, and this
    // is the only durable record that says so.
  ...
  if (verifiedAssertion === null) {
    ...
    for (const c of usableCerts) {
      ...
      // A signature that VERIFIED over the WRONG NODE. dsig returns ok, and the element it verified is
      // NOT the Response root we asked it to cover -- so a valid signature exists over some other subtree. The
      // guard is correct (we refuse), and without a record here the branch would be FULLY SWALLOWED: no reason
      // set and nothing recorded, so the single most alarming shape in the SAML surface would be statistically
      // invisible. It is recorded here, at the site that knows it, and the refusal itself is unchanged.
