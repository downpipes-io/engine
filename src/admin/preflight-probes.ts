...
import { DISCOVERY_TOKEN_MAX_DAYS } from "./discovery-health.ts";
...
// whole set up to PREFLIGHT_BINDING_CAP, so the discriminating tail is not thrown away.
const BINDING_DETAIL_CAP = 6;

// classifyProbeError coarsens ONE probe throw into a closed PreflightErrorClass. It reads the message
...
export function classifyProbeError(e: unknown): PreflightErrorClass {
  ...
  // misclassify a transport fault as a permission one.
...
export async function probeDurableObjectsAndCron(items: PreflightItem[], scheduler: DurableObjectStub): Promise<void> {
  ...
  try {
    ...
    items.push({
      ...
      required: true,
      status: "failed",
      // The truncated message is kept for the operator's screen; the CLOSED class beside it is what
      // survives into the pack, so "the DO is unreachable" does not depend on surviving as a prefix of an
      // 80-character truncated string.
...
export async function probeDestination(items: PreflightItem[], env: Env, scheduler: DurableObjectStub): Promise<void> {
  ...
  }
  {
    ...
    try {
      ...
      // credentialed store is not reported as a FALSE-GREEN "verified". A
      ...
      items.push({
        ...
        // The sanitised destination evidence is truncated at 80 chars, which cuts the S3 error code off
...
export async function probeKeys(items: PreflightItem[], env: Env): Promise<void> {
  ...
  // WHICH recipient key matters: BREAK_GLASS_PUBLIC and OPERATIONAL_PUBLIC have OPPOSITE remedies if either
  // fails to parse. A bad BREAK_GLASS_PUBLIC means the key ceremony did not land and NOTHING can be sealed; a
  // bad OPERATIONAL_PUBLIC leaves break-glass sealing perfectly healthy and silently disables the in-account
  // read-back drill. Each slot is parsed SEPARATELY (loadRecipientPublic, the same parser loadRecipients calls
  // per slot) and the FAILING slot is named as a closed enum. Break-glass is checked first because it is the
  // required one.
...
export async function probeSlicedRuns(items: PreflightItem[], env: Env): Promise<void> {
  ...
  if (ns && typeof ns.idFromName === "function") {
    ...
    items.push({
      ...
      required: true,
      status: "unconfigured",
      // A run over one invocation's budget dies at the invocation budget, and without this the failure has
      // nothing linking it back to the missing RUNSEAL migration. The binding-absent CLASS is that link: it
      // survives every evidence clamp, and it is the one preflight state in which such a run can NEVER complete.
...
export async function probeSourceBindings(items: PreflightItem[], env: Env, downpipes: DownpipeState[] | null): Promise<void> {
  if (downpipes === null) {
    ...
    if (needed.size === 0) {
      ...
      // The STRUCTURED list. The evidence line above still names six (it is a one-line summary), but
      ...
      // (64, matching the sourcesDetached cap), sorted, with the RESERVED-name collision flagged per binding
...
export async function probeSourceResources(items: PreflightItem[], env: Env, downpipes: DownpipeState[] | null): Promise<void> {
  ...
  // The UNPROVEN set is named here (sorted, capped) on every branch rather than folded to a bare count
  // ("(3 present but unproven)"), because an unprovable source is a source whose next backup may die deep in
  // the crawl -- naming which one matters even when the item is otherwise VERIFIED.
...
export async function probeApiSourceDiscoveryToken(items: PreflightItem[], env: Env, scheduler: DurableObjectStub, downpipes: DownpipeState[] | null): Promise<void> {
  ...
  // resolveDiscoveryTokenDetailed reports WHY the token is null, distinguishing a DO fault on the config read
  // from a genuinely-unset token. They are opposite tickets: one says "paste a token", the other
  // says "your scheduler is down and the token you already pasted is probably fine", so conflating them into
  // one failed item would misdirect the fix.
  const { token, doFault } = await resolveDiscoveryTokenDetailed(scheduler, env);
  if (token !== null) {
    // A token that resolves is also a token that must be term-limited. The route that stores a
    // console-set token already refuses one without an expiry; the DISCOVERY_API_TOKEN env fallback never
    // passed through that route, so the same verify runs here and a token with no expiry, or one further
    // out than DISCOVERY_TOKEN_MAX_DAYS, is a failed item naming the remedy. The value is never read into
    // evidence.
    const expiry = await readTokenExpiry(token, typeof env.CF_ACCOUNT_ID === "string" ? env.CF_ACCOUNT_ID : "", fetch);
    const expiryProblem = discoveryTokenExpiryVerdict(expiry.expiresOn, DISCOVERY_TOKEN_MAX_DAYS);
    if (expiryProblem !== null) {
      items.push({
        id: "api-source-discovery-token",
        name: "API source discovery token",
        requires: "the engine's read-only discovery token, term-limited to 90 days or less",
        required: false,
        status: "failed",
        tokenResolve: doFault ? "do-fault" : "resolved",
        probeErrorClass: "no-expiry",
        evidence: `the read-only discovery token resolves for the ${apiSources.length} configured API-based source(s) but is not term-limited: ${expiryProblem}`,
        remediation: `recreate the token with an Expiry of ${DISCOVERY_TOKEN_MAX_DAYS} days or less and paste the new value on the Sources screen (or redeploy DISCOVERY_API_TOKEN with it)`,
      });
      return;
    }
    items.push({
...
// before it becomes preflight evidence: the literal DEST_ENDPOINT / DEST_BUCKET
