...
// (incl. the self-redeploy-retry variants) plus the best-effort restore-proven stamp. The hub (router-core)
// imports callerHeaders from here and re-exports every symbol other files reference by name. This spoke
// imports only the shared helpers (router-helpers) and identity/audit types; it never imports router-core
// or router.ts (the de-cycled boundary is preserved).
...
export async function recordAudit(
  ...
  // The append's RESPONSE must be checked. A DO that ANSWERS a non-2xx (a route chain skewed by a
  // partial deploy, storage pressure) would otherwise drop an audit event in silence -- and configEvents, the
  // pack's "what CHANGED" excerpt, then reads as though the mutation never happened. Absence of evidence
  // reading as absence of change is the worst failure mode this pack has, so the LOSS is counted. The
  // parse below still surfaces a non-2xx to the caller unchanged.
  if (!resp.ok) {
...
//
// The runtime names a Durable Object reset in more than one form. A single rollout can log two different
// messages on two stubs inside the same request: one append gets Cloudflare's OPAQUE form ("internal
// error; reference = ...") and is misclassified as a real fault and abandoned on attempt one with NO
// retry, while another gets the named form ("Durable Object reset because its code was updated"). The
// second pattern below is not a guess about what the runtime might say: it is the string the runtime
// actually says on the request that would otherwise lose the row.
//
...
// counted and logged as such, which is why the exhaustion path exists.
//
// The opaque form is measured against a real runtime rollout fault: a keys-installed audit append after a
// secret write throws "internal error; reference = n3rmrlv4r241rk2ob8u84pfq" while the same rollout resets
// the Durable Object, so both message shapes are the one event and both must be ridden out.
...
// `kind` is the closed dropped-write kind to COUNT when the write is finally lost (a non-reset fault,
// an exhausted retry ladder, or an op that returned false). These writes run right after a self-redeploy --
// which is exactly the owner's #1 fear window ("did a deploy drop my source bindings?") -- so a silently lost
...
// attach/detach rewrites the Worker's own settings, and an engine-version promote/rollback/ramp
...
//
// WHY IT EXISTS. A POST /admin/keys/install can return 200 with all four secrets genuinely written and
// NO keys-installed row of any outcome reaching the chain, contiguous either side: the append never
// reached it. The cause is not a posture branch in the handler and not "the first append to a virgin
// chain" (a virgin chain takes a first install cleanly on a sibling estate). Each Cloudflare secret PUT
// rolls a NEW WORKER VERSION -- the versions API stamps one per secret, source "api", trigger "secret",
// inside the couple of seconds the install runs -- and updating a worker's script RESETS its Durable
// Objects. So the append that follows the PUTs races the rollout and can throw "Durable Object reset
// because its code was updated" on a stub the isolate is already holding. It is intermittent.
//
// The same remedy applies here as for the source-attach appends above, which redeploy the engine in
// exactly the same way. This binds it to the other half the key ceremony needs: recordAudit does
...
// proven by the attach or update-apply that just succeeded: both read
// /accounts/{a}/workers/scripts/{name} for the engine's own script name before
...
// transiently fail even though the version change already landed. Unlike its best-effort
...
export async function stampRestoreProven(
  ...
  // CHECKED best-effort. The stamp stays fail-open (a proof must never be turned into a failure by a
  // bookkeeping hiccup), but a LOST stamp must not be silent: unrecorded, it would make a downpipe that HAS
  // been proven restorable read as never-proven in the pack (status.restorabilityProven and
  // downpipes[].restoreProven both under-report), which is precisely the claim a customer's auditor asks about.
...
export async function stampRestoreTested(
  ...
  // CHECKED best-effort, same discipline as stampRestoreProven. A lost recency stamp would leave "Last
