// The acknowledge-only control-plane recovery latch clear is a one-route spoke split out on its own rather
// than grown into router-identity.ts (which sits at the file's line-budget ceiling): /control-plane/restore
// and /control-plane/apply-staged (router-identity.ts) both refuse forever once a latched plane organically
// un-empties, because both require an EMPTY plane AND an empty role table. This route is the narrow escape:
// gated on access.policy (never the bare break-glass token -- an owner who can authenticate at all means the
// role table is already non-empty, so there is always a real owner to ask), it forwards to the DO's
// acknowledgeControlPlaneRecovery (scheduler-do-control-plane.ts), which clears ONLY the latch fields, never
// imports, never touches bootstrapConsumed, and independently refuses over an empty role table -- see that
// method's own comment for the full rationale and the rejected alternatives (a dual-control reconcile-over-
// live route; an export-equality check; an unconditional clear, which risks a silent empty-table/spent-
// bootstrap deadlock).
//
// The refusal carries a CLOSED refusalClass, so the console codes the branch by set membership rather than
// classifying a sentence it does not own (never guess a class from prose), and the two refusals reach
// different stable codes on screen. The refusal is also RECORDED on its own surface, so a customer who
// cannot clear their own banner leaves a durable trace instead of a screenshot. The DO remains the
// enforcement point and re-asserts both guards itself.
