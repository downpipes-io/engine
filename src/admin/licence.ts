...
import { consumeLicenceTokenReadFault } from "./test-faults.ts";
...
// open-set feature strings). enterprise keeps its own 7 features. There is no legacy tier compatibility:
// an unrecognised tier string is never aliased to a known one.
...
export type LicenceReasonCode =
  ...
  | "future-tier"
  | "expired"
  // The BACKSTOP catches. An unexpected throw inside the licence path (a crypto import that fails, a DO
  // read that returns a shape the parser chokes on) is an ENGINE fault, not a token fault. Reporting it
  // as "body-malformed" -- the code for a CORRUPT TOKEN -- would send support chasing token corruption
  // when the token was never the problem. internal-error says plainly "this is OUR fault, not your
  // token": it is never returned for any token defect, only from a catch.
  | "internal-error";
...
export interface LicenceStatus {
  ...
  // one this licence is bound to: true when it is, false when it is not. This computes from
  // `claims.boundAccounts` (membership) when that list is present and non-empty; a token minted without
  // that field falls back to the ORIGINAL v1 check, `claims.account === engineAccountTag`. Computed ONLY when the
...
interface LicenceClaims {
  ...
  notAfter: string;
  features: string[];
  // boundAccounts (optional/additive): the Cloudflare account
...
function validClaims(v: unknown): v is LicenceClaims {
  ...
  // boundAccounts is OPTIONAL (a token minted without this field carries no such key at all): absent is
  // valid. When present it must be a string array, same shape discipline as
...
function checkExpiry(claims: LicenceClaims, source?: LicenceSource, engineAccountTag?: string): LicenceStatus {
  ...
  // never a block).
  //
  // A NON-EMPTY boundAccounts list is the authoritative check (membership, since a self-serve licence
  // can bind up to its band's estate count, not just one account); an ABSENT or EMPTY list falls back to
  // the original v1 single-account comparison, so a token minted without this field (or an
  // Enterprise/operator-mint token, which always carries an explicit `[account]` list and so never falls
  // back at all) verifies the same way.
...
export async function verifyLicenceToken(token: string | null | undefined, pinnedSignerPublic: string | undefined, source?: LicenceSource, engineAccountTag?: string): Promise<LicenceStatus> {
  try {
    ...
    // Any unexpected error fails open to community, never out of the handler. This is an ENGINE fault,
    // not a token fault, so it is classed internal-error and NEVER "body-malformed" (which would send support
    // hunting a corrupt token the customer does not have). The human `reason` stays the same either way.
...
export async function readLicence(env: Env, scheduler?: DurableObjectStub): Promise<LicenceStatus> {
  try {
    ...
    //
    // This read sits INSIDE the backstop: the "fail-open is absolute" contract covers every statement in
    // readLicence that could throw, this one included.
    ...
    if (scheduler) {
      ...
      try {
        // Test-harness only: a licence-token-read-fail fault, if armed, forces this read down
        // the SAME catch below a real DO hiccup takes, single-shot. Gated on testFaultsEnabled first inside
        // consumeLicenceTokenReadFault, so a production/dev/uat/internal engine never touches the fault store
        // and this line is a no-op false read.
        if (await consumeLicenceTokenReadFault(env, scheduler)) throw new Error("harness test-fault: licence-token read forced to fail (licence-token-read-fail)");
        const resp = await scheduler.fetch(doURL("/licence-token"), { method: "GET" });
        // A RESOLVED NON-2XX must not bypass this catch: the DO answering 500 during a storage
        // incident would otherwise silently flip a console-activated Enterprise licence to whatever the
        // deploy token holds (or to community) with doReadFellBack UNSET, and the pack would then show a
        // tier drop with no cause at all. A non-2xx is the same loss of the console token as a throw, and
        // is recorded as such.
      ...
      //
      // doReadFellBack is a LIVE-READ boolean -- it says the CURRENT read fell back, and the next
      // healthy read erases it, so an INTERMITTENT DO fault (the tier dropping to community for a few
      // minutes at a time) leaves no trace by the time anyone generates a pack. The counter is the durable
      // rate: any non-zero licence-source-flip means the effective licence source has been flipping under
      // the customer, however healthy it looks now. Best-effort and never awaited into the response (the
      // licence read is fail-open and must stay fast).
    ...
    // The route-facing backstop. An ENGINE fault, never a token fault (see the LicenceReasonCode note).
