...
import { ACK_OUTCOMES, DELIVERY_FAIL_CODES, RESOLVED_SINK_VERDICTS, SINK_SCREEN_VERDICTS, sanitiseEmailPlatformCode } from "../notify.ts";
import { doURL } from "../do-url.ts";
import { clampInt, clampTs } from "./support-shared.ts";

// InertReason is the CLOSED vocabulary for WHY a notification rule can never deliver. Naming it as a
...
// needs, derived from the SAME single fetch of the 1000-entry DO ring:
...
export async function fetchNotifyHistory(scheduler: DurableObjectStub): Promise<NotifyHistoryProjection> {
  {
    ...
    // channelId, channelKind, delivered, detail }, NOT { entries: [{ at, deliveries: [...] }] }. The old
    // code read body.entries (undefined on a bare array) and e.deliveries (the entry is flat), so the pack's
    // notify history was SILENTLY EMPTY for every customer (catalogue: notifications domain root cause).
    ...
    const arr = (Array.isArray(raw)
      ...
      : Array.isArray((raw as { entries?: unknown[] } | null)?.entries)
        ...
        : []) as Array<{ ts?: string; event?: string; severity?: string; downpipeId?: string | null; channelKind?: string; delivered?: boolean; deliveryCode?: unknown; platformCode?: unknown; recovered?: unknown; sinkScreen?: unknown; resolvedScreen?: unknown; unconfirmed?: unknown; ackOutcome?: unknown; recipientIndex?: unknown; test?: unknown }>;
    ...
    const project = (e: (typeof arr)[number]): Record<string, unknown> => ({
      ...
      // A MANUAL TEST press, not a routed engine event. The engine flags the row test:true when the test-send
      // is persisted, so a failed Test press does not read like a real alert that never reached the customer.
      // The test row also carries a
      ...
      ...(typeof e.resolvedScreen === "string" && RESOLVED_SINK_VERDICTS.has(e.resolvedScreen) ? { resolvedScreen: e.resolvedScreen } : {}),
      // unconfirmed + ackOutcome (async-create-unconfirmed): a JSM/ServiceNow create is ACCEPTED
      // asynchronously (202), so `delivered:true` means "the sink took it", NOT "the page exists". The engine
      // records `unconfirmed` for this. ackOutcome is the CLOSED verdict of the
      ...
      // recipientIndex: WHICH entry in the channel's recipient list was rejected. An e-mail channel
      // sends to all its recipients in ONE call, so a single typo'd address kills delivery to EVERY recipient
      // of that channel, and naming only that the channel failed leaves the bad entry unidentified.
    ...
    // FAILURE-BIASED window: the 20 most recent undelivered rows, wherever they are in the ring.
    const failureIdx = new Set<number>();
    for (let i = 0, n = 0; i < arr.length && n < 20; i++) {
      if (arr[i]?.delivered === false) { failureIdx.add(i); n++; }
    }
    // SMART-RETAIN THE BOUNDARY ROW: the OLDEST undelivered row per channelKind (the ring is newest-
    ...
    // PER-KIND ROLLUP over the WHOLE ring: failCount + the last failure and last SUCCESS per channel
...
export async function fetchNotifyConfig(scheduler: DurableObjectStub): Promise<{ channelCount: number; channelKinds: string[]; disabledChannelCount: number; ruleCount: number; danglingChannelRefs: number; rulesInert?: { count: number; byReason: Record<string, number> }; rules: Array<{ events?: unknown; minSeverity?: string; scope?: string }> }> {
  {
    ...
    // rulesInert (inert-notification-rule): a rule that can NEVER deliver -- it has no channel at all,
