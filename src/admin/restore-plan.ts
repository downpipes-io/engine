...
export function buildRestorePlan(env: Env, body: RestoreRequest, run: Run, inRecordScope: (name: string) => boolean, confirm: boolean, sourceBindings?: ReadonlyMap<string, string>, d1Restrict?: { database: string; tables: ReadonlySet<string> }): RestorePlanState | ReservedRefusal {
  ...
  for (const rec of run.records) {
    ...
    if (rec.sourceType === "secrets") {
      ...
      // The sentence names the CAUSE and the REMEDY, like every other out-of-band class. It used to be the
      // bare string "restore out of band", which named neither, while the documentation told customers the
      // guidance would name both. The remedy is real and is deliberately worded as what the operator does:
...
export async function runDryRun(body: RestoreRequest, run: Run, state: RestorePlanState, bufferedMaxBytes: number, runId: string, isLatest: boolean, sampleCap: number, cfApiFactory: (token: string) => CfApi): Promise<RestorePlan> {
  ...
  // sitting between the two instants used to read not-lapsed here and lapsed there, which dropped it with no
  // row in the plan the operator approved. That is the silent case this warning exists to close, surviving
  // inside it.
  //
  ...
  //   (2) THE CLOCK STARTS AT THE PLAN, AND THE ENGINE HAS TO MAKE THAT TRUE. The DO used to stamp
  //       expiresAt at REQUEST time, so the deadline was measured from an instant strictly later than this
  //       one, by however long the operator spent reading the plan -- unbounded, and repeatable for ever
  //       because a plan hash carries no timestamp. The dry-run route now records the plan anchor
  //       (PLAN_SEEN_PREFIX) and the DO stamps the TTL from it, so the deadline below is the deadline the
  //       approval will actually carry rather than an optimistic guess about it.
  //
  ...
  // A SINK-RESOLUTION REFUSAL IS A WAY A RECORD FALLS OUT OF A PLAN, so both verdict fields read it: a plan
  // that can write NOTHING AT ALL must never present as `ok:true, plannedWrites:0, bytes:0, sample:[],
  // complete:true` with every record sitting only in the `skipped` array that neither field reads -- that is
  // the last screen an operator sees before authorising a write-back, and it has to be accurate.
  //
  ...
  // incomplete -- a false claim in the other direction.
  //
  // `complete` means "every record is in this plan", so it accounts for BOTH ways a record can
  // involuntarily fall out of one: the windowed case (complete:false, outOfWindow, and a named remedy) and a
  // wholesale sink-resolution refusal. Five records left out of scope is honestly incomplete, and so is a
  // run where all eight records are unwritable.
  return {
