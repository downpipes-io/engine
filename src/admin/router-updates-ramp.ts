// router-updates-ramp.ts -- the OPT-IN gradual-ramp halves of the update routes (the ramp start and the
// ramp settle), split out of router-updates.ts so the route hub stays within the module
...
import { gate, jsonError, jsonResponse, ownerActionGate, ownerActionQueuedResponse, rateLimited, recordAudit, recordAuditAfterSelfDeploy, recordBookkeepingAfterSelfDeploy, requireAllUpdateApproval, updateNeedsDualControl, validateDeployToken } from "./router-core.ts";
...
// handleRampStart is POST /update/ramp: upload + ramp `percentage`% of LIVE traffic to the
...
// control applies exactly as for apply (gated only for migration/breaking); the token is required +
...
export async function handleRampStart(a: RampRouteArgs): Promise<Response> {
  ...
  // "The gradual ramp always errors" -- and the outer catch returned a fixed sentence and dropped the
  ...
  try {
    ...
    // Same refusal as apply (see pendingUpdateRefusal) -- a second ramp/apply
    ...
    if (pendingBlock) {
      // The closed guard class, not an audit-only refusal whose detail the pack's excerpt strips.
    ...
    // Same anti-rollback floor + freshness enforcement as the atomic apply -- and the same error-handling
    // bracket above. readUpdateFloor is a SCHEDULER DO READ with no catch of its own, so a DO outage here throws into
    ...
    // OPT-IN DUAL CONTROL, identical rule to apply: a ramp ships new engine code, so a migration/
    ...
    if (updateNeedsDualControl(riskClass, await requireAllUpdateApproval(scheduler))) {
    ...
    if (!validateDeployToken(token)) {
      // The ramp-START token guard recorded nothing, while the ramp-SETTLE token guard four lines of code away
    ...
    // No hgate is built here: a gate created at ramp/upload time would be stale by the time
    ...
    // startGradualRamp never flies the canary inline -- it cannot, this call is still
    ...
    if (r.outcome === "ramp-pending" && r.fromVersion && r.toVersion) {
      // rampVersion above just shifted live traffic to the new version (cf-deploy.ts's ramp
...
// handleRampSettle is POST /update/ramp/settle: a genuinely SEPARATE request from
...
export async function handleRampSettle(a: RampRouteArgs): Promise<Response> {
  ...
  try {
    ...
    if (!rec.pending || typeof rec.pending.percentage !== "number") {
      // "We kept trying to settle the ramp overnight." Every settle guard here returned a bare 400 and
    ...
    if (s.outcome === "inconclusive") {
      ...
      //
      // This is the counter's ONLY producer, deliberately. "The ramp has been pending verification
    ...
    // A "rolled-back" outcome above just redeployed fromVersion at 100% (settleAfterRamp's
