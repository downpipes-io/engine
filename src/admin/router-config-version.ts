...
// (the dispatch-split, MOVE-ONLY). Each case body was MOVED VERBATIM from handleAdmin's switch in router.ts;
// the only change is reading the shared locals off the RouterCtx the hub builds once, so the posture.read /
// posture.riskaccept / access.policy / downpipe.read / keys.ceremony gate on each route runs here exactly
// as it did inline. Imports shared primitives + the posture-via-DO core + the disarm-alert router; never
// imports router.ts.
...
export async function handleConfigVersion(ctx: RouterCtx): Promise<Response | null> {
  ...
  switch (`${req.method} ${sub}`) {
    ...
    case "GET /posture": {
      ...
      // returns immediately after the computation, so a floating alert promise dies with the context (measured
      // 0 of 3). Passing waitUntil keeps the read as fast as it was and the alert as durable as its siblings.
    ...
    case "POST /config/signin-context-policy": {
      // OWNER-OPT-IN unusual-location sign-in notify toggle (V6.3.5). Owner-only via the owner-exclusive
    ...
    case "POST /config/update-approval-policy": {
      // OWNER-OPT-IN "require a second owner for EVERY engine update" toggle. Same owner-exclusive
      // keys.ceremony gate as the policy toggles above, and the DO re-resolves owner in
      // setRequireAllUpdateApproval. It applies immediately and is audited (config-policy-change).
      //
      // WHAT IT IS FOR, in one sentence, because the flag reads like a duplicate of the dual-control gate and
      // is not: the update-apply gate is consulted only when the VENDOR-SIGNED manifest declares the release
      // migration or breaking, so the vendor, not the customer, decides whether the customer's second-owner
      // gate is asked. With this on, every live apply is gated whatever the manifest says. The current value
      // is read via GET /config/approval-policy, whose view carries requireAllUpdateApproval alongside the
      // other flags.
      const denied = gate(caller, "keys.ceremony");
      if (denied) return denied;
      const limited = await rateLimited(scheduler, caller);
      if (limited) return limited;
      return scheduler.fetch(doURL("/config/update-approval-policy"), { method: "POST", body: await req.text(), headers: callerHeaders(caller) });
    }
    ...
    case "POST /config/egress-allowlist": {
      // The operator-configured outbound egress allowlist (ASVS V13.2.4). Owner-only via the owner-exclusive
      // keys.ceremony capability, the same gate the attended-cadence toggle uses; the DO re-resolves owner in
      // setEgressAllowlist. It applies immediately (no dual-control arm; see the DO method's own comment for
      // why) and is audited as config-policy-change, carrying only the resulting host count.
      const denied = gate(caller, "keys.ceremony");
      if (denied) return denied;
      const limited = await rateLimited(scheduler, caller);
      if (limited) return limited;
      return scheduler.fetch(doURL("/config/egress-allowlist"), { method: "POST", body: await req.text(), headers: callerHeaders(caller) });
    }
    case "GET /config/egress-allowlist": {
      // Reading the list needs only downpipe.read, like the attended cadence: it is the operator's own
      // configured hostnames, not a secret. Writing it stays owner-only above.
      const denied = gate(caller, "downpipe.read");
      if (denied) return denied;
      return scheduler.fetch(doURL("/config/egress-allowlist"), { method: "GET", headers: callerHeaders(caller) });
    }
