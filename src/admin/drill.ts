...
export interface DrillResult {
  ...
  // durationMs / bytesVerified are the MEASURED recovery-test cost (RTO signal): the wall-clock the
  ...
  // oom is the restore-subsystem OOM-risk marker: the largest single record this
  ...
  // These fields carry the DISCRIMINATING detail a failed drill would otherwise throw away. "The restore test
  // failed" is the same sentence whether ONE record of 500 is corrupt or all 500 are, whether a bucket lifecycle
  // rule expired the archive object, or whether a deploy wiped SIGNER_PRIVATE and the archive is perfectly fine.
  // These name it.
  //
...
// test) advances and persists the returned cursor so coverage rotates across ticks.
...
// isolate. The in-account restore drill has NO streaming path -- measureSample /
...
// the largest SINGLE record this drill buffered whole via restoreRecord (0 when
...
// WindowMeasure extends SampleMeasure with the windowed-cursor bookkeeping: the advanced cursor,
...
interface WindowMeasure extends SampleMeasure {
  ...
  firstFailedIndex: number | null; // the index of the FIRST record that would not decrypt-and-verify
}
...
async function measureSample(run: Awaited<ReturnType<typeof openRun>>): Promise<SampleMeasure> {
  ...
  for (let i = 0; i < total && samplesRestored < want; i += stride) {
    ...
    if (value.length > maxRecordBytes) maxRecordBytes = value.length; // OOM-risk observation
    samplesRestored++;
  }
...
// instead of re-sampling the same strided records forever. It DRAINS per-record failures (a
...
async function measureWindow(run: Awaited<ReturnType<typeof openRun>>, cursor: number, window: number): Promise<WindowMeasure> {
  ...
  for (let k = 0; k < count; k++) {
    ...
    try {
      ...
      if (firstFailedIndex === null) firstFailedIndex = i; // WHICH record, not merely that one failed
    }
  }
  return {
    bytesVerified,
...
// oomMarker builds the DrillResult OOM-risk marker from the largest single record
...
export async function runDrill(env: Env, runId: string, destOverride?: RuntimeDestConfig | null, deepVerify?: DeepVerifyWindow): Promise<DrillResult> {
  if (!env.OPERATIONAL_PRIVATE) {
    // An HONEST refusal, not a failure. The closed reason classifies to `posture-unexercisable` (never
    ...
    // IT NAMES THE IN-PRODUCT ROUTES FIRST: offering the offline CLI as the only way forward would read as
    // "there is nothing you can do from here" and contradict the product's own no-customer-CLI rule. Two
    // in-product routes exist: an ATTENDED verification proves recovery from the browser on exactly this
    // posture (attended-cadence.ts is the whole module for it), and POST /admin/keys/add-operational installs
    // a fresh operational pair, which brings unattended drills back for runs sealed after it. The offline
    // route stays, last, because it is the only one that reaches archives sealed before an operational key
    // was ever present.
    return {
      ok: false,
      runId,
  ...
  // The archive object key the destination could not produce. Captured AT THE THROW SITE (never parsed
  ...
  try {
    ...
    const store: ObjectStore = {
      get: async (k: string) => {
        ...
        if (!r) {
          missingObjectKey = k; // name the object, at the site that knows it
    ...
    try {
      // Windowed mode (the scheduled restore test): decrypt this tick's window and report the
      ...
      if (deepVerify) {
        ...
        // ZERO RECORDS IS NOT A PASS: this matches the clause restore-verify.ts:127 also carries --
        // `ok: failures.length === 0 && recordsVerified > 0`. Without it, the two routes would answer the SAME
        // question about the SAME run in the SAME minute with ok:false and ok:true. The drill's verdict is the
        // one that travels: it stamps lastRestoreTestOk, which posture-checks.ts reads as "restore tested
        // recently", and it writes the drill-evidence note. So a run over a genuinely empty source must never
        // accumulate an evidence row reading "scheduled restore test passed (records verified: 0)" -- a
        // compliance artefact asserting a restore test passed over nothing.
        //
        // The RTO measurement is deliberately withheld here, because a zero-record run measures no recoverable
        // work; that withholding is separate from the verdict, which must still read as not-a-pass.
        ...
        return {
          ...
          // The OOM-risk marker: present whenever a record was buffered this window, even on
          ...
          // The blast radius + the first failing index. A single corrupt record and a wholly-corrupt
      ...
      // ZERO RECORDS IS NOT A PASS, the MANUAL-mode half of the same clause: `ok: true` must not be
      // unconditional, because a run holding no records would otherwise return ok:true with
      // recordsVerified:0 and sampleRestored:false. router-restore.ts and router-pipelines.ts both stamp
      // lastRestoreTestOk from this field, so an unconditional ok:true here would write the same false pass
      // as the scheduled route.
      ...
      return {
        ...
        // The OOM-risk marker: present whenever the drill buffered at least one record.
        ...(sOom ? { oom: sOom } : {}),
      };
    } finally {
      run.dispose();
    ...
    // Which ENGINE BINDING was absent, when that is what refused. req() names the env var it is missing;
