...
export async function runPreflight(env: Env, scheduler: DurableObjectStub): Promise<PreflightReport> {
  ...
  // The roster read feeds THREE probes, so a fault in it must not be invisible: probeSourceBindings
  // raises its loud DO-unreachable failure, but the other two would otherwise go QUIET against a fabricated
  // empty fleet -- and a NON-ARRAY body (silently coerced to []) would produce a FALSE-GREEN source-bindings
  // item: a preflight that PASSES while twenty downpipes have no bindings at all. `degraded` is set for BOTH
  // degradations and for NEITHER a genuinely empty fleet (a new account must not look like an outage).
  let degraded = false;
  try {
