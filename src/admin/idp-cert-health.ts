...
//
// THE TICKET: "SSO died the day our old IdP cert expired." The replacement certificate was pasted CORRUPT
...
//     but the fact that it fired is diagnostic gold, and nothing else records it);
...
export interface IdpCertObservation {
  ...
  readonly windowUnenforced: boolean; // at least one usable cert has NO readable window, so the freshness check ran DISARMED for it
...
export interface IdpCertHealth {
  ...
  readonly windowUnenforcedVerifies: number; // cumulative sign-ins whose signature check ran with the freshness window DISARMED
