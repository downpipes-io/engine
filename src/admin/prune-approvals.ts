...
// later restore). The standing platform precedent for a write this consequential is restore's own
// request -> approve -> apply (admin/approvals.ts): a single identity plus step-up is NOT enough on its
...
//
// SINGLE-OWNER ESTATES. This module invents NO carve-out for a sole owner: exactly like restore's D2, a
...
import { APPROVAL_MIN_DWELL_MS, approvalDwellRefusal } from "./approvals.ts";
...
// ASVS 2.1.6 lease-guarded) -> consumed (single-use, on a successful apply) or, on a failed/errored
...
export interface PruneApproval {
  ...
  // this email, so requestedBy is not display/audit only.
  requestedBy: string;
...
// The approval-dwell floor (ASVS V2.4.2, approvals.ts) must sit strictly below this TTL too, for the same
// reason owner-action.ts asserts it against OWNER_ACTION_TTL_MS: a request could otherwise expire before
// the floor gating its own approve ever clears.
if (APPROVAL_MIN_DWELL_MS >= PRUNE_APPROVAL_TTL_MS) {
  throw new Error("APPROVAL_MIN_DWELL_MS must be strictly below PRUNE_APPROVAL_TTL_MS");
}
...
// approvalTimestampUnparseable mirrors approvals.ts's guard: a stored expiresAt that does not parse
...
// retryAfterSeconds/reasonCode are populated only for the ASVS V2.4.2 too-soon refusal below; every other
// refusal here leaves both absent, exactly as before.
export type PruneGovernanceDecision = { ok: true } | { ok: false; reason: string; reasonCode?: "too-soon"; retryAfterSeconds?: number };

// canApprovePrune mirrors canApprove: a self-approval (by stable subject) is refused even for an Owner;
// only a "requested" record can be approved (not already approved/rejected/consumed/expired/applying). It
// also refuses a same-EMAIL self-approval (belt-and-suspenders, on top of the subject check), mirroring
// canApproveChange / canApproveOwnerAction -- one human can legitimately hold two distinct,
// IdP-bound subjects sharing one verified email, so the subject check alone does not catch a
// request-as-A / approve-as-B self-approval by that human. ASVS V2.4.2 (approval-dwell): once every other
// refusal has cleared, the approve must also arrive at least APPROVAL_MIN_DWELL_MS after requestedAt.
