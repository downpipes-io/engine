// Support-pack section gatherers: the ENGINE SELF-DIAGNOSTIC domain -- fault ledgers with
// recorders but no pack projection:
//
//   - the scheduler DO's fault ledger (GET /sched-diag): the opaque errorId ring a customer quotes
//     from a console toast, the DO-side structural ceremony/recovery fault classes the Worker
//     edge cannot see, the Worker<->DO contract faults that would otherwise be coerced in silence,
//     the env-default destination's reachability heartbeat, and the downpipes whose
//     staleness rule can never arm;
//   - the per-downpipe PRE-RUN seal-dispatch fault class (GET /seal-errors);
//   - the Prometheus /metrics scrape surface's own health (GET /metrics-health);
//   - the structural WebAuthn fault classes split by ceremony phase (GET /webauthn-faults);
//   - the recorders' OWN dropped writes (GET /dropped-writes) -- the "how much of this
...
//   - the account-global rate-limit DO's fail-open health (RATELIMIT_DO GET /health).
//
...
import {
  // The sched vocabularies. AUTHZ_GATES is re-derived from the product's own ALL_CAPABILITIES at its
  // source, so the pack's gate cannot drift from the capability model.
...
// The closed sets the chain-break latch is re-gated against at the pack boundary (defence in depth).
...
// The closed sets the wiring-check ring is re-gated against at the pack boundary (defence in depth: the
...
// The store layer's OWN closed reason vocabulary, reused rather than re-declared: "unknown" without it
// names no remedy, and its members ("denied" / "not-implemented") are the two intents this split exists to separate.
...
// The two COMPOSITE key spaces (surface|reason, surface|dropClass) are re-derived as
...
// The scheduler-DO ledger key spaces. The two COMPOSITE aggregates are keyed "<a>|<b>", so the pack
...
// ---- the scheduler-DO ledger key spaces -------------------------------------------------------------
...
// The REFUSAL key space, built as the full cross-product of the two closed vocabularies taken from the
...
export async function fetchSchedDiag(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  {
    ...
    const j = (await r.json()) as {
      ...
      // ---- all served by readSchedDiag on the SAME GET /sched-diag read ----
    ...
    // ceremonyFaults: the DO-side STRUCTURAL ceremony + recovery fault classes the Worker edge
    ...
    // contractFaults: every Worker<->DO contract fault that would otherwise be coerced in silence -- an
    ...
    // recentErrors: the OPAQUE errorId the customer quotes from a console toast, with its ceremony/route
    ...
    // defaultDestination: the ENV-DEFAULT destination's own reachability heartbeat. The common
    ...
    // freshnessFaults: the downpipes whose staleness rule can NEVER arm (a non-finite cadence, an
    ...
    // ---- the scheduler-DO ledgers: chain, roster and recovery faults --------------------------------------
    // vocabDrops: the VERSION-SKEW blind spot. A closed-set gate in the DO can drop an
    // out-of-vocabulary token in silence, so after a partial deploy (a Worker newer than its DO, or the
    // reverse) a whole class of evidence can stop landing while every aggregate downstream reads clean.
    // ANY non-zero count here means the pack sections it names are a LOWER BOUND. The cruellest member is
    // deferral-kind: a restore drill that COULD NOT RUN would otherwise render as "tested and failed".
    ...
    // expiryObserveFaults: a credential-expiry observation that never armed the warning ladder. An
    // EMPTY licenceExpiryTracker / expiryWarnings is self-explaining -- "we could not observe it" is a
    // different fact from "there is nothing to warn about", and unparseable-date would otherwise be
    // indistinguishable from "this account has no licence expiry at all".
    ...
    // approvalFaults: the dual-control restore lifecycle's refusals. Two members are ESCALATIONS, not
    ...
    // storageAnomalies: every silent self-heal, coercion and safe-default read. The headline member is
    ...
    // rosterDiscards: WHAT a roster repair threw away. A delete-ghost over a row that DOES carry an
    ...
    // chainBreaks: the tamper verdict that a retention rollover would otherwise ERASE. The pack
    ...
    // fail and proves nothing about the bodies (signing-key-rotated -- that row is latched only when the
    ...
    //
    // It is a LIST, not one row per chain: keying the latch by chain name alone would fold a later,
    // genuinely different break into the first and lose it -- a seq-gap at 41 followed by an edited snapshot
    // at 12 would read as ONE row, seq-gap@41, detections:2, and "detections: 2" would then mean either "a
    // standing break" or "two unrelated tamper events, one of which is thrown away". Each distinct
    // (chain, cause, seq) has its own row.
    //
    ...
    // testOutcomes: the Test / Verify button's ANSWER, which would otherwise die with the browser tab. Per
    ...
    for (const [surface, rows] of Object.entries((typeof j.testOutcomes === "object" && j.testOutcomes !== null ? j.testOutcomes : {}) as Record<string, unknown>)) {
      ...
      const ring = rows.slice(-TEST_OUTCOMES_PER_SURFACE).flatMap((raw) => {
        ...
        // The destination-verify discriminator. Without these the pack CANNOT tell a healthy destination
        ...
        // The delete verdict's own status class, re-gated and admitted ONLY beside a delete that FAILED --
        ...
        // WHAT THE EMAIL SERVICE ACTUALLY SAID. Re-gated through the same E_UPPER_SNAKE shape gate the
        ...
        // WHAT THE SINK ACTUALLY DID, on the surface the ticket names. Re-gated against the channel layer's
    ...
    // recoveryResume: what the DR auto-heal SILENTLY dropped, re-pointed and stripped. defaultRepointed
    ...
    // ---- the scheduler-DO ledgers: destination, governance and client faults -------------------------------
    // destResolveFallbacks: the resolver deviations that silently write to a destination the customer
    ...
    // canaryLosses: the canary's own losses. lost-flight means a flight was allocated and its
    ...
    // drillDrops: the restore-test pipeline's silent discards. member-requeued is the "stuck at 3
    ...
    // notifyDrops: WHICH alert died, not just how many. notifyHealth carries four integers; the
    ...
    // governanceFaults: the dual-control / change-control plane's refusals, keyed "<stage>|<class>".
    ...
    // governanceRefusals: the REASON, keyed "<stage>|<reason>". governanceFaults above says a guard
    ...
    //
    // Any of these could otherwise collapse into the single "guard-refused" class, or into nothing at all.
    ...
    // refusalRings: the SEQUENCE, not the last-only collapse. A customer who retried a licence
    ...
    // schedAdminRefusals: what the DO itself refused, keyed "<route>|<reason>" over two closed
    ...
    // dest-remove guard split (in-use vs orphan-guard), which the console would otherwise tell apart by
    // SUBSTRING-MATCHING the refusal prose.
    ...
    // clientDiagnostics: the BROWSER's own evidence,
    ...
    // ---- the scheduler-DO ledgers: config, authz and posture evidence ---------------------------------------
    // configRejections: EVERY refused configuration save, recorded at the ONE funnel every DO
    ...
    // break-glass reconcile or estate-import refusal would otherwise write NOTHING AT ALL.
    ...
    // authzRefusals: every capability gate, owner gate, escalation guard and break-glass guard,
    ...
    // alertingHealth: the alerting plane's own liveness. detection-off-no-sink fires at BOTH detection
    ...
    // so the record is never destroyed in the same statement that abandons the incident.
    ...
    // configCoercions: the config a save ACCEPTED and then silently DROPPED. The count is the number of
    ...
    // capTruncations: the rows a bounded record DROPPED. This matters far beyond cosmetics: the
    ...
    // capTruncationSubjects: WHICH subjects the cap dropped, for the three surfaces whose
    ...
    // auditEgress: the audit feed's DELIVERY health. mirrorFailures counts the committed audit
    ...
    // posture: the posture EVALUATION itself, plus the three ways its inputs arrive unusable.
    // identity-read-failed is a DROPPED IDP_KV BINDING -- a permanent one-line fix that would otherwise be
    // indistinguishable from a transient blip ("admin-strong-auth has said cannot-verify for weeks"). status-slice-malformed
...
// freshnessUncomputable is the downpipes[] join: the set of downpipe ids whose staleness rule cannot
...
// capTruncationSubjectIndex is the downpipes[] join for the subjects a cap REFUSED,
// taken from the schedDiag section ALREADY fetched (no extra DO round trip). Pure over its input.
//
// This is what reaches the row. `freshnessUncomputableIndex` above can only mark a downpipe that is
// IN the freshness map; a downpipe past the cap is absent from it, carries no marker, and its row reads as
// a downpipe with a working staleness rule while the caveat sits in the schedDiag section instead. `named`
// is the set this row-level marker can be attributed to exactly. `incomplete` says at least one refused
// subject could NOT be named, and a reader must then treat EVERY unmarked row as unverified rather than
// clean -- which is why the caller marks the whole population on it.
...
// fetchDroppedWrites projects the recorders' OWN dropped writes (GET /dropped-writes). ANY
...
// fetchSealErrors projects the per-downpipe PRE-RUN seal-dispatch fault (GET /seal-errors). The throw
// lands BEFORE a run index is allocated, so the loop's /complete is a runId-"" history no-op, and an
// anonymous scheduler.ticks.sealErrors integer alone would not say WHICH downpipe, or why. Each
// downpipe instead carries a CLOSED class (destination unresolvable / config unreadable / wrap key invalid / the
...
// fetchMetricsHealth projects the Prometheus /metrics scrape surface's own health (GET /metrics-health).
...
// /webauthn-faults). Without the split, all 14 collapse into the one coarse bad_request the ceremony returns,
// so "half my fleet cannot enrol a key" and "this user can never sign in again because their STORED COSE
// record is corrupt" would read as the same pack evidence: none. The key is "<phase>:<class>"; cose-unsupported-alg carries the
...
// These are the projections of the two ledgers recorded at the fault site:
...
// "_unavailable" surface names WHY -- auth / entitlement / server-error / size-cap / page-cap -- four
// different tickets that would otherwise produce identical packs. Keyed by the customer's own downpipe
// label; the ids are engine-chosen product tokens and one-way handles.
...
// presence boolean.
//
// strandedAborts rides through the SAME sanitiseDestFaults chokepoint, which re-gates it against the
// closed ABORT_FAILURE_CLASSES. It has to be forwarded EXPLICITLY, because this projector rebuilds the
// snapshot field by field rather than passing the DO's body through -- and a field the rebuild forgets is a
// recorder writing into a void, which is the precise failure this guards against. A stranded abort is
...
// /restore-faults). The `phase` field is the one a diagnosis reads FIRST: `verify` means NOTHING
...
export async function fetchRestoreFaults(scheduler: DurableObjectStub): Promise<unknown[]> {
  ...
  for (const raw of rows.slice(-RESTORE_FAULTS_RING_CAP)) {
    ...
    out.push({
      ...
      // The drill's discriminating detail. `index` is the FIRST failing record's index in the run, so
      ...
      // and fail: this engine holds no in-account read-back key, so the drill COULD NOT RUN, which is
      // otherwise shape-identical to a genuine not-configured failure.
...
// and the dest-config / dest-io counters. READ THIS ALONGSIDE droppedWrites: ANY
...
// fetchRateLimitHealth reads the account-global CF API rate-limit DO's own health. infra
...
export async function fetchRateLimitHealth(env: Env): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    // A read fault is NOT a healthy limiter. Flag it so a diagnoser can tell the two apart.
