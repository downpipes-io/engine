...
//
// THE TICKET. "Was this estate behind Access when the change was made, and how long did it run on the shared
...
// -- the facts that BOUND the degraded window, which is what the ticket actually asks for -- are never deferred).
...
// eviction regardless (the window's first event was written immediately), so the ticket's answer does not.
