...
// This is the CANONICAL source of the closed unions; the console and the diagnosis bot MIRROR these exact
// lists in their own repos and a conformance/drift test asserts they match (the AUTO_HEAL_REFUSAL_CODES
// pattern), because console/engine are separate repos.
//
// STRUCTURALLY VALUE-FREE: the client-diagnostics record has ZERO free-string fields. Every
...
// kind -- what fired (43). A closed product classifier, never error text.
export const CLIENT_DIAG_KINDS = [
  ...
  // this array by matching double-quoted strings, so a quotation inside a comment here would be read as an
  // extra kind and fail the drift guard.)
  "read-degraded",
  ...
  "onboarding-step",
  // discovery-connect -- what the account-discovery token actually SAW in the browser. This is the console's
  // own half of the picture: it separates a token that was accepted and then saw zero accounts, an account whose product
  ...
  // it whatsoever: this row is the only place that fact is recorded.
  "claim-exchange",
  ...
  "contract-skew",
  // fleet-drill -- one fleet-drill SESSION, as the console assembled it. This engine records every drill it
  // HANDLED per downpipe, so the per-pipe outcomes are already here. What it cannot see is the shape
  ...
  "fleet-drill",
  // Each of the following six kinds is a browser-side fact the engine cannot hold: a dual-control refusal the
  // console decided (or that the engine refused and only the console can attribute), the grants a custom-role
  // delete silently downgraded, a resource the browser's own CSP blocked, part of a paste the console dropped
  // before any request was made, a wizard hand-off that lost the operator's pick between steps, and a
  // key-ceremony step that ran entirely in the browser. Admitted here or the rows are DROPPED on arrival and the
  // browser evidence silently vanishes from the pack.
  ...
  "handoff-dropped",
  "ceremony-step",
  // Both of the following two kinds are BROWSER-ENVIRONMENT facts that no engine can ever observe: a store the
  // browser refused to keep anything in, and which renderer the topology map actually got. Admitted here or the
  ...
  // was itself the navigation. This is the only member of this vocabulary that can see the accessibility
  // surface at all: without it, a keyboard user who could not arrow past the first tab produces a pack
  ...
  //
  // The console's reader distinguishes no intent was declared from the declared element was not mounted, and
  // the second of those two IS the defect. focusOutcome is that fact, kept. It is a STATE row on the tablist
  // population rather than a fault row, for the same reason `none` is a member of degradeCause: without an
  // honoured row the pack cannot say the mechanism was working, and a tablist that is broken and a tablist the
  // customer never touched carry identical evidence.
  //
...
// screen -- the route TEMPLATE id, sourced from console src/screens/** (a compile-time literal attached
...
export const CLIENT_DIAG_SCREENS = [
  ...
  "keys",
  "security",
  // config-changes / owner-actions -- the two APPROVAL INBOXES, split out from the single `security` screen id
  // the console's route table used to drop them both into. A render throw inside a .then with no .catch is
  // nearly always a TypeError, so the config-approvals screen frozen on skeleton rows and the owner-approvals
  // screen frozen on skeleton rows would otherwise produce the byte-identical tuple and coalesce into one row;
  // naming both screens lets support tell them apart.
  "config-changes",
  "owner-actions",
  "updates",
  "support",
...
// reasonClass -- bulk-outcome only (6).
...
// surface -- capability-fault only (8). WHICH CEREMONY asked for the capability, and therefore WHAT the
...
// buildCheckClass -- console-build-check only (6). What the ORIGIN was serving when the console asked it
...
// would otherwise coalesce into one member and one row: support could not tell the update verdict was computed
...
// rollbackClass -- console-rollback only (7). The first five ARE this engine's own
...
// fieldClass -- wire-anomaly only (21). WHICH CLASS of engine-supplied field was unusable. A class, never
...
// Four of these families are ones the console COARSENS rather than crashes on, each of which renders a
// plausible-looking verdict from a value it could not read: an unrecognised whoami method is presented as the
// break-glass TOKEN FALLBACK posture (a false claim that the customer's security posture is degraded), an
// unknown downpipe status becomes unknown on the map, an unparseable last-run instant skips the staleness
// test entirely (a month-old pipe reads FRESH), and a non-finite byte figure becomes 0.
export const CLIENT_DIAG_FIELD_CLASSES = [
  ...
  "cadence",
  // The three protection-statement recency stamps, the licence/estate/artefact malformations and the
  // drill-evidence stamp. Each names a DIFFERENT customer-visible assurance that a corrupt value silently
  // disarms, so each is its own member: as one generic `timestamp` row they would coalesce and support could
  // not say which assurance the customer lost.
  "restore-test-at",
...
// anomaly -- wire-anomaly only (6). HOW the value was wrong.
// unknown-enum: the value arrived, it PARSED, and it is simply not a member of the closed set that console
// build knows. Distinct from `unparseable` (corruption) and `missing` (absence), and the distinction matters:
// an unknown enum member after an engine update is VERSION SKEW against THIS engine, and the remedy is to
// update the console. The unrecognised member itself never rides.
...
// transportClass -- transport-fault only (10). The console's own resolved transport
...
// this engine over a service binding. If a console redeploy drops that binding, those paths fall through to the
// console's static assets, which answer the SPA's index.html at HTTP 200: the console would read its own shell
// as engine data, a /metrics scraper would read HTML as a healthy scrape, and this engine -- which never
// received the request -- would appear perfectly healthy in the pack. A pack showing a healthy engine and a
// customer reporting a dead console would then be the same pack, with the fault actually in the console's
// deploy. The console worker instead answers its own 503 with a frozen code, and the browser records this
// class.
//
// THE BOUND ON THIS CLASS IS STATED RATHER THAN PAPERED OVER. A clientDiagnostics section rides ONLY on the
// customer's deliberate Generate action, which POSTs to /admin/support/bundle -- an ENGINE surface, reached
// over the very service binding that is missing. So the console cannot build a pack at all while this fault is
// live: the row is recorded only when the pack that would carry it is not refused. The class is therefore
// reachable in a pack in exactly one shape, and it is a POST-MORTEM: a browser session that observed the 503s
// and generated the pack after the binding was restored, in the same tab (the ring is in-memory, so a reload
// ends the observation). Every pack that carries this class was necessarily produced with the binding present,
// so it must be read in the past tense and never sent a support engineer to restore a binding whose absence
// would have stopped the pack existing. This class does not cover the live fault: a console that cannot reach
// this engine cannot report to it either, and an engine-side counter would require a background beacon, which
// this topology cannot carry regardless (the engine is reachable only over the service binding and has no
// public hostname). What the live fault does get is the console's own 503 on /metrics, which alarms the
// customer's monitoring instead of feeding it HTML at 200.
...
// callClass -- read-degraded only (7). WHICH swallowed console read went quiet. The console's screen
...
// obStep / obOutcome -- onboarding-step only (6 and 17). WHICH wizard step and HOW it ended. The steps
...
// exists), and it is its own step because a failed precheck must not read as a failed install-POST: coalescing
// the two would report this engine never saw the request when in fact no request for keys was ever made. When
// the read fails the wizard tells an operator whose engine already holds working keys to go back and generate
// them again, so the step earns its own evidence.
...
export const CLIENT_DIAG_ONBOARDING_OUTCOMES = [
  ...
  "poll-exhausted",
  // THE FOUR FAILED-POLL MEMBERS BELOW ANSWER TWO QUESTIONS: did the poll EVER read this engine, and did its
  // FINAL tick fail because this engine REFUSED the read or because it could not be REACHED? A member whose
  // meaning asserts a fact may only be written where the code established that fact: `readiness-unread` and
  // `poll-went-quiet` each assert, in their own definitions and in the bot's rendering of them ("fix the
  // engine's reachability first"), that this engine did not answer, which would be false for an engine that is
  // reachable and has been answering with refusals (a 500 forty times, say) that sit in its own logs. The
  // console classifies the poll's every failed tick through the same total mapper the rest of the wizard uses,
  // and splits answered-refusal from unreachable on both legs.
  //
  // readiness-unread -- the poll ran to its bound, NEVER ONCE READ THIS ENGINE, and its final tick could
  // not REACH it. This engine was already gone when the readiness card mounted.
  "readiness-unread",
  ...
  // Without this distinct member the state would coalesce into readiness-unread, which the support bot reads
  // out as an unreachable engine -- the single most misleading claim the console could make here.
  "readiness-refused",
  ...
  // (installing the key secrets sets Worker secrets, which is itself a redeploy). readiness-unread does not fit:
  // that member demands that EVERY tick failed, which happens only if the engine was already dead before the
  // readiness card mounted. Without this member the state falls through to poll-exhausted carrying a lastRead
  // taken minutes earlier, which asserts this engine had gone on answering and reporting the keys absent when it
  // had in fact stopped answering -- sending support to re-run the key ceremony when the actual remedy is an
  // unreachable engine. It carries no obSecret, because a stale read is not evidence of what this engine held
  // when it died.
  "poll-went-quiet",
  // poll-refused -- THIS ENGINE ANSWERED, AND THEN STARTED REFUSING. The poll read a status at least once
  // and its FINAL tick was a refusal, not a transport failure. This engine is UP and turning the status read
  // away, which is a different situation and a different remedy from one that vanished. Without this member the
  // state would be recorded as poll-went-quiet, whose definition asserts the engine is not reachable now, which
  // would be wrong here. No obSecret, for the same reason: the last successful read is stale and is not evidence
  // of what this engine holds now.
  "poll-refused",
  // engine-binding-absent -- THE CONSOLE'S OWN DEPLOY HAS NO ENGINE SERVICE BINDING, so the wizard's call never
  // left the console: its worker answers the engine surface itself, with a 503 carrying a frozen code. THIS
  // ENGINE RECEIVES NOTHING AND IS NOT AT FAULT, and the remedy is the console's wrangler configuration and a
  // console redeploy. It is a member because the console's classifier keys on a trailing numeric status and the
  // throw for this state carries a marker instead, so without it the state falls through to `transport-error`
  // and to the poll's unreachable legs, every one of which asserts that THIS ENGINE did not answer -- telling
  // support to go and revive an engine that is up, healthy and idle, which is the same misdirection the
  // transportClass member of this name exists to stop, arriving one level down inside the wizard.
  //
  ...
  // base, so it meets the same 503. The member is still admitted, because the console's alternative is not
  // silence, it is a false claim about this engine's reachability.
  "engine-binding-absent",
...
// back unverified. This engine hands the console the CLOSED cause on UpdateStatus.channelFault (updates.ts,
...
//
// bad-url and bad-signer-key are the MISCONFIGURED channel. The console's emit is gated on this engine's
// `channelIntended` flag (env-var presence) rather than on `configured`, which is a VERDICT and not the
// operator's INTENT: checkUpdates answers configured:false for an unparseable UPDATE_CHANNEL_URL, a non-https
// one, and an UPDATE_SIGNER_PUBLIC that will not parse -- three states in which both env vars are set. Gating on
// `configured` would record NOTHING for a truncated paste of the pinned signer key, producing a pack
// byte-identical to a healthy channel.
...
// claimResult -- claim-exchange only (10). How the licence claim-code exchange with the VENDOR control
// plane ended. This engine is not on that path at all. `http-5xx` (a vendor outage, which the console reports
// to the customer as check your connection) and `empty-token-200` (a control-plane deploy that changed
...
// The record shape. Every string field is one of the closed unions above; every numeric is a
// clamped integer. NO other string fields exist. `count` is a coalesce-repeat count (never a sequence,
...
export const CLIENT_DIAG_ADMIN_OPS = [
  ...
  "terminate-session", // ending ONE of the caller's own sessions by id
  "terminate-all-sessions",
  "recovery-codes-regenerate",
  // The save-confirm panel's own POST, fired after a self-add enrolment staged a fresh set over an existing
  // live one, most often the forced re-enrolment after a recovery-code sign-in. See POST
  // /admin/auth/recovery-codes/confirm (router-auth-flow.ts).
  ...
  // row to name the rollover, not the connection edit they never attempted. This list is the RECEIVER:
  // projectClientDiagnostics checks adminOp by set membership and fails the whole record closed on a member
  // this set does not hold (client-diag-receive.ts:220), so this engine's copy of the vocabulary must be kept
  // ahead of the console's, or a console-emitted row naming a new op is dropped whole rather than landed.
  ...
  "source-detach",
  "source-reattach",
  // The DOWNPIPE and DESTINATION ops. These are the controls a role gate most often greys out (the Delete
  // button is greyed out for our admin and we do not know why), and a gov-gate row cannot say WHICH control was
  // refused without an op to name it. They are privileged writes in their own right, so they belong in this
  // vocabulary whether or not a gate ever fires on them.
  "downpipe-run",
  "downpipe-edit",
  "downpipe-toggle",
  "downpipe-delete",
...
// recoveryOp -- recovery-refusal only (6). WHICH manual disaster-recovery flow was refused. The
// six share one code vocabulary and are six different situations.
//
...
// cp-acknowledge is the ACKNOWLEDGE-ONLY latch clear, its own op rather than a re-use of
...
export const CLIENT_DIAG_RECOVERY_CODES = [
  ...
  "DP-R13",
  "DP-R14",
  // DP-R15/R16/R17 separate three refusal reasons that DP-R12 otherwise lumps together: DP-R12 is EVERY 400
  // this engine answers, so signature, shape-check and no-custody would otherwise coalesce into one row on the
  // tuple key. This engine returns a CLOSED `refusalClass` member in its 400 body and the console admits it by
  // SET MEMBERSHIP, mapping it to a code; a body carrying no recognised member still lands on DP-R12, so the
  // mapper stays total and an older engine keeps working.
  ...
  // The browser-unseal-only codes. DP-R15/18/19/20/21 above are ALSO reachable straight from the browser (the
  // console verifies the SEALED wrapper's signature locally, byte-identical to this engine's own check, before
  // ever decrypting) -- same meaning, just caught one round-trip earlier. These seven are STATES that have no
  // plaintext-import equivalent, because only a browser holding the private identity can ever observe them
  // (this engine holds no key that opens a sealed artefact it did not seal itself).
  ...
  // The acknowledge-only latch clear's three refusals, each its own branch under the
...
// probeOutcome -- probe-outcome only (22). HOW the test ended, in ONE union whose members are surface-
...
// formField -- form-rejected only (56). The FIELD CATALOGUE control id the console's own validator acted
...
// This vocabulary is the RECEIVING TWIN of the console's, so it may only admit what the console can actually
// emit. A form-rejected row means the CONSOLE'S OWN validator refused the operator in the browser, and six
// destination controls -- the R2/S3 bucket, the account id, the region, and the access key/secret pair -- carry
// no client-side validator: a bucket name, a region, an account id and an access key/secret pair are not
// decidable in a browser. THIS ENGINE verifies them, live, and its refusal is already carried as an engine-call
// row, which is what it is. Admitting a member the console cannot send is a promise this pack cannot keep, so
...
export const CLIENT_DIAG_FORM_FIELDS = [
  ...
  "secretName",
  "dest-endpoint",
  // These SEVEN ids are the CATALOGUE'S OWN ids. The whole point of carrying a field id rather than a screen
  // name is that the row JOINS to the field catalogue and feeds its divergence detector, and an id present in
  // no catalogue row joins to nothing.
  "dest-worm-days",
  "dest-role-arn",
  "dest-sts-duration",
  "dest-price-storage",
  ...
  "dest-price-classb",
  "dest-price-egress",
  // The CONSOLE-WIDE funnel. Every control built by components/field.ts runs its validator through ONE function,
  // and a refusal there never leaves the browser: the form will not accept my cron / bucket name / endpoint, and
  // the split button stays disabled, are two of the commonest console tickets with no engine-side evidence at
  // all. These are the catalogued control ids of every field that HAS a validator to refuse with, so the row can
  // say WHICH control refused. The id is a product constant from the field catalogue; the typed VALUE is never
  // read, and on this family that matters more than most: these fields hold licence tokens, deploy tokens,
  ...
  //
  // dp-bucket is answerable from the add-source and destination forms and from the downpipe editor, where a
  // downpipe's bucket override is edited; dp-sched-cron and dp-sched-tz are on the funnel alongside it.
  //
  // DELIBERATELY ABSENT: idp-var (the IdP preset's dynamic required-variable controls), saml-idp-entity and
  // saml-sp-entity. Every one of them is `required: true` with a validator that only rejects the EMPTY string,
  // so field()'s validate() hits the required-and-empty branch first and returns before the validator ever runs.
  // Their sole possible refusal is therefore the one refusal the funnel deliberately does not record (an
  // operator part-way through a form is not a fault), which means a member for them could never be produced by
  // any code path. A vocabulary member with no producer reads like coverage and is not: it tells a support
  // engineer the evidence was looked for and not found. The same reasoning excludes channel-name,
  // channel-routing-key, demo-reset-token, group-role-group, idp-client-id, idp-label, licence-token, saml-label,
  // update-component-token, update-deploy-token, update-ramp-token, update-rollback-token,
  // update-rollback-token-urgent and update-settle-token: each control HAS a validator, but every one of those
  // validators is an emptiness test and nothing more, so any non-empty string passes it and no console build can
  // ever send a refusal for it. This list is the RECEIVER: a member admitted here that the console can never
  // send is the same dead promise read from the other end.
  "channel-addresses",
  "channel-url",
  "cost-cadence-custom",
  "cost-churn",
  ...
  "cost-retention-runs",
  "cost-source",
  // The Shamir split pickers (custody-step-panels.ts). Two ids, not one, and that IS the discrimination:
  ...
  // the connection form's certificate field, and a refusal on one is a different situation from a refusal on the
...
// catalogueClass -- catalogue-degraded only (20). WHY the Cloudflare-configuration offer was withheld or
...
export const CLIENT_DIAG_CATALOGUE_CLASSES = [
  ...
  // open) throws for several different reasons, and two of them are not faults at all: a lapsed Cloudflare
  // Access session (the ordinary overnight tab, on an Access-fenced console) and a 401 record NOTHING. The five
  // members here are five different remedies, and they are the twins of the cf-rediscover-* members: the
  // ADDRESS (a web page answered), the ROLE (the engine's own 403), an EDGE RULE or the address (a 403 that did
  // not speak the engine's refusal vocabulary), a retry (429), and a dropped call. The console's own 500 and its
  // missing ENGINE binding stay on the residual: no request reached any engine, and the console-side rows in the
  // same ring name them.
  ...
  // cf-rediscover-denied is the ENGINE'S OWN 403 (the refusal body is one of its shapes: engine-capability,
  ...
  // address where no engine is deployed. Splitting the two avoids sending support to fix a role on an engine
  // that may not exist.
  ...
  // remedy is the ADDRESS. This is distinct from cf-rediscover-transport, whose note says the call got no answer
  // at all. A lapsed Cloudflare Access session, the other statusless throw, deliberately has NO member: it
  // records nothing (the ordinary overnight tab), and a member with no producer reads like coverage.
...
// featureOutcome -- feature-probe only (13). THE VERDICT THE CONSOLE REACHED, which is the field the whole
...
//                           built yet. This is the common conflation this row exists to separate.
...
export const CLIENT_DIAG_FEATURE_OUTCOMES = [
  ...
  "not-implemented",
  "server-error",
  // forbidden is a 403 THE ENGINE ITSELF REFUSED, decided by the SHAPE of the refusal body (engine-capability,
  ...
  // nothing is deployed at that address and a foreign host refused. This is distinct from `forbidden`, whose
  // note assumes the remedy is never the engine's address -- exactly the one case here where the address IS the
  // fault.
  "refused-not-by-engine",
  "rate-limited",
  "network",
  "not-an-engine",
  // engine-binding-absent: no request reached any engine, because the console's OWN worker has no ENGINE
  // service binding and answered the call itself. It is a console deploy fault and is NOT `network`, whose note
  ...
  // REACHED ANY ENGINE. On the bare status this would land on `server-error`, whose note says this engine is
  // broken and saw the call, sending support to read refusals that cannot exist -- the inversion of the state,
  // which is an engine that is absent or down reported as up and refusing.
  // IT HAS A SECOND PRODUCER: a 5xx with an HTML body, in the PROXIED topology. The header gate only catches a
  // 500 the console worker LIVED to answer; when the
...
// skewClass -- console-skew only (8). THE CONSOLE'S BUILD, EXPRESSED AS ITS RELATION TO THE ENGINE. A raw
...
// fieldFamily -- contract-skew only (32). WHICH family of data broke. A closed PRODUCT vocabulary,
...
export const CLIENT_DIAG_FIELD_FAMILIES = [
  ...
  // `vendor-mark` is deliberately absent: its only possible producer in the console is guarded on a
  // CONSOLE-SIDE CONSTANT (every caller passes a mark key off the console's own integrations catalogue, and every
  // key in it resolves), so no engine of any version could ever fail to satisfy it, and the engine never names a
  // vendor. A contract-drift family for it would have no producer.
  ...
  //
  // ADMITTED HERE BECAUSE THE ALTERNATIVE IS SILENCE, not a downgraded row: a field family the console emits
  // that this list does not hold makes projectRecord() return null, which drops THE WHOLE RECORD on arrival, so
  // every browser record of a malformed roster row would otherwise vanish on its way into the support pack and
  // the pack would read clean.
  "downpipe-config",
  ...
  // The TOKEN-SOURCE CAPABILITY families, one per capability, in lockstep with the console. A single combined
  // flag would fire only when the engine advertised NOT ONE capability, and a subset is not zero: an engine
  // offering cf-config but not Workers would record nothing and read like a healthy one, which is exactly the
  // build a rollback lands on. fieldFamily is in the ring's tuple key, so the SET of rows is the evidence and
  // the families do not coalesce.
...
// would leave a clean partial drill and an aborted one carrying identical evidence.
...
// drillFact -- fleet-drill only (6). WHICH of the session's counts the row carries;
...
// The following vocabularies mirror the console's (src/lib/client-diag/vocab.ts). Every member here is one the
// CONSOLE emits: a member missing from this file is a row the receiver DROPS on arrival, so the browser
// recorded the fault, the customer sent the pack, and support sees nothing.

// ownerActionCode -- owner-action-refusal only (12). WHY a dual-control approve/reject did not go through.
...
//
// expired / already-decided both cover an action that no longer exists to act on, which these routes return as
// a plain 400 with no distinguishing status. The console re-reads its own inbox after a refusal: the listing
// drops terminal records, so an action that is GONE reached a terminal state, and the record's expiresAt says
// which one. The two have opposite remedies (propose it again, versus read who decided it).
//
// identity-unresolved: the refusal landed while whoami had not resolved, so the console held NO identity to
// judge the caller by. Without it an unresolved caller would be recorded as "not-owner", which would assert a
// fact nothing established. unreachable: the call got no answer from an engine at all, so no engine-side record
// can exist.
//
// bare-token: the caller is on the ADMIN_TOKEN break-glass, which canApproveOwnerAction refuses by design (dual
// control needs an attributable approver). The console holds that fact before it calls, rather than folding it
// into the generic engine-refused. answer-unreadable: the engine answered 2xx and the body would not parse,
// which on an approve means the action WAS taken; recording it as `unreachable` would assert no engine saw it,
// no engine-side record of it can exist, sending support away from an action that has already run.
// not-owner is recorded only off the engine's own 403, never inferred from the console's own role mirror, and
// the action's fate is read only for a caller whose listing is complete (an owner), because the listing is
// caller-scoped and absence from it is otherwise true by construction for everybody else.
//
// AND A BARE 403 IS NOT ONE FACT: THIS ENGINE ANSWERS 403 FOUR WAYS ON THE SAME CALL, and the console splits
// them by the SHAPE of the body this engine sends. not-owner is the route capability gate
...
// approve, so the caller being told no is a perfectly good second owner and a not-owner row there would be a lie.
...
// cspDirective / cspBlocked -- csp-violation only (7 + 5). The browser blocked a resource under the
...
// `frame-src` is deliberately absent: the console's own policy (worker.ts buildCsp) states no frame-src and
// the console embeds no frames, so the browser can never name that directive to it and the console can never
// send this member. An injected off-origin frame is still carried, as `other`.
...
// handoffClass -- handoff-dropped only (3). WHERE a wizard or deep-link hand-off lost the operator's pick.
// Twin of the console's CLIENT_DIAG_HANDOFF_CLASSES. A vocabulary member with no producer reads like coverage;
// it is not, so a class is retired the moment no console build can ever send it.
//
// wizard-spec-account-absent / editor-refused-account-absent split one loss into its two distinct outcomes: an
// account can go missing from a binding-backed source spec on the way from the wizard's discovered-row picker
// to the request the console sends, or the account id can be entirely absent when the advanced editor tries to
// save. wizard-spec-account-absent is the wizard SENDING an accountless spec (there is an engine 400 to go and
// find); editor-refused-account-absent is the advanced editor refusing the save locally and making NO REQUEST AT
// ALL. Same kind, same screen: only the class says which happened, and therefore whether the engine ever heard
// about it.
...
// and the class cannot answer. Three symptoms arrive in one breath (lost wizards, an engine URL that will not
// stick, an auto-refresh that re-enables itself against a motion pause) and they are three surfaces, so they
// are three rows. The draft ID is never recorded: a draft id can carry a run id.
...
// guard fires and a healthy browser reads as raf-frozen; without this member an ordinary backgrounded tab would
// manufacture the same false positive and coalesce with a genuine freeze. Telling it apart is what makes
// raf-frozen believable.
...
// The engine-side wrapper (CLIENT-ASSERTED PROVENANCE): source/receivedAt/engineAttempts are stamped by
...
export interface ClientDiagnosticsSection {
  ...
  // consoleBuild: WHICH CONSOLE BUILD the browser was running. The pack also carries engine.version; without
  // consoleBuild an audit row rendering unrecognised target (newer engine?) could not be answered with deploy
  // console X, because the console's own version would not be in the pack at all.
  //
