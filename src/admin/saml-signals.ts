// THE SAML PIPELINE'S ISOLATE-LOCAL SIGNAL LEDGER.
//
...
// call site:
//
//   The SOC question: "was last week's SSO failure spike an ATTACK?" XML-Signature-Wrapping (a second
...
//
//   The silent degradation of a SUCCESSFUL sign-in: a misspelt verified-flag attribute silently drops
...
//
//   The start path: a sign-in that dies while BUILDING the AuthnRequest never reaches the ACS, so it
...
// aggregate the pack already carries. The same idiom as format/integrity-fault-ledger.ts and the IdP cert
// health ledger, and for the same reason.
//
...
// The ATTACK SHAPES. Each is recorded at the refusal site that KNOWS the shape -- never inferred from a
...
export const SAML_ATTACK_SHAPES = [
  ...
  "saml-shape-verified-wrong-target", // a signature VERIFIED but over the WRONG NODE (the verified element is not the assertion we would consume): the single most alarming shape in the list
...
// The SILENT DEGRADATIONS of a sign-in that SUCCEEDED. No failure record exists for any of these today
...
// The START path. recordSsoFail is written only by the ACS (callback) wrapper, so a sign-in that dies
...
// (declared but not otherwise wired -- the exact failure mode this signal exists to close); the URL class is new.
