...
// verdict, and the SIEM audit-feed grant state). support.ts assembles the bundle from these
// gatherers; each keeps its own redaction contract (see the per-function comments). The
// control-plane recovery latch and export-lag gatherers live in support-sections-recovery.ts.
...
// The restore-receipt vocabularies, taken from the module that CLASSIFIES at the fault site so the
// pack's gate and the classifier cannot drift apart.
...
// The media-restore gates: the closed fault classes, the per-apply conflict-digest cap, and the bare
...
const CONFIG_EVENT_ALLOWLIST = new Set([
  ...
  // Control-plane lifecycle: a recovery export was written, the plane was found empty (amnesia latched),
  // the no-authority resume slice ran, or a break-glass reconcile rebuilt the plane. Without these the
  // whole amnesia/auto-heal/reconcile lifecycle would be absent from the excerpt.
  "control-plane-exported", "control-plane-empty", "control-plane-resumed", "control-plane-reconciled",
  // control-plane-recovery-acknowledged: an authenticated owner acknowledged an organically-recovered
  // plane (the latch cleared with no import).
  ...
  // The KEY-CEREMONY FAILURE + PRESENCE-LOSS events. The engine recorded a secret APPEARING and
  ...
  // posture-override-set: an owner RISK-ACCEPTED a failing posture check, which makes a red check read
...
// Beyond the owner-#1 deploy/detach/dest markers, the latest restore outcome (restore-apply-incomplete
// forensics), IdP connection change (sso-disruption attribution), and approved config change (the
// notify-broke-after-a-config-change correlation) are each bot-signal inputs that a 40-event churn of
// role/owner-action noise could otherwise roll out of the excerpt. One extra capped action-filter query
// runs for each, only when absent from the recent window.
...
type AuditExcerptEvent = {
  ...
  ts: string;
  action: string;
  // actorEmail is READ (never projected) so the excerpt can carry the ATTRIBUTED boolean: the console
  ...
  target?: {
    ...
    connKind?: string;
    op?: string;
    // The restore-apply summary fields (redaction-safe counts + booleans; projected by restoreApplyFields).
    complete?: unknown;
    allVerified?: unknown;
    ...
    d1Total?: unknown;
    d1Verified?: unknown;
    // The restore-receipt target (projected by restoreApplyFields): the D1 fault LOCUS + class,
    ...
    changeKind?: string;
    // The key-ceremony failure fields (the key-ceremony audit target; projected by keyCeremonyFields):
    // the CLOSED step (an env-var NAME, never a value) + cause, and Cloudflare's own status class + numeric
    // error codes. The media fields (the restore-receipt target; projected by restoreApplyFields): the
    ...
    // The access-policy target's enrichment (projected by accessPolicyFields) and the FORCED dest
    // removal's blast radius (affectedDownpipeNames, projected by destChangeFields). The cf-config
    ...
    reasonClass?: unknown; // the checker's CLOSED restore-rejection reason

    failuresByClass?: unknown; // per-CLASS counts of the records a restore could not land
...
// RESTORE_APPLY_COUNT_FIELDS are the redaction-safe INTEGER-COUNT fields a restore-verified anchor carries.
...
// restoreApplyFields projects the restore-APPLY summary from a restore-verified / restore-apply excerpt
...
// CF_CONFIG_SKIP_CLASS_SET is the closed cf-config restore-skip vocabulary, taken from the module that
...
// restoreRejectFields: WHY the checker turned a restore down. The restore-reject event was in the
...
function restoreApplyFields(e: AuditExcerptEvent): Record<string, unknown> {
  ...
  // configSkipReasons: the cf-config restore's per-item skip CLASSES. `skipped: 20` was an integer with
  ...
  // failuresByClass: the per-CLASS breakdown of a restore's failed records, re-gated on the closed
  // vocabulary here (defence in depth: anything outside the set is DROPPED, never propagated, so a future
  // change at the fault site cannot smuggle a free-text class into the pack). "My restore applied with 3
  // failures, which three?" used to be answerable only as "3", which put a dest token with no write scope, a
  // readback that could not PROVE bytes that in fact landed, a refused Cloudflare config surface and a video
  // that would not re-upload into one integer. The record NAMES and the per-record reason prose never ride.
  ...
  // mediaFaults: "some videos restored, some failed" is otherwise ONE anonymous {failures: n} integer.
  // The class is the whole remediation, and the classes call for OPPOSITE actions: over-size-cap is a PLATFORM
  // limit (recover out of band; retrying cannot help), upload-failed says retry, conflict-different-bytes says
  // the id is occupied by a DIFFERENT live asset and the operator must choose to overwrite or remap, and the
  // readback split separates readback-unreadable (a transient 5xx -- RETRY) from readback-mismatch (the bytes
  // WERE read and are wrong -- a real, durable restore failure). Those last two used to be byte-identical
  // verified:false. Closed class keys + clamped counts only.
  ...
  // mediaConflictDigests: on a conflict, the archived + live SHA-384 PAIR, so a "that image WAS ours"
  ...
  // d1Fault: the D1 restore's fault LOCUS. "the restore said it worked and half our tables are empty"
  // was one anonymous `failures` integer. failedBatchIndex of batchTotal SIZES the loss (batch 1 of 2 = roughly
  // half the rows never landed) and the class routes the remedy: `constraint` is the customer's own schema
  // rejecting our rows, `target-not-empty` is a REFUSAL that wrote NOTHING AT ALL (the restore is safe and the
  // fix is to drop the target), and those two used to be the same evidence. residualTableCount is what is
  ...
  // d1SchemaObjectsFiltered: the schema objects a subset restore SILENTLY dropped -- a table restored
  ...
  // metadataFieldsDropped: the record RESTORED and a piece of its metadata did not. A KV key whose
...
// keyCeremonyFields projects the KEY-CEREMONY FAILURE evidence off a key-install-failed /
...
//           evidence: none.
// cfStatusClass / cfCodes carry Cloudflare's OWN verdict: cfErr already extracted the numeric error
...
// secretAbsentFields projects the PRESENCE-LOSS event: WHICH key-presence boolean flipped true ->
...
// customRoleFields projects the DELETION CONSEQUENCE: how many member + group grants still named the
...
function destChangeFields(e: AuditExcerptEvent): Record<string, unknown> {
  ...
  // affectedDownpipeNames: WHICH downpipes lost their only proven copy on a FORCED removal.
...
// accessPolicyFields projects the `access-policy` audit target's enrichment. The target is FIELD-LESS
...
// fetchLatestForcedDestRemoval finds the newest FORCED removal of a destination that was the ONLY
...
// fetchConfigEvents returns { events, fetched }. `fetched`
...
export async function fetchConfigEvents(scheduler: DurableObjectStub): Promise<{ events: unknown[]; fetched: boolean }> {
  {
    ...
    // The FORCED removal of the only proven copy, retained on its own PREDICATE rather than on its
    ...
    const events = [...out.values()].sort((a, b) => a.seq - b.seq).map((e) => ({
      ...
      at: e.ts,
      action: e.action,
      // attributed: was this governance mutation made by a VERIFIED identity, or by the shared
      ...
      ...restoreApplyFields(e), // the restore-APPLY summary, the per-class media faults + conflict digests, and the per-class failure breakdown
      ...restoreRejectFields(e), // WHY the checker rejected the restore (a closed class, never prose)
      ...destChangeFields(e), // dest-config-set/cleared: the redaction-safe dest-change diagnostic signals
      ...accessPolicyFields(e), // WHICH governance policy moved, and in which DIRECTION (closed enums + booleans)
      ...configChangeFields(e), // config-change-*: the closed-enum changeKind (notify-config mutation legibility)
      ...keyCeremonyFields(e), // the ceremony step + cause it died at, and Cloudflare's own verdict
      ...secretAbsentFields(e), // WHICH key presence flipped true -> false, and (from the row's `at`) WHEN
      ...customRoleFields(e), // how many grants a custom-role DELETION floors to the viewer floor
    }));
    return { events, fetched: true };
  }
}

// fetchAuditStatus pulls the audit-chain VERIFY VERDICT into the pack. GET /audit/verify
...
export async function fetchAuditStatus(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  {
    ...
    return {
      ...
      // headTruncated: THE ONE BREAK CLASS THAT LEAVES `intact` TRUE. verifyChain walks the RETAINED
      // entries, so deleting the NEWEST ones -- the entries that record what the attacker just did -- leaves the
      // survivors perfectly linked while `intact` alone would read true with nothing beside it. The engine's own
      // head anchor (written on every append, never lowered by a rollover) proves the tail is gone, and its
      // verdict rides HERE, next to the verdict it qualifies, so `intact` can never stand alone against a
      // truncation the same call established. headTruncatedAt is the seq the anchor committed to.
      ...
      // headAnchorUnreadable / auditCountRecovered carry THE SAME REASONING AS headTruncated ABOVE, for the two
      // verdicts that would otherwise leave the pack asserting `intact: true` and a bare `auditCount` with their
      // caveats sitting in a DIFFERENT SECTION (schedDiag.storageAnomalies) -- a clean assertion on the row and
      // the reason in another block, which lets a reader consulting the audit block alone fail to tell an
      // engine whose head pointer is trustworthy from one whose count had to be RECONSTRUCTED, nor one whose
      // tail-deletion witness ran from one where it could not run at all.
      //
      // They ride BESIDE the verdicts they qualify rather than instead of them: `intact` still means the
      // retained chain links, and `auditCount` is still the best available count. What changes is that neither
...
// fetchAuditFeedState pulls the audit-feed INGEST GRANT state so support can see whether a SIEM PULL
// channel is wired and being used. The
...
export async function fetchAuditFeedState(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  {
    ...
    // Read the RAW stored value, not the clamped one. clampTs drops an EMPTY string to undefined, and
...
// fetchMetricsFeed mirrors fetchAuditFeedState for the METRICS ingest scope: a Prometheus-compatible
...
export async function fetchMetricsFeed(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  {
    ...
    const lastPullAt = pulls.length > 0 ? clampTs(pulls[pulls.length - 1]?.at) : undefined;
    const expiresAt = clampTs(j.grant.expiresAt);
    // The audit-feed block's twin, same reasoning, same three states (see fetchAuditFeedState above).
    const expiry = grantExpiryState(j.grant.expiresAt);
    const expired = expiry.readable ? expiry.expired : true;
    return {
      configured: true,
...
// ---------------------------------------------------------------------------------------------------------
// THE FAILED AUDIT-LOG EXPORT ATTEMPT.
//
...
// and when the forward failed the engine recorded NOTHING: the pack showed an intact chain, every event
// present, and no hint that the customer could not actually get any of it out. "We cannot produce our audit
// log" is the same class of harm as losing it, and the exported log is structurally incapable of recording its
// own failure to export.
//
