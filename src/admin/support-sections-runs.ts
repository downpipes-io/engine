...
// lifecycle, the deploy-identity baseline, and the DO-owned status facts). support.ts
...
// UNKNOWN_CODE is used by the update-availability classifiers below.

// STATE_REFUSED_CLASSES mirrors the closed StateRefusedClass vocabulary (sched/types.ts). The
...
// vocabulary. boundedIncompleteIds drops them (correctly: the pack must never carry an unknown key's
...
export async function fetchRunHistory(scheduler: DurableObjectStub): Promise<Record<string, unknown[]>> {
  ...
  for (const [id, rows] of Object.entries(byDownpipe ?? {})) {
    ...
    // the OLDEST FAILING row still in the ring -- the first status flip, whose error / causeDigest /
    ...
    runs[id] = selected.map((r) => ({
      ...
      // In-scope objects that vanished (were deleted) mid-crawl between list and value read (redaction-
      ...
      // Per-kind ATTRIBUTION: WHICH surface/object each incompleteness kind hit this run (redaction-safe
      ...
      // droppedUnknownCount: incompleteness KINDS on this row outside the closed MARKER_KEYS vocabulary.
      ...
      // Verify-at-seal verdict per run: redaction-safe status/tier/sampled/at + coarse reason,
      // with the three fields a SUSPECT verdict on a 50,000-record run actually needs. The verdict
      // is projected FIELD BY FIELD here, so a future DO
...
export async function fetchDriveBudgetYield(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  try {
    ...
    // A read fault is NOT honest absence (no yields recorded). Flag it so the two are distinguishable.
...
// fetchSchedulerSignals pulls the redaction-safe scheduler-liveness aggregate into the pack: the
// per-cron-tick OUTCOME ring (the false-green detector -- a tick that dispatched 0 of N due, overdrew its
// subrequest budget, crashed a pass, or missed ticks), the due-index parity snapshot, and the runlog
// counter vs the max run-history index (derived as counter < maxHistoryIndex). Every field is a
...
export async function fetchSchedulerSignals(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  {
    ...
    // listCaps: the DO's per-list page size + paging guard + the live history-ring
    ...
    // storageFaults: the DISTINCT persist-state
    ...
    if (j.storageFaults && typeof j.storageFaults === "object") {
      ...
      if (total > 0) {
        out.storageFaults = {
          ...
          // lastDownpipeId: WHICH downpipe's dp: record was being persisted when the put threw. A
    ...
    // schedHealth: the scheduler HOUSEKEEPING faults that are otherwise fully silent -- a best-effort
    ...
    // stateRefusals: the downpipes whose persisted dp: record this engine REFUSED to read at the
...
//
// It exists because the sentence is not safe to project. The update paths build it as
// `...the previously-live version is still serving...: ${msg(e)}`, which carries the raw Cloudflare
// deploy-driver error, so it can embed a URL, an account id or a script name. A 200-char clamp does not fix
// this: a clamp is not a redaction, it only bounds the length of the leak.
// A SUCCESSFUL update names no cause. Classifying one would fabricate a fault out of a healthy state, which is
// the noise failure this campaign refuses elsewhere: a support engineer must never be sent to investigate a
// clean apply. Only a non-success outcome is classified at all.
...
// platform strings the update lifecycle would otherwise put in the pack. The `reason` sentence is classified
// because it carries the raw Cloudflare deploy error (see updateReasonClass above); these two fields carry the
// same class of leak, so a 160-char clamp on either is not a redaction: it bounds the LENGTH of a leak, not
// its content.
//
...
export const SETTLE_STEP_DETAIL_CLASSES = [
  ...
  // THREE members are absent from this vocabulary for the SAME reason, and the reason is the
...
// fetchUpdateStatus surfaces the safe-apply update lifecycle + the engine-account-marked
...
export async function fetchUpdateStatus(env: Env, scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  {
    ...
    // READBACK_VERDICT_SET: the closed apply-time release-digest cross-check verdict.
    ...
    if (j.last && typeof j.last === "object") {
      const l = j.last;
      // The two DERIVED booleans below. `outcome` carries the honest closed values (rollback-failed,
      // rollback-failed-still-split, applied-unconfirmed), but a downstream reader -- the console badge,
      // the notify rules, the diagnostics bot's update-failed-or-rollback-needed signal -- that does not
      // recognise one of these closed members reads it as unknown and falls through to its default, which is
      // silence. These booleans are the unambiguous, version-skew-proof statement of the two facts that
      // actually matter, and they cannot be misread by a consumer that has not been taught the closed members:
      //
      ...
      out.last = {
        ...
        // selfCheckOk + settleTrace: the DECISION TRACE a rollback/keep-pending
        ...
        //
        // `detail` is NOT PROJECTED. probeSettleVerdict's flight arm (update-gate.ts)
        // builds `the canary flight errored: ${e.message}` from a RAW THROWN fetch error, so a Cloudflare/network
        // message -- which can embed a URL, a hostname or a script name -- would otherwise ride into the sealed
        // bundle. A CLAMP IS NOT A REDACTION: it bounds the LENGTH of the leak, not its content, so the raw text
        // is dropped rather than clamped. The step
        ...
        // reason is NOT projected: the update paths interpolate the
        // raw deploy-driver error into it (update-rollback.ts, update-apply.ts, update-ramp.ts,
        // update-orchestrate.ts all build `...: ${msg(e)}`), so a Cloudflare API message, which can embed a URL,
        // an account id or a script name, would otherwise ride into the sealed bundle behind a 200-char clamp. A
        // clamp is not a redaction: it bounds the length of the leak, not its content.
        //
        ...
        // confirmationPending: the update applied via self-check and is awaiting the hourly canary's
        ...
        // readback: the apply-time release-digest cross-check. A `mismatch` is incident-grade
        ...
        //
        // `detail` is NOT PROJECTED. Two of its three producers are engine-authored; the third
        // (update-apply.ts readBackUploaded's catch arm) is `detail: msg(e)`, the RAW
        // THROWN error from a fetch to the Cloudflare API, so a platform message carrying a URL, an account id
        // or a script name would otherwise ride into the sealed bundle. A clamp bounds the LENGTH of a leak, it
        // does not redact it, so the raw text is dropped rather than clamped. The closed class below carries the
        // whole diagnosis; the operator still reads the full sentence in their OWN console (it is on the API
        // response) and in a tail.
        ...(l.readback && typeof l.readback === "object"
          ? {
              readback: {
                ...
                // account id, script name), the same class of leak as `reason`.
    ...
    // lastConsole: the CONSOLE component's last self-apply outcome (the engine `last` above is the
    ...
    // history: the bounded update outcome ring (already capped DO-side at UPDATE_HISTORY_CAP), so a
    ...
    // available: the UPDATE-AVAILABILITY GATING VERDICTS. The pack carries version strings and the
...
// CHANNEL_ISSUE_CLASSES is the CLOSED vocabulary of why the signed update channel did not verify.
...
// updateAvailability projects checkUpdates into the closed, redaction-safe availability verdicts. It
...
// fails, and consoleDestSet drives the pack's statusSource ("live" vs "env-fallback"): a failed dest
// read leaves it absent, so the pack honestly reports env-fallback. Only counts / booleans / the dest KIND
...
export async function fetchStatusDoOpts(scheduler: DurableObjectStub): Promise<BuildStatusOptions> {
  ...
  try {
    ...
    // source:"deploy" (ensureDeployDestSeeded) is present:true with no
