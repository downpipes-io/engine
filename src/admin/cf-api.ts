// The Cloudflare API plumbing half of the in-product attach and key install (see attach.ts for the
...
// secret. attach.ts re-exports nothing from here; these functions are internal to this module.
//
...
import { CF_API_BASE as CF_API } from "../lib/outbound.ts";
...
// ---- The Cloudflare API fault, as bounded, redaction-safe evidence ---------------------------------------
//
// cfErr extracts Cloudflare's numeric error codes but callers keep only `why`, the joined human message,
// which is interpolated into a thrown Error that becomes an HTTP response and is not recorded anywhere else.
// Without the codes, an attach or a key install that fails with "Cloudflare: HTTP 502" during a Cloudflare
// incident is indistinguishable, remotely, from one that failed on a token scope: there is no way to tell an
// EDGE OUTAGE (wait) from a TOKEN PROBLEM (recreate the token) from the message alone. And
// `.json().catch(() => null)` masks a non-JSON body (an HTML edge error page, itself the signature of an
// incident) as a bare status.
//
// The numeric codes are Cloudflare's own fixed vocabulary and carry no customer data, so they are safe to
// carry. CfApiFault tags the throw at the site that knows, with a bounded, closed shape; the caller records
// the tag, never the message. The thrown message itself is unchanged, so every operator-facing string, test
// and HTTP response stays byte-identical.
//
// The raw body (which can embed account names, script names and Cloudflare prose) never rides. Only: a
// clamped HTTP status, its coarse class, up to 8 integer error codes, and a closed body class.
 ...
 * the body, never the body itself. A null body (the `.json().catch(() => null)` mask) is reported as
 * "non-json", since an HTML edge error page is itself the signature of a Cloudflare incident, not just a bare
 * status.
 *
...
async function diagnoseRead(token: string, accountId: string, scriptName: string, readWhy: string, fetchImpl: typeof fetch): Promise<string> {
  ...
  try {
    const resp = await fetchImpl(`${CF_API}/accounts/${accountId}/workers/scripts`, { method: "GET", headers: auth, redirect: "manual" });
...
export async function readDeployedBindings(token: string, accountId: string, scriptName: string, fetchImpl: typeof fetch, quiet = false): Promise<LiveBinding[]> {
  ...
  const resp = await fetchImpl(url, { method: "GET", headers: auth, redirect: "manual" });
  ...
  if (!resp.ok || body?.success !== true) {
    const { why } = cfErr(body, resp.status);
    // Tag both arms with the bounded fault, so the attach-refusal record can name a Cloudflare edge outage
    // (5xx + non-json body) apart from a token scope problem (4xx + code 10000). The messages are unchanged.
...
// interpolates the token's not_before / expires_on and Cloudflare's own wording, so it can never be recorded
// as-is. The cause can, and it is the whole diagnosis: a future Start Date, an expired token, and a token
// Cloudflare will not confirm at all are three different situations for the customer, kept distinct instead
// of collapsing into one undifferentiated "attach failed".
export async function checkTokenWindow(token: string, accountId: string, fetchImpl: typeof fetch): Promise<{ problem: string | null; cause?: AttachRefusalCause; tokenId?: string; expiresOn?: string }> {
  try {
    const r = await fetchImpl(`${CF_API}/accounts/${encodeURIComponent(accountId)}/tokens/verify`, { method: "GET", headers: { authorization: `Bearer ${token}` }, redirect: "manual" });
    ...
    // Date nobody was able to check. This is recorded as such, rather than presenting the unchecked window as
    // a good one.
    return { problem: null, cause: "window-unreadable" };
  }
}

// readTokenExpiry asks Cloudflare for a token's PUBLIC id and expiry, the two redaction-safe facts the
// credential lifecycle registry keys on, without ever capturing the value: a standing backend credential
// must be term-limited. It tries /user/tokens/verify first (a user-owned token) and
// falls back to /accounts/{id}/tokens/verify (an account-owned token); either answers with the token's own
// record when the bearer IS that token. A verify that cannot complete yields no facts, which the caller
// treats as "no expiry" and refuses, so an unreadable window never passes as a bounded one. Never throws.
export async function readTokenExpiry(token: string, accountId: string, fetchImpl: typeof fetch): Promise<{ tokenId?: string; expiresOn?: string }> {
  const auth = { authorization: `Bearer ${token}` };
  for (const url of [`${CF_API}/user/tokens/verify`, `${CF_API}/accounts/${encodeURIComponent(accountId)}/tokens/verify`]) {
    try {
      const r = await fetchImpl(url, { method: "GET", headers: auth, redirect: "manual" });
      if (!r.ok) continue;
      const b = (await r.json().catch(() => null)) as { success?: boolean; result?: { id?: unknown; expires_on?: unknown } } | null;
      if (b?.success !== true) continue;
      const facts: { tokenId?: string; expiresOn?: string } = {};
      if (typeof b.result?.id === "string") facts.tokenId = b.result.id;
      if (typeof b.result?.expires_on === "string" && Number.isFinite(Date.parse(b.result.expires_on))) facts.expiresOn = b.result.expires_on;
      return facts;
    } catch {
      // try the next endpoint; a failure on both yields no facts
    }
  }
  return {};
}

// discoveryTokenExpiryVerdict applies the term limit: an expiry must be present and no further out than
// maxDays. Returns null when acceptable, else the operator-facing reason.
export function discoveryTokenExpiryVerdict(expiresOn: string | undefined, maxDays: number, now: number = Date.now()): string | null {
  if (expiresOn === undefined) return `the token has no expiry. A standing account-read credential must be term-limited: recreate the token with an Expiry of ${maxDays} days or less`;
  const at = Date.parse(expiresOn);
  if (!Number.isFinite(at)) return `the token's expiry could not be read: recreate it with an Expiry of ${maxDays} days or less`;
  if (at - now > maxDays * 24 * 60 * 60 * 1000) return `the token expires more than ${maxDays} days from now (${expiresOn}). Recreate it with an Expiry of ${maxDays} days or less`;
  return null;
}
...
//
// A plain boolean would collapse four different worlds into the same false: a Cloudflare 403 (the token
// genuinely lacks the capability), a Cloudflare 500 (the token was never assessed), a 429 (the token was
// never assessed) and a socket reset (the token was never assessed). Collapsing them would let the caller
// refuse the change as "token-scope" -- a class whose MEANING is "the engine PROVED the token cannot do
// this" -- for cases where nothing about the token was actually established, sending an operator to re-mint
// a perfectly good token while a real Cloudflare outage or throttle goes unrecorded. Only `refused`
// establishes a fact about the token; the rest establish a fact about Cloudflare, and the caller must say so.
...
// Cloudflare's OWN error codes for "the API token you sent is not usable" arrive on an HTTP 400, not a 401,
// so a classifier that looked only at the status code would put them in `inconclusive` and tell the operator
// the token was fine -- the one state where the token IS the fault would become the one state where the
// engine says to leave the token alone. These codes are checked explicitly to avoid exactly that inversion.
// Numbers only; no Cloudflare prose is read.
...
export async function auditToken(token: string, caps: Capability[], fetchImpl: typeof fetch): Promise<{ cap: Capability; outcome: CapProbeOutcome }[]> {
  ...
  return Promise.all(caps.map(async (cap) => {
    try {
      const r = await fetchImpl(`${CF_API}${cap.path}`, { method: "GET", headers: auth, redirect: "manual" });
...
export async function putSecret(token: string, accountId: string, scriptName: string, name: string, value: string, fetchImpl: typeof fetch): Promise<void> {
  ...
  if (!resp.ok || body?.success !== true) {
    const { why } = cfErr(body, resp.status);
    // Tag the throw with the bounded fault (status class + Cloudflare's own numeric codes + the body class)
    // so the key-ceremony route can audit a half-applied install by cause instead of an unrecorded message.
    // The message itself is unchanged, so the operator-facing 400 is byte-identical.
...
export async function deleteSecret(token: string, accountId: string, scriptName: string, name: string, fetchImpl: typeof fetch): Promise<void> {
  ...
  if (!resp.ok || body?.success !== true) {
    const { why } = cfErr(body, resp.status);
    // The same bounded tag on the removal path. A break-glass-only switch that dies after deleting
    // OPERATIONAL_PRIVATE but before OPERATIONAL_PUBLIC would otherwise leave the engine half-posture with no
    // record of why.
    throw new CfApiFault(
