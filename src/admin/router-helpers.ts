...
// admin/restore) plus seal/verify-at-seal and seal/runstate. Extracted to break the
// router.ts<->spoke import cycles: the spokes import these helpers, router.ts imports the
// spokes, and neither imports the other. Definitions MOVED VERBATIM from router.ts; the
// hub re-exports them so callers that import them by name from router.ts keep working.
...
// recordAuthSignalEdge is the shared BEST-EFFORT recorder for a Worker-EDGE auth-signal: a defensive branch
...
// recordAuthzRefusalEdge posts ONE closed authorisation-guard gate from a router-decided refusal into the
...
// resolveDiscoveryTokenDetailed is resolveDiscoveryToken with the DO-fault flag the null hides.
//
...
export async function resolveDiscoveryTokenDetailed(scheduler: DurableObjectStub, env: Env): Promise<{ token: string | null; doFault: boolean }> {
  ...
  try {
    ...
    if (cfg && cfg.token !== undefined) {
      ...
      // lie this guards against, so it is counted like a DO read failure and reported as a fault.
    ...
    // The DO could not be read, so the resolver silently falls back to the env var -- or to null, which
...
// the route allocates the run in the DO, then seals it in the invocation's background.
// MOVED here from router.ts (the dispatch-split, MOVE-ONLY) so the route spokes can share the shape
// without importing router.ts; router.ts re-exports it so importers by name are unchanged.
export interface AdminRuntime {
  ...
  // remaining synchronous work to give it a scheduling window either, so it is not merely theoretically at
  // risk: two independent live drives against `probe` reproduced the loss
  // deterministically on passkey-session-mint-failed, every time the path was driven. Optional for the same
