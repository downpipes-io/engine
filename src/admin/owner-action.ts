...
//
// NOT EVERY OWNER-ACTION KIND REACHES THIS GATE UNCONDITIONALLY. The fourteen DO-side
...
import { APPROVAL_MIN_DWELL_MS, approvalDwellRefusal, type GovernanceDecision } from "./approvals.ts";
...
export type OwnerActionKind =
  ...
  // SIEM push destination: where the hash-chained audit trail is forwarded
  ...
  // posture flip is NOT a gated kind because the engine has no setter for it: the
...
export const HIGH_BLAST_ALWAYS_GATED: ReadonlySet<OwnerActionKind> = new Set<OwnerActionKind>([
  ...
  "push-dest-set",
  // Opening or repointing the OTLP metrics push destination is the same blast class: it chooses
...
// The approval-dwell floor (approvals.ts) must sit strictly below this TTL too, or a pending
// owner action could expire before the floor that gates its own approve ever clears. Asserted here (rather
// than in approvals.ts) because comparing the two constants without an import cycle needs the TTL's own
// module to do the comparing.
if (APPROVAL_MIN_DWELL_MS >= OWNER_ACTION_TTL_MS) {
  throw new Error("APPROVAL_MIN_DWELL_MS must be strictly below OWNER_ACTION_TTL_MS");
}
...
export interface PendingOwnerAction {
  ...
  // proposedBySubject is the proposer's STABLE principal AT PROPOSE TIME. The
  ...
  // lastAttempt is the LAST FAILED APPROVE/EXECUTE ATTEMPT on this record (see PendingConfigChange.lastAttempt
  // for the config-change twin). Without it a record that will not execute just sits there "approved", with
  // every refusal reaching one browser and nothing else; on the CONSUME side a replayed one-shot approval
  // would be refused with no durable trace at all. Records a closed outcome class and a time only;
...
// ownerActionTimestampUnparseable is the owner-action queue's twin of approvalTimestampUnparseable: the same
// lazy-expiry idiom, the same Number.isFinite guard, the same consequence. A queued owner action whose
// expiresAt does not parse never reads as expired, so an armed second-owner approval outlives its TTL with
...
//   - maker != checker on the STABLE SUBJECT axis (the authority axis), the SAME
...
export function canApproveOwnerAction(
  ...
  // Approval-dwell: only reached once every other refusal above has cleared, so an owner
  // action that is expired/terminal/self-approved/not-owner is classified as THAT, never as too-soon.
  // proposedAt is the record's own raise instant (the maker's proposal, never the checker's).
...
// under a config object). The OTLP metrics push destination (otlp-push-dest-set) carries its
...
// record of a spent action. It is the SAME projection the inbox listing applies, deliberately: the listing is
// the single declared answer to "which fields carry a live secret", and a second answer here
