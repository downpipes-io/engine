...
// supportability view + support-bundle download + ingest-credential mint/revoke. Each case body reads
// the shared locals off the RouterCtx the hub builds once, so the per-route capability/role gate
// runs inline. Imports shared primitives + status/preflight/support cores; never
  ...
  switch (`${req.method} ${sub}`) {
    case "GET /status": {
      ...
      try {
        ...
        // An empty roster is indistinguishable from a fleet the DO could not read, so /status reports
      ...
      try {
        ...
        // source:"deploy" is the synthetic record standing in for the
        ...
        // so an estate with no REAL console destination still reads its status from env facts.
      ...
      // Resolve the engine's SELF-STAMPED artefact SHA-384 (the real hash of its own
      ...
      // cfAccountId's DO-persisted fallback: read the
      ...
      // Engine-observed events: hand the DO the presence booleans +
      ...
      // CHECKED. This observation IS the owner's #1 keystone -- it is what mints the
        ...
        await recordDiagWrite(scheduler, "status-observation", async () => {
          const resp = await scheduler.fetch(doURL("/audit-status"), {
            ...
            body: JSON.stringify({
              ...
              cfVersionId: report.cfVersionId, // deploy identity, lets the DO record a redeploy (owner-#1), not just a software upgrade
            }),
    ...
    case "GET /support/bundle": {
      ...
      // accessPerimeter is threaded from THIS request: the edge injects cf-access-jwt-assertion on every
    ...
    case "POST /support/bundle": {
      // The console GENERATE path (REQUEST-SCOPED ONLY): identical to the GET download but it also
      ...
      const denied = gate(caller, "posture.read");
      if (denied) return denied;
      // Treat the POST as the MOST untrusted input in the pack. Cap Content-Length BEFORE parse so an
    ...
    case "POST /support/credentials": {
      ...
      if (caller.role !== "owner") {
        // A refused mint is recorded like a refused role write. The DO
      ...
      // attributes the grant's audit event (the /roles discipline).
      // CHECKED. The mint SHOWS the operator their bearer exactly once, below. If this persist is
    ...
    case "POST /support/credentials/delete": {
      ...
      // (the DO records the success at the commit point, only when a grant existed).
      // CHECKED. A dropped REVOKE is the dangerous direction: the operator is told the credential is
