...
// ---- STANDALONE one-click rollback (revert to the recorded known-good version) ----------------
...
// takes), so by design it needs NO second-owner approval (second-owner approval gates only the
// consequential apply/keep direction). Owner-gated (keys.ceremony) + rate-limited; the one-shot deploy token is required + never
...
export async function handleStandaloneRollback(a: RollbackRouteArgs): Promise<Response> {
  ...
  // A rollback is a real deploy. Its catch says "if no rollback is recorded there, nothing was
  ...
  try {
    ...
    if (!validateDeployToken(token)) {
      // "we tried to roll back at 02:00 and it refused." A rollback refused for want of a deploy
      // token, and one refused because there is no recorded target, are different tickets with different
      // fixes, so a bare 400 into a browser at two in the morning is not enough on its own.
      await noteUpdateGuardRefusal(scheduler, caller, sourceIp, "deploy-token");
      await noteUpdateAttempt(scheduler, "rollback-refused");
      return jsonError('paste the one-shot Cloudflare deploy token (the "Edit Cloudflare Workers" template); it is used once and never stored', 400);
    }
    ...
    if (r.outcome === "reverted" || r.outcome === "reverted-unverified") {
      // The rollback above just redeployed the known-good version, the same self-redeploy
      // that resets this DO, so both follow-up writes use the self-redeploy-safe helpers.
      stage = "persist"; // the revert applied
      ...
      // THE FAILED standalone rollback. The operator pressed "Roll back" on an engine they know is
      // bad; if the revert deploy does not land, a lifecycle record must still say so, rather than leaving
      // GET /update/status (and therefore the pack, the console badge and the bot) to show whatever the
      // PREVIOUS settle left there: very often "applied", on the very build the operator just tried to
      // escape, with only a 100-character clipped prose fragment on an `update-refused` audit row -- an
      // action name with no outcome -- as the record of the attempt.
      //
      ...
      // gone, and every consumer reads that from the enum rather than from a truncated sentence.
      stage = "persist";
