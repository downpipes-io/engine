// router-destinations.ts -- the archive-destination routes (the dispatch-split, MOVE-ONLY): the single
// console-set destination (view / verify-before-store set / re-verify), the multiple-destination
// collection (list / add+edit / remove / default), the guided-setup consolidated read and the email
// delivery test. Each case body was MOVED VERBATIM from handleAdmin's switch in router.ts; the only change
// is reading the shared locals off the RouterCtx the hub builds once, so the keys.ceremony gate on each
// write runs here exactly as it did inline. Imports shared primitives + the dest factory/probe helpers;
// never imports router.ts.
...
async function validateAndProbeDestConfig(env: RouterCtx["env"], rawConfig: unknown, scheduler?: DurableObjectStub): Promise<ValidatedDest | Response> {
  ...
  if (isInternalSinkHost(endpointHost)) {
    // The SSRF guard HELD, and nothing anywhere recorded that it had to. "Did anyone aim the destination
  ...
  switch (`${req.method} ${sub}`) {
    ...
    case "GET /destination": {
      ...
      try {
        ...
        // "Honestly absent" is exactly the trap -- the console then tells an account WITH a destination
      ...
      // The default MAY resolve to the synthetic source:"deploy"
    ...
    case "POST /destination": {
      ...
      if (validated instanceof Response) {
        // "We tried to add our S3 destination three times last week." EVERY refusal on this path -- a
      ...
      // Real-time alert on the destination surface (repoint = exfil, remove = destroy). Fail-open.
    ...
    case "POST /destination/verify": {
      ...
      try {
        ...
        // The engine's OWN config could not be read (a rotated wrap key). That is an `internal` test
      ...
      // "Verify destination keeps failing", and its quieter, worse sibling -- "verify says OK and our
      ...
      //
      // deleteProbe, objectLock and the object-lock probe's OWN closed unknown-reason plus a default-retention
      ...
      //                             These two are distinct and this record keeps them apart.
    ...
    case "POST /destinations": {
      ...
      const { submitted, storedSecret, worm, probe, pricing } = validated;
      // envDestConfigured is read HERE (the router is the one place
    ...
    case "GET /setup-state": {
      ...
      // Persist the in-DO demo marker on a DEMO_MODE engine (fire-and-forget, presence-
      ...
      // /setup-state is the guided wizard's whole view of the account, and every one of these five reads
      ...
      // A source:"deploy" record is present:true (ensureDeployDestSeeded)
    ...
    case "POST /email/test": {
      ...
      // The email platform's rejection code (an un-onboarded sender domain, an unverified sender, a
      ...
      // ({"ok":false,"reasonClass":"other"}) with three different remedies. The code now RIDES, gated to the
