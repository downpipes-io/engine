...
export interface RestoreTestsData {
  ...
  // within, in whole days, or 0 when none is stated.
  //
...
//   rtoEstimateSeconds / rtoBasedOnDrills are the RTO companion: the derived recovery-time estimate
...
export interface WormForReport {
  ...
  // the strong claim below depends on: ObjectLockEnabled does not retain anything by itself, so an archive
  // written with no retention header to a lock-enabled bucket with no default rule has no retention window
  // at all. Honestly absent means the rule could not be read, which is a cannot-confirm state and never a
  // substitute for either boolean.
...
export function buildSlaComplianceReport(
  ...
  const rows: SlaDownpipeRow[] = downpipes
    .map((d) => {
      ...
      // that point, so counting the whole period against it is what produced ~2160 expected against 3-4
      // successful on a downpipe a few hours old (the day-one ~0.2%-compliant misread this clamp fixes).
...
// is the platform's own tamper-evidence unless a confirmed object-lock flag is wired in), and
...
export function buildImmutabilityReport(status: StatusForReport, worm?: WormForReport): ImmutabilityData {
  ...
  if (status.destConfigured && status.destKind !== null) {
    destinations.push({
      ...
      // The destination KIND rides along because it decides WHICH MECHANISM the sentence may name: this
      // report is signed and handed to an auditor, and the mechanism differs by provider, e.g. Azure's is
      // version-level immutability rather than S3-style Object-Lock.
  ...
  const attestation =
    ...
    // This text is an ATTESTATION: a customer hands it to an auditor, so both branches have to say what is
    // true rather than what is reassuring. The present branch states what the operational key actually
    // buys: verification at seal and the hourly canary both use the run's own per-run key rather than the
    // stored operational one, so the key's real value is the UNATTENDED work it enables (scheduled restore
    // tests, automated drills, in-account retention pruning), not self-test or self-restore.
    //
    // The absent branch states not only what is missing but the assurance that REMAINS -- stating only the
    // omission would let an auditor reasonably conclude nothing is verified in this posture, when the part
    // they are there to check still holds.
...
//
// The claim this function guards is the strongest sentence in the report: "a compromised delete-credential
// cannot hard-delete an archive within its retention window". Testing bucketEnforces === true first and
// alone would assert that sentence on every lock-enabled bucket, including two states where NO archive has
// a retention window at all: an INVALID policy (which arms no lock headers) and NO policy (which arms none
// either). Object-Lock being enabled on a bucket does not retain anything by itself; only a per-object
// retention header or the bucket's own default retention rule does. The probe already reads the default
// rule (defaultRetention, router-posture.ts), and the report conditions the strong claim on one of the two
// actually being present.
//
// Every non-enforcing reading must be kept distinct, since collapsing them to one sentence -- "archives are
// tamper-evident but not write-once-locked" -- is true only of the invalid-policy reading. On a bucket that
// does not enforce Object-Lock there are no archives to be tamper-evident: R2 answers a lock-bearing PUT
// with 501 NotImplemented and AWS S3 with ObjectLockConfigurationNotFoundError, both measured. Neither
// store ignores the headers.
//
...
function wormProperty(worm: WormForReport | undefined, tamperEvident: string, kind: DestProvider | null): string {
  ...
  if (worm.configured) {
    ...
    if (worm.bucketEnforces !== false) {
      return `${tamperEvident}. NOTE: a WORM policy is configured and the engine could not read whether the ${store} enforces Object-Lock, so store-enforced Object-Lock is NOT in force as a claim of this report. This report states neither that archives are write-once-locked nor that they are not, and it does not say whether this destination accepts writes.`;
    }
...
// an auditor reads, and its configured-but-unenforced branch must not read "immutability is tamper-evidence
// only until the bucket enforces Object-Lock", which would tell the reader backups are still arriving. On a
...
export interface ChangeRequestsData {
  ...
  // THE SILENT EXCLUSIONS. This report is SIGNED, so a row the projector quietly dropped becomes a
...
export function buildChangeRequestsReport(events: AuditEvent[], period: ReportPeriod): ChangeRequestsData {
  ...
  // stamping the target the reader expects), and an unparseable ts is a corrupt row -- a bare `continue` on
  // either would leave a SIGNED report that under-reports with no hint that it did, so both are counted.
  let excludedShape = 0;
