// The CLOSED vocabulary of auth / RBAC "defensive-branch" events the engine records for the support pack (P2).
//
...
export const AUTH_SIGNAL_NAMES = [
  ...
  "admin-token-ratelimited", // the bare ADMIN_TOKEN break-glass compare was throttled: either its per-IP cap tripped (a guessing campaign) or the limiter's backing DO was unavailable (fails closed, so a legitimate operator's break-glass was denied too). RETAINED for the vocabulary's stability; the two causes also fire their own split names below
  // --- The AUTH-PLANE OUTAGE branches, which produce responses IDENTICAL to genuine auth verdicts ---
  //
  ...
  "limiter-verdict-malformed", // the per-caller rate limiter ANSWERED with a shape that carried no boolean verdict, and the request was ADMITTED (fail-open). The limiter is silently not limiting: an availability choice made deliberately, and otherwise invisible
  ...
  "break-glass-check-shape-invalid", // the break-glass RETIRE check ANSWERED and its verdict carried no boolean, so the token was denied fail-closed. Otherwise indistinguishable from an INTENTIONAL retire (which records nothing at all), which is why "my break-glass token stopped working" could not be answered
  ...
  //
  // The worst case is "I regenerated my recovery codes, it errored, and now NEITHER old nor new
  // codes work", and the one fact that answers it is: were the old codes ALREADY INVALIDATED engine-side?
  ...
  // The CORRUPTION half of recovery-code lockout: the two states in which recovery codes are broken for reasons the OPERATOR
  ...
  "session-revoked-idp-epoch",
  "session-revoked-sid", // a session was refused because the operator ended THAT session singly (V7.5.2): the sessionRevoked:<sid> row exists and is unexpired // an oidc/saml session was refused because it predates its connection's idpEpoch (the connection was disabled / deleted / its secret rotated - everyone on it re-auths)
  ...
  // --- Cloudflare Access verify sub-causes: the ~7 causes that collapse into cf-access-verify-failed ---
  ...
  // name below is the CLOSED skip/failure class already computed at the send site and otherwise discarded.
  ...
  // and it is otherwise indistinguishable from a genuine first-ever sign-in, which would make the check silently
  // adopt an unrecognised network as the new baseline and then alert on the operator's OWN next sign-in. This
  // count rides beside that alert, so the pack can answer "why did I get a new-location warning from my own office?"
  ...
  //
  // The ATTACK shapes (saml-shape-*). XSW wrapping (a second root, a second Assertion, a duplicate ID, a
  ...
  ...SAML_ATTACK_SHAPES,
  // The SILENT degradations (sso-*) of a sign-in that SUCCEEDED -- which is exactly why no failure record
  ...
  ...SAML_START_SIGNALS,
  // --- The SECURITY-RELEVANT REFUSALS that are recorded NOWHERE, not even in Workers Logs -----------
  //
  ...
  "reserved-binding-refused", // a restore named one of the engine's OWN reserved bindings as a write target (the confused-deputy attempt). Refused at the sink chokepoint, and otherwise invisible
  ...
  //   ship a counter that can only ever read zero -- dead evidence, which this vocabulary exists to avoid.
  //   The check itself is a CONTROL change (recompute the cues server-side at
 ...
 *
 * `n` EXISTS BECAUSE THE HOT-PATH RECORDER COUNTS WINDOWS, NOT EVENTS. recordAuthSignalThrottled
 * writes a name at most once per minute per isolate, and adding exactly one per surviving write would make
 * a fleet-wide lockout of 600 rejected session verifies inside one minute record `today: 1`, the SAME row
 * as one stale browser tab, and TEN TIMES SMALLER than a harmless ten-per-hour trickle: the tighter the burst,
 * the smaller the pack would say it was. The throttle instead ACCUMULATES the events it defers and flushes them
 * with a count, so the storage-write rate stays unchanged (the DoS property the throttle exists for) while the
 * numbers stay event counts. A non-finite or non-positive n is coerced to 1 (a bump is a bump).
 *
 ...
 *
 * AGEING THE RING ONLY ON WRITE IS THE ONE MOMENT IT DOES NOT NEED IT. Confining the shift to
 * bumpAuthSignalEntry alone would mean a ring only ever moves when the signal FIRES AGAIN -- and "it stopped
 * firing" is the entire question the ring exists to answer. A 10,000-failure burst that ended thirty days ago
 * would then stay frozen at days[0]=10000 for ever, and a projector assuming days[0] is today would derive
 * last24h=10000. That would not merely fail to discriminate the dead burst from the live one: it would AFFIRM,
 * in the direction that pages someone, that ten thousand failures happened in the last day, when the true
 * count is zero.
 *
 * Ageing therefore happens on the READ path too, against the clock the PACK IS BUILT WITH. A signal that
 ...
 * the honest one.
 *
 * THIS IS A UTC CALENDAR-DAY RING, and the projector's field names say so (support-sections-auth.ts). days[0]
 * is the current day TO DATE, not a rolling 24 hours, and calling it "last24h" would be a second false assertion: at
 * 00:30 UTC a burst ninety minutes old sits in days[1] and the pack would report zero in the last day.
 *
