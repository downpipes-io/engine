// router-sources-discovery.ts -- the account-wide source discovery slice of the router-sources spoke:
// the discovery-account/config types, the credentialed Cloudflare-API caller, the discovery-account and
// engine-account resolvers, the paginating lister, and the per-account product lister. Definitions are
// re-exported from router-sources.ts so importers are unchanged; this leaf imports only the Env type, so
// the boundary stays acyclic.
...
// the live Cloudflare API rather than fabricating a single account record from env.CF_ACCOUNT_ID: a
// short-circuit like that would accept a wrong-account or garbage-shaped token with a confident "1
// account visible" answer and leave multi-account browsing permanently inert on any estate that pins
// CF_ACCOUNT_ID. The caller that legitimately wants a no-API-call pin (resolveEngineAccountId, directly
// below) already short-circuits on env.CF_ACCOUNT_ID one level above its own call into this function, so
// asking the API here changes no caller's behaviour except the two verify/browse routes that need the
// token actually consulted.
...
// DISCOVERY_LIST_CAP is EXPORTED because the cap is itself a diagnostic fact: a listing that returns
