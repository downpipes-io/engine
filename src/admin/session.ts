...
// SESSION_IDLE_MS is the inactivity (idle) bound: a session with no activity for this long is
...
// STEPUP_FRESH_MS: a PASSKEY session authenticated within this window satisfies a
// step-up (re-authentication) requirement for a sensitive action WITHOUT a separate fresh-factor proof - it
// just authenticated with a UV WebAuthn assertion. This covers a just-logged-in passkey session AND a
// recovery-code sign-in (the break-glass operator's recent factor, minted as method "passkey", so they are
// never locked out of a sensitive action). An OIDC or SAML session does NOT get this exemption:
// the engine cannot verify the factor the IdP used and iat is the engine's callback instant, not the IdP's
// auth_time, so it is treated as single-factor and must present a passkey assertion; the one exception is
// enrolling its first passkey (stepUpCheck's "enrol-passkey" purpose). An OLDER passkey session must present
// a single-use step-up token (a fresh passkey assertion), or re-authenticate. Short (5 min) so the post-login
// exemption is narrow. iat is the ORIGINAL login instant (the idle-slide preserves it), so this measures time
// since the last strong authentication.
...
// TOKEN_SESSION_SUBJECT is the one subject a method "token" session may carry. The bare
// ADMIN_TOKEN is a static secret; the console exchanges it ONCE at POST /admin/session/bootstrap for a
// dynamically minted session and discards it, so the credential that rides every later request is this
// signed, expiring, revocable token rather than the secret itself. The session carries no email (the bearer
// has none) and this fixed subject, and it resolves downstream to the same all-or-nothing owner break-glass
// the bare bearer resolves to. It is refused the moment the token is retired in-app or disabled by env.
export const TOKEN_SESSION_SUBJECT = "bootstrap-admin-token";
...
export interface SessionPayloadV3 {
  v: 3;
  method: AuthMethod; // how the session was established: passkey | oidc | saml | token (the exchanged bootstrap bearer; never access, which is not cookie-borne)
  email: string; // the verified, canonical email (display/audit; the pending-invite bind keys on it on first auth); "" for a token session, which has none
  ...
  sid?: string; // the per-session id; absent only on a token minted before ids existed, which then cannot be ended singly
...
export interface VerifiedSession {
  ...
  exp: number;
  lastSeen: number;
  // sid is the per-session id, or null for a token minted before ids existed (V2, or an early V3).
  sid: string | null;
}
...
export interface SessionMintInput {
  ...
  slide?: { iat: number; exp: number; sid?: string };
  // sid is the per-session identifier: 16 CSPRNG bytes, base64url, minted here when absent and
  // PRESERVED across a slide (slide.sid), so one browser session keeps one id for its whole life. It is what
  // lets an operator see and end ONE session rather than every session for the account.
  sid?: string;
}

// SESSION_SID_BYTES is the per-session id length: 128 random bits, unguessable and unique for the account's
// lifetime, so a revocation keyed on it can never collide with another live session.
export const SESSION_SID_BYTES = 16;
export function mintSessionId(): string {
  return b64urlEncode(crypto.getRandomValues(new Uint8Array(SESSION_SID_BYTES)));
}
...
export async function signSession(rawKey: Uint8Array, input: SessionMintInput, now: number): Promise<string> {
  ...
  const payload: SessionPayloadV3 = {
    ...
    epoch: input.epoch,
    sid: input.slide?.sid ?? input.sid ?? mintSessionId(),
    iat,
    exp,
    lastSeen: now,
  };
...
// same set.
//
// verifySession itself still returns a bare null: that is an ANTI-ORACLE property for the CALLER (a client is
// never told which check it failed), and its contract, return type and the generic 401 the caller sees are all
// unchanged. The class exists so an idle-timeout window tuned too tight, an EXPECTED one-time re-auth after a
// token-shape upgrade, a signing-key rotation, and a MAC MISMATCH (a forgery attempt -- a security signal, not
// a config fault) are distinguishable from one another rather than collapsing into one undifferentiated
// silence. The class is recorded ENGINE-SIDE ONLY and never returned to the caller.
export const SESSION_FAULT_CLASSES = [
  ...
  "session-fault-idle", // idle past SESSION_IDLE_MS
...
export async function verifySessionClassified(rawKey: Uint8Array, token: string, now: number): Promise<VerifiedSession | { fault: SessionFaultClass }> {
  try {
    ...
    // A token session is the one shape with an EMPTY email: the bearer it was exchanged from has none.
    if (typeof email !== "string" || (email.length === 0 && p.method !== "token") || email.length > SESSION_EMAIL_MAX) return { fault: "session-fault-upgrade-reauth" };
...
function verifyV2Body(c: SessionCommonFields): VerifiedSession {
  return { email: c.email, subject: passkeySubject(c.email), method: "passkey", connId: null, epoch: c.epoch, iat: c.iat, exp: c.exp, lastSeen: c.iat, sid: null };
}
...
// Every refusal below returns its CLOSED class instead of a bare null. The body here is MAC-AUTHENTICATED
...
function verifyV3Body(p: Record<string, unknown>, c: SessionCommonFields, now: number): VerifiedSession | { fault: SessionFaultClass } {
  ...
  if (method !== "passkey" && method !== "oidc" && method !== "saml" && method !== "token") return { fault: "session-fault-shape-invalid" };
  if (typeof subject !== "string" || subject.length === 0 || subject.length > SESSION_SUBJECT_MAX) return { fault: "session-fault-shape-invalid" };
  // A token session carries exactly the fixed break-glass subject and no connection: a signed body
  // claiming method "token" with any other subject is malformed, so a token session can never impersonate
  // an attributable principal.
  if (method === "token" && (subject !== TOKEN_SESSION_SUBJECT || p.connId !== undefined)) return { fault: "session-fault-shape-invalid" };
  ...
  if (connIdRaw !== undefined) {
    ...
    // / saml:<connId>|... can never collide) is guaranteed BY THE VERIFIER, independent of the
  ...
  // lastSeen (idle timeout): a V3 token MUST carry a finite lastSeen. A missing/non-finite
  ...
  // A missing lastSeen is precisely the PRE-IDLE-UPGRADE token: its own class, so support can say "that logout
  // wave was the expected one-time re-auth after your update" rather than guessing at an idle-timeout problem.
  ...
  // sid is optional on read (an early V3 token carries none) but when present it must be the shape the mint
  // produces; a MAC-verified body with a malformed sid is a corrupt record, refused like any other bad shape.
  const sidRaw = p.sid;
  let sid: string | null = null;
  if (sidRaw !== undefined) {
    if (typeof sidRaw !== "string" || !/^[A-Za-z0-9_-]{22}$/.test(sidRaw)) return { fault: "session-fault-shape-invalid" };
    sid = sidRaw;
  }
  return { email: c.email, subject, method, connId, epoch: c.epoch, iat: c.iat, exp: c.exp, lastSeen, sid };
}
...
//
// The split matters because a single shared counter cannot distinguish two OPPOSITE diagnoses behind
// "every console save started 403ing": CONSOLE_ORIGIN UNSET on the engine (a deploy dropped the var, so
// originAllowed fails CLOSED and EVERY cookie-borne mutation 403s for EVERY user -- a configuration fault, fixed
// by setting the var) versus a FOREIGN Origin genuinely being presented (a cross-site attempt -- a security
// event). The raw Origin value NEVER rides; only which of the three branches fired.
...
// This is a DEFENCE-IN-DEPTH layer ON TOP of originAllowed (the strict-Origin guard), NEVER a
...
// router-account-session.ts imports the CLASSIFIER directly, because the route needs the closed fail
// class (header missing, cookie missing, mismatch) for its auth signal, not just true/false.
