// Platform-issued ingest credentials + the credentialed /support pull routes. support.ts
// keeps the bundle assembly/signing this module serves.
...
import { logSurfaceAuth, RATE_LIMITED_ERROR, rateLimitedResponse } from "./router-core.ts";
...
// classifyIngestCredentialCheck is checkIngestCredential's SINGLE implementation, returning the CLOSED reason
// the check failed instead of a bare false. The bare boolean left the /metrics scrape
// surface's own health invisible: a never-minted credential, an EXPIRED one and a rotated secret the scraper
// was never updated with all produced the same 401, so "Prometheus started getting 401s with an unexpired
// credential" was undiagnosable from the pack.
//
// ANTI-ORACLE (unchanged): the reason is recorded PACK-SIDE only. Every caller still answers the client with
...
//
// A raw NaN-blind comparison would render a grant whose expiresAt does not parse
// as a healthy, live credential. `expired` instead answers the question every existing reader is ACTUALLY
// asking -- "is this credential still usable" -- for which an unreadable expiry is `true`, because the gate
// refuses it. The discriminating fact rides BESIDE it rather than replacing it (the same shape used for the
// other refusal counters): `expiryUnreadable` appears only in the third state, so a reader that knows about it can say
// "unreadable" instead of "expired", and one that does not is at worst coarse rather than INVERTED. It never
// carries the timestamp text that would not parse.
...
// THE REFUSAL CLASS THE PULL SURFACE RECORDS, NOT ONLY THE GENERIC COUNT.
//
// classifyIngestCredentialCheck exists precisely so a refused presentation names WHICH of the five ways it
// failed: a never-minted credential, an EXPIRED one, and a rotated secret the scraper was never updated with
// would otherwise all produce the same 401, leaving "Prometheus started getting 401s with an unexpired
// credential" undiagnosable from the pack. The METRICS scrape already threads this outcome into
// METRICS_SCRAPE_OUTCOMES (admin/metrics.ts); the two /support pulls carry it the same way here.
//
// The SIEM audit feed -- the pull whose whole purpose is to be answerable -- can then say not only "you have
// had 4,000 rejected pulls since Tuesday" but whether the collector holds a stale clientId from a prior mint
// (re-issue the credential), a secret from before a rotation (update the collector), or nothing at all. Those
// are three different remedies on the CUSTOMER's side.
//
// The generic counter is KEPT and bumped alongside, in the SAME single write: everything that reads it (the
// pack projection, validate-admin-fault-evidence's vocabulary assertion) stays valid, and the discriminating
// name rides beside it rather than replacing it. bumpAdminCounters takes both names in ONE DO round-trip, so
// the anti-amplification property the throttle exists for holds -- one write per window.
//
...
const INGEST_PULL_REFUSAL_COUNTERS: Readonly<Record<Exclude<IngestCheckOutcome, "ok">, AdminCounterName>> = {
  ...
  // Kept APART from credential-expired on purpose. "Re-mint, the TTL ran out" and "the stored grant is
  // corrupt, re-mint AND look at what corrupted it" are different tickets, and blending them would repeat
  // the same discriminating-class mistake one member later.
...
// INGEST_PULL_RATE_LIMIT_MAX_PER_WINDOW caps SUCCESSFUL /support/audit-feed pulls per clientId, sharing
// the DO's generic per-caller rate-check window and matching the admin front door's own budget in shape
// (router-core.ts, rateLimited/RATE_LIMIT_MAX_PER_WINDOW: 120 per 60 seconds) -- a deliberately generous
// ceiling for a route a third party's own scheduler drives, not a hair-trigger. It is a distinct constant,
// not a re-export of the admin cap, so the two can be tuned independently even though they start equal.
// This applies to the audit-feed pull only: a SIEM collector polls on a schedule and can accidentally
// tighten its own cadence, where a vendor's manual diagnostics pull cannot realistically hit any per-
// window cap at all, so there is nothing to guard there.
const INGEST_PULL_RATE_LIMIT_MAX_PER_WINDOW = 120;

/**
 * ingestPullRateLimited asks the scheduler DO whether this clientId is still within the per-window pull
 * budget, keyed apart from every other rate-check bucket ("ingest:" is never used by the admin caller
 * buckets, "sub:"/"token", or the auth-ceremony per-IP bucket, "ip:"). Returns a 429 Response built by the
 * SAME shared builder every other 429 on this engine uses (router-core.ts's rateLimitedResponse: RFC 9457
 * body, whole-second Retry-After, advisory RateLimit/RateLimit-Policy headers), or null to let the pull
 * proceed.
 *
 * FAIL OPEN (deliberate, matching router-core.ts's rateLimited): an unreadable verdict or a thrown DO
 * fetch admits the request. This route's whole purpose is to be answerable to a collector on its own
 * schedule, so a limiter that failed closed when its own backing store hiccuped would turn an engine
 * fault into a SIEM outage -- exactly the harm a per-credential cap exists to avoid causing.
 */
async function ingestPullRateLimited(scheduler: DurableObjectStub, clientId: string): Promise<Response | null> {
  try {
    const resp = await scheduler.fetch(doURL("/rate-check"), {
      method: "POST",
      body: JSON.stringify({ key: `ingest:${clientId}`, max: INGEST_PULL_RATE_LIMIT_MAX_PER_WINDOW }),
      headers: { "content-type": "application/json" },
    });
    const verdict = (await resp.json()) as { allowed?: boolean; retryAfterMs?: number };
    // A missing/garbled verdict is treated as "admit", the same fail-open posture router-core.ts's
    // rateLimited uses for the identical failure shape.
    if (typeof verdict.allowed !== "boolean") return null;
    ...
    void bumpAdminCounter(scheduler, "ingest-pull-rate-limited");
    return rateLimitedResponse(RATE_LIMITED_ERROR, retryAfterSeconds, INGEST_PULL_RATE_LIMIT_MAX_PER_WINDOW);
  } catch {
    // FAIL OPEN: the limiter's own backing store is unavailable. Admit the pull rather than turn an
    // engine-side fault into a collector-visible outage.
    return null;
  }
}
...
export async function handleSupportPull(req: Request, env: Env, scheduler: DurableObjectStub): Promise<Response> {
  ...
  // Short-circuit an absent bearer BEFORE the DO round-trip: an unauthenticated flood of
  ...
  if (presented === "") {
    logSurfaceAuth("support-pull", "bearer-missing"); // V16.3.1: a log line costs no DO round trip, so the zero-cost refusal keeps its property
  ...
  try {
    ...
    // The grant could not be READ, so the pull is refused FAIL-CLOSED. The collector is locked out by an
    // ENGINE fault, not by a credential fault -- an opposite diagnosis that would otherwise be invisible.
  ...
  // Classify ONCE and keep the class, rather than running the same classifier through the boolean
  ...
  logSurfaceAuth("support-pull", outcome); // V16.3.1: the per-attempt line; the windowed counter below stays as it is
  if (outcome !== "ok") {
    // Without this, "our SIEM stopped collecting the audit feed" arrives with a lastPullAt lapse and NO cause.
    // The outward 401 is UNCHANGED and stays flat (the anti-enumeration property is preserved): the count is
    // recorded pack-side only, so support can say "you have had 4,000 rejected pulls since Tuesday".
    //
    ...
    // is the established idiom for a recorder on an attacker-reachable path: the counter counts the
    ...
    //
    // The DISCRIMINATING class rides in the SAME single write as the generic count, so the write rate
    // (and therefore the anti-amplification property above) is unchanged.
  ...
  // The audit-feed pull is the only leg with a per-credential budget: a SIEM collector polls on a fixed
  // schedule and can accidentally tighten its own cadence, where a vendor's manual diagnostics pull cannot
  // realistically hit any per-window cap. Checked and answered BEFORE the pull is recorded, so a refused
  // (over-budget) request never appears in the customer-visible pull trail as though it had been served.
  if (scope === "audit-feed" && grant !== null) {
    const limited = await ingestPullRateLimited(scheduler, grant.clientId);
    if (limited !== null) return limited;
  }
  // Record the pull (best-effort; the customer-visible usage trail on the grant).
  //
  // CHECKED: this trail is the ONLY evidence a credential is in live use, so a dropped write makes an
  ...
  if (scope === "diagnostics") {
    // accessPerimeter: a pull that ARRIVED means the collector got THROUGH the perimeter, but the header
    ...
    //
    // GUARDED: an unguarded await here would let a throw inside the seal (the diagnostics leg builds and SIGNS
    // the whole bundle) 500 the pull anonymously, so the vendor receives nothing and the engine records nothing,
    // which is the worst possible failure for the one surface whose entire job is remote diagnosis.
    try {
  ...
  // export in memory.
  //
  // Strict cursor: afterSeq is the collector's CHECKPOINT, so a malformed value must not be
  ...
  if (afterSeqParam !== null && afterSeqParam !== "") {
    ...
    if (!Number.isFinite(n) || n < 0) {
      // A corrupt persisted cursor produces WEEKS of 400s from a client bug that would otherwise be invisible
      // in the pack (the grant looks healthy; the feed simply never advances). The 400 is unchanged; the count makes it visible.
  ...
  // GUARDED: an unguarded await here would let an audit-export fault 500 the feed anonymously while the
  // grant keeps looking perfectly healthy -- the "our SIEM just stopped collecting" ticket with no cause at all.
  //
  // The collector's HALF of the export-attempt record. The counter above is a rate; this is the ledger
  ...
  // delivered (a silent audit GAP the headSeq alone could not reveal). gapBefore surfaces that
  ...
  // A SERVED retention gap is a compliance fact ("were audit entries destroyed before the collector could
  // take them?"). The feed already tells the COLLECTOR; nothing tells the pack, so an auditor could never prove
  // it from the engine's own evidence. A count only -- never a seq range, an event or a caller.
