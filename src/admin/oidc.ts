...
// exchange the code for tokens server-side and verify the id_token with the verifier (oidc-verify.ts).
//
...
export async function exchangeCode(
  ...
  // answers `temporarily_unavailable`. Reading and discarding that field is why "SSO broke overnight and we
  // changed nothing" would otherwise arrive with a pack saying only "connection". oauthErrorMarkFromBody parses
...
// verifyOptionsFor maps a connection + endpoints into the VerifyIdTokenOptions the verifier consumes.
...
// verifyIdTokenWithRotation drives the verifier and performs the ONE bounded JWKS refetch on a
...
export interface ResolvedPrincipal {
  ...
  groups: string[];
  issuer: string;
  // authContext: the IdP's advisory acr/amr/auth_time, bounded + redaction-safe. STRICTLY
  ...
  // claimDrops: the CLOSED counter names of the claims this id_token asserted and the engine bounded
...
export async function completeOidcLogin(
  ...
  // Read the IdP's advisory acr/amr/auth_time off the VERIFIED claims. Purely advisory + non-gating
