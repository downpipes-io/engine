// router-notify.ts holds the engine's outbound ALERT-ROUTING helpers: the two-phase (resolve channels in
// the DO, deliver via the channel adapters here where env lives, post the redaction-safe outcomes back for
// history) routers for a recovery-code alert, an auth-affecting change, a dual-control disarm, and a
// posture regression. All are fully fail-open. router.ts and its other spokes import them; this module
// imports neither.
...
//
// EVERY stage below can silently swallow a SECURITY alert. The resolve round trip can throw or answer
...
// The class is an ARGUMENT, not a lookup on emission.event. See the note over ALERT_CLASSES: the notify
...
//
// IT RETURNS ITS WRITE INSTEAD OF DROPPING IT. A bare `void bumpAdminCounter(...)` here let the counter
// leave routeEmission's promise chain at the moment it
// was fired. Every call site of every routeXxxAlert wrapper therefore held a promise that resolved BEFORE the
// only durable record of the dropped alert had been written, and the repair those call sites carry -- handing
// that promise to the runtime's waitUntil -- could not cover a write that was never in it. Driven under real
// workerd with the DO dispatch a tick late, the waitUntil hold read 0 of 3, byte-identical to the bare void it
// replaced, while the same alert with the write awaited read 3 of 3. Returning the promise is what makes the
// keep-alive at the call site mean anything. bumpAdminCounters swallows its own faults, so awaiting this can
// never throw into an alert path that is contracted to be fail-open.
...
async function routeEmission(env: Env, scheduler: DurableObjectStub, emission: NotifyEmission, cls: AlertClass, label: string): Promise<void> {
  ...
  try {
    ...
    // READ THE VERDICT, NOT JUST THE CHANNEL LIST. resolveNotify answers {now:[], digestedCount:0,
    // emission:null} when parseEmission REJECTS the emission -- BEFORE a single rule or channel is consulted --
    // so reading only `now` would file a rejection as "no-channel". Those are opposite remedies: a customer
    // with a live channel AND a matching rule would get the row that says "wire a rule", and no rule change
    // could ever help, because the alert never reached rule matching at all. The discriminator sits in the
    // response and must be read, not discarded.
    const parsed = (await resolveResp.json()) as { now?: NotifyChannel[]; emission?: NotifyEmission | null; egressAllowlist?: string[] };
  ...
  // THE RECORD STAGE MUST READ THE BODY, NOT THE STATUS. Reading `resp.ok` is not enough -- the DO's
...
export async function routeRecoveryAlert(
  ...
  // THE CLASS IS THE EVENT: a single class covering both events would coalesce them. A recovery code that
  // WORKED means somebody is signed in as an admin right now; rate-limited attempts mean somebody is being
  // kept OUT. They are opposite facts with different notify rules (the customer's rules key on the event, so
  // the "wire a rule" remedy differs), and one counter summing them would put both in one row. A single name
  // would also assert a fact neither event establishes on its own: a lawful owner break-glass with no
  // channel wired must not be filed as "recovery-abuse".
...
// routeSignInContextAlert fires the unusual-location notify (sign-in-new-context, V6.3.5): a
