...
//
// THE DESIGN (writer-side, no in-flight persistence): recordDiagWrite CHECKS the response. On a failure it
// NOTES the closed kind in an ISOLATE-LOCAL pending tally, and that tally is FLUSHED (piggy-backed) on the
// next diagnostic write that succeeds. So the gap is durably recorded the moment the DO comes back -- which
// is exactly when the pack is generated. An isolate that dies while holding a pending tally loses it: that is
// the irreducible floor (there is nowhere durable to put it during a total DO outage), and it is strictly
// better than zero durability.
//
