...
//   idpsecret:<id>  the do-plaintext client secret, as a CONFIG_WRAP_KEY envelope (or, on the back-compat
//                   floor and for a record written before the wrap shipped, the bare VALUE); write-only,
//                   resolveIdpSecret is internal-only, for the
...
import type { WrappedSecret } from "./config-secret.ts";
import { IDP_SECRET_AAD, resolveConfigSecret } from "./config-secret.ts";
...
// putIdpSecret stores the do-plaintext client secret under idpsecret:<id>. It is WRITE-ONLY from the
// console's perspective: there is no list/read route to it; only the flow (resolveIdpSecret) reads it,
// just-in-time, at the token exchange. Overwriting rotates the secret. This mirrors the console-set
// destination secret, including its at-rest form.
//
// THE VALUE IS AN ENVELOPE, AND THE WRAPPING HAPPENS ABOVE THIS LINE. The router seals the operator's
// secret under IDP_SECRET_AAD before it ever reaches the DO (router-identity.ts, POST /idp/connections),
// exactly as it seals a destination's secretAccessKey, so the Durable Object holds only ciphertext and
// never the wrap key. A bare string is still accepted: that is the back-compat floor (no CONFIG_WRAP_KEY
// configured) and the pre-encryption records already in storage, which keep working and migrate on the
// next write, the same lazy migration every sibling secret class uses.
export async function putIdpSecret(storage: KvStorage, id: string, value: string | WrappedSecret): Promise<void> {
  await storage.put(secretKey(id), value);
}

// getIdpSecret reads the STORED FORM of the client secret: an envelope, or a bare string on the floor.
// INTERNAL-ONLY, and it must NEVER be wired to a console-reachable route. Its remaining honest uses are
// PRESENCE checks (is a do-plaintext connection actually configured?), which is why it stays: they need no
// key and must not fail when one is absent. Use resolveIdpSecret to get a usable secret.
export async function getIdpSecret(storage: KvStorage, id: string): Promise<string | WrappedSecret | undefined> {
  return storage.get<string | WrappedSecret>(secretKey(id));
}

// resolveIdpSecret is the READ path the token exchange uses: it reads the stored form and opens it under
// IDP_SECRET_AAD. A bare string (the floor, or a legacy record) is returned unchanged. An ENVELOPE with no
// wrap key in hand THROWS out of resolveConfigSecret rather than being mistaken for a secret, which is the
// point: signing in with a base64url blob as the client secret would fail at the IdP with a message about
// the wrong credential instead of naming the dropped CONFIG_WRAP_KEY.
//
// Callers here are inside the DO and must never throw (handleOidcCallback's contract), so both call sites
// catch and turn the fault into a reason string.
export async function resolveIdpSecret(storage: KvStorage, id: string, wrapKey: Uint8Array | undefined): Promise<string | undefined> {
  const stored = await getIdpSecret(storage, id);
  if (stored === undefined) return undefined;
  return resolveConfigSecret(wrapKey, stored, IDP_SECRET_AAD);
}
...
async function resolveOidcClientSecret(
  storage: KvStorage,
  conn: OidcConnection,
  wrapKey: Uint8Array | undefined,
): Promise<{ ok: true; clientSecret: string | undefined } | { ok: false; reason: string }> {
  const mode = conn.secretRef.mode;
  if (mode === "do-plaintext") {
    // The unwrap can THROW (an envelope with no key bound, a damaged record, the wrong key after a
    // rotation). handleOidcCallback's contract is that it never throws, and a thrown UnwrapFaultError here
    // would leave the caller with a 500 and no SSO-failure row, so it is caught and named. The closed cause
    // rides in the reason for the pack's SSO sub-cause classifier; no key, envelope or plaintext enters it.
    let clientSecret: string | undefined;
    try {
      clientSecret = await resolveIdpSecret(storage, conn.id, wrapKey);
    } catch (e) {
      return { ok: false, reason: `stored client secret could not be opened at rest (${(e as Error).name})` };
    }
...
//      mode, opening the at-rest envelope under IDP_SECRET_AAD with the wrap key the caller passes; the
//      'secrets-store' and 'private-key-jwt' modes are not wired in this build and are refused with
...
export async function handleOidcCallback(
  ...
  doFetch: typeof fetch,
  nowMs: number,
  // wrapKey opens the stored client-secret envelope (IDP_SECRET_AAD). UNDEFINED is the back-compat floor and
  // is not an error on its own: a plaintext record still resolves. It only bites on an ENVELOPE, which then
  // fails loudly rather than being exchanged as a credential.
  wrapKey?: Uint8Array,
  ...
  const secret = await resolveOidcClientSecret(storage, conn, wrapKey);
...
export async function handleOauth2Callback(
  ...
  doFetch: typeof fetch,
  nowMs: number,
  // wrapKey opens the stored client-secret envelope, exactly as in handleOidcCallback above.
  wrapKey?: Uint8Array,
  ...
  let clientSecret: string | undefined;
  if (mode === "do-plaintext") {
    // Caught for the same reason as the OIDC branch: this orchestrator never throws, so an at-rest unwrap
    // fault has to become a reason string rather than a 500 with no SSO-failure row behind it.
    try {
      clientSecret = await resolveIdpSecret(storage, conn.id, wrapKey);
    } catch (e) {
      return { ok: false, reason: `stored client secret could not be opened at rest (${(e as Error).name})` };
