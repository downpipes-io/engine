...
// CONN_ORDINAL_SHAPE gates the OPAQUE per-connection ordinal. The connection axis is deliberately NOT
...
export async function fetchSsoFailures(scheduler: DurableObjectStub): Promise<Record<string, { count: number; lastAt: string }>> {
  {
    ...
    for (const [code, v] of Object.entries(j)) {
      // An out-of-vocabulary code does not DISAPPEAR. A classifier that grows a new code after the pack
...
// empty kinds dropped. A fetch/parse fault PROPAGATES to section() (recorded "error", not "empty"). Closes the largest idp gap (coarse SSO reasons, no
...
// the largest dark gap: fail-closed rate-limits (lockout / shared-NAT), SCIM 401/503/last-owner
...
export async function fetchAuthSignals(scheduler: DurableObjectStub, nowMs: number = Date.now()): Promise<Record<string, Record<string, unknown>>> {
  {
    ...
    for (const [name, v] of Object.entries(j)) {
      ...
      const lastAt = typeof rec.lastAt === "string" ? rec.lastAt.slice(0, 40) : "";
      if (count === 0) continue;
      // The TEMPORAL SHAPE, still counts only. days[0] is TODAY, days[13] thirteen days back, so a
      // 10,000-in-one-day BURST ([10000,0,0,...]) and a month-long TRICKLE ([1,1,1,...]) are finally two
      ...
      // moving at its cap, so every number below it is a LOWER BOUND -- a caveat that would otherwise be invisible.
      //
      // THE RING IS AGED HERE, TO THE CLOCK THE PACK IS BUILT WITH. The recorder ages it
      // only when the signal fires AGAIN, so a ring that has stopped moving is frozen at the instant of its
      // last bump. Copying it through verbatim would mean a burst that
      // ended thirty days ago still projects days[0]=10000 and reads as last24h. The
      // pack would assert ten thousand failures in the last day when the true number is zero, in the direction
      // that pages someone. Ageing on read makes "is it STILL happening?" answerable, which is the only reason
      // the ring exists: a dead burst now reads days=[0,...], last24h=0, last7d=0, with count and lastAt
      ...
      if (Array.isArray(rec.days) && rec.days.length > 0) {
        entry.days = days;
        // THESE FIELDS SAY WHAT THE CODE ESTABLISHES.
        //
        // days[] is a UTC CALENDAR-DAY ring, so days[0] spans [00:00 UTC, buildTime) -- a window whose true
        // length is uniform on (0, 24h] with a MEAN OF TWELVE HOURS. It was projected as `last24h`, a name that
        // asserts a ROLLING 24-hour window the ring cannot compute. A pack built at 01:00 UTC with a burst of
        // 300 failures NINETY MINUTES EARLIER (the gap's own ticket, "a 10,000-failure burst an hour ago")
        // reported last24h = 0. That is not a coarseness, it is a false negative pointing AWAY from a live
        // lockout, and it made a firing signal and a dead one byte-identical apart from lastAt.
        //
        //   today          the count SO FAR in the current UTC calendar day. This is exactly days[0], and now
        //                  it is exactly what it is called.
        //   last7dToDate   the current day-to-date plus the six whole days before it. Not a rolling week.
        //   hoursIntoDay   how much of days[0] has elapsed, so a reader knows how short that window is.
        //   last24hLower / last24hUpper  the BOUNDS a UTC-day ring can honestly place on a rolling 24 hours:
        //                  the true count is at least days[0] and at most days[0] + days[1]. The 90-minute-old
        //                  burst above now reads lower 0, upper 300 -- "it may all have happened in the last
        //                  day" -- which is the fact, and the dead burst reads lower 0, upper 0.
        //
        // All clamped ints off the engine's own build clock. No new leak surface, and no member of any closed
        // vocabulary changes.
        //
        // THE HOT-PATH NAMES ARE A LOWER BOUND, BY AT MOST 25. The session-verify / RBAC family is
        ...
        // by hand or check this projector's arithmetic. It was written to storage and never projected, and the
        // gap's own proposed evidence listed it as a pack field. It is an integer day number from the engine's
...
export async function fetchAuthPosture(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  {
    ...
    // doPlaintextSecretsMissingConns: WHICH confidential connections are secretless, by OPAQUE ORDINAL.
    ...
    // methodUsage: WHICH credential path the estate actually RAN on, and WHEN. Everything above is
    ...
    for (const method of ADMIN_AUTH_METHODS) {
      ...
      if (Array.isArray(e.days) && e.days.length > 0) {
        ...
        entry.days = days;
        // The same honest window names the auth-signal ring carries: `today` is the count so far in the
...
/**
 * fetchLockoutPosture pulls the DO's OWN lockout pre-flight into the pack.
 *
 * WHY THIS MATTERS. `GET /policy/lockout-preflight` is the one surface in the product built to answer
 * "can this account still get back in"; the console's require-access wizard is its only other caller
 * (`router-ops.ts:355`). Without this projection a customer whose role roster is corrupt (`rosterUnreadable`,
 * a count of the stored role records BOTH readers refuse to parse -- how an account loses its only Owner in
 * silence) would send a pack byte-identical to a healthy one. `passkeyOwnerEvidence` carries the same weight
 * one field along: it distinguishes `unknown` from `never-asserted` on the FIRST credential an account ever
 * enrols, so the pack can show which is true.
 *
...
export async function fetchLockoutPosture(scheduler: DurableObjectStub): Promise<Record<string, unknown>> {
  ...
  // given. This section's gate exists precisely to close that failure mode. So the
  // presence of the two REQUIRED booleans is the gate, and their absence is honest absence.
