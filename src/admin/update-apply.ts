...
//
// The two outcomes below exist because the closed enum must never assert the opposite of reality for the
...
//                        ITSELF FAILED. The engine is STILL SERVING THE BAD BUILD. Collapsing this into
//                        "rolled-back" would tell the pack, the badge and the bot that the customer had been
//                        safely reverted while they are, in fact, live on the dead version. It is the single
//                        most dangerous lie the update path could tell, so it gets its own member, not a
...
//                        the engine cannot prove the intended version is the live one. "applied" would claim
//                        proof that was never obtained; this claims exactly what happened and no more.
...
export interface UpdateApplyResult {
  ...
  // confirmationPending is additive: true only on an "applied" outcome that was
...
export interface SafeApplyInput {
  ...
  // settledHighWaterMark is the monotonic anti-rollback floor (the highest version ever successfully
  ...
  // knownRollbackTarget/knownToVersion are threaded straight through to verifyAndGuard: the
  ...
  // readbackMode selects the post-upload read-back gate's behaviour: "warn" (the default when
...
// decideKeep lives in update-types.ts, shared so settleAfterPromote AND settleAfterRamp (update-ramp.ts)
// can both call the SAME rule without the sibling orchestration files importing each other, imported +
// re-exported at the top of this module. Its behaviour there is the OPTIMISTIC rule, adopted after live
// drills rolled back healthy updates: a verified atomic promote is KEPT unless a completed flight is DEAD
// (a data-integrity failure); an ailing/pending flight (the post-swap DO-reset window) always keeps,
// confirmation pending, and the self-check no longer gates it -- see update-types.ts for the full
// rationale. settleAfterRamp applies its OWN additional baseline-regression guard on top of this
// (update-ramp.ts), which this guard does not weaken.
...
export interface PromoteResult {
  ...
  steps: StepLog[];
  reason?: string;
  // readback is additive: the post-upload read-back verdict (see update-types.ts
...
export async function planAndPromote(driver: DeployDriver, gate: Pick<HealthGate, "baseline">, input: SafeApplyInput): Promise<PromoteResult> {
  ...
  // target) runs in verifyAndGuard so a guard change lands in one place. The
  ...
  // 7..8. Deployment phase: upload (not live) then atomic promote. Extracted so the
...
// leaves the engine on the prior version. Split from planAndPromote so the
...
async function uploadAndPromote(
  ...
  // The READ-BACK GATE sits deliberately BETWEEN upload and promote: the one place a mismatch is free to
...
export interface SettleResult {
  phase: "settle";
  // rollback-failed is a REAL settle outcome and the type must be able to hold it. Widened to the full
...
export async function settleAfterPromote(driver: DeployDriver, gate: HealthGate, args: SettleArgs): Promise<SettleResult> {
  ...
  if (decision.keep) {
    ...
    // confirmationPending: the canary itself only decides on an ALIVE verdict; anything else
  ...
  try {
    ...
    // This branch must not return outcome "rolled-back": the engine is on the BAD version, and the enum
    // saying it had been reverted would be a lie nothing that reads this record could catch, since only a
    // prose `reason` field would tell the truth.
