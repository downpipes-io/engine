...
export function coarseCfReason(e: unknown): string {
  ...
  // The not-attempted case FIRST, because it is the more specific reading of "snapshot incomplete" and
  ...
  // ASVS V5.2.2: the media type-check refusal (validateMediaBytesType) carries its own precise message
  // (which media kind was found and which was expected); surface it rather than the generic fallback below.
  if (/media re-upload refused: the decrypted bytes are a recognised/i.test(m)) return m;
