// router-session.ts -- the session/identity RESOLVERS the admin gate depends on: the WebAuthn
// origin/rpId resolver, the DO-backed passkey-session verifier and the break-glass-retired resolver
// (both injected into authorise()), and resolveCaller/doWhoami that turn a positive verdict into the
// caller (role + custom-role + isOnlyOwner). This spoke imports no other router spoke (and never
// router.ts), so the boundary is acyclic.
...
// sourceIp is the edge address the router read off CF-Connecting-IP. It is threaded in because this
// resolution WRITES on a fresh engine (the first Access/passkey caller bootstraps as Owner, and that append is
// an attributed human audit row), and that row needs a real address rather than a null one that would
// otherwise read as a capture fault. It is SERVER-supplied (the edge header, never a client-controlled
// value) and is used for the audit row only; no gate reads it and no role is derived from it.
// fireInBackground: see router-identity.ts's identical helper. The bare-token limiter below records its
// edge signals as the LAST action before returning the verdict that denies the caller, and handleAdmin
// then answers the 429 with no awaited work left, so under real workerd the write would otherwise be
// abandoned with the request context. `runtime` is threaded from router.ts, where the resolver closure is
// built; a direct call with no fetch runtime falls back to a bare void.
...
export async function resolveCaller(
  ...
  verdict: { ok: true; method: AuthMethod; email: string | null; subject?: string; authIat?: number; groups?: string[]; identityProvider?: string; connId?: string },
  ...
  // SECURITY: ONLY the bare-token method gets the all-or-nothing Owner break-glass. An emailless Access
  // OR passkey caller must NEVER fold into this branch, since that would be a privilege escalation; the
  // token break-glass carries no IdP identity, so no groups and no identity provider, and its basis is
  // owner-token. A passkey caller is email-bearing and is resolved below exactly like Access.
  ...
  if (!email || !subject) {
    // The SILENT DOWNGRADE: an Access/passkey caller that reaches here without a usable email or subject
    // is failed closed to a least-privilege viewer -- which is precisely what an owner whose role was REVOKED
    // also looks like. Recording that the downgrade happened (never the email or subject itself) is what
    // lets "I am an owner but I am treated as a viewer" be told apart from a legitimate revocation.
    void recordAuthSignalEdge(scheduler, "caller-verdict-degraded");
    return { caller: { method, email: email ?? null, subject, role: "viewer", groups: [], ...(verdict.authIat !== undefined ? { authIat: verdict.authIat } : {}) }, isOnlyOwner: false, roleSource: "default" };
  }
...
// kept separate from resolveCaller so that function stays short. CUSTOM ROLE: when the
...
function buildCaller(
  ...
  subject: string,
  groups: string[],
  verdict: { identityProvider?: string; connId?: string; authIat?: number },
  ...
  return {
    caller: {
      ...
      ...(verdict.authIat !== undefined ? { authIat: verdict.authIat } : {}),
...
export async function doWhoami(
  ...
  // The ONE write this read-shaped route can perform is the first-Owner bootstrap, which appends an
  // attributed human audit row. Forward the edge address alongside email/subject/method/groups so that row
  // carries the same provenance every other human row does, rather than a null address that would otherwise
  // read as a capture fault. This is the router's own read of CF-Connecting-IP (never a client-supplied
  // value); it travels on the internal DO fetch only and reaches nothing but the audit draft, so the role is
  // resolved without it.
  ...
  // A NON-2xx here is not an error -- the body is parsed anyway, and a body without a `role` resolves the
  // caller to the viewer default. So a DO blip can tell a genuine Owner that their own console is read-only
  // ("every security-centre and credentials control is greyed out, owner only"), with nothing recorded that
  // the identity report was ABSENT rather than LOW. The behaviour is unchanged; the degradation is counted.
  if (!resp.ok) void bumpAdminCounter(scheduler, "degraded-read-whoami");
  // The same degradation, recorded in the aggregate the console's own symptoms are read against.
  // degraded-read-whoami is an admin counter (a total); this is the authSignals entry, which carries
  // firstAt, lastAt and a 14-day per-day ring, so "every control has been greyed out since Tuesday" can be
  // answered with a date rather than a number. The console's honest degradation to viewer is what the
  // customer is looking at; this is the only thing that says the engine, not their role, is why.
...
export async function verifyPasskeySessionViaDO(scheduler: DurableObjectStub, token: string, onSlide?: (token: string) => void): Promise<{ email: string; subject: string; method: AuthMethod; connId: string | null; groups: string[] } | null> {
  try {
    ...
    // TELL AN OUTAGE FROM A LOGGED-OUT USER. Every branch below fails the request CLOSED to a null
    ...
    //
    // The check keys on this closed verdict token rather than on payload shape: the DO returns
    // `{ email: null }` on all six of its ordinary rejection paths, so inferring "the DO answered" from
    // `res.email !== undefined` would fire session-shape-invalid on every EXPIRED COOKIE -- an exact-count
    // signal, so also a storage write per rejected request on the hottest path in the product -- while a real
    ...
    // A token session is the one method with an EMPTY email; every other method must carry one.
    const isTokenSession = res.method === "token";
    const shapeOk = typeof res.email === "string" && (res.email.length > 0 || isTokenSession) && typeof res.subject === "string" && res.subject.length > 0 && (res.method === "passkey" || res.method === "oidc" || res.method === "saml" || isTokenSession);
    if (!shapeOk) void recordAuthSignalEdge(scheduler, "session-shape-invalid");
    if (typeof res.email !== "string" || (res.email.length === 0 && !isTokenSession)) return null;
    if (typeof res.subject !== "string" || res.subject.length === 0) return null;
    const method = res.method;
    if (method !== "passkey" && method !== "oidc" && method !== "saml" && method !== "token") return null;
    ...
    // Idle-slide: when the DO re-minted the session (lastSeen was stale), hand the fresh token to the
    ...
    // The fail-closed DENIAL here is identical to an expired session, so without this signal a "the whole
    // team got logged out for an hour" outage would leave the authSignals aggregate empty during exactly the
    // outage it exists to explain. Fire-and-forget; never delays the denial. (If the DO is down this write is
    // dropped too, and recordAuthSignalEdge's own droppedWrites tally is what says the pack is
    // under-recording.)
...
export async function breakGlassRetiredViaDO(scheduler: DurableObjectStub): Promise<boolean> {
  try {
    ...
    //
    // THE ANSWERED OUTAGE, which underlies the whole "my break-glass token stopped working" ticket. There
    // are three ways the token gets denied here, told apart on the DO's own answer:
    //
    ...
    //                               does not throw on an HTTP error status, so this never reaches the catch.
    //
    // The third case gets its own row (break-glass-check-shape-invalid), so the pack can say "your retire
    // check is broken" rather than leaving the support engineer to guess between a rotten DO answer and a
    // deliberate retire that records nothing.
    ...
    // Records that the bare-token fallback was DENIED because the retire-latch check could not reach the DO,
    // so a "my break-glass token stopped working" ticket is diagnosable as a DO-availability event rather
    // than an intentional retire. Fire-and-forget; never blocks.
...
// adminTokenRateLimitedViaDO is the TokenRateLimiter the router injects into authorise(): the
// bare-token break-glass compare has no ceremony of its own to throttle it (unlike Access or the
...
// a limiter outage denying a legitimate operator - is visible in the support pack.
export async function adminTokenRateLimitedViaDO(scheduler: DurableObjectStub, req: Request, runtime?: AdminRuntime): Promise<boolean> {
  ...
  try {
    ...
    // The OVER-CAP arm is gated STRICTLY on an explicit allowed === false, not on any non-admit answer:
    // scheduler.fetch does not throw on an HTTP error status, so an up-but-broken limiter DO (its own JSON
    // 500, a drifted route) can answer without a verdict, and gating on that loosely would file it as "the
    // limiter is WORKING and someone is guessing your break-glass token" and send the ticket hunting an
    ...
    // The OUTAGE arm: the two are opposite diagnoses. Over-cap says "someone is guessing your break-glass
    // token"; unavailable says "your scheduler is down and it denied a legitimate operator's break-glass
    // because it fails closed". Labelling an outage as throttling would send every "my break-glass token
    // stopped working" ticket looking for an attacker.
    fireInBackground(runtime, recordAuthSignalEdge(scheduler, "admin-token-ratelimited"));
