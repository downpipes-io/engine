...
export function classifyRestoreFailure(e: unknown): string {
  ...
  // that telling an operator their data is corrupt when it is merely absent points them away from the replica
  // that would have restored them.
  //
  // The second half of the old justification is stale too. It said this must be integrity so a deleted
  // segment can "NEVER silently fall through to a 3-2-1 replica". A fallback here is no longer silent:
  // withRunDestFallback records every refusal, stamps a RestoreDestFallback onto any result it did not get
  // from the first choice, and threads the same fact into the signed receipt. Its own header says so in as
  // many words. Neither of the two reasons written here still holds.
  //
  // WHAT ACTUALLY BLOCKS FOLLOWING THE READER, measured rather than assumed. The reader may return
  // ExitDangling only when EVERY per-record failure was an absent segment (cmd/downpipe/restore.go,
  // perRecordExit / DanglingOnly); one altered record alongside them returns 2 or 4, so a real integrity
  // finding always wins. That predicate is sound there because the reader completes the pass and inspects
  // every failure. This engine cannot compute it: restore-apply.ts's phase-1 verify loop and its write phase
  // both `return { aborted: true }` on the FIRST failing record, and restore-plan.ts's dry run does the same,
  // so a single error escapes to the one catch in restore.ts and classifyRestoreFailure sees exactly that one
  // error. A run whose record 1 is absent and whose record 5 was altered aborts at record 1 and never reads
  // record 5. Making this reason fallback-eligible TODAY would therefore serve that run from a replica with
  // the tamper on this copy never looked at, which is worse than the wrong sentence it would fix.
  //
  // So the order of work is fixed: make the restore walk AGGREGATE per-record failures, so dangling-only is
  // computable here, and only then split this reason. Splitting it first is unsafe, and leaving it
  // unexplained is how the next reader concludes it was decided on purpose.
