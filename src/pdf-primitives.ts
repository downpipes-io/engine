...
// 72 (one inch), not 54. This is a CONSEQUENCE of dropping BODY_SIZE, not an independent taste
// change: at 9.5pt in the old 504pt column the longest line ran to 123 glyphs, WORSE than the 116
// it carried at 10pt, because smaller type in an unchanged measure packs more per line. Shrinking
// the type alone would have made the reading measure worse while the number that names the problem
// got better. 468pt of column holds ~114, under the gate's 115 ceiling.
...
// 60, not 46: at 46 the first body baseline landed 48.0pt from the trim on the evidence-pack
// fixture, under the 60pt floor the website's pdf-appearance-gate holds every published document
// to. These reports are the SAME artefact class as the trust-centre packs (the console does not
// build a PDF, it serves these bytes), so they are held to the same floor rather than a laxer one.
...
// 9.5, not 10, and the leading opens with it rather than staying put. Dropping the size alone would
// have made the page DENSER, which is half of the complaint these two numbers exist to answer; the
// ratio goes 1.40 -> 1.58. 9.5 is the ceiling the appearance gate holds, with zero slack, so this
