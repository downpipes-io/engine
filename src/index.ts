...
import { inboundTlsBelowFloor, plaintextRedirectResponse, plaintextRefusedResponse, plaintextVerdict, tlsFloorResponse } from "./admin/tls-floor.ts";
import { connectionSpecificHeaderRefusal, connectionSpecificRefusedResponse } from "./admin/connection-specific-guard.ts";
...
// Object (the seal never runs inside the DO). The cron driver body (drive)
// and its per-pass helpers live in ./cron/* to keep this entry thin; this
...
//
// WHAT THIS AVOIDS. /admin/health is the console's independent liveness probe, and the console's whole CORS
// fingerprint rests on it: "the unauthenticated health probe was reachable WHILE the data call was blocked" is
// what separates "set CONSOLE_ORIGIN on the engine" from "your engine is down". Gating /admin/health behind the
// SAME single origin-equality gate as every data route would mean that when CONSOLE_ORIGIN is wrong -- THE EXACT
// SCENARIO -- the browser blocks the health probe too, the probe throws, and the console records engine-
// unreachable, byte-identical to what a genuinely dead engine records. The one class that should tell support to
// fix CONSOLE_ORIGIN would then be UNREACHABLE IN THE STATE THAT NEEDS IT, and reachable only in a state where
// the advice is wrong.
//
...
// cross-origin-resource-policy:same-origin (ASVS V3.5.8) refuses a cross-origin NO-CORS embed of any
// engine response: a <script>, <img> or <link> in an attacker's page pointing at an /admin route can no
// longer pull the bytes into their document.
//
...
// So the policy is enforced on no-cors loads only, the console's CORS fetches are unaffected, and the
// control discriminates: the two permissive settings really do let the embed through, so the block above
// is the header doing work rather than the probe failing.
...
// The request-correlation header the engine echoes on EVERY response. The coarse `err:XXXXXXXX`
// id in the operational log is FNV-1a of ClassName:message and is irreversible, so an operator staring
// at a 500 in the browser has no thread back to the matching `wrangler tail` line without it. RESPONSE_ID
...
// noteDispatchFault is the ONE guarded sink for the last-resort dispatch recorders. It exists because a
...
async function handleScimRoute(req: Request, env: Env, cid: string, ctx: ExecutionContext): Promise<Response> {
  try {
    ...
    // "our IdP SCIM deprovision keeps 500ing" -- an offboarding that silently does not happen is a
    // SECURITY event, and without this the pack would carry no trace of it whatsoever.
...
//
// A bare `.catch(() => {})` here would mean that when the scheduler DO is unavailable -- usually the very
// reason the dispatch failed -- the completion vanishes SILENTLY. The downpipe would then dangle in flight
// until the 30-minute lease expires, so the pack would show the STALL (downpipes[].stalled) with NO recorded
// cause, byte-identical to a genuinely wedged seal. The two are different tickets ("the DO was flapping" vs
// "the seal is stuck"), and support could not tell them apart.
//
// Instead it is CHECKED (admin/diag-writer.ts): a throw or a non-2xx NOTES the closed kind "run-completion"
// in the isolate-local pending tally, which is flushed to the DO's bounded droppedWrites aggregate on the
// next diagnostic write that succeeds -- i.e. the moment the DO comes back, which is when the pack is
// generated. It stays FAIL-OPEN (it never throws, and the caller's path is unchanged); it only stops the
// loss from being invisible.
//
...
// bytes instead of stranding the downpipe in flight; canaryNow flies the canary immediately for
      ...
      ctx.waitUntil(
        (async () => {
          ...
          try {
            ...
            // The run row says only "destination configuration unreadable" and names no destination and
          ...
          if ("allDown" in sel) {
            // THE ALL-DOWN half: selectSealDestination probes each destination through destinationReachable.
            // A bare boolean there would discard the probe's real error (an expired credential vs a deleted
            // bucket vs DNS vs a 5xx) inside cron/seal-dispatch.ts before it could reach here, and recording a
            // GUESSED reason would be worse than recording none (support would chase the wrong system with
            // confidence). The reason is captured AT the probe, into the cron ledger's isolate-local rows;
            // this drains them so the RUN-NOW path records the same per-destination causes the cron tick
            // does, instead of leaving them to be discarded by the next tick's reset.
            // The drain carries the accumulator's own refusal count out with the rows: on a fleet wider than
            // DEST_PROBE_MAX the destinations it refused are the ones this very message -- "all destinations
            // unreachable" -- is about, and they never reach the DO by any other route.
    ...
    canaryNow: () =>
      ctx.waitUntil(
        runCanaryIfDue(env, schedulerStub(env)).catch(async (e) => {
          ...
          // A manual canary that dies leaves the operator staring at a stale bird with no cause anywhere.
...
async function handleAdminRoute(req: Request, env: Env, ctx: ExecutionContext, cid: string): Promise<Response> {
  ...
  // An Origin was PRESENT and did not match CONSOLE_ORIGIN, so corsHeaders returned {} and the browser
  ...
  try {
    ...
    // Without this, a 500 here exists ONLY as a Workers Logs line the vendor structurally cannot read. The ring
    // carries the closed surface, the closed route family (derived from the path PREFIX -- never the path, the
    // query string or an id in it) and the SAME irreversible 8-hex errId the log line carries, which is the one
    // join key between a customer's "the console 500s on Notifications" and the log the customer can export.
...
async function handleNonAdminRoute(req: Request, env: Env, url: URL, cid: string, ctx: ExecutionContext): Promise<Response> {
  try {
    ...
    // SELF-REFERENTIAL: this catch covers /support/diagnostics itself. A support endpoint that
...
export default {
  async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // One correlation id per request, echoed on EVERY response (withCorrelation) so an operator's
    ...
    // V4.2.3: a hop-by-hop header that survived the edge is refused before routing. See connection-specific-guard.ts.
    if (connectionSpecificHeaderRefusal(req.headers) !== undefined) return withCorrelation(connectionSpecificRefusedResponse(), cid);
    // V12.1.1: refuse a request that arrived over TLS 1.1 or older, before any route sees it. See tls-floor.ts.
    const belowFloor = inboundTlsBelowFloor(req);
    if (belowFloor !== null) return withCorrelation(tlsFloorResponse(belowFloor), cid);
    // V12.3.1: a request over plaintext http is refused (credentialed or sensitive) or sent to https; never served.
    const plain = plaintextVerdict(req, url);
    if (plain === "refuse") return withCorrelation(plaintextRefusedResponse(), cid);
    if (plain === "redirect") return withCorrelation(plaintextRedirectResponse(url), cid);
...
// buildAdapter (in seal/adapters.ts) is re-exported as the unchanged behaviour-neutral test
...
//
// destinationReachable + selectSealDestination (in cron/seal-dispatch.ts), flushDigests +
// runScheduledRestoreTest (in cron/notify-passes.ts and cron/restore-test-pass.ts) are re-exported
