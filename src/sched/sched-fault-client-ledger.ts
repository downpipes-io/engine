// clientDiagnostics: the BROWSER's own evidence, which rides IN the customer's support pack rather than in any
// background beacon.
//
// It is its own module because it is the one subject in this family that is not an engine fault at all: every
// field is a fact only the browser held. The closed vocabularies, the ring shape, the pure applier and the
// recorder that gates on them are all here, so the module boundary is the FAMILY boundary. The redaction rule
// these records are written under is stated once, in ./sched-fault-ledger.ts, and binds this file.

import { type FaultCountAgg, type LedgerStorage, bumpCount, writeLedger } from "./sched-fault-core.ts";

// ---- clientDiagnostics: the browser evidence that rides IN the pack ---------------------------------------
//
...
// The multi-item operations whose casualty list would otherwise live only in a modal.
...
// Where the update settle/verify journey ENDED.
...
// The console<->engine CONTRACT skew a partial upgrade produces.
...
// The console-side write/dispatch faults the ENGINE never sees.
export const CLIENT_FAULT_KINDS = [
  "evidence-write-failed", // the POST that records a drill-evidence row failed (404/500/network/quota) and was caught-and-discarded, so the dated trail silently has a hole
  "worker-unhandled-500", // the console WORKER (a separate script) threw and served an "internal error" page
  "worker-asset-miss", // the console shell asked for a chunk-<hash>.js the deploy no longer has (the blank page after a deploy)
  "worker-url-parse-failed", // the console worker could not parse its own request URL
] as const;
...
export function applyClientDiagnostics(prior: ClientDiagnostics | undefined, body: unknown, nowIso: string): ClientDiagnostics {
  ...
  // Settle journeys: counts, booleans and one closed terminal state. Never the unexpected outcome's
