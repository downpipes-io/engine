// route() is the engine's
// INTERNAL RPC surface (reached only by the router's own scheduler.fetch); it contains ZERO gate()/authorise()
// calls (the router pre-gates every public path, and each handler re-checks its own DO-side authority), so the
// dispatch carries no auth of its own to route. route() is a thin chain of per-subsystem sub-dispatch methods
// (routeCore, routeRbac, ...), each a switch over `${method} ${pathname}` that returns the handler Response for
// a key it owns or null ("not my route") so the next link is tried. Because the key sets across sub-dispatch
// methods are DISJOINT, the chain order is immaterial to which handler runs; the grouping is by subsystem for
// readability. RoutingMixin's `this` is SchedulerDOSurface (like every sibling mixin), so each sub-dispatch
// calls the owning handler (this.gatedConfigMutation / this.listDownpipes / this.idpConnCreate / ...) with a
// single, shared `this` binding. This module is a leaf this file's assembly depends on, never the reverse
// (madge 0 cycles).
//
// So no routing file exceeds the module-size guardrail, the 22
// sub-dispatch methods are split across four cohesive sibling sub-mixins, all chained by route() through `this`:
...
// regardless of which sub-mixin owns it), so the dispatch result for every key comes from exactly one owner.
...
import { purgeRunHistoryEntries, RUN_HISTORY_MAX_AGE_MS } from "./history-retention.ts";
...
// optionalLockKey reads the per-destination RUNLOG-lock key from an OPTIONAL request body. A
...
export function RoutingMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    // the 404. Every key resolves through exactly one owning sub-dispatch.
    async route(req: Request): Promise<Response> {
      const url = new URL(req.url);
      // The router-internal caller header is PRESENT but did not decode
      ...
      // too but carried nothing). Best-effort + fire-and-forget: the handlers still decode and fail closed, and
      // only the closed event name is ever stored (never the header value).
      const rawCaller = req.headers.get(CALLER_HEADER);
      // No bounding tally hangs off this decode: decodeCaller re-applies limits every producer of a Caller has
      // already applied (200 / 256, identically), so it drops nothing a real request can carry -- the only
      // input that could fire such a tally here is a hand-built header the product cannot emit. That drop is
      // tallied at the IdP boundary that carries the oversized claim instead. The fail-closed observation below
      // is unaffected.
      ...
      // errors, the /metrics scrape health, the structural WebAuthn fault classes and the
      // recorders' own dropped writes. A disjoint key set, so the position in the chain is immaterial.
      (await this.routeSupportDiag(req, url)) ??
      // HARNESS-ONLY test-fault store (TestFaultMixin): the internal arm/status/disarm/consume endpoints the
      // Worker's HARNESS_TEST_FAULTS-gated, Owner-gated /admin/test-fault/* paths call. A disjoint key set
      // (/test-fault/*), never reached in production (the Worker never routes here without the flag).
      (await this.routeTestFault(req, url)) ??
      (await this.routeControlPlane(req, url));
      if (matched !== null) return matched;
      // An unmatched key 404s SILENTLY. After a partial/mixed deploy (a newer console or Worker calling a
      // route this DO build does not have) a console screen 404s on every load, and without this count the
      // pack would show nothing at all -- "quiet" and "the whole surface is 404ing" would be the same picture.
      // Count the fall-through (the route CLASS only; never the path, method, body or header). Best-effort: the
      // 404 itself is unaffected.
      await recordContractFault(this.state.storage, "unmatched-route", "unknown-route");
      return this.notFoundResponse();
    }

    // notFoundResponse is the shared 404 arm route() and every sub-dispatch use (an unmatched key yields
    // exactly one "not found"/404 response).
    ...
    // config history attributed to the caller (gate OFF). Routing it outside this gate would let an inline
    // write escape the gate, the audit trail and config-history attribution. Modelled on removeDownpipe
    // (downpipe-delete): the downpipe.write capability is enforced by the router gate on POST, and at approve
    // by BOTH halves of the maker-checker pair -- CHANGE_WRITE_CAPABILITY gates the CHECKER through
    // canApproveChange, and approveChange's maker floor gates the PROPOSER against their live re-resolved
    // authority. That second half is why this method can still take no caller: the maker floor re-resolves the
    // proposer's live authority at approve time, so a lapsed proposer's queued mode change cannot apply just
    // because the checker's gate passed. See approveChange in scheduler-do-change-control.ts.
    ...
    // mutation, not a special 404). Idempotent (sets a field), so the replay discipline needs no special case.
    ...
    // purgeRunHistoryPass applies the ASVS V14.1.2 run-history retention window
    // (sched/history-retention.ts) to EVERY downpipe's ring in one pass: reads every `hist:` row, keeps
    // only the entries the pure purgeRunHistoryEntries retains, and writes back ONLY the rings that
    // actually shrank (an unaffected ring costs no write). maxAgeMs is read from the cron's own request
    // body -- never an env binding, since the DO receives no env -- and falls back to the constant
    // default when the body omits or malforms it, so a legacy caller with no body still gets the policy.
    async purgeRunHistoryPass(body: { maxAgeMs?: unknown }): Promise<{ downpipesScanned: number; entriesPurged: number }> {
      const maxAgeMs = typeof body.maxAgeMs === "number" && Number.isFinite(body.maxAgeMs) && body.maxAgeMs > 0 ? body.maxAgeMs : RUN_HISTORY_MAX_AGE_MS;
      const now = Date.now();
      const rings = await this.listAllByPrefix<RunHistoryEntry[]>("hist:");
      let downpipesScanned = 0;
      let entriesPurged = 0;
      for (const [key, ring] of rings) {
        downpipesScanned++;
        const { kept, purged } = purgeRunHistoryEntries(Array.isArray(ring) ? ring : [], now, maxAgeMs);
        if (purged > 0) {
          entriesPurged += purged;
          await this.state.storage.put(key, kept);
        }
      }
      return { downpipesScanned, entriesPurged };
    }
    ...
    async routeCore(req: Request, url: URL): Promise<Response | null> {
      ...
      case "POST /cf-config/discovery": {
        ...
        // The discovery record carries a fault-CLASS map and a truncation list, and it rides
        // straight into the support pack via downpipes[].cfConfigDiscovery. Storing it VERBATIM would only be
        // safe because the sole writer is the engine's own probe; sanitising makes that trust explicit:
        // sanitiseCfConfigDiscovery is the REDACTION CHOKEPOINT -- every surface id must be a member of the
      ...
      case "POST /cf-config/mode":
        ...
        // (attributed to the caller); gate ON queues a pending change (202) a second owner must approve. Routing
        // it outside this gate would let an inline write escape the gate, the audit trail and config-history
        // attribution.
      ...
      case "GET /runs/at":
        // Point-in-time resolution: resolve the run to restore from as of ?at=<rfc3339> for the
      ...
      case "GET /rto":
        // RTO estimate (the recovery-time companion to RPO/freshness): the per-downpipe + fleet
      ...
      // Run-history retention purge (ASVS V14.1.2 first increment, sched/history-retention.ts): the
      // cron driver's own age-based purge of every downpipe's run-history ring, INTERNAL (reached only
      // by the cron driver's own scheduler.fetch, like /tick-outcome above). The count-bounded ring
      // (RING_CAP) never trips for a low-traffic downpipe; this closes that gap without touching the
      // ring's existing count behaviour.
      case "POST /history-retention-pass":
        return this.json(await this.purgeRunHistoryPass((await req.json()) as { maxAgeMs?: unknown }));
    ...
    async routeIngestCredential(req: Request, url: URL): Promise<Response | null> {
      ...
      case "POST /ingest-credential/set": {
        ...
        // The DO re-checks OWNER from the forwarded caller at the commit point
        ...
        // Opening a vendor-readable pull surface is recorded at the
      ...
      case "POST /ingest-credential/clear": {
        ...
        if (body.scope !== "diagnostics" && body.scope !== "audit-feed" && body.scope !== "metrics") throw new Error("unknown credential scope");
        // The same owner re-check as /set (the revoke closes the surface, but it is
    ...
    // The SIEM audit-log push destination: the outbound egress config +
    ...
    async routeSiemPush(req: Request, url: URL): Promise<Response | null> {
      ...
      case "POST /push/delete": {
        ...
        // customer's SIEM stops receiving it -- and the console prompts the operator for a change
        ...
        // does not reach enforceChangeControl through gatedOwnerAction, and without the explicit call below the
        // reference the operator supplies would be DISCARDED. Asking for a change number and recording nothing
        // is worse than never asking, because
    ...
    // The OTLP/HTTP metrics push destination (mon-otlp): the SNAPSHOT sibling of the
    ...
    async routeOtlpPush(req: Request, url: URL): Promise<Response | null> {
      ...
      case "POST /otlp-push/delete": {
        ...
        // console prompts for a change reference before calling the route, and without the explicit call below
        // the reference would be collected and thrown away. Kind "otlp-clear", the console's own name for the
        // control.
        const caller = decodeCaller(req.headers.get(CALLER_HEADER));
    ...
    async routeRunlogRate(req: Request, url: URL): Promise<Response | null> {
      ...
      // The cron driver's SINGLE-FLIGHT tick lease: drive() takes it at the top of a scheduled()
      ...
      // RBAC. The role table is keyed by the verified Access email; the DO is the single
    ...
    // RBAC: whoami, the role table, the group->role mapping, and composable custom roles.
    // Returns the handler Response for a key this sub-dispatch owns, or null ("not my route") so route()
    // tries the next link.
    async routeRbac(req: Request, url: URL): Promise<Response | null> {
      ...
      case "GET /whoami":
        ...
        // sourceIp is the router's OWN read of the edge CF-Connecting-IP header, forwarded like the
      ...
      case "POST /roles/delete": {
        ...
        if (isScimOffboardCaller(rdCaller)) {
          // ASVS V7.4.1/V7.4.2: the SCIM facade's own internal caller bypasses the human
          ...
          // reach role-delete (a pure revocation, never a grant), and queuing it would produce a stuck state
          // (a pending change nobody could ever approve). Any OTHER
      ...
      // authenticated role, which the router enforces as roles.read, the sibling GET /roles gate
      // (the read arm below takes no caller, so this rule must be enforced by the router alone -- left
      // unenforced there, "any authenticated role" would be a rule that runs nowhere); the two writes
      ...
      // router as roles.read like GET /group-roles above (the read arm below takes
      ...
      // Audit. The chain lives in this DO (the single storage authority). The router records
    ...
    // Audit: the hash-chained event log append/read/verify/export and the status-diff hook.
    // Returns the handler Response for a key this sub-dispatch owns, or null ("not my route") so route()
    // tries the next link.
    async routeAudit(req: Request, url: URL): Promise<Response | null> {
      ...
      // Dual control. The approval state machine lives in this DO (the single authority), so
    ...
    // Dual control for restore + the drill-evidence log.
    // Returns the handler Response for a key this sub-dispatch owns, or null ("not my route") so route()
    // tries the next link.
    async routeRestoreApproval(req: Request, url: URL): Promise<Response | null> {
      ...
      case "POST /restore/reserve":
        // ASVS 2.1.6: the atomic gate-to-write reservation. The router calls this immediately
    ...
    async routePruneApproval(req: Request, url: URL): Promise<Response | null> {
      ...
      case "POST /prune-approval/reserve":
        // ASVS 2.1.6 parity: the atomic gate-to-write reservation, flipping approved -> applying
