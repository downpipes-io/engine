...
// the six founding ledgers whose vocabularies and recorders form one block.
//
...
//
// SPLIT AXIS. What must never separate is a closed vocabulary and the recorder that GATES on it: a member
// added to one half and not the other is a silent redaction hole rather than a compile error. That pairing is
// per FAMILY, so the FAMILY is the unit any refactor moves as a whole. The redaction rule these records are
// written under is stated once, in sched-fault-ledger.ts, and binds this file.
//
// What each ledger exists for:
//   ceremonyFaults  the DO-SIDE structural auth-ceremony failure classes. The Worker EDGE
...
//   recentErrors    a bounded ring keyed by the OPAQUE errorId the DO already mints and the console
...
//   contractFaults  the Worker<->DO contract faults that are SILENTLY COERCED today: a malformed
...
//   droppedWrites   the recorders' OWN fire-and-forget write failures. A diagnostic write that is
...
//   defaultDest     the reachability heartbeat for the ENV-DEFAULT destination. recordReplicationState
...
//   freshnessFaults the downpipes whose staleness rule can NEVER ARM: a non-positive/non-finite
...
export interface LedgerStorage {
  ...
  // OPTIONAL: the posture projection needs to enumerate the `posture-accept:` override records. It is
...
export const CEREMONY_FAULT_KINDS = [
  ...
  // the isolate-local authn-failure audit window is holding AUTHN_FAILURE_AUDIT_KEY_CAP distinct row-identities
  // and none could be reclaimed, so a further failed-authentication row went unwritten. The key space is bounded by
  // the tenant's own connections and roster, so this should not be reachable in practice; if it climbs, the cap is
  // wrong for that estate and the chain is under-recording sweeps.
  "authn-failure-audit-key-cap",
  // --- recovery-code break-glass: which limiter tripped, and the silent generation failures ---
...
export const DROPPED_WRITE_KINDS = [
  ...
  "freshness-fault",
  "vocab-drop",
  "expiry-observe-fault",
  "approval-fault",
  "storage-anomaly",
  "recovery-resume",
  "roster-discard",
  "dest-resolve-fallback",
  "canary-loss",
  "drill-drop",
  "notify-drop",
  "governance-fault",
  "governance-refusal", // the closed reason a governance surface refused (the discriminator guard-refused erases)
  "refusal-ring",
  "admin-refusal",
  "client-diag", // the console's own ring never reached the pack it was about to ride in
  "config-rejection",
  "authz-refusal",
  "alerting-health",
  "config-coercion",
  ...
  "cap-truncation",
  "posture-input-fault",
  "chain-break", // a lost chain-break LATCH write means a tamper verdict that later rolls over leaves no trace at all
  "test-outcome", // a lost wiring-check outcome means the pack cannot say whether the operator's Test button passed
] as const;
export type DroppedWriteKind = (typeof DROPPED_WRITE_KINDS)[number];

// ---- defaultDest reachability ---------------------------------------------------------------------
...
// ---- capTruncations: the capped record that never says it was capped -----------------------
//
...
//
// A COUNT ALONE IS NOT ENOUGH. Declaring that N ROWS WERE DROPPED without saying WHICH SUBJECT, for the three
// subject-keyed surfaces, leaves the consequence surviving in the one place a reader actually reads:
// `freshnessUncomputableIndex` joins the freshness map onto downpipes[], a downpipe past the cap is ABSENT from
// the map, carries no marker, AND ITS PACK ROW STILL READS AS A DOWNPIPE WITH A WORKING STALENESS RULE. A clean
// assertion on the row and a caveat sitting in a different section leaves the reader with only the clean
// assertion. The SUBJECTS ledger below carries the ids, and support.ts joins them back onto the row.
//
...
// `export * from "./sched-fault-core.ts"` re-exports the whole block, so every import site sees it unchanged.
export const CAP_TRUNCATION_SURFACES = [
  ...
  "dest-prune-map", // foldDestPruneRecord: the freshest DEST_PRUNE_MAP_MAX destinations' prune sidecar rows kept, older evicted
  ...
  // The two SIBLINGS of freshness-faults. Both are subject-keyed maps that REFUSE a new subject rather than
  // evicting an old one, so past the cap the subject is ABSENT, and an absent subject in a fault record reads
  // as a subject with no fault.
...
// recordFreshnessFault marks ONE downpipe as having an UNARMABLE staleness rule. Keyed by the
...
//
// THE CAP REFUSES A NEW SUBJECT, AND THAT REFUSAL IS DECLARED. This is not a ring: it does not
...
// rule can never arm reads as a downpipe whose staleness rule is fine -- which is the exact failure this
// recorder exists to prevent: "the map said Fresh for weeks while my backups had stopped" is asserted by the
// pack rather than merely unrecorded. A fleet-wide fact alone is not what the per-downpipe join reads.
//
...
// recordCompletionDiagnostics is the ONE per-completion observation the scheduling core makes, kept here so
// the classify/clamp/redaction logic lives with the vocabularies it draws on rather than in the run-state
// machine. It takes FACTS, never the completion body: the closed status, the coarse error class
...
//   - ENV-DEFAULT DESTINATION: a run that names NO destination id is the single-destination tenant whose
...
//   - FRESHNESS: mark (or clear) the downpipe whose staleness rule can never arm.
