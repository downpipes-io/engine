// ObservabilityMixin: the read-mostly observability cluster (contract section 5 scheduled restore tests and
// section 7 security-centre posture), layered over a base whose `this` is SchedulerDOSurface. It calls
// `this.appendAudit`, `this.listDownpipes`, `this.requireCapabilityResolved`, `this.state` etc. exactly as
// the base class expects. The section 6 reporting + RTO + coverage/gap detection sibling lives in
// ./scheduler-do-reporting.ts (ReportingMixin); each mixin is kept in its own module to stay under the
// module-size guardrail. Dispatch and `this` binding are consistent across every mixin; no storage key,
// route string, status code, response body or auth gate differs by which mixin defines the method, and the
// same capability re-checks run on the same paths in the same order.
...
// The closed beacon failure-cause vocabulary the cron already computes; the DO re-validates against it.
...
// RETENTION_PASS_KEY holds the LATEST retention-prune pass record. A single slot, not a ring: the
...
export function ObservabilityMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    // It carries the closed FAILURE CLASS the cron computes (a malformed BEACON_URL vs a network fault vs a
    // non-2xx rejection vs a DO read fault) and keeps a bounded RING of the last few attempts, because a
    // last-attempt-only ok:false with no status says neither WHY the beacon is not landing nor whether it is
    // intermittent. errorClass is re-validated HERE against the closed BEACON_FAIL_CLASSES set (an internal
    // call, but the DO bounds its own inputs), so no caller-derived string can enter the record; the URL and
    // the ingest key are never passed and can never be stored.
    ...
    async restoreTestsDue(): Promise<{ due: DownpipeState[] }> {
      ...
      const due = all.filter((d) => {
        ...
        // Never tested: due once there is a RUN to drill. A brand-new downpipe due immediately would run its
        // first scheduled test before its first backup, deferring "no completed run yet" as ok:false, so the
        // console would read "Last test failed" on a downpipe that has never even run. The first successful
        // run requests the first test via the retest stamp above instead, so nothing is lost by waiting.
    ...
    async completeRestoreTest(req: { id?: string; ok?: boolean; at?: number; reason?: string; deferred?: string; durationMs?: number; bytesVerified?: number; recordsVerified?: number; deepVerify?: { runId?: string; cursor?: number; records?: number; wrapped?: boolean }; oom?: { maxRecordBytes?: number; safeBytes?: number; overSafe?: boolean } }): Promise<{ ok: true }> {
      ...
      if (!ds) {
        // The drill ran, and its verdict is thrown away because the downpipe was renamed or deleted between
        // dispatch and callback. Answering ok:true is right (the campaign must still converge), and without a
        // count "we drilled the fleet and half the results never appeared" would have no visible cause. Count
        // the drop.
      ...
      if (req.ok !== true && (req.deferred === "no-run" || req.deferred === "posture" || req.deferred === "no-records")) {
        ...
        // A NEW deferral kind from a newer cron falls through this gate (version skew between components), so
        // a drill that could not run at all is recorded as a plain not-ok completion and the console tells
      ...
      const oom = req.oom;
      // An OOM measurement the DO cannot use is IGNORED, so the isolate-OOM early warning silently stops
      ...
      // Windowed deep-verify cursor: persist the advanced cursor so the next scheduled tick
      ...
      // A deep-verify cursor the DO cannot use is IGNORED, so the next tick re-covers the SAME window
      ...
      // estimate.
      //
      // "basedOnDrills says 2 but we ran 15 restore tests" is explained here: a drill that completes OK but
      // whose measurement is unusable is silently NOT sampled, so the RTO estimate is built on a fraction of
      // the drills the pack shows, with no visible explanation for the gap between the two counts.
      //
      // The DROP is right (a NaN duration or a zero-byte drill carries no throughput signal and would corrupt
      // the estimate); leaving it unrecorded is what removes the explanation. Recorded ONLY when a measurement was CLAIMED and could not
      ...
      // A scheduled restore test's completion ALSO advances an active on-demand fleet-drill
    ...
    async recordReconcileInventory(body: Record<string, unknown>): Promise<{ ok: true }> {
      ...
      // A hand-rolled field-by-field rebuild would silently drop the split counts (undeterminedWithinGrace/
      // Unreadable/PendingClassify, the neverReferenced tamper-vs-crashed-finalise split, and the
      // circuit-breaker fraction) because a rebuild only carries what it names.
      ...
      if (keys.length > RECONCILE_SIGNAL_MAX) {
        ...
        // A large-fleet tenant's OLDEST destinations are EVICTED from this map, and the reader cannot tell an
        // evicted destination from one that never reconciled -- so a 40-destination estate's 8 oldest buckets
        // would vanish from the reconcile view with no marker. Count the evicted rows (a
    ...
    // the allowlist, so naming the fields here as well only risks the two drifting apart and silently dropping
    // retention prune evidence (deferClass / skipped / unparseableTime / superseded / partial).
    async recordSealFault(body: Record<string, unknown>): Promise<{ ok: true }> {
      const fault = sanitiseSealFault(body, Date.now());
      if (fault === null) {
        // The seal DO is a SEPARATE Worker script, so a version skew between them can leave it emitting
    ...
    // recordRetentionPass persists the LATEST retention-prune pass record. The cron's retention pass builds one
    // bounded, redaction-safe record per tick and POSTs it here; without this slot the post would be a silent
    // no-op and the whole "storage keeps growing despite a retention policy" family would stay dark. It is a
    ...
    async recordRetentionPass(body: Record<string, unknown>): Promise<{ ok: true }> {
      ...
      // The latest-pass slot keeps its documented meaning (what did the CRON tick do): an attended record
      // covers ONE downpipe, so it folds into the per-destination sidecar below but never supersedes the
      // cron's whole-tick record.
      if (clean.attended !== true) await this.state.storage.put(RETENTION_PASS_KEY, clean);
      // Fold the sanitised record into the per-destination prune sidecar (DestConfigMixin), the rows
    ...
    async recordRestoreProven(
      ...
      if (!ds) {
        // A restorability proof that PASSED is being discarded. This is affirmative evidence the customer
    ...
    // identical: nothing is weakened, the full-decrypt drill machinery is inherited), and the same
    ...
    async fleetDrillNextBatch(capRaw: number): Promise<{ due: DownpipeState[]; campaignId: string | null; done: boolean }> {
      ...
      for (const [id, at] of Object.entries(campaign.inFlight)) {
        if (typeof at !== "number" || now - at >= FLEET_DRILL_INFLIGHT_TIMEOUT_MS) {
          ...
          // THE "stuck at 3 remaining for two days" SIGNAL. The self-heal re-queues a member whose
      ...
      while (due.length < cap && campaign.pending.length > 0) {
        ...
        if (!ds) {
          ...
          // This is a DELETED downpipe being reported to the customer as a drill FAILURE -- an alarming
    ...
    async advanceFleetDrill(id: string, passed: boolean): Promise<void> {
      try {
        ...
        // Swallowing the fault is right (the recency record must land) and it means the campaign's counts
        // are SHORT by one and it may never converge -- the other half of "stuck at N remaining". Count it.
    ...
    async gatherPostureState(now: number): Promise<{
      ...
      const downpipes: PostureDownpipeInput[] = states.map((d) => ({
        ...
        // makes the critical check actually evaluate: gating on a `lastRunAt` epoch the DownpipeState never
        // holds would leave the check always passing.
        ...
        // Verify-at-seal verdict -> the seal-verification check: a SUSPECT last verdict
    ...
    async gatherPostureIdentity(): Promise<PostureIdentityInput | undefined> {
      try {
        ...
        // The honest cannot-verify is right, and it is also invisible: the posture check ABSTAINS and the
        // report simply omits the identity verdict, so a persistently faulting read looks like a posture
        ...
        // Also count it as a POSTURE INPUT FAULT, so it rides in the posture section beside the finding it
        // degraded: a DROPPED IDP_KV BINDING (a permanent, one-line fix) would otherwise read exactly like a
        // transient blip, with admin-strong-auth reporting cannot-verify indefinitely and no visible cause.
    ...
    async postureEvaluationDue(req: { intervalMs?: unknown }): Promise<{ due: boolean; lastEvaluatedAt: string | null }> {
      ...
      if (!Number.isFinite(at)) {
        // A corrupt snapshot timestamp silently forces a re-evaluation every tick and makes
    ...
    async computePostureReport(req: { status?: unknown; beaconEnabled?: unknown; callerEmail?: unknown; worm?: unknown; configWrapKeyConfigured?: unknown; liveVersionId?: unknown }): Promise<{ report: PostureReport; regressions: PostureRegression[] }> {
      const now = Date.now();
      // Two SLICE faults would otherwise produce a SPURIOUS or a flattened finding with nothing recording it.
      ...
      // Off-account audit mirror: is the audit trail pushed to a holder outside this account? Read through
      // getSiemPushView(), the REDACTED view, deliberately: the raw record carries the sealed auth secret and
      // the posture path has no business holding it. All DO-owned; nothing here needs env.
      const pushView = await this.getSiemPushView();
      const lastPush = pushView.trail.length > 0 ? pushView.trail[pushView.trail.length - 1] : undefined;
      const auditMirror = {
        configured: pushView.present,
        enabled: pushView.enabled === true,
        ...(lastPush !== undefined ? { lastAttemptOk: lastPush.ok, lastAttemptAt: lastPush.at } : {}),
      };
      ...
      // Update integrity: the supply-chain observables for the update-apply-provenance +
      ...
      // After a FAILED rollback the engine is still serving the version it tried to leave -- which the record
      // holds in toVersion, exactly as an applied one does. Falling through to `undefined` here would stop the
      // drift check in the one state where a version drift is not a drift at all but a known, unresolved
      // incident. applied-unconfirmed is the same shape: the promote landed, so toVersion is what SHOULD be
      // live, and the whole point is that it could not be confirmed.
      ...
      // Alternate legitimately-live ids: a ramp-settled applied outcome is
