...
// their list views) is split out of scheduler-do-rbac.ts so no RBAC file exceeds the module-size
// guardrail. This is a sibling sub-mixin of
// RbacMixin (the role-resolution primitives) and RbacAuthorityMixin (caller authority + the
// anti-escalation guards); all layer over a base whose `this` is SchedulerDOSurface, so each write still
// re-resolves the caller through this.roleForCaller and re-checks this.requireGrantWithinAuthority /
// this.requireNotOwnerEscalation / this.countOwners with the SAME dispatch and `this` binding a single
// class would have. The owner-clamp, the last-Owner guard, the maker-checker and least-privilege rules
// hold across the split; no storage key, route, status code, response body or auth gate differs between
// mixins. This module is a leaf the assembly depends on, never the reverse.
...
export function RbacMutationsMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    async setRole(
      ...
      // The last-Owner guard is RAISED to a two-Owner floor while dual control is armed (the single-owner
      ...
      // the last Owner, the long-standing refusal) and TWO while Require Approver is on
      ...
      if (targetIsCurrentOwner && !newRoleIsOwner) {
        ...
        if (!verdict.ok) {
          // Record WHICH guard fired, HERE, where it is known. Both refusals throw a plain Error
          // and answer 400 (never the DO's AuthError funnel), so without this the last-Owner guard would be dead on
          // this path and a governance refusal would coalesce with a shape refusal into one indistinguishable
      ...
      // THE STRAND INVARIANT (owner-floor.ts wouldStrandDualControl). The floor guard above
      ...
      if (assigningCustom) {
        await this.appendAudit({
          ...
          outcome: "success",
          // Record the ASSIGNED custom role's real capability count, not 0. customRec is
      ...
      //
      // inviteState is returned on BOTH arms, and that is the point of it. A grant that comes back with
    ...
    async deleteRole(
      ...
      const pendingCurrent = pending.find((p) => p.email === email);
      const current: RoleEntry | PendingRoleEntry | undefined = boundCurrent ?? pendingCurrent;
      // AN OFFBOARD OF SOMEBODY THIS TABLE CANNOT SEE IS STILL AN OFFBOARD. resolveRole is
      // max(email-table role, highest group-mapped role, viewer), and groupRoleFor needs no row of any
      // kind, so a member whose authority comes ONLY from an IdP GROUP CLAIM has nothing under `role:`.
      // Without the bump below, /roles/delete would remove nothing, revoke nothing and audit nothing for
      // exactly the people the engine is the session authority for: a native-IdP session mapped to
      // operator by a group would keep answering GET /admin/whoami 200 {role:"operator",roleSource:"group"}
      // after a POST /roles/delete answered {deleted:false}. revokeSignInFactors does not close it either:
      // it scopes its own epoch bump to a revocation that closed a way in, and such a member holds no
      // passkey, no recovery record and no invite, so it reports sessionsTerminated:false and the session
      // survives that too. The SCIM leaver path posts only /roles/delete, so an automated deprovision
      // reporting success must not leave the leaver with a full-authority session for the rest of the 12h
      // cookie.
      //
      ...
      // The offboard guard is RAISED to a two-Owner floor while dual control is armed (rule b, the same invariant
      ...
      if (targetIsCurrentOwner) {
        const verdict = checkOwnerRemoval(this.countOwners(entries, pending, now), await this.getRequireConfigApproval());
        if (!verdict.ok) {
          // THE OFFBOARDING GUARD, recorded where WHICH guard fired is known (both refusals throw a
      ...
      //
      // WHAT THIS DOES NOT REACH, said plainly: this deletes GRANTS, and it deletes nothing else. In
      // particular it cannot reach a lingering pending WEBAUTHN REGISTRATION INVITE: a registration invite
      // is stored under `passkeyInvite:<TOKEN>`, keyed by the secret itself, and an email-keyed delete has
      // nothing to match on. Without a separate roster gate on the invite arm of
      // resolveRegistrationAuthorisation, an offboarded person's outstanding invite link would still enrol a
      // standing passkey and a fresh set of never-expiring recovery codes for up to seven days after the
      // offboard; that redemption is refused there instead. The invite is BURNED, along with the credentials
      // and the recovery record, only by revokeSignInFactors, which is a separate operator act on purpose.
      // Sweeping outstanding invites at offboard time as well remains known, untracked debt.
      let deleted = false;
      if (boundCurrent !== undefined) {
    ...
    async setGroupRole(
      ...
      // An optional connId SCOPES this mapping to logins through that ONE IdP connection (closing the
      // cross-IdP group-name collision); absent = GLOBAL. Validate it when present.
      ...
      if (assigningCustom) {
        await this.appendAudit({
          ...
          action: "custom-role-change",
          outcome: "success",
          // Record the MAPPED custom role's real capability count, not 0. customRec is the
    ...
    async addCustomRole(
      ...
      if (!result.ok) {
        // An attempted PRIVILEGE ESCALATION through a custom role is its own counted, dated fact.
    ...
    async deleteCustomRole(
      ...
      // Count the grants that STILL name this role, BEFORE it goes. Every one of them drops to the
      // viewer floor at resolution on its holder's next request, and without this count the deletion FACT
      // rides in the audit trail while its CONSEQUENCE rides nowhere: "Bob lost access to restores after a
      // role tidy-up" could not be linked to the tidy-up. Both tables confer the role and a group grant can
      // carry a whole team, so counting only the member table would under-report the blast radius.
      //
      ...
      if (deleted) {
        // Deleting a custom role that still has HOLDERS silently demotes every one of them to their
