// SupportDiagMixin: the SchedulerDO's diagnostic recorders and read paths for the cron and admin subsystems.
// It is the sibling of scheduler-do-diag.ts (which owns the scheduler's own storage/housekeeping/state-refusal
// recorders) and follows that module's shape exactly: classify into a closed vocabulary, clamp everything,
// write a bounded counter or stamp, and never let the observation break the path it observed.
//
// What each recorder exists for:
//   recordSealError     a pre-run seal dispatch throw -- the destination record a downpipe pins was
...
//   recordMetricsScrape the Prometheus /metrics scrape surface's own health: a DO hiccup that is
...
//   recordWebauthnFault the 14 structural CBOR/COSE/DER/authData defects that all coarsen to the one
...
//   recordDroppedWrites the diagnostic recorders' own dropped writes, so the pack stops under-counting
//                      during the exact outage it is meant to explain.
//
...
export function SupportDiagMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    // ---- per-downpipe pre-run seal errors ---------------------------------------------------------
    ...
    // ---- the Prometheus /metrics scrape surface ---------------------------------------------------------
    ...
    async recordWebauthnFault(phase: string, cls: string, coseAlg?: number): Promise<void> {
      try {
        ...
        // A storage fault here would otherwise make the counter vanish in silence -- and the window in which
        // this recorder is MOST likely to fail (a DO under stress) is the window in which a total-lockout
        // investigation most needs it. A pack that shows an EMPTY WebAuthn aggregate during a lockout, with
        // nothing saying the aggregate itself could not be written, is the worst reading available: it says
        // the ceremonies were fine. The auth path is unchanged (still best-effort, still never throwing); the
        // loss is counted in memory and folded into the shared aggregate by the next write that succeeds.
    ...
    // (a corrupt stored approval, expiry row or custom role is read back on EVERY request that touches
    ...
    // ---- the run's SOURCE + DEST fault evidence -----------------------------------------------------------
    ...
    // ---- the FORMAT + CRYPTO integrity fault aggregate -----------------------------------------------------
    ...
    async recordIntegrityFaults(body: { id?: unknown; snapshot?: unknown }): Promise<{ ok: true }> {
      try {
        ...
        // This record is read as the answer to "which downpipes failed verification", and a downpipe absent
        // from it reads as one whose archives verified -- so a systematic fault (a mis-provisioned wrap key, a
        // format drift) that reaches the whole fleet at once must never be under-reported by a cap on the
        // record's size. Booked AFTER the record write so a failed declaration can never cost the evidence
        // that produced it.
        //
    ...
    // readIntegrityFaults is the pack read (GET /integrity-faults): per downpipe, WHICH verification
    ...
    // ---- the last-resort dispatch fault ring -----------------------------------------------------------
    ...
    // a surface threw a 500 the customer saw that the pack carries NO other trace of.
    ...
    // ---- the CRON INVOCATION's health --------------------------------------------------------------------

    // The cron cadence is the engine's own tick interval (cron/cron-cadence.ts, pinned to the wrangler
    // literal). The DO derives the tick GAP from it against its OWN clock, so a wedged or hostile edge clock
    // can neither fabricate nor conceal a missed tick. The FALLBACK below reads CRON_CADENCE_MS directly rather
    // than a hand-copied literal, so the interval can never drift from the wrangler definition in the one
    // place it is reached precisely when the caller did not supply it.
    ...
    async recordCronHealth(body: { delta?: unknown; cronIntervalMs?: unknown }): Promise<{ ok: true }> {
      try {
        const interval = typeof body.cronIntervalMs === "number" && Number.isFinite(body.cronIntervalMs) && body.cronIntervalMs > 0 ? body.cronIntervalMs : CRON_CADENCE_MS;
    ...
    async recordDestProbeFaults(body: { rows?: unknown; refusedUpstream?: unknown }): Promise<{ ok: true }> {
      try {
        ...
        // A failing destination can be dropped silently at up to three cuts. `refusedUpstream` is the FIRST
        // of them, counted in the Worker's own cron accumulator before this DO is ever called, and it is the
        // only one visible from there; the other two are counted by the writer above. It is our own number
        // rather than a caller's, but it is clamped here anyway because this route is the redaction and shape
        // chokepoint for everything on this body.
    ...
    // ---- the standing destination-BUILD health -----------------------------------------------------------
    ...
    // ---- the runtime wrap-key UNWRAP fault timeline --------------------------------------------------------
    ...
    // DROPPED binding, or proves a concurrent writer's lost update, survives beyond the operator's own
    // browser tab rather than being lost while the pack keeps showing two healthy-looking sources-attached
    // audit rows. applyBindingAlarm is the redaction chokepoint (closed kind; binding names control-stripped,
    // clamped and capped). Best-effort and never throwing.
    ...
    // the owner's #1 fear caught in the act -- a change to the engine's bindings that did not preserve them,
    ...
    // could not be READ is an ENGINE fault, distinguished here from a genuinely missing approval.
    // applyAdminRefusal is the redaction chokepoint (both axes closed; nothing else is read).
    ...
    // ---- the pinned IdP signing-certificate health ---------------------------------------------------------
    ...
    async readDiscoveryHealth(): Promise<{ health: DiscoveryHealth | null; lastTokenSet: DiscoveryTokenSet | null }> {
      return {
        ...
        // the token-SET verdict rides with the discover health because it is the same question one step
    ...
    // recordDiscoveryTokenSet stores the verdict of ONE pasted-token verification. It is a DIFFERENT event
    ...
    // ---- the ATTACH / RE-ATTACH outcome ------------------------------------------------------------------

    // recordAttachHealth folds ONE attach / re-attach attempt into the standing record. reattach-missing is the
    // HEAL for the owner's stated #1 fear -- a bare `wrangler deploy` resetting the worker's bindings and
    // silently dropping every console-attached source -- and both routes ended in a catch that returned the
    // refusal as an HTTP RESPONSE and recorded nothing. applyAttachHealth is the redaction chokepoint: a closed
    ...
    // ---- the CONTROL-PLANE RECOVERY refusal --------------------------------------------------------------

    // recordRecoveryRefusal folds ONE refused recovery action into the standing record, so on the path where
    // the pack may be the only surviving artefact, a refusal is not left living only in the operator's
    // browser. applyRecoveryRefusal is the redaction chokepoint: a closed surface and a closed class, so no
    ...
    // "signature" is the whole gap: without it, a damaged recovery KIT and a tampered EXPORT read as the same refusal.
    ...
    async routeSupportDiag(req: Request, url: URL): Promise<Response | null> {
      switch (`${req.method} ${url.pathname}`) {
        // the cron seal loop posts ONE closed class per pre-run dispatch fault, attributed to the
        ...
        // the /metrics handler posts its scrape outcome (authorised or not); the pack reads the health.
        ...
        // the pack read of the structural WebAuthn fault aggregate (the RECORDING is DO-internal: the
        ...
        // the Worker edge posts the PROJECTED fault rows of one finished restore / drill / verify (a
        ...
        // the Worker edge bumps the silent-fallback / silent-exclusion counters.
        case "POST /diag/admin-counters":
          return this.json(await this.recordAdminCounters((await req.json()) as { bumps?: unknown }));
        // the Worker edge files ONE wiring-check outcome (a Test button's result). recordTestOutcome is
        ...
        case "POST /diag/test-outcome": {
          ...
          await recordTestOutcome(this.state.storage, {
            ...
            // forwarded as-is; recordTestOutcome is the redaction chokepoint and closed-set gates both
            ...
            // the email platform code, forwarded as-is. recordTestOutcome re-runs the E_UPPER_SNAKE shape
            ...
            // the notify-channel delivery code, forwarded as-is. recordTestOutcome re-gates it against the
        ...
        // the seal / restore path posts ONE drained integrity-fault
        ...
        // the Worker's four last-resort catches post their surface + errId; the pack reads the ring.
        ...
        // the destination-selection pass posts the per-destination probe verdicts for the pack to read back.
        ...
        // the sizing probe posts its outcome class per source type; the pack reads the record.
        ...
        // dest/factory.ts posts every destination-BUILD outcome (a failure always; a success once per
        ...
        // the Worker edge flushes its isolate-local dropped-write tally; the pack reads it back.
        ...
        // the read-for-use sites post ONE closed unwrap cause; the pack reads the timeline back.
        ...
        // the attach pipeline posts a post-write binding-safety alarm; the pack reads the ring back.
        ...
        // the update pipeline posts its classified fault rows; the pack reads them.
        ...
        // the pack read of the pinned IdP signing-cert health (the RECORDING is DO-internal: the SAML ACS
        ...
        // the token-SET route posts its CLOSED fail class, decided where the Cloudflare STATUS is known.
        ...
        // the discover route posts its CLASSIFIED per-product outcome; the pack reads the record back.
        ...
        // the two export channels (the console download, the collector feed) post ONE classified attempt
        ...
        // the attach / re-attach routes post their classified outcome + the heal plan's shape.
        ...
        // the update / licence routes' outermost catches post the route + the last checkpoint passed.
        ...
        // the Worker edge posts the change-reference header faults it alone can see (a garbled header,
        ...
        // the EDGE half of the guard ledger. The two DO-side guards (last-Owner, last-passkey) call
        ...
        // the control-plane import / reconcile / download refusals post their closed surface + class.
        ...
        // the admin routers post a refused write's closed (surface, reason); the pack reads the aggregate.
