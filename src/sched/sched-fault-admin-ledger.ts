...
//
//   adminRefusals    the engine-computed refusal the browser showed once and forgot
//   configRejections the 400 that reached only the browser
//   authzRefusals    the 403 that is audited only when it SUCCEEDS
//   configCoercions  the write that was ACCEPTED, then quietly narrowed
//
// Each family lives here WHOLE: its closed vocabulary, the private Set built from that vocabulary, its pure
// classifier and the recorder that gates on the Set. The module boundary is the FAMILY boundary rather than a
// layer boundary, so a member added without its gate is still a compile error, and each gating Set is
// unreachable from any other module. The redaction rule these records are written under is stated once, in
// ./sched-fault-ledger.ts, and binds this file.

// The capability names are the closed PRODUCT vocabulary the authz-refusal gate axis is built from, imported
// rather than re-listed so a capability added to the product cannot silently fall out of the aggregate.
// identity-rbac.ts is a pure leaf (types + closed sets), so this adds no cycle.
...
export const ADMIN_REFUSAL_ROUTES = [
  // `dest-verify` is deliberately absent. THIS LEDGER IS THE DO'S, and the live Verify-and-save PROBE is a
  // network call (probeDestination): the DO does no network I/O, by design, so it never sees a probe verdict
  // and no site in this file can classify one. The refusal is not unrecorded -- it is recorded on the OTHER side of
  ...
  "drill-evidence", // a drill-evidence append was refused (the append validator's refusal half)
...
export const ADMIN_REFUSAL_REASONS = [
  ...
  // The notify-channel URL validator's own closed reject codes (additive, disjoint from the
  ...
  // the pack is otherwise blind to: every rejection is a 400 to the browser and nothing else.
  ...
  // ASVS V13.2.4: the sink's host is not on the account's configured outbound egress allowlist.
...
// reason vocabulary. Both halves are closed sets, so the mapping is total and can never widen what
...
//
// `reserved-binding` is deliberately its own reason: a source binding naming SIGNER_PRIVATE (or any
...
export const CONFIG_SURFACES = [
  ...
  // --- the DR-critical break-glass path: a failed reconcile/import writes NOTHING at all ---
...
export const CONFIG_REJECT_REASONS = [
  "reserved-binding", // SECURITY-SIGNIFICANT: a source binding named an engine-reserved binding (SIGNER_PRIVATE, a destination credential). Never a typo; never coarsened into one.
...
export const AUTHZ_GUARD_GATES = [
  ...
  // `posture-riskaccept-refused` is deliberately absent. The gate it names is real and its refusals ARE
  // counted -- as the capability `posture.riskaccept`. requirePostureRiskAccept throws the standard capability
  // AuthError ("forbidden: posture.riskaccept capability required"), and classifyAuthzGate's capability arm
  // (which runs over ALL_CAPABILITIES, before every named-guard arm) matches it first and always. Adding the
  // named member would be an ALIAS for a gate that already has a live producer: two names for one refusal
  // would split its count in half.
  //
  // THE GUARDS THAT WOULD OTHERWISE REFUSE IN SILENCE. `last-passkey-guard` and `first-party-session-required`
  // throw a PLAIN Error and answer 400, so they never enter the DO's AuthError funnel and would never reach
  // this ledger through classifyAuthzGate alone. The console's admin-write row maps EVERY non-special 4xx to
  // `refused-validation`, so on the pack a guard refusing an offboarding and a plain shape refusal on the same
  // POST would be the byte-identical row {admin-write, access-security, role-delete, refused-validation},
  // coalescing a lock-out guard into an ordinary validation failure. "Offboarding a departed employee keeps
  // being refused and nobody can say why" is the ticket this would produce.
  //
  // They are recorded AT THE GUARD, where WHICH GUARD FIRED is known as a fact rather than inferred from a
  // status. The HTTP answer is unchanged (still a 400 carrying the guard's own sentence to the operator): only
  // the closed counter is new, and no email, subject, credential id or role name has a field to travel in.
...
export const CONFIG_COERCION_SURFACES = [
  ...
  // "discovery-sources" is deliberately absent. setEnabledSources REFUSES an unknown source type rather than
  // dropping it, so no code path can produce this coercion; a member with no possible producer is dead
  // vocabulary. support-sections-diag.ts's projectCountAgg only lets a key through when it is IN the current
  // CONFIG_COERCION_SURFACES x CONFIG_DROP_CLASSES cross product, so any stored count from before that
  // refusal is silently excluded from every pack regardless. Re-introducing the member would bring the
  // silent-drop behaviour back.
...
export const CONFIG_DROP_CLASSES = [
  ...
  // "unknown-source-type" is deliberately absent too: it would pair only with "discovery-sources" above, and
  // neither producer can fire while setEnabledSources refuses rather than drops.
