// The RBAC caller-authority cluster (the
// effective-authority resolution for a forwarded caller, the whoami bootstrap/bind path, and the
// anti-escalation + grant-within-authority guards) split out of scheduler-do-rbac.ts so no RBAC file
// exceeds the module-size guardrail. This is a sibling sub-mixin of RbacMixin (the role-resolution
// primitives stay there; the role/group/custom-role writes live in scheduler-do-rbac-mutations.ts); all
// layer over a base whose `this` is SchedulerDOSurface, so roleForCaller still reaches resolveAuthority /
// resolveBoundEntry / capabilitiesOfCustomRole and the writes still reach these guards through `this` with
// the SAME dispatch and `this` binding the single class had. Behaviour-preserving move-only: the
// owner-clamp, the maker-checker and least-privilege rules are unchanged; no storage key, route, status
// code, response body or auth gate changed. A leaf the assembly depends on, never the reverse.
...
// scopeGroupMapping filters the group-role mapping to those that apply to THIS caller's connection before
...
export function RbacAuthorityMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    // issued at a departed person's re-used address resurrects their armed approval AND consumes the new
    // person's grant into the departed subject: an approval refused as
    // maker-authority-lapsed becomes usable again the moment `role:pending:<same-email>` is written for
    // somebody else. The subject axis is untouched, so a replayed identity that is
    ...
    async roleForCaller(
      ...
      // SECURITY: ONLY the bare-token method is the Owner break-glass. An emailless/subjectless
      ...
      // SECURITY: this branch trusts the ASSERTED `method`, and that is correct PRECISELY BECAUSE
      ...
      // a recycled email never inherits a prior member's role (ASVS V10.3.3 / V10.5.2).
    ...
    async resolveStoredIdentityAuthority(subject: string | undefined, email: string | undefined, recordedGroups: readonly string[] | undefined): Promise<ReadonlySet<Capability>> {
      ...
      // account invited next. Proved by an attack scenario in test/validate-dualcontrol.ts section 22.
    ...
    // actual authority instead of refusing it on its "viewer" built-in floor. For a BUILT-IN caller the
    ...
    parseGroupsParam(raw: string | null): string[] {
      ...
      try {
        ...
        // A param that PARSED but is not an array is corrupt (a router/DO version skew, a tampered
    ...
    async whoami(
      ...
      // The edge address the ROUTER read, forwarded for the one WRITE this route can perform (the
      ...
      // SECURITY (ASVS V10.3.3): an Access/passkey caller with no STABLE subject is NOT the break-glass;
      ...
      // The recovery-required latch and the first-Owner
      ...
      if (await this.roleTableIsEmpty()) {
        ...
        if (await this.getBootstrapConsumed()) {
          // THE DEADLOCK. The role table is EMPTY and the one-shot bootstrap latch is SPENT, so nobody
          // holds authority and nobody can claim it: every caller resolves to viewer forever and the "email
          // the owner a link" button is permanently inert. Without this signal it is indistinguishable, in a
          // pack, from "an account whose members are all viewers" -- and the two have completely different
          // remedies (the second is a role grant; this one is the break-glass recovery). Recorded THROTTLED
          // because whoami is on every authenticated request. Count only: no subject, no email.
        ...
        if (email !== null) {
          ...
          // The address goes WITH it: the ACCESS first-owner bootstrap, the very first call a fresh console
          // makes, threads the source IP the same way its passkey twin
          // (scheduler-do-passkey.ts markBootstrapConsumed(email, "passkey", sourceIp)) does, so it never
          // bumps the missing-source-IP capture-fault counter on a healthy engine.
    ...
    async requireCapabilityResolved(
      ...
      // A capability denial is an authorisation refusal -> AuthError -> 403 (not a 400),
