...
// DESTINATION (scheduler-do-dest-config.ts): console-set, owner-exclusive, and the engine must reconstruct
// the live secret on every delivery (unlike the ingest-credential grant, which only ever stores a one-way
// hash). This mixin owns the storage read/write + validation + redaction + audit; the outbound fetch itself
// lives OUTSIDE the DO (cron/siem-push-pass.ts -- the DO never makes a network call to a customer-controlled
// host).
...
// rather than re-typed here, so the format and sink lists cannot drift out of step with what the support
// pack and this mixin each enumerate. test/validate-siem-push.ts asserts its per-format loop list is EXACTLY
// that authority's set, in both directions, so a ninth format cannot be exercised by no loop while every
// loop still reports green.
...
// field carries neither "endpoint" nor "format" (missing-fields), keeping the closed classes accurate. The
// syslog host's internal-address refusal is worded to the same rule, so an SSRF refusal files as
// endpoint-invalid rather than claiming a field was missing.
...
//
// The returned value never ends in a slash: cleaning strips control characters and then trims the slashes,
// and only then is the length checked, so nothing sliced afterwards can reintroduce a trailing slash or a
// length the cleaned value does not have. Ordering matters both ways -- cleaning before capping and capping
// before cleaning can each leave a stray control character or a trailing slash behind -- so the guarantee
// that holds is the function's own postcondition on the value it actually returns, not a fixed step order.
//
// The over-length case is a REFUSAL rather than a trim. An S3 key prefix's only job is to name a location, so
// a value trimmed to fit would address somewhere else: a value whose only job is to match something
// elsewhere must not be silently reshaped. No legitimate prefix approaches 512 characters.
...
// syslog-tls transport only bounds the config shape here (host required; a bad/absent port falls back to the
// default). The message carries neither "endpoint" nor "format", so a missing host files as the
// missing-fields reject class.
//
// The syslog host IS screened against private/loopback/link-local addresses (including cloud metadata),
// despite an on-prem SIEM on a private address being the norm for syslog in general: notify/siem-syslog-sender.ts
// connects with connect({ hostname: host, port }, { secureTransport: "on" }), so a stored private host gets a
// real TLS connection attempted against it carrying the estate's audit stream, and this engine runs at the
// Cloudflare edge with no route into a customer's private network. The host is screened here, at the router,
// and again in the sender.
function buildPushSyslogTarget(raw: unknown): PushSyslogTarget {
  ...
  if (isInternalSinkHost(canonicaliseBareHost(host))) {
    // The word "endpoint" is load-bearing: classifyPushRejectReason files a throw carrying it as
    // endpoint-invalid, and this refusal is an unacceptable destination address, not a missing field. Worded
    // without it the audit event read rejectReason "missing-fields" for a submission that was complete.
    throw new Error("push destination syslog endpoint host resolves to a private/loopback/link-local address (incl. cloud metadata); these are refused");
  }
...
export function SiemPushMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    async recordSiemPushOutcome(req: {
      ...
      // The join key is a 12-hex one-way digest by construction. SHAPE-GATE it here (the redaction
