...
// downpipes (GET /due) and runs each out of the DO (trigger -> seal -> complete).
//
// The shared substrate (imports, constants, record types, AuthError, the SchedulerDOSurface instance API, and
// BaseSchedulerDO which holds the one `state` field + constructor) lives in the leaf module ./scheduler-do-base.ts.
// Cohesive method groups live in mixin factories in sibling ./scheduler-do-*.ts files; each mixin's `this` is typed
// as SchedulerDOSurface so a method that lives in one mixin still calls a method in another with the same dispatch
// and `this` binding. This file assembles the mixin chain over the base, declares the final SchedulerDO, and
// re-exports the public surface so every importer sees the same names. Base and mixins are leaves this file
// depends on, never the reverse, so there are no import cycles.
//
// The route() RPC dispatch lives in ./scheduler-do-routing.ts as a thin chain of per-subsystem sub-dispatch
// methods. Only fetch() (the error-to-status mapping) stays here, and the step-up ceremony lives in
// ./scheduler-do-stepup.ts. What remains on the final SchedulerDO is the downpipe-core apply path (addDownpipe +
// the listAllByPrefix/list/destination-for-run/remove helpers), the runlog lock, the per-caller rate-check and the
// Response helpers, all reached through `this` from the mixins.
//
// Key decisions in the base/mixins: DO alarms are durable (the primary timer; the Worker cron is only a coarse
// reconciliation tick) with jitter spreading same-cadence downpipes; an in-flight downpipe is COALESCED, never
// queued twice; runlogIndex is allocated EARLY in the storage transaction and gaps are tolerated (RUNLOG chains per
// downpipe via prevRunId, so a global gap is fine and a rollback is not).

import type { ConfigChangeKind, } from "../admin/change-control.ts";
import { TOO_SOON_REFUSAL_CODE, TooSoonRefusal } from "../admin/approvals.ts";
...
import { TestFaultMixin } from "./scheduler-do-test-fault.ts";
...
// Assemble the mixin chain over BaseSchedulerDO. Each mixin factory is constrained to a constructor that yields
// SchedulerDOSurface; BaseSchedulerDO supplies only `state` + the constructor, so it is cast up to SchedulerDOCtor
// ONCE here (the standard mixin-base cast) so each layered mixin's `this` sees the WHOLE instance API. Runtime is a
// normal prototype chain, so the methods, their `this` binding and their dispatch behave as a single class would.
// The mixins hold DISJOINT method sets, so the LAYERING ORDER below does not affect dispatch; it is grouped only
// for readability. The remaining methods stay on the final SchedulerDO below, where `this` sees the surface too.
...
// HARNESS-ONLY test-fault store (scheduler-do-test-fault.ts): one armed update-lifecycle fault, reached only
// through the Worker's HARNESS_TEST_FAULTS-gated, Owner-gated /admin/test-fault/* paths. Inert in production.
SchedulerDOAssembled = TestFaultMixin(SchedulerDOAssembled);
// SIEM audit-log push destination: a sibling of the destination config above (console-set, owner-exclusive
// egress config + cursor + trail). Its routes are dispatched by RoutingMixin's routeSiemPush.
SchedulerDOAssembled = SiemPushMixin(SchedulerDOAssembled);
// OTLP/HTTP metrics push destination: the snapshot sibling of the SIEM push above (console-set,
// owner-exclusive egress config + bounded trail; no cursor). Its routes are dispatched by RoutingMixin's
// routeOtlpPush.
...
// vocabulary shape as SchedulerDiagMixin, for the pre-run seal faults the cron loop swallows, the
// /metrics scrape surface's own health, the structural WebAuthn fault classes, and the recorders' own
// dropped writes. Layered here so the passkey mixin (which records a structural fault) and the routing
// chain (which dispatches its routes) both reach it through the same `this`.
...
// The control-plane recovery subsystem (no-custody signed export + recovery-required latch +
...
// The route() RPC dispatch. route() + the core data-plane sub-dispatch live in RoutingMixin; the
// remaining sub-dispatches are split across three cohesive sibling sub-mixins (signals / config governance /
// identity) so no routing file exceeds the module-size guardrail. All four are layered LAST only for
// readability; each one's `this` is the same SchedulerDOSurface, so route() (in RoutingMixin) reaches every
// sub-dispatch regardless of which sub-mixin owns it, and each sub-dispatch reaches every handler the same
// way. The four key sets are disjoint, so the layering order is immaterial to dispatch.
...
// runlogLockSlot maps a destination id to its RUNLOG-lock storage slot.
...
export class SchedulerDO extends SchedulerDOAssembled {
  async fetch(req: Request): Promise<Response> {
    try {
      ...
      // Branch on the error type rather than collapsing every failure into a 400 with the raw message.
      ...
      if (e instanceof AuthError) {
        ...
        // THE authorisation-refusal funnel. Every capability gate, owner gate, escalation guard and
        // break-glass guard in this DO throws AuthError and lands exactly here, so counting the closed
        // GATE at this one point cannot miss a site and cannot be forgotten at a new one. The public body
        // stays the bare "forbidden" (anti-enumeration); classifyAuthzGate reads the INTERNAL detail only
        // to select a closed gate member and returns it -- the detail, and the caller's email, subject and
        // IP (never in it to begin with), are stored nowhere. Best-effort: recordAuthzRefusal never
        // throws, so the 403 is returned whether or not the ledger write lands.
        await recordAuthzRefusal(this.state.storage, classifyAuthzGate(e));
        return new Response(JSON.stringify({ error: "forbidden" }), { status: 403, headers: { "content-type": "application/json" } });
      }
      // Approval-dwell: a restore/owner-action/config-change approve that arrived less than
      // APPROVAL_MIN_DWELL_MS after the record was raised is a THROTTLE, not a bad request, so it answers 429
      // with a whole-second Retry-After the caller can act on, rather than the generic 400 every other approve
      // refusal returns. Modelled on OwnerFloorRefusal / PreconditionRefusal below (a typed refusal this catch
      // recognises by TYPE, carrying its own status), not on router-core.ts's rateLimitedResponse: that builder
      // is the per-caller REQUEST-FLOOD limiter's own RFC 9457 shape, a different refusal class answering a
      // different question, and this is a DO-internal throw, not a router call site it is built for.
      if (e instanceof TooSoonRefusal) {
        return new Response(JSON.stringify({ error: e.message, refusal: TOO_SOON_REFUSAL_CODE, retryAfterSeconds: e.retryAfterSeconds }), {
          status: 429,
          headers: { "content-type": "application/json", "retry-after": String(e.retryAfterSeconds) },
        });
      }
      ...
      if (e instanceof TypeError || e instanceof RangeError || e instanceof ReferenceError) {
        ...
        // File the OPAQUE id the customer will quote from their console toast in the bounded ring
      ...
      // THE rejected-config-write funnel. Every refused configuration save in this DO -- a downpipe upsert,
      // a cron/retention/blackout edit, a source binding (including the SECURITY-significant
      // reserved-binding attempt), a role grant, a group mapping, a custom role, a notify channel/rule, an
      // IdP connection or the cert rollover that expires tomorrow, an expiry item, an OTLP/SIEM destination,
      // and the DR-critical break-glass control-plane reconcile/import -- reaches the customer's browser as
      // this 400, and is recorded here so a rejected write leaves a durable trace beyond the browser toast.
      // The SURFACE comes from the DO's own static route key (the DO's routes carry no path ids), and
      // classifyConfigReject reads the validator message only to SELECT a closed reason and returns it: the
      // message quotes the submitted cron string, bucket, URL, email or account id, and is discarded at
      // that boundary. The 400 body carries only the error message.
      {
      ...
      // status, the `error` field and the sentence itself are the same shape every 400 refusal returns;
      // `refusal` and `ownerFloor` are additive, and they are the only structured facts about the refusal
      // that leave this boundary. It is inside the 400 arm rather than ahead of it deliberately: an
      // Owner-floor refusal IS a rejected config write, and lifting it out would quietly drop it from
  ...
  async addDownpipe(
    ...
    // Cross-account confused-deputy guard: validateConfig above only shape-checks a
    // token-authenticated source's accountId (a hex regex); it never checks that the id is one the Owner's
    ...
    // Compute the first run time AND the cron-resolution outcome in one pass: a create/edit with a
    // cron that no longer parses, an unknown timezone, or an impossible date falls back to cadence, and
    // the closed cronResolve class records that for the support pack (never the cron string).
    ...
    const ds: DownpipeState = {
      ...
      // Preserve the latest-run pointer across a re-upsert of the SAME source: a config edit
      ...
      // "no completed run yet" and DEFER on a downpipe with a full archive (recorded ok:false,
      // which the console reads as a failed test), and the console's run hints would read "no runs yet"
      // until the next run. The pointer describes THIS source's archive, so it survives any edit
      // that keeps the source identity and resets only when the source itself changes (a different
      // resource honestly has no runs yet). The seal path is unaffected either way: RUNLOG-1 relinks
      // prevRunId from the destination-local tail, never from this pointer.
      ...
      // Blackout-resolution outcome: a hop-ceiling exhaustion fires the run INSIDE the declared change
      ...
      // Preserve the latest verify-at-seal verdict across a config edit: editing the
      ...
      // The never-reported replica: carry the monotone sealed-run counter across the edit -- it counts
  ...
  // a single storage.list() returns at most ~1000 keys, so a bigger prefix silently TRUNCATES, and for `dp:` that
  // means downpipes past the page are NEVER enumerated, hence never scheduled. It pages with an explicit limit +
  // startAfter cursor (the prior page's last key, exclusive) until a page comes back SHORT, the exhaustion signal;
  // ascending key order means the cursor advances with no overlap and no gap. The guard bound is a defence-in-depth
  // ceiling against a store that never shrinks a page (a degenerate store breaks the loop rather than spinning). The
  // test mock ignores cursor/limit (one whole-set page) so it is page-1-short and identical; the fleet-scale
  // validator's mock HONOURS startAfter+limit, proving >DO_LIST_PAGE entries are all enumerated.
  // Exhausting the guard bound TRUNCATES the enumeration -- silently, so the caller would act on an incomplete
  // fleet and the missing downpipes would read as NONEXISTENT everywhere (the pack included). It is counted.
  ...
  // findRunRing is the shared run -> ring resolver that destinationForRun and destinationsForRun both go
  // through. It maps a runId to the downpipe whose recent-run ring carries it, returning that downpipe's
  // state and the matched history row (or null when the run is in no ring, having aged out of the ring or
  // belonged to a removed downpipe). The lookup is BATCHED: issuing one storage.get(`hist:<id>`) per
  // downpipe would be an O(N) round-trip fan that grows with the fleet. The platform's
  // DurableObjectStorage.get accepts an ARRAY of up to DO_GET_BATCH keys and returns a Map in ONE round
  // trip, so it collects every `hist:<id>` key and reads them in chunks of DO_GET_BATCH, then searches the
  // rings, bounding the read count to ceil(N / DO_GET_BATCH) batches instead of N. The rings are searched
  // in the SAME order listAllByPrefix yields (ascending key order via map.values()), and the FIRST
  // downpipe whose ring contains the run wins. Legacy rows (no recorded origin) are handled by the
  // callers.
  private async findRunRing(runId: string): Promise<{ ds: DownpipeState; entry: RunHistoryEntry } | null> {
    ...
    // each ring key alongside its state. map.values() is ascending key order, so the first-match
    // semantics below are deterministic.
  ...
  // two-object window). A lease bounds a crashed holder; the next run takes
  ...
  //
  // PER-DESTINATION KEYING: each self-contained destination archive has its OWN RUNLOG object, so
  // finalisers writing DIFFERENT destinations never contend -- only those racing for the SAME
  // destination's one RUNLOG must serialise. `key` is that destination id; an absent/blank key (the
  // default destination, and the legacy unkeyed callers) maps to the historical "runlogLock" slot,
  // keeping the persisted lease shape stable for them. A storm of concurrent writers spreads across as
  // many lock slots as there are destinations instead of all queueing on one, raising the throughput
  // ceiling.
  ...
  async releaseRunlogLock(req: { token: string; key?: string }): Promise<{ ok: true }> {
    ...
    // never sent to a client and never leaves the DO. It is not a secret requiring a constant-time compare,
  ...
  async rateCheck(req: { key?: string; cost?: number; max?: number }): Promise<{ allowed: boolean; retryAfterMs: number }> {
    ...
    if (cur.count + cost > max) {
      ...
      // The tighter per-IP `ip:` bucket is the unauthenticated auth-ceremony limiter, so a cap hit here is
  ...
  // shapes the Response:
  //  - applied (gate OFF): auto-snapshot config history (best-effort) and return the mutation's own
  //    result as a plain 200, with the same body, status and side effect the route would produce calling
  //    the mutation directly. proposeConfigMutation runs the SAME method the route would call directly,
  //    so its result is preserved verbatim.
  ...
  // maps to a 400 { error }, so a bad/unauthorised request is rejected at propose time on BOTH paths.
...
// Re-export the this-free helpers that were exported from this file, so every existing by-name importer
// (src/admin/*, src/seal/*, src/cron/*) keeps working. The definitions now live in the leaf sibling
// modules imported above; these modules are leaves that this file depends on, never the reverse, so
