...
export interface CfConfigDiscovery {
  ...
  unavailable: string[];
  // unavailableByClass is the CLOSED status-class -> count map behind `unavailable`. The id list alone
  ...
  // truncated are surfaces whose probe read hit a pagination truncation. They are classified `present`
...
export interface DownpipeConfig {
  id: string;
  name: string;
  cadenceSeconds: number; // requested cadence; the EFFECTIVE floor is the engine cron tick (cron/cron-cadence.ts, pinned to the wrangler crons line, */15 today), so a smaller value dispatches at the next tick, not sooner (the console floors the picker at Hourly). The floor follows that line rather than any platform limit: Cloudflare cron triggers accept minute granularity. cron parsing is a refinement
  enabled: boolean;
  source: SourceSpec;
...
interface DownpipeState {
  ...
  createdAt?: number;
  // sealedRuns is the downpipe's MONOTONE lifetime count of SUCCESSFUL runs: incremented by completeRun
  ...
  sealedRuns?: number;
  // replAnchors maps each CONFIGURED fan-out destination id to the value of sealedRuns when that
  ...
  // in-flight flag as a LEASE: a run whose worker invocation is evicted, hits a limit, or is
  ...
  // lastRestoreTestReason / restoreTestConsecutiveFailures record WHY the most recent scheduled restore
  // test FAILED and how many have failed IN A ROW. The failure
  ...
  // lastRestoreOom is the most recent restore-subsystem OOM-risk marker, recorded by
  ...
  // restoreTestStartedAt is set to epoch ms just before the cron
  ...
  // lastSealVerify is this downpipe's MOST RECENT verify-at-seal verdict: the result of
  ...
  // recoverySamples (RTO estimate) is a bounded ring (RTO_SAMPLE_CAP, newest-last) of MEASURED
  ...
  // deepVerify is the per-downpipe windowed cursor for the scheduled restore test's rotating
  ...
  // cronResolve is the MOST RECENT outcome of resolving this downpipe's cron schedule at reschedule time (set by
  ...
  // blackoutResolve is the MOST RECENT outcome of
...
export interface SealVerification {
  ...
  recovered?: boolean;
  // tier0Cause is WHY a "verified" verdict ran only Tier-0
  ...
  // The evidence a SUSPECT verdict on a 50,000-record run needs and could not carry. All three are
...
// RestoreOomMarker is the restore-subsystem OOM-risk marker recorded on a downpipe
...
export interface RunHistoryEntry {
  ...
  // destination-local RUNLOG's freshness.prevRunId) -- null when this is the downpipe's first
  ...
  // key / R2 object DELETED between the list page and the GET -- a mid-crawl live-source race). It surfaces
  // on the run row so a churning source is visible ("N objects vanished mid-crawl") rather
  ...
  // recordsIncomplete is how many records this run sealed as INCOMPLETENESS SENTINELS: a
  ...
  // incompleteByMarker is the PER-MARKER breakdown of recordsIncomplete:
  ...
  // incompleteIds is the PER-KIND ATTRIBUTION of incompleteByMarker: for each marker kind, a bounded,
  // deduplicated list of WHICH surface/object was short. NO-CUSTODY: only
  ...
  // opCounts is the exact per-resource Cloudflare op tally this run made: KV/R2/D1 reads,
  ...
  destinationId?: string;
  // Verify-at-seal verdict: the result of reading this run's archive BACK from the
...
// secret; note is redaction-safe free text (build-contract 1.4.7). The log is NOT hash-chained
...
// FleetDrillCampaign is an ON-DEMAND bulk restore-test ("drill the whole fleet now") campaign.
...
// at the END of each invocation. A Worker cron completing without throwing is a FALSE GREEN: the tick can
...
// StorageFaultKind is the CLOSED classification of a persist-state storage.put failure:
// "value-too-large" is the DO 128 KiB per-value limit; "put-failed"
...
// ---- Scheduler housekeeping-health counters -----------------------------------------------------
...
// ---- Per-downpipe state-refusal stamp -----------------------------------------------------------
