...
// functions (no Date.now, no storage) kept apart from scheduler-do.ts so the class file stays
// lean: scheduler-do.ts imports these back and re-exports scheduleTimeZone / deferPastBlackouts.
// This module is a leaf that depends only on the other leaves cron.ts and types.ts, never on
// scheduler-do.ts, so madge stays at 0 cycles.
...
//
// The direction is deliberate: a cadence on a backup is a promise about how stale the newest archive
// may get, so overshooting it is the harmful direction. Adding the offset instead of subtracting it
// would make a "daily" downpipe read as 24h to 26.4h and breach the interval the operator asked for.
// Subtracting keeps the promise, at the cost of running slightly sooner and doing slightly more work.
//
...
// end-walk-exhausted class rather than deferring on a value it could not prove is outside the window.
...
// deferPastBlackoutsResolved is deferPastBlackouts with its RESOLUTION OUTCOME (guarding against blackout
// windows being silently violated). Scheduling behaviour is unchanged; the closed class the caller stamps
// on the downpipe for the support pack distinguishes:
