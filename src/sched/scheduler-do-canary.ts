...
// is a mixin. CanaryMixin layers these methods over a base whose `this` is SchedulerDOSurface;
// dispatch and `this` binding are byte-identical. The canary state key, the lease discipline, the
// flight-plan/liveness shapes and the multi-destination aggregation are unchanged. No storage key,
  ...
  return class extends Base {
    ...
    async resolveEffectiveDests(s: CanaryState): Promise<(string | null)[]> {
      ...
      if (s.config.destinationIds === null) {
        ...
        // EVERY pinned destination has been deleted, so the flight silently goes to the DEFAULT instead.
      ...
      // A destination collection larger than CANARY_MAX_DESTS is TRUNCATED here, so the destinations past
    ...
    async setCanaryConfig(
      ...
      if (req.destinationIds === null) {
        ...
        for (const id of ids) {
          if (!list.some((d) => d.id === id)) {
            await recordAdminRefusal(this.state.storage, "canary-config", "not-found"); // the refusal the operator saw once
      ...
      await this.appendAudit({
        ...
        outcome: "success",
        // Enable and disable must not write the SAME event: proving the canary was switched off before
        // the incident needs to be answerable from the trail. canaryOp is a closed enum naming WHICH control moved
    ...
    // canaryOpOf names WHICH canary control an owner's change actually moved. PURE. Ordering is the
    ...
    async canaryDue(nowMs: number): Promise<{ due: boolean; run?: CanaryFlightPlan }> {
      ...
      // This ALLOCATES a flight while the PREVIOUS one is still marked in flight -- i.e. its
    ...
    async canaryComplete(req: { run?: unknown; results?: unknown }): Promise<{ transitions: Array<{ destinationId: string | null; label: string; transitioned: "dead" | "recovered" }> }> {
      ...
      if (!run || !Array.isArray(run.dests) || !Array.isArray(results)) {
        // An unusable completion body throws past EVERY destination's liveness update in that flight, so
      ...
      for (const result of results) {
        if (!result || typeof result.status !== "string") {
          // ONE destination's result row was unusable and is skipped, so THAT destination's bird FREEZES
    ...
    //
    // THIS IS THE SECOND PLACE AN EXPIRED LEASE IS RECLAIMED, and it must say so exactly as the first one does.
    // canaryDue counts the same abandonment as `lost-flight` -- the record exists, in the ledger's own
    // words, because "a flight allocated under a lease that then expired is silently re-allocated on the next
    // tick, so 'lastRunAt keeps sliding' has no cause". Reclaiming it HERE without counting it would clear
    // `inFlight` silently, so the count canaryDue would otherwise take on the next tick is never taken and the
    // flight is lost with nothing saying so. The route this sits behind is POST /admin/canary/run, the console
    // "Fly now" button: the action an operator takes BECAUSE the bird looks stuck must not be the action that
    // erases the evidence of why it was stuck. Booked through the same closed vocabulary rather than a second
    // mechanism.
    //
    // The two sites must also agree at the boundary EXACTLY, avoiding the "each branch correct about its own
    // case" trap: canaryDue holds the lease while `elapsed < CANARY_LEASE_MS`, so elapsed === the lease is
    // EXPIRED there; if this one reclaimed only past `> CANARY_LEASE_MS`, then at that one instant the cron
    // path would reclaim-and-count while the operator path left the bird wedged. They share one edge.
    async canaryRunNow(): Promise<{ ok: boolean }> {
      const s = await this.ensureCanaryState();
      if (!s.config.enabled) {
        await recordAdminRefusal(this.state.storage, "canary-run", "disabled");
    ...
    async getCanaryTransitions(): Promise<{ enabled: boolean; status: CanaryLiveness; deadDestinations: number; transitionCount: number; transitions: CanaryTransition[]; nextRunAt: number | null; deadDetail: Array<{ destinationId: string | null; at: string | null; deadReason: string | null; byteDelta: number | null; failedAspects: string[] }> }> {
      ...
      // deadDetail: the WHY of each current death, from the destination's
