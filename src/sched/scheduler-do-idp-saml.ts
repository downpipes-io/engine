// The native SAML SP mixin, kept apart from scheduler-do-idp.ts so neither
// file exceeds the module-size guardrail (the sign-in-context wiring pushes the combined module
// over 800 lines). Same mixin discipline as every sibling: `this` is SchedulerDOSurface, so the
// methods reach nativeSessionIssue / recordSignInContext / appendAudit / recordSsoFail exactly as
// a single class would; the assembly chains IdpSamlMixin right after IdpMixin in scheduler-do.ts.
// A leaf the assembly depends on, never the reverse (madge 0 cycles). No behaviour, route, storage
// key, status code or response differs from a single-class implementation.
...
export function IdpSamlMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    // auth-signal aggregate the pack already carries. The ledger is a SET, so one hostile
    ...
    async idpSamlMetadata(body: { connId?: unknown; acsUrl?: unknown }): Promise<{ ok: true; metadata: string } | { ok: false; reason: string }> {
      ...
      // The metadata path matters because a refusal here is where a SAML rollout silently stalls: the SP
      // EntityDescriptor is what the customer uploads INTO their IdP, and the SSO aggregate is written only
      // by the ACS, so without this the pack shows nothing at all for a metadata failure. Record the closed
      // metadata-failure code (best-effort; the returned refusal is unchanged) with the connection's OPAQUE
      // ORDINAL where one is resolvable, never the connId.
      if (!isConnId(connId)) {
    ...
    async idpSamlStart(body: { connId?: unknown; returnTo?: unknown; acsUrl?: unknown }): Promise<{ ok: true; redirectUrl: string; browserBind: string } | { ok: false; reason: string }> {
      const connId = typeof body.connId === "string" ? body.connId : "";
      // The start path matters for the same reason: "Users clicking our old SSO link get an error page" is a
      // deleted or disabled connection refusing at /start, and because the SSO aggregate is written only by
      // the ACS wrapper, without this the pack shows zero SSO failures while every sign-in attempt dies.
      // Record the closed start-failure code on each refusal (best-effort; the returned refusal is
      // byte-identical), attributed to the connection's OPAQUE ORDINAL where one is resolvable, never the
      // connId.
      if (!isConnId(connId)) {
      ...
      // buildRedirectUrl THROWS when the stored SSO URL is unusable or the DEFLATE leg faults, and the
      // throw propagates out of the start route as a bare 500 -- the "SAML sign-in button dead-ends" case, with
      // zero SSO failures in the pack because recordSsoFail is only ever written by the ACS. The pure builder
      // notes the closed class; drain it here on BOTH paths, then re-throw so the caller's behaviour is unchanged.
      ...
      try {
        ...
        // The same start-path blindness applies on the THROW leg (an unusable stored SSO URL, a DEFLATE
        // fault): the bare 500 propagates and nothing enters the SSO aggregate without this. Counted with
        // the closed code + ordinal before the re-throw, so the caller's behaviour is unchanged.
        await this.recordSsoFailCode("start-failure", "saml", connId);
        throw e;
      }
      await this.drainSamlSignalsToAggregate();
    ...
    async idpSamlAcs(body: { connId?: unknown; samlResponse?: unknown; relayState?: unknown; acsUrl?: unknown; browserBind?: unknown; sourceIp?: unknown; userAgent?: unknown }): Promise<{ ok: true; token: string; returnTo: string } | { ok: false; reason: string }> {
      ...
      // The ACS failure also carries the connection's OPAQUE ORDINAL, so "SAML sign-ins failing
      // intermittently" on a two-SAML-connection tenant says WHICH one. The connId is passed only into
      // ssoConnOrdinal (the redaction chokepoint) and never becomes a counter key or a pack field.
      if (!r.ok) {
        const failConnId = typeof body.connId === "string" ? body.connId : undefined;
        await this.recordSsoFail(r.reason, "saml", failConnId);
        // V16.3.1: the counters above are a saturating {count, lastAt} map capped at SSO_FAIL_COUNT_CAP. They
        // are not on the hash chain, they are not ordered, and once saturated they cannot separate a sweep that
        // stopped in July from one running now. So EVERY rejected ACS -- the SP-initiated-only refusal, the
        // RelayState connId mismatch, the browser-binding mismatch that is a forced-login attempt, the disabled
        // connection, every signature / Conditions / InResponseTo failure, and the seen-assertion replay refusal
        // -- also goes on the tamper-evident chain as an authn-failure row, coalesced per minute by the shared
        // recorder. No identity is verified on a failure, so the actor fields stay null; the coarse source IP
        // the router forwarded from the edge CF-Connecting-IP is carried, which is more forensics than the
        // passkey sibling structurally has.
        //
        // connId IS RECORDED ONLY WHEN THE CONNECTION RESOLVED. On a probe at /admin/saml/acs the submitted
        // connId is an attacker-chosen path segment, and letting an unauthenticated caller write a chosen string
        // into the chain is the mistake the recovery path already refused with an unverified email. When it did
        // not resolve, the ceremony alone carries the fact.
        //
        // IT STAYS IN THE WRAPPER, NOT THE IMPL. Every failure of the impl already funnels through here, so no
        // branch of the impl gains a step another lacks and the anti-enumeration timing on the ACS is unchanged.
        // Moving this inside the impl to reach a local is exactly how an oracle would be introduced.
        const resolved = failConnId !== undefined && isConnId(failConnId) ? await getIdpConnectionRaw(this.idpKv, failConnId) : undefined;
        await this.appendAuthnFailureAudit({
          ceremony: "sso-callback",
          actorMethod: "saml",
          sourceIp: typeof body.sourceIp === "string" && body.sourceIp.length > 0 ? body.sourceIp : null,
          ...(resolved !== undefined && failConnId !== undefined ? { connKind: "saml" as const, connId: failConnId } : {}),
        });
      }
      // DRAIN the pure pipeline's signal ledger on EVERY outcome, not just the failing one. That is
    ...
    async idpSamlAcsImpl(body: { connId?: unknown; samlResponse?: unknown; relayState?: unknown; acsUrl?: unknown; browserBind?: unknown; sourceIp?: unknown; userAgent?: unknown; priorToken?: unknown }): Promise<{ ok: true; token: string; returnTo: string } | { ok: false; reason: string }> {
      const connId = typeof body.connId === "string" ? body.connId : "";
      ...
      // DRAIN the pinned-cert health observation the (pure) verifier just computed, BEFORE the failure
      ...
      // The assertion VERIFIED and an advisory claim was BOUNDED AWAY (an over-length AuthnContextClassRef,
      // an unparseable AuthnInstant). The sign-in SUCCEEDS, so the recording has to happen here, on the
      // success path, or nowhere at all. Mirrors the OIDC/OAuth2 callback's recorder. The counters name the
      // boundary and the drop KIND; the claim value never leaves the bounder. Best-effort: it never blocks
      // the mint.
      ...
      const minted = await this.nativeSessionIssue(result.principal, connId, "saml", nowMs, result.sessionNotOnOrAfter, body.priorToken, { sourceIp: typeof body.sourceIp === "string" ? body.sourceIp : null, userAgent: typeof body.userAgent === "string" ? body.userAgent : null });
      if (!minted.ok) return minted;
      // V6.3.5: same opt-in coarse-context check as the OIDC success block; see the note there.
