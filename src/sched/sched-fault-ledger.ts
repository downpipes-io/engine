// The SCHEDULER-DO fault ledger: small, best-effort, never-throwing recorders that turn scheduler and identity
// faults that would otherwise leave no durable trace into closed-vocabulary, redaction-safe evidence the
// support pack can carry.
//
...
// recorder that gates on it are never in different modules; the split follows the family seam the section
// banners mark. Every symbol the siblings export is re-exported here, so every caller keeps importing from
// this one path.
//
...
// The siblings are re-exported so every caller keeps importing this one path. A split across this family
// always moves a whole vocabulary together with the recorder that gates on it, never one without the other,
// so the import surface stays unchanged for callers.
export * from "./sched-fault-core.ts";
export * from "./sched-fault-admin-ledger.ts";
export * from "./sched-fault-client-ledger.ts";

// ---- vocabDrops: the closed-set gates that DROP an out-of-vocabulary observation ----------
//
...
// ---- expiryObserveFaults: the credential-expiry warning ladder that never ARMED ------------
//
...
// ---- approvalFaults: the dual-control restore lifecycle's REFUSALS and lease anomalies -----
//
...
// ---- storageAnomalies: the SILENT self-heals, coercions and safe-default reads -------------
//
...
// ---- rosterDiscards: WHICH claimant a roster repair threw away --------------------------------
//
...
// ---- testOutcomes: the wiring-check button whose ANSWER died with the browser tab --------------
//
...
export const TEST_REASON_CLASSES = [
  ...
  // "delete-denied" is deliberately not part of this vocabulary, which is why the dead-vocab gate exists: to
  // catch a class like it reappearing with no producer.
  //
  ...
  // contain the words, so it would have ZERO producers.
  //
  // The fact is real and worth carrying; it is simply not a FAILURE REASON. It rides on its own discriminator
  // (deleteProbe below), beside the probe's object-lock verdict, on a row whose ok is honestly true.
...
// ---- the DELETE-DENIED destination: a PASS that hides a broken retention -----------------------------
//
...
// default-retention boolean. Every one of those four fields is a MEASUREMENT, never an intent:
//
...
// representable.
//
// "denied" IS A PERMISSION FACT, not an inference from the bare fact of a throw. s3-read-ops del() throws on
// ANY non-204/404 status and on a transport fault, so a 503 SlowDown, a 500 store error, a 429 throttle and a
// dropped socket must not all land as "denied" -- that word sends support to an IAM allow-list that is already
// correct, and files a DURABLE row saying this customer's retention is permanently broken because one request
// was unlucky. In the other direction, collapsing every case to one class would let a genuine least-privilege
// denial get waved away as "the store was flaky".
//
// del() reads the status off the response before it throws, and throws it TAGGED (dest/types.ts
// DestStatusError), so the probe classifies what it MEASURED:
...
export interface TestOutcomeEntry {
  ...
  // The EMAIL surface ONLY, and only on a FAILED send: WHAT THE EMAIL SERVICE ACTUALLY SAID, as the bounded
  ...
  // domain, an unverified sender and a platform throttle would be ONE row -- {"ok":false,"reasonClass":"other"} --
  // with three different remedies, and "our test email never arrives" would be unanswerable from the pack. It is
  // the same bounded token the notify history already carries (sanitiseEmailPlatformCode): a shape-gated code,
  // never the platform's message. It is also admitted on the NOTIFY-CHANNEL surface, because the console's OTHER
  // Test button (POST /admin/notify/test, on a kind:"email" channel) asks the same Email Service the same
  // question through the same adapter, and would otherwise collapse E_SENDER_DOMAIN_NOT_AVAILABLE,
  // E_SENDER_NOT_VERIFIED and an invalid EMAIL_FROM into one {"ok":false,"reasonClass":"rejected"} row.
  platformCode?: string;
  // THE NOTIFY-CHANNEL SURFACE'S OWN DISCRIMINATOR. deliverToChannel ALREADY returns a closed DeliveryFailCode
  // (http-bad-request, http-gone, http-4xx, email-platform-rejected, email-from-invalid, ...); laundering it
  // through `reason` into classifyTestFailure would coarsen ALL of those to "rejected" via its text arm. So a
  // DEPROVISIONED Slack webhook (410: recreate the integration), a wrong path on a live sink (404: fix the path)
  // and an engine payload regression (400: OUR bug) would be one byte-identical row with three opposite
  // remedies. The code rides here, unlaundered, on the notify-channel surface only, and only on a FAILED test.
...
export function classifyTestFailure(reason: unknown, status?: unknown): TestReasonClass {
  ...
  // There is no delete-denied arm: a denied delete never fails the probe, so no reason string reaching here
  // can ever carry those words. Adding one would only make a dead class look alive to a reader (and to a
  // literal-scanning gate).
...
export async function recordTestOutcome(
  ...
  // Closed-set validated HERE like every other field, and admitted ONLY on dest-verify -- the one surface
  ...
  // The delete's own status class, admitted ONLY where it describes something -- a delete that FAILED. On an
  ...
  // The reason is admitted ONLY where it means something (objectLock "unknown"), and the retention boolean
  ...
  // The email platform code, re-gated HERE through the same shape gate the notify path uses, and admitted
  ...
  // The SAME gate also covers the notify-channel surface, which asks the SAME question: the notify-channel Test
  // button on a kind:"email" channel drives the identical sendEmail primitive and gets the identical platform
  // code back. The gate stays the shape gate (an E_UPPER_SNAKE token or nothing, never the platform's message),
  // so covering that surface too admits no new leak surface. Every other surface (push, idp, dest-verify) still
  // cannot carry one.
  ...
  // The closed DeliveryFailCode, re-gated HERE against the channel layer's own allow-list (the recorder is
...
// ---- chainBreaks: the tamper verdict that SELF-HEALS and leaves no trace -----------------------
//
...
/**
 * chainBreakKey is the latch's TUPLE KEY. Keying on the chain name alone would let a later, genuinely
 * DIFFERENT break fold into an earlier one and disappear: latch a seq-gap at 41, then detect an edited
 * snapshot at 12, and a chain-only key would show one row -- seq-gap@41, detections:2. The edited snapshot at
 * version 12 would be invisible, and "detections: 2", read as evidence of a standing break, would be
...
/**
 * recordChainVerdict latches ONE chain-verify outcome. A BREAK sets (or re-touches) the latch; an
 * INTACT verdict does NOT clear it -- it stamps healedAt, so "it shows intact now" and "it showed a break
 * earlier" are both in the pack, together, which is the only way that ticket is answerable.
 *
 * Never throws (a diagnostic must not break a verify).
 *
 * It CONSULTS THE ROTATED-KEY FACT before it latches a forgery. The config-history digests are HMAC'd
 ...
 * CONTEXT. Filing that state under a cause whose documentation asserts a forgery -- "the record's attribution
 * was forged, or the record was fabricated without the DO's HMAC key" -- would give a customer whose key
 * rotated a durable, tamper-shaped row about an attacker who does not exist. A rotated key has its own cause,
 * and it displaces every digest-family cause, because with the key gone the digest check proves nothing at
 * all.
 *
 * AND IT REFUSES TO SAY "NOTHING WAS TAMPERED" UNTIL THE UNKEYED EVIDENCE AGREES.
 *
 * verifyConfigChain returns at the FIRST break, so with the key gone it fails on version 1's digest and never
 * looks at 2..N. Displacing the whole digest family to signing-key-rotated would therefore assert non-tamper
 * about versions the code has never examined -- and the key can be destroyed by an OWNER BUTTON
 * (terminate-all-sessions deletes the passkey session key), so any actor with the DO-storage write access a
 * tamper already requires could press it and turn every config-history tamper into the row that says nothing
 * happened. A one-button mask.
 *
 * The content hashes, the parent links and the id contiguity are UNKEYED: a rotated key cannot fake or break one
 * of them. The caller runs that pass over the WHOLE chain and passes its verdict here. signing-key-rotated is
 ...
 * IT ALSO LATCHES A TRUNCATED HEAD, WHICH A RECOMPUTE PASS ALONE IS BLIND TO.
 *
...
export async function recordChainVerdict(
  ...
  await writeLedger<ChainBreakLatches>(storage, CHAIN_BREAKS_KEY, "chain-break", (prior) => {
    ...
    // A ROTATED OR LOST in-DO signing key makes every keyed digest fail, so every digest-family cause the
    ...
    // The KEY-FREE evidence decides whether the key fact may speak for the whole chain. A rotated key cannot
...
// ---- recoveryResume: what the DR auto-heal SILENTLY dropped, repointed and stripped ---------
//
// After a control-plane wipe the auto-heal replays the signed export. It is deliberately TOLERANT (one
// malformed downpipe must not block the fleet's resume), and every tolerated defect is otherwise invisible: a
// skipped downpipe survives only as the integer resumeSkipped, a dangling default destination is silently
// re-pointed at whatever destination happens to be first (backups then land in an UNEXPECTED BUCKET), an
// import validator drops a WORM/addressing/storage-class/pricing field it does not accept (the customer's
// retention posture quietly vanishes), and a staged apply refusal records no class at all. This record is
...
// back" case, whose id would otherwise survive only as an integer).
...
// ---- destResolveFallback: the pipe that SILENTLY writes to a destination it was not pinned to ----
//
...
// ---- canaryLosses: the flights that were never flown, and the results that were never read ----
//
...
// ---- drillDrops: the restore-test/fleet-drill pipeline's SILENT discards --------------
//
...
// alarms the customer about a downpipe that no longer exists. The drill-evidence half of this ledger covers
// the dated evidence trail the customer's auditor reads: it is written best-effort, so an empty trail cannot
// be told apart from every evidence write having failed since a route regression.
export const DRILL_DROP_KINDS = [
  ...
  "evidence-refused", // a drill-evidence append was REFUSED by validation (a bad runId/kind/note): the dated trail silently has a hole
  "evidence-write-failed", // the drill-evidence append itself FAULTED on storage: the drill happened (downpipes[].lastRestoreTestAt proves it) and its evidence row does not exist
] as const;
...
// ---- notifyDrops: WHICH alert died, not just how many ----------------------------------------
//
...
// ---- governanceFaults: the dual-control / change-control apply attempts that leave no trace ----
//
...
// ---- governanceRefusals: the REASON, which "guard-refused" erases -----------------------------------
//
...
export const GOVERNANCE_REFUSAL_REASONS = [
  ...
  "too-soon", // the approve arrived less than APPROVAL_MIN_DWELL_MS after the request was raised: no checker could have reviewed it in that time
] as const;
...
// ---- refusalRings: the last-only counters that collapse five causes into one ------------------
//
...
export const REFUSAL_SURFACES = [
  "licence-activation", // POST /admin/licence refusals, including the DO-side shape reject (which would otherwise bypass the counter entirely)
...
// The closed config-snapshot failure classes: without this recorder the root cause would live only in Workers
// Logs, invisible to the pack.
...
// ---- alertingHealth: the alerting pipeline's own health ----------------------------------
//
// The alerting pipeline is the thing a customer trusts to tell them their backups broke, and its OWN health
// is otherwise invisible. Every member here is a way the pipeline goes quiet WITHOUT any evidence:
...
// ---- auditEgress: the SIEM audit feed's own health ----------------------------------------
//
...
// capTruncations lives in sched-fault-core.ts, one layer down, because the freshnessFaults cap is
...
// ---- posture: the security-centre evaluation, carried into the pack ------------------------
//
// The Security Centre's verdicts drive real customer action ("retire the break-glass token", "admin-strong-auth
// cannot be verified"). Without the last evaluation, the overrides, and the INPUT FAULTS that make a check read
// cannot-verify, a dropped IDP_KV binding (a permanent, one-line fix) reads exactly like a transient blip, a
// version-skewed status slice produces a SPURIOUS regression, and a real WORM misconfiguration is flattened to
// "not configured".
//
...
export interface SchedDiagBundle {
  ...
  vocabDrops: FaultCountAgg;
  expiryObserveFaults: FaultCountAgg;
  approvalFaults: FaultCountAgg;
  storageAnomalies: FaultCountAgg;
  rosterDiscards: RosterDiscardEntry[];
  chainBreaks: ChainBreakLatches; // the tamper verdict a rollover would otherwise erase
  testOutcomes: TestOutcomes; // every Test button's outcome; without this ledger it dies with the browser tab
  recoveryResume: RecoveryResumeDiag | null;
  destResolveFallbacks: DestResolveFallbackRecord | null;
  canaryLosses: CanaryLossRecord | null;
  drillDrops: FaultCountAgg;
  notifyDrops: NotifyDropRecord | null;
  governanceFaults: FaultCountAgg;
  governanceRefusals: FaultCountAgg; // the closed reason behind a governance refusal, which guard-refused erases
  refusalRings: RefusalRings;
  adminRefusals: FaultCountAgg;
  clientDiagnostics: ClientDiagnostics | null;
  configRejections: FaultCountAgg; // surface|reason -> {count,lastAt}
  authzRefusals: FaultCountAgg; // gate -> {count,lastAt}
  alertingHealth: FaultCountAgg; // closed event -> {count,lastAt}
  configCoercions: FaultCountAgg; // surface|dropClass -> {count = dropped items, lastAt}
  auditEgress: AuditEgressHealth | null;
  capTruncations: FaultCountAgg; // surface -> {count = dropped rows, lastAt}
  capTruncationSubjects: CapTruncationSubjects; // surface -> which subjects were dropped, and whether any could not be named
  posture: PostureDiag;
}
