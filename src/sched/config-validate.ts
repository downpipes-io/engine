...
// Durable Object storage key. scheduler-do.ts imports these back under the same names, and re-exports
// RESERVED_BINDINGS so the existing importers (attach.ts, bindings-sync.ts, preflight.ts, restore.ts,
// router-sources.ts, seal/adapters.ts) keep working with no change. This module depends only on the leaves
// cron.ts / types.ts / schedule-window.ts, never on scheduler-do.ts, so madge stays at 0 cycles.
...
//
// Re-add "artifacts" HERE to re-enable it, and nowhere else. This allow-list, the discovery-source
// allow-list in scheduler-do-account-config.ts, and an ARTIFACTS_GA constant in the console must stay in
// step: flipping the console's alone would offer the operator a source this validator then refuses, which
// is a source they can select and never protect. Both other sites derive from this list, and the console
// has a cross-repo check that fails if it offers a type this refuses, so the gate opens in one edit or not
// at all.
  ...
  if (c.schedule !== undefined) {
    ...
    if (sch.blackoutWindows !== undefined) {
      ...
      for (const w of sch.blackoutWindows) {
        ...
        // a change freeze. Refuse it at the write path so the degenerate case becomes a counted refusal
