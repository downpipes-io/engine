  ...
  return class extends Base {
    ...
    async setRequireChangeNumber(
      ...
      await this.appendAudit({
        ...
        target: { kind: "access-policy", policyName: "change-number", newValue: req.requireChangeNumber === true }, // name the policy + the direction
      });
    ...
    async enforceChangeControl(
      ...
      if (!verdict.ok) {
        ...
        // The tally keeps {count, lastAt, lastActionKind}, so "14 refusals" shows only the LAST action
        ...
        // The closed REASON, alongside the action kind. changeControlRefusals gives count + lastActionKind
    ...
    async readEmergencyChangeMarker(): Promise<EmergencyChangeMarker> {
      ...
      // A marker that EXISTS but reads back malformed coerces to zero, which is the compliance posture
    ...
    async getChangeControlRefusals(): Promise<ChangeControlRefusalState> {
      ...
      // A marker that EXISTS but reads back malformed coerces to zero, the SAME coercion
