// The control-plane RECOVERY subsystem on the SchedulerDO.
//
...
export function ControlPlaneMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    async buildControlPlaneExport(): Promise<ControlPlaneExport> {
      ...
      // reconcile RESTORES FROM. A head pointer whose count has been zeroed must never be carried into the
      // export verbatim: a disaster-recovery artefact taken over a damaged record would anchor the recovered
      // chain at seq 0. The trust rule derives the true head from the chain on exactly the records that cannot answer,
      // and the loss is booked; the export's SHAPE is unchanged, because it is signed and validated and this
      // is a repair to what goes in it rather than to what it is.
    ...
    //
    // runlogSeed: the caller's own verified read of
    ...
    async applyResumeDownpipes(exp: ControlPlaneExport, tolerant: boolean): Promise<number> {
      ...
      for (const config of exp.downpipes) {
        try {
          ...
          // This skip is the "one of my backups never came back" ticket. The integer resumeSkipped alone tells
          // support HOW MANY were dropped but never WHICH or WHY, and the customer's downpipe simply does not
          // exist any more. File the downpipe's own id (the label class
    ...
    async applyResumeDestinations(exp: ControlPlaneExport): Promise<number> {
      ...
      // When the exported default destination is NOT in the imported set, the default
      ...
      // The import re-validates the optional non-secret config through the live
      // validators, so a value this build does not accept is DROPPED. That is right, and it means an
    ...
    async reconcileControlPlane(
      ...
      if (!(await this.roleTableIsEmpty())) {
        // Naming only the reason and no way forward would leave "refused, and here is nothing" as the whole
        // operator experience, reached by exactly the accounts whose banner offers nothing else. Reaching this
        // line means the plane IS empty (the check above) and authority SURVIVED, which is not amnesia at
        // all: whoami does not degrade a caller over a non-empty role table
    ...
    // acknowledgeControlPlaneRecovery is the NARROW escape reconcileControlPlane and
    // applyControlPlaneAuthoritySlice cannot be: once a latched
    ...
    //     (scheduler-do-rbac-authority.ts): if the role table is EMPTY (the plane un-emptied only via
    ...
    async acknowledgeControlPlaneRecovery(caller: CallerLike): Promise<{ ok: true; acknowledged: true }> {
      ...
      // silent deadlock (empty table + spent bootstrap = every caller resolves to viewer, unexplained).
      if (await this.roleTableIsEmpty()) {
        // The guard holds regardless (the deadlock reasoning above still applies); the TAIL below is
        // conditional, because "use the break-glass reconcile instead" is true on
        // only one side of it. reconcileControlPlane refuses a NON-EMPTY plane (above), so on an estate whose
        // plane came back while the role table stayed empty -- the break-glass token can create downpipes
        // during a latch without granting any role, which is precisely how that state arises -- sending the
        // operator to a route that must refuse them is the commonest way this defect shows up, not a wording
        // nicety.
    ...
    // accountIdAbsent: the export, or this engine, could not NAME its Cloudflare account. sameControlPlane
    ...
    async importControlPlaneDefinition(
      ...
      return await this.blockConcurrencyWhile(async () => {
        ...
        await this.writeReconcileBridge(caller);
        // PERSIST the outcome. Riding it ONLY on the response body, a one-shot artefact the operator's browser
        // consumes and drops, would leave nothing for the support pack, which is generated AFTER the incident,
        // when the response is long gone. So "the estate is back on my screen and not one backup is running"
        // would reach support with no import row at all, and a GENUINE cross-account import would be
        // indistinguishable from an accountIdAbsent one (which disables every downpipe anyway, via the fail-safe
        // null compare). Same DO
        // state, same audit trail, same pack. Two rows now, with opposite remedies: re-point every source, or
        // set the account id and re-import (re-pointing sources fixes nothing at all in that second world).
    ...
    async applyControlPlaneResumeSlice(): Promise<{ ok: true; downpipes: number; downpipesExpected: number; resumeSkipped: number; appliedVersion: number; destinations: number } | { ok: false; reason: string }> {
      ...
      if (!candidateVersionMatchesExport({ version: staged.version }, staged.export)) {
        // The recovery STALLS here on every cron tick, and the pack's recovery section says only that a resume
        // has not happened. Recording the class (a relabelled or replayed export) is the whole diagnosis.
      ...
      return await this.blockConcurrencyWhile(async () => {
        ...
        // Resume-apply diagnostics. The tolerant apply SKIPS a malformed downpipe so one bad
    ...
    async applyControlPlaneAuthoritySlice(
      ...
      if (!(await this.roleTableIsEmpty())) {
        // This is the sibling of reconcileControlPlane's refusal above, reached by the same accounts, split on
        // the plane's own emptiness because the two halves have different next steps rather than different
        // wording.
        const planeEmpty = await this.controlPlaneIsEmpty();
        throw new Error(
          planeEmpty
