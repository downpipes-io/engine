// The SRE alerting subsystem (the on-call persona:
// backup-failure/stale and replication alert detection and reconciliation) as a mixin on the
// SchedulerDO, kept separate from the DO's other responsibilities.
// SreAlertingMixin layers these over a base whose `this` is SchedulerDOSurface; dispatch and `this`
// binding are byte-identical to a method defined directly on the class. The alert-cooldown discipline
// and the classify/shouldAlert detection live here, and the DO does NO network I/O at all (the Worker delivers).
...
// Records the alerting pipeline's OWN health. Every recorder below is best-effort and never throws, so the
...
// downpipe relapses, or a time bound is hit: a transient blip during recovery would otherwise
...
// emission path is otherwise unaffected. It lives here (its only consumer) rather than in the
...
export function SreAlertingMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    // bumpReplicationCopyCountInvalid records the copy-count-invalid counter through the SAME bounded applier the Worker edge
  ...
  async bumpReplicationCopyCountInvalid(): Promise<void> {
    ...
    // seal), which keeps delivery's fail-open behaviour entirely in the Worker and means a DO
    // storage op here can never be blocked on a customer's slow endpoint.
    //
    // Two-phase delivery for the first-alert retry. Cooldowns for STATE-TRANSITION alerts (where
    ...
    //
    // AUTO-RESOLVE (mon-autoresolve): the RECOVERY mirror of the alert stream. `recoveries` carries
    // one NotifyEmission per downpipe that just crossed back to healthy (the falling edge of the SAME
    // cooldown this method already tracked), built at the same site the cooldown is deleted below.
    ...
    async reconcileAlerts(): Promise<{ alerts: DownpipeAlert[]; pendingTransitionIds: string[]; successes: NotifyEmission[]; recoveries: NotifyEmission[]; pendingRecoveryIds: string[] }> {
      ...
      if (channelCount === 0) {
        // DETECTION IS NOW OFF, and the short-circuit says so to nobody. A tenant that deletes its last
      ...
      // Owed-resolve markers: a downpipe here has a recovery whose delivery previously failed, so
      ...
      // the falling-edge branch and the retry loop below so a retried resolve is byte-identical to the first
      ...
      for (const ds of downpipes) {
        ...
        if (toAlert !== null) {
          ...
          // run-failed (the audit mirror of a genuine new failure): gated on isTransition AND
          // toAlert === "failed" specifically, NOT on classify() alone -- isTransition fires on ANY
          // alert-state change, including a healthy-to-stale transition, and a stale downpipe has not
          // failed a run. This appends once per genuine transition INTO failed (never per tick, and
          // never on a same-state cooldown re-nudge, since isTransition is false then), mirroring the
          // engine-observed pattern diffStatus already uses in admin/audit-status.ts. The target's
          // runId is the most recent RESOLVED run in the history ring, which toAlert === "failed"
          // guarantees is itself a failed run (classify only returns "failed" from that same run).
          if (isTransition && toAlert === "failed") {
            ...
            if (lastResolved !== undefined) {
              await this.appendAudit({
                ...
                action: "run-failed",
                outcome: "failed",
                target: { kind: "run", runId: lastResolved.runId },
              });
            }
          }
          // This downpipe is alerting again: any owed resolve from a prior recovery is superseded (the
          // incident is re-opened by this trigger). The retry loop below drops its marker (never suppressed).
          ...
          //
          // Two-phase retry: the recovery is EMITTED once here on the falling edge and the
          // cooldown deleted (so a relapse is a fresh transition, never suppressed). Delivery is not
          // fire-and-forget: the Worker reports the outcome, and a FAILED recovery delivery writes a
          // pending-recovery marker (recordRecoveryDelivery) that the retry loop below re-emits on later
          // ticks until the resolve lands, the downpipe relapses, or it ages out -- so a transient blip
          // during recovery cannot strand a PagerDuty/JSM/ServiceNow incident open.
        ...
        if (latestOkRun !== undefined) {
          ...
          if (lastNotifiedRunId !== latestOkRun.runId) {
            ...
            if (recent && ds.config.enabled) {
              // A run that sealed incompleteness sentinels (markers, not the real bytes) is NOT a
      ...
      // RECOVERY RETRY LOOP. Re-emit an owed resolve for a downpipe whose recovery delivery
      ...
      for (const [key, marker] of pendingRecoveryMap) {
        ...
        if (now - marker.at > RECOVERY_RETRY_WINDOW_MS) {
          // THE OWED RESOLVE IS BEING ABANDONED. The backup recovered, the close was owed, delivery kept
    ...
    // RECOVERY batch, the recovery mirror of markAlertsDelivered. deliveredIds clear any owed-
    ...
    // runCronDeadManSweep is the cron-independent staleness dead-man (the dead-man's-dead-man),
    ...
    async runCronDeadManSweep(): Promise<void> {
      try {
        ...
        // THE DEAD-MAN HAS TRIPPED. Reaching here means the cron driver has been silent past
        ...
        // THE BATCH REACHED NO SINK, and it never can from here. The env-free backstop holds no channel
        ...
        // A fault INSIDE the backstop is exactly the "the dead-man was dead too" case, and it lands in a
    ...
    //
    // AUTO-RESOLVE (mon-autoresolve): `recoveries` carries one NotifyEmission per downpipe whose HELD
    ...
    async reconcileReplicationAlerts(): Promise<{ emissions: NotifyEmission[]; pendingTransitionIds: string[]; recoveries: NotifyEmission[] }> {
      ...
      const channelCount = (await this.state.storage.list<NotifyChannel>({ prefix: NOTIFY_CHANNEL_PREFIX })).size;
      if (channelCount === 0) {
        // The replication half of the same detection-off short-circuit (the one that would have told the
      ...
      for (const ds of downpipes) {
        ...
        // classifyReplication returns copyCountInvalid when the configured copy count is unreadable --
        ...
        // wrong, so the detector switching ITSELF off is a counted fact. The alerting decision stays unchanged
        // (an undecidable count is still not "degraded", because a guessed alert is worse than none); only the
