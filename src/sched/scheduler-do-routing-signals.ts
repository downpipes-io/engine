// The OPERATIONAL-SIGNAL and REPORTING half of the SchedulerDO's RPC dispatch: this sub-mixin owns the
// per-subsystem sub-dispatch methods for SRE alerting + notifications + the expiry tracker (the
// cooldown-throttled, two-phase reconcile/deliver signal hooks) and the reporting surfaces (scheduled
// restore-tests, posture, the report data the router signs, and coverage). Each method is a switch over the
// SAME `${method} ${pathname}` key that returns the handler Response for a key it owns or null ("not my
// route"); route() (in scheduler-do-routing.ts) chains these among the other sub-dispatches. This sub-mixin's
// `this` is SchedulerDOSurface (like every sibling mixin), so each sub-dispatch calls the owning handler
// (this.reconcileAlerts / this.listNotifyChannels / this.computePostureReport / ...) with the same dispatch
// and `this` binding the composed class has. A leaf the assembly depends on, never the reverse (madge 0
// cycles): no auth, route, status code, response body or dispatch order varies by how the switch is split.
  ...
  return class extends Base {
    ...
    async routeSreAlerting(req: Request, url: URL): Promise<Response | null> {
      ...
      case "POST /reconcile-alerts":
        // The cron heartbeat: stamp that the CRON-driven alert sweep ran this tick, so the DO's own
      ...
      case "POST /source-drift/reconcile": {
        // The `?? []` coercion below is not benign - an EMPTY list means "nothing is detached any more",
      ...
      case "POST /volume-regression/reconcile": {
        // Same `[]` coercion, same consequence - a malformed body clears the volume-regression marker, so
      ...
      // POST /recovery-delivered is the two-phase callback for the auto-resolve RECOVERY batch,
      ...
      // caller, so this DO does not independently re-check that gate on reads, unlike the write arms
      // above). The two INTERNAL
    ...
    async routeNotify(req: Request, url: URL): Promise<Response | null> {
      ...
      case "POST /notify/history/test-send":
        ...
        // code + platformCode ride the body so the persisted test row carries a REASON. Both are
      ...
      // The Worker reports a SILENT drop in the notify pipeline so "alerts stopped"
      ...
      case "POST /notify/health-bump": {
        ...
        if (field === null) {
          // An out-of-vocabulary field is REJECTED here while the Worker's fire-and-forget bump ignores
      ...
      // are any authenticated role (the router gates on posture.read; the arm below takes no caller, so
      // this DO does not independently enforce this gate on reads either). The route returns COMPUTED
      // ExpiryStatus[] (the
    ...
    async routeExpiry(req: Request, url: URL): Promise<Response | null> {
      ...
      case "POST /expiry/reconcile": {
        // ASVS V13.3.4: the cron driver forwards which env bindings are present + the public-key
        // fingerprints it derived from them (the DO holds no env), so the secret-rotation cadence seed can
        // run before the ladder walk. A malformed/absent body degrades to {} (no bindings reported
        // present), matching every existing no-body caller of this route unchanged.
        const body = (await req.json().catch(() => ({}))) as { presentBindings?: Record<string, boolean>; fingerprints?: Record<string, string | null>; adminTokenDisabled?: boolean };
        return this.json(await this.reconcileExpiry(body));
      }
      case "POST /secret-rotation/confirm":
        // Owner-exclusive (keys.ceremony; the router gates first, this re-checks). An operator attests a
        // bearer/destination credential named in the rotation schedule was rotated.
        return this.json(await this.confirmSecretRotation((await req.json()) as { secret?: unknown }, decodeCaller(req.headers.get(CALLER_HEADER))));
    ...
    async routeRestoreTests(req: Request, url: URL): Promise<Response | null> {
      ...
      // Retention-prune pass record: the cron's retention pass posts ONE bounded,
      ...
      // many prune AUDIT writes were lost -- and the support bundle reads the latest back (GET). Without this
      // record the cron's post would be a silent no-op, leaving the whole "storage keeps growing despite my
      // retention policy" family (enforce never set / perpetually deferred / half-applied / WORM-refused) invisible.
      // DIAGNOSTIC only: nothing here reads or mutates an archive, RUNLOG or the seal path;
      ...
      // projects for the DO-side ceremony faults, the errorId ring a customer's quoted
      // "err:ab12cd" is dereferenced against, the Worker<->DO contract faults, the recorders' own
      // dropped writes (a non-zero count means every other counter in the pack is a LOWER BOUND for that
      // window), the env-default destination's reachability heartbeat and the downpipes whose staleness
      // rule can never arm. Redaction-safe by construction: closed-enum keys, integer counts, clamped
      ...
      // CLIENT DIAGNOSTICS. The console keeps a
      ...
      case "POST /client-diag":
        ...
        return this.json({ ok: true });
      // On-demand fleet drill: start a bulk restore-test campaign over the whole fleet (or a
    ...
    async routeCoverage(req: Request, url: URL): Promise<Response | null> {
      ...
      // Config-as-a-source versioning (the DEFAULT self-contained layer). The hash-chained,
