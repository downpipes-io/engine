...
//   recordStorageFault  persist-state storage.put faults, ATTRIBUTED to the downpipe being persisted, so
...
//   recordSchedHealth   the three housekeeping faults that are fully silent otherwise: a housekeeping sweep that
...
//   recordStateRefusal  a persisted dp:<id> record REFUSED at read time by migrateOrRejectConfig:
...
//                       ever created), and the one fact that explains it otherwise lives only in a Workers Logs
//                       line the vendor cannot pull. The stamp is cleared once the record reads clean again.
//
...
export function SchedulerDiagMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    // from a generic storage failure, and ATTRIBUTING it to the downpipe whose record was being written.
    ...
    // recordSchedHealth PROMOTES a scheduler HOUSEKEEPING fault into the bounded counter record at
    ...
    // recordStateRefusal STAMPS a read-time state REFUSAL for one downpipe. The stamp goes to its
