// The IDENTITY-and-AUTH half of the SchedulerDO's RPC dispatch, a sub-mixin of the route() dispatcher
// kept in its own file so no routing file exceeds the module-size guardrail. This sub-mixin owns the
// per-subsystem sub-dispatch methods
// for the engine's own WebAuthn/passkey IdP (register/login, bootstrap mint, the signed session lifecycle and
// credential/session management), the step-up re-auth ceremony, the native external IdP (OIDC + SAML)
// connection custody and sign-in flows, and the recovery-code break-glass. Each method is a switch over the
// SAME `${method} ${pathname}` key that returns the handler Response for a key it owns or null ("not my
// route"); route() (in scheduler-do-routing.ts) chains these among the other sub-dispatches. Every case KEY,
// its body, the status code, the response and the dispatch result are byte-identical to the former single
// switch; the arms (including the failed-login authn-failure audit block) moved verbatim. This sub-mixin's
// `this` is SchedulerDOSurface (like every sibling mixin), so each sub-dispatch calls the owning handler
// (this.passkeyRegisterBegin / this.idpOidcCallback / this.recoveryRecover / ...) with the SAME dispatch and
// `this` binding the single class had. A leaf the assembly depends on, never the reverse (madge 0 cycles).
// Move-only: no auth, route, status code, response body or dispatch order changed.
  ...
  return class extends Base {
    ...
    async routePasskey(req: Request, url: URL): Promise<Response | null> {
      ...
      case "POST /passkey/login/finish": {
        ...
        // a reviewer reading the chain. No identity is verified on a failure, so the actor fields are null, and
        // sourceIp is structurally null here: the login-finish route strips it off the inbound body, so the DO
        // has no IP to thread and never will (scheduler-do-audit.ts records what that once did to the
        // source-IP-gap counter). The shared recorder carries the best-effort try/catch, the drop counter and
        // the per-minute coalescing, so this arm no longer holds its own copy of any of them.
        //
        // THE TARGET MOVED, AND THE OLD ONE WAS A LIVE MIS-RENDER. This row used to carry a FIELD-LESS
        // access-policy target, which the console renders as "Access policy change (intent)", so every failed
        // sign-in on a customer's chain read "Sign-in attempt failed / Access policy change (intent)". Rows
        // already written keep their old target and their hashes, so the chain is untouched and the two shapes
        // coexist permanently; new rows say what they are.
        if (!finRes.ok) await this.appendAuthnFailureAudit({ ceremony: "passkey-login", actorMethod: "passkey", sourceIp: null });
      ...
      case "POST /passkey/session/issue":
        return this.json(await this.passkeySessionIssue((await req.json()) as { email?: unknown; priorToken?: unknown; method?: unknown }));
      ...
      case "GET /passkey/session/list":
        // V7.5.2: the caller's own live sessions, the presented cookie flagged current; metadata only. The
        // presented cookie rides a router-internal header (a GET carries no body).
        return this.json(await this.listOwnSessions(decodeCaller(req.headers.get(CALLER_HEADER)), { token: req.headers.get("x-downpipe-session-token") }));
      case "POST /passkey/session/terminate-one":
        // V7.5.2: end ONE of the caller's own sessions by its id; a foreign or unknown id is not-found.
        return this.json(await this.terminateOneSession(decodeCaller(req.headers.get(CALLER_HEADER)), (await req.json()) as { sid?: unknown }));
    ...
    async routeStepUp(req: Request, url: URL): Promise<Response | null> {
      ...
      case "POST /stepup/check":
        return this.json(await this.stepUpCheck((await req.json()) as { token?: unknown; stepUpToken?: unknown; purpose?: unknown }));
    ...
    async routeIdp(req: Request, url: URL): Promise<Response | null> {
      ...
      case "POST /idp/conn/removal-preflight": {
        // Lockout guard (READ-only, not gated): the DO-side facts the router combines with CF Access + the
      ...
      case "POST /idp/conn/cert": {
        // GATED owner op (idp-conn-cert): a ZERO-DOWNTIME SAML signing-cert rollover. A signing cert is
      ...
      // SSO sign-in FAILURE aggregate (support diagnosis): the bounded per-code counter the OIDC/SAML callback
      ...
      // The per-connection-KIND breakdown of the same aggregate (which PROTOCOL is failing). Same INTERNAL read,
      ...
      // The per-CONNECTION breakdown of the same aggregate (which of your two SAML connections is
      ...
      // The bounded auth/RBAC defensive-branch signal aggregate. GET is the INTERNAL read the support pack
      ...
      case "POST /auth-signal": {
        ...
        // recordAuthSignal DROPS an out-of-vocabulary name (the redaction boundary) while the
      ...
      // Auth posture probes for the pack - the session signing-key presence + age (a young key + a verify
      ...
      // doPlaintextSecretsMissingConns names WHICH connections are secretless by their OPAQUE ORDINAL
      ...
      case "GET /auth-posture": {
        ...
        return this.json({
          ...
          // WHICH credential path the estate actually RAN on, and when. adminCredentialPaths above is a
      ...
      // The router posts ONE authenticated admin request's credential path here (throttled edge-side to at
      ...
      //    their OWN email. A no-op (promoted:false) when nothing is staged.
        default:
          return null;
      }
    }
