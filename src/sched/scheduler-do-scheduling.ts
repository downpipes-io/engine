// SchedulerCoreMixin is the per-downpipe SCHEDULING CORE, kept separate from the unrelated dual-control
// subsystem in scheduler-do-dual-control.ts so neither file exceeds the module-size guardrail. It owns the
// due-time index (dueIndexKeyFor + persistDownpipeState + the rebuildDueIndex/maybeReconcileDueIndex
// backstop; the O(due) optimisation), the run-state machine (due/leased/trigger/heartbeat/completeRun +
// runlogIndex allocation), the replication state, the run-history + point-in-time reads, and the DO alarm
// (alarm/rearmAlarm + nextWithJitter); the cron driver in index.ts is the seal driver. It is applied as a
// mixin over a SchedulerDOSurface base, with no circular dependency between the two.
  ...
  return class extends Base {
    ...
    async persistDownpipeState(ds: DownpipeState): Promise<void> {
      ...
      try {
        ...
        // Stamp the schema version at the storage boundary, mirroring the seal checkpoint's `v:1`: the
        ...
        // The fault is ATTRIBUTED to the downpipe whose record was being persisted: without it, a climbing
    ...
    async rebuildDueIndex(): Promise<void> {
      ...
      // enumeration is PAGED via listAllByPrefix: a bare un-paged list() returns at most
      ...
      try {
        ...
        /* the parity stamp is diagnostic only; never fail the index rebuild for it. But a FAILED stamp is
           itself a fault worth counting: the pack's dueIndex block then serves the PRIOR snapshot as if
    ...
    // DO.
    //
    // SCALE: instead of scanning EVERY downpipe (O(N) per tick), it lists only the
    ...
    async due(): Promise<{ due: DownpipeState[] }> {
      ...
      for (let guard = 0; guard < DO_LIST_MAX_PAGES; guard++) {
        ...
        for (const key of page.keys()) {
          ...
          // Read-time schema guard: this is THE load-bearing source-selection read (the seal loop
          ...
          try {
            ...
            // Without a stamp, a skipped downpipe produces no run row, no failed run, no marker -- just growing
            // staleness the pack could not explain. STAMP the
    ...
    async trigger(req: { id: string }): Promise<{ runId: string; index: number; prevRunId: string | null; config: DownpipeConfig } | { skipped: string }> {
      ...
      // Read-time schema guard: trigger returns this config to the seal (the manual run-now path,
      ...
      // The refusal is STAMPED before it is re-thrown: a run-now that 500s leaves no failed-trigger
      ...
      const now = Date.now();
      // This downpipe's destinationIds list is PRESENT but holds no usable entry, so
      ...
      // idempotent on (runId,index), so a reclaimed re-run is safe.
    ...
    //
    // It is IDEMPOTENT and guarded against a stale or duplicate completion. The join key
    ...
    async completeRun(req: CompleteRunReq): Promise<{ ok: true }> {
      ...
      // A completion that owns nothing is DISCARDED in silence. Usually that is the benign late/duplicate
      ...
      // run, must not flip inFlight=false out from under the live run. The in-flight LEASE
      // remains the backstop that reclaims a run whose completion never lands at all (e.g.
      // the narrow drive-catch index-0 path where the trigger committed but the seal never started).
      if (ds && ownsInFlight) {
        ds.inFlight = false;
        delete ds.inFlightSince; // clear the lease on completion
        ...
        // Advance the MONOTONE sealed-run counter on a SUCCESSFUL run, on
        ...
        // Record the latest verify-at-seal verdict for the posture seal-verification check
        ...
        // Stamp the blackout-resolution outcome (blackout-windows-silently-violated) on the same terms: set
      ...
      // The row keeps its allocated runId even on failure (req.runId is "" by convention).
      if (hist && row !== null && ownsInFlight) {
        ...
        // In-scope objects that vanished (were deleted) mid-crawl between list and value read. A
        ...
        // Records sealed as incompleteness sentinels (markers, not the real bytes). A non-zero
        ...
        // Per-marker breakdown: WHICH incompleteness kinds the run sealed (the support pack carries it
        ...
        // Per-kind attribution: WHICH surface/object was short (redaction-safe closed/product-token ids
        ...
        if (req.opCounts !== undefined) row.opCounts = req.opCounts; // exact per-resource op tally
        // Record the verify-at-seal verdict on the row so the run list / status can show
        ...
        // A FAILED run carries the destination id(s) PROVEN down this run (the failover down set,
        ...
        // classifyReplicationReason REDUCES the driver's coarse run-error class (itself a closed vocabulary
        // carrying the store's sanitised code) to ONE closed member, distinguishing an EXPIRED CREDENTIAL from
        // a WORM refusal from a timeout (three faults with three different owners and three different fixes)
        // rather than every down destination reading the same undifferentiated "unreachable". It DISCARDS the
        // string: no S3 error text, endpoint host or bucket name ever reaches the record.
        ...
        // The env-default-destination and never-arms-staleness-rule observations are both per-completion
    ...
    // runAt is the POINT-IN-TIME resolver: GET /runs/at?downpipe=&at=<rfc3339> -> the latest
    ...
    async runAt(downpipeId: string | null, atRaw: string | null): Promise<PointInTimeRun & { error?: string }> {
      ...
      // A SUCCESSFUL run whose startedAt will not parse is EXCLUDED from this resolution, so a run that
    ...
    // acquireTickLease is the cron driver's SINGLE-FLIGHT guard. drive() (the scheduled() body) takes it
    ...
    // It also returns the housekeeping-health counters + a parityStampStale flag, and the
    ...
    async schedulerSignals(): Promise<{
      ...
      // schedHealth: the housekeeping faults that are otherwise fully silent. parityStampStale is DERIVED:
      ...
      // stateRefusals: the downpipes whose persisted record this engine REFUSED to read (a rollback
    ...
    // the DO never seals inline. Precise alarm-driven dispatch (the alarm
    ...
    async alarm(): Promise<void> {
      ...
      try {
        ...
        // Prune expired step-up artefacts: single-use assertion challenges (the stepup: scope) and
        ...
        // Session ref and single-revocation rows past their exp.
        await this.sweepSessionRecords(nowMs);
      } catch {
        /* housekeeping is best-effort: a sweep error must never break the alarm re-arm. But a PERSISTENTLY
           failing sweep grows DO storage for months with zero evidence until the storage limits bite, so
      ...
      // The cron-independent staleness dead-man. This alarm is the ONE timer that survives a
    ...
    async rearmAlarm(): Promise<void> {
      ...
      if (!Number.isFinite(next)) {
        // A CORRUPT DUE-INDEX KEY STOPS THE ALARM CHAIN DEAD. Returning without setAlarm means this DO
      ...
      // Clamp the armed time so it is never in the past. A wedged enabled downpipe (a crash in
      ...
      // and does not yet dispatch the seal; precise alarm-driven dispatch is a deferred
    ...
    // nextWithJitter is the SINGLE place a downpipe's next nextRunAt is derived, used by both the
    ...
    //   - no cron -> the cadenceSeconds path, with the existing backwards jitter (thundering-herd spread).
    ...
    //
    // nextWithJitter also classifies the cron-resolution OUTCOME (SCHED cron-runtime-fallback-to-cadence /
    // invalid-tz-runtime / impossible-cron-runtime) so the caller can stamp it on the downpipe state for the
    // support pack: a resolved cron fires at its computed time; any fault or no cron falls back to the
    // jittered cadence; deferPastBlackouts then applies on either path. cronResolve is null for a
    // cadence-only downpipe (nothing to resolve), so the caller CLEARS any stale stamp when the cron is
    // removed.
    //
    // It also classifies the BLACKOUT-window resolution outcome: a deferral that exhausts the hop ceiling
    // and fires INSIDE a declared change-freeze window, and a window saved with equal start/end minutes that
    // therefore covers nothing and never applies, are both otherwise silent (blackout windows are
    // deliberately never carried in the pack). blackoutResolve is null for a downpipe with no windows, so
    // the caller CLEARS a stale stamp.
