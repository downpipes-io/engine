// The reporting + RTO + coverage cluster of
// the read-mostly observability subsystem (contract section 6 reporting and the coverage/gap detection),
// kept in its own file so neither it nor scheduler-do-observability.ts exceeds the module-size guardrail. This is
// the sibling sub-mixin of ObservabilityMixin (restore tests + posture stay there); both layer over a base
// whose `this` is SchedulerDOSurface, so a method here still calls this.listDownpipes / this.rtoInputs /
// this.requireCapabilityResolved and a method there still calls this.coverage with the SAME dispatch and
// `this` binding the single class had. Dispatch is byte-identical; no storage key, route string, status
// code, response body or auth gate changed. The mixin is a leaf the assembly depends on, never the reverse.
...
export function ReportingMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    async slaReportData(req: { period?: unknown }): Promise<SlaComplianceData> {
      ...
      // Derive the per-downpipe RTO estimate so the SLA report surfaces recovery TIME next to the
    ...
    // rtoData is the GET /rto handler (the RTO companion to RPO/freshness): the per-downpipe derived
    ...
    async setCoverageInventory(
      ...
      if (!result.ok) {
        // The refusal reached only the immediate HTTP caller. The customer believes they uploaded an
    ...
    async coverage(): Promise<CoverageReport> {
      ...
      // The downpipes the MATCHER silently drops. A source type the matcher does not know, or a source
