...
// a tracked, not-yet-enabled work item; until the
// RATELIMIT_DO binding is present, accountPacer() falls back to the per-isolate CfPacer and the round-trip is
// never on the path. Under concurrent isolates the per-isolate pacing alone may not hold the account-global
// Cloudflare API limit, which is why the shared-bucket go-live is tracked. See accountPacer in src/cf-pace.ts.
...
// ---- fail-open health ("Cloudflare is 429-ing our crawls" while the limiter has been failing open) ----
//
// This DO FAILS OPEN by design (a broken limiter must never stop backups), which without more risks COMPLETE
// SILENCE: the pack carries infra.rateLimitDoConfigured, a PRESENCE bit, so a limiter degraded to "no
// throttle" for weeks - the actual cause of a customer's 429 storm - is indistinguishable from a healthy
// one. Same for the knob: an invalid CF_API_RATE_PER_SEC silently falls back to the conservative default, so
// an operator who sets it to "10/s" (unparseable) and expects 10/s has no way to learn otherwise. Both facts
// are persisted as counts + a clamped timestamp + a boolean, read back by GET /health for the support pack.
// Counts only; the storage error text never rides.
  ...
  async fetch(req: Request): Promise<Response> {
    ...
    if (req.method === "POST" && url.pathname === "/take") {
      ...
      try {
        ...
        // The fail-open is the whole point, but it must not be SILENT. Record it (count + time only) so
        // the pack can say "this limiter has been degraded to no-throttle 4,102 times since Tuesday", which is
        // the answer to the "Cloudflare is 429-ing our crawls" ticket. Best-effort: recording a fault must never
        // turn the fail-open into a failure, so a throwing recorder is swallowed too.
