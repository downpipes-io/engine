// scheduler-do-keys.ts -- the scheduler DO's STORAGE-KEY and persisted-state-shape vocabulary: a pure
// leaf beside scheduler-do-limits.ts and scheduler-do-records.ts. It is re-exported from
// scheduler-do-base.ts so every mixin and router keeps one import path; the leaf itself imports
// nothing. It holds storage-key prefixes, bounded size/count caps and the small closed persisted-state
// shapes they key -- no behaviour, no secrets, no custody.

// The SchedulerDO auth subsystem mixins (passkey/oidc/saml/session/step-up/owner-action) import these
// module consts directly. They are storage-key prefixes and bounded size/count limits with no
// behaviour.
...
// PASSKEY_WITNESS_SINCE_KEY dates the usability witness itself: the RFC-3339 instant at which this DO first ran
...
// is kept separate from the session key because the two have opposite lifetimes: the session key is
// DELETED to sign everyone out, and if the same key hashed recovery codes, deleting it would also make
// every banked recovery code unverifiable, so the break-glass an operator reaches for after a suspected
// compromise would destroy the break-glass they would need if the compromise were real. A recovery code
// must survive a global sign-out; a session token must not.
//
...
// TEST_FAULT_KEY holds the ONE armed harness test-fault (the owner-gated engine test-fault-hook, see
// scheduler-do-test-fault.ts). It is written and read ONLY through the Worker's HARNESS_TEST_FAULTS-gated,
// Owner-gated /admin/test-fault/* paths; a production engine never sets that flag, so this key is never
// touched there. Single-shot: consumeTestFault deletes it the instant it fires. Redaction-safe (a closed
// kind enum, the operator's own binding label, a timestamp).
export const TEST_FAULT_KEY = "test-fault:armed";
// BeaconAttempt is ONE beacon POST outcome. Keeping a cause (not just a last-attempt flag) matters because
// "the vendor deploy ledger shows our account dark" could not otherwise distinguish a malformed BEACON_URL
// (a permanent, one-line operator fix) from a network blip (self-healing), or tell a hard-down beacon from
// an INTERMITTENT one. errorClass is the closed BeaconFailClass the cron already computes (cron-fault-ledger.ts:
// url-invalid | network | non-2xx | do-read | unknown); status is the clamped HTTP status. Never BEACON_URL,
// never the ingest key, never the throw message.
...
// invocation subrequest budget ran low before every due downpipe was dispatched. The yield happens BEFORE a run is allocated (the undispatched downpipes stay DUE and carry to
...
// CONFIG_HISTORY_HEAD_KEY is the config-history chain's HEAD ANCHOR: the id and contentHash of the
