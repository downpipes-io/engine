// Part 2 of the SchedulerDO's shared TYPE + CONSTANT vocabulary, split out of scheduler-do-base.ts so that
// foundation file holds only the irreducible
...
export function validateDestPricing(v: unknown): DestinationPricing | null {
  ...
  // The stored currency label must be trimmed. Trimming and THEN slicing to 8 is not enough on its own: a
  // value whose ninth character onwards was the only non-space tail would come back with the whitespace the
  // trim had just taken off the ends, e.g. "AUD     Y" stored as "AUD     ". The function's own postcondition
  // is what matters here (that the value it returns is actually trimmed), not the order of the two operations.
  // The truncation stays: a currency label's tail costs detail rather than identity, it names nothing
  // elsewhere, and the console defaults it to USD. The second trim is what makes the postcondition hold, and
  // it can never empty the value, because the guard above has already established a non-space first character.
...
export interface DestinationConfig {
  ...
  // A bare `catch` would call every one of those "denied", which is a permission fact only a 401/403
  // establishes.
...
export interface StoredDestination extends DestinationConfig {
  ...
  // FIRST console-managed destination is added alongside it. It
...
export interface DestinationCollection {
  ...
  // removal guard.
  //
  // WHY IT IS NEEDED. An UNPINNED downpipe (what the console wizard creates) seals to whatever the
  // default is and records NO origin id on its history row -- selectSealDestination returns
  // `destinationId: primaryDestinationId(config)`, which is undefined when nothing is pinned.
  // uncoveredOriginRuns therefore has to ATTRIBUTE those rows to a destination, using the CURRENT
  // default. That is the right answer only while the default has never moved: repointing the default
  // from A to B and then removing A would make every default-routed run invisible to the guard, so the
  // destination holding their only copy would be removed with no refusal, no force flag and
  // `uncoveredOriginRunCount: 0` in the audit.
  //
...
export interface DestStatusView {
  ...
  //
  // "entra" matters because the console renders an Authentication row ONLY for "sts"
  // (screens/destination-cards.ts): without it an Entra destination answers "keys" and the card shows
  // NOTHING. A customer could not tell from the console that the destination authenticates as a service
  // principal against Microsoft's identity platform rather than with a storage account key: the product
  // does something and the interface does not say so.
  //
  ...
  pricing?: DestinationPricing;
  // lastPrune is the per-destination retention-prune sidecar: what the most recent prune pass (cron
...
// ---- SIEM audit-log push destination ----------------------------------------------------------------
...
// PUSH_FORMATS is the ONE AUTHORITY for the closed envelope-shape selector. raw-json is the pull feed's
// own body shape, POSTed instead of GET-returned;
...
//
// WHY THE ARRAY IS THE AUTHORITY AND THE TYPE IS DERIVED FROM IT, not the other way round. A separate
// hand-typed allow-list at each boundary (the router in admin/router-push.ts, the DO in
// sched/scheduler-do-siem-push.ts, the support pack in admin/support-sections-push.ts) can drift from the
// real eight formats: a pack allow-list of `new Set(["cef", "leef", "json", "ecs"])`, for instance, drops six
// of the eight real formats including splunk-hec, and two of its four members ("json", "ecs") are not engine
// formats at all and can never match. The consequence is silent on the support bundle: a Splunk
// destination's `format` is simply absent from the pack, indistinguishable from an older engine that never
// reported one, so a Splunk push bundle carries nothing anywhere in it naming Splunk. Deriving the type from
// the array makes a new member reach every boundary that imports the array with one edit, and makes a
// boundary that re-types the list a compile error rather than a silent subset.
...
//
// THE DEFECT THIS CLOSES. An RFC 5424 record's MSG is a CEF or a LEEF line; there is no HEC-over-syslog, no
// NDJSON-over-syslog and no GELF-over-syslog on the enterprise auto-parse path. Nothing else says so: without
// this check, a destination configured with sink "syslog-tls" and format "splunk-hec" would pass the
// router's format check, pass the DO's, and then SILENTLY SHIP CEF, because the sender's record builder
// reads `format === "leef" ? shapeLeef(...) : shapeCef(...)` and everything that is not LEEF falls into the
// CEF arm. A combination that validates and then sends a different wire than the operator chose is worse
// than one that is refused: the operator's SIEM sees plausible bytes under the wrong parser, and no counter,
// no trail row and no audit event records the substitution. Both boundaries refuse the combination, and the
// sender's own record builder takes a narrowed two-member type so the fallback is a compile error rather
...
//
// THE RULE HAS TWO DIRECTIONS, and both must hold: which formats may travel over syslog-tls, and which
// sinks CEF and LEEF may travel to. Because both boundaries call THIS function, the two cannot go out of
// step: the console refuses cef/leef over http and s3 and the docs state the rule, but a validation that
// lives only in the screen is not a validation, since the engine is the only server-side enforcement point on
// this surface. An API client or a replayed approval must not be able to walk straight past it. Both
// directions are enforced here, in the same call, from the same closed set.
...
export interface PushDestinationRecord {
  ...
  // carries it into the support pack, so the one thing the config could not otherwise say is on the record.
  //
...
export interface PushDeliveryAttempt {
  ...
  fromSeq?: number;
  toSeq?: number;
  // causeDigest is the 12-hex one-way SHA-384 prefix of the RAW fault the coarse `reason` was derived
...
// ---- OTLP/HTTP metrics push destination --------------------------------
...
export interface OtlpPushDeliveryAttempt {
  ...
  // snapshot length, so the trail can never over-report what was sent.
  ...
  // an over-cap fleet is thus a LOUD trail signal, never a silent partial.
  truncated?: boolean;
  // droppedCount is HOW MANY downpipes the cap dropped: truncated:true alone cannot answer "how many of our
  // downpipes are missing from the dashboards?" once the fleet outgrows the 5000-point cap, since a boolean
  // cannot size a loss. A non-negative int; absent when nothing was dropped.
  droppedCount?: number;
  // causeDigest is the 12-hex one-way SHA-384 prefix of the RAW fault the coarse `reason` was derived
  // from -- byte-identical to the `[cause <hex>]` the engine's own redacted log line carries for the same
  // fault, and the SAME construction the failed run rows already use. It is the JOIN KEY: the customer can
  ...
  // success. Absent when nothing was rejected.
...
// OtlpDownpipeMetrics is ONE downpipe's canonical backup-health facts, the snapshot input BOTH the OTLP push
// drain (cron/otlp-push-pass.ts) and the Prometheus /metrics endpoint read from
// scheduler-do-otlp-push.ts's otlpMetricsSnapshot(). Every field
...
export interface OtlpDownpipeMetrics {
  ...
  // from a live green job before firing a staleness alert.
  enabled: boolean;
  ...
  // time() - last_success > threshold idiom.
  backupSuccess?: 0 | 1;
...
// warns a per-IP limiter harms shared-NAT customers) because there is no authenticated identity yet, and a
// shared egress is at worst throttled on sign-in attempts for one window, never locked out of an established
// session. The router's authRateLimited FAILS CLOSED on a limiter outage (router-core.ts); the documented
// model is docs/security/identity-sessions-and-files.md section 2.6.
...
// one per email. Without it, an enrolment that runs while a live record ALREADY exists (self-add, including
// the forced re-enrolment after a recovery-code sign-in) would mint the fresh set straight over the live key,
// so the OLD codes would go dead the instant the DO stored the new ones, before the operator has even seen
// the console's save-confirm panel. A closed tab, a crashed browser or simply not noticing the panel would
// then mean every remaining break-glass code is gone with nothing saved to replace it. The staged key lets
// the DO show the fresh plaintext (generateRecoveryStaged) without
// touching the live record; only confirmRecoveryStaged (POST /admin/auth/recovery-codes/confirm, fired by
// the console's save-confirm "Continue") promotes it to live, which is the ACTUAL invalidation boundary. A
// staged set the operator never confirms is simply abandoned: the live (old) set keeps working, which is
// the safe failure. A bootstrap or invite enrolment has no prior record to protect, so it keeps minting
// straight to the live key via generateRecoveryFor exactly as before; this key exists only for the case
// where there is something to lose.
export const RECOVERY_STAGED_PREFIX = "recovery-staged:";

// There is deliberately NO recovery-ack latch key here. An account-level boolean set the moment an Owner's
// codes are generated and never re-evaluated would let the lockout pre-flight and the break-glass-token
// dispose gate read it as proof that a way back in exists; because it would record a past event rather than
// a present fact, it would stay true through every way that fact can stop being true. The verdict is instead
// derived live by SchedulerDO.recoveryBreakGlassVerdict from the role table, the record and the signing key,
// so no key holds this answer.

// RECOVERY_RATE_PREFIX / the recovery rate caps: the recover endpoint is a brute-force target on a
// high-value credential, so it is limited HARDER than ordinary sign-in AND FAILS CLOSED (like the per-IP
// auth limiter, unlike the fail-open per-caller limiter): if the limiter's own store hiccups, deny rather than admit, because an
...
//
// A refusal on this path does NOT report retryAfterMs so the client can back off. Over 12 consecutive
// attempts in one window: attempt 6 is the first denial, and recoveryRateCheck answers exactly
// `{ allowed: false }`. There is no retryAfterMs anywhere on this path: recoveryRateAllow narrows to a bare
// boolean, recoveryRecover returns
// { ok:false, alert }, and handleRecovery answers a bare 401 with no Retry-After header. The DIFFERENTIAL is
// the ordinary limiter one file over, which answers { allowed:false, retryAfterMs: 60000 } for the same class
// of refusal, so the omission is specific to this path rather than a house style.
//
// WHETHER THAT IS A FAULT IS AN OPEN QUESTION AND IS DELIBERATELY LEFT OPEN HERE. Recovery is anti-enumeration
// by design: a wrong code, an unknown email, an exhausted set and a throttle all return ONE generic 401 so no
// oracle exists, and a Retry-After on the per-EMAIL bucket would tell an unauthenticated caller that somebody
// is currently attempting recovery on that address. So the silence may be correct. What is NOT correct is a
// comment asserting a back-off signal the caller never receives, because the next reader plans around it.
//
// WHAT IS NOT A DESIGN QUESTION: the limiter is NOT self-deepening. A denied attempt does not increment the
// counter and does not push windowStart, so a caller who keeps retrying cannot extend their own lockout, and
// the wait is bounded at RECOVERY_RATE_WINDOW_MS. That matters because the console's copy on a failed
// recovery ends "then try again": walking a partly-spent sheet on `warm` with five spent codes fills the
// bucket, and codes 6 to 10 are refused UNEXAMINED, reading as dead while they are live.
...
// restore-test campaign: the in-progress campaign and the most-recently finished one. Singletons
...
// downpipes) is enumerated by looping with a startAfter cursor instead of silently
...
// ---- Persisted due-time index: due() in O(due), not O(N) ----------------------------
// Scanning EVERY downpipe (listDownpipes -> listAllByPrefix("dp:")) on every cron tick to find the few
// whose nextRunAt has fallen due is O(N) per tick and caps the single account-wide SchedulerDO at a few
// thousand downpipes. A SECONDARY index keyed by due time avoids this, so the dispatcher can read only the
// due candidates:
//
...
// silent -- an alarm housekeeping sweep that keeps THROWING (DO storage grows unbounded for months until
...
// STATE_REFUSED_PREFIX namespaces the per-downpipe read-time REFUSAL stamp: staterefused:<id> holds
...
// ---- The cron-independent staleness dead-man (the dead-man's-dead-man) -----------------
...
//
// The dead-man thresholds below are multiples of the shipped Worker cron-trigger cadence, so an operator
// who retunes the cron updates the cadence in one place: it comes from cron/cron-cadence.ts, which
// test/validate-cron-cadence-pin.ts holds to the wrangler literal, rather than a local copy of the number
// written down separately here, in cron/drive.ts, in scheduler-do-support-diag.ts and in the wrangler files,
// with nothing comparing them. It is the EXPECTED interval between cron-driven alert sweeps.
...
// AUDIT_HEAD_KEY persists the audit chain HEAD pointer: the highest seq, its hash, and the
