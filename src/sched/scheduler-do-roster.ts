...
// application. `this` is SchedulerDOSurface (like every sibling mixin), so they call
// this.listAllByPrefix / this.persistDownpipeState / this.rebuildDueIndex directly, and the routing
// dispatch (scheduler-do-routing.ts) reaches them through the composed class.
...
export function RosterMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    async rosterHygiene(): Promise<RosterHygieneReport> {
      ...
      // The ghost/never-ran lists are capped at ROSTER_LIST_CAP. The report SAYS so (ghostsTruncated /
    ...
    async reconcileRoster(): Promise<{ removed: number; rehomed: number; ghostsRemaining: number }> {
      ...
      for (const action of plan) {
        if (action.act === "rehome") {
          ...
          // A delete-ghost over a row that DOES carry an embedded config id is not a malformed-row
