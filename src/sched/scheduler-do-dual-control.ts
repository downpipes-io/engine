// This mixin implements the opt-in dual control for HIGH-BLAST-RADIUS OWNER OPERATIONS (the
// requireConfigApproval toggle, default OFF). Gate OFF (default): gatedOwnerAction runs the action inline.
// Gate ON: it records a PENDING OWNER-ACTION APPROVAL bound to an actionHash over the decision params and the
// proposer identity, and a second owner must approve (maker != checker on the stable subject axis) before the
// same method runs with the proposer's authority re-resolved live (owner-action.ts is the pure logic; this DO
// is the single storage authority). It shares the org-policy gate state (OrgPolicyMixin) and the scheduling
// core (SchedulerCoreMixin) with the rest of the DO through `this` over a SchedulerDOSurface base.
...
export function DualControlMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    async gatedOwnerAction<T>(
      ...
      // no-op when the policy is OFF. dual-control-disable (the OFF switch) is
      ...
      if (!gateOn) {
        // Gate OFF: run inline, with no dual-control wrapping.
    ...
    async recordOwnerAction(
      ...
      const record: PendingOwnerAction = {
        ...
        proposedBySubject,
        proposedByGroups,
        // The edge address the PROPOSE arrived from. The approve-time execute replays this action as the
        // proposer and audits it as an attributed human; without an IP on that caller the row would carry a
        // null sourceIp and the engine would count its own governed replay as a capture fault.
    ...
    async verifyOwnerActionIntegrity(record: PendingOwnerAction): Promise<void> {
      ...
      // crying-wolf shape a fault counter must never take, and it would be worse here, because the signal it
      // would drown is a real tamper.
      //
      ...
      if (recomputed !== record.actionHash) {
        // A TAMPER on the stored owner-action record is security-significant, so it is recorded here as its
        // own closed class, never conflated with a guard refusal.
        await recordGovernanceFault(this.state.storage, "owner-action-execute", "integrity-failed");
        await this.annotateOwnerActionAttempt(record.id, "integrity-failed");
        throw new Error("the action record failed its integrity check and cannot be carried out");
      }
    ...
    // replayCaller. sourceIp is the PROPOSE-TIME address off the record: the execute audits itself as the
    // proposer, an attributed human, so using the address the engine captured at propose time keeps every
    // approved owner action correctly attributed instead of registering as a source-IP capture failure.
    //
    // replay: true suppresses resolveBoundEntry's bind-on-first-auth for this replayed call. Authorisation
    // keys on the subject, so a recycled email at a genuinely new subject inherits nothing on its own -- but
    // bind-on-first-auth matches a pending grant BY EMAIL and writes it onto whatever subject is presented.
    // Without the flag, an invite issued at a departed proposer's re-used address would bind onto the
    // proposer's subject on this replay and the pending row would be deleted, so the proposer would inherit
    // the recycled email's grant. A pending OWNER invite is the one grant an owner-action execute needs, so
    // this is the highest-privilege shape that defect can take. The flag suppresses the bind only; the
    // subject-keyed re-resolution and the owner re-check are unchanged.
    ...
    // readOwnerActionRecord is the ONE read every owner-action path goes through, the twin of
    // readApprovalRecord's: a queued owner action whose expiresAt does not PARSE never reads as expired
    // (effectiveOwnerActionStatus guards on Number.isFinite), so an ARMED second-owner approval for a
    // high-blast-radius operation would otherwise outlive its TTL, silently. Throttled + best-effort; counts
    // only.
    ...
    async approveOwnerAction(
      ...
      if (!verdict.ok) {
        // The closed reason matters here: an owner action stuck awaiting a second approver because the FIRST
        // owner keeps trying to approve their own proposal (self-approval) is a different situation from one
        // whose TTL lapsed (expired), even though both would otherwise just look like "the approve button does
        // nothing".
      ...
      if (record.routerExecuted === true) {
        ...
        // This echo is a SECOND read path into the same at-rest params the listing redacts:
      ...
      //
      // A foreseeable auth refusal must not reset the DO: executeOwnerActionDO re-resolves the PROPOSER's LIVE
      ...
      // pending, single-use preserved, and no approve/execute audit is committed); the AuthError is then
      // re-thrown OUTSIDE the guard, where the fetch() catch maps it to 403.
      // This brings the DO-executed path to the shape the ROUTER-executed path already has (consumeOwnerAction
      // re-resolves owner OUTSIDE its critical section). The SAME treatment also covers the two OTHER foreseeable
      ...
      const outcome = await this.blockConcurrencyWhile(async (): Promise<{ ok: true; record: PendingOwnerAction } | { ok: false; error: Error }> => {
        ...
        // would reset the DO for the race's loser exactly as the comment above describes.
        ...
        if (!innerVerdict.ok) {
          ...
          // TooSoonRefusal survives the sentinel (it IS an Error) and is re-thrown outside the
          // guard below, where it maps to 429 exactly as the outer check's throw does.
        ...
        // of them with NOTHING written (no audit, no status flip) and re-throw OUTSIDE the guard, where the
        // fetch() catch maps an AuthError to
        ...
        await this.appendAudit({
          ...
          actorMethod: "access",
          // sourceIp is the PROPOSE-TIME address off the record, the SAME address proposerReplayCaller
          ...
          // address (the APPROVER'S) would mislabel where the proposer acted from -- the record HAS the
          // proposer's address: propose stores it as proposedBySourceIp. A null address here would leave an
          // ATTRIBUTED HUMAN row with no address on EVERY successful approve, and appendAudit counts exactly
          // that shape as a source-IP CAPTURE FAULT (scheduler-do-audit.ts). The two-owner estate is the default
          // gate (effectiveOwnerActionGateOn auto-gates every high-blast kind once a second owner exists), so
          // stamping the proposer's address here keeps a perfectly healthy engine from registering a false
          // capture fault on every destination edit, IdP change, discovery-token set and break-glass retire --
          // the commonest owner op there is.
      ...
      // in the fetch() catch) and a plain Error maps to 400, without resetting the DO to get there.
      if (!outcome.ok) {
        // Every failed owner-action approve/execute attempt passes through here, so the record does not just
        // SIT there "approved" with no record of the attempts or their causes. Closed class only:
      ...
      // Same reasoning as the router-executed ARM branch above: redact the echoed params
    ...
    async consumeOwnerAction(
      ...
      const outcome = await this.blockConcurrencyWhile(async (): Promise<{ ok: true; id: string; actionHash: string } | { ok: false; error: Error }> => {
        ...
        const now = Date.now();
        // The owner-action twin of the restore approval's unenforceable TTL, at the CONSUME -- the call
      ...
      // without resetting the DO to get there.
      if (!outcome.ok) {
        // THE REPLAY DETECTOR: consumeOwnerAction is the atomic single-use gate on a router-executed
        // privileged op, and its "the approval was already used" refusal is a DETECTED REPLAY of a one-shot
        // approval, which is security-significant. classifyGovernanceOutcome maps
    ...
    // annotateOwnerActionAttempt stamps the record with its LAST FAILED attempt. Best-effort and
    ...
    //   - gate OFF -> { gate: "off" }: the router runs the op inline.
    ...
    async rejectOwnerAction(
      ...
      // A vetoed action can never run, so its params are spent the moment the reject commits. This matters
      // most for a WITHDRAWN proposal: an owner who realises they pasted the wrong credential into a
      // destination edit withdraws it, and this scrub is what stops the wrong credential sitting in storage
      // for ever.
      ...
      // Same reasoning as elsewhere: a reject needs no maker != checker check (any Owner may veto), so this echo is the
    ...
    // support pack. listPendingOwnerActions is caller-scoped (an approver sees only what they may
