// ===========================================================================================
// WHY THIS FILE STAYS OVER THE 500-LINE GUARDRAIL.
...
//       dispatch a single class would give). That interface is ~1500 lines on its own and is a single source
//       of truth generated from the live method signatures (tsc declaration emit); splitting it across files
//       would fragment the one surface every mixin depends on, for no real gain, so it is deliberately kept
//       whole. Every cleanly separable cluster lives elsewhere: the run-state machine, due-time index and
//       alarm in scheduler-do-scheduling.ts, and the type and constant vocabulary in the two pure leaves
//       scheduler-do-records.ts and scheduler-do-limits.ts (re-exported below). Apart from AuthError, what
//       is left here is the constructor and the surface interface, which cannot be reduced without
//       fragmenting it. The file is therefore intentionally over 500 lines.
// ===========================================================================================
//
// The scheduler Durable Object: the single authority plane for an account's downpipes.
// It owns the schedules, the per-downpipe run lock, and the monotonic
...
//   not), so a failed run never causes a double-allocation.

// Pure this-free helpers live in sibling files and are imported back here so every call site in this
// class is unchanged. RESERVED_BINDINGS, primary/replica/allDestinationIds, scheduleTimeZone and
// deferPastBlackouts are re-exported below so external importers keep working unchanged.


import type { RestoreApproval } from "../admin/approvals.ts";
import type { AuditActorMethod, AuditDraft, AuditEvent, StatusObservation } from "../admin/audit.ts";
...
// scheduler-do-limits.ts) so this foundation file holds only the irreducible DO surface (the
// SchedulerDOSurface API + the constructor). Re-exported verbatim here so every module that imports
// these by name from scheduler-do-base.ts is unchanged, and imported back below for the
// SchedulerDOSurface interface's own method signatures.
export * from "./scheduler-do-records.ts";
// The storage-key prefixes, bounded caps and small persisted-state shapes live in scheduler-do-keys.ts.
...
//     unchanged.
//   - a PRESENT but non-number schemaVersion (a corrupt or hand-edited stamp, such as the string "2") is
//     REFUSED: it throws, so a malformed stamp cannot silently defeat the guard and let this code read
//     new-shape state.
//   - an ABSENT schemaVersion is a LEGACY record written before this stamp shipped: treated as v1 and
//     ACCEPTED unchanged (forward-compatible, so every existing record stays readable).
...
//   - any OTHER numeric value (BELOW current, once this code is itself a later version) is ACCEPTED, so the
//     only refusal paths are the two above. A future v2->v1 migration would be added here, accepting and
//     upgrading the older shape, exactly as migrateCanaryState upgrades a pre-multi-destination canary record.
//
// The refusal throws a TYPED StateRefusedError carrying the closed class and the stored version, so the
// caller (due()/trigger()) can STAMP the refusal as pack-visible evidence instead of coarsening it to a
// Workers Logs line.
...
// AuthError is the typed authorisation refusal. A capability/role/owner gate that
...
// ===========================================================================================
// SchedulerDOSurface is the FULL structural instance API of the SchedulerDO. Cohesive method groups live
// in mixin factories in sibling files rather than one single class; each mixin's `this` is typed as this
// surface, so a method in one mixin can call a method in another (or in the base/final class) with the
// SAME dispatch and the SAME `this` binding a single class would give. This gives only the cross-group
// TYPE visibility a single class would give implicitly; it changes no runtime behaviour. The body is
// generated from the live method signatures (tsc declaration emit) and kept in lockstep by the compiler.
// It declares the one instance field (state) plus the instance method set (class statics stay on their
// mixin, not here).
// ===========================================================================================
export interface SchedulerDOSurface {
  ...
  // The route() RPC dispatch is split across four cohesive sub-mixins (RoutingMixin holds route() + the
  // core data plane; RoutingSignalsMixin / RoutingConfigMixin / RoutingIdentityMixin hold the rest).
  // route() chains every per-subsystem sub-dispatch through `this`, so each must be on the surface for the
  // cross-mixin call to resolve (the same cross-group TYPE visibility a single class would give implicitly).
  // Each returns the handler Response for a key it owns, or null to pass.
  ...
  // The CRON + ADMIN diagnostic recorders / pack reads (SupportDiagMixin).
  ...
  // the support pack so a lost or failing beacon is visible without the beacon path
  ...
  // The cumulative cron seal-loop budget-yield record: recorded by the cron
  ...
  // The cumulative change-controlled-action refusal tally: bumped by
  ...
  // The cumulative config auto-snapshot failure tally: a count + last time,
  ...
  // The lightweight config-history integrity verdict: the chain verify
  ...
  // The console licence-activation refusal tally: recorded by the router on a
  ...
  // The engine deploy-identity marker recorded at each status-snapshot baseline: the version + Cloudflare
  // deploy id + time observed when the baseline was (re-)established, read by the support pack so a
  // snapshot RESET is attributable. Null until the first observation. Written by observeStatus, never a value.
  ...
  // verdict computed from the rows that parsed can say so.
  ...
  // The DO's own bounding of a groups list. `drops`/`boundary` are the OPTIONAL claim-drop tally: the
  ...
  // the disaster-recovery export so the two cannot disagree about a corrupt record.
  ...
  // The ONE write path for every `authn-failure` row, shared by the passkey-login arm in the routing
  // mixin, the SAML ACS and OIDC/OAuth2 callback wrappers in the IdP mixins, and the step-up mixin. It is on
  // the surface precisely because it is cross-mixin: a per-mixin copy is how the throttle would end up present
  // on one authentication path and absent on the next. See appendAuthnFailureAudit in scheduler-do-audit.ts.
  appendAuthnFailureAudit(input: {
  ...
  verifyAudit(): Promise<{
    ...
    brokenAt?: number;
    // The head-anchor verdict. `intact` speaks only for the entries still retained; this says whether the
  ...
  reconcileAlerts(): Promise<{
    ...
    // recoveries: one NotifyEmission per downpipe whose backup-failure/backup-stale
    // cooldown just cleared (recovered:true, same event + downpipeId as the trigger it closes), PLUS a
    // re-emission for any downpipe with an owed resolve whose prior delivery failed.
    recoveries: NotifyEmission[];
    // pendingRecoveryIds: the downpipe ids whose recovery was re-emitted from an owed-resolve
  ...
  reconcileReplicationAlerts(): Promise<{
    ...
    pendingTransitionIds: string[];
    // recoveries: one NotifyEmission per downpipe whose replication-degraded/
  ...
  // The per-downpipe alert-cooldown state (SreAlertingMixin), so "why didn't I get re-alerted" is
  // answerable -- a still-broken pipe is suppressed until at + cooldownMs.
  ...
  // The notify-pipeline drop counters (whole passes skipped / per-channel records
  ...
  reconcileExpiry(forwarded?: {
    // Which env bindings the router sees present on the Worker (the DO holds no env of its own). Absent or
    // false means "not in service": its tracked rotation row (if any) is dropped rather than left ticking.
    presentBindings?: Record<string, boolean>;
    // The current fingerprint for each PUBLIC-key binding the router can derive (SIGNER_PRIVATE's own
    // fingerprint; BREAK_GLASS_PUBLIC/OPERATIONAL_PUBLIC's recipient fingerprint), null when absent or
    // unparseable. Never a value, hash, prefix or length of a bearer/destination credential.
    fingerprints?: Record<string, string | null>;
    adminTokenDisabled?: boolean;
  }): Promise<{
    emissions: ExpiryEmission[];
  }>;
  // confirmSecretRotation serves POST /admin/secrets/rotated (owner-exclusive): an operator attests they
  // rotated a bearer/destination credential the engine cannot fingerprint. Refuses a fingerprint-observed
  // (key) secret id, since attesting one of those would let an operator claim a rotation the engine can
  // independently observe and disagree with.
  confirmSecretRotation(body: {
    secret?: unknown;
  }, caller: {
    method: AuthMethod;
    email: string | null;
  ...
  expiryWarningCount(): Promise<{
    ...
    // be absorbed into the zero that means "nothing to warn about".
  ...
  // Attended verification: the NARROW per-run compliance stamp + the session state
  ...
  configHistoryList(): Promise<{
    ...
    verify: {
      ...
      earliestId: number;
      brokenAt?: number;
      // The head-anchor verdict, so the console's chain banner cannot read "verified" over a deleted tail.
      headTruncated?: boolean;
      headTruncatedAt?: number;
    };
  }>;
  ...
  getRequireAllUpdateApproval(): Promise<boolean>;
  ...
  // The operator-configured outbound egress allowlist (OrgPolicyMixin). On the surface so
  // addNotifyChannel (NotifyMixin) can read the current list when validating a channel's url.
  getEgressAllowlist(): Promise<string[]>;
  setEgressAllowlist(req: { egressAllowlist?: unknown }, caller: { method: AuthMethod; email: string | null; subject: string | null; groups: string[]; sourceIp?: string | null } | null): Promise<{ egressAllowlist: string[] }>;
  ...
  // Control-plane recovery (ControlPlaneMixin): the no-custody signed-export slice, the export
  ...
  // CROSS-ENVIRONMENT estate import (recovery keystone, ControlPlaneMixin): rebuild only the DEFINITION
  ...
  importControlPlaneDefinition(exp: ControlPlaneExport, crossAccount: boolean, caller: {
    ...
    // The imported downpipes are disabled because NO ACCOUNT ID COULD BE COMPARED, not because the
  ...
  // Auto-heal (ControlPlaneMixin): the auto-reconcile-on-detect pieces. getControlPlaneRecoveryRecord
  ...
  // The recovery latch's own self-diagnosis (the amnesia-probe class, ControlPlaneMixin),
  ...
  // The last ESTATE IMPORT's outcome (ControlPlaneRecordsMixin). The import answers crossAccount +
  // accountIdAbsent on its one-shot HTTP response and persists neither, so a genuine cross-account rebuild and
  // a pre-account-discovery export (which disables every downpipe ANYWAY, via the fail-safe null compare) leave
  ...
  // The ACKNOWLEDGE-ONLY latch clear. Both
  ...
  // The lightweight support-pack view of the canary's liveness (CanaryMixin) and its bounded
  // transition ring, so a standing "paged once" death stays observable.
  ...
  // TestFaultMixin (HARNESS-ONLY; scheduler-do-test-fault.ts): the ONE armed update-lifecycle fault. Reached
  // only through the Worker's HARNESS_TEST_FAULTS-gated, Owner-gated /admin/test-fault/* paths, never in
  // production. routeTestFault is on the route() chain; the rest are its handlers.
  armTestFault(req: { kind?: unknown; binding?: unknown }): Promise<{ armed: boolean }>;
  readTestFault(): Promise<{ armed: { kind: string; binding?: string } | null }>;
  disarmTestFault(): Promise<{ disarmed: boolean }>;
  consumeTestFault(req: { kinds?: unknown }): Promise<{ fault: { kind: string; binding?: string } | null }>;
  routeTestFault(req: Request, url: URL): Promise<Response | null>;
  ...
  // The per-destination retention-prune sidecar (DestConfigMixin): the fold recordRetentionPass
  ...
  getOrgPolicyView(): Promise<{
    ...
    requireAllUpdateApproval: boolean;
  }>;
  ...
  setRequireAllUpdateApproval(req: {
    requireAllUpdateApproval?: unknown;
  }, caller: {
    method: AuthMethod;
    email: string | null;
    subject: string | null;
  ...
  revokePresentedSession(token: unknown, now: number): Promise<void>;
  // Per-session records: every mint writes a ref row, the operator can list and end one session.
  mintSessionRecorded(key: Uint8Array, input: import("../admin/session.ts").SessionMintInput, now: number, client?: { sourceIp?: string | null; userAgent?: string | null }): Promise<string>;
  listOwnSessions(caller: { email: string | null } | null, body: { token?: unknown }): Promise<{ ok: true; sessions: Array<import("./scheduler-do-records.ts").SessionRef & { current: boolean }> } | { ok: false }>;
  terminateOneSession(caller: { method: AuthMethod; email: string | null; subject: string | null; sourceIp?: string | null } | null, body: { sid?: unknown }): Promise<{ ok: true } | { ok: false; reason: "not-found" | "no-identity" }>;
  sweepSessionRecords(nowMs: number): Promise<void>;
  ...
  passkeySessionIssue(body: {
    ...
    priorToken?: unknown;
    method?: unknown;
    client?: { sourceIp?: string | null; userAgent?: string | null };
  }): Promise<{
    ok: true;
    token: string;
  } | {
  ...
  nativeSessionIssue(principal: {
    ...
    groups: string[];
  }, connId: string, method: "oidc" | "saml", nowMs: number, sessionNotOnOrAfter?: number | null, priorToken?: unknown, client?: { sourceIp?: string | null; userAgent?: string | null }): Promise<{
    ok: true;
    token: string;
  } | {
    ok: false;
  ...
  // The bounded SSO sign-in FAILURE aggregate (support diagnosis): a map keyed by the closed classifier code
  ...
  // The same write path for a failure whose closed code is known by its PATH (start / metadata) rather
  ...
  // The per-connection-KIND breakdown of the same aggregate: connKind (oidc|oauth2|saml) -> code -> {count,lastAt}.
  readSsoFailuresByKind(): Promise<Record<string, Record<string, { count: number; lastAt: string }>>>;
  // The bounded auth/RBAC defensive-branch signal aggregate: a closed event name -> { count, lastAt }. The
  ...
  // The same recorder carrying HOW MANY events one write stands for (the throttled recorder flushes the
  ...
  // The per-METHOD credential-path usage aggregate (a posture record, not a fault signal). Same bounded
  ...
  // The bounded ADMIN-COUNTER aggregate. It is
  ...
  // The THROTTLED variant, for anomalies observed on a HOT READ path (a corrupt approval, expiry row
  ...
  // The pinned IdP signing-CERTIFICATE health record. Cross-mixin: the SAML ACS (IdpSamlMixin) drains the
  ...
  // Auth posture probes: session signing-key presence + age + adequate-length (never the key), the count of
  ...
  // Which confidential connections are secretless, by OPAQUE ORDINAL (never a connId).
  ...
  // onStored fires the moment the FRESH record is persisted over the old one: the exact line past which
  ...
  // Staged recovery codes: mint-without-invalidating + the confirm that promotes it.
  ...
  stepUpFinish(body: {
    ...
    email: string | null;
    subject: string | null;
    // method and sourceIp are read ONLY by the authn-failure wrapper, which is the one authn-failure row in
    // the engine that can name a person. decodeCaller already carries both; the narrower shape this took
    // before simply threw them away. Optional, because a direct DO caller may omit them.
    method?: AuthMethod;
    sourceIp?: string | null;
  } | null): Promise<{
    ok: true;
  ...
  // The uncoalesced implementation. stepUpFinish wraps it so EVERY refused step-up funnels through one
  // authn-failure recorder rather than each return growing its own copy.
  stepUpFinishImpl(body: {
    challengeId?: unknown;
    credential?: unknown;
    rpId?: unknown;
    origin?: unknown;
  ...
  recordSiemPushOutcome(req: {
    ...
    toSeq?: unknown;
    gen?: unknown;
    causeDigest?: unknown; // the 12-hex Workers-Logs join key, shape-gated by the recorder
  }): Promise<{ ok: true }>;
