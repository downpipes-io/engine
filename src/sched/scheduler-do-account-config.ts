// AccountConfigMixin layers the console-set account-config subsystems (account-discovery API token +
// selected accounts, the assurance licence record, and the safe-apply update-lifecycle bookkeeping) over
// a base whose `this` is SchedulerDOSurface, with dispatch and `this` binding shared consistently across
// the whole SchedulerDO. The discoveryConfig/licenceToken/updateLifecycle keys, the fail-open licence
// semantics and the owner-only gates are the contract other code depends on: no storage key, route,
// status code, response body or auth gate may change without updating every caller.
...
// verify shape reject, which has no verify reasonCode). It is duplicated here on purpose (the DO must not
// import the admin licence module) so the DO defensively bounds its own input: only a member of this closed
// vocabulary is stored, so a licence VALUE or free text can never ride into the counter.
// An out-of-vocabulary code does not merely lose its label: recordLicenceActivationRefusal
// drops it to undefined, and the `if (reasonCode !== undefined)` guard below then skips the bounded refusal
// RING as well, so the count rises and the sequence that explains it is never written.
//   "internal-error" is the backstop code, the one that says "this is OUR fault, not your token". It is
// in the pack's own LICENCE_REFUSAL_CODE_SET (support-sections-config.ts) with a comment calling it
// LOAD-BEARING and warning that omitting it there would silently drop the new code at the pack boundary. It
// was omitted HERE instead, one layer earlier, so the drop it warns about was already happening and the pack
// never had the chance to forward what the DO refused to store.
//   "supersedes-current-term" is the refusal added with the term-regression check on POST /admin/licence: a
// genuinely signed, unexpired token whose term ends EARLIER than the active licence. Like "malformed-token"
// it is not a verify outcome, so it is not a member of the admin LicenceReasonCode union, which is the
// precedent for it living in this set and not in that one.
...
// every read-modify-write: the anti-rollback floors (the per-component map + its legacy engine-scalar
// mirror), the freshness watermark (sequence + issuedAt), the console component's last outcome, and the
...
// component's history (which would re-open the gaps the anti-rollback floor and the per-component split close).
...
// semverMax keeps the anti-rollback high-water mark MONOTONIC: it returns `candidate` only when it is strictly greater
...
export function AccountConfigMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    async setDiscoveryToken(
      req: { token?: unknown; accountsSeen?: unknown; tokenId?: unknown; expiresOn?: unknown },
      ...
      if (req.token === null) {
        ...
        await this.deleteObservedItem("discovery-token"); // V13.2.1: the registry row leaves with the credential
      ...
      // THE 100-ACCOUNT TRUNCATION. An org with more than 100 Cloudflare accounts has its tail cut here,
      ...
      // Entries the loop above SKIPPED (no usable id) are dropped silently too. Count them separately
      ...
      // V13.2.1: the credential is term-limited (the router refused it otherwise), and its expiry is a
      // registry row so the 30/14/7/1-day ladder and the credential-expiry posture check cover it. The row
      // carries the public token id and the expiry, never the value.
      if (typeof req.expiresOn === "string" && Number.isFinite(Date.parse(req.expiresOn))) {
        await this.upsertObservedItem({
          id: "discovery-token",
          label: "Cloudflare read-only discovery token",
          kind: "token",
          lifecycleClass: "functional",
          purpose: "Lists and reads the account's resources for source discovery and the API-read sources. Recreate it with a fresh expiry before this date and paste the new value on the Sources screen.",
          permissionSummary: "Read all resources (read-only)",
          expiresAt: req.expiresOn,
          ...(typeof req.tokenId === "string" && req.tokenId.length > 0 ? { tokenRef: req.tokenId } : {}),
        });
      }
      await this.appendAudit({
        actorEmail: caller?.email ? caller.email : null,
    ...
    async setDiscoveryAccounts(
      ...
      // THE "I SELECTED THREE ACCOUNTS AND ONLY TWO SHOW UP" TICKET. An id that is not in accountsSeen is
      ...
      // An engineAccountId the engine cannot resolve is silently COERCED TO NULL (the engine then has no
    ...
    async setEnabledSources(
      ...
      // A source type outside the closed set is REFUSED, not filtered out. A silent drop here would read to
      // the customer as "I added it and it never appeared", and would only be visible in a support pack nobody
      // opens until the customer has already lost the time. Nothing legitimate sits outside this set -- the
      // console's radio group can emit only its members, and the set is DERIVED from the config validator's
      // own allow-list, so a type this call accepted and the validator refused would be a control that appears
      // to work and protects nothing. A version-skew case (a newer console against an older engine) is better
      // served by a plain refusal naming the type than by a success that is not one.
    ...
    // ---- engine-account verification -------------
    // DISCOVERY_KEY.engineAccountId only ever gets set when an operator pastes a
    ...
    async recordVerifiedEngineAccount(req: { accountId?: unknown; via?: unknown }): Promise<{ accountId: string | null }> {
      ...
      // audit-status.ts's engine-observed events), NOT "access": the structural gate refuses any
    ...
    async setLicenceToken(
      ...
      if (token.length < 32 || token.length > 20000 || !/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/.test(token)) {
        // The router posts /licence-activation-refusal on ITS refusal path, so a token the router accepted and
        // the DO then rejected on shape would otherwise never touch the counter at all: the customer's "I
        // pasted my licence and it still says community" would have no record whatsoever. Route it through the
        // SAME recorder as every other refusal, with the closed pre-verify code.
    ...
    async recordLicenceActivationRefusal(req: { reasonCode?: unknown }): Promise<{ ok: true }> {
      ...
      // {count, lastAt, lastReasonCode} COLLAPSES a mixed-cause retry storm. A customer who tried five
    ...
    async getUpdateRecord(): Promise<UpdateRecord> {
      ...
      // READ-TIME MIGRATION (the migrateCanaryState precedent): a single-component estate's anti-rollback
      // floor is the engine scalar settledHighWaterMark; a multi-component estate's is the per-component
    ...
    async setUpdatePending(p: UpdatePending): Promise<UpdateRecord> {
      ...
      // ADVANCE the freshness watermark from the descriptor this promote accepted. The route only promotes
    ...
    async setUpdateSettled(last: UpdateLast): Promise<UpdateRecord> {
      ...
      // A SUCCESSFUL settle ("applied") advances THAT component's monotonic anti-rollback floor to the
      ...
      // applied-unconfirmed advances the floor exactly as applied does. The floor is the ANTI-ROLLBACK
      ...
      // A settle that is itself the accepting act for a channel descriptor (a console-only apply, which
    ...
    async claimRollbackNeeded(req: { recommendedVersion?: unknown; toVersion?: unknown; canaryVerdict?: unknown }): Promise<{ shouldAlert: boolean }> {
      ...
      const next: UpdateRecord = {
        ...
        last: rec.last,
        ...(rec.lastAlertedVersion != null ? { lastAlertedVersion: rec.lastAlertedVersion } : {}),
        ...stickyUpdateFields(rec), // carry the anti-rollback floor + freshness watermark forward (never drop them on a claim)
