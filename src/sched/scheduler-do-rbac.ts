// The RBAC subsystem (the role model keyed on the STABLE subject, the optional group->role mapping, and
// the composable custom roles). RbacMixin holds the role-resolution PRIMITIVES (normalise/list/resolveRole/
// resolveAuthority/customRoleNamesFor/capGroupRole); the caller-authority resolution + whoami + the
// anti-escalation guards live in the sibling ./scheduler-do-rbac-authority.ts (RbacAuthorityMixin), and the
// role/group/custom-role WRITES + their list views live in ./scheduler-do-rbac-mutations.ts
// (RbacMutationsMixin), keeping each RBAC file within the module-size guardrail. All three layer over a
// base whose `this` is SchedulerDOSurface, so the resolution helpers (resolveRole / roleForCaller /
// resolveAuthority / capGroupRole / can-checks) the route guards call resolve identically across the
// files; the guards themselves stay with their handlers. The role/group/custom keys, the owner-clamp, the
// maker-checker and least-privilege rules apply consistently across all three files.
...
// resolution: approver, the highest cumulative role a group is allowed to express (a defence-in-depth
// behaviour: a planted owner mapping resolves to approver, never owner). It is a module constant, used
// only by capGroupRole below.
const GROUP_CAP_OWNER_FALLBACK: Role = "approver";

export function RbacMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    // ---- RBAC role model keyed on the STABLE subject (ASVS V10.3.3 / V10.5.2) ---------
    // The role table keys authorisation on the immutable subject, not the mutable email, so a
    // recycled/reassigned email can never inherit a departed member's role.
    // THAT IS ONE DIRECTION AND THE MIRROR IS NOT CLOSED BY IT. A NEW person at an old address
    ...
    // A legacy `role:<email>` entry is read as a PENDING entry so it binds to a subject on next login and
    // is never silently dropped. Resolution, the bootstrap-Owner rule and the
    ...
    normaliseSubject(raw: unknown): string | null {
      ...
      // <sub>) can exceed a fixed 512-character floor, and if the resolution cap were lower than the session cap a long
    ...
    async listRoleEntries(): Promise<RoleEntry[]> {
      ...
      // A role row that does not parse is SILENTLY FILTERED OUT of authority resolution, which means that
    ...
    //
    // WHY THIS EXISTS AS ITS OWN READ. The malformed-row filter above already books the drop as an auth SIGNAL, and a signal is a thing
    ...
    async listPendingEntries(): Promise<PendingRoleEntry[]> {
      ...
      // The pending twin of listRoleEntries' malformed-row filter. A pending row that does not parse is
    ...
    asPending(email: string, rec: Record<string, unknown>): PendingRoleEntry {
      // PROVENANCE FABRICATION. A row without a usable grantedBy is read as granted by "unknown", and a
    ...
    async resolveBoundEntry(
      ...
      // IdP-asserted email - the "open-bind" account-takeover this guards against. A passkey/Access
      ...
      // The STRENGTH of the email verification behind this auth. An OWNER invite is the highest-privilege
      ...
      // Refuse to bind a pending OWNER invite off a weakly-verified email (a trust-idp SAML connection). The
      ...
      if (match.role === "owner" && !emailVerifiedStrong) {
        // The owner INVITE is refused because this connection does not genuinely verify the address
    ...
    effectiveRole(entry: RoleEntry | PendingRoleEntry | undefined, now: number): Role {
      ...
      if (entry.expiresAt) {
        ...
        // An expiresAt that does not PARSE fails OPEN by convention (malformed data must not itself
    ...
    // source of truth); this closes ASVS V8, where the custom-role axis had no expiry check at all.
    ...
    // through unchanged. It is not a rank ceiling (the two narrow roles are not on the cumulative
    ...
    capGroupRole(role: Role): Role {
      // This clamp is a TAMPER DETECTOR. A group mapping is not allowed to confer owner (rejected at
    ...
    boundGroupList(raw: unknown[], drops?: ClaimDropTally, boundary?: ClaimBoundary): string[] {
      // A groups list longer than GROUPS_MAX is TRUNCATED here, so a role-bearing group past the cap is
      ...
      const out: string[] = [];
      // THE CAP IS TESTED AFTER NORMALISE AND DEDUPE, as the OIDC bounder already does (oidc.ts boundGroups).
      ...
      // unbounded list to loop over): the scan stops at SCAN_MAX entries, and a list longer than that HAS
      ...
      for (let i = 0; i < scanned; i++) {
        // When the caller passes a tally, record WHICH bound threw a group away and at WHICH parse
    ...
    // customRole (if any AND not expired -- ASVS V8: an expired email grant contributes NO
    ...
    // customRole reference is likewise excluded (customRoleNamesFor's own expiry gate, ASVS V8), so
    ...
    resolveAuthority(
      ...
      for (const name of names) {
        ...
        if (rec === undefined) {
          // THE "three engineers lost console access on Tuesday" SIGNAL. The custom role this grant names
        ...
        // The clamp inside capabilitiesOfCustomRole drops an owner-reserved capability a stored custom
        ...
        // THE TAMPER SIGNAL NOBODY WAS TOLD ABOUT. validateCustomRole bars an owner-reserved capability
