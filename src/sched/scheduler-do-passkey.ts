...
// sibling ./scheduler-do-passkey-invites.ts (PasskeyInvitesMixin), kept in its own file so neither module
// exceeds the size guardrail. Both layer over a base whose `this` is SchedulerDOSurface, so the ceremonies
// issue + single-use consume challenges, resolve the registration authority and bootstrap the first Owner
// through `this` (the login ceremony reaches the bounding helpers here through `this` too). The
// account-takeover fix (the BOUND email is derived from the proof, never the client field), the single-use
// invite/challenge consumption, the empty-table + bootstrap-latch checks, the clone-detection signCount
// persistence and the recovery-code mint on enrolment all hold across both files: dispatch and `this`
// binding are identical, and no storage key, route, status code, response body or auth gate differs
// between them.
...
export function PasskeyMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  ...
  //
  // This is the SINGLE chokepoint every ceremony failure passes through (register/begin, register/finish,
  ...
  async resolveRegistrationAuthorisation(
    ...
    if (tableEmpty) {
      ...
      if (await this.getBootstrapConsumed()) {
        // The same DEADLOCK whoami detects, seen from the enrolment door: the role table is EMPTY and the
    ...
    if (inviteTokenRaw !== null) {
      ...
      // An invite token that is unknown, expired or already consumed. The caller only ever sees the
  ...
  > {
    ...
    // An rp.id the ROUTER could not supply is a deploy-config fault (an unset/unparseable CONSOLE_ORIGIN
    ...
    try {
      ...
      // This is the "the invite link fails for my colleague" ticket. The coarse forbidden is deliberately
      ...
      const publicKey = {
        ...
        attestation: "none",
        // BOUNDED BY THE BROWSER'S OWN CEILING: an unbounded `existing.map(...)` list, every credential the
        // email holds, breaks past 64 entries, where the browser refuses the whole ceremony with a RangeError
        // before any authenticator is consulted. See EXCLUDE_CREDENTIALS_MAX for the measurement and for why an
        // unbounded list is a permanent-lockout path rather than a blemish: enrolment is also what mints a
        // fresh recovery-code set, so an account over the ceiling spends a break-glass code on every
        // sign-in and replaces none of them.
  ...
  async passkeyRegisterFinish(
    ...
    | { ok: true; email: string; bootstrapped: boolean; role: Role; recoveryCodes: string[]; recoveryCodesPending: boolean; enrolmentPath: "bootstrap" | "invite" | "self-add" }
    | { ok: false; reason: string; errorId?: string }
  > {
    ...
    if (!clientEmail) return { ok: false, reason: "bad_request" };
    // As in begin, an absent rp.id/origin is the SERVER's deploy config, reported to the user as their
    ...
    try {
      ...
      if (!auth.allowExistingCreds) {
        ...
        if (existingForEmail.length > 0) {
          // An INVITE redeemed for an address that is ALREADY enrolled. The invitee completes the whole
      ...
      //
      // The witness must open BEFORE this credential's createdAt is taken, so createdAt is at or after it,
      // ordering the two CLOCK READS rather than the two stores. ensurePasskeyWitnessSince awaits a storage
      // read before it stamps, so a millisecond can tick in between and leave createdAt STRICTLY BEFORE
      // witnessSince if the order were reversed. lockoutPreflight reads `createdMs < witnessSinceMs` as
      // "predates the record-keeping" and would answer `unknown` instead of `never-asserted`, permanently,
      // for the FIRST credential on the account -- the only one that opens the witness, and the one an
      // operator is most likely to be reading that screen about. Taking the clock reads in the stated order
      ...
      if (auth.path === "bootstrap" && (await this.roleTableIsEmpty()) && !alreadyConsumed) {
        ...
        // This is the SILENT DEMOTION. A caller who came in on the BOOTSTRAP path and reaches this branch was
      ...
      //
      // An enrolment that runs while a LIVE record ALREADY exists is a self-add -- which includes the forced
      // re-enrolment a recovery-code sign-in requires. That
      ...
      try {
        ...
        // This is the "I never got recovery codes at enrolment" ticket. The user is now enrolled with an EMPTY
      ...
      // enrolmentPath is the DO's PROVEN classification of this enrolment (bootstrap | invite | self-add), so the
      // router can gate the credential-change alert on a fact the DO established rather than on
      // recoveryCodesPending, which is a recovery-code state and says nothing about who enrolled (V6.3.7).
      return { ok: true, email, bootstrapped, role, recoveryCodes, recoveryCodesPending, enrolmentPath: auth.path };
    } catch (e) {
