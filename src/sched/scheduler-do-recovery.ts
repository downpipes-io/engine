// RecoveryMixin implements the recovery-codes subsystem: the admin sign-in break-glass of
// generate/regenerate/verify single-use recovery codes with a per-IP and per-email rate limiter. It
// layers onto a base whose `this` is SchedulerDOSurface, so recovery mints a session via
// `this.sessionSigningKey` and consults the role table through `this`. Storage keys live under the
// `recovery:` and `recovery-rate:` prefixes; verification is constant-time and each code is single-use.
...
export function RecoveryMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    // on first use. It is a separate key from the session-signing key: terminateAllSessions DELETES the
    // session key to invalidate every outstanding token, so if recovery hashes were computed under that same
    // key, a global sign-out would silently kill every banked recovery code in the account. The two secrets
    // have opposite lifetimes, so they live in two independent records.
    //
    // ADOPTION, BELOW, RE-HASHES NOTHING. An account with no recovery key yet has recovery records (if any)
    // whose HMACs were computed under the CURRENT session key bytes, so on first use this copies those exact
    // bytes into the recovery record. Every stored hash keeps verifying unchanged: no record is rewritten, no
    // code is re-minted, and there is no window in which a banked code is dead. From that point the two keys
    // are independent and only an explicit regenerate replaces a code.
    //
    // WHY THE SESSION KEY IS READ FROM STORAGE AND NEVER THROUGH sessionSigningKey(): that accessor GENERATES
    // and persists a fresh key when the record is absent or corrupt. Calling it here would let the adoption
    // freeze a brand-new key that no stored hash was ever computed under, and it would do it durably. A direct
    // read cannot.
    //
    ...
    //
    // A cold account with NO session key falls through to a fresh CSPRNG key. That is the correct answer in
    // both cases it can arise: a brand-new DO (which has no recovery record to invalidate), and an account
    // whose session key was already deleted by a sign-out that predates the recovery key (whose codes are
    // already dead, and for which no key anywhere can revive them). Both are recorded as closed auth signals
    // so the pack can tell them apart.
    async recoverySigningKey(): Promise<Uint8Array> {
      ...
      // would silently invalidate every hash computed under the old bytes), so the common case is one cheap
      // read with no gate.
    ...
    // itself, the recovery-side twin of sessionSigningKeyHealth. The session key's health does not answer "can
    // a banked recovery code still verify?", since the two keys are independent, so this is the diagnostic
    // surface for the recovery key specifically. `materialised` false is not a fault: it means no recovery
    // operation has run on this DO yet, so the key is still to be adopted from the session key on first use.
    // Redaction-safe: booleans and an int age, never the key bytes.
    ...
    // key import, so an unknown email costs the same HMAC work as a known one rather than an extra pass on top
    // of it (which would be a timing oracle on enrolment). Needing no key, it takes none. It is never
    // persisted and can never match a real presented code (its "hashes" are uniform random, not real HMAC
    // output), and the caller forces the match off when the real record was null. The email field is a fixed
    // placeholder; it is never read for the dummy.
    ...
    // The signing key is the in-DO RECOVERY key (never leaves the DO). It audits the generation as a
    // redaction-safe access-policy event (who + when, never a code). It is the single internal mint path
    ...
    //
    // onStored is the INVALIDATION BOUNDARY, published to the caller. The put below REPLACES the email's
    // record, so the prior codes verify before it and are dead after it. Everything that follows the put can
    // throw (the audit append), and when it does the caller 500s and the fresh plaintext is never returned --
    // the operator then holds NEITHER set. Only this function knows which side of the put the throw happened
    // on, so it says so rather than making the caller guess.
    async generateRecoveryFor(canonicalEmail: string, actorMethod: AuthMethod, sourceIp: string | null = null, onStored?: () => void): Promise<string[]> {
      // The RECOVERY key, not the session key: minting under the session key would make a global sign-out
      // destroy every banked recovery code, and a set minted here survives it.
      const key = await this.recoverySigningKey();
      // A recovery signing key too short to import breaks EVERY recovery operation (generateRecoveryCodes
      // throws below and the caller 500s), so the last way back into a locked-out account would fail with
      // nothing recorded. Record the signal, then let the throw happen. Counts only; the key is never read,
      // logged or returned.
      ...
      // The old set is now dead. Past this line a failure costs the operator BOTH sets.
      onStored?.();
      // NO LATCH IS WRITTEN HERE. A write-once boolean recording "an Owner has recovery codes", evaluated only
      // at mint time, would survive the codes being spent, the Owner being demoted, the record being deleted
      // and the key being rotated -- telling operators it was safe to destroy the only credential that still
      // worked. recoveryBreakGlassVerdict re-derives all of it from live state on every read instead,
      // including the passkey-bound-Owner test, so there is nothing for a latch to remember and no way for one
      // to go stale.
    ...
    // generateRecoveryStaged mints a fresh set for canonicalEmail WITHOUT touching the live record: the live
    // set (if any) keeps verifying unchanged. It exists for the one case generateRecoveryFor's immediate mint
    // is wrong for -- an enrolment that runs
    ...
    async recoveryRegenerate(body: { email?: unknown; ip?: unknown; method?: unknown }): Promise<{ ok: true; codes: string[] } | { ok: false; reason?: "off-roster" }> {
      ...
      // An email that does not normalise refuses here with a bare ok:false, which the console shows as a
      // generic failure -- the operator is nagged to regenerate forever. Counted; the response is unchanged.
      if (!email) {
      ...
      // A regenerate that errors partway through can leave the operator in one of two different states.
      // Everything from here on either happens BEFORE the new record is stored (the old codes survive, and the
      // operator should keep using them) or AFTER it (the old codes are gone and the new plaintext never left
      // the engine, because the caller got a 500 instead of the codes). Those are opposite remedies: "use the
      // set you already have" against "you have no set at all; recover through another Owner, the break-glass
      // token or a passkey".
      //
      // The throw is RE-THROWN unchanged, so the DO's outer catch answers the usual 500 and the router's own
      // recovery-regenerate-failed signal fires. Counts only, no code, hash, salt, email or message.
      let stored = false;
      let codes: string[];
      try {
        ...
        // it terminates sessions issued before the rotation: bump the email's session epoch. A FIRST enrolment
        // (the generateRecoveryFor enrolment hook path) does not bump -- only this explicit regenerate does, so
        // initial setup never signs the operator out. It is INSIDE the guarded region: it runs after the store,
        // so a throw here costs the operator the old set with no new one to show for it.
    ...
    async recoveryRecover(
      body: { email?: unknown; code?: unknown; ip?: unknown; priorToken?: unknown; userAgent?: unknown },
    ): Promise<
    ...
    > {
      ...
      if (!rateOk) {
        // NO ACTOR ON A THROTTLED ATTEMPT. This is the ONE audit row on this route that is written before a
        // single byte has been compared against anything: the limiter denies ahead of the record read, so
        // `email` here is nothing but a string an UNAUTHENTICATED caller put in a request body. See the note
        ...
        // Record the bounded auth-signal so that a recovery sign-in being refused is diagnosable (the hard
        // per-IP/per-email fail-closed limiter denying, or a limiter outage failing closed) -
        // recovery-ratelimited. Best-effort; never blocks the deny.
      ...
      // TWO KEYS, DELIBERATELY. `key` verifies the presented CODE and must survive a global sign-out; the
      // session token minted on success below is signed under the SESSION key, which must not. Reading both
      // here (unconditionally, before the record read) keeps the anti-enumeration timing uniform: a known and
      // an unknown email do the identical storage work.
      const key = await this.recoverySigningKey();
      ...
      // The same signing-key observation as the mint path. verifyCode throws under an unusable key, so
      ...
      // A REAL record (never the synthetic anti-enumeration dummy, whose slots are random bytes) whose
      ...
      if (!result.matched || record === null || email === null) {
        // A REJECTED code (as opposed to a THROTTLED attempt) is recorded into the existing closed auth-signal
        // vocabulary (recovery-code-invalid), not a parallel one. The generic client response and the
        // no-oracle property are unchanged.
        ...
        //
        // THE ROW NAMES NO ACTOR. No identity is verified on a failure, so the actor fields are null, matching
        // how scheduler-do-routing-identity.ts writes a failed passkey login. POST /admin/auth/recovery is
        // UNAUTHENTICATED, so `email` on this branch is a string the caller typed and nothing has verified
        // that they are that person, or that the address belongs to this account at all. Recording it as the
        // actor would let an unauthenticated caller from the public internet choose the name the
        // tamper-evident chain records as the ACTOR of a break-glass attempt, with the console rendering that
        // name beside an action whose own label reads "Recovery code used".
        //
        // This mirrors routeRecoveryAlert, which carries the email on a successful use and stays generic on a
        // failure: a failed attempt's email is an unverified guess that must not be echoed as if it were a
        // real account.
        //
        // NOTHING DIAGNOSABLE IS LOST. The row keeps the source IP and the recovery method; `recovery-code-invalid`
        // (and `recovery-ratelimited` above) already count the class into the bounded auth-signal vocabulary the
        // pack reads; and the SUCCESS row below is untouched, because there the code match is what verifies the
        // identity. The generic 401, the no-oracle property and the uniform timing are all unchanged: this
        // writes a different value into a record the caller can never read.
        await this.appendAudit({
          actorEmail: null,
          actorMethod: "recovery",
          sourceIp: ip,
      ...
      // A recovery sign-in from a browser that already holds a session ends that session first.
      await this.revokePresentedSession(body.priorToken, Date.now());
      const token = await this.mintSessionRecorded(await this.sessionSigningKey(), { method: "passkey", email, subject: passkeySubject(email), epoch: await this.getSessionEpoch(email) }, Date.now(), { sourceIp: typeof body.ip === "string" ? body.ip : null, userAgent: typeof body.userAgent === "string" ? body.userAgent : null });
    ...
    // attempt is DENIED, the opposite of the fail-open per-caller limiter (rateLimited), because a guessing attack must never
    ...
    async recoveryRateAllow(emailKey: string, ipKey: string | null): Promise<boolean> {
      try {
        ...
        // The three ways this route denies -- the per-EMAIL bucket (this one account is being sprayed), the
        // per-IP bucket (a whole office behind one NAT locking itself out) and the limiter's own store failing
        // CLOSED (nothing is wrong with the codes at all) -- are recorded under distinct fault names here so
        // they are diagnosable from the pack. The caller's verdict, the audit row and the generic client
        // response are all the same regardless (no oracle is created): the bucket class is recorded pack-side
        // only, and never the email, IP, prefix or code.
    ...
    // AS AN OWNER. It re-derives the answer from live state on every call rather than relying on a write-once
    // latch set at mint time, which would stay true after every code was consumed, after that Owner was
    // demoted, after the record was deleted, and after the key the codes were hashed under was gone. That kind
    // of latch feeds buildDisposeBootstrapToken, which tells the operator "it is now safe to dispose of it"
    // about the ADMIN_TOKEN, and gates setBreakGlassTokenRetired, so a factor that does not work must never be
    // able to authorise destroying the only credential that did.
    //
    ...
    // recordPasskeyAssertion stamps the usability witness. Call it, and only call it, on the success side of a
    // verifyAssertion: at that point a signature over server-chosen challenge bytes has verified under this
    // credential's stored public key, so the private half provably existed a moment ago. The stamp is a record
    // of that proof. Every other field on a PasskeyCred is inert to the destruction of the authenticator, so a
    // dead credential can otherwise keep answering as enrolled indefinitely.
    //
    // THE WRITE IS UNCONDITIONAL, and that is the whole point. Writing only when `result.newSignCount !==
    // record.signCount` would miss it: platform passkeys routinely never advance the counter (see the
    // clone-detection carve-out in admin/passkey.ts, which exists because zero-to-zero is the common healthy
    // case), so a witness gated on the counter changing would be blind to precisely the authenticators
    // signCount is already blind to -- a second inference wearing a proof's clothes. So this absorbs the
    // signCount persistence too: one put, both facts, on every verified assertion.
    ...
    async listPasskeyCredentials(
      ...
      // target. Answering that with an empty list would be BYTE-IDENTICAL to "this account holds none", so the
      // one read an operator would use to establish that an account carries no leftover credentials would
      // return the reassuring answer in exactly the case where nothing had been looked at. A bearer token is
      // exactly that caller: GET /whoami answers `email:null` for one, so an ADMIN_TOKEN sweep with no target
      // resolved would read a clean bill of health regardless of what credentials the estate actually holds.
      //
      // Refusing instead is the load-bearing choice: a 200 carrying an "I could not resolve a target" flag
      // would still read as an empty array to any client that does not check the flag, which recreates the
      // same ambiguity; a client cannot mistake a 400 for a clean account. The plain Error is the DO's
      // established validation-refusal mechanism (scheduler-do.ts fetch() maps it to 400 with this message).
      // No console path regresses: the console only calls this without an email from a cookie session, which
      // always has one.
    ...
    //
    // WHY THIS REPORTS RATHER THAN REVOKING ON deleteRole. deleteRole does not touch `passkeyCred:` records, so
    // a removed member's credentials outlive their role. The tempting approach is to have deleteRole revoke
    // them. That would be WRONG, and not as a matter of taste: groupRoleFor (scheduler-do-rbac.ts) confers a
    // role from IdP GROUP CLAIMS with no `role:` row involved at all, so an email with no entry in the role
    // table can still be a fully legitimate operator. Destroying their credential when their row is deleted
    // would lock out a real user whose authority never depended on that row. The inverse of "harmless because
    // authorisation fails elsewhere" applies: it would be HARMFUL because authorisation SUCCEEDS elsewhere.
    //
    ...
    async listAllPasskeyCredentialsForAccount(
      ...
      for (const p of pending) {
        if (!p.email) continue;
        // An EXPIRED pending invite is not a way in, so it does not make a credential look accounted for. Gate
        // on the expiry itself, not on the resolved role: effectiveRole never returns null, so testing for
        // that would never skip an expired invite, and testing `!== "viewer"` would incorrectly drop
        // legitimate unexpired viewers out of the accounted-for set. listSignInFactors gates the same way for
        // the same reason.
    ...
    // revokePasskeyCredential deletes one WebAuthn credential (the theft/loss revocation).
    ...
    async revokePasskeyCredential(
      ...
      if (targetIsOwner && this.countOwners(entries, pending, now) === 1) {
        ...
        if (remaining <= 1) {
          // THE LOCK-OUT GUARD, RECORDED. This throws a plain Error and answers 400, bypassing the AuthError
          // funnel, so this records the refusal explicitly -- without it, a lost-device revoke refused by this
          // guard and one refused by a malformed credential id would be indistinguishable on the engine side,
          // leaving the operator locked out of their own console with no evidence of why the revoke would not
          // take.
          await recordAuthzRefusal(this.state.storage, "last-passkey-guard");
          // The LOCKOUT-PREVENTION check is a policy guardrail, so it counts on the route x reason ledger too
          // ({rbac-guardrail, guardrail}: last owner, reserved capability, a lockout-prevention check). The
          // rate is the diagnosis: an operator hammering Revoke on a key they no longer hold, refused every
          // time, is a lockout in progress.
      ...
      if (deleted) {
        // Revoking an authentication factor terminates sessions that predate it: the bumped epoch is folded
        // into the session MAC, so an older cookie fails verify on its next request.
    ...
    async sweepAndBoundLoginChallenges(now: number): Promise<void> {
      ...
      // LIVE (unexpired) login challenges evicted by the cap fail with the same generic "challenge" reason an
      // expiry gives, so a begin flood that pushes real users' challenges out is otherwise indistinguishable
      // from ordinary sign-in failures. Record it so the two are diagnosable apart. Best-effort and never
      // throwing (this sweep must never fail a begin).
