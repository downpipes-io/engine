// The invite + login half of the WebAuthn / passkey subsystem (the first-Owner bootstrap-invite slot, the
// single-use teammate invites, and the login assertion ceremony) is kept separate from scheduler-do-passkey.ts
// so neither file exceeds the module-size guardrail. This is the sibling sub-mixin of PasskeyMixin (the
// registration ceremony + the shared input-bounding/coarse-error helpers stay there); both layer over a base
// whose `this` is SchedulerDOSurface, so the login ceremony still single-use consumes its challenge through
// this.consumeChallenge and bounds its fields through this.decodeB64urlField / this.passkeyRpId (which live
// on the registration sub-mixin) with the SAME dispatch and `this` binding a single class would have.
// Dispatch is byte-identical; no storage key, route, status code, response body or auth gate changes because
// of the split. A leaf the assembly depends on, never the reverse.
...
export function PasskeyInvitesMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  ...
  async mintBootstrapInvite(rawEmail: unknown, now: number): Promise<string | null> {
    // Three DISTINCT refusals collapse into one silent null, and the route's no-oracle 200 means the
  ...
  async bootstrapInviteMatches(token: string, now: number): Promise<string | null> {
    ...
    // Compare the presented and stored tokens with the shared constant-time helper
    ...
    if (!constantTimeEqual(enc.encode(token), enc.encode(rec.token))) {
      // The ONE invite refusal where a record was FOUND, is live, and the presented secret is simply not
  ...
  async peekInvite(token: string, now: number): Promise<string | null> {
    // The register/BEGIN arm of the same three refusals ("the invite link does not work" is reported far
  ...
  async consumeInvite(token: string, now: number): Promise<string | null> {
    // Every arm below returns the SAME null, which the caller maps to one coarse "forbidden" -- so a
  ...
  > {
    ...
    // An absent rp.id is the SERVER's deploy config (an unset/unparseable CONSOLE_ORIGIN after a host
  ...
  > {
    ...
    try {
      ...
      try {
        ...
        // The ENGINE's own stored credential is corrupt, yet the wire reason is `bad_request` - so the
      ...
      // in one unconditional put, only AFTER a positive verification. The condition that used to wrap this
      // write (`result.newSignCount !== record.signCount`) is deliberately gone: it is the right condition for
      // a counter and the wrong one for a witness, because a platform passkey that never advances its counter
      // would then produce a verified assertion that left NO trace, which is the same blindness that let
      // `passkeyOwnerEnrolled` count seven dead credentials as a factor. Writing on every success also keeps
      // the fact honest in the other direction: a failed assertion still writes nothing.
      await this.recordPasskeyAssertion(record, result.newSignCount, "login");
      return { ok: true, email: record.email };
    } catch (e) {
      const res = this.coarsePasskeyError("login/finish", e);
      // The recentErrors ring classes an errorId from the COARSE wire reason, so the id a locked-out user
