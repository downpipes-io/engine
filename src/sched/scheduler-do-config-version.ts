// ConfigVersionMixin layers the config-as-a-source versioning subsystem (the default self-contained
// history layer) over a base whose `this` is SchedulerDOSurface, so gatherConfigSnapshot reads the
// per-subsystem state held on sibling mixins through `this`, with dispatch and `this` binding shared
// consistently across the whole SchedulerDO surface. The CONFIG_HISTORY_* keys, the hash chain and the
// snapshot/diff shapes are the contract the rest of the engine depends on: no storage key, route, status
// code, response body or auth gate may change without updating every caller.
...
export function ConfigVersionMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    // ---- Config-as-a-source versioning (the DEFAULT self-contained layer) --------------
    ...
    async snapshotConfigNow(author: string | null): Promise<{ created: false } | { created: true; version: ConfigVersion }> {
      ...
      // Committing the HEAD ANCHOR in the same storage turn as the version (the DO is single-threaded, so
      // there is no torn read) is what makes a TAIL DELETION visible: every verify pass walks the retained
    ...
    async autoSnapshotConfig(author: string | null): Promise<void> {
      try {
        ...
        try {
          // Read through the VALIDATING reader, not raw. The raw read used to trust storage.get<T>'s type
          // parameter, which is an assertion rather than a check, and `(prior?.count ?? 0) + 1` then applied
          // JavaScript's + to whatever came back: a count of "7" became "71", then "711", so the record got
          // FURTHER from repair with every failure and the health read stayed at zero for ever. Its two sibling
          // markers (bumpEmergencyMarker, bumpChangeControlRefusal) both bump through their reader and so heal
          // in one write; this one LATCHED. Reading through getConfigSnapshotHealth makes the three agree, and
          // books the coercion on the way past.
          ...
          // The count says N config changes were never versioned; the ROOT CAUSE lives only in the
    ...
    async getConfigSnapshotHealth(): Promise<ConfigSnapshotFailureState> {
      ...
      // The same present-but-malformed coercion readEmergencyChangeMarker books, on the tally that says
    ...
    async configHistorySigningKeyRotated(key: Uint8Array): Promise<boolean> {
      ...
      if (stored === null) {
        ...
        // Establishing the baseline OPENS a structural false-negative window. Until the next comparison
    ...
    // configHistoryUnkeyedVerdict runs the KEY-FREE integrity pass over the whole retained chain: the
    ...
    //
    // It ALSO carries the BODY-RETENTION scan. Everything above is about the chain's headers; a version
    // whose SNAPSHOT BODY is gone still links, still lists, and still shows in the console as a version you
    // could roll back to. You find out it is not at the moment you try, in the incident. The scan runs FIRST
    // and the verify is guarded, deliberately: verifyConfigChain hashes each snapshot, so a missing body makes
    // it THROW -- which used to take the whole probe down and leave the pack with an unexplained
    // "config-history unavailable" instead of the actual diagnosis. verifyFaulted says the chain could not be
    // checked at all, which is a different (and worse) statement than "the chain is broken at version N".
    async configHistoryHealth(): Promise<{
      ...
      try {
        ...
        verifyFaulted = true;
        // A MISSING BODY is its own cause class. verifyConfigChain hashes each snapshot, so a version
        // whose body is gone makes the verify THROW rather than report -- and a thrown verify used to read
        // identically to a tampered chain. A gone body is a lost rollback point, not a tamper; they need
        // opposite answers.
        verdict = { intact: false, checkedThrough: -1, earliestId: all.length > 0 ? all[0]!.id : -1, causeClass: "missing-version" };
      }
      // The rotated-key fact is read BEFORE the latch, not after it: a regenerated or lost in-DO key makes
      // every digest fail, and a latch reading straight off the keyed verify would file that as a forgery.
      const signingKeyRotated = await this.configHistorySigningKeyRotated(key);
      // AND THE KEY FACT MAY NOT SPEAK FOR THE WHOLE CHAIN ON ITS OWN. verifyConfigChain returns at the
      ...
      // And the HEAD ANCHOR, which is the only check that can see the newest versions being deleted.
      ...
      return {
        ...
        verify: {
          ...
          // And beside it, what the KEY-FREE pass found over the whole chain. Without this the configIntegrity
          ...
          // THE ANCHOR SPEAKS IN THE VERDICT, not only in the latch. A DELETED TAIL leaves the retained
          // versions linking perfectly, so the keyed verify reads intact:true, and the projector only surfaces
          // a history that has something to say -- so the anchor must ride with the verify it qualifies, or the
          // latch becomes the truncation's ONLY carrier while the pack's own configIntegrity section says
          // nothing contradicts a clean bill of health.
    ...
    async configHistoryList(): Promise<{
      ...
      // The SAME rotated-key consultation the health probe makes: a latch written from this route must
      // not claim a forgery on an estate whose signing key was simply regenerated. And the SAME key-free pass
      ...
      // The SIBLING READER, which is the screen the customer is actually looking at. This route computes the
      // head-anchor verdict for the latch and then returned a verify block without it, so the console rendered
      // "Chain verified" over a history whose newest versions had just been deleted -- the same lie the pack told,
      // on the surface a customer sees first. The anchor rides out with the verdict here too.
      const headAnchor = await this.configHistoryHeadAnchorVerdict(all);
