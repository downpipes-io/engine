// RestoreApprovalMixin layers the restore dual-control subsystem (the restore APPROVAL state machine)
// plus the drill-evidence log over a base whose `this` is SchedulerDOSurface, so the maker/checker gates
// (requestRestore/approveRestore/rejectRestore/gateRestore/consumeApproval) and the drill-evidence
// rollover call `this.appendAudit` etc through the shared surface, with dispatch and `this` binding
// consistent across the whole SchedulerDO. The approval:/drill-evidence: keys, the approval TTL and the
// maker != checker rule are the contract other code depends on: no storage key, route, status code,
// response body or auth gate may change without updating every caller.

import { APPROVAL_PREFIX, APPROVAL_TTL_MS, type ApprovalSpendVerdict, approvalAnchorMs, approvalIdentitiesStillAuthorised, approvalKey, approvalTimestampUnparseable, canApprove, canReject, canRequesterSee, classifyRestoreApplyRefusal, effectiveStatus, isUsableApproval, PLAN_SEEN_PREFIX, RESTORE_APPLY_DEADLINE_MS, RESTORE_REJECT_REASON_SET, type RestoreApproval, type RestoreRejectReason, TooSoonRefusal, viewStatus } from "../admin/approvals.ts";
...
//
// THE SWEEP USED TO DEFEAT THE REFUSAL IT WAS MEANT TO SERVE, and that is the reason requestRestore refuses on
// an ABSENT anchor and not only on a stale one. The sweep runs on every dry run, for every plan hash, on the
// SAME condition the stale-plan refusal tests: `now - at > RESTORE_APPLY_DEADLINE_MS`. So when the refusal
// read a missing anchor as "no preview recorded, anchor to now", any colleague previewing any unrelated plan
// deleted exactly the anchors the refusal existed to catch, and the operator holding yesterday's card was
// granted a fresh 24 hours -- once per cycle, indefinitely, which is the hole the anchor was introduced to
// close. The refusal could only ever fire in the gap between an anchor going stale and the next dry run
// anybody ran.
//
// It is closed by making the two sets COINCIDE rather than by remembering what was swept. An entry is swept
// only when it is already past RESTORE_APPLY_DEADLINE_MS, which is exactly when a request anchored to it must
// be refused; and a request with no anchor at all is refused too. So the sweep's deletion set is a SUBSET of
// the refusal set, and deleting an anchor can no longer change any request's verdict. A tombstone was the
// other way to tell "swept" from "never recorded", and it is unnecessary once both answers are the same one;
// it also needs its own retention, and whatever horizon that took would be a horizon past which the reopen
// came back. Anchors younger than the deadline -- every card an operator could still legitimately request
// against -- are untouched by the sweep, so ordinary traffic cannot refuse a customer holding a valid plan.
//
...
//
// NULL IS NOT A FALLBACK. It used to mean "no preview was recorded, so the request route's own re-plan is the
// plan and `now` is the correct anchor", and that reading is what the sweep turned into an indefinite reopen:
// a swept anchor and an anchor that never existed are indistinguishable here, so every stale card became a
// fresh 24-hour window as soon as anybody ran any dry run. requestRestore now refuses on null, so an approval
// is only ever minted against a preview this engine recorded and can date. The re-plan the request route does
// is not a substitute, because its fidelity warnings are never shown to anyone: the operator approved the
// card they read, not the plan the request route recomputed behind them.
//
...
export function RestoreApprovalMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    // ---- Dual control: the restore APPROVAL state machine ----------------------------
    ...
    // an in-flight "applying" RESERVATION (ASVS 2.1.6): without this, a re-request would silently
    ...
    // readApprovalRecord is the ONE read every approval path goes through. It is a plain
    ...
    async requestRestore(
      ...
      // RESERVATION (effective status applying, ASVS 2.1.6): either would silently drop a checker's
      ...
      if (existingStatus === "approved" || existingStatus === "applying") {
        // Every refusal in this state machine is a thrown Error whose text reaches one operator's screen
      ...
      const record: RestoreApproval = {
        ...
        // destinationId: WHICH archive copy the cues were computed against, so the approver's
    ...
    async approveRestore(
      ...
      // subject differs is a legitimate distinct checker. R-93: it ALSO compares the caller's email to the
      ...
      if (!verdict.ok) {
        ...
        // recomputing it from checkerSubject alone would have missed the R-93 email-floor refusal case.
        ...
        // The closed reasonCode the decision helper ITSELF returned, so the refusal's cause survives into
        ...
        // ASVS V2.4.2: a too-soon refusal is a typed error so the fetch() catch answers 429 with a
        // Retry-After the operator's console can actually wait out, rather than the generic 400.
    ...
    async rejectRestore(
      ...
      if (!verdict.ok) {
        // The refused VETO records under its OWN stage, so "the approval was refused" and "a reject of a live
        // restore kept failing while the apply went through" -- the highest-signal event this machine
        // produces -- are never coalesced into one count under a shared key (e.g. restore-approve|applying-lease).
      ...
      // The CLOSED rejection reason, re-gated HERE (defence in depth: the router validates it too, and a
    ...
    // makes the apply exclusive (ASVS 2.1.6: the single-use transition is atomic with the START
    ...
    async gateRestore(req: { planHash?: string }): Promise<{ usable: boolean; approval: RestoreApproval | null; required: boolean }> {
      ...
      if (!usable) {
        // THE ROW THE WHOLE GAP IS ABOUT. This gate is the FIRST approval check a restore apply makes,
    ...
    // reserveRestore is the ATOMIC gate-to-write reservation (ASVS 2.1.6): the router calls this
    ...
    async reserveRestore(req: { planHash?: string }): Promise<{ reserved: boolean }> {
      ...
      if (!isUsableApproval(record, now)) {
        // Five distinct causes ("nobody raised this plan", "it expired", "it was already used", "another
    ...
    async releaseRestore(req: { planHash?: string }): Promise<{ released: boolean }> {
      ...
      if (record === null || record.status !== "applying") {
        // A STALE release (the lease lapsed and a fresh reserve reclaimed the record) is correctly
    ...
    async consumeApproval(req: { planHash?: string }): Promise<{ consumed: boolean; approval?: RestoreApproval }> {
      ...
      const record = await this.readApprovalRecord(req.planHash);
      if (record === null || record.status !== "applying") {
        // THE ACCOUNTING HOLE. This branch is reached AFTER a restore apply SUCCEEDED: the write landed
    ...
    // optional note. The log is BOUNDED by DRILL_EVIDENCE_CAP: recordDrillEvidence
    ...
    async recordDrillEvidence(
      ...
      // The DATED evidence trail is what a customer's auditor reads, and the console's POST that writes
      ...
      try {
        ...
        // The drill HAPPENED (downpipes[].lastRestoreTestAt proves it) and its evidence row does not
      ...
      // Roll over the OLDEST rows once the count exceeds DRILL_EVIDENCE_CAP, mirroring
