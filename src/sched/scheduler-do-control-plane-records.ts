// The control-plane RECOVERY-RECORD + observability accessors live here, split out of
...
// assembly depends on, never the reverse (madge 0 cycles).
...
// The DO-side redaction gate for the per-destination export reason: the vocabulary is taken from the
...
export function ControlPlaneRecordsMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    async recordControlPlaneRecoveryRefused(reason: string, code?: AutoHealRefusalCode, extra?: { subCause?: AutoHealSubCause; scan?: CandidateScan }): Promise<void> {
      await this.state.storage.put(CONTROL_PLANE_STAGED_KEY, {
        refused: {
          ...
          at: nowMillisISO(),
          ...(code !== undefined ? { code } : {}),
          // The closed sub-cause (WHY, within the code) and the candidate-scan counts (what the bucket
      ...
      // A refusal recorded with NO closed code carries only the operator-facing free text, which the
    ...
    // attempt. The recovery latch's own blind spots (no bucket to probe; a bucket whose RUNLOG is
    // absent/unreadable) leave no signal, so a support engineer could not otherwise distinguish a healthy new
    // account from an amnesia the latch cannot confirm. Redaction-safe (a closed enum + a timestamp).
    ...
    // recordControlPlaneDeployObservation is the DETERMINISTIC per-tick deploy-identity observer. The
    ...
    // recordControlPlaneExportHealth stores the outcome of the last control-plane export pass: whether it
    // wrote a fresh generation, the closed skip reason when it did not, and the PER-DESTINATION write outcome so a partial
    ...
    async recordControlPlaneExportHealth(health: ControlPlaneExportHealth): Promise<void> {
      ...
      // The per-destination REASON rides alongside ok. It is gated against the closed CpExportFailClass
      ...
      // Without a true total and truncated flag, a large-fleet tenant's destinations past the cap would read
      // as fully-covered -- a MISSING RECOVERY COPY, invisible to the self-backup posture check that reads
      // this record to decide allDestinationsWrote. The record carries the TRUE total and an explicit
      // truncated flag (the cfConfigDiscovery unavailableCount pattern), and the dropped rows are counted in
      // the ledger too. Counts only; the dropped destinations' ids stay out.
    ...
    // recordControlPlaneImportOutcome persists the OUTCOME of the last estate import. The import answers
    // with crossAccount + accountIdAbsent on its HTTP response; persisting them here is what lets the
    // support pack (built long after that response is gone) tell a genuine cross-account rebuild apart from
    // a pre-account-discovery export -- two states with the same disabled roster and OPPOSITE remedies.
    // Every field is a boolean, a count or a timestamp.
