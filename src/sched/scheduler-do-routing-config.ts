// The CONFIG-GOVERNANCE half of the SchedulerDO's RPC dispatch, kept in its own module so no routing file
// exceeds the module-size guardrail. This sub-mixin owns the per-subsystem sub-dispatch methods for the
// demo-only markers/reset, the config-as-a-source version history, the break-glass/discovery and the
// archive-destination/licence/canary owner-action surfaces, and the dual-control pending-change + owner-action
// approval inbox. Each method is a switch over the SAME `${method} ${pathname}` key that returns the handler
// Response for a key it owns or null ("not my route"); route() (in scheduler-do-routing.ts) chains these among
// the other sub-dispatches. This sub-mixin's `this` is SchedulerDOSurface (like every sibling mixin), so each
// sub-dispatch calls the owning handler (this.setDestConfig / this.gatedOwnerAction / this.approveOwnerAction /
// ...) with the same dispatch and `this` binding. A leaf the assembly depends on, never the reverse (madge 0
// cycles).
...
export function RoutingConfigMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    async routeDemo(req: Request, url: URL): Promise<Response | null> {
      ...
      case "POST /demo/mark": {
        // The router persists the in-DO demo marker on a DEMO_MODE-gated path (it
      ...
      case "POST /demo/reset": {
        ...
        // FAIL-CLOSED defence in depth. Refuse unless THIS DO carries the demo marker
        ...
        // Re-persist the demo marker after the wipe (the list+delete above cleared it),
    ...
    // Config-as-a-source versioning plus the OPT-IN dual-control gate's own administer routes.
    // Returns the handler Response for a key this sub-dispatch owns, or null ("not my route") so route()
    // tries the next link. The arms are verbatim from the original single switch.
    async routeConfigVersion(req: Request, url: URL): Promise<Response | null> {
      ...
      case "POST /config/signin-context-policy": {
        // OWNER-OPT-IN unusual-location sign-in notify toggle (V6.3.5). Owner-only (the router gates
      ...
      case "GET /config/update-approval-policy":
        // The read the update-apply path itself makes (router-core.ts requireAllUpdateApproval). It returns
        // one boolean and no caller detail, exactly like GET /policy/break-glass-retired, which the router
        // also reads with no caller header on an internal decision path. The console reads the same flag
        // through GET /config/approval-policy, whose view carries it beside the other policy flags.
        return this.json({ requireAllUpdateApproval: await this.getRequireAllUpdateApproval() });
      case "POST /config/update-approval-policy": {
        // OWNER-OPT-IN "require a second owner for EVERY engine update" toggle. Owner-only (the
        // router gates keys.ceremony; the DO re-resolves owner in setRequireAllUpdateApproval). It applies
        // immediately in both directions and is audited as config-policy-change. See the setter for why
        // arming carries no two-owner floor and why the disarm is not asymmetric.
        const uaCaller = decodeCaller(req.headers.get(CALLER_HEADER));
        const uaBody = (await req.json()) as { requireAllUpdateApproval?: unknown };
        return this.json(await this.setRequireAllUpdateApproval(uaBody, uaCaller));
      }
      ...
      case "POST /config/egress-allowlist": {
        // The operator-configured outbound egress allowlist (ASVS V13.2.4). Owner-only, re-resolved inside
        // setEgressAllowlist exactly as the attended-cadence toggle is: it applies immediately (no
        // dual-control arm). This route also serves the notify-channel delivery path's own read of the
        // CURRENT list (the router's channel-delivery code calls GET /config/egress-allowlist internally,
        // with no caller header, the same trust boundary /notify/channel already relies on).
        const eaCaller = decodeCaller(req.headers.get(CALLER_HEADER));
        const eaBody = (await req.json()) as { egressAllowlist?: unknown };
        return this.json(await this.setEgressAllowlist(eaBody, eaCaller));
      }
      case "GET /config/egress-allowlist": {
        // Read-only. Reached both by the public admin router (gated on downpipe.read there) and, with no
        // caller header, by the router's own internal notify-delivery code before it delivers to a
        // customer-configured sink -- the same INTERNAL-route trust boundary /notify/channel and
        // /notify/resolve already rely on (only the Worker's own scheduler binding can reach the DO at all).
        return this.json({ egressAllowlist: await this.getEgressAllowlist() });
      }
    ...
    async routePolicy(req: Request, url: URL): Promise<Response | null> {
      ...
      // Lockout pre-flight: the DO-owned second-factor facts the require-access wizard OR's with CF Access
      ...
      // engine-account verification: the STATUS read is
    ...
    async routeDestConfig(req: Request, url: URL): Promise<Response | null> {
      ...
      // 0.1.5 UX background confirmation (design §3): the hourly canary clears confirmationPending on the
      ...
      // One-shot "new version available" claim: alert ONLY when the recommended version differs from the
      ...
      // The canary cron claims a one-shot CRITICAL "rollback needed" alert when a promoted-but-
    ...
    async routeConfigChanges(req: Request, url: URL): Promise<Response | null> {
      ...
      // The caller-independent queue aggregate for the support pack: counts + oldest age + kinds,
