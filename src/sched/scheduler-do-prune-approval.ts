...
// request -> approve (maker != checker) -> reserve (atomic, ASVS 2.1.6) -> consume/release -- over
...
export function PruneApprovalMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    async readPruneApprovalRecord(planHash: string): Promise<PruneApproval | null> {
      ...
      // A stored record whose expiresAt does not parse must not silently read as
    ...
    // reservePruneApproval is the ATOMIC gate-to-write reservation (ASVS 2.1.6), mirroring
