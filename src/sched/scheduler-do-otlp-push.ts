// OTLP/HTTP metrics push destination: the outbound egress config + bounded
...
// OUTSIDE the DO (cron/otlp-push-pass.ts -- the DO never makes a network call to a
...
export function OtlpPushMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    buildOtlpPushRecord(
      ...
      // Defence in depth for the plaintext no-wrap-key floor: a wrapped value was screened by the
    ...
    async setOtlpPushDestination(
      ...
      // THE KEEP-SECRET PATH IS A SILENT NARROWING. An absent-or-unusable authHeaderValue on a set that
    ...
    async recordOtlpPushOutcome(req: {
      ...
      const entry: OtlpPushDeliveryAttempt = {
        ...
        // The SIZE of the truncation, clamped to a non-negative int. truncated:true alone could not tell
        ...
        // The join key, SHAPE-GATED at the redaction chokepoint (bare 12-hex or nothing), so a drifted
    ...
    // backup-health metric set (the metric VALUES come from the same DO state
    ...
    // SRE's own alerting rule, the idiom time() - last_success > threshold).
