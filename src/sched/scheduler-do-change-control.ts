// The OPT-IN dual-control change-control gate for CONFIG mutations: the single apply chokepoint plus the
// propose/dry-run/approve/reject path and the pending-change inbox. ChangeControlMixin layers over a base
// whose `this` is SchedulerDOSurface, so it reaches every validated DO mutation method through
// this.applyConfigMutation's dispatch (this.addDownpipe, this.setRole, ...) and the destination/policy/audit
// helpers (this.loadDestinations, this.roleForCaller, this.getRequireConfigApproval, this.appendAudit) with
// the same dispatch and `this` binding those methods use everywhere else. This module is a leaf dependency:
// other modules depend on it, never the reverse.

import { TooSoonRefusal } from "../admin/approvals.ts";
...
import { pickDownpipeConfig } from "./config-pick.ts";
...
import { AuthError, type MutationCaller, type SchedulerDOCtor } from "./scheduler-do-base.ts";
  ...
  return class extends Base {
    ...
    async setRequireConfigApproval(
      ...
      await this.appendAudit({
        ...
        outcome: "success",
        // The target carries only the policy NAME and the DIRECTION (a closed enum and a boolean), never an
        // operator-supplied value, so the audit entry stays redaction-safe while still letting an incident
        // timeline distinguish a dual-control disarm from an unrelated policy toggle such as the change-number
        // flag.
    ...
    //
    // THE PER-METHOD CAPABILITY RE-CHECK IS NOT UNIVERSAL. Fifteen kinds re-check the caller's live authority
    // inside the method they dispatch to (requireCapabilityResolved on the people/access-policy writes;
    // requireNotifyConfig, requirePostureRiskAccept and requireCapability on the notify, posture and expiry
    // writes). THREE DO NOT. downpipe-upsert reaches addDownpipe, which asks only for the NARROWER
    // scheduledtest.config and only on a cadence change; downpipe-delete reaches removeDownpipe and
    // cf-config-mode-set reaches setCfConfigMode, and BOTH ARE DISPATCHED BELOW WITH NO CALLER ARGUMENT AT ALL.
    // On the gate-off and dry-run paths the router's own gate() is what stands in front of those three. On the
    // APPROVE path nothing does, so without a further check a proposer whose authority had lapsed could still
    // have their queued downpipe create, downpipe delete or capture-mode change applied on somebody else's
    // approval. That is closed in approveChange by an explicit maker floor over CHANGE_WRITE_CAPABILITY,
    // applied to every kind rather than to a declared list of three, so a kind added here cannot be added into
    // the gap.
    async applyConfigMutation(
      ...
      switch (kind) {
        ...
        case "downpipe-upsert": {
          ...
          // After the precondition key is stripped, every remaining key must be a documented field of the
          // downpipe (at every nesting level), or the save is refused naming the key. Single, bulk and the
          // approved-change replay all reach this arm, so the allowlist cannot be bypassed by route.
          return this.addDownpipe(pickDownpipeConfig(config), caller, { stated, guardResurrect: true });
        }
    ...
    async dryRunConfigMutation(
      ...
      // then leave the un-captured tail un-restored. The paged scan is complete regardless of tenant scale.
      ...
      // added keys past the page on a large keyspace).
      //
      // If the rollback ITSELF faults (a storage error part-way through the restore), the dry run has left
      // REAL, unattributed writes behind: a proposal that was never approved has half-applied itself, and
      // the only statement of that was the exception the operator's browser rendered. Record the class
      ...
      // The rollback is complete, so the mutation's own rejection can now propagate to the caller.
    ...
    //    the route's response and behaviour are byte-identical to an ungated direct call (the caller
    //    auto-snapshots after). No pending record is created.
    ...
    > {
      ...
      // MutationCaller carrying it. For a CUSTOM-ROLE proposer this is what lets the by-role re-checks
      ...
      if (!gateOn) {
        // Gate OFF: inline apply, byte-identical to an ungated direct call. The route auto-snapshots after.
      ...
      // The proposer's STABLE subject AT PROPOSE TIME, bound into the hash alongside their email + groups: the
      // apply re-resolves the proposer's authority by this immutable subject against the LIVE role table, so a
      // stored-record tamper of the actor identity (swapping in a more privileged proposer) is caught at
      // approve by the same recompute that catches a params tamper. An attributable proposer always carries a
      // subject (a subjectless caller is the bare-token break-glass, refused above), so this is non-null on
      // any queued record.
      const proposedBySubject = caller ? caller.subject : null;
      // The proposer's propose-time groups are bound into the hash alongside their email + subject, so a
      ...
      const record: PendingConfigChange = {
        ...
        // email never inherits their authority. Bound into contentHash above, so a stored-record tamper of it
        // is caught at approve by the recompute.
        proposedBySubject,
        ...
        // cannot gain groups. The same value is bound into contentHash above, so a tamper of the stored groups
        // is caught at approve by the recompute (the apply re-resolves authority from this field, so it is
        // decision-relevant and must be integrity-bound).
        proposedByGroups,
        // The edge address THIS propose request arrived from. The approve-time replay runs as the proposer
    ...
    async approveChange(
      ...
      // The checker's STABLE subject is the primary maker != checker axis; the email is the
      // belt-and-suspenders floor. normaliseSubject mirrors how roleForCaller keys authority.
      const checkerSubject = caller ? this.normaliseSubject(caller.subject) : null;
      const now = Date.now();
      const verdict = canApproveChange(record, checker, checkerSubject, approverCaps.has(CHANGE_WRITE_CAPABILITY[record.kind]), now);
      if (!verdict.ok) {
        // The reason code matters, not just the refusal: "the console demands an approval I already gave" is
        // a terminal-state, "cannot approve your own change" is a self-approval, and a superseded record is a
        // base-moved change. Each is recorded under its own reason code so the three stay distinguishable.
        await recordGovernanceRefusal(this.state.storage, "change-approve", verdict.reasonCode);
        // A too-soon refusal answers 429 with Retry-After, not the generic 400.
        if (verdict.reasonCode === "too-soon") throw new TooSoonRefusal(verdict.reason, verdict.retryAfterSeconds ?? 1);
      ...
      // apply re-resolves authority from), or the base were tampered. The proposedBySubject + proposedByGroups
      ...
      if (recomputed !== record.contentHash) {
        // A stored-record TAMPER is the most security-significant thing this machine can detect, and without
        // this check it would be seen only by the approver's browser. It is counted as its own closed class,
        // so it can never be confused with an ordinary guard refusal.
      ...
      // Every FORESEEABLE refusal inside blockConcurrencyWhile returns a {ok:false,error} sentinel rather than
      // throwing, exactly as approveOwnerAction does. A throw escaping the guard RESETS the Durable Object
      // (wiping in-memory scheduler state) and surfaces as a 500,
      ...
      const outcome = await this.blockConcurrencyWhile(async (): Promise<{ ok: true; record: PendingConfigChange } | { ok: false; error: Error }> => {
        ...
        const innerVerdict = canApproveChange(fresh, checker, checkerSubject, approverCaps.has(CHANGE_WRITE_CAPABILITY[fresh.kind]), Date.now());
        if (!innerVerdict.ok) {
          ...
          // TooSoonRefusal survives the sentinel (it IS an Error) and is re-thrown outside the guard, where it
          // maps to 429 exactly as the outer check's throw does.
          return { ok: false, error: innerVerdict.reasonCode === "too-soon" ? new TooSoonRefusal(innerVerdict.reason, innerVerdict.retryAfterSeconds ?? 1) : new Error(innerVerdict.reason) };
        }
        ...
        //   - a recycled email cannot inherit the proposer's authority (the lookup keys on the immutable
        //     subject, not the mutable email);
        ...
        //     ceiling; AND the proposer's live-resolved CAPABILITY SET is forwarded so a CUSTOM-ROLE proposer
        //     who legitimately holds notify.config / expiry.config has their queued change APPLY (the by-role
        //     re-checks consult the set via callerHolds) instead of being refused on their viewer floor. The
        //     set is the proposer's OWN live authority (re-resolved here, bounded by requireGrantWithinAuthority
        ...
        // sourceIp is the PROPOSE-TIME address, carried on the record. The replayed mutation audits itself as
        // the proposer -- an attributed HUMAN actor -- so a null IP here would read as a missing capture rather
        // than a deliberate drop, and would trip the audit-human-event-missing-source-ip check on every
        // approved change under dual control. Carrying the propose-time address here means the propose row and
        // the replay row carry the same address, which is the truth: it is the proposer's request, running late.
        // replay: true marks this as one of three replay sites that construct a caller object standing in for
        // the proposer (the others are resolveStoredIdentityAuthority and proposerReplayCaller). The flag
        // matters because roleForCaller's bind-on-first-auth would otherwise match a pending grant BY EMAIL and
        // bind it onto whatever subject is presented: without it, an ordinary invite issued at a departed
        // proposer's re-used address could bind to the new subject at approve time, delete the pending row, and
        // apply the departed proposer's queued change with the new person's authority while the new person
        // resolves to viewer with nothing to explain why. The flag also gates the connection-liveness axis, so
        // a proposer whose IdP connection was deleted no longer has their queued change applied. It rides on
        // the object, so it reaches the downstream requireCapabilityResolved re-checks inside the replayed
        // mutation too, not just the call below.
        const replayCaller = {
        ...
        // THE APPLY-TIME MAKER FLOOR sits here rather than in the dispatched methods because for three of the
        // eighteen kinds there is no method to put it in. Fifteen kinds re-check the caller's live authority
        // inside the method they dispatch to: setRole and its people/access-policy siblings call
        // requireCapabilityResolved, and the notify/posture/expiry writes call requireNotifyConfig /
        // requirePostureRiskAccept / requireCapability, all reading the LIVE-resolved authority this line just
        // computed. The other three ask nothing at all. addDownpipe re-checks only the NARROWER
        ...
        // caller either: its capability is enforced by the router gate on POST and re-checked at approve via
        // CHANGE_WRITE_CAPABILITY, which covers the CHECKER (canApproveChange takes approverCaps) but says
        // nothing about the MAKER. This explicit floor is what covers the maker side for those three kinds,
        // since the identity binding, the bind-on-first-auth suppression and the connection-liveness axis are
        // each resolved above but have no per-method check to read them for these three.
        //
        // This floor is reachable by exactly two routes. The obvious attack -- offboard or demote the
        // proposer -- is closed already: that is itself a config mutation, it auto-snapshots, and the
        // baseMoved check above fires BEFORE this re-resolution, so the change supersedes. What moves no
        // config head is a JIT EXPIRY (a clock event; nothing is written when a time-boxed grant lapses) and
        // an IdP connection being deleted or disabled (not part of the snapshot); those are the two routes
        ...
        // commit (snapshot/put/audit below) still propagates uncaught.
        try {
      ...
      // This is the ONE place every failed apply attempt passes through, so it is the one place to record it.
      // classifyGovernanceOutcome reads the refusal ONLY to select a closed class and returns it: the guard's
      // operator-facing prose (which interpolates emails, ids and params) is never stored. The count makes the
      // RATE visible; the record annotation makes the pending change explain ITSELF in the inbox.
      if (!outcome.ok) {
      ...
      // The approve RESPONSE is a caller-facing echo like the listing, so it goes through the same projection:
      // no caller-facing echo carries a credential, whether the caller is listing the queue or arming one
      // entry of it. By this point the stored record has already been scrubbed anyway, so on the
      // notify-channel-set path the two agree; the projection is here so they still agree if the scrub is ever
      // narrowed.
    ...
    // annotateChangeAttempt stamps the pending record with its LAST FAILED APPLY ATTEMPT: a closed outcome
    // class + the time. It is BEST-EFFORT and never throws: an annotation must never turn a clean refusal (a
    // 400/403 the caller understands) into a 500. It re-reads the record so it cannot resurrect a stale copy,
    // and it writes ONLY the annotation field, so it can never disturb the record's status, integrity hash or
    // lifecycle.
    ...
    async rejectChange(
      ...
      // url withdraws the change; without this scrub the withdrawal would leave the url in storage
      // indefinitely.
    ...
    //
    // EVERY RECORD GOES THROUGH viewConfigChange. The pending params are not uniformly safe to expose: that
    // holds for seventeen kinds but not for notify-channel-set, whose params are the submitted channel and
    // therefore hold the delivery url, the PagerDuty routing key or the JSM/ServiceNow api key. This route is
    // gated on downpipe.read, while the channels route the same value is served from is gated on
    // notify.config, so serving raw params here would let a viewer, a restore-operator or an access-admin read
    // a credential they are refused on the more specific route. The projection is a pure copy and the stored
    // record is untouched, so the approve-time replay and its hash recompute are unaffected.
    ...
    async getOrgPolicyView(): Promise<{ requireRestoreApproval: boolean; requireConfigApproval: boolean; requireChangeNumber: boolean; notifyNewSignInContext: boolean; requireAllUpdateApproval: boolean }> {
      return { requireRestoreApproval: await this.getRequireRestoreApproval(), requireConfigApproval: await this.getRequireConfigApproval(), requireChangeNumber: await this.getRequireChangeNumber(), notifyNewSignInContext: await this.getNotifyNewSignInContext(), requireAllUpdateApproval: await this.getRequireAllUpdateApproval() };
    }
  };
}
