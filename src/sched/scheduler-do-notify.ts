// The Notifications subsystem (contract section 2: channels, rules, history, routing/digest) is a mixin
// factory. NotifyMixin layers these methods over a base whose `this` is SchedulerDOSurface, so the methods
// call `this.callerHolds` / `this.state` like any instance method. The requireNotifyConfig re-check runs on
// every write in the same order. The channels/rules/history live in THIS DO under the notify-*: prefixes;
// the DO does NO network I/O (the Worker delivers).
...
export function NotifyMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    // reader mistakes it for a routed engine event. Without this, a green test leaves NO durable trace
    // that the channel was ever verified. notify.config re-checked (the router gates too; defence in
    // depth). An unknown channel degrades to { ok:false } rather than a throw: the test itself has
    // already reported delivery, and history is best-effort for a manual test.
    async recordTestSend(
      ...
      // The WHY, on the one durable artefact a Test press leaves. A bare delivered:false makes
      // "my Slack test keeps failing" a failure with no cause in the pack, while the
      // very same fields (deliveryCode, platformCode) ride every REAL failed delivery on this same ring.
      // Re-gated here against the closed sets exactly as recordNotifyDelivery does (the DO is
    ...
    async addNotifyChannel(
      ...
      this.requireNotifyConfig(caller);
      const v = validateChannel(raw, validateRecipients, await this.getEgressAllowlist());
      if (!v.ok) {
        // Record the CLOSED reject code before re-throwing. The operator's 400 and its sentence are
        // byte-unchanged (nothing is oracled back to the caller); the engine keeps a durable count, so
        // a customer stuck in a save-validation loop -- pasting an http:// URL, or a private sink with no
        // internal-sink opt-in -- is diagnosable from the pack rather than invisible.
      ...
      // ENABLED is submitted by the caller and honoured here. validateChannel deliberately
      ...
      // and defaults to on for a create (a new channel starts enabled). Writing prior.enabled unconditionally on
      // an edit would let a submitted disable return 200 while silently discarding it and the channel keeps
      // delivering; the rule path already honours its submitted enabled (addNotifyRule spreads v.rule), and
      // the channel path matches it.
      ...
      const channel: NotifyChannel = {
        ...
        // allowInternalSink is carried here: validateChannel validates and returns it, but omitting it here
        // would leave a channel configured with the SSRF opt-in validating fine at config time and then being
        // silently RE-BLOCKED at send time (deliverPayload reads channel.allowInternalSink, which would
        // otherwise be undefined post-store). Carries across every
    ...
    //
    // THE SAME POLICY GOVERNS A DOWNPIPE-SCOPED RULE, stated here because a rule whose scope names a
    ...
    // dropContextOf salvages the NAMEABLE fields off an emission the parser REJECTED. The whole point of
    ...
    //    event is "batched, not sent per occurrence" rather than delivered immediately; the flush is a
    //    separate step), and
    ...
    //
    // egressAllowlist (ASVS V13.2.4) rides on EVERY answer (even the empty/rejected ones) so a caller can
    // thread it into deliverEmission with one round trip rather than a second DO fetch per notification.
    async resolveNotify(req: { emission?: unknown }): Promise<{ now: NotifyChannel[]; digestedCount: number; emission: NotifyEmission | null; egressAllowlist: string[] }> {
      const emission = this.parseEmission(req.emission);
      const egressAllowlist = await this.getEgressAllowlist();
      // A rejected emission (bad enum / overlong free-text detail) is counted so the pack sees an alert was
      // dropped at the input boundary, not silently.
      if (emission === null) {
        await this.bumpNotifyHealth("parseRejects");
        // parseRejects=3 does not answer the customer's question, which is always "MY backup-failure
        ...
        return { now: [], digestedCount: 0, emission: null, egressAllowlist };
      }
      ...
      // A rule that SELECTED this emission and routes to a channel id that no longer exists is skipped by
      ...
      return { now: resolved.now, digestedCount: resolved.digested.length, emission, egressAllowlist };
    }
    ...
    //
    // egressAllowlist (ASVS V13.2.4) rides beside batches so the flush can screen each digest delivery
    // against the account's CURRENT allowlist without a second DO round trip per batch.
    async digestDue(req: { nowMs?: unknown }): Promise<{ batches: DigestBatch[]; egressAllowlist: string[] }> {
      const egressAllowlist = await this.getEgressAllowlist();
      const map = await this.state.storage.list<PendingDigestEntry>({ prefix: NOTIFY_DIGEST_PREFIX });
      if (map.size === 0) return { batches: [], egressAllowlist };
      ...
      // groupDigestDue's guards suppress CONSERVATIVELY on an unparseable timestamp -- a digest that does
      // not flush, or an entry silently dropped from the window -- and an unobserved suppression leaves no trace,
      // so "our digest just stopped arriving" would be evidence-free. The pure module tallies isolate-locally; drain
      ...
      return { batches, egressAllowlist };
    }
    ...
    async digestSent(req: { ids?: unknown }): Promise<{ cleared: number }> {
      ...
      // A malformed clear-id is skipped, so the entry the cron JUST DELIVERED is NOT cleared and will be
      // delivered AGAIN next window, and the one after that: "we get the same digest email every hour" is a
      // permanent duplicate loop whose only trace is a silent `continue`. Distinct from "not yet due".
      if (unclearable > 0) await recordNotifyDrop(this.state.storage, "digest-clear-failed");
      return { cleared };
    }

    // digestPending reports the PENDING-DIGEST queue depth + the oldest deferred entry's timestamp, plus the
    // per-cadence lifecycle state: for each period the QUEUE DEPTH, the OLDEST deferred
    ...
    async recordNotify(req: { emission?: unknown; records?: unknown }): Promise<{ recorded: number; skipped: number }> {
      ...
      // never went out, instead of a silent early return.
      if (emission === null) {
        await this.bumpNotifyHealth("parseRejects");
        // The cruel one: a reject HERE discards an ENTIRE emission's per-channel delivery outcomes -- the
      ...
      for (const rec of records) {
        ...
        // can say WHY a delivery failed. Only carried on a FAILURE / a resolve.
        const code = !delivered && typeof r.code === "string" && DELIVERY_FAIL_CODES.has(r.code) ? (r.code as DeliveryFailCode) : undefined;
        // A version-skew failure that CARRIED a code we do not hold in the closed set lands with NO
        ...
        // The send-time sink-screen verdict is validated against the CLOSED
        ...
        // The same skew gate: a verdict this build does not hold silently erases the rebind-exposure
        ...
        // The RESOLVE verdict rides the same way, so a resolver outage (resolve-unavailable) is distinguishable in
        // the history from a clean resolve rather than the abstention being dropped one frame up.
        const resolvedScreen = typeof r.resolvedScreen === "string" && RESOLVED_SINK_VERDICTS.has(r.resolvedScreen) ? (r.resolvedScreen as ResolvedSinkVerdict) : undefined;
        if (typeof r.resolvedScreen === "string" && resolvedScreen === undefined) await recordVocabDrop(this.state.storage, "sink-screen");
        // unconfirmed: JSM/Opsgenie's async-accepted (202) outcome. Only meaningful on a
        ...
        // ackOutcome: the CLOSED verdict of the async-create status poll a JSM/ServiceNow deliver runs
        ...
        await this.appendNotifyHistory(emission, channelId, channelKind, delivered, { ...(code !== undefined ? { code } : {}), ...(platformCode !== undefined ? { platformCode } : {}), ...(recovered ? { recovered: true } : {}), ...(sinkScreen !== undefined ? { sinkScreen } : {}), ...(resolvedScreen !== undefined ? { resolvedScreen } : {}), ...(unconfirmed ? { unconfirmed: true } : {}), ...(ackOutcome !== undefined ? { ackOutcome } : {}) });
        recorded++;
      }
      ...
      if (skipped > 0) {
        ...
        // A skipped outcome row means a channel's delivery result is MISSING from the history the
    ...
    // bumpNotifyHealth increments one notify-pipeline drop counter. It is a small
    ...
    async appendNotifyHistory(emission: NotifyEmission, channelId: string, channelKind: ChannelKind, delivered: boolean, extra?: { code?: DeliveryFailCode; platformCode?: string; recovered?: boolean; sinkScreen?: SinkScreenVerdict; resolvedScreen?: ResolvedSinkVerdict; test?: boolean; unconfirmed?: boolean; ackOutcome?: AckOutcome }): Promise<void> {
      ...
      const entry: NotifyHistoryEntry = {
        ...
        // The send-time sink-screen verdict, on url channels only (success + failure).
        ...(extra?.sinkScreen !== undefined ? { sinkScreen: extra.sinkScreen } : {}),
        ...(extra?.resolvedScreen !== undefined ? { resolvedScreen: extra.resolvedScreen } : {}),
        ...
        // unconfirmed: JSM/Opsgenie's async-accepted (202) outcome, so "delivered" here is never
        // silently conflated with a positively-confirmed success.
        ...(extra?.unconfirmed === true ? { unconfirmed: true } : {}),
        // ackOutcome: the closed verdict of the follow-up create-status poll, so the pack can separate a
      ...
      // trigger rollover and history would grow unbounded). Deleting a key below
