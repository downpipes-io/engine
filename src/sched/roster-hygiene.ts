...
//
// THE PROBLEM this closes (the "permanent grey line on the map" support case). Every consumer of
...
// storage shells. House rules: Australian English, no em dashes, precise claims.
...
export interface RosterHygieneReport {
  ...
  // The two lists are CAPPED and the caps carry no marker, so a reader holding only the pack cannot
...
export function analyseRoster(
  ...
  // State the truncation rather than making the reader derive it from count-vs-length.
  return {
