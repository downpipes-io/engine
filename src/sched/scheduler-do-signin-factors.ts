...
// module owns together (`passkeyCred:`, `recovery:` and `passkeyInvite:`), and because the credential route
// it supersedes was landed hours earlier: reshaping that route to grow a recovery arm would have been a
// second design against one store at a time, which is the mistake this exists to stop repeating.
...
export function SignInFactorsMixin<TBase extends SchedulerDOCtor>(Base: TBase) {
  return class extends Base {
    ...
    // the credential route (526878cf): a bearer token carries no email, so every ADMIN_TOKEN sweep silently
    ...
    // narrowing is the original defect. The 400 follows the same reasoning as 526878cf: a new field on a 200
    ...
    async revokeSignInFactors(
      ...
      // THE UNATTENDED-OWNER SKIP (SCIM-REVOKE plan, connector-misfire mitigation 1, amended in review).
      //
      ...
      // SELF-REVOCATION IS REFUSED, AND UNTIL NOW IT WAS NOT. The authority comment above says this route has
      // "NO self arm", which is true of the CAPABILITY (there is no self-service exemption from roles.write)
      // and was read, including by a brief written off it, as a refusal of a self-targeted revoke. There was
      // no such refusal. Driven live against a real Durable Object, an Owner with a second Owner present
      // revoked their own factors, got 200, and lost their credential and their recovery record together.
      // That is the comment-describes-a-guard-the-code-does-not-have shape, and the guard it describes is the
      // right one: revoking your own every factor is a self-lockout with no legitimate use, because rotating
      // your own codes is regenerate and dropping a lost key is the credential route.
      //
      ...
      // been offboarded serves nobody -- one estate is carrying 77 of them. So the sweep is scoped to the named
