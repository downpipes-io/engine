// Part 1 of the SchedulerDO's shared TYPE + CONSTANT vocabulary, split out of scheduler-do-base.ts so that
// foundation file holds only the irreducible DO surface (the constructor + the SchedulerDOSurface instance
// API every mixin extends). This leaf declares
// the governance + run + notify/expiry + discovery/licence/update at-rest record shapes (OrgPolicy,
// MutationCaller, DestReplState, ReplRecordReq, CompleteRunReq, GroupRoleEntry, the alert/expiry cooldown +
// emission shapes, RateWindow, DiscoveryConfig, LicenceTokenRecord, the Update* records, ...) and the
// storage-key prefixes, bounds and caps those domains key on (the drill-evidence / group-role / custom-role /
// alert / repl-alert / success-notify / notify-digest / org-policy / discovery / dest / canary / licence /
// update / demo prefixes + keys, the group/reason/note bounds, and the lease/cooldown/window constants). It
// is a pure leaf (no `this`, no DO state) that base.ts re-exports verbatim, so every module importing these by
// name from scheduler-do-base.ts is unchanged. The sibling scheduler-do-limits.ts holds the destination
// config types and the rate-limit/recovery/list/due/audit limits. Move-only: every value, key, cap, bound and
// type is byte-identical. A leaf the base depends on, never the reverse.
...
export interface OrgPolicy {
  ...
  //
  // IT DEFAULTS OFF BECAUSE AN UNCONDITIONAL GATE WOULD LOCK SOLO ESTATES OUT OF RESTORE ENTIRELY.
  // canApprove refuses a same-subject AND a same-email approval, so one human can never approve
  ...
  // break-glass-only estate the engine holds no key to reopen a run it sealed earlier, so attended
  // verification is what replaces unattended proof; without this field, the
  // only thing that knows when it is due is a preference in one browser.
  //
  ...
  // controlPlaneRecoveryRequired: set true when the control plane has been detected as EMPTY
  ...
  // requireAllUpdateApproval: the OWNER-OPT-IN "gate EVERY engine update" policy (default OFF).
  //
  // WHAT IT TAKES BACK. The update-apply dual control is consulted only when the VENDOR-SIGNED manifest
  // declares the release "migration" or "breaking" (updateNeedsDualControl, router-core.ts). The risk class
  // rides on the manifest, so the party who picks whether the customer's second-owner gate is consulted is
  // the vendor, not the customer. That is the sharpest edge of the published residual: a holder of the
  // release signer private could mark a consequential release "routine" and the gate would never be asked.
  // With this ON, every LIVE apply consults the gate whatever the manifest says.
  //
  // IT DOES NOT CREATE A GATE, it removes the vendor's say over an existing one. The gate still has to be
  // ON for the account (requireConfigApproval, or the high-blast auto-apply), and a DRY-RUN is still never
  // gated: this flag only decides whether the risk class may excuse an apply from being asked.
  //
  // DEFAULT OFF so an estate that never sets it behaves byte-identically, matching every other flag here.
  requireAllUpdateApproval?: boolean;
  // egressAllowlist (ASVS V13.2.4): the account-wide list of external hosts the operator has permitted the
  // engine to reach for a CUSTOMER-CONFIGURED sink (a notify webhook/Slack/Teams/JSM/ServiceNow channel
  // today; see screenEgressAllowlist for the exact match rule). Each entry is a canonical lowercase
  // hostname or a single-label "*.suffix" wildcard (validateEgressAllowlist enforces the shape, at most
  // EGRESS_ALLOWLIST_MAX_ENTRIES entries). OPTIONAL, and ABSENT OR EMPTY means UNRESTRICTED: an account that
  // has never configured one reaches any public https host from its notify channels, matching the
  // OFF-by-default discipline every other opt-in policy in this record follows. A FIXED vendor host by
  // EXACT match (isFixedEgressHostExact) is always permitted and never needs an entry here: the operator's
  // list governs what THEY chose to reach, not what the engine reaches on its own initiative.
  // Toggled OWNER-ONLY (setEgressAllowlist), applied immediately (a positive allowlist is not the kind of
  // control a compromised owner benefits from disarming solo, unlike a dual-control gate), and recorded as
  // a config-policy-change audit event carrying only the host COUNT (never a hostname).
...
// of refusing them on their "viewer" built-in floor. For a built-in caller it is absent and the
...
// their only users; none was referenced by the DO class body.
...
export interface DestReplState {
  ...
  // over-claim: keepRuns can exceed the 50-run history ring,
...
export interface CompleteRunReq {
  ...
  // KV key / R2 object deleted between the list page and the GET -- a mid-crawl live-source race).
  ...
  // How many records this run sealed as INCOMPLETENESS SENTINELS: a marker landed in the
  ...
  // incompleteIds is the PER-KIND ATTRIBUTION of the markers: which surface/object was short, as a
  ...
  destinationId?: string;
  // The destination id(s) PROVEN down for this run (the failover down set, or, for a single-
  ...
  // The verify-at-seal verdict the seal driver computed by reading the just-written
...
// DRILL_EVIDENCE_CAP bounds how many drill-evidence rows the DO retains, mirroring
// AUDIT_CAP for the audit chain and RING_CAP for the run-history ring: every OTHER record family in
// this DO has a bound, and without one here a long-lived account running scheduled
...
// validateFreeText moved to the leaf sibling scheduler-helpers.ts;
...
export interface GroupRoleEntry {
  ...
  customRole?: string;
  // connId, when present, SCOPES this mapping to logins through that ONE IdP connection: the group name
...
// NOTIFY_HEALTH_KEY holds a small
...
// JITTER_FRACTION moved to the leaf sibling schedule-window.ts alongside jitteredCadence, its only user.

// ALARM_MIN_DELAY_MS is the floor rearmAlarm applies so the next alarm is never scheduled in the
// past. nextRunAt advances only on completion, so a wedged enabled downpipe (a run whose
...
// via the in-flight LEASE on the next cron-driven trigger, and precise alarm-driven
// dispatch is a separate, still-deferred refinement (it needs the engine wired as a service
...
// TICK_LEASE_MS bounds the cron driver's SINGLE-FLIGHT tick lease: drive() takes it at the top of a
...
// Per-session records (ASVS V7.5.2). sessionRef:<email>:<sid> is written at every mint and refreshed on a slide,
// so an operator can LIST their live sessions; it carries coarse metadata only (method, connection, instants,
// a coarse client family and network prefix), never the token. sessionRevoked:<sid> = exp is written when ONE
// session is ended singly, and consulted at every verify; both families are swept once past their exp.
export const SESSION_REF_PREFIX = "sessionRef:";
export const SESSION_REVOKED_PREFIX = "sessionRevoked:";
export interface SessionRef {
  ...
  exp: number;
  lastSeen: number;
  clientFamily: string | null; // a coarse user-agent family (browser or platform name), never the raw string
  networkPrefix: string | null; // the sign-in network as an IPv4 /24 or IPv6 /48 prefix, never the address
}

// CONTROL_PLANE_EXPORT_STATE_KEY records the LAST signed control-plane export the cron pass
...
// CONTROL_PLANE_STAGED_KEY holds the ControlPlaneRecoveryRecord: the verified signed
...
// CONTROL_PLANE_AMNESIA_PROBE_KEY records the OUTCOME CLASS of the cron health pass's last
...
// no signal without it. This holds a closed probe-class enum + a clamped timestamp only (no secret, no config
...
// CONTROL_PLANE_DEPLOY_OBS_KEY records a DETERMINISTIC per-tick deploy-identity observation the
...
// CONTROL_PLANE_EXPORT_HEALTH_KEY records the OUTCOME of the cron control-plane export pass: whether it was
// skipped (no signer / budget-yielded) and the PER-DESTINATION write outcome. Without it a budget-yield or a
...
// CONTROL_PLANE_LAST_IMPORT_KEY records the OUTCOME of the last estate import: whether it landed
...
//
// Same DO state, same audit trail (one reconcile bridge), and without this record the same support pack: a
// pre-account-discovery export silently imports everything disabled, and silently is exactly right, because
// nothing anywhere says which of the two happened. Booleans, counts and one clamped timestamp; no account id,
// no export content, no downpipe id.
...
// ENGINE_ACCOUNT_VERIFIED_KEY holds the engine's own
...
// DEMO_MODE_MARKER_KEY is the in-DO, fail-closed defence-in-depth marker that proves
...
export interface UpdatePending {
  ...
  // riskClass is the normalised risk of the release being settled (routine|migration|breaking), persisted
  ...
  // channelSeq/channelIssuedAt are the freshness claims from the SIGNED channel descriptor this promote
  ...
  // percentage is set ONLY by the opt-in gradual ramp (never by the atomic apply): its presence
  ...
  // artefactSha384 (ADDITIVE) is the SIGNED CHANNEL's digest for the artefact this
  ...
  // readback (ADDITIVE) is the post-upload read-back verdict for this promote (verified | mismatch
...
export interface UpdateLast {
  ...
  component?: string;
  // channelSeq/channelIssuedAt (ADDITIVE): a settle that is itself the accepting act for a channel
  ...
  channelSeq?: number;
  channelIssuedAt?: string;
  // confirmationPending (0.1.5 UX design §3, ADDITIVE): true only on an ENGINE "applied" outcome that was
  ...
  // minEngineVersion (0.1.5 UX design §6, ADDITIVE, CONSOLE-APPLY ONLY): the applied console component's
  ...
  // ---- Settle DECISION TRACE (ADDITIVE). ----
  // The one question a rolled-back / kept-pending settle must always answer -- WHY -- lives ONLY in the
  // settle HTTP response (probe.attempts) unless it is persisted here: that response races the isolate
  // swap and is lost, so without these fields neither a re-read, the audit log, nor a SUPPORT PACK can
  // reconstruct it.
  // selfCheckOk: did the new build boot + answer + self-identify + pass its critical preflight? Recorded
  // even though it does not GATE the keep (decideKeep is optimistic): a rollback with selfCheckOk=true
  ...
  // artefactSha384 (ADDITIVE) is the signed channel digest of the artefact this
  ...
  artefactSha384?: string;
  // readback (ADDITIVE): the promote's read-back verdict, copied forward from the pending record at
...
export interface UpdateRecord {
  ...
  // fired for. It dedupes the one-shot alert: the cron claims a version via claimUpdateAlert, which
  ...
  // settledHighWaterMark is the MONOTONIC highest engine version ever successfully SETTLED ("applied",
  ...
  // floors (multi-component updates, ADDITIVE + MIGRATING) is the per-component anti-rollback map:
  ...
  history?: UpdateLast[];
  // lastChannelSeq/lastChannelIssuedAt are the freshness watermark: the highest channel sequence and the
