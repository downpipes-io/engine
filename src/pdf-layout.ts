...
// Table.colWidths are ABSOLUTE POINTS chosen by each caller, and its own doc comment only says the sum
// "should be" <= USABLE_WIDTH. Nothing enforced that, so every table was silently sized against whatever
// USABLE_WIDTH happened to be when it was written. Narrowing the body column to fix the reading measure
// made tables sized for the old 504pt column overrun the new one by ~36pt, pushing ink past the right
// margin while the LEFT margin measured correctly - a page that looks lopsided rather than obviously
// broken, which is why it needs to be structural and not a caller-by-caller correction. Scaling here
// means a table can never outrun its column no matter what a caller passes, including future callers.
...
export function paginateContent(tagged: TaggedElement[]): string[] {
  ...
  for (let ti = 0; ti < tagged.length; ti++) {
    ...
    const h = t.el.height;
    // KEEP A TABLE HEADER WITH ITS FIRST ROW. A header row is an element like any other, so it was placed
    // whenever it fitted, and the first body row then started a new page: the reader met the header alone
    // above dead space at the foot of one page and again at the top of the next. Found by rasterising the
    // evidence pack and looking at it; the appearance gate cannot see it, because its stranded-heading
    // check reads text headings and this is a table header BAND, and every margin and size it measures is
    // correct on that page. Breaking BEFORE the header costs the same whitespace but spends it where it
    // reads as a page break rather than as a mistake.
