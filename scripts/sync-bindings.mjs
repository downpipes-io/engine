...
// --config <path> (default wrangler.toml) selects which committed toml to reconcile. This is what lets an ENV
// deploy (wrangler.dev.toml / .uat.toml / .internal.toml -- each pins its OWN worker name/WORKER_NAME) preserve
// its OWN worker's live sources instead of the default worker's. The generated superset is <config>.deploy.toml
// (wrangler.dev.toml -> wrangler.dev.deploy.toml); the prove-or-refuse guard is identical for every config.
