// Build-time artefact-hash stamp, giving the engine build provenance. It produces the engine's self-reported SHA-384 so
...
// commit, same config, nothing else touched:
//
//   run 1 (build-stamp.ts held the committed placeholder)   digest 8388068b623e...  4168290 bundle bytes
//   run 2 (build-stamp.ts held run 1's digest, not reset)   digest fbf7552caeb5...  4168371 bundle bytes
//
// Unrelated values, from identical source. harness/scripts/deploy.mjs runs this script once per estate
// against the ONE shared ../engine checkout during a fleet sweep, so every estate after the first inherited
// the previous estate's leftover stamp as ITS input, and twelve estates in one sweep produced twelve
// distinct, meaningless digests. `manifest/engine-artefact.json`'s expectation (built fresh each time from
// a clean checkout, so it never carried this defect) could then never match any of them.
//
...
// times this script has already run against the checkout or in what order. A config difference between
// estates is ruled OUT as a cause (this script never receives a per-estate config; `wrangler` always resolves
// the one committed engine/wrangler.toml here), and the reference build stays decoupled from the functional
// per-estate deploy that follows it.
//
